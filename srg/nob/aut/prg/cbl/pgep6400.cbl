      ...
      *                       Ultima revisione:    NdK del 25/10/16    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pgep6401:        *
      *                                                                *
      *                    Stampa analisi di portafoglio clienti per   *
      *                    zona                                        *
      *                                                                *
      *                    SU MISURA PER NOBILI - ESPORTAZIONE FILE    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " ANALISI DI PORTAFOGLIO CLIENTI PER ZONA"       .
      ...
      *        *-------------------------------------------------------*
      *        * Tipo di ordinamento zone                              *
      *        * - 01 : Per descrizione                                *
      *        * - 02 : Per codice                                     *
      *        * - 03 : Per mnemonico                                  *
      *        *                                                       *
      *        * Non significativo se selezionato una sola zona        *
      *        *-------------------------------------------------------*
           05  rr-ord-zon                 pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Tipo di scadenze da includere                         *
      *        * - 01 : Tutte                                          *
      *        * - 02 : Rim.Dir.,Bonif.,Dilazioni concordate,insoluti  *
      ...
      *        *-------------------------------------------------------*
           05  w-exp-ord-zon.
               10  w-exp-ord-zon-num      pic  9(02)       value 03   .
               10  w-exp-ord-zon-lun      pic  9(02)       value 15   .
               10  w-exp-ord-zon-tbl.
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tip-scd.
               10  w-exp-tip-scd-num      pic  9(02)       value 17   .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-300.
      *                  *---------------------------------------------*
      *                  * Tipo ordinamento zone                       *
      *                  *---------------------------------------------*
           perform   acc-ord-zon-000      thru acc-ord-zon-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Tipo ordinamento zone                           *
      *              *-------------------------------------------------*
           perform   pmt-ord-zon-000      thru pmt-ord-zon-999        .
      *              *-------------------------------------------------*
      *              * Tipi scadenza da includere                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Tipo ordinamento zone         *
      *    *-----------------------------------------------------------*
       pmt-ord-zon-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo richieste : Codice zona                *
      *    *-----------------------------------------------------------*
       acc-cod-zon-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
      *                  * Memorizzazione dati zona                    *
      *                  *---------------------------------------------*
           if        rr-cod-zon           =    zero
                     move  "Tutti"        to   rr-cod-zon-des
           else      move  w-let-arc-zst-des
                                          to   rr-cod-zon-des         .
      *                  *---------------------------------------------*
      *                  * Visualizzazione dati zona                   *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Trattamento tipo ordinamento zone           *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      zero                 to   rr-ord-zon             .
           perform   vis-ord-zon-000      thru vis-ord-zon-999        .
       acc-cod-zon-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codice zona , descrizione               *
      *    *-----------------------------------------------------------*
       vis-cod-zon-des-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Tipo ordinamento per zone                  *
      *    *-----------------------------------------------------------*
       acc-ord-zon-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-ord-zon-lun    to   v-car                  .
           move      w-exp-ord-zon-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      09                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-ord-zon-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-ord-zon           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *
      *                  *---------------------------------------------*
           if        rr-ord-zon           not  = zero
                     go to acc-ord-zon-500.
           if        v-key                =    "UP  "
                     go to acc-ord-zon-600
           else      go to acc-ord-zon-100.
       acc-ord-zon-500.
      *                  *---------------------------------------------*
      *                  * Se valore non a zero                        *
      *                  *---------------------------------------------*
           if        rr-ord-zon           >    w-exp-ord-zon-num
                     go to acc-ord-zon-100.
       acc-ord-zon-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Tipo ordinamento per zone               *
      *    *-----------------------------------------------------------*
       vis-ord-zon-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
