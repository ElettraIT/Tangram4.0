      ...
      *                       Versione attuale:    NdK del 12/07/99    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su codici modulo             *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    INTERROGAZIONE SU CODICI MODULO    "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
      *        *-------------------------------------------------------*
      *        * Codice modulo min, da i.p.c.                          *
      *        *-------------------------------------------------------*
           05  rr-cod-min                 pic  x(08)                  .
      *        *-------------------------------------------------------*
      *        * Codice modulo max, da i.p.c.                          *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-ipc.
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'cod-mod' letta ad inizio pro-    *
      *        * gramma per determinare una eventuale limitazione ai   *
      *        * soli codici modulo con l'iniziale dichiarata          *
      *        *-------------------------------------------------------*
           05  w-ipc-cod-mod              pic  x(08)                  .
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'non-tgr' letta ad inizio pro-    *
      *        * gramma per determinare una eventuale esclusione del-  *
      *        * la visualizzazione dei moduli tipografici             *
      *        *                                                       *
      *        *  - Spaces : Non escludere i moduli tipografici        *
      *        *  - N      : Escludere i moduli tipografici            *
      *        *-------------------------------------------------------*
           05  w-ipc-non-tgr              pic  x(01)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-pre-exe-pgm      .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'cod-mod'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-cdm-000      thru ipc-dlp-cdm-999        .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'non-tgr'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-ntg-000      thru ipc-dlp-ntg-999        .
       pre-exe-pgm-999.
           exit.
      ...
      *              * Codice min                                      *
      *              *-------------------------------------------------*
           move      w-ipc-cod-mod        to   rr-cod-min             .
      *              *-------------------------------------------------*
      *              * Codice max                                      *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun codice modulo memorizzato !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
       ...
       qry-sel-rec-100.
      *              *-------------------------------------------------*
      *              * Eventuale esclusione dalla visualizzazione dei  *
      *              * moduli tipografici                              *
      *              *                                                 *
      *              * Questo test e' stato inibito                    *
      *              *-------------------------------------------------*
           go to     qry-sel-rec-200.
      *
           if        w-ipc-non-tgr        not  = "N"
                     go to qry-sel-rec-200.
           if        w-mod-tip-mod        not  = "L" and
                     w-mod-tip-mod        not  = "S"
                     move  "#"            to   w-cnt-qry-flg-sub
      ...
      *                  *---------------------------------------------*
           ...
           move      16                   to   v-pos                  .
           move      "+"                  to   v-edm                  .
           move      w-mod-cod-mod        to   w-mpn-cod-mod          .
           move      w-mpn                to   v-cnt                  .
           move      w-mod-cod-mod        to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Descrizione codice modulo                   *
      *                  *                                             *
      ...
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'pss-mod'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *                  *---------------------------------------------*
           call      "swd/xpg/prg/obj/pxpg4300"                       .
           cancel    "swd/xpg/prg/obj/pxpg4300"                       .
       qry-trt-fun-999.
           exit.
      ...
      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'cod-mod' per determinare i   *
      *    * caratteri iniziali del codice modulo                      *
      *    *-----------------------------------------------------------*
       ipc-dlp-cdm-000.
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-ipc-cod-mod
           else      move  spaces         to   w-ipc-cod-mod          .
       ipc-dlp-cdm-999.
           exit.

      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'non-tgr' per determinare u-  *
      *    * na eventuale esclusione dalla visualizzazione dei moduli  *
      *    * tipografici                                               *
      *    *-----------------------------------------------------------*
       ipc-dlp-ntg-000.
           move      "CV"                 to   s-ope                  .
           move      "non-tgr"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-ipc-non-tgr
           else      move  spaces         to   w-ipc-non-tgr          .
       ipc-dlp-ntg-999.
           exit.
