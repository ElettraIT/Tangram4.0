      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampa Codici Stampante                     *
      *                                                                *
      *================================================================*
      ...
      *            * Codice stampante                                  *
      *            *---------------------------------------------------*
               10  srt-cod-stp            pic  x(08)                  .
      *        *-------------------------------------------------------*
      *        * Dati                                                  *
      *        *-------------------------------------------------------*
           05  srt-dat.
      *            *---------------------------------------------------*
      *            * Area per ridefinizione codice stampante           *
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Codici stampante da stampare               *
      *        *-------------------------------------------------------*
           05  w-exp-tip-stp.
      ...
      *    *===========================================================*
      *    * Prompt : Codici stampante da stampare                     *
      *    *-----------------------------------------------------------*
       pmt-tip-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Codici stampante da stampare    :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-tip-stp-999.
      ...
      *    *===========================================================*
      *    * Accettazione : Codici stampante da stampare               *
      *    *-----------------------------------------------------------*
       acc-tip-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codici stampante da stampare            *
      *    *-----------------------------------------------------------*
       vis-tip-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-010.
      *              *-------------------------------------------------*
      *              * Deviazione a seconda del tipo codici stampante  *
      *              * da stampare                                     *
      *              *-------------------------------------------------*
           if        rr-tip-stp           =    01
           ...
           move      "Manca la scelta sui codici stampante da stampare !
      ...
      *                  *---------------------------------------------*
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Se solo un codice stampante                     *
      *              *-------------------------------------------------*
       tdo-ric-sel-305.
      *                  *---------------------------------------------*
      *                  * Test sul codice stampante                   *
      *                  *---------------------------------------------*
           if        rr-cod-stp           not  = spaces
                     go to tdo-ric-sel-310.
           move      "Manca il codice della stampante !                 
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
           go to     tdo-ric-sel-999.
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Start su codici stampante di [pss], se errata : *
      *              * uscita                                          *
      *              *-------------------------------------------------*
           move      "stp "               to   pss-tre                .
           if        rr-tip-stp           =    02
                     move  rr-cod-stp     to   pss-kre
       ...
       stp-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Next su codici stampante di [pss], se fine : u- *
      *              * scita                                           *
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-200.
      *              *-------------------------------------------------*
      *              * Test max su codici stampante di [pss], se non   *
      *              * superato : uscita                               *
      *              *-------------------------------------------------*
           if        pss-tre              not  = "stp "
                     go to stp-srt-inp-900.
      ...
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-350.
      *              *-------------------------------------------------*
      *              * Test max per richiesta di singolo codice stam-  *
      *              * pante                                           *
      *              *-------------------------------------------------*
           if        rr-tip-stp           not  = 02
                     go to stp-srt-inp-400.
           if        rr-cod-stp           =    spaces
                     go to stp-srt-inp-400.
           if        w-stp-cod-stp        not  = rr-cod-stp
                     go to stp-srt-inp-900.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *                  * Chiave                                      *
      *                  *---------------------------------------------*
           move      w-stp-cod-stp        to   srt-cod-stp            .
      *                  *---------------------------------------------*
      *                  * Dati                                        *
      *                  *---------------------------------------------*
           move      w-stp                to   srt-dat                .
       stp-srt-inp-500.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun codice stampante da stampare entro i limiti
      -              " assegnati !"       to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *
           if        w-cod-stp-chr (1)    =    "#"
                     move  "(locale)"     to   p-alf
           else      move  w-stp-can-stp  to   p-alf                  .
      *
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      ...
      *
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Inibizione salto pagina a fine stampa in    *
      *                  * spool                                       *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
      ...
      *
           if        w-stp-pwd-mms        =    "S"
                     move  "Inibito"      to   p-alf
           else      move  "Abilit."      to   p-alf                  .
      *
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "CODICI STAMPANTE"   to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      " Codice           Descrizione / Ubicazione        
      -              "    Tipo                Canale di uscita          
      -              "    Pass.St.  Pass.Mo.  Salto P."
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sta-400.
