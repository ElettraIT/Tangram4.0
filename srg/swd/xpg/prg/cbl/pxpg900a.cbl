      ...
      *                                                                *
      *                    Stampa di un file, gia' in formato ASCII,   *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   STAMPA DI UN FILE IN FORMATO ASCII   "       .
      ...
      *        *                                                       *
      *        * - 01 : No                                             *
      *        * - 02 : Log di spedizione mail                         *
      *        * - 03 : Log di caricamento fatture XML fornitori       *
      *        *-------------------------------------------------------*
           05  rr-pth-sys                 pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Pathname file ASCII da stampare                       *
      *        *-------------------------------------------------------*
           05  rr-pth-fil                 pic  x(40) value spaces     .
      *        *-------------------------------------------------------*
      ...
      *            * Stampa numero riga                                *
      *            *---------------------------------------------------*
               10  rr-opz-stp-nri         pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Stampa interlinea                                 *
      *            *---------------------------------------------------*
               10  rr-opz-stp-inl         pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Solo fincatura                                    *
      *            *---------------------------------------------------*
               10  rr-opz-stp-fnc         pic  x(01)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Pathname di sitema                         *
      *        *-------------------------------------------------------*
           05  w-exp-pth-sys.
               10  w-exp-pth-sys-num      pic  9(02)       value 03   .
               10  w-exp-pth-sys-lun      pic  9(02)       value 25   .
               10  w-exp-pth-sys-tbl.
                   15  filler             pic  x(25) value
                            "No                       "               .
                   15  filler             pic  x(25) value
                            "LOG spedizione Mail      "               .
                   15  filler             pic  x(25) value
                            "LOG ricezione fatture XML"               .
      *        *-------------------------------------------------------*
      *        * Work per : Opzioni di stampa                          *
      *        *-------------------------------------------------------*
           05  w-exp-opz-stp.
               10  w-exp-opz-stp-num      pic  9(02)       value 03   .
      ...
      *                  * stampare                                    *
      *                  *---------------------------------------------*
           move      rr-pth-fil           to   w-cnt-tit-des-azi      .
      ...
      *                  * Pathname di sistema                         *
      *                  *---------------------------------------------*
           perform   acc-pth-sys-000      thru acc-pth-sys-999        .
      ...
      *                  * Pathname di sistema                         *
      *                  *---------------------------------------------*
           perform   pmt-pth-sys-000      thru pmt-pth-sys-999        .
      *                  *---------------------------------------------*
      *                  * Pathname file ASCII da stampare             *
      *                  *---------------------------------------------*
           perform   pmt-pth-fil-000      thru pmt-pth-fil-999        .
      *                  *---------------------------------------------*
      *                  * Ampiezza della linea stampa da considerare  *
      *                  *---------------------------------------------*
      ...
      *                  * Pathname di sistema                         *
      *                  *---------------------------------------------*
           perform   vis-pth-sys-000      thru vis-pth-sys-999        .
      *                  *---------------------------------------------*
      *                  * Pathname file ASCII da stampare             *
      *                  *---------------------------------------------*
           perform   vis-pth-fil-000      thru vis-pth-fil-999        .
      *                  *---------------------------------------------*
      *                  * Ampiezza della linea stampa da considerare  *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Pathname di sistema                  *
      *    *-----------------------------------------------------------*
       acc-pth-sys-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-pth-sys-lun    to   v-car                  .
           move      w-exp-pth-sys-num    to   v-ldt                  .
           move      "NMX#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-pth-sys-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      05                   to   v-lin                  .
           move      31                   to   v-pos                  .
           move      rr-pth-sys           to   v-num                  .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-pth-sys-999.
       acc-pth-sys-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-pth-sys             .
       acc-pth-sys-400.
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *
      *                  *---------------------------------------------*
           if        rr-pth-sys           not  = zero
                     go to acc-pth-sys-600.
           if        v-key                =    "UP  "
                     go to acc-pth-sys-600.
           go to     acc-pth-sys-100.
       acc-pth-sys-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Eventuale precablaggio pathname             *
      *                  *---------------------------------------------*
           if        rr-pth-sys           =    02
                     move  "/abd/asc/log/t_sender_ssl.log"
                                          to   rr-pth-fil
           else if   rr-pth-sys           =    03
                     move  "/abd/asc/log/t_xml_for_imp.log"
                                          to   rr-pth-fil
           else      move  spaces         to   rr-pth-fil             .
       acc-pth-sys-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Pathname di sistema                     *
      *    *-----------------------------------------------------------*
       vis-pth-sys-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           move      rr-pth-fil           to   w-sav-pth-fil          .
       acc-pth-fil-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-pth-fil           not  = spaces
                     go to acc-pth-fil-450.
           if        v-key                =    "UP  "
                     go to acc-pth-fil-600
           else      go to acc-pth-fil-100.
       acc-pth-fil-450.
      *                  *---------------------------------------------*
      *                  * Che il file esista                          *
      *                  *---------------------------------------------*
       acc-pth-fil-452.
           if        rr-pth-fil           =    w-sav-pth-fil
                     go to acc-pth-fil-475.
       acc-pth-fil-454.
           perform   opn-fil-seq-000      thru opn-fil-seq-999        .
           if        f-seq-sts            =    e-not-err
                     go to acc-pth-fil-456
           else      go to acc-pth-fil-458.
       acc-pth-fil-456.
