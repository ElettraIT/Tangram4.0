      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Rubrica telefonica Banche (.tb)             *
      *                                                                *
      *================================================================*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-arc        to   w-cod-des-cbp-cod      .
           move      07                   to   w-cod-des-cbp-lin      .
           move      16                   to   w-cod-des-cbp-pos      .
           move      07                   to   w-cod-des-cbp-dln      .
           move      29                   to   w-cod-des-cbp-dps      .
      ...
      *              *-------------------------------------------------*
           move      v-alf                to   w-tes-cod-arc          .
       acc-cod-cbp-300.
      *              *-------------------------------------------------*
      *              * Se 'Expand'                                     *
      *              *-------------------------------------------------*
           if        v-key                not  = "EXPD"
                     go to acc-cod-cbp-320.
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                not  = "PRSC"
                     go to acc-cod-cbp-340.
      *                  *---------------------------------------------*
      *                  * Subroutine di trattamento                   *
      *                  *---------------------------------------------*
           move      "-"                  to   w-def-tip-ric          .
           perform   rou-trt-nxs-cbp-000  thru rou-trt-nxs-cbp-999    .
      *                  *---------------------------------------------*
      *                  * A reimpostazione                            *
      *                  *---------------------------------------------*
           go to     acc-cod-cbp-100.
      ...
      *                  *---------------------------------------------*
           go to     acc-cod-cbp-400.
       acc-cod-cbp-365.
      *              *-------------------------------------------------*
      *              * Trattamento Pf2                                 *
      *              *-------------------------------------------------*
           if        v-key                not  = "[2] "
                     go to acc-cod-cbp-370.
      *                  *---------------------------------------------*
      *                  * Forzatura selettore tipo contatto           *
      *                  *---------------------------------------------*
           if        w-tes-tip-sel        =    spaces
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se codice impostato a spazi           *
      *                  *---------------------------------------------*
           if        w-tes-cod-arc        not  = spaces
                     go to acc-cod-cbp-420.
           if        v-key                =    "UP  "
                     go to acc-cod-cbp-800
           else      go to acc-cod-cbp-100.
       acc-cod-cbp-420.
      *                  *---------------------------------------------*
      *                  * Normalizzazione record [cbp]                *
      *                  *---------------------------------------------*
           move      "NO"                 to   f-ope                  .
           move      "pgm/gep/fls/ioc/obj/iofcbp"
                                          to   s-pat                  .
           ...
           call      s-pat               using f
                                               rf-cbp                 .
      *                  *---------------------------------------------*
      *                  * Lettura record da file [cbp]                *
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCBP    "         to   f-key                  .
           move      02                   to   rf-cbp-tip-cbp         .
           move      w-tes-cod-arc        to   rf-cbp-cod-cbp         .
           move      "pgm/gep/fls/ioc/obj/iofcbp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * Messaggio di errore                 *
      *                          *-------------------------------------*
           move      "Anagrafica banca non trovata !"
      ...
      *                          *-------------------------------------*
           perform   buf-rec-cbp-000      thru buf-rec-cbp-999        .
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-def-tip-ord        =    zero
                     go to rou-trt-nxs-cbp-800
           else if   w-def-tip-ord        =    1
                     go to rou-trt-nxs-cbp-100
           else if   w-def-tip-ord        =    2
                     go to rou-trt-nxs-cbp-200
           else      go to rou-trt-nxs-cbp-800.
       rou-trt-nxs-cbp-100.
      *              *-------------------------------------------------*
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      02                   to   rf-cbp-tip-cbp         .
           move      w-tes-cod-arc-rgk    to   rf-cbp-des-key         .
           move      w-tes-cod-arc        to   rf-cbp-cod-cbp         .
           move      "pgm/gep/fls/ioc/obj/iofcbp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to rou-trt-nxs-cbp-900.
      *                  *---------------------------------------------*
      *                  * A Read-Next per [cbp]                       *
      *                  *---------------------------------------------*
           go to     rou-trt-nxs-cbp-500.
       rou-trt-nxs-cbp-200.
      *              *-------------------------------------------------*
      *              * Codice                                          *
      *              *-------------------------------------------------*
      ...
      *                  * Bufferizzazione                             *
      *                  *---------------------------------------------*
           move      rf-cbp-cod-cbp       to   w-tes-cod-arc          .
           perform   buf-rec-cbp-000      thru buf-rec-cbp-999        .
       rou-trt-nxs-cbp-620.
      *              *-------------------------------------------------*
      *              * Visualizzazione dati letti                      *
      ...
      *                  * Anagrafica                                  *
      *                  *---------------------------------------------*
           move      rf-cbp-des-cbp       to   w-tes-cod-arc-rag      .
           move      rf-cbp-des-key       to   w-tes-cod-arc-rgk      .
           move      spaces               to   w-tes-cod-arc-mne      .
           move      w-all-str-alf        to   w-tes-cod-arc-via      .
           move      rf-cbp-sgl-ccb       to   w-tes-cod-arc-loc      .
           move      spaces               to   w-tes-cod-arc-nom      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-cbp-ide-cbp       to   d-con-arc-cod-arc      .
       ...
       buf-rec-cbp-200.
           ...
           move      d-con-arc-num-con
                    (w-tes-con-ctr)       to   w-tes-num-con
                                              (w-tes-con-ctr)         .
           go to     buf-rec-cbp-200.
       buf-rec-cbp-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     buf-rec-cbp-999.
       buf-rec-cbp-999.
           exit.
