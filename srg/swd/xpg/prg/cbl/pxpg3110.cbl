      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su codici comandi            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    INTERROGAZIONE SU CODICI COMANDI    "       .
      ...
      *            *---------------------------------------------------*
      *            * Flag di primo elemento dettaglio                  *
      *            *---------------------------------------------------*
               10  w-cnt-qry-det-uno      pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Literals                                              *
      *        *-------------------------------------------------------*
           05  w-cnt-lit.
      ...
      *    *-----------------------------------------------------------*
       01  rr.
      *        *-------------------------------------------------------*
      *        * Codice comando min, da i.p.c.                         *
      *        *-------------------------------------------------------*
           05  rr-cod-min                 pic  x(06)                  .
      *        *-------------------------------------------------------*
      *        * Codice comando max, da i.p.c.                         *
      *        *-------------------------------------------------------*
           05  rr-cod-max                 pic  x(06)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-ipc.
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'cod-cmd' letta ad inizio pro-    *
      *        * gramma per determinare una eventuale limitazione ai   *
      *        * soli codici comando con l'iniziale dichiarata         *
      *        *-------------------------------------------------------*
           05  w-ipc-cod-cmd              pic  x(06)                  .
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'des-cmd' letta ad inizio pro-    *
      *        * gramma per determinare una eventuale ricerca tramite  *
      *        * la descrizione od una parte di essa                   *
      *        *-------------------------------------------------------*
           05  w-ipc-des-cmd              pic  x(40)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-rot-l02.
               10  w-rot-l02-are-cmd      pic  x(03)                  .
      *        *-------------------------------------------------------*
      *        * 1. livello di rottura                                 *
      *        *-------------------------------------------------------*
           05  w-rot-l01.
               10  w-rot-l01-set-cmd      pic  x(03)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-pre-exe-pgm      .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'cod-cmd'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-cdc-000      thru ipc-dlp-cdc-999        .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'des-cmd'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-ddc-000      thru ipc-dlp-ddc-999        .
       pre-exe-pgm-999.
           exit.
      ...
      *              * Codice min                                      *
      *              *-------------------------------------------------*
           move      w-ipc-cod-cmd        to   rr-cod-min             .
      *              *-------------------------------------------------*
      *              * Codice max                                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Eventuale ritaratura funzionamento automatico   *
      *              *-------------------------------------------------*
           if        w-ipc-des-cmd        not  = spaces
                     move  "S"            to   w-cnt-fun-snx-aut      .
      *              *-------------------------------------------------*
      *              * Eventuale normalizzazione                       *
      *              *-------------------------------------------------*
           if        w-ipc-des-cmd        =    spaces
                     go to reg-ric-sel-999.
           move      w-ipc-des-cmd        to   w-all-str-alf          .
           move      40                   to   w-all-str-lun          .
           perform   all-nor-atz-000      thru all-nor-atz-999        .
           move      w-all-str-alf        to   w-ipc-des-cmd          .
       reg-ric-sel-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun codice comando memorizzato !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
       ...
       qry-sel-rec-100.
      *              *-------------------------------------------------*
      *              * Se il programma e' stato richiamato per una     *
      *              * ricerca, non si visualizzano i comandi relativi *
      *              * ai menu                                         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su variabili di i.p.c.                 *
      *                  *---------------------------------------------*
           if        w-ipc-des-cmd        =    spaces and
                     w-ipc-cod-cmd        =    spaces
                     go to qry-sel-rec-999.
      *                  *---------------------------------------------*
      *                  * Test su tipo comando : solo 'task'          *
      *                  *---------------------------------------------*
       ...
       qry-sel-rec-200.
      *              *-------------------------------------------------*
      *              * Selezione tramite eventuale componente della    *
      *              * descrizione eventualmente passata in i.p.c.     *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  * Test                                        *
      *                  *---------------------------------------------*
           move      w-cmd-des-cmd        to   w-all-str-alf          .
           move      40                   to   w-all-str-lun          .
           perform   all-nor-atz-000      thru all-nor-atz-999        .
      ...
      *                  *---------------------------------------------*
           move      w-ipc-des-cmd        to   w-all-str-cat (1)      .
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-cmp-rot-000.
           move      w-cmd-are-cmd        to   w-rot-l02-are-cmd      .
           move      w-cmd-set-cmd        to   w-rot-l01-set-cmd      .
       qry-cmp-rot-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *              * Se il programma e' stato richiamato per una     *
      *              * ricerca, l'intestazione si esegue a inizio      *
      *              * ciclo                                           *
      *              *-------------------------------------------------*
           if        w-ipc-des-cmd        =    spaces and
                     w-ipc-cod-cmd        =    spaces
      ...
      *              * ciclo                                           *
      *              *-------------------------------------------------*
           if        w-ipc-des-cmd        not  = spaces or
                     w-ipc-cod-cmd        not  = spaces
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-int      .
      *              *-------------------------------------------------*
      *              * Inizializzazione marker di primo giro dettaglio *
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-det-uno      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se menu comando da stampare            *
      *                  *---------------------------------------------*
           if        w-cnt-qry-det-uno    not  = spaces
                     go to qry-liv-det-300.
      *                  *---------------------------------------------*
      *                  * Marker di primo giro dettaglio              *
      *                  *---------------------------------------------*
           move      "#"                  to   w-cnt-qry-det-uno      .
      *                  *---------------------------------------------*
      *                  * Assemblaggio                                *
      *                  *---------------------------------------------*
           ...
           move      w-rot-l02-are-cmd    to   w-all-str-cat (2)      .
           move      w-rot-l01-set-cmd    to   w-all-str-cat (3)      .
      ...
      *                  *---------------------------------------------*
           ...
           move      17                   to   v-pos                  .
           move      "+"                  to   v-edm                  .
           move      w-cmd-cod-cmd        to   w-mpn-cod-cmd          .
           move      w-mpn                to   v-cnt                  .
      ...
      *                  * Descrizione comando                         *
      *                  *                                             *
      *                  * Nota : Viene eventualmente eliminato il ca- *
      *                  *        rattere '*' a posizione 40           *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      40                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Indicatore tipo task                        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        w-cmd-tip-cmd        >    zero
                     go to qry-liv-det-600.
      *                      *-----------------------------------------*
      *                      * Test se NON standard                    *
      *                      *-----------------------------------------*
           if        w-cmd-sis-cmd        =   "pgm"
                     go to qry-liv-det-350
           else if   w-cmd-sis-cmd        =   "swd"
                     move "{sistema}"     to   w-all-str-alf
                     go to qry-liv-det-400
           else      move "<su misura>"   to   w-all-str-alf
                     go to qry-liv-det-400.
       qry-liv-det-350.
      *                      *-----------------------------------------*
      *                      * Assemblaggio                            *
      ...
      *
           if        w-cmd-tip-tsk        =    01
                     move  "Gestione   "  to   w-all-str-cat (2)
           else if   w-cmd-tip-tsk        =    02
                     move  "Interrogaz."  to   w-all-str-cat (2)
           else if   w-cmd-tip-tsk        =    03
                     move  "Stampa     "  to   w-all-str-cat (2)
      ...
      *
           if        w-cmd-tip-cmd        =    01
                     move  "menu'"        to   w-all-str-cat (2)
      *
           else      move  "alias"        to   w-all-str-cat (2)      .
           move      "]"                  to   w-all-str-cat (3)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
      *                      *-----------------------------------------*
      *                      * Indicatore                              *
      ...
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'auc-cmd'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *                  *---------------------------------------------*
           call      "swd/xpg/prg/obj/pxpg3100"                       .
           cancel    "swd/xpg/prg/obj/pxpg3100"                       .
       qry-trt-fun-999.
           exit.
      ...
      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'cod-cmd' per determinare i   *
      *    * caratteri iniziali del codice comando                     *
      *    *-----------------------------------------------------------*
       ipc-dlp-cdc-000.
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-ipc-cod-cmd
           else      move  spaces         to   w-ipc-cod-cmd          .
       ipc-dlp-cdc-999.
           exit.

      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'des-cmd' per la ricerca      *
      *    *-----------------------------------------------------------*
       ipc-dlp-ddc-000.
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-ipc-des-cmd
           else      move  spaces         to   w-ipc-des-cmd          .
       ipc-dlp-ddc-999.
           exit.
