      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampa Codici Modulo                        *
      *                                                                *
      *================================================================*
      ...
      *            * Codice modulo                                     *
      *            *---------------------------------------------------*
               10  srt-cod-mod            pic  x(08)                  .
      *        *-------------------------------------------------------*
      *        * Dati                                                  *
      *        *-------------------------------------------------------*
           05  srt-dat.
      *            *---------------------------------------------------*
      *            * Area per ridefinizione codice modulo              *
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Codici modulo da stampare                  *
      *        *-------------------------------------------------------*
           05  w-exp-tip-stp.
      ...
      *    *===========================================================*
      *    * Prompt : Codici modulo da stampare                        *
      *    *-----------------------------------------------------------*
       pmt-tip-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Codici modulo da stampare  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-tip-stp-999.
      ...
      *    *===========================================================*
      *    * Accettazione : Codici modulo da stampare                  *
      *    *-----------------------------------------------------------*
       acc-tip-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codici modulo da stampare               *
      *    *-----------------------------------------------------------*
       vis-tip-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-010.
      *              *-------------------------------------------------*
      *              * Deviazione a seconda del tipo codici modulo     *
      *              * da stampare                                     *
      *              *-------------------------------------------------*
           if        rr-tip-stp           =    01
           ...
           move      "Manca la scelta sui codici modulo da stampare !   
      ...
      *                  *---------------------------------------------*
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Se solo un codice modulo                        *
      *              *-------------------------------------------------*
       tdo-ric-sel-305.
      *                  *---------------------------------------------*
      *                  * Test sul codice modulo                      *
      *                  *---------------------------------------------*
           if        rr-cod-mod           not  = spaces
                     go to tdo-ric-sel-310.
           move      "Manca il codice del modulo !                      
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
           go to     tdo-ric-sel-999.
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Start su codici modulo di [pss], se errata :    *
      *              * uscita                                          *
      *              *-------------------------------------------------*
           move      "mod "               to   pss-tre                .
           if        rr-tip-stp           =    02
                     move  rr-cod-mod     to   pss-kre
       ...
       stp-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Next su codici modulo di [pss], se fine : u-    *
      *              * scita                                           *
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-200.
      *              *-------------------------------------------------*
      *              * Test max su codici modulo di [pss], se non su-  *
      *              * perato : uscita                                 *
      *              *-------------------------------------------------*
           if        pss-tre              not  = "mod "
                     go to stp-srt-inp-900.
      ...
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-350.
      *              *-------------------------------------------------*
      *              * Test max per richiesta di singolo codice modu-  *
      *              * lo                                              *
      *              *-------------------------------------------------*
           if        rr-tip-stp           not  = 02
                     go to stp-srt-inp-400.
           if        rr-cod-mod           =    spaces
                     go to stp-srt-inp-400.
           if        w-mod-cod-mod        not  = rr-cod-mod
                     go to stp-srt-inp-900.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *                  * Chiave                                      *
      *                  *---------------------------------------------*
           move      w-mod-cod-mod        to   srt-cod-mod            .
      *                  *---------------------------------------------*
      *                  * Dati                                        *
      *                  *---------------------------------------------*
           move      w-mod                to   srt-dat                .
       stp-srt-inp-500.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun codice modulo da stampare entro i limiti as
      -              "segnati !"          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *                  *---------------------------------------------*
           ...
           if        w-mod-tip-mod        =    "T"
                     ...
                     move  "Specifico non tipografico     "
                                          to   p-alf
           else      move  "Libero                        "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      041                  to   p-pos                  .
           move      w-mod-a72-mod        to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-125.
      *              *-------------------------------------------------*
      *              * Interlinee possibili                            *
      *              *-------------------------------------------------*
       prn-liv-det-126.
      *                  *---------------------------------------------*
      *                  * Test se numero righe residue sufficienti    *
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-131.
           ...
           if        w-det-inx-001        >    20
                     go to prn-liv-det-137.
           if        w-mod-int-mod
                    (w-det-inx-001)       =    zero
                     go to prn-liv-det-131.
       prn-liv-det-132.
           if        w-det-ctr-001        =    zero
                     go to prn-liv-det-134.
           if        p-res                >    1
                     go to prn-liv-det-133.
           perform   int-pag-sta-000      thru int-pag-sta-999        .
           if        w-cnt-prn-flg-int    not  = spaces
                     go to prn-liv-det-999.
       prn-liv-det-133.
      ...
      *              *-------------------------------------------------*
           move      "CODICI MODULO"      to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
