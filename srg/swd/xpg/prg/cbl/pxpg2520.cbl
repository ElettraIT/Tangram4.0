      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampa Procedure di salvataggio             *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
               10  srt-cod-bak            pic  x(12)                  .
      *        *-------------------------------------------------------*
      *        * Dati                                                  *
      *        *-------------------------------------------------------*
           05  srt-dat.
      *            *---------------------------------------------------*
      *            * Area per ridefinizione procedura di salvataggio   *
      *            *---------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    STAMPA PROCEDURE DI SALVATAGGIO     "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Procedure da stampare                      *
      *        *-------------------------------------------------------*
           05  w-exp-tip-stp.
      ...
      *    *===========================================================*
      *    * Accettazione : Procedure da stampare                      *
      *    *-----------------------------------------------------------*
       acc-tip-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione : Procedure da stampare                   *
      *    *-----------------------------------------------------------*
       vis-tip-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Codice procedura di slvataggio             *
      *    *-----------------------------------------------------------*
       acc-cod-bak-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codice procedura di slvataggio          *
      *    *-----------------------------------------------------------*
       vis-cod-bak-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-010.
      *              *-------------------------------------------------*
      *              * Deviazione a seconda del tipo procedura di sal- *
      *              * vataggio da stampare                            *
      *              *-------------------------------------------------*
           if        rr-tip-stp           =    01
                     go to tdo-ric-sel-100
           else if   rr-tip-stp           =    02
                     go to tdo-ric-sel-200.
           move      "Manca la scelta sui tipi procedura da stampare !  
      ...
      *                  *---------------------------------------------*
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Se solo una procedura                           *
      *              *-------------------------------------------------*
       tdo-ric-sel-305.
      *                  *---------------------------------------------*
      *                  * Test sul codice procedura                   *
      *                  *---------------------------------------------*
           if        rr-cod-bak           not  = spaces
                     go to tdo-ric-sel-310.
           move      "Manca la sigla della procedura !                  
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
           go to     tdo-ric-sel-999.
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Start su procedure di salvataggio di [auc], se  *
      *              * errata : uscita                                 *
      *              *-------------------------------------------------*
           move      "bak "               to   auc-tre                .
           if        rr-tip-stp           =    02
                     move  rr-cod-bak     to   auc-kre
       ...
       stp-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Next su procedure di salvataggio di [auc], se   *
      *              * fine : uscita                                   *
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-200.
      *              *-------------------------------------------------*
      *              * Test max su procedure di salvataggio di [auc],  *
      *              * se non superato : uscita                        *
      *              *-------------------------------------------------*
           if        auc-tre              not  = "bak "
                     go to stp-srt-inp-900.
       stp-srt-inp-300.
      ...
      *              *-------------------------------------------------*
           move      auc-dat              to   w-bak                  .
       stp-srt-inp-350.
      *              *-------------------------------------------------*
      *              * Test max per richiesta di singola procedura di  *
      *              * salvataggio                                     *
      *              *-------------------------------------------------*
           ...
           if        rr-cod-bak           =    spaces
                     go to stp-srt-inp-400.
           if        w-bak-cod-bak        not  = rr-cod-bak
                     go to stp-srt-inp-900.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *                  * Chiave                                      *
      *                  *---------------------------------------------*
           move      w-bak-cod-bak        to   srt-cod-bak            .
      *                  *---------------------------------------------*
      *                  * Dati                                        *
      *                  *---------------------------------------------*
           move      w-bak                to   srt-dat                .
       stp-srt-inp-500.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessuna procedura di salvataggio da stampare entro
      -              " i limiti assegnati !         "
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
      ...
      *                  *---------------------------------------------*
           ...
           if        w-bak-tut-abl        =    01
                     move  "Tutti                         "
                                          to   p-alf
           else if   w-bak-tut-abl        =    02
                     move  "Solo l'utente supervisore     "
                                          to   p-alf
      ...
      *                  *---------------------------------------------*
           ...
           move      039                  to   p-pos                  .
           if        w-bak-sts-bak        =    01
                     move  "Puo' essere eseguita          "
                                          to   p-alf
           else if   w-bak-sts-bak        =    02
                     move  "Bloccata                      "
                                          to   p-alf
           else      move  spaces         to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
      ...
      *                  *---------------------------------------------*
           ...
           if        w-bak-tip-dev        =    01
                     ...
                     move  "Unita' per aggiornam. software"
                                          to   p-alf
       ...
       prn-liv-det-150.
      *              *-------------------------------------------------*
      *              * Pathname unita' speciale o nome della procedu-  *
      *              * ra batch                                        *
      *              *-------------------------------------------------*
       prn-liv-det-151.
      *                  *---------------------------------------------*
      *                  * Se non si tratta di unita' speciale : no    *
      *                  * stampa                                      *
      *                  *---------------------------------------------*
           if        w-bak-tip-dev        not  = 99 and
                     w-bak-tip-dev        not  = 55
      ...
      *                  *---------------------------------------------*
           ...
           if        w-bak-tip-dev        =    99
                     move  "Pathname unita' speciale ..... :"
                                          to   p-alf
           else      move  "Nome procedura batch ......... :"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-154.
      *                  *---------------------------------------------*
      *                  * Pathname unita' speciale o nome della pro-  *
      *                  * cedura batch                                *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      006                  to   p-pos                  .
           move      "Directory di base ............ :"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-180.
      *                  *---------------------------------------------*
      *                  * Pathname della directory di base            *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      40                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-212.
           ...
           if        w-det-inx-001        >    32
                     go to prn-liv-det-217.
           if        w-bak-pth-edd
                    (w-det-inx-001)       =    spaces
                     go to prn-liv-det-212.
       prn-liv-det-213.
           if        w-det-ctr-001        =    zero
                     go to prn-liv-det-215.
           if        p-res                >    1
                     go to prn-liv-det-214.
           perform   int-pag-sta-000      thru int-pag-sta-999        .
           if        w-cnt-prn-flg-int    not  = spaces
                     go to prn-liv-det-999.
       prn-liv-det-214.
