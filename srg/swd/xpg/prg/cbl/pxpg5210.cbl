      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione Referenze                    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  srt-key.
      *            *---------------------------------------------------*
      *            * Codice referenza, come da definizione             *
      *            *---------------------------------------------------*
               10  srt-key-drf-cod        pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Numero progressivo                                *
      *            *---------------------------------------------------*
               10  srt-key-ref-prg        pic  9(03)                  .
      *        *-------------------------------------------------------*
      *        * Dati                                                  *
      *        *-------------------------------------------------------*
           05  srt-dat.
      ...
      *            *  - T : Text                                       *
      *            *---------------------------------------------------*
               10  srt-dat-drf-tip        pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Numero caratteri, se alfanumerica o numerica      *
      *            *---------------------------------------------------*
               10  srt-dat-drf-cas        pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Numero decimali, se numerica                      *
      *            *---------------------------------------------------*
               10  srt-dat-drf-dec        pic  9(01)                  .
      *            *---------------------------------------------------*
      *            * Segno algebrico, se numerica                      *
      *            *---------------------------------------------------*
               10  srt-dat-drf-sgn        pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Referenza singola o multipla                      *
      *            *---------------------------------------------------*
               10  srt-dat-drf-som        pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Codice referenza, come valore                     *
      *            *---------------------------------------------------*
               10  srt-dat-ref-cod        pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Valore referenza, se alfa o text                  *
      *            *---------------------------------------------------*
               10  srt-dat-ref-alf        pic  x(80)                  .
      *            *---------------------------------------------------*
      *            * Valore referenza, se numerica                     *
      *            *---------------------------------------------------*
               10  srt-dat-ref-num        pic s9(13)v9(05) trailing
                                                           separate
                                                           character  .
      *            *---------------------------------------------------*
      *            * Valore referenza, se data                         *
      *            *---------------------------------------------------*
               10  srt-dat-ref-dat        pic  9(07)                  .
      ...
      *            *---------------------------------------------------*
               10  w-ipc-dlp-nmr.
      *                *-----------------------------------------------*
      *                * Nome referenza passato                        *
      *                *-----------------------------------------------*
                   15  w-ipc-dlp-nmr-nom  pic  x(32)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-nep-ref.
               10  w-mpn-nom-ref          pic  x(32)                  .
               10  w-mpn-prg-ref          pic  9(03)                  .
      ...
      *              * referenza da considerare                        *
      *              *-------------------------------------------------*
           perform   ipc-dlp-nmr-000      thru ipc-dlp-nmr-999        .
           move      w-ipc-dlp-nmr-nom    to   rr-nom-min             .
           move      w-ipc-dlp-nmr-nom    to   rr-nom-max             .
      ...
      *    *-----------------------------------------------------------*
       int-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Start su definizioni referenze                  *
      *              *-------------------------------------------------*
           move      "drf "               to   auc-tre                .
       ...
       int-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale definizioni referenze       *
      *              *-------------------------------------------------*
           read      auc    next
                            with no lock
                            at end
      ...
      *                  *---------------------------------------------*
           move      "ref "               to   ref-tre                .
           move      w-drf-cod-drf        to   ref-cod                .
           move      zero                 to   ref-prg                .
      ...
      *                      * Sulla chiave                            *
      *                      *-----------------------------------------*
           move      ref-cod              to   w-key-ref-key          .
           perform   sep-key-ref-000      thru sep-key-ref-999        .
           if        w-key-ref-pre        not  = w-drf-cod-drf
                     go to int-srt-inp-600.
       int-srt-inp-525.
      *                  *---------------------------------------------*
      *                  * Selezione su valori                         *
      *                  *---------------------------------------------*
       int-srt-inp-527.
      *                      *-----------------------------------------*
      *                      * Sul numero progressivo, se referenza    *
      *                      * multipla                                *
      *                      *-----------------------------------------*
           if        w-drf-som-drf        not  = "M"
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   srt-key-drf-cod        .
           move      zero                 to   srt-key-ref-prg        .
           move      spaces               to   srt-dat-drf-des        .
           move      spaces               to   srt-dat-drf-tip        .
           move      zero                 to   srt-dat-drf-cas        .
           move      zero                 to   srt-dat-drf-dec        .
           move      spaces               to   srt-dat-drf-sgn        .
           move      spaces               to   srt-dat-drf-som        .
           move      spaces               to   srt-dat-ref-cod        .
           move      spaces               to   srt-dat-ref-alf        .
           move      zero                 to   srt-dat-ref-num        .
           move      zero                 to   srt-dat-ref-dat        .
       int-srt-inp-809.
           exit.
       int-srt-inp-810.
      *              *-------------------------------------------------*
      *              * Composizione area per definizione               *
      *              *-------------------------------------------------*
           move      w-drf-cod-drf        to   srt-key-drf-cod        .
           move      w-drf-des-drf        to   srt-dat-drf-des        .
           move      w-drf-tip-drf        to   srt-dat-drf-tip        .
           move      w-drf-car-drf        to   srt-dat-drf-cas        .
           move      w-drf-dec-drf        to   srt-dat-drf-dec        .
           move      w-drf-sgn-drf        to   srt-dat-drf-sgn        .
           move      w-drf-som-drf        to   srt-dat-drf-som        .
       int-srt-inp-819.
           exit.
       int-srt-inp-820.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-drf-som-drf        =    "M"
                     move  ref-prg        to   srt-key-ref-prg        .
           move      ref-cod              to   srt-dat-ref-cod        .
           if        w-drf-tip-drf        =    "A"
                     move  ref-alf        to   srt-dat-ref-alf
           else if   w-drf-tip-drf        =    "N"
                     move  ref-num        to   srt-dat-ref-num
           else if   w-drf-tip-drf        =    "D"
                     move  ref-dat        to   srt-dat-ref-dat
           else if   w-drf-tip-drf        =    "T"
                     move  ref-txt        to   srt-dat-ref-alf        .
       int-srt-inp-829.
           exit.
       int-srt-inp-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessuna referenza da visualizzare entro i limiti a
      -              "ssegnati !"         to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *                      *-----------------------------------------*
       ...
       liv-det-dfz-330.
      *                      *-----------------------------------------*
      *                      * Nome referenza, ovvero il codice come   *
      *                      * da definizione                          *
      *                      *-----------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      15                   to   v-pos                  .
           move      "+"                  to   v-edm                  .
           move      srt-key-drf-cod      to   w-mpn-nom-ref          .
           move      zero                 to   w-mpn-prg-ref          .
           move      w-mpn                to   v-cnt                  .
           move      srt-key-drf-cod      to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       liv-det-dfz-400.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Nome referenza, ovvero il codice come   *
      *                      * da valore se la referenza esiste, al-   *
      *                      * trimenti il codice come da record valo- *
      *                      * ri                                      *
      *                      *-----------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      15                   to   v-pos                  .
           move      "+"                  to   v-edm                  .
           if        srt-dat-ref-cod      not  = spaces
                     move  srt-dat-ref-cod
                                          to   w-mpn-nom-ref
           else      move  srt-key-drf-cod
                                          to   w-mpn-nom-ref          .
           if        srt-dat-ref-cod      not  = spaces
                     move  srt-key-ref-prg
                                          to   w-mpn-prg-ref
           else      move  zero           to   w-mpn-prg-ref          .
           move      w-mpn                to   v-cnt                  .
           if        srt-dat-ref-cod      not  = spaces
                     move  srt-dat-ref-cod
                                          to   v-alf
      ...
      *                          *-------------------------------------*
      *                              *---------------------------------*
      *                              * Se la referenza non esiste :    *
      *                              * semplice literal                *
      *                              *---------------------------------*
           if        srt-dat-ref-cod      =    spaces
                     go to liv-det-vlr-524.
      *                              *---------------------------------*
      *                              * Se numero progressivo a zero :  *
      *                              * semplice literal                *
      *                              *---------------------------------*
           if        srt-key-ref-prg      =    zero
                     go to liv-det-vlr-524.
      *                              *---------------------------------*
      ...
      *                      *-----------------------------------------*
       liv-det-vlr-532.
      *                          *-------------------------------------*
      *                          * Deviazione a seconda se referenza   *
      *                          * esistente oppure no                 *
      *                          *-------------------------------------*
           if        srt-dat-ref-cod      not  = spaces
      ...
      *                              * campo                           *
      *                              *---------------------------------*
           if        srt-dat-drf-tip      =    "A"
                     go to liv-det-vlr-538
           else if   srt-dat-drf-tip      =    "N"
                     go to liv-det-vlr-540
           else if   srt-dat-drf-tip      =    "D"
                     go to liv-det-vlr-542
           else if   srt-dat-drf-tip      =    "T"
      ...
      *                              *---------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      srt-dat-drf-cas      to   v-car                  .
           if        v-car                >    66
                     move  66             to   v-car                  .
           move      v-lnr                to   v-lin                  .
           move      15                   to   v-pos                  .
           move      srt-dat-ref-alf      to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           go to     liv-det-vlr-600.
       liv-det-vlr-540.
      ...
      *                              *---------------------------------*
           ...
           move      "N"                  to   v-tip                  .
           move      srt-dat-drf-cas      to   v-car                  .
           move      srt-dat-drf-dec      to   v-dec                  .
           move      srt-dat-drf-sgn      to   v-sgn                  .
           move      "9B"                 to   v-edm                  .
           move      v-lnr                to   v-lin                  .
           move      15                   to   v-pos                  .
           move      srt-dat-ref-num      to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           go to     liv-det-vlr-600.
       liv-det-vlr-542.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'nep-ref'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *    *===========================================================*
      *    * Lettura i.p.c. da livello precedente per nome referenza   *
      *    * da considerare                                            *
      *    *-----------------------------------------------------------*
       ipc-dlp-nmr-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione iniziale                        *
      *              *-------------------------------------------------*
           move      spaces               to   w-ipc-dlp-nmr-nom      .
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "nom-ref" per tipo record passato               *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                not  = spaces
                     go to ipc-dlp-nmr-999.
           move      s-alf                to   w-ipc-dlp-nmr-nom      .
       ipc-dlp-nmr-999.
           exit.
