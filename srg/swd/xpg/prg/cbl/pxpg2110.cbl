      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su codici azienda            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    INTERROGAZIONE SU CODICI AZIENDA    "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
      *        *-------------------------------------------------------*
      *        * Codice azienda min, da i.p.c.                         *
      *        *-------------------------------------------------------*
           05  rr-cod-min                 pic  x(04)                  .
      *        *-------------------------------------------------------*
      *        * Codice azienda max, da i.p.c.                         *
      *        *-------------------------------------------------------*
           05  rr-cod-max                 pic  x(04)                  .

      *    *===========================================================*
      *    * Work-area per ridefinizione contenuto del mark-point      *
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-cod-azi              pic  x(04)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-ipc.
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'azi-ute' letta ad inizio pro-    *
      *        * gramma per determinare se la visualizzazione deve     *
      *        * essere ristretta alle sole aziende per cui l'uten-    *
      *        * te risulta abilitato                                  *
      *        *-------------------------------------------------------*
           05  w-ipc-azi-ute              pic  x(01)                  .
           05  w-ipc-cod-ute              pic  x(08)                  .
           05  w-ipc-ctr-001              pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Variabile di i.p.c. 'cod-azi' letta ad inizio pro-    *
      *        * gramma per determinare una eventuale limitazione ai   *
      *        * soli codici azienda con l'iniziale dichiarata         *
      *        *-------------------------------------------------------*
           05  w-ipc-cod-azi              pic  x(04)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-pre-exe-pgm      .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'azi-ute'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-azu-000      thru ipc-dlp-azu-999        .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'cod-azi'           *
      *              *-------------------------------------------------*
           perform   ipc-dlp-cda-000      thru ipc-dlp-cda-999        .
       pre-exe-pgm-999.
           exit.
      ...
      *              * Codice min                                      *
      *              *-------------------------------------------------*
           move      w-ipc-cod-azi        to   rr-cod-min             .
      *              *-------------------------------------------------*
      *              * Codice max                                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/maucmf"
                                        using  j                      .
      *              *-------------------------------------------------*
      *              * Se la visualizzazione deve essere ristretta al- *
      *              * le sole aziende per cui l'utente risulta abili- *
      *              * tato                                            *
      *              *-------------------------------------------------*
           if        w-ipc-azi-ute        not  = "S"
                     go to qry-opn-fls-999.
      *              *-------------------------------------------------*
      *              * Estrazione informazioni generali da segreteria  *
      *              *-------------------------------------------------*
           move      "IG"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                        using  s                      .
           move      s-ute                to   w-ipc-cod-ute          .
       qry-opn-fls-100.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      "RD"                 to   j-ope                  .
           move      "UTE"                to   j-tre                  .
           move      w-ipc-cod-ute        to   j-kre                  .
           call      "swd/mod/prg/obj/maucmf"
                                        using  j                      .
           if        j-rsc                not  = spaces
                     move  spaces         to   w-ipc-cod-ute
           else      move  j-dat          to   w-ute                  .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Test se visualizzazione ristretta alle sole a-  *
      *              * ziende abilitate, ma utente mancante            *
      *              *-------------------------------------------------*
           if        w-ipc-azi-ute        =    "S" and
                     w-ipc-cod-ute        =    spaces
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun codice azienda memorizzato !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *                  *---------------------------------------------*
      *                  * Test se utente supervisore                  *
      *                  *---------------------------------------------*
           if        w-ute-tip-ute        =    02
                     go to qry-sel-rec-999.
      *                  *---------------------------------------------*
      *                  * Test se utente abilitato ad ogni azienda    *
      *                  *---------------------------------------------*
           if        w-ute-num-azi        =    zero
                     go to qry-sel-rec-999.
      *                  *---------------------------------------------*
      *                  * Controllo su codici azienda abilitati       *
      *                  *---------------------------------------------*
           move      zero                 to   w-ipc-ctr-001          .
       qry-sel-rec-100.
           add       1                    to   w-ipc-ctr-001          .
           if        w-ipc-ctr-001        >    20
                     go to qry-sel-rec-900.
           if        w-ipc-ctr-001        >    w-ute-num-azi
                     go to qry-sel-rec-900.
      ...
      *                  *---------------------------------------------*
           ...
           move      18                   to   v-pos                  .
           move      "+"                  to   v-edm                  .
           move      w-azi-cod-azi        to   w-mpn-cod-azi          .
           move      w-mpn                to   v-cnt                  .
      ...
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'auc-azi'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'azi-ute' per determinare se  *
      *    * la visualizzazione deve essere ristretta alle sole azien- *
      *    * de per cui l'utente risulta abilitato                     *
      *    *-----------------------------------------------------------*
       ipc-dlp-azu-000.
           move      "CV"                 to   s-ope                  .
           ...
           if        s-ves                =    spaces and
                     s-alf                =    "S"
                     move   "S"           to   w-ipc-azi-ute
           else      move   "N"           to   w-ipc-azi-ute          .
       ipc-dlp-azu-999.
           exit.

      *    *===========================================================*
      *    * Lettura variabile di i.p.c. 'cod-azi' per determinare i   *
      *    * caratteri iniziali del codice azienda                     *
      *    *-----------------------------------------------------------*
       ipc-dlp-cda-000.
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-ipc-cod-azi
           else      move  spaces         to   w-ipc-cod-azi          .
       ipc-dlp-cda-999.
           exit.
