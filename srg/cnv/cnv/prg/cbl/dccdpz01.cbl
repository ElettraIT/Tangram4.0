      ...
      *                                   Fase:    dccdpz              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 14/02/19    *
      *                       Ultima revisione:    NdK del 14/02/19    *
      *                    ------------------------------------------- *
      ...
      *                                                                *
      *                    Il programma riporta voci descrittive e     *
      *                    spese dall'anagrafica del cliente principa- *
      *                    le alle dipendenze                          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  srt-dat.
      *            *---------------------------------------------------*
      *            * Si/No addebito spese e modalita'                  *
      *            *---------------------------------------------------*
               10  srt-add-spe occurs 06.
      *                *-----------------------------------------------*
      *                * Si/No addebito spese e modalita'              *
      *                *                                               *
      *                * - 01 : No addebito                            *
      *                * - 02 : Si addebito, secondo le modalita' e-   *
      *                *        spresse nel modo di funzionamento per  *
      *                *        la spesa                               *
      *                * - 03 : Si addebito a percentuale, solo se il  *
      *                *        funzionamento della spesa e' tipo 05   *
      *                * - 04 : Si addebito ad importo, solo se il     *
      *                *        funzionamento della spesa e' tipo 05   *
      *                *                                               *
      *                * Per i clienti con fatturazione in valuta non  *
      *                * si tiene conto dei tipi funzionamento spesa   *
      *                * 02 e 04, che vengono rispettivamente conside- *
      *                * rati come se fossero 03 o 05                  *
      *                *-----------------------------------------------*
                   15  srt-snm-spe        pic  9(02)                  .
      *                *-----------------------------------------------*
      *                * Percentuale di addebito della spesa           *
      *                *-----------------------------------------------*
                   15  srt-per-spe        pic  9(02)v9(01)            .
      *                *-----------------------------------------------*
      *                * Importo di addebito della spesa, espresso in  *
      *                * valuta base                                   *
      *                *-----------------------------------------------*
                   15  srt-imp-spe        pic  9(09)                  .
      *            *---------------------------------------------------*
      *            * Voci descrittive                                  *
      *            *---------------------------------------------------*
               10  srt-vde-fat occurs 06.
                   15  srt-vde-cod        pic  x(03)                  .
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    Aggiornamento dipendenze clienti    "       .
      ...
      *            * Codice archivio                                   *
      *            *---------------------------------------------------*
               10  w-det-prg-dcc-arc      pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Progressivo                                       *
      *            *---------------------------------------------------*
               10  w-det-prg-dcc-prg      pic  9(05)                  .
      ...
      *              *-------------------------------------------------*
           move      zero                 to   w-det-prg-dcc-prg      .
       stp-srt-inp-dcc-100.
      *              *-------------------------------------------------*
      ...
      *                  * Dati per il sort                            *
      *                  *---------------------------------------------*
           move      rf-dcc-snm-spe (1)   to   srt-snm-spe (1)        .
           move      rf-dcc-snm-spe (2)   to   srt-snm-spe (2)        .
           move      rf-dcc-snm-spe (3)   to   srt-snm-spe (3)        .
           move      rf-dcc-snm-spe (4)   to   srt-snm-spe (4)        .
           move      rf-dcc-snm-spe (5)   to   srt-snm-spe (5)        .
           move      rf-dcc-snm-spe (6)   to   srt-snm-spe (6)        .
      *
           move      rf-dcc-per-spe (1)   to   srt-per-spe (1)        .
           move      rf-dcc-per-spe (2)   to   srt-per-spe (2)        .
           move      rf-dcc-per-spe (3)   to   srt-per-spe (3)        .
           move      rf-dcc-per-spe (4)   to   srt-per-spe (4)        .
           move      rf-dcc-per-spe (5)   to   srt-per-spe (5)        .
           move      rf-dcc-per-spe (6)   to   srt-per-spe (6)        .
      *
           move      rf-dcc-imp-spe (1)   to   srt-imp-spe (1)        .
           move      rf-dcc-imp-spe (2)   to   srt-imp-spe (2)        .
           move      rf-dcc-imp-spe (3)   to   srt-imp-spe (3)        .
           move      rf-dcc-imp-spe (4)   to   srt-imp-spe (4)        .
           move      rf-dcc-imp-spe (5)   to   srt-imp-spe (5)        .
           move      rf-dcc-imp-spe (6)   to   srt-imp-spe (6)        .
      *
           move      rf-dcc-vde-cod (1)   to   srt-vde-cod (1)        .
           move      rf-dcc-vde-cod (2)   to   srt-vde-cod (2)        .
           move      rf-dcc-vde-cod (3)   to   srt-vde-cod (3)        .
           move      rf-dcc-vde-cod (4)   to   srt-vde-cod (4)        .
           move      rf-dcc-vde-cod (5)   to   srt-vde-cod (5)        .
           move      rf-dcc-vde-cod (6)   to   srt-vde-cod (6)        .
       stp-srt-inp-dcc-700.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Max su [dcc], se non superato : ad uscita       *
      *              *-------------------------------------------------*
           if        rf-dcc-cod-cli       =    srt-cod-cli
                     go to prn-liv-det-400.
      *                  *---------------------------------------------*
      *                  * Se non superato                             *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Rilascio del record [dcc]               *
      *                      *-----------------------------------------*
           move      "RL"                 to   f-ope                  .
           move      "pgm/dcc/fls/ioc/obj/iofdcc"
                                          to   s-pat                  .
      ...
      *              * Aggiornamento dati                              *
      *              *-------------------------------------------------*
           move      srt-snm-spe (1)      to   rf-dcc-snm-spe (1)     .
           move      srt-snm-spe (2)      to   rf-dcc-snm-spe (2)     .
           move      srt-snm-spe (3)      to   rf-dcc-snm-spe (3)     .
           move      srt-snm-spe (4)      to   rf-dcc-snm-spe (4)     .
           move      srt-snm-spe (5)      to   rf-dcc-snm-spe (5)     .
           move      srt-snm-spe (6)      to   rf-dcc-snm-spe (6)     .
      *
           move      srt-per-spe (1)      to   rf-dcc-per-spe (1)     .
           move      srt-per-spe (2)      to   rf-dcc-per-spe (2)     .
           move      srt-per-spe (3)      to   rf-dcc-per-spe (3)     .
           move      srt-per-spe (4)      to   rf-dcc-per-spe (4)     .
           move      srt-per-spe (5)      to   rf-dcc-per-spe (5)     .
           move      srt-per-spe (6)      to   rf-dcc-per-spe (6)     .
      *
           move      srt-imp-spe (1)      to   rf-dcc-imp-spe (1)     .
           move      srt-imp-spe (2)      to   rf-dcc-imp-spe (2)     .
           move      srt-imp-spe (3)      to   rf-dcc-imp-spe (3)     .
           move      srt-imp-spe (4)      to   rf-dcc-imp-spe (4)     .
           move      srt-imp-spe (5)      to   rf-dcc-imp-spe (5)     .
           move      srt-imp-spe (6)      to   rf-dcc-imp-spe (6)     .
      *
           move      srt-vde-cod (1)      to   rf-dcc-vde-cod (1)     .
           move      srt-vde-cod (2)      to   rf-dcc-vde-cod (2)     .
           move      srt-vde-cod (3)      to   rf-dcc-vde-cod (3)     .
           move      srt-vde-cod (4)      to   rf-dcc-vde-cod (4)     .
           move      srt-vde-cod (5)      to   rf-dcc-vde-cod (5)     .
           move      srt-vde-cod (6)      to   rf-dcc-vde-cod (6)     .
       prn-liv-det-600.
      *              *-------------------------------------------------*
      *              * Update record [dcc]                             *
      *              *-------------------------------------------------*
