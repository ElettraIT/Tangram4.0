      ...
      *                                   Fase:    fdxhtm              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 19/01/21    *
      *                       Ultima revisione:    NdK del 20/06/22    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Generazione distinte 'html' da 'fdx' (2022) *
      *                                                                *
      *                    STANDARD                                    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   Creazione distinte 'html' da 'fdx'   "       .
      ...
      *        *-------------------------------------------------------*
           05  w-det-fil-htm.
               ...
               10  w-det-fil-htm-dmn      pic  9(09)                  .
               10  w-det-fil-htm-dmx      pic  9(09)                  .
      ...
      *                  * Anno                                        *
      *                  *---------------------------------------------*
           perform   acc-ann-exe-000      thru acc-ann-exe-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-ann-exe-999.
       acc-ann-exe-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-ann-exe             .
       acc-ann-exe-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Controllo su anno                           *
      *                  *---------------------------------------------*
           if        rr-ann-exe           not  = spaces
                     go to tdo-ric-sel-999.
           move      "ME"                 to   v-ope                  .
           move      "Manca l'anno !"     to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-fdx-000.
      *              *-------------------------------------------------*
      *              * Determinazione valore minimo per la start       *
      *              *-------------------------------------------------*
           move      "CV"                 to   v-ope                  .
           move      04                   to   v-car                  .
           move      rr-ann-exe           to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-num                to   w-det-fil-htm-ann      .
           subtract  1900                 from w-det-fil-htm-ann      .
           multiply  1000000              by   w-det-fil-htm-ann
                                        giving w-det-fil-htm-dmn      .
      *              *-------------------------------------------------*
      *              * Determinazione valore massimo per la start      *
      *              *-------------------------------------------------*
           move      w-det-fil-htm-dmn    to   w-det-fil-htm-dmx      .
           add       000999999            to   w-det-fil-htm-dmx      .
       stp-srt-inp-fdx-100.
      *              *-------------------------------------------------*
      *              * Start su archivio [fdx]                         *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "NUMDST    "         to   f-key                  .
           move      w-det-fil-htm-dmn    to   rf-fdx-num-dst         .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-fdx-num-dst       >    w-det-fil-htm-dmx
                     go to stp-srt-inp-fdx-900.
       stp-srt-inp-fdx-400.
      *              *-------------------------------------------------*
      *              * Selezioni sul record [fdx]                      *
      *              *-------------------------------------------------*
       stp-srt-inp-fdx-600.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-fdx-700.
      *                  *---------------------------------------------*
      *                  * Rilascio record 'carico' al sort            *
      *                  *---------------------------------------------*
           release   srt-rec                                          .
       stp-srt-inp-fdx-800.
      *                  *---------------------------------------------*
      *                  * Riciclo a record [fdx] successivo           *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      srt-num-dst          to   w-rot-l01-num-dst      .
       prn-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      06                   to   p-car                  .
           move      "<B"                 to   p-edm                  .
           move      w-rot-l01-num-dst    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
...
______*    perform   emi-htm-dst-ptn-000  thru emi-htm-dst-ptn-999    .
      ...
      *                  *---------------------------------------------*
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      w-det-rec-fil-rec    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Output                                      *
      ...
      *    *-----------------------------------------------------------*
       emi-htm-dst-lnk-000.
      *              *-------------------------------------------------*
      *              * Test su data registrazione distinta             *
      *              *-------------------------------------------------*
           if        rf-fdx-dat-reg       <    1220000
                     go to emi-htm-dst-lnk-100
           else      go to emi-htm-dst-lnk-500.
       emi-htm-dst-lnk-100.
      *              *-------------------------------------------------*
      *              * Pre 2022                                        *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-all-str-cat (6)    to   f-xxx-fln              .
      *                  *---------------------------------------------*
      *                  * Anno documento                              *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
