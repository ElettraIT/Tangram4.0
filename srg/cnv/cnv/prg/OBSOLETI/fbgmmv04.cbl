      ...
      *                                   Fase:    aggmmv              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 28/02/05    *
      *                       Ultima revisione:    NdK del 28/02/05    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Aggiornamento file [mmv]                    *
      *                                                                *
      *                    Update per sistemazione Schede LIFO (TUTTE) *
      *                                                                *
      *                    scaglione 1995 (IX) - FORALBERG             *
      *                                                                *
      *                    Rilettura saldo giacenza al 31/12/95        *
      *                                                                *
      *                    Richiesto da Mirco e Gianni (09/03/05)      *
      *                                                                *
      *================================================================*
      ...
      *                  * Chiave per il sort                          *
      *                  *---------------------------------------------*
           move      rf-mmv-ann-ese       to   srt-ann-ese            .
           move      rf-mmv-tip-mag       to   srt-tip-mag            .
           move      rf-mmv-num-mag       to   srt-num-mag            .
       stp-srt-inp-mmv-700.
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       agg-rec-mmv-000.
      *              *-------------------------------------------------*
      *              * Determinazione giacenza di proprieta' 31/12/95  *
      *              *-------------------------------------------------*
           move      "SL"                 to   d-sld-mag-tip-ope      .
           move      0100                 to   d-sld-mag-tip-sld      .
           move      0951231              to   d-sld-mag-dat-sld      .
           move      "T"                  to   d-sld-mag-uot-dpz      .
           move      01                   to   d-sld-mag-tip-mag      .
           move      srt-num-mag          to   d-sld-mag-num-mag      .
      ...
      *              *-------------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "ESECOD    "         to   f-key                  .
           move      srt-ann-ese          to   rf-mmv-ann-ese         .
           move      srt-tip-mag          to   rf-mmv-tip-mag         .
           move      srt-num-mag          to   rf-mmv-num-mag         .
           move      "pgm/mag/fls/ioc/obj/iofmmv"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
       ...
       agg-rec-mmv-600.
      *              *-------------------------------------------------*
      *              * Test sul valore calcolato                       *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Determinazione indice scaglione 1995        *
      *                  *---------------------------------------------*
           move      zero                 to   w-wrk-inx-001          .
           if        srt-ann-ese          <    096
                     go to agg-rec-mmv-800.
           if        srt-ann-ese          =    104
                     move  9              to   w-wrk-inx-001
           else if   srt-ann-ese          =    103
                     move  8              to   w-wrk-inx-001
           else if   srt-ann-ese          =    102
                     move  7              to   w-wrk-inx-001
           else if   srt-ann-ese          =    101
                     move  6              to   w-wrk-inx-001
           else if   srt-ann-ese          =    100
                     move  5              to   w-wrk-inx-001
           else if   srt-ann-ese          =    099
                     move  4              to   w-wrk-inx-001
           else if   srt-ann-ese          =    098
                     move  3              to   w-wrk-inx-001
           else if   srt-ann-ese          =    097
                     move  2              to   w-wrk-inx-001
           else if   srt-ann-ese          =    096
                     move  1              to   w-wrk-inx-001          .
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        w-wrk-inx-001        =    zero
                     go to agg-rec-mmv-800.
           if        w-wrk-inx-001        >    10
                     go to agg-rec-mmv-800.
           if        d-sld-mag-sld-mag    =    rf-mmv-rim-pre
                                              (w-wrk-inx-001)
                     go to agg-rec-mmv-800.
           if        d-sld-mag-sld-mag    not  < zero and
                     rf-mmv-ump-pre
                    (w-wrk-inx-001)       >    zero
      ...
      *    *===========================================================*
      *    * Controlli su numero di utenza                             *
      *    *-----------------------------------------------------------*
       ctl-utz-num-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione flag di controllo               *
      *              *-------------------------------------------------*
           move      spaces               to   w-ctl-utz-num-flg      .
       ctl-utz-num-010.
      *              *-------------------------------------------------*
      *              * Se a spazi : uscita                             *
      *              *-------------------------------------------------*
           if        w-ctl-utz-num-num    =    spaces
                     go to ctl-utz-num-900.
       ctl-utz-num-100.
      *              *-------------------------------------------------*
      *              * Sostituzione eventuali separatori con spazi     *
      *              *-------------------------------------------------*
           inspect   w-ctl-utz-num-num
                                     replacing all "-" 
                                               by  " "                .
      *
           inspect   w-ctl-utz-num-num
      ...
      *
       ...
       ctl-utz-num-200.
      *              *-------------------------------------------------*
      *              * Test che il prefisso non superi i 5 caratteri   *
      *              *-------------------------------------------------*
           move      w-ctl-utz-num-num    to   w-ctl-utz-num-str      .
           move      zero                 to   w-ctl-utz-num-ctr      .
       ctl-utz-num-210.
           add       01                   to   w-ctl-utz-num-ctr      .
           if        w-ctl-utz-num-ctr    >    20
                     go to ctl-utz-num-220.
           if        w-ctl-utz-num-chr
                    (w-ctl-utz-num-ctr)   not  = " "
                     go to ctl-utz-num-210.
       ctl-utz-num-220.
           if        w-ctl-utz-num-ctr    not  > 06
                     go to ctl-utz-num-300.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       ctl-utz-num-300.
      *              *-------------------------------------------------*
      *              * Estrazione del prefisso e del numero Fax        *
      *              *-------------------------------------------------*
           move      zero                 to   w-ctl-utz-num-ctr      .
           move      spaces               to   w-ctl-utz-num-pre      .
           unstring  w-ctl-utz-num-num
                                delimited by " "
                                          into w-ctl-utz-num-pre
                                    count in   w-ctl-utz-num-ctr      .
           add       2                    to   w-ctl-utz-num-ctr      .
           move      spaces               to   w-ctl-utz-num-utz      .
           unstring  w-ctl-utz-num-num    into w-ctl-utz-num-utz
                                  with pointer w-ctl-utz-num-ctr      .
       ctl-utz-num-400.
      *              *-------------------------------------------------*
      *              * Controlli su prefisso                           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Che esista                                  *
      *                  *---------------------------------------------*
           if        w-ctl-utz-num-pre    not  = spaces
                     go to ctl-utz-num-420.
      *                  *---------------------------------------------*
      *                  * Manca il prefisso telefonico                *
      *                  *---------------------------------------------*
           go to     ctl-utz-num-900.
       ctl-utz-num-420.
      *                  *---------------------------------------------*
      *                  * Che non sia '049'                           *
      *                  *---------------------------------------------*
           if        w-ctl-utz-num-pre    not  = "049  "
                     go to ctl-utz-num-440.
      *                  *---------------------------------------------*
      *                  * Prefisso cittadino                          *
      *                  *---------------------------------------------*
           go to     ctl-utz-num-900.
      ...
      *                  * Che non sia estero                          *
      *                  *---------------------------------------------*
           if        w-ctl-utz-num-pre
                    (1 : 2)               not  = "00"
                     go to ctl-utz-num-500.
      *                  *---------------------------------------------*
      *                  * Prefisso estero                             *
      *                  *---------------------------------------------*
           go to     ctl-utz-num-900.
      ...
      *              *-------------------------------------------------*
           ...
           move      "1055"               to   w-all-str-cat (1)      .
           move      w-ctl-utz-num-pre    to   w-all-str-cat (2)      .
           move      w-ctl-utz-num-utz    to   w-all-str-cat (3)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *
           move      w-all-str-alf        to   w-ctl-utz-num-num      .
       ctl-utz-num-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     ctl-utz-num-999.
       ctl-utz-num-999.
           exit.
