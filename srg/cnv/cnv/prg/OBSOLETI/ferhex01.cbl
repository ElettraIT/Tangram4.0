      ...
      *                                   Fase:    ferhex              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 12/04/02    *
      *                       Ultima revisione:    NdK del 12/04/02    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Sistemazione [hex]                          *
      *                                                                *
      *                    FERRAMENTA ITALIANA                         *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
               10  srt-ide-rec            pic  x(04)                  .
      *            *---------------------------------------------------*
      *            * Chiave alfanumerica del record                    *
      *            *---------------------------------------------------*
               10  srt-kal-rec            pic  x(40)                  .
      *        *-------------------------------------------------------*
      *        * Dati                                                  *
      *        *-------------------------------------------------------*
           05  srt-dat.
      ...
      *              *-------------------------------------------------*
           move      "OP"                 to   f-ope                  .
           move      "fer/gpa/fls/ioc/obj/iofhex"
                                          to   s-pat                  .
      ...
      *              * Ciclo per [hex]                                 *
      *              *-------------------------------------------------*
           perform   stp-srt-inp-hex-000  thru stp-srt-inp-hex-999    .
       stp-srt-inp-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-hex-000.
      *              *-------------------------------------------------*
      *              * Start su archivio [hex]                         *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "IDEREC    "         to   f-key                  .
           move      "haf "               to   rf-hex-ide-rec         .
           move      spaces               to   rf-hex-kal-rec         .
           move      "fer/gpa/fls/ioc/obj/iofhex"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-srt-inp-hex-900.
       stp-srt-inp-hex-200.
      *              *-------------------------------------------------*
      *              * Read-next su archivio [hex]                     *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "fer/gpa/fls/ioc/obj/iofhex"
                                          to   s-pat                  .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-hex-ide-rec       not  = "haf "
                     go to stp-srt-inp-hex-900.
       stp-srt-inp-hex-400.
      *              *-------------------------------------------------*
      *              * Selezioni sul record [hex]                      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Normalizzazione record [haf]                *
      *                  *---------------------------------------------*
           move      "NO"                 to   f-ope                  .
      ...
      *                  * Record [haf]                                *
      *                  *---------------------------------------------*
           move      rf-hex-dat-rec       to   rf-haf                 .
      *                  *---------------------------------------------*
      *                  * Test su codice listino                      *
      *                  *---------------------------------------------*
           if        rf-haf-fda-pif       not  = spaces
                     go to stp-srt-inp-hex-200.
      *                  *---------------------------------------------*
      *                  * Test su ultima modifica                     *
      *                  *---------------------------------------------*
           if        rf-haf-ide-dat       <    1020408
                     go to stp-srt-inp-hex-200.
       stp-srt-inp-hex-600.
      *              *-------------------------------------------------*
      ...
      *                  * Chiave per il sort                          *
      *                  *---------------------------------------------*
           move      rf-hex-ide-rec       to   srt-ide-rec            .
           move      rf-hex-kal-rec       to   srt-kal-rec            .
      *                  *---------------------------------------------*
      *                  * Dati per il sort                            *
      *                  *---------------------------------------------*
           move      rf-hex               to   srt-rec-hex            .
       stp-srt-inp-hex-700.
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           perform   agg-rec-hex-000      thru agg-rec-hex-999        .
       prn-liv-det-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "IDEREC    "         to   f-key                  .
           move      srt-ide-rec          to   rf-hex-ide-rec         .
           move      srt-kal-rec          to   rf-hex-kal-rec         .
           move      "fer/gpa/fls/ioc/obj/iofhex"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-hex                 .
           if        f-sts                not  = e-not-err
                     go to agg-rec-hex-999.
       agg-rec-hex-200.
      *              *-------------------------------------------------*
      *              * Aggiornamento record [hex]                      *
      *              *-------------------------------------------------*
           move      all spaces           to   rf-hex-dat-rec         .
       agg-rec-hex-400.
      *              *-------------------------------------------------*
      *              * Update record                                   *
      *              *-------------------------------------------------*
