      ...
      *                                   Fase:    zttadc              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 16/04/13    *
      *                       Ultima revisione:    NdK del 06/06/13    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Email sito WEB per ZATTI da file            *
      *                    'utenti.txt' in /abd/asc/tmp/               *
      *                                                                *
      *                    Upgrade per anagrafiche 'adc'               *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "     Email clienti ZATTI da sito WEB    "       .
      ...
      *        *-------------------------------------------------------*
      *        * Numero di records scritti [adc]                       *
      *        *-------------------------------------------------------*
           05  w-pds-adc                  pic  9(06)                  .
      *        *-------------------------------------------------------*
      ...
      *            * Codice cliente                                    *
      *            *---------------------------------------------------*
               10  w-lds-001-cod-cli      pic  x(08)                  .
      *            *---------------------------------------------------*
      *            * User ID                                           *
      *            *---------------------------------------------------*
               10  w-lds-001-usr-idx      pic  x(08)                  .
      *            *---------------------------------------------------*
      *            * Mail                                              *
      *            *---------------------------------------------------*
               10  w-lds-001-ema-ilx      pic  x(57)                  .
      *            *---------------------------------------------------*
      *            * Data                                              *
      *            *---------------------------------------------------*
               10  w-lds-001-dat-reg      pic  x(10)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-enc-adc-val-pre          pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * Valore post incremento                                *
      *        *-------------------------------------------------------*
           05  w-enc-adc-val-pos          pic  9(07)                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      20                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Numero records scritti [adc]   :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       xcv-opn-fls-100.
      ...
      *    *-----------------------------------------------------------*
       xcv-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessuna anagrafica da elaborare !"
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       xcv-nes-ela-999.
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Conversione codice cliente                  *
      *                  *---------------------------------------------*
           move      "CV"                 to   v-ope                  .
           move      08                   to   v-car                  .
           move      w-lds-001-cod-cli    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-num                to   w-det-cod-cli-cod      .
      *                  *---------------------------------------------*
      *                  * Conversione data registrazione              *
      *                  *---------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Indirizzo mail in lowercase                 *
      *                  *---------------------------------------------*
           move      w-lds-001-ema-ilx    to   w-all-str-alf          .
           move      57                   to   w-all-str-lun          .
           perform   all-str-low-000      thru all-str-low-999        .
           move      w-all-str-alf        to   w-lds-001-ema-ilx      .
       xcv-trt-pds-400.
      *                  *---------------------------------------------*
      *                  * Aggiornamento del record [adc]              *
      *                  *---------------------------------------------*
           perform   wrt-rec-adc-000      thru wrt-rec-adc-999        .
      *                  *---------------------------------------------*
      *                  * Incremento numero records scritti [adc]     *
      *                  *---------------------------------------------*
           add       1                    to   w-pds-adc              .
      *                  *---------------------------------------------*
      ...
      *              * esiste                                          *
      *              *-------------------------------------------------*
           move      "RC"                 to   d-con-arc-tip-ope      .
           move      02                   to   d-con-arc-tip-arc      .
           move      w-det-cod-cli-cod    to   d-con-arc-cod-arc      .
           move      spaces               to   d-con-arc-dpz-arc      .
           move      "EML"                to   d-con-arc-tip-sel      .
           move      w-lds-001-ema-ilx    to   d-con-arc-num-con (1)  .
           move      "pgm/azi/prg/obj/dconarc0"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        d-con-arc-num-ele    =    zero
                     go to wrt-rec-adc-500.
      *                  *---------------------------------------------*
      *                  * Test su interlocutore                       *
      *                  *---------------------------------------------*
           if        d-con-arc-int-con
                    (1)
                    (01 : 01)             =    "<"
                     go to wrt-rec-adc-500.
       wrt-rec-adc-200.
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Normalizzazione area record [adc]           *
      *                  *---------------------------------------------*
           move      "NO"                 to   f-ope                  .
           ...
           call      s-pat               using f
                                               rf-adc                 .
      *                  *---------------------------------------------*
      *                  * Lettura con lock [adc]                      *
      *                  *---------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "ARCCON    "         to   f-key                  .
           move      02                   to   rf-adc-tip-arc         .
           move      w-det-cod-cli-cod    to   rf-adc-cod-arc         .
      ...
      *                  *---------------------------------------------*
           ...
           move      "<web="              to   w-all-str-cat (1)      .
           move      w-lds-001-dat-reg    to   w-all-str-cat (2)      .
           move      ">"                  to   w-all-str-cat (3)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
           move      w-all-str-alf        to   rf-adc-int-con         .
       wrt-rec-adc-300.
      *                  *---------------------------------------------*
      *                  * Aggiornamento record file [adc]             *
      *                  *---------------------------------------------*
           move      "UP"                 to   f-ope                  .
           move      "pgm/azi/fls/ioc/obj/iofadc"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Ricerca del primo progressivo disponibile   *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-lds-001-ema-ilx    to   rf-adc-num-con         .
