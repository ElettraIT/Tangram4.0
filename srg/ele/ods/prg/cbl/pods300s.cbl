      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Inizializzazione flag di primo giro             *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Numero ordine per stampa barcode            *
      *                  *                                             *
      *                  * ELETTRA                                     *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
      ...
      *                      * Test se da stampare                     *
      *                      *-----------------------------------------*
           if        rf-ost-spe-snx (1)   not  = 1
                     go to stp-tes-doc-250.
           if        rf-ost-spe-imp (1)   =    zero
                     go to stp-tes-doc-250.
      *                      *-----------------------------------------*
      *                      * Prompt                                  *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      16                   to   p-lin                  .
           move      023                  to   p-pos                  .
           move      rf-ost-spe-imp (1)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-tes-doc-250.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       stp-tes-doc-800.
      *              *-------------------------------------------------*
      *              * Stampa dati precablati da 'ods450'              *
      *              *                                                 *
      *              * ELETTRA                                         *
      *              *-------------------------------------------------*
           perform   stp-pie-doc-hsx-000  thru stp-pie-doc-hsx-999    .
       stp-tes-doc-850.
      *              *-------------------------------------------------*
      ...
      *    *                                                           *
      *    * Modifica del 23.11.17 Mario Bellini ______________________*
      *    * Se ods a cliente si vanno a leggere e memorizzare le con- *
      *    * dizioni di vendita del [dcc]                              *
      *    *-----------------------------------------------------------*
       stp-tes-doc-rda-000.
      *              *-------------------------------------------------*
      ...
      *                      * tivo : a raccolta dati normale          *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-tes-doc-rda-080.
      *                  *---------------------------------------------*
      *                  * Se codice condizioni di vendita a zero: ol- *
      *                  * tre                                         *
      *                  *---------------------------------------------*
           if        rf-dcc-cod-cdv       =    zero
                     go to stp-tes-doc-rda-080.
      *                      *-----------------------------------------*
      *                      * Lettura record [zcv]                    *
      *                      *-----------------------------------------*
           ...
           move      rf-dcc-cod-cdv       to   rf-zcv-cod-cdv         .
           move      "pgm/orc/fls/ioc/obj/iofzcv"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-zcv                 .
      *                      *-----------------------------------------*
      *                      * Se lettura record [zcv] errata: oltre   *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-tes-doc-rda-080.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      rf-zcv-des-cdv       to   w-stp-tes-aps          .
      *                  *---------------------------------------------*
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-cor-doc-420.
      *                  *---------------------------------------------*
      *                  * Lettura ordine cliente per riferimenti      *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se prima riga da stampare          *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           call      s-pat               using f
                                               rf-oct                 .
      *                  *---------------------------------------------*
      *                  * Stampa riferimenti cliente (matricola)      *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      40                   to   p-car                  .
      ...
      *                      *                                         *
      *                      * ATTUALMENTE INIBITA (Giorgio 23/01/24)  *
      *                      *-----------------------------------------*
           go to     stp-cor-doc-650.
      ...
      *                      *-----------------------------------------*
      *                      * Determinazione numero di parametri      *
      *                      *-----------------------------------------*
           move      d-prm-ubi-ubi-lit    to   w-all-str-alf          .
           move      " "                  to   w-all-str-del          .
           perform   all-str-ext-000      thru all-str-ext-999        .
           if        w-all-str-num        <    4
                     go to stp-cor-doc-775.
      *                      *-----------------------------------------*
      *                      * Riassemblaggio parametri                *
      *                      *-----------------------------------------*
           move      28                   to   w-all-str-lun          .
           move      03                   to   w-all-str-num          .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                      *-----------------------------------------*
      *                      * Parametri di ubicazione riassemblati    *
      *                      *                                         *
      *                      * Attualmente solo 3 parametri            *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      18                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
       ...
       stp-cor-doc-800.
      *                  *---------------------------------------------*
      *                  * Eventuale stampa della quantita' cumulata   *
      *                  * sovrascrivendo le ubicazioni                *
      *                  *                                             *
      *                  * ATTUALMENTE INIBITA (Giorgio 07/08/23)      *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-820.
______*
           if        rf-osr-qta-ven       =    w-rig-dti-qcr           
                                              (w-rig-ctr-ele)     
                     go to stp-cor-doc-820.
           if        w-rig-dti-qcr           
                    (w-rig-ctr-ele)       =    zero
      ...
      *                      * Si/no carpenteria                       *
      *                      *-----------------------------------------*
           move      spaces               to   w-rig-key-crp-13
                                              (w-rig-num-ele)         .
      *                      *-----------------------------------------*
      *                      * Indice di percorso                      *
      *                      *-----------------------------------------*
           move      9999999              to   w-rig-key-inx-13
                                              (w-rig-num-ele)         .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione codice prodotto         *
      *                      *-----------------------------------------*
      ...
      *                      * Si/no carpenteria                       *
      *                      *-----------------------------------------*
           if        rf-dcp-cla-pro       =    00400
                     move  spaces         to   w-rig-key-crp-13
                                              (w-rig-num-ele)
           else      move  "X"            to   w-rig-key-crp-13
                                              (w-rig-num-ele)         .
      *                      *-----------------------------------------*
      *                      * Determinazione ubicazione               *
      *                      *-----------------------------------------*
      ...
      *                      * Indice di percorso                      *
      *                      *-----------------------------------------*
           move      d-prm-ubi-ubi-inx    to   w-rig-key-inx-13
                                              (w-rig-num-ele)         .
           if        w-rig-key-inx-13
                    (w-rig-num-ele)       =    zero
                     move  9999999        to   w-rig-key-inx-13
      ...
      *                  *---------------------------------------------*
       ...
       stp-cor-doc-rdd-700.
      *              *-------------------------------------------------*
      *              * Stampa eventuale riga aggiuntiva per la IV      *
      *              * ubicazione                                      *
      *              *                                                 *
      *              * ATTUALMENTE INIBITA (Giorgio 23/01/24)          *
      *              *-------------------------------------------------*
           go to     stp-cor-doc-rdd-800.
      *                  *---------------------------------------------*
      *                  * Determinazione lunghezza stringa            *
      *                  *---------------------------------------------*
           move      d-prm-ubi-ubi-lit    to   w-all-str-alf          .
           perform   all-str-lun-000      thru all-str-lun-999        .
           if        w-all-str-lun        not  > 19
                     go to stp-cor-doc-rdd-800.
      *                  *---------------------------------------------*
      *                  * Determinazione numero di parametri          *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           add       w-stp-rig-ctr-rcs    to   p-lin                  .
           move      113                  to   p-pos                  .
           move      w-all-str-cat (4)    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-cor-doc-rdd-800.
      *              *-------------------------------------------------*
      *              * Stampa eventuale riga aggiuntiva per la quanti- *
      *              * ta' cumulata                                    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * ATTUALMENTE A LIVELLO CODICE PRODOTTO       *
      *                  * ___                                         *
      *                  *---------------------------------------------*
      ...
      *    * Routine di stampa piede documento                         *
      *    *                                                           *
      *    * Subroutine di stampa elementi precablati                  *
      *    *                                                           *
      *    * ELETTRA                                                   *
      *    *-----------------------------------------------------------*
       stp-pie-doc-hsx-000.
      ...
      *
       ...
       stp-pie-doc-hsx-250.
      *              *-------------------------------------------------*
      *              * Prompt per Tipo bolla prevista                  *
      *              *-------------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      21                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
           if        rf-hsx-tra-cur       not  = 30
                     go to stp-pie-doc-hsx-400.
           if        rf-hsx-cod-vet       =    zero
                     go to stp-pie-doc-hsx-400.
______*    if        rf-hsx-cod-vet       =    rf-ost-cod-vet
______*              go to stp-pie-doc-hsx-400.
      *                  *---------------------------------------------*
      *                  * Prompt                                      *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
      ...
      *                  * Test se da stampare                         *
      *                  *---------------------------------------------*
           if        rf-hsx-voc-des (1)   =    spaces
                     go to stp-pie-doc-hsx-500.
______*    if        rf-hsx-voc-des (1)   =    rf-ost-voc-des (1)
______*              go to stp-pie-doc-hsx-500.
      *                  *---------------------------------------------*
      *                  * Prompt                                      *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      69                   to   p-lin                  .
           move      032                  to   p-pos                  .
           move      rf-hsx-voc-des (1)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Valore                                      *
      ...
      *                  * Test se da stampare                         *
      *                  *---------------------------------------------*
           if        rf-hsx-voc-des (2)   =    spaces
                     go to stp-pie-doc-hsx-600.
______*    if        rf-hsx-voc-des (2)   =    rf-ost-voc-des (2)
______*              go to stp-pie-doc-hsx-600.
      *                  *---------------------------------------------*
      *                  * Prompt                                      *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      70                   to   p-lin                  .
           move      032                  to   p-pos                  .
           move      rf-hsx-voc-des (2)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Valore                                      *
      ...
      *                  * Test se da stampare                         *
      *                  *---------------------------------------------*
           if        rf-hsx-spe-spd       =    zero
                     go to stp-pie-doc-hsx-700.
      *                  *---------------------------------------------*
      *                  * Prompt                                      *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      67                   to   p-lin                  .
           move      089                  to   p-pos                  .
           move      rf-hsx-ann-rig (1)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Test se da stampare                         *
      ...
      *                  *---------------------------------------------*
           ...
           add       01                   to   p-lin                  .
           move      089                  to   p-pos                  .
           move      rf-hsx-ann-rig (2)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Test se da stampare                         *
      ...
      *                  *---------------------------------------------*
           ...
           add       02                   to   p-lin                  .
           move      089                  to   p-pos                  .
           move      rf-hsx-ann-rig (3)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Test se da stampare                         *
      ...
      *                  *---------------------------------------------*
           ...
           add       03                   to   p-lin                  .
           move      089                  to   p-pos                  .
           move      rf-hsx-ann-rig (4)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-pie-doc-hsx-900.
      *              *-------------------------------------------------*
      ...
      *                          *-------------------------------------*
           perform   box-int-pag-000      thru box-int-pag-999        .
      *                      *-----------------------------------------*
      *                      * Box per Codice prodotto x Produttore    *
      *                      *-----------------------------------------*
           move      025                  to   w-box-int-pag-lin      .
           move      057                  to   w-box-int-pag-pin      .
           move      027                  to   w-box-int-pag-lfi      .
           move      073                  to   w-box-int-pag-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      025                  to   w-box-int-pag-lin      .
           move      073                  to   w-box-int-pag-pin      .
           move      027                  to   w-box-int-pag-lfi      .
           move      082                  to   w-box-int-pag-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      025                  to   w-box-int-pag-lin      .
           move      082                  to   w-box-int-pag-pin      .
           move      027                  to   w-box-int-pag-lfi      .
           move      092                  to   w-box-int-pag-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      025                  to   w-box-int-pag-lin      .
           move      092                  to   w-box-int-pag-pin      .
           move      027                  to   w-box-int-pag-lfi      .
           move      103                  to   w-box-int-pag-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      025                  to   w-box-int-pag-lin      .
           move      103                  to   w-box-int-pag-pin      .
           move      027                  to   w-box-int-pag-lfi      .
           move      112                  to   w-box-int-pag-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      * Box per firma 2                         *
      *                      *-----------------------------------------*
______*    move      066                  to   w-box-int-pag-lin      .
______*    move      045                  to   w-box-int-pag-pin      .
______*    move      071                  to   w-box-int-pag-lfi      .
______*    move      088                  to   w-box-int-pag-pfi      .
______*    perform   box-int-pag-000      thru box-int-pag-999        .
      *                      *-----------------------------------------*
      *                      * Box per firma 3                         *
      *                      *-----------------------------------------*
______*    move      066                  to   w-box-int-pag-lin      .
______*    move      088                  to   w-box-int-pag-pin      .
______*    move      071                  to   w-box-int-pag-lfi      .
______*    move      132                  to   w-box-int-pag-pfi      .
______*    perform   box-int-pag-000      thru box-int-pag-999        .
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      "Codice Produtt."    to   w-rou-stp-pmt-alf      .
           move      15                   to   w-rou-stp-pmt-car      .
           move      26                   to   w-rou-stp-pmt-lin      .
           move      058                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       det-des-rig-000.
      *              *-------------------------------------------------*
      *              * Lettura preliminare estensione in file [osx]    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Normalizzazione record [osx]                *
      *                  *---------------------------------------------*
