      ...
      *                       Ultima revisione:    NdK del 18/02/20    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione per il programma psvf2601:       *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per cliente - classe statistica prodotto.   *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "FATTURATO PER CLIENTE - CLASSE PRODOTTO "       .
      ...
      *            *---------------------------------------------------*
      *            * Numero classi trattati per il cliente             *
      *            *---------------------------------------------------*
               10  w-liv-cli-num-csp      pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Totale generale                                       *
      *        *-------------------------------------------------------*
           05  w-liv-gen.
      ...
      *        *-------------------------------------------------------*
      *        * Contatore di comodo per selezione clienti             *
      *        *-------------------------------------------------------*
           05  w-sel-fit-ctr-cli          pic  9(03)                  .
      *        *-------------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se un solo codice cliente da selezionare e  *
      *                  * il codice cliente non corrisponde a quello  *
      *                  * da selezionare : uscita per selezione non   *
      *                  * superata                                    *
      *                  *---------------------------------------------*
           if        rr-cod-cli-sns       =    zero
                     go to sel-fit-rec-132
           else      go to sel-fit-rec-134.
       sel-fit-rec-132.
           if        rr-cod-cli           not  = zero   and
                     rf-fit-cod-cli       not  = rr-cod-cli
                     go to sel-fit-rec-900
           else      go to sel-fit-rec-200.
       sel-fit-rec-134.
           move      zero                 to   w-sel-fit-ctr-cli      .
       sel-fit-rec-136.
           add       1                    to   w-sel-fit-ctr-cli      .
           if        w-sel-fit-ctr-cli    >    36
                     go to sel-fit-rec-900.
           if        rr-cod-cli-eco
                    (w-sel-fit-ctr-cli)   =    zero
                     go to sel-fit-rec-900.
           if        rr-cod-cli-eco
                    (w-sel-fit-ctr-cli)   =    rf-fit-cod-cli
                     go to sel-fit-rec-200
           else      go to sel-fit-rec-136.
       sel-fit-rec-200.
      *              *-------------------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 01 and
                     ...
                     rr-tor-cli           not  = 04 and
                     rr-tor-cli           not  = 05
                     move  spaces         to   srt-k01-rag-cli
                     go to cmp-srt-rec-225.
      *                          *-------------------------------------*
      *                          * Memorizzazione                      *
      *                          *-------------------------------------*
           move      w-sel-fit-rag-cli    to   srt-k01-rag-cli        .
       cmp-srt-rec-225.
      *                      *-----------------------------------------*
      *                      * Mnemonico cliente                       *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 07
                     move  spaces         to   srt-k01-mne-cli
                     go to cmp-srt-rec-230.
      *                          *-------------------------------------*
      *                          * Memorizzazione                      *
      *                          *-------------------------------------*
           move      w-sel-fit-mne-cli    to   srt-k01-mne-cli        .
       cmp-srt-rec-230.
      *                      *-----------------------------------------*
      *                      * Flag di codice a zero o diverso da zero *
      *                      *-----------------------------------------*
      ...
      *                      * Codice cliente                          *
      *                      *-----------------------------------------*
           move      w-sel-fit-cod-cli    to   srt-k01-cod-cli        .
       cmp-srt-rec-240.
      *                      *-----------------------------------------*
      *                      * Codice dipendenza del cliente           *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-prs-svf-cli-rgd    =    "S"
                     move  spaces         to   srt-k01-dpz-cli
      ...
      *                      * prodotto nell'ambito del cliente        *
      *                      *-----------------------------------------*
      ...
      *                  * Codice cliente                              *
      *                  *---------------------------------------------*
           move      srt-cod-cli-cod      to   w-rot-l02-cod-cli      .
      *                  *---------------------------------------------*
      *                  * Codice dipendenza del cliente               *
      *                  *---------------------------------------------*
           if        w-prs-svf-cli-rgd    =    "S"
                     move  spaces         to   w-rot-l02-dpz-cli
           else      move  srt-cod-cli-dpz
                                          to   w-rot-l02-dpz-cli      .
       prn-cmp-rot-999.
           exit.
      ...
      *                  * Numero classi trattati per il cliente       *
      *                  *---------------------------------------------*
           move      zero                 to   w-liv-cli-num-csp      .
      *                  *---------------------------------------------*
      *                  * Numero di clienti trattati in totale        *
      *                  *---------------------------------------------*
           add       1                    to   w-liv-gen-num-cli      .
       prn-ini-lr2-200.
      *              *-------------------------------------------------*
      *              * Deviazione a seconda se le personalizzazioni    *
      *              * indicano un salto pagina oppure no ad inizio    *
      ...
      *                  *---------------------------------------------*
      *                  * Numero classi trattate per il cliente       *
      *                  *---------------------------------------------*
           add       1                    to   w-liv-cli-num-csp      .
       prn-ini-lr1-200.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-031.
           move      "PER CLIENTE - 1. CODICE STATISTICO PRODOTTO       
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-032.
           move      "PER CLIENTE - 2. CODICE STATISTICO PRODOTTO       
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-033.
           move      "PER CLIENTE - 3. CODICE STATISTICO PRODOTTO       
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-034.
           move      "PER CLIENTE - CLASSE MERCEOLOGICA PRODOTTO        
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-050.
