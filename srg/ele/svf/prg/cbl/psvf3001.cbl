      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione per il programma psvf3001:       *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per classificazione statistica cliente.     *
      *                                                                *
      *                    SU MISURA PER ELETTRA - 'Inverter'          *
      *                                                                *
      *================================================================*
      ...
      *                *-----------------------------------------------*
      *                * Codice classificatore statistico clienti      *
      *                *-----------------------------------------------*
      ...
      *            *---------------------------------------------------*
               10  w-liv-csc-cst-max      pic  9(02)     value 36     .
               10  w-liv-csc-cst-ctr      pic  9(02)                  .
               10  w-liv-csc-cst-ct1      pic  9(02)                  .
               10  w-liv-csc-cst-ct2      pic  9(02)                  .
               10  w-liv-csc-cst-ct3      pic  9(02)                  .
               10  w-liv-csc-cst-cst occurs 36.
      *                *-----------------------------------------------*
      *                * Descrizione classificatore                    *
      *                *-----------------------------------------------*
                   15  w-liv-csc-cst-des  pic  x(40)                  .
      *                *-----------------------------------------------*
      *                * Totale fatturato                              *
      *                *-----------------------------------------------*
                   15  w-liv-csc-cst-val  pic s9(13)                  .
      *        *-------------------------------------------------------*
      *        * Totale generale                                       *
      *        *-------------------------------------------------------*
           05  w-liv-gen.
      ...
      *        *-------------------------------------------------------*
      *        * Sub-work per calcolo prodotti 'inverter'              *
      *        *                                                       *
      *        * ELETTRA                                               *
      *        *-------------------------------------------------------*
           05  w-stp-inv-inv.
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' cliente periodo 1    *
      *            *---------------------------------------------------*
               10  w-stp-inv-cli-p01      pic s9(13)                  .
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' categoria periodo 1  *
      *            *---------------------------------------------------*
               10  w-stp-inv-cat-p01      pic s9(13)                  .
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' totale periodo 1     *
      *            *---------------------------------------------------*
               10  w-stp-inv-tot-p01      pic s9(13)                  .
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' cliente periodo 2    *
      *            *---------------------------------------------------*
               10  w-stp-inv-cli-p02      pic s9(13)                  .
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' categoria periodo 2  *
      *            *---------------------------------------------------*
               10  w-stp-inv-cat-p02      pic s9(13)                  .
      *            *---------------------------------------------------*
      *            * Comodo per cumulo 'inverter' totale periodo 2     *
      *            *---------------------------------------------------*
               10  w-stp-inv-tot-p02      pic s9(13)                  .
      ...
      *                          *-------------------------------------*
           if        w-prs-svf-cli-rgd    =    "S"
                     move  w-sel-fit-dpz-cli
                                          to   srt-k02-dpz-cli
           else      move  spaces         to   srt-k02-dpz-cli        .
       cmp-srt-rec-400.
      *                  *---------------------------------------------*
      *                  * Subchiave 3 : Per documento                 *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        w-prs-svf-cli-rgd    =    "S"
                     move  srt-cod-cli-dpz
                                          to   kk0-dpz-cli
           else      move  spaces         to   kk0-dpz-cli            .
           read      kk0    invalid key
      ...
      *                  *---------------------------------------------*
           ...
           else      move  spaces         to   w-rot-l01-dpz-cli      .
      *              *-------------------------------------------------*
      *              * 2. livello di rottura                           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Normalizzazione comodi per cumulo 'inverter'    *
      *              *            				               *
      *              * ELETTRA					        *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-inv-tot-p01      .      
           move      zero                 to   w-stp-inv-tot-p02      .      
       prn-ini-cic-050.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      zero                 to   w-liv-csc-cst-ctr      .
       prn-ini-cic-200.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-fin-cic-310.
      *                  *---------------------------------------------*
      *                  * Test se 'inverter' da stampare              *
      *                  *                                             *
      *                  * ELETTRA                                     *
      *                  *---------------------------------------------*
           if        w-stp-inv-tot-p01    =    zero and
                     w-stp-inv-tot-p02    =    zero
                     go to prn-fin-cic-312.
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 1 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-tot-p01    >    999999999 or
                     w-stp-inv-tot-p01    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           move      c-dec                to   p-dec                  .
           move      "S"                  to   p-sgn                  .
           move      p-lnr                to   p-lin                  .
           move      107                  to   p-pos                  .
           move      w-stp-inv-tot-p01    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 2 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-tot-p02    >    999999999 or
                     w-stp-inv-tot-p02    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      120                  to   p-pos                  .
           move      w-stp-inv-tot-p02    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-fin-cic-312.
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Normalizzazione comodo per cumulo 'inverter'    *
      *              * 						        *
      *              * ELETTRA					        *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-inv-cat-p01      .      
           move      zero                 to   w-stp-inv-cat-p02      .      
       prn-ini-lr2-100.
      *              *-------------------------------------------------*
      *              * Inizializzazione work area per livello di rot-  *
      ...
      *                      *-----------------------------------------*
           add       1                    to   w-liv-csc-cst-ctr      .
      *                      *-----------------------------------------*
      *                      * Test se superato l'ultimo elemento      *
      *                      *-----------------------------------------*
           if        w-liv-csc-cst-ctr    >    w-liv-csc-cst-max
                     move  w-liv-csc-cst-max
                                          to   w-liv-csc-cst-ctr
                     move  "(ALTRE)"      to   w-liv-csc-cst-des
                                              (w-liv-csc-cst-max)
                     add   w-liv-csc-tot-p1r
                                          to   w-liv-csc-cst-val
                                              (w-liv-csc-cst-max)
                     go to prn-fin-lr2-390.
      *                      *-----------------------------------------*
      *                      * Bufferizzazione                         *
      *                      *-----------------------------------------*
           move      w-liv-csc-des-csc    to   w-liv-csc-cst-des
                                              (w-liv-csc-cst-ctr)     .
           if        w-liv-csc-des-csc    =    spaces
                     move  "(Non Classificati)"
                                          to   w-liv-csc-cst-des
                                              (w-liv-csc-cst-ctr)     .
      *
           move      w-liv-csc-tot-p1r    to   w-liv-csc-cst-val
                                              (w-liv-csc-cst-ctr)     .
       prn-fin-lr2-390.
      *                  *---------------------------------------------*
      *                  * Test se 'inverter' da stampare              *
      ...
      *                  * ELETTRA                                     *
      *                  *---------------------------------------------*
           if        w-stp-inv-cat-p01    =    zero and
                     w-stp-inv-cat-p02    =    zero
                     go to prn-fin-lr2-392.
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 1 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-cat-p01    >    999999999 or
                     w-stp-inv-cat-p01    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      107                  to   p-pos                  .
           move      w-stp-inv-cat-p01    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 2 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-cat-p02    >    999999999 or
                     w-stp-inv-cat-p02    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      120                  to   p-pos                  .
           move      w-stp-inv-cat-p02    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-fin-lr2-392.
      *                  *---------------------------------------------*
      ...
      *              * ELETTRA                                         *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-inv-cli-p01      .      
           move      zero                 to   w-stp-inv-cli-p02      .      
       prn-ini-lr1-100.
      *              *-------------------------------------------------*
      *              * Inizializzazione work area per livello di rot-  *
      *              * tura per cliente                                *
      ...
      *                  *---------------------------------------------*
           ...
           else      move  spaces         to   w-liv-cli-dpz-cli      .
      *                  *---------------------------------------------*
      *                  * Ragione sociale del cliente                 *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Subroutine di calcolo voce 'inverter'       *
      *                  *---------------------------------------------*
           perform   prn-fin-lr1-ele-000  thru prn-fin-lr1-ele-999    .
       prn-fin-lr1-127.
      *                  *---------------------------------------------*
      *                  * Preparazione                                *
      *                  *---------------------------------------------*
      ...
      *                  * ELETTRA                                     *
      *                  *---------------------------------------------*
           if        w-stp-inv-cli-p01    =    zero and
                     w-stp-inv-cli-p02    =    zero
                     go to prn-fin-lr1-482.
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 1 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-cli-p01    >    999999999 or
                     w-stp-inv-cli-p01    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      107                  to   p-pos                  .
           move      w-stp-inv-cli-p01    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Stampa valore cumulato 'inverter' periodo 2 *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           if        w-stp-inv-cli-p02    >    999999999 or
                     w-stp-inv-cli-p02    <   -999999999
                     move  11             to   p-car
                     move  "B"            to   p-edm
           else      move  9              to   p-car
                     move  "GB"           to   p-edm                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      120                  to   p-pos                  .
           move      w-stp-inv-cli-p02    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-fin-lr1-482.
      *                  *---------------------------------------------*
      ...
      *    * ELETTRA                                                   *
      *    *                                                           *
      *    * Modifica del 04.01.12                                     *
      *    *                                                           *
      *    * Si scansionano i codici prodotti numerici da 39794 a      *
      *    * 39795 secondo la seguente tabella:     		        *
      *    *                                                           *
      *    * PV1500 : 39794		                                    *
      *    * PV2800 : 39795		                                    *
      *    * PV4600 : 39796		                                    *
      ...
      *    *-----------------------------------------------------------*
       prn-fin-lr1-ele-000.
      *              *-------------------------------------------------*
      *              * Scansione delle righe fattura per determinare   *
      *              * il valore dei prodotti 'inverter' per il pri-   *
      *              * mo periodo                                      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start su [fir]                              *
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      w-rot-l01-cod-cli    to   rf-fir-cod-cli         .
           move      01                   to   rf-fir-tip-mag         .
           move      39794                to   rf-fir-num-pro         .
           move      rr-p1d-min           to   rf-fir-dat-doc         .
      ...
      *                  * Test sul massimo                            *
      *                  *---------------------------------------------*
           if        rf-fir-cod-cli       not  = w-rot-l01-cod-cli
                     go to prn-fin-lr1-ele-500.
           if        rf-fir-tip-mag       not  = 01
                     go to prn-fin-lr1-ele-500.
           if        rf-fir-num-pro       >    39799
                     go to prn-fin-lr1-ele-500.
       prn-fin-lr1-ele-140.
      *                  *---------------------------------------------*
      *                  * Selezioni                                   *
      ...
      *                      * Codice dipendenza cliente               *
      *                      *-----------------------------------------*
           if        rf-fir-dpz-cli       not  = w-rot-l01-dpz-cli
                     go to prn-fin-lr1-ele-200.
      *                      *-----------------------------------------*
      *                      * Data max, primo periodo                 *
      *                      *-----------------------------------------*
           if        rf-fir-dat-doc       >    rr-p1d-max
                     go to prn-fin-lr1-ele-200.
      *                      *-----------------------------------------*
      *                      * Data min, primo periodo                 *
      *                      *-----------------------------------------*
           if        rf-fir-dat-doc       <    rr-p1d-min
                     go to prn-fin-lr1-ele-200.
       prn-fin-lr1-ele-160.
      *                  *---------------------------------------------*
      *                  * Cumulo 'inverter' periodo 1                 *
      *                  *---------------------------------------------*
           add       rf-fir-imp-rig       to   w-stp-inv-cli-p01      .      
           add       rf-fir-imp-rig       to   w-stp-inv-cat-p01      .      
           add       rf-fir-imp-rig       to   w-stp-inv-tot-p01      .      
       prn-fin-lr1-ele-200.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
           go to     prn-fin-lr1-ele-100.
       prn-fin-lr1-ele-500.
      *              *-------------------------------------------------*
      *              * Scansione delle righe fattura per determinare   *
      *              * il valore dei prodotti 'inverter' per il se-    *
      *              * condo periodo                                   *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start su [fir]                              *
      *                  *---------------------------------------------*
           ...
           move      01                   to   rf-fir-tip-mag         .
           move      39794                to   rf-fir-num-pro         .
           move      rr-p2d-min           to   rf-fir-dat-doc         .
           move      zero                 to   rf-fir-num-prt         .
           move      zero                 to   rf-fir-num-prg         .
           move      "pgm/fat/fls/ioc/obj/ioffir"
                                          to   s-pat                  .
      ...
      *                      *-----------------------------------------*
           if        rf-fir-dpz-cli       not  = w-rot-l01-dpz-cli
                     go to prn-fin-lr1-ele-700.
      *                      *-----------------------------------------*
      *                      * Data max, secondo periodo               *
      *                      *-----------------------------------------*
           if        rf-fir-dat-doc       >    rr-p2d-max
                     go to prn-fin-lr1-ele-700.
      *                      *-----------------------------------------*
      *                      * Data min, secondo periodo               *
      *                      *-----------------------------------------*
           if        rf-fir-dat-doc       <    rr-p2d-min
                     go to prn-fin-lr1-ele-700.
       prn-fin-lr1-ele-660.
      *                  *---------------------------------------------*
      *                  * Cumulo 'inverter' periodo 2                 *
      *                  *---------------------------------------------*
           add       rf-fir-imp-rig       to   w-stp-inv-cli-p02      .      
           add       rf-fir-imp-rig       to   w-stp-inv-cat-p02      .      
           add       rf-fir-imp-rig       to   w-stp-inv-tot-p02      .      
       prn-fin-lr1-ele-700.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
           go to     prn-fin-lr1-ele-600.
       prn-fin-lr1-ele-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     prn-fin-lr1-ele-999.
       prn-fin-lr1-ele-999.
           exit.
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-031.
           move      "PER ZONA DI APPARTENENZA DEI CLIENTI              
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-032.
           move      "PER CATEGORIA DI CLIENTE                          
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-033.
           move      "PER CLASSIFICAZIONE STATISTICA DEI CLIENTI        
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-050.
      ...
      *              *-------------------------------------------------*
           ...
           move      w-liv-csc-cst-ctr    to   m-gen-grf-num-prd      .
      *
           if        rr-tcs-cli           =    01
                     ...
                     move  "Statistica per Codice libero"
                                          to   m-gen-grf-tit-grf
           else      move  "Statistica di vendita "
                                          to   m-gen-grf-tit-grf      .
      *
      ...
      *              * Ciclo dati                                      *
      *              *-------------------------------------------------*
           move      zero                 to   w-liv-csc-cst-ct1      .
           move      1                    to   w-liv-csc-cst-ct2      .
           move      zero                 to   w-liv-csc-cst-ct3      .
       exe-sqz-grf-500.
           add       1                    to   w-liv-csc-cst-ct1      .
           add       1                    to   w-liv-csc-cst-ct3      .
           if        w-liv-csc-cst-ct1    >    36
                     go to exe-sqz-grf-800.
           if        w-liv-csc-cst-ct3    >    12
                     add   1              to   w-liv-csc-cst-ct2
                     move  1              to   w-liv-csc-cst-ct3      .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione                             *
      *                  *---------------------------------------------*
           move      w-liv-csc-cst-des
                    (w-liv-csc-cst-ct1)   to   m-gen-grf-mes-des
                                              (w-liv-csc-cst-ct2,
                                               w-liv-csc-cst-ct3)     .
           move      w-liv-csc-cst-val
                    (w-liv-csc-cst-ct1)   to   m-gen-grf-mes-val
                                              (w-liv-csc-cst-ct2,
                                               w-liv-csc-cst-ct3)     .
       exe-sqz-grf-600.
      *                  *---------------------------------------------*
