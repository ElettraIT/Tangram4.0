      ...
      *                       Ultima revisione:    NdK del 16/05/22    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf3601:        *
      *                                                                *
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-250.
      *                  *---------------------------------------------*
      *                  * Tipo di ordinamento per la classe statisti- *
      *                  * ca cliente                                  *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Codice classe statistica cliente                 *
      *    *-----------------------------------------------------------*
       pmt-ccs-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       pmt-tor-csc-000.
           ...
           move      01                   to   v-pos                  .
           if        rr-tcs-cli           =    01
                     move  "Ordinamento zone           :"
                                          to   v-alf
           else if   rr-tcs-cli           =    02
                     move  "Ordinamento categorie      :"
                                          to   v-alf
           else if   rr-tcs-cli           =    03
                     move  "Ordinamento codici stat.   :"
                                          to   v-alf
           else      move  "Ordinamento codici stat.   :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-tor-csc-999.
      ...
      *                      *-----------------------------------------*
           ...
           perform   vis-ccs-cli-000      thru vis-ccs-cli-999        .
       acc-tcs-cli-665.
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       acc-tcs-cli-675.
      *                      *-----------------------------------------*
      *                      * Prompt per tipo ordinamento classe sta- *
      *                      * tistica                                 *
      *                      *-----------------------------------------*
           perform   pmt-tor-csc-000      thru pmt-tor-csc-999        .
       acc-tcs-cli-680.
      *                      *-----------------------------------------*
      *                      * Tipo ordinamento classe statistica      *
      *                      *-----------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Codice classe statistica clienti           *
      *    *-----------------------------------------------------------*
       acc-ccs-cli-000.
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo classe stati-   *
      *              * stica cliente                                   *
      *              *-------------------------------------------------*
           if        rr-tcs-cli           =    01
                     go to acc-ccs-cli-100
           else if   rr-tcs-cli           =    02
                     go to acc-ccs-cli-200
           else if   rr-tcs-cli           =    03
                     go to acc-ccs-cli-300
           else      go to acc-ccs-cli-999.
       acc-ccs-cli-100.
      *              *-------------------------------------------------*
      *              * Se classe statistica 01 : Zona                  *
      *              *-------------------------------------------------*
           perform   acc-ccs-cli-zon-000  thru acc-ccs-cli-zon-999    .
           go to     acc-ccs-cli-999.
       acc-ccs-cli-200.
      *              *-------------------------------------------------*
      *              * Se classe statistica 02 : Categoria             *
      *              *-------------------------------------------------*
           perform   acc-ccs-cli-cat-000  thru acc-ccs-cli-cat-999    .
           go to     acc-ccs-cli-999.
       acc-ccs-cli-300.
      *              *-------------------------------------------------*
      *              * Se classe statistica 03 : Codice statistico     *
      *              *-------------------------------------------------*
           perform   acc-ccs-cli-stt-000  thru acc-ccs-cli-stt-999    .
           go to     acc-ccs-cli-999.
       acc-ccs-cli-999.
           exit.

      *    *===========================================================*
      *    * Accettazione : Codice Zona                                *
      *    *-----------------------------------------------------------*
       acc-ccs-cli-zon-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-ccs-cli           to   w-cmn-zst-001-cod      .
           move      05                   to   w-cmn-zst-001-lin      .
           move      30                   to   w-cmn-zst-001-pos      .
           move      07                   to   w-cmn-zst-001-dln      .
       ...
       acc-ccs-cli-zon-110.
           ...
           if        w-cmn-zst-001-ope    =    "F+"
                     go to acc-ccs-cli-zon-115.
           if        w-cmn-zst-001-ope    =    "AC"
                     go to acc-ccs-cli-zon-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-ccs-cli-zon-115.
           perform   cmn-zst-001-foi-000  thru cmn-zst-001-foi-999    .
           go to     acc-ccs-cli-zon-110.
       acc-ccs-cli-zon-120.
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-ccs-cli           to   w-let-arc-zst-cod      .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-ccs-cli           to   w-cmn-zst-002-cod      .
           move      05                   to   w-cmn-zst-002-lin      .
           move      30                   to   w-cmn-zst-002-pos      .
           move      07                   to   w-cmn-zst-002-dln      .
       ...
       acc-ccs-cli-cat-110.
           ...
           if        w-cmn-zst-002-ope    =    "F+"
                     go to acc-ccs-cli-cat-115.
           if        w-cmn-zst-002-ope    =    "AC"
                     go to acc-ccs-cli-cat-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-ccs-cli-cat-115.
           perform   cmn-zst-002-foi-000  thru cmn-zst-002-foi-999    .
           go to     acc-ccs-cli-cat-110.
       acc-ccs-cli-cat-120.
      ...
      *    *===========================================================*
      *    * Accettazione : Codice Statistico                          *
      *    *-----------------------------------------------------------*
       acc-ccs-cli-stt-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-ccs-cli           to   w-cmn-zst-003-cod      .
           move      05                   to   w-cmn-zst-003-lin      .
           move      30                   to   w-cmn-zst-003-pos      .
           move      07                   to   w-cmn-zst-003-dln      .
       ...
       acc-ccs-cli-stt-110.
           ...
           if        w-cmn-zst-003-ope    =    "F+"
                     go to acc-ccs-cli-stt-115.
           if        w-cmn-zst-003-ope    =    "AC"
                     go to acc-ccs-cli-stt-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-ccs-cli-stt-115.
           perform   cmn-zst-003-foi-000  thru cmn-zst-003-foi-999    .
           go to     acc-ccs-cli-stt-110.
       acc-ccs-cli-stt-120.
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codice classe statistica clienti        *
      *    *-----------------------------------------------------------*
       vis-ccs-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione : Codice classe statistica clienti, de-   *
      *    *                   scrizione                               *
      *    *-----------------------------------------------------------*
       vis-ccs-cli-des-000.
           move      "DS"                 to   v-ope                  .
      ...
      *                  * tistica clienti diverso da zero : no accet- *
      *                  * tazione                                     *
      *                  *---------------------------------------------*
           if        rr-ccs-cli           =    zero
                     go to acc-tor-csc-050
