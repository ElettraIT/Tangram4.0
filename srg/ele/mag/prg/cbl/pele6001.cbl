      ...
      *                       Ultima revisione:    NdK del 05/09/00    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione per il programma pele6001:       *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " VENDUTO PER CLASSE STATISTICA PRODOTTO "       .
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Start su archivio [mmr] per data                *
      *              *                                                 *
      *              * 1. ciclo                                        *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           ...
           move      rr-p1d-min           to   rf-mmr-dat-reg         .
      ...
      *              *-------------------------------------------------*
           if        rf-mmr-dat-reg       >    rr-p1d-max
                     go to stp-srt-inp-500.
           if        rf-mmr-tip-mag       not  = 01
                     go to stp-srt-inp-500.
       stp-srt-inp-260.
      *              *-------------------------------------------------*
      *              * Sel max su archivio [mmr]                       *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su causale                             *
      *                  *---------------------------------------------*
           if        rf-mmr-cod-cau       not  = 00200 and
                     rf-mmr-cod-cau       not  = 00401
      ...
      *                  * Anagrafica prodotto                         *
      *                  *---------------------------------------------*
           move      rf-mmr-num-mag       to   w-let-arc-dcp-cod      .
      ...
      *                      * Quantita' fatturata                     *
      *                      *-----------------------------------------*
           move      rf-mmr-qta-mov       to   srt-val-sta-qta        .
      *                      *-----------------------------------------*
      *                      * Importo fatturato                       *
      *                      *-----------------------------------------*
           move      zero                 to   srt-val-sta-val        .
      *                      *-----------------------------------------*
      *                      * Incidenza sulla statistica              *
      *                      *-----------------------------------------*
           move      "P"                  to   srt-val-sta-pon        .
      *                      *-----------------------------------------*
      *                      * Incidenza sul 1. periodo di riferimento *
      *                      *-----------------------------------------*
           move      "S"                  to   srt-val-sta-sn1        .
      *                      *-----------------------------------------*
      *                      * Incidenza sul 2. periodo di riferimento *
      *                      *-----------------------------------------*
           move      "N"                  to   srt-val-sta-sn2        .
      *                      *-----------------------------------------*
      *                      * Incidenza sul 3. periodo di riferimento *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "DATMAG    "         to   f-key                  .
           move      rr-p2d-min           to   rf-mmr-dat-reg         .
           move      01                   to   rf-mmr-tip-mag         .
           move      zero                 to   rf-mmr-num-mag         .
           move      zero                 to   rf-mmr-num-prt         .
           move      zero                 to   rf-mmr-num-prg         .
      ...
      *                      * Incidenza sul 1. periodo di riferimento *
      *                      *-----------------------------------------*
           move      "N"                  to   srt-val-sta-sn1        .
      *                      *-----------------------------------------*
      *                      * Incidenza sul 2. periodo di riferimento *
      *                      *-----------------------------------------*
           move      "S"                  to   srt-val-sta-sn2        .
      *                      *-----------------------------------------*
      *                      * Incidenza sul 3. periodo di riferimento *
      *                      *-----------------------------------------*
           move      "N"                  to   srt-val-sta-sn3        .
      ...
      *                  * statistica prodotto, del mnemonico, e della *
      *                  * descrizione                                 *
      *                  *---------------------------------------------*
       prn-fin-lr1-330.
      *                      *-----------------------------------------*
      ...
      *                  * senza eseguire una interlinea di separa-    *
      *                  * zione                                       *
      *                  *---------------------------------------------*
      ...
      *                      * classe statistica prodotto, del mne-    *
      ...
      *                  * la classe statistica prodotto, bensi' uno   *
      *                  * successivo                                  *
      *                  *---------------------------------------------*
       prn-liv-det-680.
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-031.
           move      "PER 1. CODICE STATISTICO PRODOTTO                 
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-032.
           move      "PER 2. CODICE STATISTICO PRODOTTO                 
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-033.
           move      "PER 3. CODICE STATISTICO PRODOTTO                 
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-034.
           move      "PER CLASSE MERCEOLOGICA PRODOTTO                  
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-050.
      ...
      *                          *-------------------------------------*
           if        rr-tcs-pro           =    01 or
                     rr-tcs-pro           =    02 or
                     rr-tcs-pro           =    03
                     move  "Classe statistica del prodotto              
      -                    "            "
      ...
      *    *-----------------------------------------------------------*
       mod-fat-fir-opn-000.
           move      "pgm/svf/prg/obj/mfatfir0"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           move      s-pat                to   w-mod-fat-fir-pth      .
           move      "OP"                 to   w-mod-fat-fir-ope      .
           perform   mod-fat-fir-cll-000  thru mod-fat-fir-cll-999    .
       mod-fat-fir-opn-999.
           exit.
       mod-fat-fir-cls-000.
           move      "CL"                 to   w-mod-fat-fir-ope      .
           perform   mod-fat-fir-cll-000  thru mod-fat-fir-cll-999    .
           cancel    w-mod-fat-fir-pth                                .
       mod-fat-fir-cls-999.
           exit.
       mod-fat-fir-gts-000.
           move      "GS"                 to   w-mod-fat-fir-ope      .
           perform   mod-fat-fir-cll-000  thru mod-fat-fir-cll-999    .
       mod-fat-fir-gts-999.
           exit.
       mod-fat-fir-get-000.
           move      "GT"                 to   w-mod-fat-fir-ope      .
           perform   mod-fat-fir-cll-000  thru mod-fat-fir-cll-999    .
       mod-fat-fir-get-999.
           exit.
       mod-fat-fir-dvf-000.
           move      "VF"                 to   w-mod-fat-fir-ope      .
           perform   mod-fat-fir-cll-000  thru mod-fat-fir-cll-999    .
       mod-fat-fir-dvf-999.
           exit.
       mod-fat-fir-cll-000.
           call      w-mod-fat-fir-pth   using w-mod-fat-fir
                                               rf-fit
                                               rf-fir                 .
       mod-fat-fir-cll-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       mod-not-svf-opn-000.
           move      "pgm/svf/prg/obj/mnotsvf0"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           move      s-pat                to   w-mod-not-svf-pth      .
           move      "OP"                 to   w-mod-not-svf-ope      .
           perform   mod-not-svf-cll-000  thru mod-not-svf-cll-999    .
       mod-not-svf-opn-999.
           exit.
       mod-not-svf-cls-000.
           move      "CL"                 to   w-mod-not-svf-ope      .
           perform   mod-not-svf-cll-000  thru mod-not-svf-cll-999    .
           cancel    w-mod-not-svf-pth                                .
       mod-not-svf-cls-999.
           exit.
       mod-not-svf-dvn-000.
           move      "VN"                 to   w-mod-not-svf-ope      .
           perform   mod-not-svf-cll-000  thru mod-not-svf-cll-999    .
       mod-not-svf-dvn-999.
           exit.
       mod-not-svf-cll-000.
           call      w-mod-not-svf-pth   using w-mod-not-svf
                                               rf-fit
                                               rf-fir                 .
       mod-not-svf-cll-999.
           exit.
