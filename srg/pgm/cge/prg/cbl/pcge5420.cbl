      ...
      *                                   Fase:    cge542              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 01/02/91    *
      *                       Ultima revisione:    NdK del 19/09/17    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pcge5421:        *
      *                                                                *
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "per Mnemonico       "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Si/no partite chiuse                       *
      *        *-------------------------------------------------------*
           05  w-exp-snx-chi.
               10  w-exp-snx-chi-num      pic  9(02)       value 02   .
               10  w-exp-snx-chi-lun      pic  9(02)       value 02   .
               10  w-exp-snx-chi-tbl.
      ...
      *                  * Data estratto conto                         *
      *                  *---------------------------------------------*
           perform   acc-dat-esc-000      thru acc-dat-esc-999        .
      ...
      *                  * Data massima righe                          *
      *                  *---------------------------------------------*
           perform   acc-dat-mrp-000      thru acc-dat-mrp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-600.
      *                  *---------------------------------------------*
      *                  * Si/no partite chiuse                        *
      *                  *---------------------------------------------*
           perform   acc-snx-chi-000      thru acc-snx-chi-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Soglia minima                               *
      *                  *---------------------------------------------*
           perform   acc-sgl-min-000      thru acc-sgl-min-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Prompt 'Estratto conto alla data'               *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Data massima righe partita'             *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
           move      05                   to   v-lin                  .
           move      44                   to   v-pos                  .
           move      "Data massima righe partita :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Selezione su archivio fornitori'        *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su archivio fornitori"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt per valore minimo e massimo              *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      37                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Stampa anche partite chiuse ?'          *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Stampa anche partite chiuse ? :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Soglia minima di controllo'             *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           move      21                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Soglia minima di controllo    :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       pmt-ric-sel-999.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per data massima                  *
      *    *-----------------------------------------------------------*
       pmt-dat-chi-000.
      *                  *---------------------------------------------*
      *                  * Prompt 'Data minima delle partite chiuse'   *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      34                   to   v-car                  .
           ...
           move      38                   to   v-pos                  .
           if        rr-snx-chi           =    "S"
                     move  "Data minima delle partite chiuse :"
                                          to   v-alf
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data estratto conto        *
      *    *-----------------------------------------------------------*
       acc-dat-esc-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-dat-esc           not  = zero
                     go to acc-dat-esc-100.
      *                  *---------------------------------------------*
      *                  * Richiesta system-date a segretario          *
      *                  *---------------------------------------------*
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rr-dat-esc             .
       acc-dat-esc-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-esc-999.
       acc-dat-esc-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-esc             .
       acc-dat-esc-400.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data massima righe         *
      *    *-----------------------------------------------------------*
       acc-dat-mrp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * Preparazione eventuale default              *
      *                  *---------------------------------------------*
           if        rr-dat-mrp           not  = zero
                     go to acc-dat-mrp-100.
           move      rr-dat-esc           to   rr-dat-mrp             .
       acc-dat-mrp-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-mrp-999.
       acc-dat-mrp-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-mrp             .
       acc-dat-mrp-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-fnt-cod      .
           move      14                   to   w-cod-mne-fnt-lin      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-fnt-ope      .
           move      rr-cod-max           to   w-cod-mne-fnt-cod      .
           move      14                   to   w-cod-mne-fnt-lin      .
           move      53                   to   w-cod-mne-fnt-pos      .
           move      zero                 to   w-cod-mne-fnt-rln      .
           move      zero                 to   w-cod-mne-fnt-rps      .
           move      zero                 to   w-cod-mne-fnt-vln      .
           move      zero                 to   w-cod-mne-fnt-vps      .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/no partite chiuse       *
      *    *-----------------------------------------------------------*
       acc-snx-chi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * Preparazione eventuale default              *
      *                  *---------------------------------------------*
           if        rr-snx-chi           not  = spaces
                     go to acc-snx-chi-100.
           move      "N"                  to   rr-snx-chi             .
       acc-snx-chi-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-chi-lun    to   v-car                  .
           move      w-exp-snx-chi-num    to   v-ldt                  .
           move      "SN#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-exp-snx-chi-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-chi-999.
       acc-snx-chi-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "S"            to   rr-snx-chi
           else if   v-num                =    02
                     move  "N"            to   rr-snx-chi
           else      move  spaces         to   rr-snx-chi             .
       acc-snx-chi-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-chi           =    spaces
                     go to acc-snx-chi-100.
       acc-snx-chi-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Visualizzazione prompt per data massima     *
      *                  *---------------------------------------------*
           perform   pmt-dat-chi-000      thru pmt-dat-chi-999        .
      *                  *---------------------------------------------*
      *                  * se 'No' partite chiuse : normalizzazione    *
      *                  * data massima                                *
      *                  *---------------------------------------------*
           if        rr-snx-chi           =    "S"
                     go to acc-snx-chi-800.
           move      zero                 to   rr-dat-chi             .
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data massima chiusura      *
      *    *-----------------------------------------------------------*
       acc-dat-chi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo selezione : Data massima chiusura   *
      *    *-----------------------------------------------------------*
       vis-dat-chi-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Soglia minima                        *
      *    *-----------------------------------------------------------*
       acc-sgl-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sgl-min-999.
       acc-sgl-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sgl-min             .
       acc-sgl-min-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-sgl-min-100.
       acc-sgl-min-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Soglia minima                     *
      *    *-----------------------------------------------------------*
       vis-sgl-min-000.
           move      "DS"                 to   v-ope                  .
           move      "V"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           move      zero                 to   rr-dat-esc             .
           move      zero                 to   rr-dat-mrp             .
           ...
           move      spaces               to   rr-snx-chi             .
           move      zero                 to   rr-dat-chi             .
           move      zero                 to   rr-sgl-min             .
       nor-ric-sel-999.
           exit.
