      ...
      *                                   Fase:    cge390              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 06/02/97    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pcge3901:        *
      *                                                                *
      *                    Numeratore pagine per vidimazione giornale  *
      *                    di contabilita'                             *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " NUMERATORE PAGINE GIORNALE CONTABILITA'"       .
      ...
      *    *-----------------------------------------------------------*
       01  w-det.
      *        *-------------------------------------------------------*
      *        * Work per Controllo su anno                            *
      *        *-------------------------------------------------------*
           05  w-det-ctl-ann.
      *            *---------------------------------------------------*
      *            * Ridefinizione anno                                *
      *            *---------------------------------------------------*
               10  w-det-ctl-ann-alf      pic  x(04)                  .
               10  w-det-ctl-ann-alf-r    redefines
                   w-det-ctl-ann-alf.
                   15  w-det-ctl-ann-num  pic  9(04)                  .
      ...
      *                  * Pagina iniziale                             *
      *                  *---------------------------------------------*
           perform   acc-pag-ini-000      thru acc-pag-ini-999        .
      ...
      *                  * Pagine da stampare                          *
      *                  *---------------------------------------------*
           perform   acc-num-pag-000      thru acc-num-pag-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Anno da stampare                            *
      *                  *---------------------------------------------*
           perform   acc-ann-stp-000      thru acc-ann-stp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Pagina iniziale                                 *
      *              *-------------------------------------------------*
           perform   pmt-pag-ini-000      thru pmt-pag-ini-999        .
      *              *-------------------------------------------------*
      *              * Pagine da stampare                              *
      *              *-------------------------------------------------*
           perform   pmt-num-pag-000      thru pmt-num-pag-999        .
      *              *-------------------------------------------------*
      *              * Anno da stampare                                *
      *              *-------------------------------------------------*
           perform   pmt-ann-stp-000      thru pmt-ann-stp-999        .
       pmt-ric-sel-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione prompts per Pagina iniziale               *
      *    *-----------------------------------------------------------*
       pmt-pag-ini-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Pagine da stampare            *
      *    *-----------------------------------------------------------*
       pmt-num-pag-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Numero pagine da stampare  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-num-pag-999.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Anno da stampare              *
      *    *-----------------------------------------------------------*
       pmt-ann-stp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Pagina iniziale            *
      *    *-----------------------------------------------------------*
       acc-pag-ini-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-pag-ini-999.
       acc-pag-ini-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-pag-ini             .
       acc-pag-ini-400.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-pag-ini-100.
       acc-pag-ini-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Pagine da stampare         *
      *    *-----------------------------------------------------------*
       acc-num-pag-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-num-pag-999.
       acc-num-pag-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-num-pag             .
       acc-num-pag-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-num-pag-100.
       acc-num-pag-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Anno da stampare           *
      *    *-----------------------------------------------------------*
       acc-ann-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-ann-stp           not  = spaces
                     go to acc-ann-stp-100.
      *                  *---------------------------------------------*
      *                  * Date and time da segreteria                 *
      *                  *---------------------------------------------*
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-saa                to   w-det-ctl-ann-num      .
           add       1900                 to   w-det-ctl-ann-num      .
           move      w-det-ctl-ann-alf    to   rr-ann-stp             .
       acc-ann-stp-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-ann-stp-999.
       acc-ann-stp-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-ann-stp             .
       acc-ann-stp-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-ann-stp           not  = spaces
                     go to acc-ann-stp-500.
      *                  *---------------------------------------------*
      *                  * Visualizzazione literal 'no'                *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      02                   to   v-car                  .
      ...
      *                  * Oltre                                       *
      *                  *---------------------------------------------*
           go to     acc-ann-stp-600.
       acc-ann-stp-500.
           move      rr-ann-stp           to   w-det-ctl-ann-alf      .
           if        w-det-ctl-ann-num    not  numeric
                     go to acc-ann-stp-100.
       acc-ann-stp-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo su Pagina iniziale                    *
      *              *-------------------------------------------------*
           if        rr-pag-ini           not  = zero
                     go to tdo-ric-sel-200.
           move      "ME"                 to   v-ope                  .
           move      "Manca il numero di pagina iniziale !"
                                          to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Controllo su Pagine da stampare                 *
      *              *-------------------------------------------------*
           if        rr-num-pag           not  = zero
                     go to tdo-ric-sel-800.
           move      "ME"                 to   v-ope                  .
           move      "Manca il numero di pagine da stampare !"
                                          to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-800.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           move      zero                 to   rr-pag-ini             .
           move      zero                 to   rr-num-pag             .
           move      spaces               to   rr-ann-stp             .
       nor-ric-sel-999.
           exit.
