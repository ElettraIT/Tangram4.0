      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pcge4021:        *
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "per Partita iva     "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su partita iva                   *
      *        *-------------------------------------------------------*
           05  w-exp-sel-pti.
               10  w-exp-sel-pti-num      pic  9(02)       value 3    .
               10  w-exp-sel-pti-lun      pic  9(02)       value 40   .
               10  w-exp-sel-pti-tbl.
                   15  filler             pic  x(40) value
                            "Tutti                                   ".
                   15  filler             pic  x(40) value
                            "solo con partita iva Errata o mancante  ".
                   15  filler             pic  x(40) value
                            "solo con partita iva Doppia             ".
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su lettera d'intenti             *
      *        *-------------------------------------------------------*
           05  w-exp-sel-lic.
               10  w-exp-sel-lic-num      pic  9(02)       value 5    .
               10  w-exp-sel-lic-lun      pic  9(02)       value 40   .
               10  w-exp-sel-lic-tbl.
                   15  filler             pic  x(40) value
                            "No                                      ".
                   15  filler             pic  x(40) value
                            "si, ma solo clienti Con lettera         ".
                   15  filler             pic  x(40) value
                            "si, ma solo clienti con Lettera scaduta ".
                   15  filler             pic  x(40) value
                            "si, ma solo clienti con lettera Attiva  ".
                   15  filler             pic  x(40) value
                            "si, ma solo clienti Senza lettera       ".
      ...
      *              *-------------------------------------------------*
      *              * Lettura personalizzazione per la gestione della *
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-550.
      *                  *---------------------------------------------*
      *                  * Selezioni partita iva                       *
      *                  *---------------------------------------------*
           perform   acc-sel-pti-000      thru acc-sel-pti-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
       ...
       acc-ric-sel-600.
      *                  *---------------------------------------------*
      *                  * Selezioni su lettera d'intenti              *
      *                  *---------------------------------------------*
           perform   acc-sel-lic-000      thru acc-sel-lic-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt per riferimenti a lettera d'intenti      *
      *              *-------------------------------------------------*
           perform   pmt-sel-lic-000      thru pmt-sel-lic-999        .
       pmt-ric-sel-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione prompts per Selezione su partita iva      *
      *    *-----------------------------------------------------------*
       pmt-sel-pti-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      01                   to   v-pos                  .
           if        rr-tip-ord           =    "P"
                     move  "Tipo selezione su partita iva :"
                                          to   v-alf
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Selezione su lettera d'in-    *
      *    * tenti                                                     *
      *    *-----------------------------------------------------------*
       pmt-sel-lic-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      20                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Stampa riferimenti a lettera  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se tipo ordinamento per partita iva         *
      *                  *---------------------------------------------*
           perform   pmt-sel-pti-000      thru pmt-sel-pti-999        .
           if        rr-tip-ord           =    "P"
                     go to acc-tip-ord-800.
      *                  *---------------------------------------------*
      *                  * Normalizzazione e visualizzazione selezione *
      *                  *---------------------------------------------*
           move      spaces               to   rr-sel-pti             .
           perform   vis-sel-pti-000      thru vis-sel-pti-999        .
       acc-tip-ord-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Selezione su partita iva   *
      *    *-----------------------------------------------------------*
       acc-sel-pti-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-pti-lun    to   v-car                  .
           move      w-exp-sel-pti-num    to   v-ldt                  .
           move      "TED#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-exp-sel-pti-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "T"            to   rr-sel-pti
           else if   v-num                =    02
                     move  "E"            to   rr-sel-pti
           else if   v-num                =    03
                     move  "D"            to   rr-sel-pti
           else      move  spaces         to   rr-sel-pti             .
       acc-sel-pti-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sel-pti           =    spaces
                     go to acc-sel-pti-100.
       acc-sel-pti-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo selezione : Selezione partita iva   *
      *    *-----------------------------------------------------------*
       vis-sel-pti-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Selezione su lettera d'intenti       *
      *    *-----------------------------------------------------------*
       acc-sel-lic-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-lic-lun    to   v-car                  .
           move      w-exp-sel-lic-num    to   v-ldt                  .
           move      "NCLAS#"             to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      20                   to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-exp-sel-lic-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sel-lic           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sel-lic             .
       acc-sel-lic-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sel-lic           =    zero
                     go to acc-sel-lic-100.
       acc-sel-lic-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Selezione su lettera d'intenti    *
      *    *-----------------------------------------------------------*
       vis-sel-lic-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-sel-pti             .
           move      zero                 to   rr-sel-lic             .
           move      spaces               to   rr-snx-lic             .
       nor-ric-sel-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *              * Tipo selezione su partita iva                   *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "P"
                     if     rr-sel-pti    =    spaces
                            move   "T"    to   rr-sel-pti             .
      *              *-------------------------------------------------*
      *              * Tipo selezione su lettera d'intenti             *
      *              *-------------------------------------------------*
           if        w-prs-snx-lic        =    "S"
                     if     rr-sel-lic    =    zero
                            move   1      to   rr-sel-lic             .
      *              *-------------------------------------------------*
      *              * Si/no gestione lettera d'intenti                *
      *              *-------------------------------------------------*
           move      w-prs-snx-lic        to   rr-snx-lic             .
       reg-ric-sel-999.
           exit.
