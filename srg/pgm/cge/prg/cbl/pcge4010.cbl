      ...
      *                                   Fase:    cge401              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 19/06/92    *
      *                       Ultima revisione:    NdK del 11/06/18    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio [cli]            *
      *                                                                *
      *                    Anagrafico Clienti                          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   INTERROGAZIONE ANAGRAFICA CLIENTI   "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-rip-str.
      *        *-------------------------------------------------------*
      *        * Work-area per ripristino Start su [cli]               *
      *        *-------------------------------------------------------*
           05  w-rip-str-cli.
      *            *---------------------------------------------------*
      *            * Flag di End-of-file                               *
      *            *---------------------------------------------------*
               10  w-rip-str-cli-eof      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Parametri di Start                                *
      *            *---------------------------------------------------*
               10  w-rip-str-cli-cod      pic  9(07)                  .
               10  w-rip-str-cli-rag      pic  x(40)                  .
               10  w-rip-str-cli-mne      pic  x(10)                  .
               10  w-rip-str-cli-piv      pic  9(11)                  .
               10  w-rip-str-cli-cfi      pic  x(16)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp-rec.
           05  w-exp-rec-cli-lin          pic  9(02) value 15         .
           05  w-exp-rec-cli-ctr          pic  9(02)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-rip-str-cli-eof      .
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo ordinamento     *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "C"
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cfi-min           to   rf-cli-cod-fis         .
           move      zero                 to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
           if        rf-cli-prt-iva       <    rr-pti-min or
                     rf-cli-prt-iva       >    rr-pti-max
                     go to qry-sel-rec-900.
      *              *-------------------------------------------------*
      *              * Selezione su codice fiscale                     *
      *              *-------------------------------------------------*
           if        rf-cli-cod-fis       <    rr-cfi-min or
                     rf-cli-cod-fis       >    rr-cfi-max
                     go to qry-sel-rec-900.
       qry-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-cmp-rot-000.
           move      rf-cli-rag-soc       to   w-rot-l01-rag-soc      .
       qry-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-cli-rag-soc       to   w-buf-rag-soc          .
           move      rf-cli-via-cli       to   w-buf-via-cli          .
           move      rf-cli-loc-cli       to   w-buf-loc-cli          .
      ...
      *              *-------------------------------------------------*
           if        v-key                not  = "EXPD"
                     go to qry-trt-fun-999.
      *                  *---------------------------------------------*
      *                  * Routine per Expand su record [cli]          *
      *                  *---------------------------------------------*
           perform   exp-rec-cli-000      thru exp-rec-cli-999        .
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       exp-rec-cli-000.
      *              *-------------------------------------------------*
      *              * Salvataggio parametri per Start su [cli]        *
      *              *-------------------------------------------------*
           if        w-rip-str-cli-eof    not  = spaces
                     go to exp-rec-cli-100.
           move      rf-cli-cod-cli       to   w-rip-str-cli-cod      .
           move      rf-cli-rag-key       to   w-rip-str-cli-rag      .
           move      rf-cli-cod-mne       to   w-rip-str-cli-mne      .
           move      rf-cli-prt-iva       to   w-rip-str-cli-piv      .
           move      rf-cli-cod-fis       to   w-rip-str-cli-cfi      .
       exp-rec-cli-100.
      *              *-------------------------------------------------*
      *              * Video in 'OFF'                                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "CODCLI    "         to   f-key                  .
           move      v-cnt                to   w-mpn                  .
           move      w-mpn-cod-cli        to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           move      rf-cli-cod-cge       to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
      *                  *---------------------------------------------*
      *                  * Lettura codice esenzione iva                *
      *                  *---------------------------------------------*
           move      rf-cli-cod-iva       to   w-let-arc-zci-cod      .
      ...
      *                      *-----------------------------------------*
           if        rf-cli-cod-cge       =    zero
                     go to exp-rec-cli-720.
      *                      *-----------------------------------------*
      *                      * Codice sottoconto                       *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           ...
           move      rf-cli-cod-cge       to   w-edt-cod-pdc-cod      .
      ...
      *                      * Se a zero : soggetto                    *
      *                      *-----------------------------------------*
           if        rf-cli-cod-iva       not  = zero
                     go to exp-rec-cli-725.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      15                   to   v-car                  .
           move      14                   to   v-lin                  .
      ...
      *                      * Ciclo 1..3                              *
      *                      *-----------------------------------------*
           move      zero                 to   w-exp-rec-cli-ctr      .
       exp-rec-cli-735.
           add       1                    to   w-exp-rec-cli-ctr      .
           if        w-exp-rec-cli-ctr    >    03 or
                     w-exp-rec-cli-ctr    >    d-con-arc-num-ele
                     go to exp-rec-cli-740.
      *                      *-----------------------------------------*
      *                      * Tipo utenza                             *
      *                      *-----------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
           move      w-exp-rec-cli-lin    to   v-lin                  .
           add       w-exp-rec-cli-ctr    to   v-lin                  .
           move      03                   to   v-pos                  .
      *
           if        d-con-arc-tip-con
                    (w-exp-rec-cli-ctr)   =    "SEP"
                     move  all "-"        to   v-alf
      ...
      *
           ...
           move      rf-cli-ide-ute       to   w-all-str-cat (2)      .
      ...
      *
           move      03                   to   w-all-str-num          .
           move      w-all-str-alf        to   w-all-str-cat (1)      .
           move      rf-cli-ide-fas       to   w-all-str-cat (2)      .
           move      "]"                  to   w-all-str-cat (3)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
      *                      *-----------------------------------------*
      *                      * Visualizzazione                         *
      ...
      *                  *---------------------------------------------*
           if        w-rip-str-cli-eof    not  = spaces
                     go to exp-rec-cli-999.
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo di Start    *
      *                  * attualmente attivo                          *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    "C"
                     go to exp-rec-cli-910
           else if   rr-tip-ord           =    "R"
                     go to exp-rec-cli-920
           else if   rr-tip-ord           =    "M"
                     go to exp-rec-cli-930
           else if   rr-tip-ord           =    "P"
                     go to exp-rec-cli-940
           else if   rr-tip-ord           =    "F"
                     go to exp-rec-cli-950.
       exp-rec-cli-910.
      *                  *---------------------------------------------*
      *                  * Se start per codice                         *
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODCLI    "         to   f-key                  .
           move      w-rip-str-cli-cod    to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-cli                 .
           if        f-sts                not  = e-not-err
                     go to exp-rec-cli-990.
           go to     exp-rec-cli-980.
       exp-rec-cli-920.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "RAGKEY    "         to   f-key                  .
           move      w-rip-str-cli-rag    to   rf-cli-rag-key         .
           move      w-rip-str-cli-cod    to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODMNE    "         to   f-key                  .
           move      w-rip-str-cli-mne    to   rf-cli-cod-mne         .
           move      w-rip-str-cli-cod    to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "PRTIVA    "         to   f-key                  .
           move      w-rip-str-cli-piv    to   rf-cli-prt-iva         .
           move      w-rip-str-cli-cod    to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODFIS    "         to   f-key                  .
           move      w-rip-str-cli-cfi    to   rf-cli-cod-fis         .
           move      w-rip-str-cli-cod    to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
