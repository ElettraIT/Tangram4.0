      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su tabella tributi F24       *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    TABELLA TRIBUTI PER MODELLO F24     "       .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-des-zpu-cod      .
           move      14                   to   w-cod-des-zpu-lin      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-des-zpu-ope      .
           move      rr-cod-max           to   w-cod-des-zpu-cod      .
           move      14                   to   w-cod-des-zpu-lin      .
           move      52                   to   w-cod-des-zpu-pos      .
           move      zero                 to   w-cod-des-zpu-dln      .
           move      zero                 to   w-cod-des-zpu-dps      .
           move      "<B"                 to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-zpu-cod-tpu         .
           move      "pgm/cge/fls/ioc/obj/iofzpu"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-des-min           to   rf-zpu-des-key         .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [zpu]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/cge/fls/ioc/obj/iofzpu"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           ...
           if        rf-zpu-tip-sez       not  = rr-tip-sez
                     move  "#"            to   w-cnt-qry-flg-sub
      ...
      *              * Selezione su codice                             *
      *              *-------------------------------------------------*
           if        rf-zpu-cod-tpu       <    rr-cod-min or
                     rf-zpu-cod-tpu       >    rr-cod-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su descrizione                        *
      *              *-------------------------------------------------*
           if        rf-zpu-des-key       <    rr-des-min or
                     rf-zpu-des-key       >    rr-des-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
       qry-sel-rec-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      11                   to   v-pos                  .
           move      rf-zpu-rig-des (1)   to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-zpu-cod-tpu       to   w-mpn-cod-tpu          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-zpu-cod-tpu       to   v-alf                  .
      *
           if        rf-zpu-tip-sez       =    01
                     move  "ERAR"         to   v-alf
           else if   rf-zpu-tip-sez       =    02
                     move  "INPS"         to   v-alf
           else if   rf-zpu-tip-sez       =    03
                     move  "REG."         to   v-alf
           else if   rf-zpu-tip-sez       =    04
                     move  "IMU "         to   v-alf
           else if   rf-zpu-tip-sez       =    05
                     move  "INA."         to   v-alf
           else if   rf-zpu-tip-sez       =    06
                     move  "EN.P"         to   v-alf
           else      move  spaces         to   v-alf                  .
      *
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Editing Codice sottoconto per Debito        *
      *                  *---------------------------------------------*
           move      w-prs-liv-pdc        to   w-edt-cod-pdc-liv      .
           move      rf-zpu-stc-deb        to   w-edt-cod-pdc-cod      .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Editing Codice sottoconto per Credito       *
      *                  *---------------------------------------------*
           move      w-prs-liv-pdc        to   w-edt-cod-pdc-liv      .
           move      rf-zpu-stc-cre        to   w-edt-cod-pdc-cod      .
           move      "B"                  to   w-edt-cod-pdc-edm      .
           perform   edt-pdc-adx-000      thru edt-pdc-adx-999        .
      *                  *---------------------------------------------*
      *                  * Visualizzazione                             *
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-tpu'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      21                   to   v-pos                  .
           move      "    TABELLA TRIBUTI PER MODELLO F24     "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "des-tpu" per la descrizione                    *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
           move      "des-tpu"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                not  = spaces
