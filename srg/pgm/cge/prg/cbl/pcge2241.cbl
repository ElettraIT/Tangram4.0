      ...
      *                       Ultima revisione:    NdK del 07/07/99    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma cge224   *
      *                                                                *
      ...
      *            * Tipo classificatore                               *
      *            *---------------------------------------------------*
               10  srt-tip-cdb            pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Codice classificatore                             *
      *            *---------------------------------------------------*
               10  srt-cod-cdb            pic  9(05)                  .
      *            *---------------------------------------------------*
      *            * Codice sottoconto                                 *
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
               10  srt-des-cdb            pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Codice per riclassificazione conto economico      *
      *            *---------------------------------------------------*
               10  srt-cod-rcl            pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Dare o avere                                      *
      *            *---------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su classificatori di bilancio          *
      *                  *---------------------------------------------*
           if        rf-pdc-ccb-dar       =    zero   and
                     rf-pdc-ccb-ave       =    zero
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Per record classificatore in dare           *
      *                  *---------------------------------------------*
           if        rf-pdc-tcb-dar       =    spaces or
                     rf-pdc-ccb-dar       =    zero
      ...
      *                  * Composizione chiave                         *
      *                  *---------------------------------------------*
           move      rf-pdc-tcb-dar       to   srt-tip-cdb            .
           move      rf-pdc-ccb-dar       to   srt-cod-cdb            .
           move      rf-pdc-cod-pdc       to   srt-cod-pdc            .
      *                  *---------------------------------------------*
      *                  * Composizione dati                           *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Normalizzazione record [cdb]            *
      *                      *-----------------------------------------*
           move      "NO"                 to   f-ope                  .
           move      "pgm/cge/fls/ioc/obj/iofcdb"
                                          to   s-pat                  .
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-pdc-tcb-dar       to   rf-cdb-tip-cdb         .
           move      rf-pdc-ccb-dar       to   rf-cdb-cod-cdb         .
           move      "pgm/cge/fls/ioc/obj/iofcdb"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-cdb                 .
           if        f-sts                not  = e-not-err
                     move  all "."        to   rf-cdb-des-cdb         .
      *                      *-----------------------------------------*
      *                      * Descrizione classificatore              *
      *                      *-----------------------------------------*
           move      rf-cdb-des-cdb       to   srt-des-cdb            .
      *                      *-----------------------------------------*
      *                      * Codice per riclassificazione conto      *
      *                      * economico                               *
      *                      *-----------------------------------------*
           move      rf-cdb-cod-rcl       to   srt-cod-rcl            .
      *                      *-----------------------------------------*
      *                      * Dare o avere                            *
      *                      *-----------------------------------------*
           move      "D"                  to   srt-dar-ave            .
      *                      *-----------------------------------------*
      *                      * Descrizione sottoconto                  *
      *                      *-----------------------------------------*
      ...
      *                  * Composizione chiave                         *
      *                  *---------------------------------------------*
           move      rf-pdc-tcb-ave       to   srt-tip-cdb            .
           move      rf-pdc-ccb-ave       to   srt-cod-cdb            .
           move      rf-pdc-cod-pdc       to   srt-cod-pdc            .
      *                  *---------------------------------------------*
      *                  * Composizione dati                           *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCDB    "         to   f-key                  .
           move      rf-pdc-tcb-ave       to   rf-cdb-tip-cdb         .
           move      rf-pdc-ccb-ave       to   rf-cdb-cod-cdb         .
           move      "pgm/cge/fls/ioc/obj/iofcdb"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      srt-tip-cdb          to   w-rot-l03-tip-cdb      .
           move      srt-cod-cdb (1 : 2)  to   w-rot-l02-mas-cdb      .
           move      srt-cod-cdb          to   w-rot-l01-cod-cdb      .
       prn-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      01                   to   p-pos                  .
           if        w-rot-l03-tip-cdb    =    "A"
                     move  "Attivita'                     "
                                          to   p-alf
           else if   w-rot-l03-tip-cdb    =    "P"
                     move  "Passivita' o Capitale Netto   "
                                          to   p-alf
           else if   w-rot-l03-tip-cdb    =    "O"
                     move  "Conti d'ordine                "
                                          to   p-alf
           else if   w-rot-l03-tip-cdb    =    "C"
                     move  "Costi                         "
                                          to   p-alf
           else if   w-rot-l03-tip-cdb    =    "R"
                     move  "Ricavi                        "
                                          to   p-alf
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      11                   to   p-pos                  .
           move      w-rot-l03-tip-cdb    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Codice                                  *
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      12                   to   p-pos                  .
           move      w-rot-l02-mas-cdb    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Normalizzazione record [cdb]            *
      ...
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCDB    "         to   f-key                  .
           move      w-rot-l03-tip-cdb    to   rf-cdb-tip-cdb         .
           move      w-rot-l02-mas-cdb    to   rf-cdb-cod-cdb (1 : 2) .
           move      zero                 to   rf-cdb-cod-cdb (3 : 3) .
           move      "pgm/cge/fls/ioc/obj/iofcdb"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
       ...
       prn-ini-lr1-100.
      *              *-------------------------------------------------*
      *              * Stampa codice classificatore                    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se da stampare                         *
      *                  *---------------------------------------------*
           if        w-rot-l01-cod-cdb (3 : 3)
                                          =    zero
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      21                   to   p-pos                  .
           move      w-rot-l01-cod-cdb    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Descrizione                             *
      ...
      *                          *-------------------------------------*
           ...
           move      srt-cod-pdc          to   w-edt-cod-pdc-cod      .
