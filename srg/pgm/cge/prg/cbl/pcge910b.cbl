      ...
      *            * Descrizione fase                                  *
      *            *---------------------------------------------------*
               10  srt-des-fas            pic  x(30)                  .
      *            *---------------------------------------------------*
      *            * Note fase                                         *
      *            *---------------------------------------------------*
               10  srt-not-fas            pic  x(36)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     " CONTROLLO DOCUMENTI ACQUISTO O VENDITA "       .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tip-rec.
               10  w-exp-tip-rec-num      pic  9(02)       value 07   .
      ...
      *    *-----------------------------------------------------------*
       01  w-det.
      *        *-------------------------------------------------------*
      *        * Work per Det movimento di CGE                         *
      *        *-------------------------------------------------------*
           05  w-det-mov-cge.
      *            *---------------------------------------------------*
      *            * Flag di uscita                                    *
      *            *---------------------------------------------------*
               10  w-det-mov-cge-flg      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Messaggio                                         *
      *            *---------------------------------------------------*
               10  w-det-mov-cge-msg      pic  x(80)                  .
      *            *---------------------------------------------------*
      *            * Data registrazione                                *
      *            *---------------------------------------------------*
               10  w-det-mov-cge-dat      pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Protocollo                                        *
      *            *---------------------------------------------------*
               10  w-det-mov-cge-prt      pic  9(07)                  .
      *        *-------------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      18                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "N.B.: il programma controlla l'esistenza delle scr
      -              "itture di Primanote relative  "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *
      ...
      *              * Fatture di acquisto                             *
      *              *-------------------------------------------------*
           perform   stp-srt-inp-sff-000  thru stp-srt-inp-sff-999    .
      *              *-------------------------------------------------*
      *              * Scadenze fornitori                              *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-bit-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [bit] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    02
                     go to stp-srt-inp-bit-050
           else      go to stp-srt-inp-bit-990.
       stp-srt-inp-bit-050.
      *              *-------------------------------------------------*
      *              * [bit]                                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-dat-min           to   rf-bit-ide-dat         .
      ...
      *                  *---------------------------------------------*
           if        rr-cod-ute           =    spaces
                     go to stp-srt-inp-bit-420.
           if        rf-bit-ide-ute       not  = rr-cod-ute
                     go to stp-srt-inp-bit-200.
       stp-srt-inp-bit-420.
      *                  *---------------------------------------------*
      *                  * Test su registrazione contabile             *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-bit-dat-doc       =    zero or
                     rf-bit-prt-mgd       =    zero
                     go to stp-srt-inp-bit-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-bit-dat-doc       to   w-det-mov-cge-dat      .
           move      rf-bit-prt-mgd       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Registrazione fattura accompagnatoria"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-bit-600.
       stp-srt-inp-bit-580.
      *                  *---------------------------------------------*
      *                  * A riciclo per test superati                 *
      *                  *---------------------------------------------*
           go to     stp-srt-inp-bit-200.
       stp-srt-inp-bit-600.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-bit-num-doc
                    (06 : 06)             to   w-all-str-cat (3)      .
           move      "-"                  to   w-all-str-cat (4)      .
           move      p-edt                to   w-all-str-cat (5)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
           move      w-all-str-alf        to   srt-des-fas            .
      *                  *---------------------------------------------*
      *                  * Descrizione note                            *
      *                  *---------------------------------------------*
           move      w-det-mov-cge-msg    to   srt-not-fas            .
       stp-srt-inp-bit-700.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-fit-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [fit] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    03
                     go to stp-srt-inp-fit-050
           else      go to stp-srt-inp-fit-990.
       stp-srt-inp-fit-050.
      *              *-------------------------------------------------*
      *              * [fit]                                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "DATSYS    "         to   f-key                  .
           move      rr-dat-min           to   rf-fit-ide-dat         .
      ...
      *                      *-----------------------------------------*
           if        rf-fit-dat-doc       =    zero or
                     rf-fit-prt-mgd       =    zero
                     go to stp-srt-inp-fit-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-fit-dat-doc       to   w-det-mov-cge-dat      .
           move      rf-fit-prt-mgd       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Registrazione fattura accompagnatoria"
                                          to   w-det-mov-cge-msg
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-sdb-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [sdb] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    04
                     go to stp-srt-inp-sdb-050
           else      go to stp-srt-inp-sdb-990.
       stp-srt-inp-sdb-050.
      *              *-------------------------------------------------*
      *              * [sdb]                                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [sdb]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/gep/fls/ioc/obj/iofsdb"
                                          to   s-pat                  .
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-emi       =    zero or
                     rf-sdb-npc-emi       =    zero
                     go to stp-srt-inp-sdb-440.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-emi       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-emi       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-sto       =    zero or
                     rf-sdb-npc-sto       =    zero
                     go to stp-srt-inp-sdb-460.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-sto       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-sto       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-rsp       =    zero or
                     rf-sdb-npc-rsp       =    zero
                     go to stp-srt-inp-sdb-480.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-rsp       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-rsp       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Richiamo scadenza"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-sdb-600.
       stp-srt-inp-sdb-480.
      *                  *---------------------------------------------*
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-acs       =    zero or
                     rf-sdb-npc-acs       =    zero
                     go to stp-srt-inp-sdb-500.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-acs       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-acs       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
       ...
       stp-srt-inp-sdb-500.
      *                  *---------------------------------------------*
      *                  * Test su buon esito scadenza                 *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-nbe       =    zero or
                     rf-sdb-npc-nbe       =    zero
                     go to stp-srt-inp-sdb-520.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-nbe       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-nbe       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Buon esito scadenza"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-sdb-600.
       stp-srt-inp-sdb-520.
      *                  *---------------------------------------------*
      *                  * Test su buon esito 2 scadenza               *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sdb-drc-pbe       =    zero or
                     rf-sdb-npc-pbe       =    zero
                     go to stp-srt-inp-sdb-540.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-pbe       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-pbe       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Buon esito scadenza"
                                          to   w-det-mov-cge-msg
      ...
      *                      *-----------------------------------------*
           if        rf-sdb-drc-isp       =    zero or
                     rf-sdb-npc-isp       =    zero
                     go to stp-srt-inp-sdb-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sdb-drc-isp       to   w-det-mov-cge-dat      .
           move      rf-sdb-npc-isp       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Insoluto scadenza"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-sdb-600.
       stp-srt-inp-sdb-580.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "Scadenza Clienti :" to   w-all-str-cat (1)      .
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-ddp-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [ddp] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    05
                     go to stp-srt-inp-ddp-050
           else      go to stp-srt-inp-ddp-990.
       stp-srt-inp-ddp-050.
      *              *-------------------------------------------------*
      *              * [ddp]                                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [ddp]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/gep/fls/ioc/obj/iofddp"
                                          to   s-pat                  .
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-ddp-drc-pre       =    zero or
                     rf-ddp-npc-pre       =    zero
                     go to stp-srt-inp-ddp-440.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-ddp-drc-pre       to   w-det-mov-cge-dat      .
           move      rf-ddp-npc-pre       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-ddp-drc-act       =    zero or
                     rf-ddp-npc-act       =    zero
                     go to stp-srt-inp-ddp-460.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-ddp-drc-act       to   w-det-mov-cge-dat      .
           move      rf-ddp-npc-act       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-ddp-drc-acd       =    zero or
                     rf-ddp-npc-acd       =    zero
                     go to stp-srt-inp-ddp-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-ddp-drc-acd       to   w-det-mov-cge-dat      .
           move      rf-ddp-npc-acd       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-sff-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [sff] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    06
                     go to stp-srt-inp-sff-050
           else      go to stp-srt-inp-sff-990.
       stp-srt-inp-sff-050.
      *              *-------------------------------------------------*
      *              * [sff]                                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "DATSYS    "         to   f-key                  .
           move      rr-dat-min           to   rf-sff-ide-dat         .
      ...
      *                  *---------------------------------------------*
           if        rr-cod-ute           =    spaces
                     go to stp-srt-inp-sff-420.
           if        rf-sff-ide-ute       not  = rr-cod-ute
                     go to stp-srt-inp-sff-200.
       stp-srt-inp-sff-420.
      *                  *---------------------------------------------*
      *                  * Test su registrazione contabile             *
      *                  *---------------------------------------------*
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sff-drg-cge       =    zero or
                     rf-sff-npr-cge       =    zero
                     go to stp-srt-inp-sff-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sff-drg-cge       to   w-det-mov-cge-dat      .
           move      rf-sff-npr-cge       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Registrazione fattura di acquisto"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-sff-600.
       stp-srt-inp-sff-580.
      *                  *---------------------------------------------*
      *                  * A riciclo per test superati                 *
      *                  *---------------------------------------------*
      ...
      *                  * Data ultima modifica                        *
      *                  *---------------------------------------------*
           move      rf-sff-ide-dat       to   srt-ide-dat            .
      *                  *---------------------------------------------*
      *                  * Utente ultima modifica                      *
      *                  *---------------------------------------------*
           move      rf-sff-ide-ute       to   srt-ide-ute            .
      *                  *---------------------------------------------*
      *                  * Fase ultima modifica                        *
      *                  *---------------------------------------------*
           move      rf-sff-ide-fas       to   srt-ide-fas            .
      *                  *---------------------------------------------*
      *                  * Editing data                                *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-sfs-000.
      *              *-------------------------------------------------*
      *              * Test se archivio [sfs] da controllare           *
      *              *-------------------------------------------------*
           if        rr-tip-rec           =    01 or
                     rr-tip-rec           =    07
                     go to stp-srt-inp-sfs-050
           else      go to stp-srt-inp-sfs-990.
       stp-srt-inp-sfs-050.
      *              *-------------------------------------------------*
      *              * [sfs]                                           *
      *              *-------------------------------------------------*
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sfs-drc-rgs       =    zero or
                     rf-sfs-npc-rgs       =    zero
                     go to stp-srt-inp-sfs-440.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sfs-drc-rgs       to   w-det-mov-cge-dat      .
           move      rf-sfs-npc-rgs       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Registrazione scadenza fornitore"
                                          to   w-det-mov-cge-msg
      ...
      *                      * Test se da eseguire                     *
      *                      *-----------------------------------------*
           if        rf-sfs-drc-sto       =    zero or
                     rf-sfs-npc-sto       =    zero
                     go to stp-srt-inp-sfs-580.
      *                      *-----------------------------------------*
      *                      * Test di esistenza                       *
      *                      *-----------------------------------------*
           move      rf-sfs-drc-sto       to   w-det-mov-cge-dat      .
           move      rf-sfs-npc-sto       to   w-det-mov-cge-prt      .
           perform   det-mov-cge-000      thru det-mov-cge-999        .
           if        w-det-mov-cge-flg    not  = spaces
                     move  "Storno scadenza fornitore"
                                          to   w-det-mov-cge-msg
                     go to stp-srt-inp-sfs-600.
       stp-srt-inp-sfs-580.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      30                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      "Scadenza Fornit. :" to   w-all-str-cat (1)      .
           move      p-edt                to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
           move      w-all-str-alf        to   srt-des-fas            .
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Determinazione esistenza movimento di CGE                 *
      *    *-----------------------------------------------------------*
       det-mov-cge-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione flag di uscita                  *
      *              *-------------------------------------------------*
           move      spaces               to   w-det-mov-cge-flg      .
      *              *-------------------------------------------------*
      *              * Normalizzazione messaggio di uscita             *
      *              *-------------------------------------------------*
           move      spaces               to   w-det-mov-cge-msg      .
       det-mov-cge-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione [mgt]                           *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-det-mov-cge-dat    to   rf-mgt-dat-reg         .
           move      w-det-mov-cge-prt    to   rf-mgt-num-prt         .
           move      "pgm/cge/fls/ioc/obj/iofmgt"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        f-sts                =    e-not-err
                     go to det-mov-cge-600.
       det-mov-cge-400.
      *              *-------------------------------------------------*
      *              * Se testata non trovata                          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Flag di uscita                              *
      *                  *---------------------------------------------*
           move      "#"                  to   w-det-mov-cge-flg      .
      *                  *---------------------------------------------*
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
           go to     det-mov-cge-900.
       det-mov-cge-600.
      *              *-------------------------------------------------*
      *              * Se testata trovata                              *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "DOCUMENTI CON RIFERIMENTO ALLA PRIMANOTA NON TROVA
      -              "TO"                 to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "  Data     Utente    Fase      Elemento senza prim
      -              "anota                     Note                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
