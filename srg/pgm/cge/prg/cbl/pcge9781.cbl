      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma cge978   *
      *                                                                *
      *                    Stampa di controllo saldi iniziali          *
      *                                                                *
      *                    N.B.: solo esercizi 01/12                   *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
      *            * Codice sottoconto anagrafico                      *
      *            *---------------------------------------------------*
               10  srt-stc-ana            pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Codice archivio                                   *
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-tot.
      *        *-------------------------------------------------------*
      *        * Work per totale sottoconto                            *
      *        *-------------------------------------------------------*
           05  w-tot-stc.
      *            *---------------------------------------------------*
      *            * Totale                                            *
      *            *---------------------------------------------------*
               10  w-tot-stc-tot-imp      pic s9(13)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-det.
      *        *-------------------------------------------------------*
      *        * Work per determinazione saldo iniziale archivio       *
      *        *-------------------------------------------------------*
           05  w-det-sdo-cge.
      *            *---------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-rot-l02.
               10  w-rot-l02-tip-con      pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * 1. livello di rottura                                 *
      *        *-------------------------------------------------------*
           05  w-rot-l01.
               10  w-rot-l01-stc-ana      pic  9(07)                  .
      ...
      *    *-----------------------------------------------------------*
       stp-srt-inp-000.
      *              *-------------------------------------------------*
      *              * Test in funzione del tipo conto                 *
      *              *-------------------------------------------------*
           if        rr-tip-cnt           =    "C"
      ...
      *                  *---------------------------------------------*
      *                  * Test su saldo iniziale                      *
      *                  *---------------------------------------------*
           if        rf-mgs-sdo-ini       =    zero
      ...
      *                  * Chiave                                      *
      *                  *---------------------------------------------*
           move      "C"                  to   srt-tip-cnt            .
           move      rf-cli-cod-cge       to   srt-stc-ana            .
           move      rf-cli-cod-cli       to   srt-cod-arc            .
      *                  *---------------------------------------------*
      *                  * Dati                                        *
      *                  *---------------------------------------------*
           move      rf-cli-rag-soc       to   srt-des-arc            .
           move      rf-mgs-sdo-ini       to   srt-sdo-cge            .
       stp-srt-inp-cli-700.
      *              *-------------------------------------------------*
      ...
      *                  * Chiave                                      *
      *                  *---------------------------------------------*
           move      "F"                  to   srt-tip-cnt            .
           move      rf-fnt-cod-cge       to   srt-stc-ana            .
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      srt-tip-cnt          to   w-rot-l02-tip-con      .
           move      srt-stc-ana          to   w-rot-l01-stc-ana      .
       prn-cmp-rot-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Normalizzazione saldo totale                    *
      *              *-------------------------------------------------*
           move      zero                 to   w-tot-stc-tot-imp      .
       prn-ini-lr1-999.
           exit.
      ...
      *                      *-----------------------------------------*
           ...
           move      w-rot-l01-stc-ana    to   w-edt-cod-pdc-cod      .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Saldo iniziale letto                    *
      *                      *-----------------------------------------*
           move      rr-ann-ese           to   w-det-sdo-cge-ese      .
           move      "G"                  to   w-det-sdo-cge-tar      .
           move      w-rot-l01-stc-ana    to   w-det-sdo-cge-arc      .
           perform   det-sdo-cge-000      thru det-sdo-cge-999        .
      *                      *-----------------------------------------*
      *                      * Stampa                                  *
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      039                  to   p-pos                  .
           move      rf-mgs-sdo-ini       to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Saldo iniziale calcolato                *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "V"                  to   p-tip                  .
           move      10                   to   p-car                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      058                  to   p-pos                  .
           move      w-tot-stc-tot-imp    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-fin-lr1-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Cumulo saldi iniziali                           *
      *              *-------------------------------------------------*
           add       srt-sdo-cge          to   w-tot-stc-tot-imp      .
       prn-liv-det-100.
      *              *-------------------------------------------------*
      *              * Test se righe residue sufficienti               *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        rr-tip-cnt           =    "C"
                     move  "SALDI INIZIALI CLIENTI"
                                          to   w-cnt-tit-des-tit
           else      move  "SALDI INIZIALI FORNITORI"
                                          to   w-cnt-tit-des-tit      .
      *
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      " T                Codice e ragione sociale        
      -              "         Saldo iniziale       "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
