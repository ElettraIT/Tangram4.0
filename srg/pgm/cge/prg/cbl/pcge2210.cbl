      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio classificatori   *
      *                    di bilancio                                 *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "INTERROGAZIONE SU CLASSIFICATORI BILANCI"       .
      ...
      *        *-------------------------------------------------------*
           05  w-ipc-dlp.
      *            *---------------------------------------------------*
      *            * Per tipo classificatore                           *
      *            *---------------------------------------------------*
               10  w-ipc-dlp-cdb.
      *                *-----------------------------------------------*
      *                * Tipo classificatore passato                   *
      *                *-----------------------------------------------*
                   15  w-ipc-dlp-cdb-tip  pic  x(01)                  .

      *    *===========================================================*
      *    * Work-area richieste per interrogazione                    *
      *    *-----------------------------------------------------------*
       01  rr.
           05  rr-tip-cdb                 pic  x(01)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-tip-cdb              pic  x(01)                  .
           05  w-mpn-cod-cdb              pic  9(05)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-pre-exe-pgm      .
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c.                     *
      *              *-------------------------------------------------*
           perform   ipc-dlp-cdb-000      thru ipc-dlp-cdb-999        .
      *              *-------------------------------------------------*
      *              * Preparazione richieste                          *
      *              *-------------------------------------------------*
           if        w-ipc-dlp-cdb-tip    =    spaces
                     go to pre-exe-pgm-999.
           move      w-ipc-dlp-cdb-tip    to   rr-tip-cdb             .
       pre-exe-pgm-999.
           exit.
      ...
      *              * Tipo classificatore                             *
      *              *-------------------------------------------------*
           perform   pmt-tip-cdb-000      thru pmt-tip-cdb-999        .
       pmt-ric-sel-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione prompts per Tipo classificatore           *
      *    *-----------------------------------------------------------*
       pmt-tip-cdb-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Tipo classificatore        *
      *    *-----------------------------------------------------------*
       acc-tip-cdb-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           if        rr-tip-cdb           =    "A"
                     move  01             to   v-num
           else if   rr-tip-cdb           =    "P"
                     move  02             to   v-num
           else if   rr-tip-cdb           =    "O"
                     move  03             to   v-num
           else if   rr-tip-cdb           =    "C"
                     move  04             to   v-num
           else if   rr-tip-cdb           =    "R"
                     move  05             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "A"            to   rr-tip-cdb
           else if   v-num                =    02
                     move  "P"            to   rr-tip-cdb
           else if   v-num                =    03
                     move  "O"            to   rr-tip-cdb
           else if   v-num                =    04
                     move  "C"            to   rr-tip-cdb
           else if   v-num                =    05
                     move  "R"            to   rr-tip-cdb
           else      move  spaces         to   rr-tip-cdb             .
       acc-tip-cdb-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-tip-cdb           =    spaces
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-tip-cdb           to   rf-cdb-tip-cdb         .
      ...
      *              *-------------------------------------------------*
           if        rf-cdb-tip-cdb       not  = rr-tip-cdb
                     move  "#"            to   w-cnt-qry-flg-sub      .
       qry-tst-max-999.
           exit.
      ...
      *                  * Se raggruppamento                           *
      *                  *---------------------------------------------*
           move      rf-cdb-cod-cdb       to   w-wrk-cdb-cod          .
           if        w-wrk-cdb-ele        not  = zero
      ...
      *                      *-----------------------------------------*
           move      "+"                  to   v-edm                  .
           move      rf-cdb-tip-cdb       to   w-mpn-tip-cdb          .
           move      rf-cdb-cod-cdb       to   w-mpn-cod-cdb          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           go to     qry-liv-det-999.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'ccb-ext'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *    *===========================================================*
      *    * Lettura i.p.c. da livello precedente per Tipo classif.    *
      *    *-----------------------------------------------------------*
       ipc-dlp-cdb-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione iniziale                        *
      *              *-------------------------------------------------*
           move      spaces               to   w-ipc-dlp-cdb-tip      .
      *              *-------------------------------------------------*
      *              * Determinazione eventuale variabile di i.p.c.    *
      *              * relativa al tipo classificatore                 *
      *              *-------------------------------------------------*
           move      "GV"                 to   s-ope                  .
           move      "tip-cdb"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move   s-alf         to   w-ipc-dlp-cdb-tip
           else      move   spaces        to   w-ipc-dlp-cdb-tip      .
       ipc-dlp-cdb-999.
           exit.
