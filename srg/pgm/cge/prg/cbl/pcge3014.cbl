      ...
      *                    programma cge3010:                          *
      *                                                                *
      *                    Partitario di contabilita' generale         *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-wrk.
      *        *-------------------------------------------------------*
      *        * Saldo alla data iniziale                              *
      *        *-------------------------------------------------------*
           05  w-wrk-sdo-dti              pic s9(13)                  .
      *        *-------------------------------------------------------*
      *        * Saldo alla data finale                                *
      *        *-------------------------------------------------------*
           05  w-wrk-sdo-dtf              pic s9(13)                  .
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
      *        * Flag di visualizzazione testata completata            *
      *        *-------------------------------------------------------*
           05  w-wrk-flg-tes              pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Flag fincatura                                        *
      *        *-------------------------------------------------------*
           05  w-wrk-flg-fnc              pic  x(01)                  .
      *        *-------------------------------------------------------*
      ...
      *              *-------------------------------------------------*
______*    move      spaces               to   w-cnt-dic-ini-pgm      .
______*    perform   dic-ini-pgm-000      thru dic-ini-pgm-999        .
______*    if        w-cnt-dic-ini-pgm    not  = spaces
______*              go to main-999.
       main-100.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
______*    perform   dic-fin-pgm-000      thru dic-fin-pgm-999        .
       main-999.
           exit      program                                          .
      ...
      *                  * Saldo alla data iniziale                    *
      *                  *---------------------------------------------*
           move      w-sad-sdo-dat        to   w-wrk-sdo-dti          .
      *              *-------------------------------------------------*
      ...
      *              * Preparazione chiave                             *
      *              *-------------------------------------------------*
           move      rr-cod-arc           to   rf-mgr-cod-pdc         .
      ...
      *    *-----------------------------------------------------------*
       z-qry-tst-max-000.
           if        rf-mgr-cod-pdc       not  = rr-cod-arc or
      ...
      *              *-------------------------------------------------*
      *              * Inizializzazione flag di fincatura              *
      *              *-------------------------------------------------*
           move      spaces               to   w-wrk-flg-fnc          .
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       z-qry-fin-cic-000.
      *              *-------------------------------------------------*
      *              * Determinazione saldo alla data finale           *
      *              *-------------------------------------------------*
           move      w-wrk-sdo-dti        to   w-wrk-sdo-dtf          .
           add       w-wrk-prg-dar        to   w-wrk-sdo-dtf          .
           subtract  w-wrk-prg-ave        from w-wrk-sdo-dtf          .
      *              *-------------------------------------------------*
      *              * Visualizzazione riepilogo finale                *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Flag di fincatura a 'no fincatura'          *
      *                  *---------------------------------------------*
           move      "#"                  to   w-wrk-flg-fnc          .
      *                  *---------------------------------------------*
      *                  * Intestazione pagina                         *
      *                  *---------------------------------------------*
           perform   int-pag-vid-000      thru int-pag-vid-999        .
      ...
      *                  *---------------------------------------------*
           ...
           move      "G"                  to   v-edm                  .
           move      v-lnr                to   v-lin                  .
           if        w-wrk-sdo-dti        <    zero
                     move   64            to   v-pos
           else      move   44            to   v-pos                  .
           move      w-wrk-sdo-dti        to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Interlinee                              *
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Literal 'Progressivi movimenti visualizzati'*
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      36                   to   v-car                  .
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Progressivi movimenti visualizzati :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           go to     z-qry-liv-det-190.
       z-qry-liv-det-140.
           if        rf-mgr-com-rig       =    w-dop-com-rig
                     go to z-qry-liv-det-150.
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      40                   to   v-car                  .
           move      v-lnr                to   v-lin                  .
      ...
      *                      *-----------------------------------------*
       ...
       z-qry-liv-det-900.
      *              *-------------------------------------------------*
      *              * Se movimento di bilancio non si considera ai    *
      *              * fini dell'aggiornamento progressivi periodo     *
      *              * stampato                                        *
      *              *-------------------------------------------------*
           if        rf-mgr-snx-mob       not  = "N"
                     go to  z-qry-liv-det-920.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Titolo : 'PARTITARIO MOVIMENTI DI GENERALE'     *
      *              *-------------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      32                   to   v-car                  .
           move      v-lnr                to   v-lin                  .
           move      25                   to   v-pos                  .
           move      "PARTITARIO MOVIMENTI DI GENERALE"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su flag di fincatura                   *
      *                  *---------------------------------------------*
           if        w-wrk-flg-fnc        not  = spaces
                     go to  int-pag-vid-999.
      *                  *---------------------------------------------*
      *                  * 'Data reg.'                                 *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      rf-mgr-cod-pdc       to   w-sav-cod-pdc          .
      ...
      *              *-------------------------------------------------*
           move      w-sav-cod-pdc        to   rf-mgr-cod-pdc         .
      ...
      *                      *-----------------------------------------*
      *                      * Se il bilancio dell'esercizio relativo  *
      *                      * alla data richiesta non e' stato ancora *
      *                      * chiuso : stesso funzionamento come per  *
      *                      * data non di fine esercizio              *
      *                      *-----------------------------------------*
           if        rn-dat-bil-flg-bil   =    spaces    and
                     rn-dat-bil-ese-bil   =    w-sad-ann-ese
                     go to  det-sdo-ini-100
           else      go to  det-sdo-ini-900.
       det-sdo-ini-100.
      *                      *-----------------------------------------*
