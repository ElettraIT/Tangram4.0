      ...
      *                       Ultima revisione:    NdK del 24/01/17    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma cge402   *
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           ...
           05  rr-sel-lic                 pic  9(01)                  .
           05  rr-snx-lic                 pic  x(01)                  .
      ...
      *    *===========================================================*
      *    * Area per controllo stampa su partite iva doppie           *
      *    *-----------------------------------------------------------*
       01  w-cpi-dop.
      *        *-------------------------------------------------------*
      *        * Status                                                *
      *        * - 00 : Inizio assoluto                                *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-sta-tus          pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Buffer per record 'rf-cli' precedente                 *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-rec-pre.
               10  filler  occurs 1048    pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Buffer per partita iva record 'rf-cli' precedente     *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-piv-pre.
               10  filler                 pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Buffer per record 'rf-cli' attuale                    *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-rec-att.
               10  filler  occurs 1048    pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Buffer per partita iva record 'rf-cli' attuale        *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-piv-att.
               10  filler                 pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Numero ciclo di stampa dettaglio                      *
      *        *-------------------------------------------------------*
           05  w-cpi-dop-ncc-det          pic  9(02)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Normalizzazione status per controllo su partite *
      *              * iva doppie                                      *
      *              *-------------------------------------------------*
           move      00                   to   w-cpi-dop-sta-tus      .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-pti-min           to   rf-cli-prt-iva         .
           move      zero                 to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
       ...
       prn-sel-rec-050.
      *              *-------------------------------------------------*
      *              * Selezione su lettera d'intenti                  *
      *              *-------------------------------------------------*
           if        rr-snx-lic           not  = "S"
           ...
           if        rr-sel-lic           =    1
                     go to prn-sel-rec-100.
      *                  *---------------------------------------------*
      *                  * Lettura estremi lettera d'intenti           *
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCLI    "         to   f-key                  .
           ...
           call      s-pat               using f
                                               rf-lic                 .
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo di test     *
      *                  *---------------------------------------------*
           if        rr-sel-lic           =    2
                     go to prn-sel-rec-060
           else if   rr-sel-lic           =    3
                     go to prn-sel-rec-070
           else if   rr-sel-lic           =    4
                     go to prn-sel-rec-080
           else if   rr-sel-lic           =    5
                     go to prn-sel-rec-090
           else      go to prn-sel-rec-100.
       prn-sel-rec-060.
      *                  *---------------------------------------------*
      *                  * Se solo clienti con lettera                 *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-sel-rec-900
           else      go to prn-sel-rec-100.
       prn-sel-rec-070.
      *                  *---------------------------------------------*
      *                  * Se solo clienti con lettera scaduta         *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se esiste lettera d'intenti        *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-sel-rec-900.
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                      *-----------------------------------------*
      *                      * Confronto con data riferimento finale   *
      *                      *-----------------------------------------*
           if        rf-lic-drf-fin       not  < s-dat
                     go to prn-sel-rec-900
           else      go to prn-sel-rec-100.
       prn-sel-rec-080.
      *                  *---------------------------------------------*
      *                  * Se solo clienti con lettera attiva          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se esiste lettera d'intenti        *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           else if   rr-sel-pti           =    "D"
                     go to  prn-sel-rec-600
           else if   rr-sel-pti           =    "N"
                     go to  prn-sel-rec-660
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se mancante : a selezione superata          *
      *                  *---------------------------------------------*
           if        rf-cli-prt-iva       =    zero
                     go to  prn-sel-rec-800.
      *                  *---------------------------------------------*
      *                  * Se estero : a selezione non superata        *
      *                  *---------------------------------------------*
      ...
      *                  * Controllo formale                           *
      *                  *---------------------------------------------*
           move      rf-cli-prt-iva       to   w-cnt-piv-piv          .
       ...
       prn-sel-rec-600.
      *              *-------------------------------------------------*
      *              * Solo clienti con partita iva doppia             *
      *              *-------------------------------------------------*
       prn-sel-rec-610.
      *                  *---------------------------------------------*
      *                  * Se partita iva a zero : selezione non supe- *
      *                  * rata                                        *
      *                  *---------------------------------------------*
           if        rf-cli-prt-iva       =    zero
                     go to  prn-sel-rec-900.
      *                  *---------------------------------------------*
      *                  * Bufferizzazione record 'rf-cli' attuale     *
      *                  *---------------------------------------------*
           move      rf-cli               to   w-cpi-dop-rec-att      .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione partita iva relativa al re- *
      *                  * cord 'rf-cli' attuale                       *
      *                  *---------------------------------------------*
           move      rf-cli-prt-iva       to   w-cpi-dop-piv-att      .
       prn-sel-rec-620.
      *                  *---------------------------------------------*
      *                  * Se status a 00                              *
      *                  *---------------------------------------------*
      ...
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        w-cpi-dop-sta-tus    not  = 00
                     go to prn-sel-rec-630.
      *                      *-----------------------------------------*
      *                      * Bufferizzazione record 'rf-cli' attuale *
      *                      * in 'rf-cli' precedente                  *
      *                      *-----------------------------------------*
           move      w-cpi-dop-rec-att    to   w-cpi-dop-rec-pre      .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione partita iva relativa al *
      *                      * record 'rf-cli' attuale in partita iva  *
      *                      * relativa al record 'rf-cli' precedente  *
      *                      *-----------------------------------------*
           move      w-cpi-dop-piv-att    to   w-cpi-dop-piv-pre      .
      *                      *-----------------------------------------*
      *                      * Status a 01                             *
      *                      *-----------------------------------------*
           move      01                   to   w-cpi-dop-sta-tus      .
      *                      *-----------------------------------------*
      ...
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        w-cpi-dop-sta-tus    not  = 01
                     go to prn-sel-rec-640.
      *                      *-----------------------------------------*
      *                      * Bufferizzazione record 'rf-cli' attuale *
      *                      *-----------------------------------------*
           move      rf-cli               to   w-cpi-dop-rec-att      .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione partita iva relativa al *
      *                      * record 'rf-cli' attuale                 *
      *                      *-----------------------------------------*
           move      rf-cli-prt-iva       to   w-cpi-dop-piv-att      .
      *                      *-----------------------------------------*
      *                      * Se partita iva doppia : selezione su-   *
      *                      * perata, e status a 02                   *
      *                      *-----------------------------------------*
           if        w-cpi-dop-piv-att    =    w-cpi-dop-piv-pre
                     move  02             to   w-cpi-dop-sta-tus
                     go to prn-sel-rec-800.
      *                      *-----------------------------------------*
      *                      * Se partita iva non doppia               *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Bufferizzazione record 'rf-cli' at- *
      *                          * tuale in 'rf-cli' precedente        *
      *                          *-------------------------------------*
           move      w-cpi-dop-rec-att    to   w-cpi-dop-rec-pre      .
      *                          *-------------------------------------*
      *                          * Bufferizzazione partita iva relati- *
      *                          * va al record 'rf-cli' attuale in    *
      *                          * partita iva relativa al record      *
      *                          * 'rf-cli' precedente                 *
      *                          *-------------------------------------*
           move      w-cpi-dop-piv-att    to   w-cpi-dop-piv-pre      .
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
       ...
       prn-sel-rec-640.
      *                  *---------------------------------------------*
      *                  * Se status uguale o maggiore di 02           *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Bufferizzazione record 'rf-cli' attuale *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Se partita iva doppia : selezione su-   *
      *                      * perata, e status a 03                   *
      *                      *-----------------------------------------*
           if        w-cpi-dop-piv-att    =    w-cpi-dop-piv-pre
                     move  03             to   w-cpi-dop-sta-tus
                     go to prn-sel-rec-800.
      *                      *-----------------------------------------*
      *                      * Se partita iva non doppia               *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
       ...
       prn-sel-rec-660.
      *              *-------------------------------------------------*
      *              * Solo se non entra in allegato a 13. iva         *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       prn-liv-det-100.
      *              *-------------------------------------------------*
      *              * Se non e' una stampa per partita iva, e solo    *
      *              * per le partite iva doppie : continua            *
      *              *-------------------------------------------------*
           if        rr-tip-ord           not  = "P"
       ...
       prn-liv-det-200.
      *              *-------------------------------------------------*
      *              * Numero ciclo di stampa dettaglio per partite    *
      *              * iva doppie a zero                               *
      *              *-------------------------------------------------*
           move      zero                 to   w-cpi-dop-ncc-det      .
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Incremento numero ciclo di stampa dettaglio per *
      *              * partite iva doppie                              *
      *              *-------------------------------------------------*
           add       1                    to   w-cpi-dop-ncc-det      .
       prn-liv-det-310.
      *              *-------------------------------------------------*
      *              * Deviazione secondo il numero ciclo              *
      *              *-------------------------------------------------*
           if        w-cpi-dop-ncc-det    =    01
                     go to prn-liv-det-320
           else if   w-cpi-dop-ncc-det    =    02
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se status a 02 si prepara in 'rf-cli' il    *
      *                  * record precedente, altrimenti si prepara    *
      *                  * il record attuale                           *
      *                  *---------------------------------------------*
           if        w-cpi-dop-sta-tus    =    02
                     move  w-cpi-dop-rec-pre
                                          to   rf-cli
           else      move  w-cpi-dop-rec-att
                                          to   rf-cli                 .
      *                  *---------------------------------------------*
      *                  * A stampa                                    *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se status diverso da 02 : uscita            *
      *                  *---------------------------------------------*
           if        w-cpi-dop-sta-tus    not  = 02
                     go to prn-liv-det-999.
      *                  *---------------------------------------------*
      *                  * Si prepara in 'rf-cli' il record attuale    *
      *                  *---------------------------------------------*
           move      w-cpi-dop-rec-att    to   rf-cli                 .
      *                  *---------------------------------------------*
      *                  * A stampa                                    *
      *                  *---------------------------------------------*
           go to     prn-liv-det-400.
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-800.
      *              *-------------------------------------------------*
      *              * Se e' una stampa per partita iva, e solo per le *
      *              * partite iva doppie : ricicla su ciclo successi- *
      *              * voi eventuale                                   *
      *              *-------------------------------------------------*
           if        rr-tip-ord           not  = "P"
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Eventuale lettura estremi lettera d'intenti *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se da stampare                     *
      *                      *-----------------------------------------*
           if        rr-sel-lic           =    1
                     go to prn-liv-det-p01-050.
           if        rr-snx-lic           not  = "S"
                     go to prn-liv-det-p01-050.
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           move      w-all-str-alf        to   w-stp-t01-col-001
                                              (w-stp-t01-cco-001)     .
      *                          *-------------------------------------*
      *                          * Segnale inizio partita iva doppia   *
      *                          *-------------------------------------*
      *                              *---------------------------------*
      *                              * Test se stampa necessaria       *
      *                              *---------------------------------*
           if        rr-tip-ord           not  = "P"
                     go to prn-liv-det-p01-190.
           if        rr-sel-pti           not  = "D"
                     go to prn-liv-det-p01-190.
           if        w-cpi-dop-ncc-det   not  = 01
                     go to prn-liv-det-p01-190.
           if        w-cpi-dop-sta-tus    not  = 02
                     go to prn-liv-det-p01-190.
      *                          *-------------------------------------*
      *                          * Concatenamento con partita Iva      *
      *                          *-------------------------------------*
           move      13                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      w-stp-t01-col-001
                    (w-stp-t01-cco-001)   to   w-all-str-cat (1)      .
           move      "*"                  to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                          *-------------------------------------*
      *                          * In area di stampa                   *
      ...
      *                              *---------------------------------*
           move      40                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      "Protocollo     :"   to   w-all-str-cat (1)      .
           move      rf-lic-prt-int       to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                          *-------------------------------------*
      *                          * In area di stampa                   *
      *                          *-------------------------------------*
      ...
      *                          * In area di stampa                   *
      *                          *-------------------------------------*
           move      rf-cli-cod-mne       to   w-stp-t01-col-004
                                              (w-stp-t01-cco-004)     .
      *                          *-------------------------------------*
      *                          * Oltre                               *
      *                          *-------------------------------------*
           go to     prn-liv-det-p01-440.
       prn-liv-det-p01-420.
      *                      *-----------------------------------------*
      *                      * Partita Iva cliente                     *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        rr-sel-pti           =    "E"
                     move  "ELENCO CLIENTI - Partita Iva errata o mancan
      -                    "te"           to   w-cnt-tit-des-tit
           else if   rr-sel-pti           =    "D"
                     move  "ELENCO CLIENTI - Partita Iva doppia"
                                          to   w-cnt-tit-des-tit
           else      move  "ELENCO CLIENTI"
                                          to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
