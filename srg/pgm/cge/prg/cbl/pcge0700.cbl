      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella ubicazioni cespiti         *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-aep occurs 2.
               ...
               10  w-tes-des-ubi          pic  x(40)                  .
               10  w-tes-dpz-ubi          pic  9(02)                  .
               10  w-tes-alx-exp.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice ubicazione             *
      *    *-----------------------------------------------------------*
       pmt-cod-ubi-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo chiave : Codice ubicazione             *
      *    *-----------------------------------------------------------*
       acc-cod-ubi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-ubi        to   w-cod-mne-zuc-cod      .
           move      04                   to   w-cod-mne-zuc-lin      .
           move      30                   to   w-cod-mne-zuc-pos      .
           move      07                   to   w-cod-mne-zuc-dln      .
           move      30                   to   w-cod-mne-zuc-dps      .
           move      "<B"                 to   v-edm                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice ubicazione          *
      *    *-----------------------------------------------------------*
       vis-cod-ubi-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *                  * Descrizione classe                          *
      *                  *---------------------------------------------*
           perform   acc-des-ubi-000      thru acc-des-ubi-999        .
           if        v-key                =    "NXSC"
      ...
      *                  * % ritenuta                                  *
      *                  *---------------------------------------------*
           perform   acc-per-ubi-000      thru acc-per-ubi-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *              * Descrizione classe                              *
      *              *-------------------------------------------------*
           perform   vis-des-ubi-000      thru vis-des-ubi-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % ritenuta                                      *
      *              *-------------------------------------------------*
           perform   vis-per-ubi-000      thru vis-per-ubi-999        .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     vis-tes-reg-999.
      ...
      *              * Descrizione classe                              *
      *              *-------------------------------------------------*
           perform   pmt-des-ubi-000      thru pmt-des-ubi-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % ritenuta                                      *
      *              *-------------------------------------------------*
           perform   pmt-per-ubi-000      thru pmt-per-ubi-999        .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     pmt-tes-reg-999.
      ...
      *                  *---------------------------------------------*
           if        w-tes-des-ubi (1)    =    spaces
                     go to acc-des-ubi-100.
       acc-des-ubi-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  * l'ordinamento                               *
      *                  *---------------------------------------------*
           move      w-tes-des-ubi (1)    to   w-all-str-alf          .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-ubi        to   rf-zuc-cod-ubi         .
           move      "pgm/cge/fls/ioc/obj/iofzuc"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [zuc]                        *
      *                          *-------------------------------------*
           move      rf-zuc-ide-dat       to   w-tes-ide-dat (1)      .
           move      rf-zuc-ide-ute       to   w-tes-ide-ute (1)      .
           move      rf-zuc-ide-fas       to   w-tes-ide-fas (1)      .
           move      rf-zuc-des-ubi       to   w-tes-des-ubi (1)      .
           move      rf-zuc-des-key       to   w-tes-des-key (1)      .
           move      rf-zuc-cod-mne       to   w-tes-cod-mne (1)      .
           move      rf-zuc-dpz-ubi       to   w-tes-dpz-ubi (1)      .
           move      rf-zuc-alx-exp       to   w-tes-alx-exp (1)      .
       rou-let-reg-800.
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zuc]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zuc-000      thru wrt-rec-zuc-999        .
      ...
      *                      * Rewrite record [zuc]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zuc-000      thru rew-rec-zuc-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zuc]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zuc-000      thru del-rec-zuc-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zuc]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zuc-000.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-zuc-ide-dat         .
           move      s-ute                to   rf-zuc-ide-ute         .
           move      s-fas                to   rf-zuc-ide-fas         .
           move      w-tes-cod-mne (1)    to   rf-zuc-cod-mne         .
           move      w-tes-des-key (1)    to   rf-zuc-des-key         .
           move      w-tes-des-ubi (1)    to   rf-zuc-des-ubi         .
           move      w-tes-dpz-ubi (1)    to   rf-zuc-dpz-ubi         .
           move      w-tes-alx-exp (1)    to   rf-zuc-alx-exp         .
       cmp-rec-zuc-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zuc]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zuc-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zuc-000      thru cmp-rec-zuc-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
