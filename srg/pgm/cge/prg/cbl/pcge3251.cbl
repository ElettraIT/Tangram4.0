      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma cge325 : *
      *                                                                *
      ...
      *            *---------------------------------------------------*
               10  srt-des-pdc            pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Tipo conto                                        *
      *            *---------------------------------------------------*
               10  srt-tip-cnt            pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Record [mgt]                                      *
      *            *---------------------------------------------------*
               10  srt-rec-mgt.
      ...
      *            * Numero protocollo                                 *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-prt      pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Numero progressivo                                *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-prg      pic  9(05)                  .
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
      *            * Mnemonico contropartita                           *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-mne      pic  x(10)                  .
      *            *---------------------------------------------------*
      *            * Tipo contropartita                                *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-tip      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Commento eventuale                                *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-com      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Commento eventuale legato al conto stesso         *
      *            *---------------------------------------------------*
               10  w-det-cpt-mgr-coc      pic  x(40)                  .
      *        *-------------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-mgr                 .
      *                      *-----------------------------------------*
      *                      * Se Start errata : a elemento successivo *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-mgr                 .
      *                      *-----------------------------------------*
      *                      * Se fine file : a elemento successivo    *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-srt-inp-200.
       stp-srt-inp-430.
      ...
      *                  *---------------------------------------------*
           if        rf-mgr-dat-reg       >    rr-dat-max      or
                     rf-mgr-cod-pdc       not  = rf-pdc-cod-pdc
                     go to stp-srt-inp-200.
       stp-srt-inp-450.
      *                  *---------------------------------------------*
      *                  * Normalizzazione [mgt]                       *
      ...
      *                      * Descrizione sottoconto                  *
      *                      *-----------------------------------------*
           move      rf-pdc-des-pdc       to   srt-des-pdc            .
      *                      *-----------------------------------------*
      *                      * Tipo conto                              *
      *                      *-----------------------------------------*
           move      rf-pdc-tip-cnt       to   srt-tip-cnt            .
      *                      *-----------------------------------------*
      *                      * Record [mgt]                            *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-rot-l01-cod-pdc    to   w-par-cod-prt          .
           move      srt-des-pdc          to   w-par-des-prt          .
           move      srt-tip-cnt          to   w-sad-tip-cnt          .
      ...
      *              *-------------------------------------------------*
           move      "G"                  to   w-det-cpt-mgr-tco      .
           move      srt-cod-pdc          to   w-det-cpt-mgr-cco      .
           move      srt-dat-reg          to   w-det-cpt-mgr-drg      .
           move      srt-num-prt          to   w-det-cpt-mgr-prt      .
           move      srt-num-prg          to   w-det-cpt-mgr-prg      .
      ...
      *                      *-----------------------------------------*
      *                      * Test su tipo contropartita              *
      *                      *-----------------------------------------*
           if        w-det-cpt-mgr-tip    =    "G"
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      56                   to   p-pos                  .
           move      w-det-cpt-mgr-cod    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * A stampa mnemonico                          *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      65                   to   p-pos                  .
           move      w-det-cpt-mgr-mne    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-300.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      "PARTITARIO DI CONTABILITA' GENERALE DAL"
                                          to   w-all-str-cat (1)      .
      ...
      *                          *-------------------------------------*
           move      w-sad-ann-pre        to   rf-mgs-ann-ese         .
           move      "G"                  to   rf-mgs-tip-rec         .
           move      w-sad-cod-pdc        to   rf-mgs-cod-con         .
      *                          *-------------------------------------*
      *                          * Lettura file [mgs]                  *
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   w-det-cpt-mgr-mne      .
           move      spaces               to   w-det-cpt-mgr-tip      .
           move      spaces               to   w-det-cpt-mgr-com      .
           move      spaces               to   w-det-cpt-mgr-coc      .
      ...
      *              *-------------------------------------------------*
       ...
       det-cpt-mgr-400.
      *              *-------------------------------------------------*
      *              * Bufferizzazione commento del conto              *
      *              *-------------------------------------------------*
           if        rf-mgr-tip-arc       =    w-det-cpt-mgr-tco and
                     rf-mgr-cod-pdc       =    w-det-cpt-mgr-cco and
                     rf-mgr-num-prg       =    w-det-cpt-mgr-prg
                     move  rf-mgr-com-rig to   w-det-cpt-mgr-coc
      ...
      *                  *---------------------------------------------*
           if        rf-mgr-tip-arc       =    "G"
                     go to det-cpt-mgr-620
           else if   rf-mgr-tip-arc       =    "C"
      ...
      *                      *-----------------------------------------*
      *                      * Bufferizzazione mnemonico               *
      *                      *-----------------------------------------*
           move      rf-pdc-cod-mne       to   w-det-cpt-mgr-mne      .
           move      rf-mgr-tip-arc       to   w-det-cpt-mgr-tip      .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione commento                *
      *                      *-----------------------------------------*
           move      rf-mgr-com-rig       to   w-det-cpt-mgr-com      .
      *                      *-----------------------------------------*
      *                      * A riciclo                               *
      ...
      *                      * Codice conto                            *
      *                      *-----------------------------------------*
           move      rf-mgr-cod-arc       to   w-det-cpt-mgr-cod      .
      *                      *-----------------------------------------*
      *                      * Normalizzazione                         *
      *                      *-----------------------------------------*
           move      "NO"                 to   f-ope                  .
      ...
      *                      * Bufferizzazione mnemonico               *
      *                      *-----------------------------------------*
           move      rf-cli-cod-mne       to   w-det-cpt-mgr-mne      .
           move      rf-mgr-tip-arc       to   w-det-cpt-mgr-tip      .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione commento                *
      *                      *-----------------------------------------*
      ...
      *                      * Bufferizzazione mnemonico               *
      *                      *-----------------------------------------*
           move      rf-fnt-cod-mne       to   w-det-cpt-mgr-mne      .
           move      rf-mgr-tip-arc       to   w-det-cpt-mgr-tip      .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione commento                *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se il commento relativo al conto in esame   *
      *                  * e' diverso da spaces, si pone quello come   *
      *                  * commento di uscita solo se movimento non    *
      *                  * Iva                                         *
      *                  *---------------------------------------------*
           if        w-det-cpt-mgr-coc    not  = spaces and
                     rf-mgt-tip-iva       =    "0"
                     move  w-det-cpt-mgr-coc
                                          to   w-det-cpt-mgr-com      .
      *                  *---------------------------------------------*
      *                  * Se durante la ricerca e' stato trovato un   *
      *                  * numero di righe superiore a 1 si esce con   *
      *                  * il codice contropartita a zero              *
      *                  *---------------------------------------------*
           if        w-det-cpt-mgr-ctr    >    1
                     move  zero           to   w-det-cpt-mgr-cod      .
       det-cpt-mgr-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
