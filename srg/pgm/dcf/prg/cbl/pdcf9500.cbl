      ...
      *                                   Fase:    dcf950              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 29/01/03    *
      *                       Ultima revisione:    NdK del 08/04/22    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pdcf9501:        *
      *                                                                *
      *                    Stampa di controllo codici prodotto per la  *
      *                    cada produttrice                            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA CONTROLLO CODICI CASA PRODUTTRICE"       .
           ...
           05  w-exp-tip-slp.
               ...
               10  w-exp-tip-slp-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "ad ogni Sottogruppo "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Si/no stampa descrizioni nulle             *
      *        *-------------------------------------------------------*
           05  w-exp-snx-cdp.
               10  w-exp-snx-cdp-num      pic  9(02)       value 3    .
               10  w-exp-snx-cdp-lun      pic  9(02)       value 40   .
               10  w-exp-snx-cdp-tbl.
                   15  filler             pic  x(40) value
                            "Nessuna selezione                       ".
                   15  filler             pic  x(40) value
                            "solo prodotti Senza codice              ".
                   15  filler             pic  x(40) value
                            "solo prodotti con codice Doppio         ".
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-450.
      *                  *---------------------------------------------*
      *                  * Si/no stampa descrizioni nulle              *
      *                  *---------------------------------------------*
           perform   acc-snx-cdp-000      thru acc-snx-cdp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Si/no stampa descrizioni nulle                  *
      *              *-------------------------------------------------*
           perform   pmt-snx-cdp-000      thru pmt-snx-cdp-999        .
       pmt-ric-sel-999.
           exit.
      ...
      *    *===========================================================*
      *    * Prompt : Si/no selezioni su codici                        *
      *    *-----------------------------------------------------------*
       pmt-snx-cdp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      12                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su codici per la :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
      ...
      *    *-----------------------------------------------------------*
       acc-tip-rec-000.
      *              *-------------------------------------------------*
      *              * Forzatura a 03 del Tipo record                  *
      *              *-------------------------------------------------*
           move      03                   to   rr-tip-rec             .
       acc-tip-rec-999.
           exit.
      ...
      *                      *-----------------------------------------*
       ...
       acc-cod-fnt-480.
      *                  *---------------------------------------------*
      *                  * Se codice fornitore impostato non a zero    *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Si/no selezioni su codici                  *
      *    *-----------------------------------------------------------*
       acc-snx-cdp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-cdp-lun    to   v-car                  .
           move      w-exp-snx-cdp-num    to   v-ldt                  .
           move      "NSD#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      12                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-cdp-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
           move      rr-snx-cdp           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-cdp-999.
       acc-snx-cdp-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-cdp             .
       acc-snx-cdp-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       reg-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione Si/no stampa descrizioni nulle  *
      *              *-------------------------------------------------*
           if        rr-snx-cdp           not  = zero
                     go to reg-ric-sel-999.
           move      01                   to   rr-snx-cdp             .
       reg-ric-sel-999.
           exit.
