      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pdcf9500 *
      *                                                                *
      ...
      *            *---------------------------------------------------*
      *            * Descrizione prodotto di magazzino                 *
      *            *---------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Work area per bufferizzazione eventuale elemento          *
      *    *-----------------------------------------------------------*
       01  w-buf-cdp-pdt.
      *        *-------------------------------------------------------*
      *        * Contatore elementi letti                              *
      *        *-------------------------------------------------------*
           05  w-buf-cdp-pdt-ctr          pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Castelletto codici e descrizioni prodotto             *
      *        *-------------------------------------------------------*
           05  w-buf-cdp-pdt-c01          pic  9(02)                  .
           05  w-buf-cdp-pdt-max          pic  9(02) value 20         .
           05  w-buf-cdp-pdt-cst.
               10  w-buf-cdp-pdt-ele occurs 20.
                   15  w-buf-cdp-pdt-alf  pic  x(14)                  .
                   15  w-buf-cdp-pdt-des  pic  x(40)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-rot-l02.
               10  w-rot-l02-cod-dcf      pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * 1. livello di rottura                                 *
      *        *-------------------------------------------------------*
           05  w-rot-l01.
               10  w-rot-l01-cdp-pdt      pic  x(40)                  .
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale record [aaq]                *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcf/fls/ioc/obj/iofaaq"
                                          to   s-pat                  .
      ...
      *                          *-------------------------------------*
       ...
       stp-srt-inp-500.
      *                  *---------------------------------------------*
      *                  * Test su selettore su codice prodotto per la *
      *                  * casa produttrice                            *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se tutti                                *
      ...
      *                      *-----------------------------------------*
      *                      * Se solo senza codice prodotto           *
      *                      *-----------------------------------------*
           if        rr-snx-cdp           =    02 and
                     rf-aaq-cdp-pdt       not  = spaces
                     go to stp-srt-inp-200.
      *                      *-----------------------------------------*
      *                      * Se solo con codice doppio               *
      *                      *-----------------------------------------*
           if        rr-snx-cdp           =    03 and
      ...
      *                  * Chiave di sort                              *
      *                  *---------------------------------------------*
           move      rf-aaq-dcf-pfz       to   srt-cod-dcf            .
           move      rf-aaq-cdp-pdt       to   srt-cdp-pdt            .
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      srt-cod-dcf          to   w-rot-l02-cod-dcf      .
           move      srt-cdp-pdt          to   w-rot-l01-cdp-pdt      .
       prn-cmp-rot-999.
           exit.
      ...
      *              *-------------------------------------------------*
       ...
       prn-ini-lr1-100.
      *              *-------------------------------------------------*
      *              * Se richiesta stampa docici doppi                *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        rr-snx-cdp           not  = 03
                     go to prn-ini-lr1-900.
      *                  *---------------------------------------------*
      *                  * Normalizzazione contatore codici            *
      *                  *---------------------------------------------*
           move      zero                 to   w-buf-cdp-pdt-ctr      .
      *                  *---------------------------------------------*
      *                  * Normalizzazione castelletto                 *
      *                  *---------------------------------------------*
           move      zero                 to   w-buf-cdp-pdt-c01      .
       prn-ini-lr1-120.
           add       1                    to   w-buf-cdp-pdt-c01      .
           if        w-buf-cdp-pdt-c01    >    w-buf-cdp-pdt-max
                     go to prn-ini-lr1-900.
           move      spaces               to   w-buf-cdp-pdt-alf
                                              (w-buf-cdp-pdt-c01)     .
           move      spaces               to   w-buf-cdp-pdt-des
                                              (w-buf-cdp-pdt-c01)     .
      ...
      *                  * Test sul contatore                          *
      *                  *---------------------------------------------*
           if        w-buf-cdp-pdt-ctr    not  > 1
                     go to prn-fin-lr1-900.
       prn-fin-lr1-100.
      *              *-------------------------------------------------*
      *              * Ciclo di stampa elementi bufferizzati           *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-cdp-pdt-c01      .
       prn-fin-lr1-200.
           add       1                    to   w-buf-cdp-pdt-c01      .
           if        w-buf-cdp-pdt-c01    >    w-buf-cdp-pdt-ctr
                     go to prn-fin-lr1-900.
           if        w-buf-cdp-pdt-c01    >    w-buf-cdp-pdt-max
                     go to prn-fin-lr1-900.
       prn-fin-lr1-300.
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
      *              * Se primo elemento doppia interlinea             *
      *              *-------------------------------------------------*
           if        w-buf-cdp-pdt-c01    >    1
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      w-buf-cdp-pdt-alf
                    (w-buf-cdp-pdt-c01)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      057                  to   p-pos                  .
           move      w-rot-l01-cdp-pdt    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-fin-lr1-800.
      *              *-------------------------------------------------*
      ...
      *                  * Contatore codici                            *
      *                  *---------------------------------------------*
           add       1                    to   w-buf-cdp-pdt-ctr      .
           if        w-buf-cdp-pdt-ctr    >    w-buf-cdp-pdt-max
                     move  w-buf-cdp-pdt-max
                                          to   w-buf-cdp-pdt-ctr      .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione elemento                    *
      *                  *---------------------------------------------*
           move      srt-alf-mag          to   w-buf-cdp-pdt-alf
                                              (w-buf-cdp-pdt-ctr)     .
           move      srt-des-mag          to   w-buf-cdp-pdt-des
                                              (w-buf-cdp-pdt-ctr)     .
      *                  *---------------------------------------------*
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
      ...
      *                  * o meno della descrizione per il fornitore   *
      *                  *---------------------------------------------*
           if        p-res                >    1
      ...
      *                  *---------------------------------------------*
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      w-rot-l02-cod-dcf    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Lettura record [dcf]                        *
      *                  *---------------------------------------------*
           move      w-rot-l02-cod-dcf    to   w-let-arc-dcf-fnt      .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-let-arc-dcf-rag    to   w-all-str-cat (3)      .
           move      "-"                  to   w-all-str-cat (4)      .
           move      w-let-arc-dcf-lng    to   w-all-str-cat (5)      .
      ...
      *              *-------------------------------------------------*
      ...
      -              "no    Codice prodotto per la casa produttrice "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
