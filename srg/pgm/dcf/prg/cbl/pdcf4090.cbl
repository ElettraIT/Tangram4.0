      ...
      *                                   Fase:    dcf409              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 10/10/03    *
      *                       Ultima revisione:    NdK del 10/10/03    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampe di controllo anagrafica fornitori    *
      *                                                                *
      *                    Main                                        *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase dcf409, nella sua globalita', esegue i tipi di inter-  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * RSDACC      Controllo ragioni sociali diverse tra anagrafica   *
      *             contabile e commerciale                            *
      *                                                                *
      * CODMAN      Controllo fornitori con codici mancanti            *
      *                                                                *
      * CODERR      Controllo fornitori con codici errati              *
      *                                                                *
      * CORBLO      Controllo fornitori con blocco ordini              *
      *                                                                *
      * UTZERR      Controllo fornitori con utenze errate              *
      *                                                                *
      * ANNFNT      Controllo fornitori con annotazioni                *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPE DI CONTROLLO ANAGRAFICA FORNITORI"       .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'dcf409'           *
      *              *-------------------------------------------------*
           perform   fas-dcf-409-000      thru fas-dcf-409-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *              * stazione                                        *
      *              *-------------------------------------------------*
           move      "Controllo ragioni sociali commerciali e contabili 
      -              "           RSDACC     pdcf409a  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "Controllo fornitori mancanti di codici            
      -              "           CODMAN     pdcf409b  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "Controllo fornitori con codici tabelle errati     
      -              "           CODERR     pdcf409c  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "Controllo fornitori con blocco ordini             
      -              "           CORBLO     pdcf409d  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *       
           move      "Controllo fornitori con utenze errate             
      -              "           UTZERR     pdcf409e  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *       
           move      "Controllo fornitori con annotazioni               
      -              "           ANNFNT     pdcf409f  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-200.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-dcf-409-300
           else if   v-key                =    "PRSC"
                     go to fas-dcf-409-400
           else if   v-key                =    "NXSC"
                     go to fas-dcf-409-450
           else if   v-key                =    "DOWN"
                     go to fas-dcf-409-500
           else if   v-key                =    "EXIT"
                     go to fas-dcf-409-250
           else      go to fas-dcf-409-200.
       fas-dcf-409-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-dcf-409-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-dcf-409-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-nes
                     go to fas-dcf-409-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-dcf-409-350
           else      go to fas-dcf-409-322.
       fas-dcf-409-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-dcf-409-630
           else if   v-key                =    "PRSC"
                     go to fas-dcf-409-600
           else if   v-key                =    "NXSC"
                     go to fas-dcf-409-610
           else if   v-key                =    "UP  "
                     go to fas-dcf-409-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-dcf-409-650
           else if   v-key                =    "EXIT"
                     go to fas-dcf-409-660
           else      go to fas-dcf-409-570.
       fas-dcf-409-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-dcf-409-626
           else      go to fas-dcf-409-628.
       fas-dcf-409-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-dcf-409-636
           else      go to fas-dcf-409-638.
       fas-dcf-409-636.
      *                          *-------------------------------------*
