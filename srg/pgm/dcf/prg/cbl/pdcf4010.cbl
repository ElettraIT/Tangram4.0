      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio [dcf]            *
      *                                                                *
      *                    Anagrafico commerciale fornitori            *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
      *            * Si/No funzionamento cidcfco interrogazione        *
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-rip-str.
      *        *-------------------------------------------------------*
      *        * Work-area per ripristino Start su [dcf]               *
      *        *-------------------------------------------------------*
           05  w-rip-str-dcf.
      *            *---------------------------------------------------*
      *            * Flag di End-of-file                               *
      *            *---------------------------------------------------*
               10  w-rip-str-dcf-eof      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Parametri di Start                                *
      *            *---------------------------------------------------*
               10  w-rip-str-dcf-cod      pic  9(07)                  .
               10  w-rip-str-dcf-rag      pic  x(40)                  .
               10  w-rip-str-dcf-dpz      pic  x(04)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp-rec.
           05  w-exp-rec-dcf-lin          pic  9(02) value 11         .
           05  w-exp-rec-dcf-ctr          pic  9(02)                  .
      ...
      *              *-------------------------------------------------*
      *              * Test se tipo esecuzione cidcfco                 *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-dcf-cod      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-dcf-ope      .
           move      rr-cod-max           to   w-cod-mne-dcf-cod      .
           move      14                   to   w-cod-mne-dcf-lin      .
           move      52                   to   w-cod-mne-dcf-pos      .
           move      zero                 to   w-cod-mne-dcf-rln      .
           move      zero                 to   w-cod-mne-dcf-rps      .
           move      zero                 to   w-cod-mne-dcf-vln      .
           move      zero                 to   w-cod-mne-dcf-vps      .
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su archivio fornitori commerciali :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-rip-str-dcf-eof      .
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo ordinamento     *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "C"
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-dcf-cod-fnt         .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start per mnemonico su file [fnt]           *
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun fornitore commerciale entro i limiti assegn
      -              "ati !"              to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *                  * Selezione su codice                         *
      *                  *---------------------------------------------*
           if        rf-dcf-cod-fnt       <    rr-cod-min or
                     rf-dcf-cod-fnt       >    rr-cod-max
                     go to qry-sel-rec-900.
      *                  *---------------------------------------------*
      *                  * Selezione su ragione sociale                *
      *                  *---------------------------------------------*
           if        rf-dcf-rag-key       <    rr-rag-min or
                     rf-dcf-rag-key       >    rr-rag-max
                     go to qry-sel-rec-900.
      *                  *---------------------------------------------*
      *                  * Lettura record [fnt]                        *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-cmp-rot-000.
           move      rf-dcf-rag-soc       to   w-rot-l01-rag-soc      .
       qry-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-dcf-via-dcf       to   w-buf-via-fnt          .
           move      rf-dcf-loc-dcf       to   w-buf-loc-fnt          .
      ...
      *                  *---------------------------------------------*
      *                  * Routine per Expand su record [dcf]          *
      *                  *---------------------------------------------*
           perform   exp-rec-dcf-000      thru exp-rec-dcf-999        .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                not  = "FIND"
                     go to qry-trt-fun-999.
      *                  *---------------------------------------------*
      *                  * Routine per Find su record [dcf]            *
      *                  *---------------------------------------------*
           perform   fnd-dpz-dcf-000      thru fnd-dpz-dcf-999        .
      *                  *---------------------------------------------*
      *                  * Normalizzazione function key                *
      *                  *---------------------------------------------*
           move      spaces               to   v-key                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "                       ANAGRAFICHE COMMERCIALI FOR
      -              "NITORI                        "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *              *-------------------------------------------------*
           if        w-rip-str-dcf-eof    not  = spaces
                     go to exp-rec-dcf-100.
           move      rf-dcf-cod-fnt       to   w-rip-str-dcf-cod      .
           move      rf-dcf-rag-key       to   w-rip-str-dcf-rag      .
           move      rf-dcf-dpz-fnt       to   w-rip-str-dcf-dpz      .
       exp-rec-dcf-100.
      *              *-------------------------------------------------*
      *              * Video in 'OFF'                                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "CODFNT    "         to   f-key                  .
           move      v-cnt                to   w-mpn                  .
           move      w-mpn-cod-fnt        to   rf-dcf-cod-fnt         .
           move      spaces               to   rf-dcf-dpz-fnt
           move      "pgm/dcf/fls/ioc/obj/iofdcf"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                      * Ciclo 1..7                              *
      *                      *-----------------------------------------*
           move      zero                 to   w-exp-rec-dcf-ctr      .
       exp-rec-dcf-720.
           add       1                    to   w-exp-rec-dcf-ctr      .
           if        w-exp-rec-dcf-ctr    >    07 or
                     w-exp-rec-dcf-ctr    >    d-con-arc-num-ele
                     go to exp-rec-dcf-740.
      *                      *-----------------------------------------*
      *                      * Tipo utenza                             *
      *                      *-----------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
           move      w-exp-rec-dcf-lin    to   v-lin                  .
           add       w-exp-rec-dcf-ctr    to   v-lin                  .
           move      03                   to   v-pos                  .
      *
           if        d-con-arc-tip-con
                    (w-exp-rec-dcf-ctr)   =    "SEP"
                     move  all "-"        to   v-alf
      ...
      *
           ...
           move      rf-dcf-ide-ute       to   w-all-str-cat (2)      .
      ...
      *
           move      03                   to   w-all-str-num          .
           move      w-all-str-alf        to   w-all-str-cat (1)      .
           move      rf-dcf-ide-fas       to   w-all-str-cat (2)      .
           move      "]"                  to   w-all-str-cat (3)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
      *                      *-----------------------------------------*
      *                      * Visualizzazione                         *
      ...
      *                  *---------------------------------------------*
           if        w-rip-str-dcf-eof    not  = spaces
                     go to exp-rec-dcf-999.
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo di Start    *
      *                  * attualmente attivo                          *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    "C"
                     go to exp-rec-dcf-910
           else if   rr-tip-ord           =    "R"
                     go to exp-rec-dcf-920
           else if   rr-tip-ord           =    "M"
                     go to exp-rec-dcf-999.
       exp-rec-dcf-910.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODFNT    "         to   f-key                  .
           move      w-rip-str-dcf-cod    to   rf-dcf-cod-fnt         .
           move      w-rip-str-dcf-dpz    to   rf-dcf-dpz-fnt         .
           move      "pgm/dcf/fls/ioc/obj/iofdcf"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-dcf                 .
           if        f-sts                not  = e-not-err
                     go to exp-rec-dcf-990.
           go to     exp-rec-dcf-980.
       exp-rec-dcf-920.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "RAGKEY    "         to   f-key                  .
           move      w-rip-str-dcf-rag    to   rf-dcf-rag-key         .
           move      w-rip-str-dcf-cod    to   rf-dcf-cod-fnt         .
           move      w-rip-str-dcf-dpz    to   rf-dcf-dpz-fnt         .
           move      "pgm/dcf/fls/ioc/obj/iofdcf"
                                          to   s-pat                  .
      ...
      *    *===========================================================*
      *    * Routine per Find dipendenze fornitore commerciale         *
      *    *-----------------------------------------------------------*
       fnd-dpz-dcf-000.
      *              *-------------------------------------------------*
      *              * Salvataggio parametri per Start su [dcf]        *
      *              *-------------------------------------------------*
           if        w-rip-str-dcf-eof    not  = spaces
                     go to fnd-dpz-dcf-100.
           move      rf-dcf-cod-fnt       to   w-rip-str-dcf-cod      .
           move      rf-dcf-rag-key       to   w-rip-str-dcf-rag      .
           move      rf-dcf-dpz-fnt       to   w-rip-str-dcf-dpz      .
       fnd-dpz-dcf-100.
      *              *-------------------------------------------------*
      *              * Scrittura variabile di i.p.c. per codice        *
      *              *-------------------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "cod-fnt"            to   s-var                  .
           move      "+"                  to   s-dop                  .
      ...
      *                  *---------------------------------------------*
           if        w-rip-str-dcf-eof    not  = spaces
                     go to fnd-dpz-dcf-999.
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo di Start    *
      *                  * attualmente attivo                          *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    "C"
                     go to fnd-dpz-dcf-910
           else if   rr-tip-ord           =    "R"
                     go to fnd-dpz-dcf-920
           else if   rr-tip-ord           =    "M"
                     go to fnd-dpz-dcf-999.
       fnd-dpz-dcf-910.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-dcf                 .
           if        f-sts                not  = e-not-err
                     go to fnd-dpz-dcf-990.
           go to     fnd-dpz-dcf-980.
       fnd-dpz-dcf-920.
      *                  *---------------------------------------------*
      *                  * Se start per ragione sociale                *
      *                  *---------------------------------------------*
