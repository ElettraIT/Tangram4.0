      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su formati [aaf] per i       *
      ...
      *    *-----------------------------------------------------------*
       01  w-prs.
      *        *-------------------------------------------------------*
      *        * Personalizzazioni relative al tipo visualizzazione    *
      *        *-------------------------------------------------------*
           05  w-prs-tip-vis.
               10  w-prs-tip-vis-tip      pic  9(02)                  .
           ...
           05  w-det-txt-pdx.
               ...
               10  w-det-txt-pdx-txt.
                   15  w-det-txt-pdx-rig occurs 10
                                          pic  x(40)                  .
      *        *-------------------------------------------------------*
      *        * Work per scansione linee lette                        *
      *        *-------------------------------------------------------*
           05  w-det-txt-let.
               10  w-det-txt-let-ctr      pic  9(02)                  .
      ...
      *                  *---------------------------------------------*
           perform   prs-tip-vis-000      thru prs-tip-vis-999        .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-num          to   rr-tip-mag
           else      move  zero           to   rr-tip-mag             .
           move      "CV"                 to   s-ope                  .
      *                  *---------------------------------------------*
      *                  * Codice prodotto                             *
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-num          to   rr-num-pro
           else      move  zero           to   rr-num-pro             .
      *                  *---------------------------------------------*
      *                  * Codice fornitore                            *
      *                  *---------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-num          to   rr-cod-dcf
           else      move  zero           to   rr-cod-dcf             .
      *              *-------------------------------------------------*
      *              * Test su lettura variabili di i.p.c.             *
      *              *-------------------------------------------------*
           if        rr-tip-mag           =    zero  or
                     rr-num-pro           =    zero  or
                     rr-cod-dcf           =    zero
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/dcf/lst/dcf501[tip-vis]"
                                          to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-tip-vis
           else      move  spaces         to   w-prs-tip-vis          .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-tip-vis-tip    not  = 02
                     move  01             to   w-prs-tip-vis-tip      .
       prs-tip-vis-999.
           exit.
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-tip-mag           to   rf-aaf-tip-mag         .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun formato entro i limiti assegnati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [aaf]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcf/fls/ioc/obj/iofaaf"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           if        rf-aaf-tip-mag       not  = rr-tip-mag
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-tst-max-999.
      *              *-------------------------------------------------*
      *              * Test su Codice magazzino                        *
      *              *-------------------------------------------------*
           if        rf-aaf-num-pro       not  = rr-num-pro
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-tst-max-999.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-prs-tip-vis-tip    =    01
                     perform qry-liv-det-001-000
                                          thru qry-liv-det-001-999
           else if   w-prs-tip-vis-tip    =    02
                     perform qry-liv-det-002-000
                                          thru qry-liv-det-002-999
           else      perform qry-liv-det-001-000
                                          thru qry-liv-det-001-999    .
       qry-liv-det-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           if        rf-aaf-fda-pif       =    spaces
                     move  "(Formato base)"
                                          to   v-alf
           else      move  rf-aaf-fda-pif to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-aaf-fda-pif       to   w-mpn-fda-pif          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Si/no prezzo per quantita'                  *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      01                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      09                   to   v-car                  .
           move      c-dec                to   v-dec                  .
           add       rf-aaf-dec-prz       to   v-dec                  .
           move      spaces               to   v-sgn                  .
           move      "BG"                 to   v-edm                  .
           move      v-lnr                to   v-lin                  .
           move      17                   to   v-pos                  .
           move      rf-aaf-prz-pes (1)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Si/no trasformazione Udm                    *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      01                   to   v-car                  .
           move      v-lnr                to   v-lin                  .
           move      31                   to   v-pos                  .
           move      rf-aaf-snx-tum       to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Udm usata dal fornitore                     *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
           move      v-lnr                to   v-lin                  .
           move      34                   to   v-pos                  .
           move      rf-aaf-umf-tum       to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Numero decimali utilizzato dal fornitore    *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      01                   to   v-car                  .
      ...
      *
           move      rf-aaf-cod-dcf       to   w-det-txt-pdx-arc      .
           move      rf-aaf-num-pro       to   w-det-txt-pdx-mag      .
           move      rf-aaf-fda-pif       to   w-det-txt-pdx-fda      .
      ...
      *                      *-----------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      66                   to   v-pos                  .
           move      w-det-txt-pdx-txt    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-001-999.
           exit.
      ...
      *    * Interrogazione : Livello di dettaglio                     *
      *    *                                                           *
      *    * Visualizzazione orientata al prezzo                       *
      *    *-----------------------------------------------------------*
       qry-liv-det-002-000.
      *              *-------------------------------------------------*
      *              * Test se righe residue sufficienti               *
      ...
      *                      *-----------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      31                   to   v-pos                  .
           move      w-det-txt-pdx-rig (01)
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-002-500.
      *                  *---------------------------------------------*
      *                  * Note - righe successive alla prima          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se righe successive                *
      *                      *-----------------------------------------*
           if        w-det-txt-pdx-rig (02)
                                          =    spaces
                     go to qry-liv-det-002-600.
      *                      *-----------------------------------------*
      *                      * Ciclo di visualizzazione                *
      *                      *-----------------------------------------*
           move      01                   to   w-det-txt-let-ctr      .
       qry-liv-det-002-520.
           add       01                   to   w-det-txt-let-ctr      .
           if        w-det-txt-let-ctr    >    10
                     go to qry-liv-det-002-600.
           if        w-det-txt-pdx-rig
                    (w-det-txt-let-ctr)   =    spaces
                     go to qry-liv-det-002-600.
      *                      *-----------------------------------------*
      *                      * Test se righe residue sufficienti       *
      *                      *-----------------------------------------*
           if        v-res                >    1
                     go to qry-liv-det-002-540.
      *                          *-------------------------------------*
      *                          * Intestazione pagina                 *
      *                          *-------------------------------------*
           perform   int-pag-sta-000      thru int-pag-sta-999        .
      ...
      *                      *-----------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-det-txt-pdx-rig
                    (w-det-txt-let-ctr)   to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Riciclo                                 *
      *                      *-----------------------------------------*
           go to     qry-liv-det-002-520.
       qry-liv-det-002-600.
      *                  *---------------------------------------------*
      *                  * Scaglioni quantita' prezzo                  *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se da visualizzare                 *
      *                      *-----------------------------------------*
           if        rf-aaf-tap-pes       not  = 02
                     go to qry-liv-det-002-800.
      *                      *-----------------------------------------*
      *                      * Ciclo di visualizzazione                *
      *                      *-----------------------------------------*
           move      zero                 to   w-det-txt-let-ctr      .
       qry-liv-det-002-620.
           add       01                   to   w-det-txt-let-ctr      .
           if        w-det-txt-let-ctr    >    6
                     go to qry-liv-det-002-800.
           if        rf-aaf-qta-pes
                    (w-det-txt-let-ctr)   =    zero
                     go to qry-liv-det-002-800.
      *                      *-----------------------------------------*
      *                      * Test se righe residue sufficienti       *
      *                      *-----------------------------------------*
           if        v-res                >    2
                     go to qry-liv-det-002-640.
      *                          *-------------------------------------*
      *                          * Intestazione pagina                 *
      *                          *-------------------------------------*
           perform   int-pag-sta-000      thru int-pag-sta-999        .
      ...
      *                      * Fincatura                               *
      *                      *-----------------------------------------*
           if        w-det-txt-let-ctr    not  = 1
                     go to qry-liv-det-002-660.
      *                      *-----------------------------------------*
      *                      * Interlinea                              *
      *                      *-----------------------------------------*
           move      "LF"                 to   v-ope                  .
      ...
      *                      *-----------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      31                   to   v-pos                  .
           move      "> Quantita'        Prezzo        % di sconto      
      -              ""                   to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-002-660.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'fda-pif'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           if        w-prs-tip-vis-tip    =    01
                     perform int-pag-sta-001-000
                                          thru int-pag-sta-001-999
           else if   w-prs-tip-vis-tip    =    02
                     perform int-pag-sta-002-000
                                          thru int-pag-sta-002-999
           else      perform int-pag-sta-001-000
                                          thru int-pag-sta-001-999    .
       int-pag-sta-999.
           exit.
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "    Formato        Prezzo     T  Udm  D  Coeff.Mol
      -              "t  Coeff.Divi       Note      "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *              *-------------------------------------------------*
      ...
      -              "-  ----------  ---------------"
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Linea di trattini per separazione legenda       *
      *              *-------------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      80                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      20                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Legenda : T = Si/no trasformazione unita' di misur
      -              "a"                  to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Seconda linea legenda                           *
      *              *-------------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      51                   to   v-car                  .
           move      21                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "          D = Decimali quantita' per il fornitore"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       int-pag-sta-001-999.
