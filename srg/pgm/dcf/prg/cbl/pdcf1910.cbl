      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione tabella spese incasso per    *
      *                                                                *
      *                    gestione fornitori                          *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-wrk.
      *        *-------------------------------------------------------*
      *        * Comodo per descrizione spese incasso                  *
      *        *-------------------------------------------------------*
           05  w-wrk-des-spi              pic  x(40)                  .

      *    *===========================================================*
      *    * Work-area per ridefinizione contenuto del mark-point      *
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-cod-spi              pic  x(03)                  .
           05  w-mpn-tip-pag              pic  9(02)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Start su archivio [yin]                         *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CODSPI    "         to   f-key                  .
           move      spaces               to   rf-yin-cod-spi         .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessuna spesa incasso entro i limiti assegnati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [yin]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcf/fls/ioc/obj/iofyin"
                                          to   s-pat                  .
      ...
      *    *-----------------------------------------------------------*
       qry-cmp-rot-000.
           move      rf-yin-cod-spi       to   w-rot-l01-cod-spi      .
           move      rf-yin-des-spi       to   w-wrk-des-spi          .
       qry-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      w-rot-l01-cod-spi    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Descrizione spesa                           *
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-yin-tip-pag       =    1
                     move  "Rimessa diretta               "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    2
                     move  "Incasso elettronico           "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    3
                     move  "Ri.Ba.                        "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    4
                     move  "C.d.O. - Conferma d'ordine    "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    5
                     move  "Mav - Mediante avviso         "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    6
                     move  "Rid - Accr. preautoriz. in c/ "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    7
                     move  "Bonifico bancario             "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    8
                     move  "Conto corrente postale        "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    9
                     move  "Ricevuta bancaria             "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    10
                     move  "Tratta                        "
                                          to   v-alf
           else if   rf-yin-tip-pag       =    10
                     move  "Paghero'                      "
           ...
           move      "+"                  to   v-edm                  .
           move      rf-yin-cod-spi       to   w-mpn-cod-spi          .
           move      rf-yin-tip-pag       to   w-mpn-tip-pag          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-spi'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
           ...
           move      03                   to   s-car                  .
           move      v-cnt                to   w-mpn                  .
           move      w-mpn-cod-spi        to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'tip-pag'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "tip-pag"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           move      "N"                  to   s-tip                  .
           move      02                   to   s-car                  .
           move      zero                 to   s-dec                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Cod           Descrizione                   Tipo p
      -              "agamento          F  Ammontare"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
