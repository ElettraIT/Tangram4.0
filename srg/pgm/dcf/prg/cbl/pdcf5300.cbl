      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione condizioni di acquisto prodotti    *
      *                    da un fornitore                             *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "CONDIZIONI DI ACQUISTO PRODOTTI VENDITA "       .
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-key.
               ...
               10  w-tes-snx-apz          pic  9(01)                  .
               10  w-tes-per-apz          pic  9(03)v9(02)            .
               10  w-tes-snx-adg          pic  9(01)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no aggiornamento prezzi                 *
      *        *-------------------------------------------------------*
           05  w-exp-snx-apz.
               10  w-exp-snx-apz-num      pic  9(02)       value 3    .
               10  w-exp-snx-apz-lun      pic  9(02)       value 40   .
               10  w-exp-snx-apz-tbl.
                   ...
                   15  filler             pic  x(40) value
                       "Diminuzione in %                        "     .
      *        *-------------------------------------------------------*
      *        * Work per : Si/no aggiornamento dati generali          *
      *        *-------------------------------------------------------*
           05  w-exp-snx-adg.
               10  w-exp-snx-adg-num      pic  9(02)       value 3    .
               10  w-exp-snx-adg-lun      pic  9(02)       value 40   .
               10  w-exp-snx-adg-tbl.
                   15  filler             pic  x(40) value
                       "No                                      "     .
                   15  filler             pic  x(40) value
                       "Si, ma solo se fornitore preferenziale  "     .
                   15  filler             pic  x(40) value
                       "si, in Ogni caso                        "     .
      ...
      *            *---------------------------------------------------*
               10  w-det-new-prz-coe      pic  9(04)v9(02)            .
      ...
      *                  *---------------------------------------------*
       ...
       acc-key-reg-400.
      *                  *---------------------------------------------*
      *                  * Si/no aggiornamento prezzi                  *
      *                  *---------------------------------------------*
           perform   acc-snx-apz-000      thru acc-snx-apz-999        .
           if        w-cnt-tus-acc-key    =    "E"
                     move  spaces         to   w-cnt-tus-acc-key
                     go to acc-key-reg-100.
           if        w-cnt-tus-acc-key    not  = spaces
      ...
      *                  *---------------------------------------------*
           perform   acc-per-apz-000      thru acc-per-apz-999        .
           if        w-cnt-tus-acc-key    =    "E"
                     move  spaces         to   w-cnt-tus-acc-key
                     go to acc-key-reg-100.
           if        w-cnt-tus-acc-key    not  = spaces
      ...
      *                  *---------------------------------------------*
           perform   acc-snx-adg-000      thru acc-snx-adg-999        .
           if        w-cnt-tus-acc-key    =    "E"
                     move  spaces         to   w-cnt-tus-acc-key
                     go to acc-key-reg-100.
           if        w-cnt-tus-acc-key    not  = spaces
      ...
      *              * Si/no aggiornamento prezzi                      *
      *              *-------------------------------------------------*
           perform   vis-snx-apz-000      thru vis-snx-apz-999        .
      *              *-------------------------------------------------*
      *              * % aggiornamento prezzi                          *
      *              *-------------------------------------------------*
           perform   vis-per-apz-000      thru vis-per-apz-999        .
      *              *-------------------------------------------------*
      *              * Si/no aggiornamento dati generali               *
      *              *-------------------------------------------------*
           perform   vis-snx-adg-000      thru vis-snx-adg-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Si/no aggiornamento prezzi                  *
      *                  *---------------------------------------------*
           perform   pmt-snx-apz-000      thru pmt-snx-apz-999        .
      *                  *---------------------------------------------*
      *                  * % aggiornamento prezzi                      *
      *                  *---------------------------------------------*
           perform   pmt-per-apz-000      thru pmt-per-apz-999        .
      *                  *---------------------------------------------*
      *                  * Si/no aggiornamento dati generali           *
      *                  *---------------------------------------------*
           perform   pmt-snx-adg-000      thru pmt-snx-adg-999        .
      *                  *---------------------------------------------*
      *                  * Nota bene                                   *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Si/no aggiornamento prezzi                       *
      *    *-----------------------------------------------------------*
       pmt-snx-apz-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Si/no aggiornamento dati generali                *
      *    *-----------------------------------------------------------*
       pmt-snx-adg-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       pmt-not-ben-000.
           ...
           move      21                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "N.B.: trattamento solo di prezzi di acquisto che n
      -              "on dipendono dalla quantita'  "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       pmt-not-ben-999.
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no aggiornamento prezzi           *
      *    *-----------------------------------------------------------*
       acc-snx-apz-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-apz-lun    to   v-car                  .
           move      w-exp-snx-apz-num    to   v-ldt                  .
           move      "NAD#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-apz-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      w-tes-snx-apz        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-apz-999.
       acc-snx-apz-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-apz          .
       acc-snx-apz-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se ne' aumento ne' diminuzione, si pone a   *
      *                  * zero la percentuale                         *
      *                  *---------------------------------------------*
           if        w-tes-snx-apz        not  = 01
                     go to acc-snx-apz-800.
           move      zero                 to   w-tes-per-apz          .
           perform   vis-per-apz-000      thru vis-per-apz-999        .
       acc-snx-apz-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Si/no aggiornamento prezzi        *
      *    *-----------------------------------------------------------*
       vis-snx-apz-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : % aggiornamento prezzi               *
      *    *-----------------------------------------------------------*
       acc-per-apz-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : % aggiornamento prezzi            *
      *    *-----------------------------------------------------------*
       vis-per-apz-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no aggiornamento dati generali    *
      *    *-----------------------------------------------------------*
       acc-snx-adg-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-adg-lun    to   v-car                  .
           move      w-exp-snx-adg-num    to   v-ldt                  .
           move      "NSO#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      17                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-adg-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      w-tes-snx-adg        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-adg-999.
       acc-snx-adg-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-adg          .
       acc-snx-adg-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Si/no aggiornamento dati generali *
      *    *-----------------------------------------------------------*
       vis-snx-adg-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           perform   vis-cod-pro-umv-000  thru vis-cod-pro-umv-999    .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           if        w-tes-per-apz        not  = zero
                     go to cnt-tdo-key-200.
           move      "Manca la percentuale di aggiornamento !           
      -              "      "             to   w-err-box-err-msg      .
           go to     cnt-tdo-key-900.
       cnt-tdo-key-200.
       cnt-tdo-key-500.
      *              *-------------------------------------------------*
      *              * Eventuale normalizzazione si/no aggiornamento   *
      *              * prezzi                                          *
      *              *-------------------------------------------------*
           if        w-tes-snx-apz        not  = 2 and
                     w-tes-snx-apz        not  = 3
                     move  1              to   w-tes-snx-apz          .
      *              *-------------------------------------------------*
      *              * Eventuale normalizzazione % aggiornamento prez- *
      *              * zi                                              *
      *              *-------------------------------------------------*
           if        w-tes-snx-apz        =    1
                     move  zero           to   w-tes-per-apz          .
      ...
      *              * dati generali                                   *
      *              *-------------------------------------------------*
           if        w-tes-snx-adg        not  = 2 and
                     w-tes-snx-adg        not  = 3
                     move  1              to   w-tes-snx-adg          .
       cnt-tdo-key-800.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-key-reg-000.
           ...
           move      zero                 to   w-tes-snx-apz          .
           move      zero                 to   w-tes-per-apz          .
           move      zero                 to   w-tes-snx-adg          .
           move      spaces               to   w-tes-cnf-imp          .
       nor-key-reg-999.
           exit.
      ...
      *                          *-------------------------------------*
           if        rf-aaf-snx-tum       =    "N"
                     move  rf-dcp-umi-ven to   srt-umf-tum
      ...
      *                          *-------------------------------------*
           if        rf-aaf-tip-pza       =    01
                     move  rf-aaf-prz-pes (1)
                                          to   srt-pre-pes
           else      move  rf-dcp-prz-lst to   srt-pre-pes            .
      *                          *-------------------------------------*
      *                          * Prezzo d'acquisto attuale           *
      *                          *-------------------------------------*
           if        rf-aaf-tip-pza       =    01
                     move  rf-aaf-prz-pes (1)
                                          to   srt-prz-pes
           else      move  rf-dcp-prz-lst to   srt-prz-pes            .
      *                          *-------------------------------------*
      *                          * % sconto                            *
      *                          *-------------------------------------*
      ...
      *                      * Codice prodotto numerico                *
      *                      *-----------------------------------------*
           move      srt-num-pro          to   w-rig-num-pro          .
      *                      *-----------------------------------------*
      *                      * Codice prodotto alfanumerico            *
      *                      *-----------------------------------------*
           move      srt-alf-pro          to   w-rig-alf-pro          .
      *                      *-----------------------------------------*
      *                      * Formato di acquisizione                 *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      srt-des-pro          to   w-rig-des-pro          .
      *                      *-----------------------------------------*
      *                      * Unita' di misura di acquisto            *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           ...
           add       w-tes-per-apz        to   w-det-new-prz-coe      .
           multiply  w-det-new-prz-coe    by   w-rig-prz-pes          .
           divide    100                  into w-rig-prz-pes          .
      *                          *-------------------------------------*
      *                          * Oltre                               *
      *                          *-------------------------------------*
           go to     car-fil-rap-430.
      ...
      *                          *-------------------------------------*
           move      100,0                to   w-det-new-prz-coe      .
           subtract  w-tes-per-apz        from w-det-new-prz-coe      .
           multiply  w-det-new-prz-coe    by   w-rig-prz-pes          .
           divide    100                  into w-rig-prz-pes          .
      *                          *-------------------------------------*
      *                          * Oltre                               *
      ...
      *
           if        w-rig-prz-pes        not  = rf-aaf-prz-pes (1) and
                     w-rig-per-sco (1)    not  = rf-aaf-psr-pes
                                                (1, 1)              and
                     w-rig-per-sco (2)    not  = rf-aaf-psr-pes
                                                (1, 2)              and
                     w-rig-per-sco (3)    not  = rf-aaf-psr-pes
                                                (1, 3)              and
                     w-rig-per-sco (4)    not  = rf-aaf-psr-pes
                                                (1, 4)              and
                     w-rig-per-sco (5)    not  = rf-aaf-psr-pes
                                                (1, 5)
                     move  rf-aaf-ide-dat to   rf-aaf-uda-pes         .
      *
      ...
      *                  *---------------------------------------------*
       ...
       sca-fil-rap-400.
      *                  *---------------------------------------------*
      *                  * Eventuale aggiornamento dati generali       *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to sca-fil-rap-800.
      *                      *-----------------------------------------*
      *                      * Test se da effettuare in ogni caso      *
      *                      *-----------------------------------------*
           if        w-tes-snx-adg        =    3
                     go to sca-fil-rap-780.
      *                      *-----------------------------------------*
      *                      * Se record trovato, ma non fornitore     *
      *                      * preferenziale : a riciclo               *
      *                      *-----------------------------------------*
           if        rf-aaq-dcf-pfz       not  = w-tes-cod-fnt
                     go to sca-fil-rap-800.
       sca-fil-rap-780.
      *                      *-----------------------------------------*
      *                      * Aggiornamento [aaq]                     *
      ...
      *                  *---------------------------------------------*
           move      w-rig-uda-pes        to   rf-aaq-uda-par         .
      *                  *---------------------------------------------*
      *                  * Prezzo medio d'acquisto                     *
      *                  *---------------------------------------------*
           move      w-rig-prz-pes        to   rf-aaq-prz-acr         .
       upd-rec-aaq-400.
      *                  *---------------------------------------------*
      *                  * Update record                               *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/dcf/prg/obj/pdcf5302"                       .
       cnc-rlt-sup-999.
           exit.
