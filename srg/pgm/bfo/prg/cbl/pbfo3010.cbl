      ...
      *                       Ultima revisione:    NdK del 01/05/23    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Ricerca bolle di entrata                    *
      *                                                                *
      *                    Main                                        *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase bfo301, nella sua globalita', esegue i tipi di inter-  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * INTDIF      Ricerca documenti emessi da un fornitore      301a *
      * INTDIC      Ricerca documenti emessi da un cliente        301b *
      * INTDID      Ricerca documenti emessi da una dipendenza    301c *
      * INTDTM      Ricerca documenti per tipo movimento          301d *
      * INTDAF      Ricerca documenti in attesa di fattura        301e *
      * INTDPV      Ricerca documenti per prodotti di vendita     301f *
      * INTDMP      Ricerca documenti per materie prime           301h *
      * INTCTS      Ricerca documenti per contrassegno collo      301i *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   INTERROGAZIONI SU BOLLE DI ENTRATA   "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-prs.
      *        *-------------------------------------------------------*
      *        * Si/No gestione fatture fornitori attiva               *
      *        *-------------------------------------------------------*
           05  w-prs-ffo-snx              pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/No gestione materie prime attiva                   *
      *        *-------------------------------------------------------*
           05  w-prs-dpm-snx              pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/no accettazione del numero colli in riga           *
      *        *-------------------------------------------------------*
           05  w-prs-col-rig.
      *            *---------------------------------------------------*
      *            * Si/no accettazione                                *
      *            *---------------------------------------------------*
               10  w-prs-col-rig-snx      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Numero di interi da accettare (3a quantita')      *
      *            *---------------------------------------------------*
               10  w-prs-col-rig-car      pic  9(01)                  .
      *            *---------------------------------------------------*
      *            * Numero di decimali da accettare (3a quantita')    *
      *            *---------------------------------------------------*
               10  w-prs-col-rig-dec      pic  9(01)                  .
      *            *---------------------------------------------------*
      *            * Massimo numero elementi da accettare              *
      *            *---------------------------------------------------*
               10  w-prs-col-rig-max      pic  9(02)                  .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'bfo301'           *
      *              *-------------------------------------------------*
           perform   fas-bfo-301-000      thru fas-bfo-301-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *                  * Si/No gestione fatture fornitori attiva     *
      *                  *---------------------------------------------*
           perform   prs-ffo-snx-000      thru prs-ffo-snx-999        .
      *                  *---------------------------------------------*
      *                  * Si/No gestione materie prime attiva         *
      *                  *---------------------------------------------*
           perform   prs-dpm-snx-000      thru prs-dpm-snx-999        .
      *                  *---------------------------------------------*
      *                  * Si/no accettazione numero colli in riga     *
      *                  *---------------------------------------------*
           perform   prs-col-rig-000      thru prs-col-rig-999        .
       pre-exe-pgm-150.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Lettura personalizzazione : Si/No gestione fatturazione   *
      *    *                             fornitori attiva              *
      *    *-----------------------------------------------------------*
       prs-ffo-snx-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/ffo[snx]"       to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-ffo-snx
           else      move  spaces         to   w-prs-ffo-snx          .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-ffo-snx        =    "S" or
                     w-prs-ffo-snx        =    "N"
                     go to prs-ffo-snx-999.
           move      "N"                  to   w-prs-ffo-snx          .
       prs-ffo-snx-999.
           exit.
      ...
      *    *===========================================================*
      *    * Lettura personalizzazione : Si/no accettazione numero     *
      *    *                             colli in riga                 *
      *    *-----------------------------------------------------------*
       prs-col-rig-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/bfo/mov/bfo300[col-rig]"
                                          to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-col-rig
           else      move  spaces         to   w-prs-col-rig          .
      *              *-------------------------------------------------*
      *              * Normalizzazioni personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-col-rig-snx    not  = "S"
                     move  "N"            to   w-prs-col-rig-snx      .
      *
           if        w-prs-col-rig-car    not  numeric
                     move  zero           to   w-prs-col-rig-car      .
           if        w-prs-col-rig-car    >    6
                     move  6              to   w-prs-col-rig-car      .
      *
           if        w-prs-col-rig-dec    not  numeric
                     move  zero           to   w-prs-col-rig-dec      .
           if        w-prs-col-rig-dec    >    3
                     move  3              to   w-prs-col-rig-dec      .
           if        w-prs-col-rig-car    =    zero
                     move  zero           to   w-prs-col-rig-dec      .
      *
           if        w-prs-col-rig-max    not  numeric
                     move  zero           to   w-prs-col-rig-max      .
           if        w-prs-col-rig-max    >    98
                     move  99             to   w-prs-col-rig-max      .
           if        w-prs-col-rig-car    =    zero
                     move  zero           to   w-prs-col-rig-max      .
       prs-col-rig-999.
           exit.
      ...
      *
      ...
      -              " INTDID     pbfo301c  "
       ...
       loa-tbl-tin-102.
      *
           move      "per Tipo movimento                                
      -              " INTDTM     pbfo301d  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           if        w-prs-ffo-snx        not  = "S"
                     go to loa-tbl-tin-104.
           move      "in  Attesa di fattura                             
      -              " INTDAF     pbfo301e  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-104.
      ...
      *    *===========================================================*
      *    * Esecuzione ciclo per la fase 'bfo300'                     *
      *    *-----------------------------------------------------------*
       fas-bfo-301-000.
      *              *-------------------------------------------------*
      *              * Video in Off                                    *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-bfo-301-300
           else if   v-key                =    "PRSC"
                     go to fas-bfo-301-400
           else if   v-key                =    "NXSC"
                     go to fas-bfo-301-450
           else if   v-key                =    "DOWN"
                     go to fas-bfo-301-500
           else if   v-key                =    "EXIT"
                     go to fas-bfo-301-250
           else      go to fas-bfo-301-200.
       fas-bfo-301-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-bfo-301-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-bfo-301-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-num
                     go to fas-bfo-301-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-bfo-301-350
           else      go to fas-bfo-301-322.
       fas-bfo-301-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-bfo-301-630
           else if   v-key                =    "PRSC"
                     go to fas-bfo-301-600
           else if   v-key                =    "NXSC"
                     go to fas-bfo-301-610
           else if   v-key                =    "UP  "
                     go to fas-bfo-301-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-bfo-301-650
           else if   v-key                =    "EXIT"
                     go to fas-bfo-301-660
           else      go to fas-bfo-301-570.
       fas-bfo-301-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-bfo-301-626
           else      go to fas-bfo-301-628.
       fas-bfo-301-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-bfo-301-636
           else      go to fas-bfo-301-638.
       fas-bfo-301-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-bfo-301-725
           else      go to fas-bfo-301-750.
       fas-bfo-301-725.
      *              *-------------------------------------------------*
      *              * Se variabile esistente                          *
      *              *-------------------------------------------------*
