      ...
      *                                   Fase:    svf960              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 18/12/08    *
      *                       Ultima revisione:    NdK del 14/03/12    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Generazione grafici da stampe statistiche   *
      *                    sul fatturato di vendita                    *
      *                                                                *
      *                    Main                                        *
      *                                                                *
      *                    MEMO : passare codice dipendenza ... ???    *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase svf960, nella sua globalita', esegue i tipi di gene-   *
      ...
      * SVF300      Statistica per classificazione cliente             *
      * SVF370      Statistica per forma di pagamento                  *
      * SVF400      Statistica per classificazione geografica cliente  *
      * SVF600      Statistica per classificazione prodotto            *
      * SVF700      Statistica per agente                              *
      * SVF800      Statistica per tipo movimento                      *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "GENERAZIONE GRAFICI WEB DA STATISTICHE"       .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'svf960'           *
      *              *-------------------------------------------------*
           perform   fas-svf-950-000      thru fas-svf-950-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *                  * svf300                                      *
      *                  *---------------------------------------------*
           move      "Statistica per Classificazione Cliente    (svf300)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
           move      "SVF300    "         to   w-tin-ele-wci-alf      .
           move      "pgm/svf/prg/obj/psvf3000                "
                                          to   w-tin-ele-wci-ovy      .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *                  *---------------------------------------------*
      *                  * svf370                                      *
      *                  *---------------------------------------------*
           move      "Statistica per forma di Pagamento         (svf370)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
      ...
      *                  * svf400                                      *
      *                  *---------------------------------------------*
           move      "Statistica per Class. Geografica Cliente  (svf400)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
           move      "SVF400    "         to   w-tin-ele-wci-alf      .
           move      "pgm/svf/prg/obj/psvf4000                "
                                          to   w-tin-ele-wci-ovy      .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *                  *---------------------------------------------*
      *                  * svf600                                      *
      *                  *---------------------------------------------*
           move      "Statistica per Classificazione Prodotto   (svf600)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
           move      "SVF600    "         to   w-tin-ele-wci-alf      .
           move      "pgm/svf/prg/obj/psvf6000                "
                                          to   w-tin-ele-wci-ovy      .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *                  *---------------------------------------------*
      *                  * svf700                                      *
      *                  *---------------------------------------------*
           move      "Statistica per Agente                     (svf700)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
           move      "SVF700    "         to   w-tin-ele-wci-alf      .
           move      "pgm/svf/prg/obj/psvf7000                "
                                          to   w-tin-ele-wci-ovy      .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *                  *---------------------------------------------*
      *                  * svf800                                      *
      *                  *---------------------------------------------*
           move      "Statistica per Tipo movimento             (svf800)
      -              ""                   to   w-tin-ele-wci-des      .
           move      spaces               to   w-tin-ele-wci-ast      .
           move      "SVF800    "         to   w-tin-ele-wci-alf      .
           move      "pgm/svf/prg/obj/psvf8000                "
                                          to   w-tin-ele-wci-ovy      .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-200.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-svf-950-300
           else if   v-key                =    "PRSC"
                     go to fas-svf-950-400
           else if   v-key                =    "NXSC"
                     go to fas-svf-950-450
           else if   v-key                =    "DOWN"
                     go to fas-svf-950-500
           else if   v-key                =    "EXIT"
                     go to fas-svf-950-250
           else      go to fas-svf-950-200.
       fas-svf-950-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-svf-950-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-svf-950-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-nes
                     go to fas-svf-950-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-svf-950-350
           else      go to fas-svf-950-322.
       fas-svf-950-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-svf-950-630
           else if   v-key                =    "PRSC"
                     go to fas-svf-950-600
           else if   v-key                =    "NXSC"
                     go to fas-svf-950-610
           else if   v-key                =    "UP  "
                     go to fas-svf-950-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-svf-950-650
           else if   v-key                =    "EXIT"
                     go to fas-svf-950-660
           else      go to fas-svf-950-570.
       fas-svf-950-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-svf-950-626
           else      go to fas-svf-950-628.
       fas-svf-950-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-svf-950-636
           else      go to fas-svf-950-638.
       fas-svf-950-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-svf-950-725
           else      go to fas-svf-950-750.
       fas-svf-950-725.
      *              *-------------------------------------------------*
