      ...
      *                       Ultima revisione:    NdK del 20/07/18    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf7201:        *
      *                                                                *
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Opzione di stampa                          *
      *        *-------------------------------------------------------*
           05  w-exp-opz-dts.
               10  w-exp-opz-dts-num      pic  9(02)       value 2    .
               10  w-exp-opz-dts-lun      pic  9(02)       value 50   .
               10  w-exp-opz-dts-tbl.
                   15  filler             pic  x(50) value
                  "agente memorizzato nei Documenti                  ".
                   15  filler             pic  x(50) value
                  "agente in anagrafica Cliente                      ".
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Tipo di vendita                            *
      *        *-------------------------------------------------------*
           05  w-exp-tip-vpa.
               10  w-exp-tip-vpa-num      pic  9(02)       value 3    .
      ...
      *                  * Opzione di stampa                           *
      *                  *---------------------------------------------*
           perform   acc-opz-dts-000      thru acc-opz-dts-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-275.
      *                  *---------------------------------------------*
      *                  * Selezione su data di acquisizione           *
      *                  *---------------------------------------------*
           perform   acc-dac-cli-000      thru acc-dac-cli-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Opzione di stampa                               *
      *              *-------------------------------------------------*
           perform   pmt-opz-dts-000      thru pmt-opz-dts-999        .
      *              *-------------------------------------------------*
      *              * Tipo di dettaglio di stampa                     *
      *              *-------------------------------------------------*
      ...
      *              * Selezione su data di acquisizione               *
      *              *-------------------------------------------------*
           perform   pmt-dac-cli-000      thru pmt-dac-cli-999        .
      *              *-------------------------------------------------*
      *              * Selezione su tipo di vendita                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Tipo di ordinamento per gli clienti              *
      *    *-----------------------------------------------------------*
       pmt-tor-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Selezione su data di acquisizione                *
      *    *-----------------------------------------------------------*
       pmt-dac-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Selezione su tipo di vendita                     *
      *    *-----------------------------------------------------------*
       pmt-tip-vpa-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Opzione in stampa                          *
      *    *-----------------------------------------------------------*
       acc-opz-dts-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-opz-dts-lun    to   v-car                  .
           move      w-exp-opz-dts-num    to   v-ldt                  .
           move      "DC#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      05                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-opz-dts-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-opz-dts           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-opz-dts-999.
       acc-opz-dts-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-opz-dts             .
       acc-opz-dts-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-opz-dts           =    zero           or
                     rr-opz-dts           >    w-exp-opz-dts-num
                     go to acc-opz-dts-100.
       acc-opz-dts-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Opzione di stampa                       *
      *    *-----------------------------------------------------------*
       vis-opz-dts-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Selezione su data di acquisizione          *
      *    *-----------------------------------------------------------*
       acc-dac-cli-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
      ...
      -              " anagrafica commerciale del   "
                                          to   v-nt1                  .
           move      "cliente. Nel caso di selezione vengono esclusi i c
      -              "lienti con data a zero.       "
                                          to   v-nt2                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dac-cli-999.
       acc-dac-cli-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dac-cli             .
       acc-dac-cli-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-dac-cli-100.
       acc-dac-cli-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione : Selezione su data di acquisizione       *
      *    *-----------------------------------------------------------*
       vis-dac-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione : Tipo di vendita                         *
      *    *-----------------------------------------------------------*
       vis-tip-vpa-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           if        rr-opz-dts           not  = 02
                     move  01             to   rr-opz-dts             .
      *              *-------------------------------------------------*
      *              * Tipo di vendita                                 *
      *              *-------------------------------------------------*
           if        rr-tip-vpa           not  = 02 and
                     rr-tip-vpa           not  = 03
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   rr-cod-age-via         .
           move      spaces               to   rr-cod-age-loc         .
           move      zero                 to   rr-opz-dts             .
           move      zero                 to   rr-tip-dts             .
           move      zero                 to   rr-tor-age             .
           move      zero                 to   rr-tor-cli             .
           move      zero                 to   rr-dac-cli             .
