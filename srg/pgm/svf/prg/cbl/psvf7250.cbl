      ...
      *                       Ultima revisione:    NdK del 20/03/03    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf7251:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tor-pro.
               10  w-exp-tor-pro-num      pic  9(02)       value 9    .
               ...
               10  w-exp-tor-pro-tbl.
                  ...
                  "Ordinamento per descrizione prodotto              ".
                   15  filler             pic  x(50) value
                  "Ordinamento per classe e codice prodotto          ".
                   15  filler             pic  x(50) value
                  "Ordinamento per classe e descrizione prodotto     ".
                   15  filler             pic  x(50) value
                  "Ordinamento per classe (senza dettaglio prodotti) ".
      *        *-------------------------------------------------------*
      ...
      *                      * e si va' al controllo su tasto Do       *
      *                      *-----------------------------------------*
           if        rr-tor-pro           =    zero
      ...
      *                      *-----------------------------------------*
      *                      * A controllo su tasto Do                 *
      *                      *-----------------------------------------*
           go to     acc-fso-dcp-800.
       acc-fso-dcp-650.
      *                  *---------------------------------------------*
      *                  * Eventuale preparazione del tipo di ordina-  *
      *                  * mento per i prodotti se non gia' preparato  *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        rr-tor-pro           not  = zero
                     go to acc-fso-dcp-675.
      *                      *-----------------------------------------*
      *                      * Lettura del tipo di ordinamento rela-   *
      *                      * tivo al filtro di selezione             *
      *                      *-----------------------------------------*
           move      "TO"                 to   f-ope                  .
           ...
           call      s-pat               using f
                                               rf-dcp                 .
      *                      *-----------------------------------------*
      *                      * Preparazione del tipo ordinamento per   *
      *                      * prodotti, corrispondentemente a quanto  *
      *                      * specificato nel filtro                  *
      *                      *-----------------------------------------*
           if        f-sts                =    "01"
                     move  07             to   rr-tor-pro
           else if   f-sts                =    "02"
                     move  08             to   rr-tor-pro
           else if   f-sts                =    "03"
                     move  05             to   rr-tor-pro
           else if   f-sts                =    "04"
                     move  06             to   rr-tor-pro             .
      *                      *-----------------------------------------*
      *                      * Visualizzazione del tipo ordinamento    *
      *                      * per prodotti                            *
      *                      *-----------------------------------------*
           perform   vis-tor-pro-000      thru vis-tor-pro-999        .
       acc-fso-dcp-675.
      ...
      *              *-------------------------------------------------*
       tdo-ric-sel-810.
      *                  *---------------------------------------------*
      *                  * Eventuale normalizzazione del tipo di ordi- *
      *                  * namento per gli agenti: se selezionato un   *
      *                  * solo agenti, ordinamento per codice agente  *
      *                  *---------------------------------------------*
           if        rr-cod-age           not  = zero
                     move   06            to   rr-tor-age             .
       tdo-ric-sel-820.
      *                  *---------------------------------------------*
      *                  * Eventuale normalizzazione del tipo di ordi- *
      *                  * namento per i prodotti: se selezionato un   *
      *                  * solo codice prodotto, ordinamento per co-   *
      *                  * dice prodotto                               *
      *                  *---------------------------------------------*
           if        rr-fso-dcp           not  < 10000000 and
                     rr-fso-dcp           not  > 19999999
