      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf8601:        *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per tipo movimento - classe statistica      *
      *                    prodotto                                    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "FATTURATO X DOCUMENTO - CLASSE PRODOTTO "       .
      ...
      *        *-------------------------------------------------------*
      *        * Tipo di ordinamento per tipi movimento                *
      *        *                                                       *
      ...
      *        *  - 05 : Ordinamento per descrizione tipo movimento    *
      *        *  - 06 : Ordinamento per codice tipo movimento         *
      *        *                                                       *
      *        * Non significativo se selezionato un solo tipo movim.  *
      *        *-------------------------------------------------------*
           05  rr-tor-tmo                 pic  9(02)                  .
      *        *-------------------------------------------------------*
           ...
           05  w-exp-tip-dts.
               ...
               10  w-exp-tip-dts-tbl.
                  ...
                  "Con la lista documenti emessi per ogni classe     ".
                   15  filler             pic  x(50)                  .
      *        *-------------------------------------------------------*
      *        * Work per : Tipo ordinamento tipi movimento            *
      *        *-------------------------------------------------------*
           05  w-exp-tor-tmo.
               10  w-exp-tor-tmo-num      pic  9(02)       value 6    .
               10  w-exp-tor-tmo-lun      pic  9(02)       value 50   .
               10  w-exp-tor-tmo-tbl.
                   15  filler             pic  x(50) value
                  ...
                  "Ordinamento ABC crescente sulla quantita'         ".
                   15  filler             pic  x(50) value
                  "Ordinamento per descrizione tipo movimento        ".
                   15  filler             pic  x(50) value
                  "Ordinamento per codice tipo movimento             ".
      *        *-------------------------------------------------------*
      ...
      *                  * Tipo di ordinamento per tipi movimento      *
      *                  *---------------------------------------------*
           perform   acc-tor-tmo-000      thru acc-tor-tmo-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Tipo di ordinamento per tipi movimento          *
      *              *-------------------------------------------------*
           perform   pmt-tor-tmo-000      thru pmt-tor-tmo-999        .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Tipo di ordinamento per tipi movimento           *
      *    *-----------------------------------------------------------*
       pmt-tor-tmo-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      10                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Ordinamento tipi movimento :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-tor-tmo-999.
      ...
      *    *===========================================================*
      *    * Accettazione : Tipo ordinamento per tipi movimento        *
      *    *-----------------------------------------------------------*
       acc-tor-tmo-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-tor-tmo-lun    to   v-car                  .
           move      w-exp-tor-tmo-num    to   v-ldt                  .
           move      spaces               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      10                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-tor-tmo-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-tor-tmo           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  * Che sia un valore consentito                *
      *                  *---------------------------------------------*
           if        rr-tor-tmo           =    zero           or
                     rr-tor-tmo           >    w-exp-tor-tmo-num
                     go to acc-tor-tmo-100.
       acc-tor-tmo-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Tipo ordinamento per tipi movimento     *
      *    *-----------------------------------------------------------*
       vis-tor-tmo-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      ...
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo : Tipo di ordinamento tipi movimento  *
      *              *-------------------------------------------------*
       tdo-ric-sel-101.
           if        rr-tmo-ftr           not  = spaces
                     go to tdo-ric-sel-150.
           if        rr-tor-tmo           not  = zero
                     go to tdo-ric-sel-102.
           move      "Manca il tipo ordinamento per i tipi movimento !  
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-102.
           if        rr-tor-tmo           =    01 or
                     ...
                     rr-tor-tmo           =    06
                     go to tdo-ric-sel-150.
           move      "Tipo di ordinamento per tipi movimento errato !   
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-150.
      *              *-------------------------------------------------*
