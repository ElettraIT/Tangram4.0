      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf6201:        *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per classe statistica prodotto - cliente.   *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "FATTURATO PER CLASSE PRODOTTO - CLIENTE "       .
      ...
      *        *  - 02 : Con lista documenti emessi per ogni cliente   *
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tip-dts.
               ...
               10  w-exp-tip-dts-tbl.
                  ...
                  "Solo il totale per ogni cliente                   ".
                   15  filler             pic  x(50) value
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tor-cli.
               10  w-exp-tor-cli-num      pic  9(02)       value 7    .
               10  w-exp-tor-cli-lun      pic  9(02)       value 50   .
               10  w-exp-tor-cli-tbl.
                   15  filler             pic  x(50) value
                  "Ordinamento ABC decrescente sul fatturato         ".
                   15  filler             pic  x(50) value
                  "Ordinamento ABC decrescente sulla quantita'       ".
                  ...
                  "Ordinamento ABC crescente sulla quantita'         ".
                   15  filler             pic  x(50) value
                  "Ordinamento per ragione sociale cliente           ".
                   15  filler             pic  x(50) value
                  "Ordinamento per codice cliente                    ".
                   15  filler             pic  x(50) value
                  "Ordinamento per mnemonico cliente                 ".
      *        *-------------------------------------------------------*
      ...
      *                  * Tipo di ordinamento per clienti             *
      *                  *---------------------------------------------*
           perform   acc-tor-cli-000      thru acc-tor-cli-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Tipo di ordinamento per clienti                 *
      *              *-------------------------------------------------*
           perform   pmt-tor-cli-000      thru pmt-tor-cli-999        .
      *              *-------------------------------------------------*
      *              * Numero periodi di riferimento                   *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Tipo di ordinamento per clienti                  *
      *    *-----------------------------------------------------------*
       pmt-tor-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   w-cmn-zps-001-lin      .
           move      30                   to   w-cmn-zps-001-pos      .
           move      06                   to   w-cmn-zps-001-dln      .
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   w-cmn-zps-002-lin      .
           move      30                   to   w-cmn-zps-002-pos      .
           move      06                   to   w-cmn-zps-002-dln      .
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   w-cmn-zps-003-lin      .
           move      30                   to   w-cmn-zps-003-pos      .
           move      06                   to   w-cmn-zps-003-dln      .
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   w-cod-mne-zp1-lin      .
           move      30                   to   w-cod-mne-zp1-pos      .
           move      06                   to   w-cod-mne-zp1-dln      .
      ...
      *    *===========================================================*
      *    * Accettazione : Tipo ordinamento per clienti               *
      *    *-----------------------------------------------------------*
       acc-tor-cli-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-tor-cli-lun    to   v-car                  .
           move      w-exp-tor-cli-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      12                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-tor-cli-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-tor-cli           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-tor-cli-999.
       acc-tor-cli-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-tor-cli             .
       acc-tor-cli-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Che sia un valore consentito                *
      *                  *---------------------------------------------*
           if        rr-tor-cli           =    zero           or
                     rr-tor-cli           >    w-exp-tor-cli-num
                     go to acc-tor-cli-100.
       acc-tor-cli-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Tipo ordinamento per clienti            *
      *    *-----------------------------------------------------------*
       vis-tor-cli-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      ...
      -              "tti errato !   "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Controllo : Tipo di ordinamento per clienti     *
      *              *-------------------------------------------------*
       tdo-ric-sel-201.
           if        rr-tor-cli           not  = zero
                     go to tdo-ric-sel-202.
           move      "Manca il tipo di ordinamento per i clienti !      
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-202.
           if        rr-tor-cli           =    01 or
           ...
           move      "Tipo di ordinamento per clienti errato !
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-300.
      *              *-------------------------------------------------*
