      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione per il programma psvf7301:       *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per agente - classe statistica cliente.     *
      *                                                                *
      *                    STANDARDIZZARE TEST 'ElETTRA'               *
      *                                                                *
      *================================================================*
      ...
      *                * - Per codice                                  *
      *                *-----------------------------------------------*
                   15  srt-k03-flg-zod    pic  9(01)                  .
      *                *-----------------------------------------------*
      *                * Codice cliente                                *
      *                *                                               *
      *                * Per tutti i tipi ordinamento cliente          *
      *                *-----------------------------------------------*
                   15  srt-k03-cod-cli    pic  9(07)                  .
      *                *-----------------------------------------------*
      ...
      *        *  - 03 : Con lista documenti emessi per ogni classe    *
      *        *         statistica                                    *
      *        *-------------------------------------------------------*
      ...
      *        *  - 05 : Ordinamento per mnemonico classe              *
      *        *-------------------------------------------------------*
      ...
      *            *---------------------------------------------------*
      *            * Nr. documenti trattati per la classe statistica   *
      *            *---------------------------------------------------*
               10  w-liv-csc-num-doc      pic  9(11)                  .
      *            *---------------------------------------------------*
      *            * Numero clienti trattati per la classe statistica  *
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
      *            * Numero classi statistiche cliente trattate per    *
      *            * l'agente                                          *
      *            *---------------------------------------------------*
               10  w-liv-age-num-csc      pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Totale generale                                       *
      *        *-------------------------------------------------------*
           05  w-liv-gen.
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Trattamento del codice agente nelle         *
      ...
      *                  *---------------------------------------------*
           if        rr-tcs-cli           =    01
                     go to sel-fit-rec-450
           else if   rr-tcs-cli           =    02
                     go to sel-fit-rec-475
           else if   rr-tcs-cli           =    03
                     go to sel-fit-rec-500.
       sel-fit-rec-450.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        rr-tip-dts           not  = 02 and
                     ...
                     move  zero           to   srt-k03-flg-eon
                     move  spaces         to   srt-k03-rag-cli
                     move  spaces         to   srt-k03-mne-cli
                     move  zero           to   srt-k03-flg-zod
                     move  zero           to   srt-k03-cod-cli
                     move  spaces         to   srt-k03-dpz-cli
                     go to cmp-srt-rec-500.
       cmp-srt-rec-410.
      *                      *-----------------------------------------*
      *                      * Valore ABC                              *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 01 and
                     ...
                     rr-tor-cli           not  = 03
                     move  spaces         to   srt-k03-rag-cli
                     go to cmp-srt-rec-425.
      *                          *-------------------------------------*
      *                          * Memorizzazione                      *
      *                          *-------------------------------------*
           move      w-sel-fit-rag-cli    to   srt-k03-rag-cli        .
       cmp-srt-rec-425.
      *                      *-----------------------------------------*
      *                      * Mnemonico per il cliente                *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 05
                     move  spaces         to   srt-k03-mne-cli
                     go to cmp-srt-rec-430.
      *                          *-------------------------------------*
      *                          * Memorizzazione                      *
      *                          *-------------------------------------*
           move      w-sel-fit-mne-cli    to   srt-k03-mne-cli        .
       cmp-srt-rec-430.
      *                      *-----------------------------------------*
      *                      * Flag di codice a zero o diverso da zero *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 04
                     move  zero           to   srt-k03-flg-zod
                     go to cmp-srt-rec-435.
      *                          *-------------------------------------*
      *                          * Memorizzazione con test             *
      *                          *-------------------------------------*
           if        w-sel-fit-cod-cli    =    zero
                     move  9              to   srt-k03-flg-zod
           else      move  1              to   srt-k03-flg-zod        .
       cmp-srt-rec-435.
      *                      *-----------------------------------------*
      *                      * Codice cliente                          *
      *                      *-----------------------------------------*
           move      w-sel-fit-cod-cli    to   srt-k03-cod-cli        .
       cmp-srt-rec-440.
      *                      *-----------------------------------------*
      *                      * Codice dipendenza per il cliente        *
      *                      *-----------------------------------------*
      ...
      *                      * cliente nell'ambito dell'agente         *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       agg-abc-kk0-500.
      *              *-------------------------------------------------*
      *              * Valore ABC per codice classe statistica cliente *
      *              *-------------------------------------------------*
       agg-abc-kk0-525.
      *                  *---------------------------------------------*
      *                  * Se la statistica e' sul nuovo fatturato o   *
      ...
      *                  *---------------------------------------------*
      *                  * Solo se tipo ordinamento classe statistica  *
      *                  *   - 01 : Ordinamento ABC decrescente        *
      *                  *   - 02 : Ordinamento ABC crescente          *
      *                  * Altrimenti nessuna azione                   *
      *                  *---------------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-cli           not  = 01 and
                     rr-tor-cli           not  = 02
                     go to agg-abc-sqz-660.
      *                          *-------------------------------------*
      *                          * Preparazione valore ABC             *
      *                          *-------------------------------------*
           if        rr-num-pdr           =    02 or
                     rr-num-pdr           =    11
                     move  kk0-val-f2p    to   srt-k03-val-abc
           else if   rr-num-pdr           =    03
                     move  kk0-val-f3p    to   srt-k03-val-abc
           else      move  kk0-val-f1p    to   srt-k03-val-abc        .
      *                          *-------------------------------------*
      *                          * Se ABC decrescente si complementa   *
      *                          *-------------------------------------*
           if        rr-tor-cli           =    01
      ...
      *                          * Memorizzazione in area specifica    *
      *                          *-------------------------------------*
           move      srt-k03-val-abc      to   srt-val-abc-cli        .
       agg-abc-sqz-660.
      *                      *-----------------------------------------*
      *                      * Eventuale aggiornamento del flag per    *
      *                      * record da non selezionare               *
      ...
      *                  * Codice agente                               *
      *                  *---------------------------------------------*
           move      srt-cod-age-cod      to   w-rot-l03-cod-age      .
      *              *-------------------------------------------------*
      *              * 2. livello di rottura                           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  * tate per l'agente                           *
      *                  *---------------------------------------------*
           move      zero                 to   w-liv-age-num-csc      .
      *                  *---------------------------------------------*
      *                  * Numero di agenti trattati                   *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           perform   stp-sub-int-age-000  thru stp-sub-int-age-999    .
      *                      *-----------------------------------------*
      *                      * Se flag di interruzione forzata: uscita *
      *                      *-----------------------------------------*
           if        w-cnt-prn-flg-int    not  = spaces
      ...
      *              * documenti per ogni classe statistica cliente    *
      *              *-------------------------------------------------*
       prn-liv-det-610.
      *                  *---------------------------------------------*
      ...
      *                      * classe statistica cliente, del mnemo-   *
      *                      * nico, e della descrizione               *
      *                      *-----------------------------------------*
       prn-fin-lr1-651.
      *                          *-------------------------------------*
      *                          * Preparazione                        *
      ...
      *                  * la classe statistica cliente, bensi' uno    *
      *                  * successivo                                  *
      *                  *---------------------------------------------*
       prn-liv-det-680.
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-031.
           move      "PER AGENTE - ZONA DI APPARTENENZA DEI CLIENTI     
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-032.
           move      "PER AGENTE - CATEGORIA DI CLIENTE                 
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-033.
           move      "PER AGENTE - CLASSIFICAZIONE STATISTICA DEI CLIENT
      -              "I                             "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-050.
      ...
      *              *-------------------------------------------------*
           if        rr-num-pdr           =    02 or
                     rr-num-pdr           =    03
                     if     p-res         >    12
                            go to stp-sub-int-age-200
                     else   go to stp-sub-int-age-150
           else      if     p-res         >    11
                            go to stp-sub-int-age-200
                     else   go to stp-sub-int-age-150.
       stp-sub-int-age-150.
      ...
      *                          * Classe 03                           *
      *                          *-------------------------------------*
           move      "Classificazione statistica                        
      -              "      "             to   w-stp-fnc-tit-asx      .
           go to     stp-fnc-vrt-030.
       stp-fnc-vrt-030.
      *                          *-------------------------------------*
