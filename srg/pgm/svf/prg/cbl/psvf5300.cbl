      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf5301:        *
      *                                                                *
      *                    Stampa statistica di vendita sul fatturato  *
      *                    per prodotto - classe statistica cliente.   *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "FATTURATO PER PRODOTTO - CLASSE CLIENTE "       .
      ...
      *        *  - Se codice zero : Tutti i prodotti                  *
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
      *        * Codice numerico del prodotto, descrizione             *
      *        *  - Se codice zero : 'Tutti i prodotti'                *
      *        *-------------------------------------------------------*
           05  rr-num-pro-des             pic  x(40)                  .
      *        *-------------------------------------------------------*
      *        * Tipi di prodotto da selezionare                       *
      *        *                                                       *
      *        *  - 99 : Tutti                                         *
      *        *  - 01 : Solo merci                                    *
      *        *  - 02 : Solo servizi                                  *
      *        *  - 03 : Solo imballi                                  *
      *        *  - 09 : Solo extra attivita' aziendale                *
      *        *                                                       *
      *        * Non significativo se selezionato un solo prodotto     *
      *        *-------------------------------------------------------*
           05  rr-tip-pro                 pic  9(02)                  .
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tip-pro.
               10  w-exp-tip-pro-num      pic  9(02)       value 05   .
               10  w-exp-tip-pro-lun      pic  9(02)       value 35   .
      ...
      *                  *---------------------------------------------*
           move      rr-num-pro           to   w-let-arc-dcp-num      .
           perform   let-arc-dcp-000      thru let-arc-dcp-999        .
      *                  *---------------------------------------------*
      *                  * Memorizzazione codice alfanumerico e de-    *
      *                  * scrizione                                   *
      *                  *---------------------------------------------*
           if        rr-num-pro           =    zero
                     move  spaces         to   rr-alf-pro
                     move  "Tutti i prodotti                        "
                                          to   rr-num-pro-des
           else      move  w-let-arc-dcp-alf
                                          to   rr-alf-pro
      ...
      *                  * normalizza la voce 'tipi di prodotto'       *
      *                  *---------------------------------------------*
           if        rr-num-pro           =    zero
                     go to acc-cod-pro-650.
           if        rr-tip-pro           =    zero
                     go to acc-cod-pro-650.
           move      zero                 to   rr-tip-pro             .
           perform   vis-tip-pro-000      thru vis-tip-pro-999        .
       acc-cod-pro-650.
      *                  *---------------------------------------------*
      *                  * Se impostato un codice diverso da zero si   *
      *                  * normalizza la voce 'tipo ordinamento per    *
      *                  * prodotto'                                   *
      *                  *---------------------------------------------*
           if        rr-num-pro           =    zero
                     go to acc-cod-pro-675.
           if        rr-tor-pro           =    zero
                     go to acc-cod-pro-675.
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se e' stato selezionato un solo pro-    *
      *                      * dotto : nessuna accettazione            *
      *                      *-----------------------------------------*
      ...
      *
           if        rr-tip-pro           =    99
                     move  01             to   v-num
           else if   rr-tip-pro           =    01
                     move  02             to   v-num
           else if   rr-tip-pro           =    02
                     move  03             to   v-num
           else if   rr-tip-pro           =    03
                     move  04             to   v-num
           else if   rr-tip-pro           =    04
                     move  05             to   v-num
           else if   rr-tip-pro           =    05
                     move  09             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  99             to   rr-tip-pro
           else if   v-num                =    02
                     move  01             to   rr-tip-pro
           else if   v-num                =    03
                     move  02             to   rr-tip-pro
           else if   v-num                =    04
                     move  03             to   rr-tip-pro
           else if   v-num                =    05
                     move  09             to   rr-tip-pro
           else      move  zero           to   rr-tip-pro             .
       acc-tip-pro-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Tipi di prodotto                        *
      *    *-----------------------------------------------------------*
       vis-tip-pro-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-050.
      *              *-------------------------------------------------*
      *              * Controllo : Tipi di prodotto                    *
      *              *-------------------------------------------------*
       tdo-ric-sel-051.
           if        rr-num-pro           not  = zero
                     go to tdo-ric-sel-100.
           if        rr-tip-pro           not  = zero
                     go to tdo-ric-sel-052.
           move      "Manca la voce 'Tipi di prodotto da selezionare' ! 
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-052.
           if        rr-tip-pro           =    99 or
                     ...
                     rr-tip-pro           =    09
                     go to tdo-ric-sel-100.
           move      "Voce 'Tipi di prodotto da selezionare' errata !   
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
