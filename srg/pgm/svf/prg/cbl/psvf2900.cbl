      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf2901:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " STATISTICA PER CLIENTI SENZA FATTURATO "       .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-sel-sco.
               10  w-exp-sel-sco-num      pic  9(02)       value 9    .
               10  w-exp-sel-sco-lun      pic  9(02)       value 40   .
               10  w-exp-sel-sco-tbl.
                   15  filler             pic  x(40) value
                            "[ ] Normale                             ".
                   15  filler             pic  x(40) value
                            "[ ] Esauriti i rapporti commerciali     ".
                   15  filler             pic  x(40) value
                            "[ ] Sostituito da ns. nuovo cliente     ".
                   15  filler             pic  x(40) value
                            "[ ] Cessata attivita'                   ".
                   15  filler             pic  x(40) value
      ...
      *                  * Selezione su status commerciale             *
      *                  *---------------------------------------------*
           perform   acc-sel-sco-000      thru acc-sel-sco-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
                     go to acc-ric-sel-200.
      ...
      *              *-------------------------------------------------*
      *              * Selezione su status clienti                     *
      *              *-------------------------------------------------*
           perform   pmt-sel-sco-000      thru pmt-sel-sco-999        .
      *              *-------------------------------------------------*
      *              * 1. periodo di riferimento, data minima          *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt per Selezione su status commercia- *
      *    * le                                                        *
      *    *-----------------------------------------------------------*
       pmt-sel-sco-000.
           move      "DS"                 to   v-ope                  .
           ...
           move      14                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su status cliente:"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-sel-sco-999.
      ...
      *                  * normalizza il tipo ordinamento clienti      *
      *                  *---------------------------------------------*
           if        rr-cod-cli           =    zero
      ...
      *                  *---------------------------------------------*
           if        rr-sel-sco           =    spaces
                     move  "X        "    to   rr-sel-sco             .
       acc-sel-sco-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "C"                  to   v-tip                  .
           move      w-exp-sel-sco-lun    to   v-car                  .
           move      w-exp-sel-sco-num    to   v-ldt                  .
           move      "B"                  to   v-edm                  .
           move      14                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sel-sco-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sel-sco           to   v-alf                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sel-sco-999.
       acc-sel-sco-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-sel-sco             .
       acc-sel-sco-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo : Data acquisizione clienti min - max *
      *              *-------------------------------------------------*
           if        rr-dac-min           not  = zero
                     go to tdo-ric-sel-120.
           move      "Manca la data acquisizione cliente iniziale !     
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-120.
           if        rr-dac-max           not  = zero
                     go to tdo-ric-sel-140.
           move      "Manca la data acquisizione cliente finale !       
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-140.
           if        rr-dac-max           not  < rr-dac-min
                     go to tdo-ric-sel-300.
           move      "La data acquisizione finale e' inferiore alla data
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Status commerciale del cliente                  *
      *              *-------------------------------------------------*
           if        rr-sel-sco           =    spaces
                     move  "X        "    to   rr-sel-sco             .
      *              *-------------------------------------------------*
