      ...
      *                       Ultima revisione:    NdK del 13/07/06    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma psvf5901:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-zer.
               10  w-exp-snx-zer-num      pic  9(02)       value 4    .
      ...
      *                  * Data codifica                               *
      *                  *---------------------------------------------*
           perform   acc-dat-cod-000      thru acc-dat-cod-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Tipo giacenza                                   *
      *              *-------------------------------------------------*
           perform   pmt-tip-gia-000      thru pmt-tip-gia-999        .
      *              *-------------------------------------------------*
      *              * Si/No solo saldo diverso da zero                *
      *              *-------------------------------------------------*
           perform   pmt-snx-zer-000      thru pmt-snx-zer-999        .
      *              *-------------------------------------------------*
      *              * Data per fatturato                              *
      *              *-------------------------------------------------*
      ...
      *              * Data codifica                                   *
      *              *-------------------------------------------------*
           perform   pmt-dat-cod-000      thru pmt-dat-cod-999        .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Codice filtro per selezione      *
      *    *-----------------------------------------------------------*
       pmt-fso-dcp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Tipo salto pagina                *
      *    *-----------------------------------------------------------*
       pmt-tip-slp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Data situazione di magazzino     *
      *    *-----------------------------------------------------------*
       pmt-dat-sit-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Tipo giacenza                    *
      *    *-----------------------------------------------------------*
       pmt-tip-gia-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Si/no solo saldo diverso da zero *
      *    *-----------------------------------------------------------*
       pmt-snx-zer-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Campo attualmente forzato al default        *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Data per fatturato                         *
      *    *-----------------------------------------------------------*
       acc-dat-fat-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * in Up                                       *
      *                  *---------------------------------------------*
           if        rr-dat-fat           not  = zero
                     go to acc-dat-fat-600.
           if        v-key                =    "UP  "
                     go to acc-dat-fat-600
           else      go to acc-dat-fat-100.
       acc-dat-fat-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data per fatturato                      *
      *    *-----------------------------------------------------------*
       vis-dat-fat-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           move      "NT"                 to   v-ope                  .
           move      "Il programma si basa sulla data di codifica in ana
      -              "grafica commerciale prodotto. "
                                          to   v-nt1                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-cod-999.
       acc-dat-cod-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-cod             .
       acc-dat-cod-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-dat-cod-100.
       acc-dat-cod-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione : Data codifica                           *
      *    *-----------------------------------------------------------*
       vis-dat-cod-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      "Manca la data Non venduti dal ... !"
                                          to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-800.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      zero                 to   rr-dat-fat             .
           move      zero                 to   rr-dat-cod             .
       nor-ric-sel-999.
           exit.
