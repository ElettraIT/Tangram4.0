      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione per il programma psvf6701:       *
      *                                                                *
      ...
      *                *-----------------------------------------------*
      *                * Valore ABC per l'agente nell'ambito della     *
      ...
      *            *---------------------------------------------------*
      *            * Numero agenti trattati per la classe              *
      *            *---------------------------------------------------*
               10  w-liv-csp-num-age      pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Totale generale                                       *
      *        *-------------------------------------------------------*
           05  w-liv-gen.
      ...
      *        *-------------------------------------------------------*
      *        * Sub-work per stampa del codice agente e del nominati- *
      *        * vo agente                                             *
      *        *-------------------------------------------------------*
           05  w-stp-cda-noa.
      *            *---------------------------------------------------*
      *            * Codice numerico dell'agente                       *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-cod      pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Mnemonico dell'agente                             *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-mne      pic  x(10)                  .
      *            *---------------------------------------------------*
      *            * Nominativo dell'agente                            *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-nom      pic  x(20)                  .
      *            *---------------------------------------------------*
      *            * Ragione sociale dell'agente                       *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-rag      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Contatori di comodo                               *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-c01      pic  9(03)                  .
               10  w-stp-cda-noa-c02      pic  9(03)                  .
      *            *---------------------------------------------------*
      *            * Area editata di comodo                            *
      *            *---------------------------------------------------*
               10  w-stp-cda-noa-e10      pic  x(10)                  .
      *        *-------------------------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tor-age           not  = 01 and
                     ...
                     rr-tor-age           not  = 04 and
                     rr-tor-age           not  = 05
                     move  spaces         to   srt-k02-ron-age
      ...
      *                          *-------------------------------------*
           if        rr-tor-age           not  = 07
                     move  spaces         to   srt-k02-mne-age
                     go to cmp-srt-rec-330.
      *                          *-------------------------------------*
      *                          * Memorizzazione                      *
      *                          *-------------------------------------*
           move      w-sel-fit-mne-age    to   srt-k02-mne-age        .
       cmp-srt-rec-330.
      *                      *-----------------------------------------*
      *                      * Flag di codice a zero o diverso da zero *
      *                      *-----------------------------------------*
      ...
      *                      * Codice agente                           *
      *                      *-----------------------------------------*
           move      w-sel-fit-cod-age    to   srt-k02-cod-age        .
       cmp-srt-rec-400.
      *                  *---------------------------------------------*
      *                  * Subchiave 3 : Per documento                 *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Valore ABC per l'agente nell'ambito     *
      ...
      *                          * Memorizzazione in area specifica    *
      *                          *-------------------------------------*
           move      srt-k02-val-abc      to   srt-val-abc-age        .
       agg-abc-sqz-650.
      *                      *-----------------------------------------*
      *                      * Eventuale aggiornamento del flag per    *
      *                      * record da non selezionare               *
      ...
      *                  * Codice agente                               *
      *                  *---------------------------------------------*
           move      srt-cod-age-cod      to   w-rot-l01-cod-age      .
      *              *-------------------------------------------------*
      *              * 2. livello di rottura                           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Numero di agenti trattati per la classe     *
      *                  * statistica del prodotto                     *
      *                  *---------------------------------------------*
           move      zero                 to   w-liv-csp-num-age      .
      *                  *---------------------------------------------*
      ...
      *                  * Numero documenti trattati per l'agente      *
      *                  *---------------------------------------------*
           move      zero                 to   w-liv-age-num-doc      .
      *                  *---------------------------------------------*
      *                  * Numero agenti trattati per la classe sta-   *
      *                  * tistica prodotto                            *
      *                  *---------------------------------------------*
           add       1                    to   w-liv-csp-num-age      .
       prn-ini-lr1-200.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-fin-lr1-330.
      *                  *---------------------------------------------*
      *                  * Subroutine per la stampa del codice agente  *
      *                  * e della suo nominativo                      *
      *                  *---------------------------------------------*
       prn-fin-lr1-331.
      *                      *-----------------------------------------*
      *                      * Preparazione                            *
      ...
      *                          * Codice agente                       *
      *                          *-------------------------------------*
           move      w-liv-age-cod-age    to   w-stp-cda-noa-cod      .
      *                          *-------------------------------------*
      *                          * Mnemonico agente                    *
      *                          *-------------------------------------*
           move      w-liv-age-mne-age    to   w-stp-cda-noa-mne      .
      *                          *-------------------------------------*
      *                          * Nominativo agente                   *
      *                          *-------------------------------------*
           move      w-liv-age-nom-age    to   w-stp-cda-noa-nom      .
      *                          *-------------------------------------*
      *                          * Ragione sociale agente              *
      *                          *-------------------------------------*
           move      w-liv-age-rag-age    to   w-stp-cda-noa-rag      .
       prn-fin-lr1-332.
      *                      *-----------------------------------------*
      *                      * Esecuzione                              *
      *                      *-----------------------------------------*
           perform   stp-cda-noa-000      thru stp-cda-noa-999        .
       prn-fin-lr1-340.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      w-liv-age-val-umi    to   w-stp-sta-qfp-umi      .
           move      w-liv-age-val-dec    to   w-stp-sta-qfp-dec      .
           move      w-liv-age-qta-p1r    to   w-stp-sta-qfp-q01      .
           move      w-liv-age-tot-p1r    to   w-stp-sta-qfp-f01      .
           move      w-liv-age-qta-p2r    to   w-stp-sta-qfp-q02      .
           move      w-liv-age-tot-p2r    to   w-stp-sta-qfp-f02      .
           move      w-liv-age-qta-p3r    to   w-stp-sta-qfp-q03      .
           move      w-liv-age-tot-p3r    to   w-stp-sta-qfp-f03      .
      ...
      *                  * mento trattato per l'agente oppure no       *
      *                  *---------------------------------------------*
           if        w-liv-age-num-doc    =    1
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-031.
           move      "PER 1. CODICE STATISTICO PRODOTTO - AGENTE        
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-032.
           move      "PER 2. CODICE STATISTICO PRODOTTO - AGENTE        
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-033.
           move      "PER 3. CODICE STATISTICO PRODOTTO - AGENTE        
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-034.
           move      "PER CLASSE MERCEOLOGICA PRODOTTO - AGENTE         
      -              "                              "
                                          to   w-stp-int-ti2-stp      .
           go to     stp-int-pag-050.
       stp-int-pag-050.
      ...
      *    *===========================================================*
      *    * Subroutine per la stampa del codice agente e del nomina-  *
      *    * tivo agente                                               *
      *    *                                                           *
      *    * - Parametri in input                                      *
      *    *                                                           *
      *    *   - w-stp-cda-noa-cod : Codice agente                     *
      *    *                                                           *
      *    *   - w-stp-cda-noa-mne : Mnemonico agente                  *
      *    *                                                           *
      *    *   - w-stp-cda-noa-nom : Nominativo agente                 *
      *    *                                                           *
      *    *   - w-stp-cda-noa-rag : Ragione sociale agente            *
      *    *                                                           *
      *    *-----------------------------------------------------------*
       stp-cda-noa-000.
      *              *-------------------------------------------------*
      *              * Se codice numerico a zero e codice alfanumerico *
      *              * a spaces si prepara la dicitura di non classi-  *
      *              * ficazione                                       *
      *              *-------------------------------------------------*
       stp-cda-noa-010.
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        w-stp-cda-noa-cod    not  = zero   or
                     w-stp-cda-noa-mne    not  = spaces
                     go to stp-cda-noa-100.
       stp-cda-noa-020.
      *                  *---------------------------------------------*
      *                  * Codice                                      *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      spaces               to   p-sgn                  .
           move      spaces               to   p-edm                  .
           move      w-stp-cda-noa-cod    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      ...
      *                  * comodo                                      *
      *                  *---------------------------------------------*
           move      p-edt                to   w-stp-cda-noa-e10      .
      *                  *---------------------------------------------*
      *                  * A trattamento codice editato                *
      *                  *---------------------------------------------*
           go to     stp-cda-noa-400.
       stp-cda-noa-300.
      *              *-------------------------------------------------*
      ...
      *                  * oppure no                                   *
      *                  *---------------------------------------------*
           if        w-stp-cda-noa-mne    =    spaces
                     go to stp-cda-noa-330.
       stp-cda-noa-320.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      w-stp-cda-noa-mne    to   w-stp-cda-noa-e10      .
      *                      *-----------------------------------------*
      *                      * A trattamento codice editato            *
      *                      *-----------------------------------------*
           go to     stp-cda-noa-400.
      ...
      *                      *-----------------------------------------*
           move      spaces               to   w-stp-cda-noa-e10      .
           string    "("        delimited by   size
                     p-edt      delimited by spaces
                     ")"        delimited by size
                                          into w-stp-cda-noa-e10      .
      *                      *-----------------------------------------*
      *                      * A trattamento codice editato            *
      *                      *-----------------------------------------*
           go to     stp-cda-noa-400.
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      w-stp-cda-noa-e10    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-cda-noa-500.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      017                  to   p-pos                  .
           if        w-prs-svf-age-ron    =    "R"
                     move  w-stp-cda-noa-rag
                                          to   p-alf
           else      move  w-stp-cda-noa-nom
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-cda-noa-999.
