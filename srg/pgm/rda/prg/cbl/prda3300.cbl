      ...
      *                       Ultima revisione:    NdK del 20/02/09    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma prda3301:        *
      *                                                                *
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Dicitura per la stampa                     *
      *        *-------------------------------------------------------*
           05  w-exp-dct-stp.
               10  w-exp-dct-stp-num      pic  9(02)       value 3    .
               10  w-exp-dct-stp-lun      pic  9(02)       value 40   .
               10  w-exp-dct-stp-tbl.
                   15  filler             pic  x(40) value
                            "Gli importi sono stati versati          ".
                   15  filler             pic  x(40) value
                            "Gli importi saranno versati             ".
                   15  filler             pic  x(40) value
                            "Gli importi sono stati / saranno versati".
      *        *-------------------------------------------------------*
      ...
      *                  * Data pagamento min                          *
      *                  *---------------------------------------------*
           perform   acc-dpg-min-000      thru acc-dpg-min-999        .
      ...
      *                  * Data pagamento max                          *
      *                  *---------------------------------------------*
           perform   acc-dpg-max-000      thru acc-dpg-max-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
           perform   acc-ann-crt-000      thru acc-ann-crt-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Dicitura per stampa                         *
      *                  *---------------------------------------------*
           perform   acc-dct-stp-000      thru acc-dct-stp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      12                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Data da esporre in stampa  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Data pagamento min                         *
      *    *-----------------------------------------------------------*
       acc-dpg-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dpg-min-999.
       acc-dpg-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dpg-min             .
       acc-dpg-min-400.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-dpg-min           not  = zero
                     go to acc-dpg-min-600.
           if        v-key                not  = "UP  "
                     go to acc-dpg-min-100.
           if        w-cnt-sts-imp-ric    =    spaces
                     go to acc-dpg-min-600
           else      go to acc-dpg-min-100.
       acc-dpg-min-600.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-dpg-max           not  = zero
                     go to acc-dpg-max-100.
      *                      *-----------------------------------------*
      *                      * Date and time da segreteria             *
      *                      *-----------------------------------------*
           move      "DT"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dpg-max-999.
       acc-dpg-max-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dpg-max             .
       acc-dpg-max-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-dpg-max           not  = zero
                     go to acc-dpg-max-600.
           if        v-key                not  = "UP  "
                     go to acc-dpg-max-100.
           if        w-cnt-sts-imp-ric    =    spaces
                     go to acc-dpg-max-600
           else      go to acc-dpg-max-100.
       acc-dpg-max-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-cod-fnt           =    zero
                     move  "Tutti"        to   rr-cod-fnt-rag
                                          ...
                                          to   rr-cod-fnt-rag         .
      *                  *---------------------------------------------*
      *                  * Visualizzazione ragione sociale, indirizzo, *
      *                  * localita'                                   *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Anno di certificazione                     *
      *    *-----------------------------------------------------------*
       acc-ann-crt-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                      * Test se necessario                      *
      *                      *-----------------------------------------*
           if        rr-ann-crt           not  = spaces
                     go to acc-ann-crt-100.
           if        rr-dpg-min           =    zero
                     go to acc-ann-crt-100.
      *                      *-----------------------------------------*
      *                      * Editing secolo anno da data minima      *
      *                      *-----------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      04                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-ann-crt-999.
       acc-ann-crt-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-ann-crt             .
       acc-ann-crt-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Se a zero : reimpostazione                  *
      *                  *---------------------------------------------*
           if        rr-ann-crt           not  = zero
                     go to acc-ann-crt-600.
           if        v-key                not  = "UP  "
                     go to acc-ann-crt-100.
           if        w-cnt-sts-imp-ric    =    spaces
                     go to acc-ann-crt-600
           else      go to acc-ann-crt-100.
       acc-ann-crt-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data stampa                *
      *    *-----------------------------------------------------------*
       acc-dat-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-dat-stp           not  = zero
                     go to acc-dat-stp-600.
           if        v-key                not  = "UP  "
                     go to acc-dat-stp-100.
           if        w-cnt-sts-imp-ric    =    spaces
                     go to acc-dat-stp-600
           else      go to acc-dat-stp-100.
       acc-dat-stp-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Dicitura per la stampa               *
      *    *-----------------------------------------------------------*
       acc-dct-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-dct-stp-lun    to   v-car                  .
           move      w-exp-dct-stp-num    to   v-ldt                  .
           move      "VDM#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      14                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-dct-stp-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
      *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dct-stp-999.
       acc-dct-stp-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "V"            to   rr-dct-stp
           else if   v-num                =    02
                     move  "D"            to   rr-dct-stp
           else if   v-num                =    03
                     move  "M"            to   rr-dct-stp
           else      move  spaces         to   rr-dct-stp             .
       acc-dct-stp-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-dct-stp           =    spaces
                     go to acc-dct-stp-100.
       acc-dct-stp-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-ann-crt             .
           move      zero                 to   rr-dpg-min             .
           move      zero                 to   rr-dpg-max             .
           move      zero                 to   rr-dat-stp             .
           move      spaces               to   rr-dct-stp             .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo su data pagamento min                 *
      *              *-------------------------------------------------*
           if        rr-dpg-min           not  = zero
                     go to tdo-ric-sel-200.
           move      "La data pagamento minima non puo' mancare !"
                                          to   w-box-msg-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Controllo su data pagamento max                 *
      *              *-------------------------------------------------*
           if        rr-dpg-max           not  = zero
                     go to tdo-ric-sel-300.
           move      "La data pagamento massima non puo' mancare !"
                                          to   w-box-msg-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-300.
      *              *-------------------------------------------------*
      *              * Controllo su data pagamento min - max           *
      *              *-------------------------------------------------*
           if        rr-dpg-max           not  < rr-dpg-min
                     go to tdo-ric-sel-400.
           move      "Data pagamento massima minore della minima !"
                                          to   w-box-msg-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-400.
      *              *-------------------------------------------------*
      *              * Controllo su anno certificazione                *
      *              *-------------------------------------------------*
           if        rr-ann-crt           not  = spaces
                     go to tdo-ric-sel-500.
           move      "Manca l'anno per la certificazione !"
                                          to   w-box-msg-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-500.
      *              *-------------------------------------------------*
      *              * Controllo su data stampa                        *
      *              *-------------------------------------------------*
           if        rr-dat-stp           not  = zero
                     go to tdo-ric-sel-800.
           move      "Manca la data da esporre in stampa !"
                                          to   w-box-msg-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-800.
      *              *-------------------------------------------------*
