      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione date pagamento documenti           *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   GESTIONE DATE PAGAMENTO DOCUMENTI    "       .
      ...
      *        * Data pagamento                                        *
      *        *-------------------------------------------------------*
           05  w-rig-dat-pag              pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * Totale documento                                      *
      *        *-------------------------------------------------------*
           05  w-rig-tot-doc              pic  9(09)                  .
      *        *-------------------------------------------------------*
      *        * Un intero record [ram]                                *
      *        *-------------------------------------------------------*
           05  w-rig-rec-ram.
      ...
      *    *-----------------------------------------------------------*
       01  w-cmp-rec.
      *        *-------------------------------------------------------*
      *        * Work per composizione record [ram]                    *
      *        *-------------------------------------------------------*
           05  w-cmp-rec-ram.
      *            *---------------------------------------------------*
      *            * Protocollo                                        *
      *            *---------------------------------------------------*
               10  w-cmp-rec-ram-prt      pic  9(11)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-det-sel-ram.
               10  w-det-sel-ram-inc      pic  9(11)                  .
               10  w-det-sel-ram-sdo      pic  x(01)                  .
      *        *-------------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       vis-lin-cor-520.
      *                  *---------------------------------------------*
      *                  * Editing Data pagamento                      *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
           move      w-rig-dat-pag        to   v-dat                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-lin-imm-dat-pag      .
       vis-lin-cor-800.
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione 1. campo riga corpo espansa : Fornitore      *
      *    *-----------------------------------------------------------*
       acc-cod-fnt-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-ram-num-doc       to   w-all-str-cat (1)      .
      ...
      *                  *---------------------------------------------*
      ...
      -              " esecuzione ---               "
       ...
       stp-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Start su [ram] per data documento               *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "IDEDOC    "         to   f-key                  .
           move      w-tes-dat-min        to   rf-ram-dat-doc         .
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-ram-dat-doc       >    w-tes-dat-min
      ...
      *                  *---------------------------------------------*
           if        rf-ram-dat-doc       >    w-tes-dat-max
                     go to stp-srt-inp-900.
           go to     stp-srt-inp-400.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Selezione su pagamento                      *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se solo da pagare                  *
      *                      *-----------------------------------------*
           if        w-tes-snx-sos        not  = 01
                     go to stp-srt-inp-440.
           if        rf-ram-dat-pag       not  = zero
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-ram-cod-fnt       not  = w-tes-cod-arc
                     go to stp-srt-inp-200.
       stp-srt-inp-460.
       stp-srt-inp-600.
      *              *-------------------------------------------------*
      ...
      *                  * Record [ram]                                *
      *                  *---------------------------------------------*
           move      rf-ram               to   w-rig-rec-ram          .
           move      rf-ram-cod-fnt       to   w-rig-cod-fnt          .
           move      rf-ram-dpz-fnt       to   w-rig-dpz-fnt          .
           move      rf-ram-dat-pag       to   w-rig-dat-pag          .
           move      rf-ram-tot-ibl       to   w-rig-tot-doc          .
      *                  *---------------------------------------------*
      *                  * Valori indiretti                            *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-rig-rec-ram        to   rf-ram                 .
           move      rf-ram-num-prt       to   w-cmp-rec-ram-prt      .
       sca-fil-rap-500.
      *              *-------------------------------------------------*
      *              * Aggiornamento bolle di accompagnamento          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Ottenimento record [ram]                    *
      *                  *---------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      w-cmp-rec-ram-prt    to   rf-ram-num-prt         .
           move      "pgm/rda/fls/ioc/obj/iofram"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
      *                  * Aggiornamento data pagamento                *
      *                  *---------------------------------------------*
           move      w-rig-dat-pag        to   rf-ram-dat-pag         .
       sca-fil-rap-700.
      *                  *---------------------------------------------*
      *                  * Update record [ram]                         *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Tasto funzione "FIND" su archivio movimenti               *
      *    *-----------------------------------------------------------*
       fnd-arc-ram-000.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/rda/prg/obj/prda3202"                       .
       cnc-rlt-sup-999.
           exit.
