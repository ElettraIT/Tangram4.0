      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella tipi di contributo         *
      *                                                                *
      *================================================================*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-tcb        to   w-cod-mne-zco-cod      .
           move      04                   to   w-cod-mne-zco-lin      .
           move      30                   to   w-cod-mne-zco-pos      .
           move      07                   to   w-cod-mne-zco-dln      .
           move      30                   to   w-cod-mne-zco-dps      .
           move      "<B"                 to   v-edm                  .
      ...
      *                  * Descrizione                                 *
      *                  *---------------------------------------------*
           perform   acc-des-tcb-000      thru acc-des-tcb-999        .
           if        v-key                =    "NXSC"
      ...
      *                  * % ritenuta                                  *
      *                  *---------------------------------------------*
           perform   acc-per-tcb-000      thru acc-per-tcb-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *                  * % imponibile                                *
      *                  *---------------------------------------------*
           perform   acc-pim-tcb-000      thru acc-pim-tcb-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   vis-des-tcb-000      thru vis-des-tcb-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % ritenuta                                      *
      *              *-------------------------------------------------*
           perform   vis-per-tcb-000      thru vis-per-tcb-999        .
      *              *-------------------------------------------------*
      *              * % imponibile                                    *
      *              *-------------------------------------------------*
           perform   vis-pim-tcb-000      thru vis-pim-tcb-999        .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     vis-tes-reg-999.
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   pmt-des-tcb-000      thru pmt-des-tcb-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % ritenuta                                      *
      *              *-------------------------------------------------*
           perform   pmt-per-tcb-000      thru pmt-per-tcb-999        .
      *              *-------------------------------------------------*
      *              * % imponibile                                    *
      *              *-------------------------------------------------*
           perform   pmt-pim-tcb-000      thru pmt-pim-tcb-999        .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     pmt-tes-reg-999.
      ...
      *                  *---------------------------------------------*
           if        w-tes-des-tcb (1)    =    spaces
                     go to acc-des-tcb-100.
       acc-des-tcb-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  * l'ordinamento                               *
      *                  *---------------------------------------------*
           move      w-tes-des-tcb (1)    to   w-all-str-alf          .
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           ...
           move      spaces               to   w-tes-des-tcb (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-tcb        to   rf-zco-cod-tcb         .
           move      "pgm/rda/fls/ioc/obj/iofzco"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [zco]                        *
      *                          *-------------------------------------*
           move      rf-zco-ide-dat       to   w-tes-ide-dat (1)      .
           move      rf-zco-ide-ute       to   w-tes-ide-ute (1)      .
           move      rf-zco-ide-fas       to   w-tes-ide-fas (1)      .
           move      rf-zco-des-tcb       to   w-tes-des-tcb (1)      .
           move      rf-zco-des-key       to   w-tes-des-key (1)      .
           move      rf-zco-cod-mne       to   w-tes-cod-mne (1)      .
           move      rf-zco-per-tcb       to   w-tes-per-tcb (1)      .
           move      rf-zco-pim-tcb       to   w-tes-pim-tcb (1)      .
           move      rf-zco-alx-exp       to   w-tes-alx-exp (1)      .
       rou-let-reg-800.
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zco]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zco-000      thru wrt-rec-zco-999        .
      ...
      *                      * Rewrite record [zco]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zco-000      thru rew-rec-zco-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zco]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zco-000      thru del-rec-zco-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zco]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zco-000.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-zco-ide-dat         .
           move      s-ute                to   rf-zco-ide-ute         .
           move      s-fas                to   rf-zco-ide-fas         .
           move      w-tes-cod-mne (1)    to   rf-zco-cod-mne         .
           move      w-tes-des-key (1)    to   rf-zco-des-key         .
           move      w-tes-des-tcb (1)    to   rf-zco-des-tcb         .
           move      w-tes-per-tcb (1)    to   rf-zco-per-tcb         .
           move      w-tes-pim-tcb (1)    to   rf-zco-pim-tcb         .
           move      w-tes-alx-exp (1)    to   rf-zco-alx-exp         .
       cmp-rec-zco-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zco]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zco-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zco-000      thru cmp-rec-zco-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
