      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella codici tributo             *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " CODICI TRIBUTO PER RITENUTA D'ACCONTO  "       .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice                        *
      *    *-----------------------------------------------------------*
       pmt-cod-trb-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo chiave : Codice                        *
      *    *-----------------------------------------------------------*
       acc-cod-trb-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-trb        to   w-cod-mne-ztr-cod      .
           move      04                   to   w-cod-mne-ztr-lin      .
           move      30                   to   w-cod-mne-ztr-pos      .
           move      07                   to   w-cod-mne-ztr-dln      .
           move      30                   to   w-cod-mne-ztr-dps      .
           move      "<B"                 to   v-edm                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice                     *
      *    *-----------------------------------------------------------*
       vis-cod-trb-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *                  * Descrizione                                 *
      *                  *---------------------------------------------*
           perform   acc-des-trb-000      thru acc-des-trb-999        .
           if        v-key                =    "NXSC"
      ...
      *                  * % imponibile                                *
      *                  *---------------------------------------------*
           perform   acc-pim-trb-000      thru acc-pim-trb-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   vis-des-trb-000      thru vis-des-trb-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % imponibile                                    *
      *              *-------------------------------------------------*
           perform   vis-pim-trb-000      thru vis-pim-trb-999        .
      *              *-------------------------------------------------*
      *              * Descrizione per la stampa                       *
      *              *-------------------------------------------------*
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   pmt-des-trb-000      thru pmt-des-trb-999        .
      *              *-------------------------------------------------*
      *              * Codice mnemonico                                *
      *              *-------------------------------------------------*
      ...
      *              * % imponibile                                    *
      *              *-------------------------------------------------*
           perform   pmt-pim-trb-000      thru pmt-pim-trb-999        .
      *              *-------------------------------------------------*
      *              * Descrizione per la stampa                       *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-des-trb (1)    =    spaces
                     go to acc-des-trb-100.
       acc-des-trb-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  * l'ordinamento                               *
      *                  *---------------------------------------------*
           move      w-tes-des-trb (1)    to   w-all-str-alf          .
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           ...
           move      spaces               to   w-tes-des-trb (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-trb        to   rf-ztr-cod-trb         .
           move      "pgm/rda/fls/ioc/obj/iofztr"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [ztr]                        *
      *                          *-------------------------------------*
           move      rf-ztr-ide-dat       to   w-tes-ide-dat (1)      .
           move      rf-ztr-ide-ute       to   w-tes-ide-ute (1)      .
           move      rf-ztr-ide-fas       to   w-tes-ide-fas (1)      .
           move      rf-ztr-des-trb       to   w-tes-des-trb (1)      .
           move      rf-ztr-des-key       to   w-tes-des-key (1)      .
           move      rf-ztr-cod-mne       to   w-tes-cod-mne (1)      .
           move      rf-ztr-per-trb       to   w-tes-per-trb (1)      .
           move      rf-ztr-pim-trb       to   w-tes-pim-trb (1)      .
           move      rf-ztr-des-stp       to   w-tes-des-stp (1)      .
           move      rf-ztr-alx-exp       to   w-tes-alx-exp (1)      .
       rou-let-reg-800.
      *                      *-----------------------------------------*
      ...
      *                      * Write record [ztr]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-ztr-000      thru wrt-rec-ztr-999        .
      ...
      *                      * Rewrite record [ztr]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-ztr-000      thru rew-rec-ztr-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [ztr]                             *
      *              *-------------------------------------------------*
           perform   del-rec-ztr-000      thru del-rec-ztr-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [ztr]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-ztr-000.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-ztr-ide-dat         .
           move      s-ute                to   rf-ztr-ide-ute         .
           move      s-fas                to   rf-ztr-ide-fas         .
           move      w-tes-cod-mne (1)    to   rf-ztr-cod-mne         .
           move      w-tes-des-key (1)    to   rf-ztr-des-key         .
           move      w-tes-des-trb (1)    to   rf-ztr-des-trb         .
           move      w-tes-per-trb (1)    to   rf-ztr-per-trb         .
           move      w-tes-pim-trb (1)    to   rf-ztr-pim-trb         .
           move      w-tes-des-stp (1)    to   rf-ztr-des-stp         .
           move      w-tes-alx-exp (1)    to   rf-ztr-alx-exp         .
       cmp-rec-ztr-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [ztr]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-ztr-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-ztr-000      thru cmp-rec-ztr-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
