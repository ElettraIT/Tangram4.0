      ...
      *                       Ultima revisione:    NdK del 12/03/19    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione documenti con bollo virtuale       *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  GESTIONE DOCUMENTI CON BOLLO VIRTUALE "       .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-blv.
               10  w-exp-snx-blv-num      pic  9(02)       value 03   .
               10  w-exp-snx-blv-lun      pic  9(02)       value 40   .
               10  w-exp-snx-blv-tbl.
                   15  filler             pic  x(40) value
                            "Tutti i documenti                       ".
                   15  filler             pic  x(40) value
                            "solo quelli Con bollo virtuale          ".
                   15  filler             pic  x(40) value
                            "solo quelli Senza bollo virtuale        ".
      *        *-------------------------------------------------------*
      *        * Work per : No/Si bollo virtuale                       *
      *        *-------------------------------------------------------*
           05  w-exp-blv-snx.
               10  w-exp-blv-snx-num      pic  9(02)       value 2    .
               10  w-exp-blv-snx-lun      pic  9(02)       value 02   .
               10  w-exp-blv-snx-tbl.
      ...
      *    *-----------------------------------------------------------*
       01  w-cmp-rec.
      *        *-------------------------------------------------------*
      *        * Work per composizione record [fit]                    *
      *        *-------------------------------------------------------*
           05  w-cmp-rec-fit.
      *            *---------------------------------------------------*
      *            * Protocollo                                        *
      *            *---------------------------------------------------*
               10  w-cmp-rec-fit-prt      pic  9(09)                  .
      ...
      *                  * Si/no bollo virtuale                        *
      *                  *---------------------------------------------*
           perform   acc-snx-blv-000      thru acc-snx-blv-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * Si/no bollo virtuale                            *
      *              *-------------------------------------------------*
           perform   vis-snx-blv-000      thru vis-snx-blv-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Si/no bollo virtuale                        *
      *                  *---------------------------------------------*
           perform   pmt-snx-blv-000      thru pmt-snx-blv-999        .
       pmt-key-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per : Si/no bollo virtuale        *
      *    *-----------------------------------------------------------*
       pmt-snx-blv-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-blv-lun    to   v-car                  .
           move      w-exp-snx-blv-num    to   v-ldt                  .
           move      "TCS#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-blv-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-blv-snx        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-blv-999.
       acc-snx-blv-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-blv-snx          .
       acc-snx-blv-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-blv-snx        not  = 01 and
                     w-tes-blv-snx        not  = 02 and
                     w-tes-blv-snx        not  = 03
                     go to acc-snx-blv-100.
       acc-snx-blv-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           perform   acc-svf-snx-000      thru acc-svf-snx-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
      ...
      *                  *---------------------------------------------*
           if        w-rig-blv-snx        =    spaces
                     move  spaces         to   w-lin-imm-snx-blv
           else      move  "*"            to   w-lin-imm-snx-blv      .
       vis-lin-cor-500.
      *                  *---------------------------------------------*
      ...
      *              * Esclusione da statistiche                       *
      *              *-------------------------------------------------*
           perform   vis-svf-snx-000      thru vis-svf-snx-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *              * Esclusione da statistiche                       *
      *              *-------------------------------------------------*
           perform   pmt-svf-snx-000      thru pmt-svf-snx-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Si/no bollo virtuale             *
      *    *-----------------------------------------------------------*
       pmt-svf-snx-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-blv-snx-lun    to   v-car                  .
           move      w-exp-blv-snx-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      22                   to   v-pos                  .
           move      w-exp-blv-snx-tbl    to   v-txt                  .
      *
           if        w-rig-blv-snx        =    spaces
                     move  1              to   v-num
           else if   w-rig-blv-snx        =    "#"
                     move  2              to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-svf-snx-999.
       acc-svf-snx-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    1
                     move  spaces         to   w-rig-blv-snx
           else if   v-num                =    2
                     move  "#"            to   w-rig-blv-snx
           else      move  spaces         to   w-rig-blv-snx          .
       acc-svf-snx-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-fit-cod-tmo       to   w-all-str-cat (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-dat-min        to   rf-fit-dat-doc         .
           move      w-tes-dpz-inu        to   rf-fit-cod-dpz         .
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-fit-dat-doc       >    w-tes-dat-min
      ...
      *                  *---------------------------------------------*
           if        rf-fit-dat-doc       >    w-tes-dat-max
                     go to stp-srt-inp-900.
           go to     stp-srt-inp-400.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rf-fit-cod-dpz       not  = w-tes-dpz-inu
                     go to stp-srt-inp-200.
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Selezione su si/no bollo virtuale           *
      *                  *---------------------------------------------*
           go to     stp-srt-inp-430
                     stp-srt-inp-432
                     stp-srt-inp-434
                     depending            on   w-tes-blv-snx          .
       stp-srt-inp-430.
      *                  *---------------------------------------------*
      *                  * Nessuna selezione su si/no bollo virtuale   *
      *                  *---------------------------------------------*
           go to     stp-srt-inp-440.
       stp-srt-inp-432.
      *                  *---------------------------------------------*
      *                  * Solo con bollo virtuale                     *
      *                  *---------------------------------------------*
           if        rf-fit-flg-nbx (03)  =    spaces
                     go to stp-srt-inp-200
           else      go to stp-srt-inp-440.
       stp-srt-inp-434.
      *                  *---------------------------------------------*
      *                  * Solo senza bollo virtuale                   *
      *                  *---------------------------------------------*
           if        rf-fit-flg-nbx (03)  not  = spaces
                     go to stp-srt-inp-200
           else      go to stp-srt-inp-440.
       stp-srt-inp-440.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-fit-cod-cli       not  = w-tes-cod-cli
                     go to stp-srt-inp-200.
       stp-srt-inp-460.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-fit-cod-tmo       not  = w-tes-tip-mov
                     go to stp-srt-inp-200.
       stp-srt-inp-470.
       stp-srt-inp-600.
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      rf-fit               to   w-rig-rec-fit          .
           move      rf-fit-cod-cli       to   w-rig-cod-cli          .
           move      rf-fit-dpz-cli       to   w-rig-dpz-cli          .
           move      rf-fit-flg-nbx (3)   to   w-rig-blv-snx          .
      *                      *-----------------------------------------*
      *                      * Anagrafica cliente                      *
      *                      *-----------------------------------------*
      ...
      *                      * Valuta                                  *
      *                      *-----------------------------------------*
           move      rf-fit-sgl-vpf       to   w-rig-sgl-vpf          .
           move      rf-fit-dec-vpf       to   w-rig-dec-vpf          .
       car-fil-rap-800.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-rig-rec-fit        to   rf-fit                 .
           move      rf-fit-num-prt       to   w-cmp-rec-fit-prt      .
       sca-fil-rap-500.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-cmp-rec-fit-prt    to   rf-fit-num-prt         .
           move      "pgm/fat/fls/ioc/obj/ioffit"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        w-rig-blv-snx        =    rf-fit-flg-nbx (3)
      ...
      *                  *---------------------------------------------*
           move      w-rig-blv-snx        to   rf-fit-flg-nbx (3)     .
       sca-fil-rap-700.
      *                  *---------------------------------------------*
      *                  * Update record [fit]                         *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/fat/prg/obj/pfat9302"                       .
       cnc-rlt-sup-999.
           exit.
