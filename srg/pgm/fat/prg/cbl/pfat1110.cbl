      ...
      *                                   Fase:    fat111              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 07/01/92    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su tabella addebiti e com-   *
      *                    menti per fatturazione                      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  ADDEBITI E COMMENTI PER FATTURAZIONE  "       .
      ...
      *                      *-----------------------------------------*
      *                      * Salto in funzione del tipo ordinamento  *
      *                      *-----------------------------------------*
           if        rr-tip-ord           =    "M"
      ...
      *
           ...
           move      10                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su tabella addebiti e commenti per fattu
      -              "razione:"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo selezione : Tipo record             *
      *    *-----------------------------------------------------------*
       vis-tip-rec-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-tip-rec           to   w-cod-mne-zac-tip      .
           move      rr-cod-min           to   w-cod-mne-zac-cod      .
           move      15                   to   w-cod-mne-zac-lin      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-zac-ope      .
           move      rr-tip-rec           to   w-cod-mne-zac-tip      .
           move      rr-cod-max           to   w-cod-mne-zac-cod      .
           move      15                   to   w-cod-mne-zac-lin      .
           move      52                   to   w-cod-mne-zac-pos      .
           move      zero                 to   w-cod-mne-zac-dln      .
           move      zero                 to   w-cod-mne-zac-dps      .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-tip-rec           to   rf-zac-tip-rec         .
           move      rr-cod-min           to   rf-zac-cod-aoc         .
           move      "pgm/fat/fls/ioc/obj/iofzac"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-tip-rec           to   rf-zac-tip-rec         .
           move      rr-des-min           to   rf-zac-des-key         .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-tip-rec           to   rf-zac-tip-rec         .
           move      rr-mne-min           to   rf-zac-mne-aoc         .
           move      zero                 to   rf-zac-cod-aoc         .
           move      "pgm/fat/fls/ioc/obj/iofzac"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           if        rr-tip-rec           =    01
                     move  "Nessun tipo di addebito entro i limiti asseg
      -                    "nati !"
                                          to   v-not
           else if   rr-tip-rec           =    02
                     move  "Nessun tipo di commento entro i limiti asseg
      -                    "nati !"
                                          to   v-not
           else      move  "Nessun tipo di dicitura entro i limiti asseg
      -                    "nati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [zac]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/fat/fls/ioc/obj/iofzac"
                                          to   s-pat                  .
      ...
      *              * Test su Tipo record                             *
      *              *-------------------------------------------------*
           if        rf-zac-tip-rec       not  = rr-tip-rec
                     move  "#"            to   w-cnt-qry-flg-sub
      ...
      *              * Selezione su codice                             *
      *              *-------------------------------------------------*
           if        rf-zac-cod-aoc       <    rr-cod-min or
                     rf-zac-cod-aoc       >    rr-cod-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su descrizione                        *
      *              *-------------------------------------------------*
           if        rf-zac-des-key       <    rr-des-min or
                     rf-zac-des-key       >    rr-des-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su mnemonico                          *
      *              *-------------------------------------------------*
           if        rf-zac-mne-aoc       <    rr-mne-min or
                     rf-zac-mne-aoc       >    rr-mne-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
       qry-sel-rec-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      33                   to   v-pos                  .
           move      rf-zac-rig-aoc (1)   to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-zac-cod-aoc       to   w-mpn-cod-aoc          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-300.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-aoc'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *
           if        rr-tip-rec           =    01
                     move  "    ADDEBITI USUALI PER FATTURAZIONE    "
                                          to   v-alf
           else if   rr-tip-rec           =    02
                     move  "    COMMENTI USUALI PER FATTURAZIONE    "
                                          to   v-alf
           else      move  "  DICITURE PER DOCUMENTI FATTURAZIONE   "
                                          to   v-alf                  .
      *
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
