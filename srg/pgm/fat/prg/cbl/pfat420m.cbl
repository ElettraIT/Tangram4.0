      ...
      *                                                                *
      *                    Tasti funzione F1, F2, F3 e F4              *
      *                    per cambiare ordinamento e modalita'        *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   GESTIONE DISTINTA INVIO DOCUMENTI    "       .
      ...
      *        *-------------------------------------------------------*
      *        * Si/no bollo virtuale                                  *
      *        *-------------------------------------------------------*
           05  w-rig-snx-blv              pic  x(01)                  .
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-cmp-rec.
      *        *-------------------------------------------------------*
      *        * Work per composizione record [ddx]                    *
      *        *-------------------------------------------------------*
           05  w-cmp-rec-ddx.
      *            *---------------------------------------------------*
      *            * Protocollo                                        *
      *            *---------------------------------------------------*
               10  w-cmp-rec-ddx-prt      pic  9(09)                  .
      *            *---------------------------------------------------*
      *            * Progressivo                                       *
      *            *---------------------------------------------------*
               10  w-cmp-rec-ddx-prg      pic  9(05)                  .
      ...
      *
           if        w-all-str-num        =    2
                     move  w-all-str-cat (2)
                                          to   w-lin-imm-num-doc
           else      move  rf-ddx-num-doc to   w-lin-imm-num-doc      .
      *
           move      06                   to   w-all-str-lun          .
           move      w-lin-imm-num-doc    to   w-all-str-alf          .
      ...
      *                  *---------------------------------------------*
           if        w-rig-snx-blv        =    "#"
                     move  "*"            to   w-lin-imm-snx-blv
           else      move  spaces         to   w-lin-imm-snx-blv      .
       vis-lin-cor-550.
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Data invio                       *
      *    *-----------------------------------------------------------*
       pmt-dat-inv-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           if        w-rig-ind-eml        not  = spaces
                     perform  vis-ind-eml-000
                                          thru vis-ind-eml-999
           else      perform  vis-cod-sdi-000
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-ddx-tmo-doc       to   w-all-str-cat (1)      .
           move      rf-ddx-num-doc       to   w-all-str-cat (2)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      48                   to   v-pos                  .
      *
           if        w-rig-snx-blv        =    spaces
                     move  spaces         to   v-alf
           else      move  "{Bollo virtuale}"
                                          to   v-alf                  .
      *
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
...
______*              go to rou-let-reg-999.
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-200.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [ddx]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/fat/fls/ioc/obj/iofddx"
                                          to   s-pat                  .
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-ddx-dat-doc       to   srt-dat-key            .
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-ddx-cod-arc       to   srt-cod-key            .
           move      spaces               to   srt-rag-key            .
      *                      *-----------------------------------------*
      *                      * Oltre                                   *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      zero                 to   srt-cod-key            .
      *
           move      rf-ddx-cod-arc       to   w-let-arc-dcc-cli      .
           move      rf-ddx-dpz-arc       to   w-let-arc-dcc-dpz      .
      ...
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-640.
      *                      *-----------------------------------------*
      *                      * Ordinamento per Protocollo documento    *
      *                      *-----------------------------------------*
           move      rf-ddx-prt-doc       to   srt-prt-key            .
           move      zero                 to   srt-dat-key            .
           move      zero                 to   srt-cod-key            .
           move      spaces               to   srt-rag-key            .
      *                      *-----------------------------------------*
      ...
      *                  * Record [ddx]                                *
      *                  *---------------------------------------------*
           move      rf-ddx               to   w-rig-rec-ddx          .
           move      rf-ddx-cod-arc       to   w-rig-cod-cli          .
           move      rf-ddx-dpz-arc       to   w-rig-dpz-cli          .
      *
           if        rf-ddx-flg-ain       =    spaces
                     move  "S"            to   w-rig-snx-inv
           else      move  "N"            to   w-rig-snx-inv          .
      *
           move      rf-ddx-dat-inv       to   w-rig-dat-inv          .
      *
           if        rf-ddx-flg-esi       =    spaces
                     move  "N"            to   w-rig-snx-esi
           else      move  "S"            to   w-rig-snx-esi          .
      *
           move      rf-ddx-cod-sdi       to   w-rig-cod-sdi          .
           move      rf-ddx-eml-ind       to   w-rig-ind-eml          .
           move      rf-ddx-tip-con       to   w-rig-ind-eml-tip      .
      *                  *---------------------------------------------*
      *                  * Record [fit]                                *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                      * Flag bollo virtuale                     *
      *                      *-----------------------------------------*
           move      rf-fit-flg-nbx (3)   to   w-rig-snx-blv          .
           
______*    move      rf-fit-flg-ibv       to   w-rig-snx-blv          .
      ...
      *    *                                                           *
      *    * Subroutine di switch in funzione dei tasti F1, F2, F3, F4 *
      *    *-----------------------------------------------------------*
       rou-swc-srt-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       rou-swc-srt-100.
      *              *-------------------------------------------------*
      *              * Tasto funzione [1] - per Codice cliente         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       rou-swc-srt-200.
      *              *-------------------------------------------------*
      *              * Tasto funzione [2] - per Ragione sociale        *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su switch attuale                      *
      *                  *---------------------------------------------*
           if        w-cnt-acc-flg-sws    =   "R"
                     go to rou-swc-srt-600.
      *                  *---------------------------------------------*
      *                  * Indicatore tipo ordinamento                 *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       rou-swc-srt-300.
      *              *-------------------------------------------------*
      *              * Tasto funzione [3] - per Data documento         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su switch attuale                      *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       rou-swc-srt-400.
      *              *-------------------------------------------------*
      *              * Tasto funzione [4] - per Numero documento       *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su switch attuale                      *
      *                  *---------------------------------------------*
           if        w-cnt-acc-flg-sws    =   "N"
                     go to rou-swc-srt-600.
      *                  *---------------------------------------------*
      *                  * Indicatore tipo ordinamento                 *
      *                  *---------------------------------------------*
           move      "N"                  to  w-cnt-acc-flg-sws       .
      *                  *---------------------------------------------*
      *                  * Indicatore ordinamento                      *
      *                  *---------------------------------------------*
           move      "A"                  to  w-cnt-acc-flg-swo       .
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/fat/prg/obj/pfat4202"                       .
       cnc-rlt-sup-999.
           exit.
