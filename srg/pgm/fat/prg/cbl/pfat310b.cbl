      ...
      *                                                                *
      *                    Stampa documenti ordinata per agente        *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
      *        * Si/no fatture senza codice agente                     *
      *        *  - 1 : No                                             *
      *        *  - 2 : Si                                             *
      *        *-------------------------------------------------------*
           05  rr-snx-fsa                 pic  9(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/no fatture accompagnatorie                         *
      *        *  - 1 : No                                             *
      *        *  - 2 : Si                                             *
      *        *-------------------------------------------------------*
           05  rr-snx-fac                 pic  9(01)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Si/no fatture senza codice agente          *
      *        *-------------------------------------------------------*
           05  w-exp-snx-fsa.
               10  w-exp-snx-fsa-num      pic  9(02)       value 2    .
               10  w-exp-snx-fsa-lun      pic  9(02)       value 2    .
               10  w-exp-snx-fsa-tbl.
                   15  filler             pic  x(02) value "No"       .
                   15  filler             pic  x(02) value "Si"       .
      *        *-------------------------------------------------------*
      *        * Work per : Si/no fatture accompagnatorie              *
      *        *-------------------------------------------------------*
           05  w-exp-snx-fac.
               10  w-exp-snx-fac-num      pic  9(02)       value 2    .
               10  w-exp-snx-fac-lun      pic  9(02)       value 2    .
               10  w-exp-snx-fac-tbl.
                   15  filler             pic  x(02) value "No"       .
                   15  filler             pic  x(02) value "Si"       .
      ...
      *              * incompatibilita' per quanto riguarda la vali-   *
      ...
      *                  * Si/no fatture senza codice agente           *
      *                  *---------------------------------------------*
           perform   acc-snx-fsa-000      thru acc-snx-fsa-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Si/no fatture accompagnatorie               *
      *                  *---------------------------------------------*
           perform   acc-snx-fac-000      thru acc-snx-fac-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
      *              * Prompt si/no fatture senza codice agente        *
      *              *-------------------------------------------------*
           perform   pmt-snx-fsa-000      thru pmt-snx-fsa-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione si/no fatture senza codice      *
      *              * agente                                          *
      *              *-------------------------------------------------*
           perform   vis-snx-fsa-000      thru vis-snx-fsa-999        .
      *              *-------------------------------------------------*
      *              * Prompt si/no fatture accompagnatorie            *
      *              *-------------------------------------------------*
           perform   pmt-snx-fac-000      thru pmt-snx-fac-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione si/no fatture accompagnatorie   *
      *              *-------------------------------------------------*
           perform   vis-snx-fac-000      thru vis-snx-fac-999        .
       pmt-ric-sel-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       pmt-snx-fsa-000.
           ...
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Anche fatture senza agente :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-snx-fsa-999.
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Eventuale normalizzazione si/no stampa fat- *
      *                  * ture accompagnatorie                        *
      *                  *---------------------------------------------*
           if        rr-tip-stp           =    1 or
                     rr-tip-stp           =    4
           ...
           if        rr-snx-fac           =    1
                     go to acc-tip-stp-800.
      *                  *---------------------------------------------*
      *                  * Normalizzazione si/no stampa fatture accom- *
      *                  * pagnatorie                                  *
      *                  *---------------------------------------------*
           move      1                    to   rr-snx-fac             .
      *                  *---------------------------------------------*
      *                  * Visualizzazione si/no stampa fatture accom- *
      *                  * pagnatorie                                  *
      *                  *---------------------------------------------*
           perform   vis-snx-fac-000      thru vis-snx-fac-999        .
       acc-tip-stp-800.
      ...
      *                  *---------------------------------------------*
       ...
       acc-tip-mov-420.
      *                  *---------------------------------------------*
      *                  * Test su stampabilita' documento             *
      *                  *---------------------------------------------*
           if        w-let-arc-zfi-dst    not  = "*"
                     go to acc-tip-mov-425.
           move      "Tipo movimento non stampabile !                   
      -              "      "             to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           go to     acc-tip-mov-100.
       acc-tip-mov-425.
           if        w-let-arc-zfi-tdo    not  = 04
      ...
      *                  *---------------------------------------------*
       ...
       acc-tip-mov-700.
      *                  *---------------------------------------------*
      *                  * Eventuale preparazione di si/no stampa fat- *
      *                  * ture accompagnatorie                        *
      *                  *---------------------------------------------*
           if        rr-tip-stp           =    01 or
                     rr-tip-stp           =    02
                     go to acc-tip-mov-800.
           if        w-let-arc-zfi-ord    not  = 02
                     go to acc-tip-mov-800.
           if        rr-snx-fac           =    2
                     go to acc-tip-mov-800.
      *                  *---------------------------------------------*
      *                  * Preparazione di si/no stampa fatture accom- *
      *                  * pagnatorie                                  *
      *                  *---------------------------------------------*
           move      2                    to   rr-snx-fac             .
      *                  *---------------------------------------------*
      *                  * Visualizzazione si/no stampa fatture accom- *
      *                  * pagnatorie                                  *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      17                   to   w-cod-mne-age-lin      .
           move      30                   to   w-cod-mne-age-pos      .
           move      17                   to   w-cod-mne-age-nln      .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no fatture senza codice agente    *
      *    *-----------------------------------------------------------*
       acc-snx-fsa-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-fsa-lun    to   v-car                  .
           move      w-exp-snx-fsa-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-fsa-tbl    to   v-txt                  .
           move      rr-snx-fsa           to   v-num                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-fsa-999.
       acc-snx-fsa-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-fsa             .
       acc-snx-fsa-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-fsa           =    zero
                     go to acc-snx-fsa-100.
       acc-snx-fsa-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Si/no fatture senza codice agente       *
      *    *-----------------------------------------------------------*
       vis-snx-fsa-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no fatture accompagnatorie        *
      *    *-----------------------------------------------------------*
       acc-snx-fac-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Solo se tipo stampa : ristampa          *
      *                      *-----------------------------------------*
           if        rr-tip-stp           not  = 04
                     go to acc-snx-fac-999.
       acc-snx-fac-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-fac-lun    to   v-car                  .
           move      w-exp-snx-fac-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      20                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-fac-tbl    to   v-txt                  .
           move      rr-snx-fac           to   v-num                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
      ...
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-fac             .
       acc-snx-fac-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-fac           =    zero
                     go to acc-snx-fac-100.
       acc-snx-fac-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Regolarizzazione si/no fatture senza codice     *
      *              * agente                                          *
      *              *-------------------------------------------------*
           if        rr-snx-fsa           =    zero
                     move  1              to   rr-snx-fsa             .
      *              *-------------------------------------------------*
      *              * Regolarizzazione si/no fatture accompagnatorie  *
      *              *-------------------------------------------------*
           if        rr-snx-fac           =    zero
                     move  1              to   rr-snx-fac             .
       reg-ric-sel-999.
      ...
      *                  * Si/no fatture senza codice agente           *
      *                  *---------------------------------------------*
           move      zero                 to   rr-snx-fsa             .
      *                  *---------------------------------------------*
      *                  * Si/no fatture accompagnatorie               *
      *                  *---------------------------------------------*
           move      zero                 to   rr-snx-fac             .
       nor-ric-sel-400.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
       ...
       nor-ric-sel-600.
      *              *-------------------------------------------------*
      *              * Se non e' il primo giro                         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Data scadenza massima da stampare           *
      *                  *---------------------------------------------*
           if        rr-dat-max           =    9999999
                     move  zero           to   rr-dat-max             .
      *                  *---------------------------------------------*
      *                  * Numero fattura per accettazione max         *
      *                  *---------------------------------------------*
           if        rr-nfa-max           =    999999999
                     move  zero           to   rr-nfa-max             .
      *                  *---------------------------------------------*
      *                  * Numero fattura interno max                  *
      *                  *---------------------------------------------*
           if        rr-nfi-max           =    99999999999
                     move  zero           to   rr-nfi-max             .
       nor-ric-sel-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test su si/no fatture senza codice agente   *
      *                  *---------------------------------------------*
           if        rr-snx-fsa           not  = 1
       ...
       stp-srt-inp-410.
      *                  *---------------------------------------------*
      *                  * Test su si/no fatture accompagnatorie       *
      *                  *---------------------------------------------*
           if        rr-snx-fac           not  = 1
                     go to stp-srt-inp-415.
      *                      *-----------------------------------------*
      *                      * Se No fatture accompagnatorie           *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Test su origine del documento       *
      *                          *-------------------------------------*
           if        rf-fit-org-doc       =    02 or
                     rf-fit-org-doc       =    03
                     go to stp-srt-inp-200
           else      go to stp-srt-inp-420.
       stp-srt-inp-415.
      *                      *-----------------------------------------*
      *                      * Se Si fatture accompagnatorie           *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Test su origine del documento       *
      *                          *-------------------------------------*
           if        rf-fit-org-doc       =    03
                     go to stp-srt-inp-200
           else      go to stp-srt-inp-420.
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-fit-cod-age       not  = rr-cod-age
                     go to stp-srt-inp-200.
       stp-srt-inp-440.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Nel caso di fattura accompagnatoria si  *
      *                      * salta il test in quanto il contatore di *
      *                      * stampe eseguite non e' aggiornato. Il   *
      *                      * vero contatore di stampe eseguite per   *
      *                      * le fatture accompagnatorie si trova nel *
      *                      * record 'bit'.                           *
      *                      *-----------------------------------------*
           if        rf-fit-org-doc       =    02
                     go to stp-srt-inp-550.
           if        rf-fit-ctr-stp       not  > zero
                     go to stp-srt-inp-200.
           go to     stp-srt-inp-550.
      ...
      *                      * Codice agente                           *
      *                      *-----------------------------------------*
           move      rf-fit-cod-age       to   srt-cod-age            .
      *                      *-----------------------------------------*
      *                      * Data documento                          *
      *                      *-----------------------------------------*
