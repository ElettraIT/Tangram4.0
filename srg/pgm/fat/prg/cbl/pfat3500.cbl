      ...
      *                                   Fase:    fat350              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 05/12/08    *
      *                       Ultima revisione:    NdK del 14/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione segnale di fattura da verificare   *
      *                                                                *
      *                    Solo su personalizzazione (snx-fav)         *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    GESTIONE VERIFICA DELLE FATTURE     "       .
      ...
      *            *---------------------------------------------------*
      *            * Si/no solo verificate                             *
      *            *---------------------------------------------------*
               10  w-tes-snx-fav          pic  9(02)                  .
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no solo da verificare                   *
      *        *-------------------------------------------------------*
           05  w-exp-snx-sdv.
               10  w-exp-snx-sdv-num      pic  9(02)       value 02   .
               10  w-exp-snx-sdv-lun      pic  9(02)       value 40   .
               10  w-exp-snx-sdv-tbl.
                   15  filler             pic  x(40) value
                            "Tutti i documenti                       ".
                   15  filler             pic  x(40) value
                            "solo quelli da verificare               ".
      *        *-------------------------------------------------------*
      *        * Work per : No/Si fattura da verificare                *
      *        *-------------------------------------------------------*
           05  w-exp-snx-fav.
               10  w-exp-snx-fav-num      pic  9(02)       value 2    .
               10  w-exp-snx-fav-lun      pic  9(02)       value 02   .
      ...
      *                  * Si/no solo esclusi                          *
      *                  *---------------------------------------------*
           perform   acc-snx-sdv-000      thru acc-snx-sdv-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * Si/no solo esclusi                              *
      *              *-------------------------------------------------*
           perform   vis-snx-sdv-000      thru vis-snx-sdv-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Si/no solo esclusi                          *
      *                  *---------------------------------------------*
           perform   pmt-snx-sdv-000      thru pmt-snx-sdv-999        .
       pmt-key-reg-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-sdv-lun    to   v-car                  .
           move      w-exp-snx-sdv-num    to   v-ldt                  .
           move      "TE#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-sdv-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-snx-fav        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-sdv-999.
       acc-snx-sdv-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-fav          .
       acc-snx-sdv-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-snx-fav        not  = 01 and
                     w-tes-snx-fav        not  = 02
                     go to acc-snx-sdv-100.
       acc-snx-sdv-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  * Si/no da verificare                         *
      *                  *---------------------------------------------*
           if        w-rig-snx-fav        =    spaces
                     move  spaces         to   w-lin-imm-snx-fav
           else      move  "*"            to   w-lin-imm-snx-fav      .
       vis-lin-cor-500.
      *                  *---------------------------------------------*
      ...
      *              * Si/no da verificare                             *
      *              *-------------------------------------------------*
           perform   vis-snx-fav-000      thru vis-snx-fav-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *              * Si/no da verificare                             *
      *              *-------------------------------------------------*
           perform   pmt-snx-fav-000      thru pmt-snx-fav-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Si/no da verificare              *
      *    *-----------------------------------------------------------*
       pmt-snx-fav-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no da verificare                  *
      *    *-----------------------------------------------------------*
       acc-snx-fav-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-num                =    1
                     move  "#"           to   w-rig-snx-fav
           else if   v-num                =    2
                     move  spaces         to   w-rig-snx-fav
           else      move  spaces         to   w-rig-snx-fav          .
       acc-snx-fav-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Si/no da verificare               *
      *    *-----------------------------------------------------------*
       vis-snx-fav-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su si/no da verificare             *
      *                      *-----------------------------------------*
           if        w-tes-snx-fav        not  = 02
                     go to stp-srt-inp-440.
           if        rf-fit-flg-nbx (2)   =    spaces
                     go to stp-srt-inp-200.
       stp-srt-inp-440.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-fit-cod-age       not  = w-tes-cod-age
                     go to stp-srt-inp-200.
       stp-srt-inp-600.
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-fit-flg-nbx (2)   to   w-rig-snx-fav          .
      *                      *-----------------------------------------*
      *                      * Anagrafica cliente                      *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-rig-snx-fav        =    rf-fit-flg-nbx (2)
      ...
      *                  *---------------------------------------------*
           move      w-rig-snx-fav        to   rf-fit-flg-nbx (2)     .
       sca-fil-rap-700.
      *                  *---------------------------------------------*
      *                  * Update record [fit]                         *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/fat/prg/obj/pfat3502"                       .
       cnc-rlt-sup-999.
           exit.
