      ...
      *                       Ultima revisione:    NdK del 13/05/25    *
      *                    ------------------------------------------- *
      ...
      *                                                                *
      *                    Sottoprogramma per funzione di :            *
      *                                                                *
      *                    Stampa documenti per archiviazione PDF      *
      *                                                                *
      *                    N.B.: se si utilizza una stampante PDF      *
      *                          verificare che NON abbia la password  *
      *                          di protezione.                        *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
      *            * Numero documento per l'emissione                  *
      *            *---------------------------------------------------*
               10  srt-doc-nda            pic  x(40)                  .
      ...
      *        *                                                       *
      *        * - 1 : Esclusione di quelli che hanno la e-mail        *
      *        * - 2 : Tutti                                           *
      *        * - 2 : Solo destinatari con e-mail                     *
      *        *-------------------------------------------------------*
           05  rr-snx-edp                 pic  9(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/no anno documento come prefisso                    *
      *        *                                                       *
      *        * - 1 : Si                                              *
      *        * - 2 : No                                              *
      *        *-------------------------------------------------------*
           05  rr-snx-apn                 pic  9(01)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Si/no anno come prefisso                   *
      *        *-------------------------------------------------------*
           05  w-exp-snx-apn.
               10  w-exp-snx-apn-num      pic  9(02)       value 2    .
               10  w-exp-snx-apn-lun      pic  9(02)       value 02   .
               10  w-exp-snx-apn-tbl.
      ...
      *    *-----------------------------------------------------------*
           copy      "pgm/fat/prg/cpy/wieofat0.wkl"                   .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-500.
      *                  *---------------------------------------------*
      *                  * Si/no anno come prefisso                    *
      *                  *---------------------------------------------*
           perform   acc-snx-apn-000      thru acc-snx-apn-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      17                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Si/no anno documento come  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no anno come prefisso             *
      *    *-----------------------------------------------------------*
       acc-snx-apn-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-apn-lun    to   v-car                  .
           move      w-exp-snx-apn-num    to   v-ldt                  .
           move      "SN#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      17                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-apn-tbl    to   v-txt                  .
           move      rr-snx-apn           to   v-num                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-apn-999.
       acc-snx-apn-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-apn             .
       acc-snx-apn-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-apn           =    zero
                     go to acc-snx-apn-100.
       acc-snx-apn-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-580.
      *              *-------------------------------------------------*
      *              * Preparazione numero documento per emissione     *
      *              *-------------------------------------------------*
           perform   stp-srt-inp-num-000  thru stp-srt-inp-num-999    .
       stp-srt-inp-600.
      *              *-------------------------------------------------*
      ...
      *                      * Numero documento per l'emissione        *
      *                      *-----------------------------------------*
           move      w-wrk-doc-nda        to   srt-doc-nda            .
       stp-srt-inp-800.
      *              *-------------------------------------------------*
      ...
      *    * Input procedure per sort                                  *
      *    *                                                           *
      *    * Subroutine di preparazione numero documento               *
      *    *-----------------------------------------------------------*
       stp-srt-inp-num-000.
      *              *-------------------------------------------------*
      ...
      *              *                                                 *
      *              *  - 'AA'     = Anno documento                    *
      *              *  - 'BBB'    = Sigla numerazione (eventuale)     *
      *              *  - 'CC'     = Giornale IVA      (eventuale)     *
      *              *  - '999999' = Numero documento con zeri         *
      *              *                                                 *
      *              * AABBBCC_999999                                  *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-wrk-nfi-npg        to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-wrk-doc-nde          .
      *                  *---------------------------------------------*
      *                  * Assemblaggio                                *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-num-200.
      *              *-------------------------------------------------*
      *              * Assemblaggio finale con anno                    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se anno come prefisso                  *
      *                  *---------------------------------------------*
           if        rr-snx-apn           =    2
                     go to stp-srt-inp-num-900.
      *                  *---------------------------------------------*
      *                  * Editing anno del documento                  *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-wrk-doc-ann          .
      *                  *---------------------------------------------*
      *                  * Assemblaggio                                *
      *                  *---------------------------------------------*
           move      40                   to   w-all-str-lun          .
           move      03                   to   w-all-str-num          .
           move      w-wrk-doc-ann        to   w-all-str-cat (1)      .
           move      "/"                  to   w-all-str-cat (2)      .
           move      w-wrk-doc-nda        to   w-all-str-cat (3)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
      *                  *---------------------------------------------*
      *                  * In campo di destinazione                    *
      *                  *---------------------------------------------*
      ...
      *                  * Identificatore fase                         *
      *                  *---------------------------------------------*
           move      "docpdf"             to   w-inp-mst-fas          .
      *                  *---------------------------------------------*
      *                  * Identificatore documento                    *
      *                  *---------------------------------------------*
           move      srt-doc-nda          to   w-inp-mst-dep          .
      *              *-------------------------------------------------*
