      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pdtp3811:        *
      *                                                                *
      *                    Stampa prodotti finiti con o senza distinta *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA PRODOTTI FINITI SENZA DISTINTA B."       .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su distinta                      *
      *        *-------------------------------------------------------*
           05  w-exp-sel-dst.
               10  w-exp-sel-dst-num      pic  9(02)       value 2    .
               10  w-exp-sel-dst-lun      pic  9(02)       value 40   .
               10  w-exp-sel-dst-tbl.
                   15  filler             pic  x(40) value
                            "solo prodotti Con distinta base         ".
                   15  filler             pic  x(40) value
                            "solo prodotti Senza distinta base       ".
      *        *-------------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-200.
      *                  *---------------------------------------------*
      *                  * Selezione su distinta                       *
      *                  *---------------------------------------------*
           perform   acc-sel-dst-000      thru acc-sel-dst-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su distinta base :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Selezione su distinta                *
      *    *-----------------------------------------------------------*
       acc-sel-dst-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-dst-lun    to   v-car                  .
           move      w-exp-sel-dst-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      "CS#"                to   v-msk                  .
           move      08                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sel-dst-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
           move      rr-sel-dst           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sel-dst-999.
       acc-sel-dst-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sel-dst             .
       acc-sel-dst-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sel-dst           =    zero
                     go to acc-sel-dst-100.
       acc-sel-dst-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Selezione su distinta             *
      *    *-----------------------------------------------------------*
       vis-sel-dst-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
       ...
       reg-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione selezione su distinta           *
      *              *-------------------------------------------------*
           if        rr-sel-dst           not  = zero
                     go to reg-ric-sel-999.
           move      01                   to   rr-sel-dst             .
       reg-ric-sel-999.
           exit.
