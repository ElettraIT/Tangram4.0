      ...
      *                       Ultima revisione:    NdK del 28/05/19    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella Codici lavorazione         *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   GESTIONE TABELLA CODICI LAVORAZIONE  "       .
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-key.
               10  w-tes-cod-lav          pic  x(03)                  .
      *        *-------------------------------------------------------*
      *        * Valori attuali e precedenti                           *
      *        *-------------------------------------------------------*
           05  w-tes-val-aep occurs 2.
               10  w-tes-des-lav          pic  x(40)                  .
      ...
      *                  * Codice lavorazione                          *
      *                  *---------------------------------------------*
           perform   acc-cod-lav-000      thru acc-cod-lav-999        .
      ...
      *              * Codice lavorazione                              *
      *              *-------------------------------------------------*
           perform   vis-cod-lav-000      thru vis-cod-lav-999        .
       vis-key-reg-999.
           exit.
      ...
      *              * Codice lavorazione                              *
      *              *-------------------------------------------------*
           perform   pmt-cod-lav-000      thru pmt-cod-lav-999        .
      *              *-------------------------------------------------*
      *              * Linea di trattini                               *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice lavorazione            *
      *    *-----------------------------------------------------------*
       pmt-cod-lav-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo chiave : Codice lavorazione            *
      *    *-----------------------------------------------------------*
       acc-cod-lav-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-lav        to   w-cod-cod-zcl-cod      .
           move      04                   to   w-cod-cod-zcl-lin      .
           move      30                   to   w-cod-cod-zcl-pos      .
       ...
       acc-cod-lav-110.
           ...
           if        w-cod-cod-zcl-ope    =    "F+"
                     go to acc-cod-lav-115.
           if        w-cod-cod-zcl-ope    =    "AC"
                     go to acc-cod-lav-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-cod-lav-115.
           perform   cod-cod-zcl-foi-000  thru cod-cod-zcl-foi-999    .
           go to     acc-cod-lav-110.
       acc-cod-lav-120.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-cod-lav-999.
       acc-cod-lav-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   w-tes-cod-lav          .
       acc-cod-lav-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        w-tes-cod-lav        =    "-"
                     go to acc-cod-lav-100.
       acc-cod-lav-600.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice lavorazione         *
      *    *-----------------------------------------------------------*
       vis-cod-lav-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  * Descrizione                                 *
      *                  *---------------------------------------------*
           perform   acc-des-lav-000      thru acc-des-lav-999        .
           if        v-key                =    "NXSC"
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   vis-des-lav-000      thru vis-des-lav-999        .
           go to     vis-tes-reg-999.
       vis-tes-reg-200.
       vis-tes-reg-999.
           exit.
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   pmt-des-lav-000      thru pmt-des-lav-999        .
           go to     pmt-tes-reg-999.
       pmt-tes-reg-200.
       pmt-tes-reg-999.
           exit.
      ...
      *                  *---------------------------------------------*
           if        w-tes-des-lav (1)    not  = spaces
                     go to acc-des-lav-450.
           if        v-key                =    "UP  "
                     go to acc-des-lav-600
           else      go to acc-des-lav-100.
       acc-des-lav-450.
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           move      spaces               to   w-tes-des-lav (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-lav        to   rf-zcl-cod-lav         .
           move      "pgm/dtp/fls/ioc/obj/iofzcl"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
           move      rf-zcl-des-lav       to   w-tes-des-lav (1)      .
           move      rf-zcl-alx-exp       to   w-tes-alx-exp (1)      .
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zcl]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zcl-000      thru wrt-rec-zcl-999        .
      ...
      *                      * Rewrite record [zcl]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zcl-000      thru rew-rec-zcl-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zcl]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zcl-000      thru del-rec-zcl-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zcl]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zcl-000.
      *              *-------------------------------------------------*
      ...
      *                  * Campi non chiave                            *
      *                  *---------------------------------------------*
           move      w-tes-des-lav (1)    to   rf-zcl-des-lav         .
           move      w-tes-alx-exp (1)    to   rf-zcl-alx-exp         .
       cmp-rec-zcl-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zcl]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zcl-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zcl-000      thru cmp-rec-zcl-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
