      ...
      *                                   Fase:    dtp060              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 28/05/19    *
      *                       Ultima revisione:    NdK del 14/10/19    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella Codici macchinario         *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   GESTIONE TABELLA CODICI MACCHINARI   "       .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice macchinario            *
      *    *-----------------------------------------------------------*
       pmt-cod-mcn-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo chiave : Codice macchinario            *
      *    *-----------------------------------------------------------*
       acc-cod-mcn-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-mcn        to   w-cod-cod-zcm-cod      .
           move      04                   to   w-cod-cod-zcm-lin      .
           move      30                   to   w-cod-cod-zcm-pos      .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice macchinario         *
      *    *-----------------------------------------------------------*
       vis-cod-mcn-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  * Descrizione                                 *
      *                  *---------------------------------------------*
           perform   acc-des-mcn-000      thru acc-des-mcn-999        .
           if        v-key                =    "NXSC"
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   vis-des-mcn-000      thru vis-des-mcn-999        .
      *              *-------------------------------------------------*
      *              * Costi                                           *
      *              *                                                 *
      *              * PET                                             *
      *              *-------------------------------------------------*
           perform   vis-cst-mcn-000      thru vis-cst-mcn-999        .
           go to     vis-tes-reg-999.
       vis-tes-reg-200.
       vis-tes-reg-999.
           exit.
      ...
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           perform   pmt-des-mcn-000      thru pmt-des-mcn-999        .
           go to     pmt-tes-reg-999.
       pmt-tes-reg-200.
       pmt-tes-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Accettazione campo : Descrizione                          *
      *    *-----------------------------------------------------------*
       acc-des-mcn-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        w-tes-des-mcn (1)    not  = spaces
                     go to acc-des-mcn-450.
           if        v-key                =    "UP  "
                     go to acc-des-mcn-600
           else      go to acc-des-mcn-100.
       acc-des-mcn-450.
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Costi (PET)                       *
      *    *-----------------------------------------------------------*
       vis-cst-mcn-000.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                  *---------------------------------------------*
      *                  * Test su codice azienda                      *
      *                  *---------------------------------------------*
           if        s-azi                not  = "prv" and
                     s-azi                not  = "ptu"
                     go to vis-cst-mcn-999.
      *                  *---------------------------------------------*
      *                  * Test su codice utente                       *
      *                  *---------------------------------------------*
           if        s-ute                not  = "giulia  " and
                     s-ute                not  = "marco   " and
                     s-ute                not  = "alice   " and
                     s-ute                not  = "laura   " and
                     s-ute                not  = "master  "
      ...
      *
           if        w-tes-cod-mcn        =    "1  "
                     move  "60,00"        to   v-alf
           else if   w-tes-cod-mcn        =    "2  "
                     move  "25,00"        to   v-alf
           else if   w-tes-cod-mcn        =    "3  "
                     move  "40,00"        to   v-alf
      ...
      *              *-------------------------------------------------*
           ...
           move      "Manca la descrizione per l'unita' di misura       
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           move      spaces               to   w-tes-des-mcn (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-mcn        to   rf-zcm-cod-mcn         .
           move      "pgm/dtp/fls/ioc/obj/iofzcm"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
           move      rf-zcm-des-mcn       to   w-tes-des-mcn (1)      .
           move      rf-zcm-alx-exp       to   w-tes-alx-exp (1)      .
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zcm]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zcm-000      thru wrt-rec-zcm-999        .
      ...
      *                      * Rewrite record [zcm]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zcm-000      thru rew-rec-zcm-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zcm]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zcm-000      thru del-rec-zcm-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zcm]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zcm-000.
      *              *-------------------------------------------------*
      ...
      *                  * Campi non chiave                            *
      *                  *---------------------------------------------*
           move      w-tes-des-mcn (1)    to   rf-zcm-des-mcn         .
           move      w-tes-alx-exp (1)    to   rf-zcm-alx-exp         .
       cmp-rec-zcm-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zcm]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zcm-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zcm-000      thru cmp-rec-zcm-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
