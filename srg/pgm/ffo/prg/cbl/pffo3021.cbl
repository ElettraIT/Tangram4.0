      ...
      *                       Ultima revisione:    NdK del 11/09/03    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma ffo302   *
      *                                                                *
      ...
      *            * Numero protocollo                                 *
      *            *---------------------------------------------------*
               10  srt-num-prt            pic  9(06)                  .
      *            *---------------------------------------------------*
      *            * Data registrazione bolla                          *
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
      *            * Data documento bolla                              *
      *            *---------------------------------------------------*
               10  srt-ddo-bol            pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Numero documento                                  *
      *            *---------------------------------------------------*
               10  srt-ndo-bol            pic  x(10)                  .
      *            *---------------------------------------------------*
      *            * Tipo archivio                                     *
      *            *---------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   STAMPA ELENCO DOCUMENTI FORNITORI    "       .
      ...
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-010.
      *              *-------------------------------------------------*
      *              * Start su archivio [fft] per data registrazione  *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "IDEDOC    "         to   f-key                  .
           move      rr-dat-ini           to   rf-fft-dat-reg         .
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-020.
      *              *-------------------------------------------------*
      *              * Start su archivio [fft] per data ultima modifi- *
      *              * ca                                              *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "DATSYS    "         to   f-key                  .
           move      rr-dat-ini           to   rf-fft-ide-dat         .
      ...
      *                      *-----------------------------------------*
           ...
           if        rf-fft-ide-ute       not  = rr-cod-ute
                     go to stp-srt-inp-100.
       stp-srt-inp-330.
      *                      *-----------------------------------------*
      *                      * Test su codice fase                     *
      *                      *-----------------------------------------*
           ...
           if        rf-fft-ide-fas       not  = rr-cod-fas
                     go to stp-srt-inp-100.
       stp-srt-inp-340.
      *                      *-----------------------------------------*
      ...
      *                          * Numero protocollo                   *
      *                          *-------------------------------------*
           move      rf-fft-num-prt       to   w-det-num-doc          .
           move      w-det-num-doc-prg    to   srt-num-prt            .
      *                          *-------------------------------------*
      *                          * Data bolla                          *
      *                          *-------------------------------------*
           if        rr-tip-stp           not  = 03
                     move  zero           to   srt-dat-bol
           else      move  rf-ffr-bfo-dat to   srt-dat-bol            .
      *                          *-------------------------------------*
      *                          * Numero documento                    *
      *                          *-------------------------------------*
           if        rr-tip-stp           not  = 03
                     move  zero           to   srt-num-bol
           else      move  rf-ffr-bfo-prt to   srt-num-bol            .
      *                      *-----------------------------------------*
      *                      * Dati di sort                            *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rr-tip-stp           not  = 03
                     move  zero           to   srt-ddo-bol
           else      move  rf-ffr-bfo-ddo to   srt-ddo-bol            .
      *                          *-------------------------------------*
      *                          * Numero documento                    *
      *                          *-------------------------------------*
           if        rr-tip-stp           not  = 03
                     move  spaces         to   srt-ndo-bol
           else      move  rf-ffr-bfo-ndo to   srt-ndo-bol            .
      *                          *-------------------------------------*
      *                          * Codice archivio                     *
      *                          *-------------------------------------*
           move      rf-fft-cod-arc       to   srt-cod-arc            .
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           move      rf-fft-sgl-vpf       to   srt-sgl-vpf            .
      *                          *-------------------------------------*
      *                          * Decimali valuta documento           *
      *                          *-------------------------------------*
           move      rf-fft-dec-vpf       to   srt-dec-vpf            .
      *                          *-------------------------------------*
      ...
      *                          * Data ultima modifica                *
      *                          *-------------------------------------*
           move      rf-fft-ide-dat       to   srt-ide-dat            .
      *                          *-------------------------------------*
      *                          * Utente                              *
      *                          *-------------------------------------*
           move      rf-fft-ide-ute       to   srt-ide-ute            .
      *                          *-------------------------------------*
      *                          * Fase                                *
      *                          *-------------------------------------*
           move      rf-fft-ide-fas       to   srt-ide-fas            .
      *                          *-------------------------------------*
      *                          * Flags non bloccanti                 *
      *                          *-------------------------------------*
           move      rf-fft-flg-nbl       to   srt-flg-nbl            .
      *                          *-------------------------------------*
      *                          * Tipo bolla                          *
      *                          *-------------------------------------*
           if        rr-tip-stp           not  = 03
                     move  spaces         to   srt-tip-bol
           else      move  rf-ffr-bfo-tip to   srt-tip-bol            .
       stp-srt-inp-800.
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      srt-dat-reg          to   w-rot-l02-dat-doc      .
           move      srt-num-prt          to   w-rot-l02-num-doc      .
           move      srt-dat-bol          to   w-rot-l01-dat-bol      .
           move      srt-num-bol          to   w-rot-l01-num-bol      .
       prn-cmp-rot-999.
           exit.
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      019                  to   p-pos                  .
           move      srt-num-bol          to   w-det-num-doc          .
      ...
      *              *-------------------------------------------------*
           move      "ELENCO DOCUMENTI DI ACQUISTO"
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "| Tipo  Data reg Numero |          Codice e Ragion
      ...
      *              *-------------------------------------------------*
           if        rf-fft-sgl-vpf       not  = c-sgl
                     move  rf-fft-iiv-doc to   w-det-tot-doc-tot
           else      move  rf-fft-imp-doc to   w-det-tot-doc-tot      .
