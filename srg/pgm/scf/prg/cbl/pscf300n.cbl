      ...
      *        *-------------------------------------------------------*
      *        * Dicitura per differenze                               *
      *        *-------------------------------------------------------*
           05  rr-lit-dif                 pic  x(20)                  .
      *        *-------------------------------------------------------*
      *        * Tipo addebito :                                       *
      *        * - 01 : in C/C                                         *
      *        * - 02 : c/anticipo                                     *
      *        *-------------------------------------------------------*
           05  rr-tip-add                 pic  9(02)                  .
           ...
           05  w-exp-tip-stp.
               ...
               10  w-exp-tip-stp-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "Ristampa            "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Tipo addebito                              *
      *        *-------------------------------------------------------*
           05  w-exp-tip-add.
               10  w-exp-tip-add-num      pic  9(02)       value 2    .
               10  w-exp-tip-add-lun      pic  9(02)       value 10   .
               10  w-exp-tip-add-tbl.
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Non viene stampato il nome azienda          *
      *                  *---------------------------------------------*
      ...
      *                  * Dicitura per differenze                     *
      *                  *---------------------------------------------*
           perform   acc-lit-dif-000      thru acc-lit-dif-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Tipo addebito                               *
      *                  *---------------------------------------------*
           perform   acc-tip-add-000      thru acc-tip-add-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-lit-dif             .
           move      01                   to   rr-tip-add             .
       nor-ric-sel-999.
           exit.
      ...
      *    *===========================================================*
      *    * Accettazione : Dicitura per differenze                    *
      *    *-----------------------------------------------------------*
       acc-lit-dif-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-lit-dif-999.
       acc-lit-dif-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-lit-dif             .
       acc-lit-dif-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-lit-dif-100.
       acc-lit-dif-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Tipo addebito              *
      *    *-----------------------------------------------------------*
       acc-tip-add-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-tip-add-lun    to   v-car                  .
           move      w-exp-tip-add-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      "CA#"                to   v-msk                  .
           move      21                   to   v-lin                  .
           move      69                   to   v-pos                  .
           move      w-exp-tip-add-tbl    to   v-txt                  .
           move      rr-tip-add           to   v-num                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-tip-add-999.
       acc-tip-add-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-tip-add             .
       acc-tip-add-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-tip-add           =    zero
                     go to acc-tip-add-100.
       acc-tip-add-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Lettura personalizzazioni portafoglio   *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "Emittente :"        to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Denominazione emittente - parte 1       *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      24                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Denominazione emittente - parte 2       *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      24                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Denominazione emittente - parte 3       *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      24                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Denominazione emittente - parte 4       *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      24                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
      ...
      -              "ire la seguente disposizione di bonifico ban-  "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      ...
      *
           if        rr-tip-add           =    01
                     move  "cario su estero, che vorrete addebitare nel 
      -              "nostro c/c nr."     to   w-all-str-cat (1)
                     move  rr-cod-cbp-ccc to   w-all-str-cat (2)
                     move  "presso di Voi."
                                          to   w-all-str-cat (3)
           else      move  "cario su estero, in conto anticipo."
                                          to   w-all-str-cat (1)
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Interlinea per eventuale swift          *
      *                      *-----------------------------------------*
           move      "LF"                 to   p-ope                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      w-let-arc-bef-ibn    to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                          *-------------------------------------*
      *                          * Stampa                              *
      *                          *-------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      064                  to   p-pos                  .
      *
           if        rf-sfs-iiv-scf       =    zero
                     move  rf-sfs-imp-scf to   rf-sfs-iiv-scf         .
      *
      ...
      *              *-------------------------------------------------*
           ...
           move      rf-bef-cod-ibn       to   w-let-arc-bef-ibn      .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       let-arc-bef-600.
           ...
           move      spaces               to   w-let-arc-bef-ibn      .
       let-arc-bef-999.
           exit.
