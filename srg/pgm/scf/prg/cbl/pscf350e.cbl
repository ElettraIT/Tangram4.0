      ...
      *                                   Fase:    scf350              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 03/04/18    *
      ...
      *                                                                *
      *                    Gestione protocollo registrazione documenti *
      *                    'xml' ricevuti                              *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     "GESTIONE PROTOCOLLO REGISTRAZ. DOCUMENTI"       .
      ...
      *        * Protocollo                                            *
      *        *-------------------------------------------------------*
           05  w-rig-num-ftf              pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Totale documento                                      *
      *        *-------------------------------------------------------*
           05  w-rig-tot-doc              pic s9(11)                  .
      *        *-------------------------------------------------------*
      *        * Un intero record [xft]                                *
      *        *-------------------------------------------------------*
           05  w-rig-rec-xft.
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no solo senza codice                    *
      *        *-------------------------------------------------------*
           05  w-exp-abb-snx.
               10  w-exp-abb-snx-num      pic  9(02)       value 03   .
               10  w-exp-abb-snx-lun      pic  9(02)       value 40   .
               10  w-exp-abb-snx-tbl.
                   15  filler             pic  x(40) value
                            "solo quelli Senza protocollo            ".
                   15  filler             pic  x(40) value
                            "solo quelli Con protocollo              ".
                   15  filler             pic  x(40) value
      ...
      *                  * Si/no da abbinare                           *
      *                  *---------------------------------------------*
           perform   acc-abb-snx-000      thru acc-abb-snx-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * Si/no da abbinare                               *
      *              *-------------------------------------------------*
           perform   vis-abb-snx-000      thru vis-abb-snx-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Si/no da abbinare                           *
      *                  *---------------------------------------------*
           perform   pmt-abb-snx-000      thru pmt-abb-snx-999        .
       pmt-key-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Accettazione : Si/no da abbinare                          *
      *    *-----------------------------------------------------------*
       acc-abb-snx-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-abb-snx-lun    to   v-car                  .
           move      w-exp-abb-snx-num    to   v-ldt                  .
           move      "SCT#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-abb-snx-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-abb-snx        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-abb-snx-999.
       acc-abb-snx-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-abb-snx          .
       acc-abb-snx-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-abb-snx        not  = 01 and
                     w-tes-abb-snx        not  = 02 and
                     w-tes-abb-snx        not  = 03
                     go to acc-abb-snx-100.
       acc-abb-snx-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Si/no da abbinare                       *
      *    *-----------------------------------------------------------*
       vis-abb-snx-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Accettazione protocollo SCF                 *
      *                  *---------------------------------------------*
           perform   acc-num-ftf-000      thru acc-num-ftf-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
      ...
      *                  *---------------------------------------------*
           move      spaces               to   w-lin-imm-dpz-fnt      .
       vis-lin-cor-480.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      rf-xft-ndo-ftf       to   w-lin-imm-num-doc      .
       vis-lin-cor-540.
      *                  *---------------------------------------------*
      ...
      *              * Protocollo                                      *
      *              *-------------------------------------------------*
           perform   vis-num-ftf-000      thru vis-num-ftf-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * ID fiscale fornitore                            *
      *              *-------------------------------------------------*
           perform   vis-idf-for-000      thru vis-idf-for-999        .
       vis-rig-cor-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *              * Codice fornitore reale                          *
      *              *-------------------------------------------------*
           perform   pmt-num-ftf-000      thru pmt-num-ftf-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *              * ID fiscale fornitore                            *
      *              *-------------------------------------------------*
           perform   pmt-idf-for-000      thru pmt-idf-for-999        .
       pmt-rig-cor-999.
           exit.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Protocollo                       *
      *    *-----------------------------------------------------------*
       pmt-num-ftf-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : ID fiscale fornitore             *
      *    *-----------------------------------------------------------*
       pmt-idf-for-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      spaces               to   w-rou-exp-ana-dpz      .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-num-ftf-999.
       acc-num-ftf-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-rig-num-ftf          .
       acc-num-ftf-300.
      *              *-------------------------------------------------*
      *              * Se Find                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        v-key                not  = "FIND"
                     go to acc-num-ftf-400.
      *                  *---------------------------------------------*
      *                  * Find su archivio [sff]                      *
      *                  *---------------------------------------------*
           move      "S"                  to   w-fnd-arc-sff-sns      .
           move      w-rig-cod-fnt        to   w-fnd-arc-sff-fnt      .
      ...
      *                  * zione                                       *
      *                  *---------------------------------------------*
           if        w-fnd-arc-sff-sel    not  = spaces
                     go to acc-num-ftf-100.
      *                  *---------------------------------------------*
      *                  * Ripresa numero registrazione fattura sele-  *
      *                  * zionato                                     *
      *                  *---------------------------------------------*
           move      w-fnd-arc-sff-num    to   w-rig-num-ftf          .
      *                  *---------------------------------------------*
      *                  * Visualizzazione numero registrazione fattu- *
      *                  * ra selezionato                              *
      *                  *---------------------------------------------*
           perform   vis-num-ftf-000      thru vis-num-ftf-999        .
       acc-num-ftf-400.
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-xft-tdo-ftf       to   w-all-str-cat (1)      .
           move      rf-xft-ndo-ftf       to   w-all-str-cat (2)      .
      ...
      *                  *---------------------------------------------*
           ...
           move      03                   to   w-all-str-num          .
           move      w-all-str-alf        to   w-all-str-cat (1)      .
           move      "- Caricata:"        to   w-all-str-cat (2)      .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : ID fiscale fornitore              *
      *    *-----------------------------------------------------------*
       vis-idf-for-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       nor-nok-rig-000.
           ...
           move      spaces               to   w-rig-nom-xml          .
           move      zero                 to   w-rig-cod-fnt          .
           move      spaces               to   w-rig-cod-fnt-rag      .
           move      zero                 to   w-rig-num-ftf          .
           move      zero                 to   w-rig-tot-doc          .
           move      all spaces           to   w-rig-rec-xft          .
       nor-nok-rig-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      24                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "              --- Caricamento documenti ricevuti i
      -              "n esecuzione ---              "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       stp-srt-inp-100.
      *              *-------------------------------------------------*
      *              * Start su [xft] per data registrazione           *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CARNOM    "         to   f-key                  .
           move      w-tes-dat-min        to   rf-xft-dat-car         .
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-xft-dat-car       >    w-tes-dat-min
      ...
      *                  *---------------------------------------------*
           if        rf-xft-dat-car       >    w-tes-dat-max
                     go to stp-srt-inp-900.
           go to     stp-srt-inp-400.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Normalizzazione eventuale protocollo        *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su si/no protocollo                *
      *                      *-----------------------------------------*
           go to     stp-srt-inp-422
                     stp-srt-inp-424
                     stp-srt-inp-426
                     depending            on   w-tes-abb-snx          .
       stp-srt-inp-422.
      *                          *-------------------------------------*
      *                          * Solo quelli Senza protocollo        *
      *                          *-------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-xft-cod-fnt       not  = w-tes-cod-fnt
                     go to stp-srt-inp-200.
       stp-srt-inp-600.
      *              *-------------------------------------------------*
      ...
      *                      * Nome file                               *
      *                      *-----------------------------------------*
           move      rf-xft-nom-xml       to   w-rig-nom-xml          .
      *                      *-----------------------------------------*
      *                      * Codice fornitore                        *
      *                      *-----------------------------------------*
           move      rf-xft-cod-fnt       to   w-rig-cod-fnt          .
      *                      *-----------------------------------------*
      *                      * Ragione sociale fornitore               *
      *                      *-----------------------------------------*
           move      rf-xft-rag-ftf       to   w-rig-cod-fnt-rag      .
      *                      *-----------------------------------------*
      *                      * Protocollo                              *
      *                      *-----------------------------------------*
           if        rf-xft-num-ftf       not  numeric
                     move  zero           to   rf-xft-num-ftf         .
      *
           move      rf-xft-num-ftf       to   w-rig-num-ftf          .
      *                      *-----------------------------------------*
      *                      * Totale documento                        *
      *                      *-----------------------------------------*
           move      rf-xft-tot-ftf       to   w-rig-tot-doc          .
      *                      *-----------------------------------------*
      *                      * Un intero record [xft]                  *
      *                      *-----------------------------------------*
           move      rf-xft               to   w-rig-rec-xft          .
       car-fil-rap-800.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-rig-nom-xml        to   rf-xft-nom-xml         .
           move      "pgm/scf/fls/ioc/obj/iofxft"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           move      w-rig-num-ftf        to   rf-xft-num-ftf         .
       sca-fil-rap-700.
      *                  *---------------------------------------------*
      *                  * Update record [xft]                         *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/scf/prg/obj/pscf3502"                       .
       cnc-rlt-sup-999.
           exit.
