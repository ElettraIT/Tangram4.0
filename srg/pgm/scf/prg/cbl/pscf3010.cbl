      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazioni per movimenti scadenze       *
      *                    fornitori                                   *
      *                                                                *
      *                    Main                                        *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase scf301, nella sua globalita', esegue i tipi di inter-  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * INTFTF      Interrogazione su fatture fornitori registrate     *
      *                                                                *
      * INTSCF      Interrogazione su scadenze registrate              *
      *                                                                *
      * INTPGF      Interrogazione su pagamenti effettuati             *
      *                                                                *
      * INTCMD      Interrogazione su compensazioni scadenze           *
      *                                                                *
      * INTADP      Interrogazione su addebiti registrati              *
      *                                                                *
      * ESPSCF      Espansione movimenti effettuati su di una scadenza *
      *             (non selezionabile per impostazione)               *
      *                                                                *
      * ESPPGF      Espansione movimenti effettuati su di un pagamento *
      *             (non selezionabile per impostazione)               *
      *             *** ATTUALMENTE NON IMPLEMENTATO ***               *
      *                                                                *
      * ESPADP      Espansione movimenti effettuati su di un addebito  *
      *             (non selezionabile per impostazione)               *
      *             *** ATTUALMENTE NON IMPLEMENTATO ***               *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "     RICERCHE PER SCADENZE FORNITORI    "       .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'scf301'           *
      *              *-------------------------------------------------*
           perform   fas-scf-301-000      thru fas-scf-301-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *
      ...
      -              "*ESPSCF     pscf301w  "
           ...
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *       
           move      "Espansione movimenti effettuati su di un pagamento
      -              "*ESPPGF     pscf301x  "
                                          to   w-tin-ele-wci          .
______*    perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *       
           move      "Espansione movimenti effettuati su di un addebito 
      -              "*ESPADP     pscf301y  "
                                          to   w-tin-ele-wci          .
______*    perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *       
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-scf-301-300
           else if   v-key                =    "PRSC"
                     go to fas-scf-301-400
           else if   v-key                =    "NXSC"
                     go to fas-scf-301-450
           else if   v-key                =    "DOWN"
                     go to fas-scf-301-500
           else if   v-key                =    "EXIT"
                     go to fas-scf-301-250
           else      go to fas-scf-301-200.
       fas-scf-301-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-scf-301-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-scf-301-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-nes
                     go to fas-scf-301-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-scf-301-350
           else      go to fas-scf-301-322.
       fas-scf-301-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-scf-301-630
           else if   v-key                =    "PRSC"
                     go to fas-scf-301-600
           else if   v-key                =    "NXSC"
                     go to fas-scf-301-610
           else if   v-key                =    "UP  "
                     go to fas-scf-301-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-scf-301-650
           else if   v-key                =    "EXIT"
                     go to fas-scf-301-660
           else      go to fas-scf-301-570.
       fas-scf-301-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-scf-301-626
           else      go to fas-scf-301-628.
       fas-scf-301-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-scf-301-636
           else      go to fas-scf-301-638.
       fas-scf-301-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-scf-301-725
           else      go to fas-scf-301-750.
       fas-scf-301-725.
      *              *-------------------------------------------------*
      *              * Se variabile esistente                          *
      *              *-------------------------------------------------*
