      ...
      *                       Ultima revisione:    NdK del 19/09/18    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Analisi scadenze fornitori per fornitore    *
      *                                                                *
      *                    Sottoprogramma per esecuzione funzione di : *
      *                                                                *
      *                    Lista e riepilogo situazione scadenze for-  *
      *                    nitore                                      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
      *        * Per il fornitore ed eventuale dipendenza              *
      *        *-------------------------------------------------------*
           05  w-ipc-cod-fnt.
      *            *---------------------------------------------------*
      *            * Si/No variabile di i.p.c. relativa al fornitore   *
      *            * ed eventuale dipendenza                           *
      *            *---------------------------------------------------*
               10  w-ipc-cod-fnt-snx      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Valore della variabile di i.p.c. relativa al for- *
      *            * nitore ed eventuale dipendenza                    *
      *            *---------------------------------------------------*
               10  w-ipc-cod-fnt-val.
      *                *-----------------------------------------------*
      *                * Codice fornitore                              *
      *                *-----------------------------------------------*
                   15  w-ipc-cod-fnt-cod  pic  9(07)                  .
      *                *-----------------------------------------------*
      *                * Codice dipendenza fornitore                   *
      *                *-----------------------------------------------*
                   15  w-ipc-cod-fnt-dpz  pic  x(04)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-sts-scd.
               10  w-exp-sts-scd-num      pic  9(02)       value 05   .
      ...
      *    *===========================================================*
      *    * Lettura della variabile eventuale di i.p.c. per il for-   *
      *    * nitore e l'eventuale sua dipendenza                       *
      *    *-----------------------------------------------------------*
       ipc-cod-fnt-000.
      *              *-------------------------------------------------*
      ...
      *                  * Valore della variabile                      *
      *                  *---------------------------------------------*
           move      s-alf                to   w-ipc-cod-fnt-val      .
      *                  *---------------------------------------------*
      *                  * Controllo del valore della variabile        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Controllo sul codice fornitore          *
      *                      *-----------------------------------------*
           if        w-ipc-cod-fnt-cod    not  numeric
                     go to ipc-cod-fnt-400.
           if        w-ipc-cod-fnt-cod    =    zero
                     go to ipc-cod-fnt-400.
      *                  *---------------------------------------------*
      *                  * Preparazione parametri di accettazione      *
      *                  *---------------------------------------------*
      ...
      *                          * Bufferizzazione                     *
      *                          *-------------------------------------*
           move      w-ipc-cod-fnt-cod    to   rr-cod-fnt             .
      *                          *-------------------------------------*
      ...
      *                          * Bufferizzazione                     *
      *                          *-------------------------------------*
           move      w-ipc-cod-fnt-dpz    to   rr-dpz-fnt             .
      *                          *-------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      zero                 to   w-ipc-cod-fnt-cod      .
           move      spaces               to   w-ipc-cod-fnt-dpz      .
      *                  *---------------------------------------------*
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
           go to     ipc-cod-fnt-999.
      ...
      *              *-------------------------------------------------*
      *              * Visualizzazione codice dipendenza del fornito-  *
      *              * re, ragione sociale                             *
      *              *-------------------------------------------------*
      ...
      *                      * za di dipendenze per il fornitore in    *
      ...
      *              *-------------------------------------------------*
           ...
           move      12                   to   w-cod-cod-dcf-lin      .
           move      30                   to   w-cod-cod-dcf-pos      .
           move      12                   to   w-cod-cod-dcf-rln      .
           move      41                   to   w-cod-cod-dcf-rps      .
           move      13                   to   w-cod-cod-dcf-vln      .
           move      41                   to   w-cod-cod-dcf-vps      .
           move      14                   to   w-cod-cod-dcf-lln      .
      ...
      *                      *-----------------------------------------*
       ...
       nor-ric-sel-600.
      *                  *---------------------------------------------*
      *                  * Codice fornitore ed eventuale dipendenza    *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se non esiste una variabile di i.p.c.   *
      *                      * relativa al codice fornitore ed even-   *
      *                      * tuale sua dipendenza : no preparazione  *
      *                      * default                                 *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        w-ipc-cod-fnt-cod    =    zero
                     go to nor-ric-sel-700.
      *                      *-----------------------------------------*
      *                      * Preparazione default per codice forni-  *
      *                      * tore                                    *
      *                      *-----------------------------------------*
           move      w-ipc-cod-fnt-cod    to   rr-cod-fnt             .
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      "SCADENZE -"         to   w-all-str-cat (1)      .
      ...
      *                  *---------------------------------------------*
       ...
       vis-tot-tts-890.
      *                  *---------------------------------------------*
      *                  * Literal per 'Situazione debitoria fornito-  *
      *                  * re'                                         *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      v-lnr                to   v-lin                  .
           move      17                   to   v-pos                  .
           move      "Situazione debitoria fornitore :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       vis-tot-tts-895.
      *                  *---------------------------------------------*
      *                  * Totale 'Situazione debitoria fornitore'     *
      *                  *---------------------------------------------*
       vis-tot-tts-896.
      *                      *-----------------------------------------*
      *                      * Preparazione del totale                 *
      ...
      *              *-------------------------------------------------*
           ...
           move      rf-sfs-cod-fnt       to   w-rip-str-sfs-fnt      .
      ...
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "FNTDTS    "         to   f-key                  .
           move      w-rip-str-sfs-fnt    to   rf-sfs-cod-fnt         .
