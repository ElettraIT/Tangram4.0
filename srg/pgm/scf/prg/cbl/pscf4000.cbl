      ...
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase scf400, nella sua globalita', esegue i tipi di inter-  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * AFGLER      Lista e riepilogo situazione scadenze fornitori    *
      *             globale                                            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   ANALISI SCADENZE FORNITORI GLOBALE   "       .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'scf400'           *
      *              *-------------------------------------------------*
           perform   fas-scf-400-000      thru fas-scf-400-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *              * numero giorni di ritardo medi affinche' un pa-  *
      *              * gamento pervenga al fornitore                   *
      *              *-------------------------------------------------*
           perform   prs-grm-ppf-000      thru prs-grm-ppf-999        .
       loa-prs-scf-999.
           exit.

      *    *===========================================================*
      *    * Lettura personalizzazione : Numero giorni di ritardo medi *
      *    * affinche' un pagamento pervenga al fornitore              *
      *    *-----------------------------------------------------------*
       prs-grm-ppf-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/scf[grm-ppf]"
                                          to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-num          to   w-prs-scf-grm-ppf
           else      move  zero           to   w-prs-scf-grm-ppf      .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-scf-grm-ppf    =    zero or
                     w-prs-scf-grm-ppf    >    30
                     move  10             to   w-prs-scf-grm-ppf      .
       prs-grm-ppf-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *
           move      "Lista e riepilogo globale situazione scadenze     
      -              " AFGLER     pscf400a  "
                                          to   w-tin-ele-wci          .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-scf-400-300
           else if   v-key                =    "PRSC"
                     go to fas-scf-400-400
           else if   v-key                =    "NXSC"
                     go to fas-scf-400-450
           else if   v-key                =    "DOWN"
                     go to fas-scf-400-500
           else if   v-key                =    "EXIT"
                     go to fas-scf-400-250
           else      go to fas-scf-400-200.
       fas-scf-400-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-scf-400-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-scf-400-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-num
                     go to fas-scf-400-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-scf-400-350
           else      go to fas-scf-400-322.
       fas-scf-400-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DOWN"
                     go to fas-scf-400-630
           else if   v-key                =    "PRSC"
                     go to fas-scf-400-600
           else if   v-key                =    "NXSC"
                     go to fas-scf-400-610
           else if   v-key                =    "UP  "
                     go to fas-scf-400-620
           else if   v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-scf-400-650
           else if   v-key                =    "EXIT"
                     go to fas-scf-400-660
           else      go to fas-scf-400-570.
       fas-scf-400-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-scf-400-626
           else      go to fas-scf-400-628.
       fas-scf-400-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-scf-400-636
           else      go to fas-scf-400-638.
       fas-scf-400-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-scf-400-725
           else      go to fas-scf-400-750.
       fas-scf-400-725.
      *              *-------------------------------------------------*
      *              * Se variabile esistente                          *
      *              *-------------------------------------------------*
