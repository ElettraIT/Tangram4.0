      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma scf500   *
      *                                                                *
      ...
      *                                                                *
      *                    /abd/asc/UTENTE/exp/AZI_scf500.csv          *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
               10  w-rot-l02-saa-scf      pic  9(03)                  .
      *            *---------------------------------------------------*
      *            * Mese scadenza                                     *
      *            *---------------------------------------------------*
               10  w-rot-l02-mes-scf      pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Flag per rottura                                  *
      *            *                                                   *
      *            * - S : scaduto                                     *
      ...
      *        *-------------------------------------------------------*
      *        * Totali per indice di priorita'                        *
      *        *-------------------------------------------------------*
           05  w-tot-ipr.
               10  w-tot-ipr-tot          pic s9(11)                  .
      *        *-------------------------------------------------------*
      ...
      *                              *---------------------------------*
           ...
           if        w-det-srd-scf-cbp    not  = rr-cod-cbp
                     go to stp-srt-inp-120.
       stp-srt-inp-130.
      *                              *---------------------------------*
      *                              * Su status scadenza              *
      ...
      *
           if        rr-tip-ord           =    01
                     move  zero           to   srt-ipr-pgt
           else      move  rf-dcf-ipr-iel to   srt-ipr-pgt            .
      *
           if        rr-tip-ord           =    02 and
                     rf-dcf-ipr-iel       =    zero
                     move  02             to   srt-ipr-pgt            .
      *
      ...
      *              *-------------------------------------------------*
           ...
           move      s-saa                to   w-rot-l02-saa-scf      .
           move      s-mes                to   w-rot-l02-mes-scf      .
      *              *-------------------------------------------------*
      *              * Flag per rottura                                *
      *              *-------------------------------------------------*
           if        srt-dts-eff          =    zero
                     move  "V"            to   w-rot-l02-flg-rot
           else if   srt-dts-eff          not  > rr-dat-bcm
                     move  "S"            to   w-rot-l02-flg-rot
           else      move  "A"            to   w-rot-l02-flg-rot      .
      *              *-------------------------------------------------*
      *              * Indice di priorita'                             *
      *              *-------------------------------------------------*
           move      srt-ipr-pgt          to   w-rot-l01-ipr-pgt      .
       prn-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           move      w-rot-l02-saa-scf    to   w-tot-pme-saa
                                              (w-tot-pme-inx)         .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione mese                        *
      *                  *---------------------------------------------*
           move      w-rot-l02-mes-scf    to   w-tot-pme-mes
                                              (w-tot-pme-inx)         .
       prn-ini-lr2-100.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-rot-l02-flg-rot    not  = "V"
                     go to prn-ini-lr2-240.
      ...
      *                      *-----------------------------------------*
           move      "LM"                 to   s-ope                  .
           move      "E"                  to   s-tip                  .
           move      w-rot-l02-mes-scf    to   s-num                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                      *-----------------------------------------*
      *                      * Mese                                    *
      ...
      *                          *-------------------------------------*
           ...
           move      spaces               to   p-sgn                  .
           move      "<"                  to   p-edm                  .
           move      w-rot-l02-saa-scf    to   p-num                  .
           add       1900                 to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        w-rot-l02-flg-rot    =    "S"
                     go to prn-ini-lr2-260
           else      go to prn-ini-lr2-280.
       prn-ini-lr2-260.
      *                      *-----------------------------------------*
      *                      * Se scaduto                              *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       prn-ini-lr1-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione totale per indice di priorita'  *
      *              *-------------------------------------------------*
           move      zero                 to   w-tot-ipr-tot          .
       prn-ini-lr1-999.
           exit.
      ...
      *                  *---------------------------------------------*
      *                  * Test su indice di priorita'                 *
      *                  *---------------------------------------------*
           if        w-rot-l01-ipr-pgt    =    zero
                     go to prn-fin-lr1-999.
      *                  *---------------------------------------------*
      *                  * Test su totale                              *
      *                  *---------------------------------------------*
           if        w-tot-ipr-tot        =    zero
                     go to prn-fin-lr1-999.
       prn-fin-lr1-200.
      *              *-------------------------------------------------*
      *              * Stampa totale per indice di priorita'           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se righe residue sufficienti           *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      006                  to   p-pos                  .
      *
           if        w-rot-l01-ipr-pgt    =    01
                     move  "Priorita' ALTA  :"
                                          to   p-alf
           else if   w-rot-l01-ipr-pgt    =    02
                     move  "Priorita' MEDIA :"
                                          to   p-alf
           else if   w-rot-l01-ipr-pgt    =    03
                     move  "Priorita' BASSA :"
                                          to   p-alf
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-040.
      *                  *---------------------------------------------*
      *                  * Per indice di priorita'                     *
      *                  *---------------------------------------------*
           add       srt-imp-scf          to   w-tot-ipr-tot          .
       prn-liv-det-050.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      spaces               to   w-det-srd-scf-cbp      .
       det-srd-scf-050.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       det-srd-scf-800.
      *              *-------------------------------------------------*
      *              * Determinazione codice ns. banca                 *
      *              *-------------------------------------------------*
           move      rf-sfs-cbp-nsb       to   w-det-srd-scf-cbp      .
      *              *-------------------------------------------------*
