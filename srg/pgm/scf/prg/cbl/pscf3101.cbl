      ...
      *                       Ultima revisione:    NdK del 25/07/11    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma scf310   *
      *                                                                *
      ...
      *            *---------------------------------------------------*
               10  srt-sdo-scf            pic s9(13)       trailing
                                                           separate
                                                           character  .
      *            *---------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      zero                 to   srt-key-cod-fnt        .
      *                  *---------------------------------------------*
      *                  * Dati                                        *
      *                  *---------------------------------------------*
           move      zero                 to   srt-cod-fnt            .
           move      spaces               to   srt-rag-soc            .
           move      zero                 to   srt-sdo-cge            .
           move      zero                 to   srt-sdo-scf            .
           move      zero                 to   srt-sdo-dif            .
       stp-srt-inp-350.
      *              *-------------------------------------------------*
      *              * Preparazione dati del record di sort da anagra- *
      *              * fica contabile del fornitore                    *
      *              *-------------------------------------------------*
           move      rf-fnt-rag-key       to   srt-key-rsc-upp        .
           move      rf-fnt-cod-fnt       to   srt-key-cod-fnt        .
      ...
      *              * lettura di tutte le scadenze relative al forni- *
      *              * tore contabile in esame                         *
      *              *-------------------------------------------------*
           perform   det-sdo-scf-000      thru det-sdo-scf-999        .
       stp-srt-inp-500.
      *              *-------------------------------------------------*
      *              * Preparazione differenza tra i due saldi         *
      *              *-------------------------------------------------*
           move      srt-sdo-cge          to   srt-sdo-dif            .
           subtract  srt-sdo-scf          from srt-sdo-dif            .
       stp-srt-inp-550.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       det-sdo-scf-000.
      *              *-------------------------------------------------*
      *              * Start su file [sfs] per fornitore               *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "FNTDTS    "         to   f-key                  .
           move      rf-fnt-cod-fnt       to   rf-sfs-cod-fnt         .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to det-sdo-scf-900.
       det-sdo-scf-100.
      *              *-------------------------------------------------*
      *              * Read next da file [sfs] per fornitore           *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/scf/fls/ioc/obj/iofsfs"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
       ...
       det-sdo-scf-200.
      *              *-------------------------------------------------*
      *              * Test max su file [sfs] per fornitore            *
      *              *-------------------------------------------------*
           if        rf-sfs-cod-fnt       not  = rf-fnt-cod-fnt
                     go to det-sdo-scf-900.
       det-sdo-scf-250.
      *              *-------------------------------------------------*
      ...
      *              * lita'                                           *
      *              *-------------------------------------------------*
           if        rf-sfs-drc-rgs       not  = zero
                     move  "S"            to   w-det-agg-cge-snx
                     go to det-sdo-scf-300.
           if        rf-sfs-tip-scf       =    02 or
                     rf-sfs-tip-scf       =    03 or
                     rf-sfs-tip-scf       =    04 or
                     rf-sfs-tip-scf       =    05 or
                     rf-sfs-tip-scf       =    06 or
                     rf-sfs-tip-scf       =    09 or
                     rf-sfs-tip-scf       =    10 or
                     rf-sfs-tip-scf       =    11
                     move  "S"            to   w-det-agg-cge-snx
                     go to det-sdo-scf-300.
      ...
      *                  * TEST DA RIVEDERE                            *
      *                  *---------------------------------------------*
______*    if        w-det-agg-cge-snx    =    "S"
______*              go to det-sdo-scf-700.
      *                  *---------------------------------------------*
      *                  * Test su tipo documento di riferimento       *
      *                  *---------------------------------------------*
           if        rf-sfs-tip-ddr       =    01 or
                     rf-sfs-tip-ddr       =    02 or
                     rf-sfs-tip-ddr       =    99
                     add      rf-sfs-imp-scf
                                          to   srt-sdo-scf
      ...
      *              *-------------------------------------------------*
           if        srt-sdo-scf          not  = zero
                     multiply  -1         by   srt-sdo-scf            .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     det-sdo-scf-999.
       det-sdo-scf-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun fornitore con differenza saldi !"
                                          to   m-msg                  .
      ...
      *              *-------------------------------------------------*
           move      "CONTROLLO INCROCIATO TRA CONTABILITA' E SCADENZIAR
      -              "IO FORNITORI"       to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "     Codice e ragione sociale del fornitore       
      -              "    contabile    scadenziario      Differenza "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
