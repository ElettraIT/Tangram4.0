      ...
      *                                   Fase:    scf420              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 10/09/21    *
      *                       Ultima revisione:    NdK del 20/02/26    *
      *                    ------------------------------------------- *
      ...
      *        *-------------------------------------------------------*
      *        * Comodo per determinazione tipo documento XML          *
      *        *-------------------------------------------------------*
           05  w-det-dst-tdo-fel          pic  x(05)                  .
      ...
      *              *-------------------------------------------------*
           move      "OP"                 to   f-ope                  .
           move      "pgm/scf/num/ioc/obj/infdxnum"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-dat-min           to   rf-sff-ddo-ftf         .
           move      spaces               to   rf-sff-ndo-ftf         .
           move      zero                 to   rf-sff-ndo-ftf         .
           move      "pgm/scf/fls/ioc/obj/iofsff"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test che il numero registrazione non superi *
      *                  * il massimo consentito (35.000)              *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test                                    *
      *                      *-----------------------------------------*
           move      rf-sff-num-ftf       to   w-wrk-nft-int          .
      ...
      *                  *---------------------------------------------*
           go to     stp-srt-inp-420
                     stp-srt-inp-430
                     stp-srt-inp-440
                     depending            on   rr-tip-doc             .
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Documenti CEE                               *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se CEE (merce o servizi)           *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su regime fiscale                  *
      *                      *-----------------------------------------*
           if        rf-sff-flg-rfp      not  = "R"
                     go to stp-srt-inp-200.
      *                      *-----------------------------------------*
      *                      * Oltre                                   *
      *                      *-----------------------------------------*
      ...
      *                      * Test se Estero                          *
      *                      *-----------------------------------------*
           if        rf-sff-tpv-ftf      not  = 03
                     go to stp-srt-inp-200.
      *                      *-----------------------------------------*
      *                      * Test se Servizi                         *
      *                      *-----------------------------------------*
           if        rf-sff-flg-mos      not  = "S"
                     go to stp-srt-inp-200.
      *                      *-----------------------------------------*
      *                      * Oltre                                   *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      rf-sff-num-ftf       to   w-det-dst-num-prt      .
           perform   stp-srt-inp-fdx-000  thru stp-srt-inp-fdx-999    .
       ...
       stp-srt-inp-566.
           ...
           if        rr-cod-fnt-eco
                    (w-cix-ctr-001)       =    zero
                     go to stp-srt-inp-200.
           if        rr-cod-fnt-eco
                    (w-cix-ctr-001)       =    rf-sff-cod-fnt
                     go to stp-srt-inp-570
           else      go to stp-srt-inp-566.
       stp-srt-inp-570.
      *                  *---------------------------------------------*
      *                  * Selezione su codice fiscale fornitore       *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Normalizzazione [fnt]                   *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           call      s-pat               using f
                                               rf-fnt                 .
      *                      *-----------------------------------------*
      *                      * Selezione su si/no fornitori esteri     *
      *                      *-----------------------------------------*
       stp-srt-inp-574.
      *                      *-----------------------------------------*
      *                      * Test su codice fiscale e partita Iva    *
      *                      *-----------------------------------------*
           if        rf-fnt-prt-iva       not  = zero
                     go to stp-srt-inp-575.
           if        rf-fnt-cod-fis       not  = spaces
                     go to stp-srt-inp-575.
      *                      *-----------------------------------------*
      *                      * Trattamento messaggio di errore         *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-575.
      *                  *---------------------------------------------*
      *                  * Selezione su codice comune del fornitore    *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se Italia                          *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Forzatura da personalizzazione [scf]    *
      *                      *-----------------------------------------*
           move      rr-prm-inv (4)       to   w-wrk-pec-sdi          .
           move      rr-prm-inv (9)       to   w-wrk-cod-sdi          .
      *                      *-----------------------------------------*
      ...
      *              * Numero documento                                *
      *              *-------------------------------------------------*
           move      rf-sff-ndo-ftf       to   w-all-str-cat (6)      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-det-dst-num-prt    to   rf-fdx-prt-doc         .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-fdx-prt-doc       not  = w-det-dst-num-prt
                     go to stp-srt-inp-fdx-900.
       stp-srt-inp-fdx-400.
      *              *-------------------------------------------------*
      *              * Selezioni                                       *
      ...
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
           go to     stp-srt-inp-fdx-200.
       stp-srt-inp-fdx-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     stp-srt-inp-fdx-999.
       stp-srt-inp-fdx-999.
           exit.
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-200.
      *              *-------------------------------------------------*
      *              * Preparazione dati per scrittura [fdx]           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Tipo documento per la fattura elettronica   *
      *                  *---------------------------------------------*
           move      spaces               to   w-det-dst-tdo-fel      .
      *                  *---------------------------------------------*
      *                  * Test su flag di regime fiscale              *
      *                  *---------------------------------------------*
           if        rf-sff-flg-rfp       =    "R"
                     move  "TD16"         to   w-det-dst-tdo-fel
                     go to prn-liv-det-400.
      *                  *---------------------------------------------*
      *                  * Test su tipo provenienza                    *
      *                  *---------------------------------------------*
           if        rf-sff-tpv-ftf       =    03
                     move  "TD17"         to   w-det-dst-tdo-fel
           else      move  "TD18"         to   w-det-dst-tdo-fel      .
      *                  *---------------------------------------------*
      *                  * Test su giornale Iva                        *
      *                  *                                             *
      *                  * ELETTRA - DA MIGLIORARE !!!                 *
      *                  *---------------------------------------------*
           if        rf-sff-num-giv       =    03
                     move  "TD19"         to   w-det-dst-tdo-fel      .
           if        rf-sff-num-giv       =    04
                     move  "TD17"         to   w-det-dst-tdo-fel      .
       prn-liv-det-400.
      *              *-------------------------------------------------*
      *              * Trattamento file [fdx]                          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Write record [fdx]                          *
      *                  *---------------------------------------------*
           perform   wrt-rec-fdx-000      thru wrt-rec-fdx-999        .
       prn-liv-det-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *              * zioni [fdxnum]                                  *
      *              *-------------------------------------------------*
           move      w-num-dst-scl-ann    to   rn-fdx-num-scl-ann     .
      *              *-------------------------------------------------*
      *              * Lettura tabella numerazioni [fdxnum]            *
      *              *-------------------------------------------------*
           move      "GT"                 to   f-ope                  .
           move      "pgm/scf/num/ioc/obj/infdxnum"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
           move      rn-fdx-num-num-dst   to   w-num-dst-num-dst      .
      ...
      *                  *---------------------------------------------*
           move      w-num-dst-num-dst    to   rn-fdx-num-num-dst     .
      *                  *---------------------------------------------*
      *                  * Update record                               *
      *                  *---------------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *    *===========================================================*
      *    * Scrittura record [fdx]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-fdx-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-fdx-000      thru cmp-rec-fdx-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-fdx-ide-dat         .
           move      s-ute                to   rf-fdx-ide-ute         .
           move      s-fas                to   rf-fdx-ide-fas         .
           move      w-num-dst-num-new    to   rf-fdx-num-dst         .
           move      w-cix-num-ele        to   rf-fdx-num-prg         .
           move      s-dat                to   rf-fdx-dat-reg         .
           move      zero                 to   rf-fdx-tip-dst         .
           move      "F"                  to   rf-fdx-tip-arc         .
           move      srt-cod-fnt          to   rf-fdx-cod-arc         .
           move      srt-dpz-fnt          to   rf-fdx-dpz-arc         .
           move      zero                 to   rf-fdx-tip-ids         .
           move      rr-prm-inv (4)       to   rf-fdx-eml-mit         .
           move      srt-eml-ind          to   rf-fdx-eml-ind         .
           move      spaces               to   rf-fdx-eml-all         .
______*    move      rf-sff-tip-doc       to   rf-fdx-tip-doc         .
           move      zero                 to   rf-fdx-tip-doc         .
______*    move      rf-sff-cod-tmo       to   rf-fdx-tmo-doc         .
           move      spaces               to   rf-fdx-tmo-doc         .
           move      rf-sff-ddo-ftf       to   rf-fdx-dat-doc         .
           move      rf-sff-ndo-ftf       to   rf-fdx-num-doc         .
           move      rf-sff-num-ftf       to   rf-fdx-prt-doc         .
           move      rf-sff-npt-iva       to   rf-fdx-prt-iva         .
