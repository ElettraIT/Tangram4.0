      ...
      *                       Ultima revisione:    NdK del 21/03/25    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Consolidamento ordini clienti evasi         *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   CONSOLIDAMENTO ORDINI CLIENTI EVASI  "       .
      ...
      *        *-------------------------------------------------------*
           05  rr-dat-orc                 pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * Si/no esecuzione di prova                             *
      *        *-------------------------------------------------------*
           05  rr-snx-prv                 pic  x(01)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no esecuzione di prova                  *
      *        *-------------------------------------------------------*
           05  w-exp-snx-prv.
               10  w-exp-snx-prv-num      pic  9(02)       value 02   .
               10  w-exp-snx-prv-lun      pic  9(02)       value 02   .
               10  w-exp-snx-prv-tbl.
      ...
      *        *-------------------------------------------------------*
      *        * Comodi per scansione date                             *
      *        *-------------------------------------------------------*
           05  w-prn-dat-ini              pic  9(07)                  .
           05  w-prn-dat-fin              pic  9(07)                  .
      ...
      *                  * Si/No gestione ordini di spedizione attiva  *
      *                  *---------------------------------------------*
           perform   prs-osc-snx-000      thru prs-osc-snx-999        .
       pre-exe-pgm-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/osc[snx]"       to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-osc-snx
           else      move  spaces         to   w-prs-osc-snx          .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-osc-snx        =    "S" or
                     w-prs-osc-snx        =    "N"
                     go to prs-osc-snx-999.
           move      "N"                  to   w-prs-osc-snx          .
       prs-osc-snx-999.
           exit.
      ...
      *                  *---------------------------------------------*
      *                  * Data emissione ordini                       *
      *                  *---------------------------------------------*
           perform   acc-dat-orc-000      thru acc-dat-orc-999        .
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           move      zero                 to   rr-dat-orc             .
           move      spaces               to   rr-snx-prv             .
       nor-ric-sel-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      05                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Fino alla data di evasione :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data emissione ordini      *
      *    *-----------------------------------------------------------*
       acc-dat-orc-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-dat-orc           not  = zero
                     go to acc-dat-orc-100.
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           if        s-sts                not  =    spaces
                     ...
                     go to acc-dat-orc-040.
      *                      *-----------------------------------------*
      *                      * Bufferizzazione valore di default       *
      *                      *-----------------------------------------*
           move      s-dat                to   rr-dat-orc             .
       acc-dat-orc-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-orc-999.
       acc-dat-orc-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-orc             .
       acc-dat-orc-400.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no esecuzione di prova            *
      *    *-----------------------------------------------------------*
       acc-snx-prv-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-prv-lun    to   v-car                  .
           move      w-exp-snx-prv-num    to   v-ldt                  .
           move      "SN#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-prv-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      07                   to   v-lin                  .
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "S"            to   rr-snx-prv
           else if   v-num                =    02
                     move  "N"            to   rr-snx-prv
           else      move  zero           to   rr-snx-prv             .
       acc-snx-prv-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione si/no esecuzione di prova       *
      *              *-------------------------------------------------*
           if        rr-snx-prv           not  = "S"
                     move  "N"            to   rr-snx-prv             .
       reg-ric-sel-800.
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per ordini da consolidare            *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
           move      11                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Ordini da consolidare      :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per ordini consolidati               *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      zero                 to   w-prn-dat-ini          .
           move      zero                 to   w-prn-dat-fin          .
      *              *-------------------------------------------------*
      *              * Start su file [oct]                             *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
      ...
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-206.
      *              *-------------------------------------------------*
      *              * Determinazione se la riga ordine cliente e'     *
      *              * completamente evasa entro la data massima ri-   *
      *              * chiesta                                         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Determinazione quantita' da evadere riga    *
      *                  * ordine cliente                              *
      *                  *---------------------------------------------*
           move      "DT"                 to   d-qev-roc-tip-ope      .
           perform   det-qev-roc-cll-000  thru det-qev-roc-cll-999    .
      *                  *---------------------------------------------*
      *                  * Deviazione a seconda che si tratti o no di  *
      *                  * una riga di addebito                        *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su status riga di addebito : che   *
      *                      * sia a 'Evasa'                           *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su quantita' ancora da evadere     *
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-230.
      *                  *---------------------------------------------*
      *                  * Test su documento ultima evasione           *
      *                  *---------------------------------------------*
           if        d-qev-roc-tip-uev    not  = 01       or
                     d-qev-roc-ddo-uev    >    rr-dat-orc
                     go to stp-srt-inp-100.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      rf-oct-num-prt       to   srt-prt-orc            .
      *              *-------------------------------------------------*
      ...
      *              * Aggiornamento date                              *
      *              *-------------------------------------------------*
           if        w-prn-dat-ini        =    zero
                     move  rf-oct-dat-doc to   w-prn-dat-ini          .
           move      rf-oct-dat-doc       to   w-prn-dat-fin          .
      *              *-------------------------------------------------*
      *              * Visualizzazione numero ordini da consolidare    *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      07                   to   v-car                  .
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun ordine cliente da consolidare !"
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *              *-------------------------------------------------*
       ...
       prn-liv-det-050.
      *              *-------------------------------------------------*
      *              * Consolidamento righe ordine cliente             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start su file [ocr]                         *
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "NUMPRT    "         to   f-key                  .
           move      srt-prt-orc          to   rf-ocr-num-prt         .
           move      zero                 to   rf-ocr-num-prg         .
           move      "pgm/orc/fls/ioc/obj/iofocr"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-200.
      *                  *---------------------------------------------*
      *                  * Cancellazione records [ocp] relativi alla   *
      *                  * riga ordine                                 *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Start su file [ocp]                     *
      ...
      *                      * Test sul massimo                        *
      *                      *-----------------------------------------*
           if        rf-ocp-num-prt       not  = rf-ocr-num-prt or
                     rf-ocp-num-prg       not  = rf-ocr-num-prg
                     go to prn-liv-det-300.
      *                      *-----------------------------------------*
      *                      * Delete record [ocp]                     *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       prn-liv-det-300.
      *                  *---------------------------------------------*
      *                  * Riciclo su prossimo record [ocr]            *
      *                  *---------------------------------------------*
           go to     prn-liv-det-100.
       prn-liv-det-500.
      *              *-------------------------------------------------*
      *              * Consolidamento testata ordine cliente           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      srt-prt-orc          to   rf-oct-num-prt         .
           move      "pgm/orc/fls/ioc/obj/iofoct"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
      *              * Visualizzazione numero ordini consolidati       *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      07                   to   v-car                  .
      ...
      *                      *-----------------------------------------*
           ...
           string    "Consolidamento ordini evasi al .: "
                                delimited by   size
