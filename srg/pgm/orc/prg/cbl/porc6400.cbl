      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma porc6401:        *
      *                                                                *
      *                    Lista assegnazioni ordini per prodotto      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  w-exp-sts-ass.
               10  w-exp-sts-ass-num      pic  9(02)       value 02   .
               10  w-exp-sts-ass-lun      pic  9(02)       value 25   .
               10  w-exp-sts-ass-tbl.
                   15  filler             pic  x(25) value
                            "Solo quelli assegnati    "               .
                   15  filler             pic  x(25) value
                            "Tutti gli inevasi        "               .
      ...
      *                  *---------------------------------------------*
           perform   acc-sts-ass-000      thru acc-sts-ass-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           perform   pmt-sts-ass-000      thru pmt-sts-ass-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione status ordini da ricercare      *
      *              *-------------------------------------------------*
           perform   vis-sts-ass-000      thru vis-sts-ass-999        .
       pmt-ric-sel-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sts-ass-lun    to   v-car                  .
           move      w-exp-sts-ass-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      w-exp-sts-ass-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      12                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      rr-sts-ass           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sts-ass-999.
       acc-sts-ass-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sts-ass             .
       acc-sts-ass-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *
      *                  *---------------------------------------------*
           if        rr-sts-ass           not  = zero
                     go to acc-sts-ass-600.
           if        v-key                =    "UP  "
                     go to acc-sts-ass-600.
           go to     acc-sts-ass-100.
       acc-sts-ass-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
