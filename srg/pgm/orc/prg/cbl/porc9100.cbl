      ...
      *                                   Fase:    orc910              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 23/10/04    *
      *                       Ultima revisione:    NdK del 07/06/22    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Convalida ordine alla Sede                  *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
      *        * Tipo movimento per ordine da Dipendenza               *
      *        *-------------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Forzatura default per tipo movimento            *
      *              * ___                                             *
      *              *-------------------------------------------------*
           move      "OD   "              to   w-def-tmo-orc          .
      *              *-------------------------------------------------*
      ...
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        w-tes-tmo-orc-tip    =    "P"
                     go to acc-tmo-orc-410.
      *                      *-----------------------------------------*
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "L'ordine deve essere di tipo 'pro-forma' !        
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
      *                      *-----------------------------------------*
      *                      * A reimpostazione                        *
      ...
      *                      *-----------------------------------------*
      *                      * Ritaratura tipo documento               *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Campo accettato in comodo di accet- *
      *                          * tazione                             *
      *                          *-------------------------------------*
           move      w-tes-tmo-orc        to   w-acc-tmo-orc          .
           move      w-tes-tmo-orc-des    to   w-acc-tmo-orc-des      .
      *                          *-------------------------------------*
      *                          * Default utilizzato per la lettura   *
      *                          * del tipo movimento                  *
      *                          *-------------------------------------*
           move      w-def-tmo-orc        to   w-tes-tmo-orc          .
      *                          *-------------------------------------*
      *                          * Se default a spazi : forzatura      *
      *                          *-------------------------------------*
           if        w-tes-tmo-orc        =    spaces
                     move  "OD   "        to   w-tes-tmo-orc          .
      *                          *-------------------------------------*
      *                          * Lettura archivio [zoc]              *
      *                          *-------------------------------------*
           move      w-tes-tmo-orc        to   w-let-arc-zoc-cod      .
           perform   let-arc-zoc-000      thru let-arc-zoc-999        .
      *                          *-------------------------------------*
      *                          * Bufferizzazione valori associati al *
      *                          * tipo movimento ordini clienti       *
      *                          *-------------------------------------*
           move      w-let-arc-zoc-des    to   w-tes-tmo-orc-des      .
           move      w-let-arc-zoc-vld    to   w-tes-tmo-orc-vld      .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-att-orc-prt-num    to   w-tes-prt-att (1)      .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "D  "                to   w-att-orc-ndo-sgl      .
           perform   att-orc-ndo-000      thru att-orc-ndo-999        .
           move      w-att-orc-ndo-num    to   w-tes-ndo-att (1)      .
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       scr-mov-fil-000.
      *              *-------------------------------------------------*
      *              * Preparazione del tipo movimento                 *
      *              *-------------------------------------------------*
       scr-mov-fil-050.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      rf-ocr-num-prg       to   w-sav-num-prg          .
      *                  *---------------------------------------------*
      *                  * Aggiornamento record [ocr] originale        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Flag di chiusura                        *
      *                      *-----------------------------------------*
           move      "#"                  to   rf-ocr-flg-rch         .
      *                      *-----------------------------------------*
      *                      * Update record [ocr]                     *
      *                      *-----------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *                  *---------------------------------------------*
      *                  * Scrittura record [ocr] subentrante          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Numero protocollo                       *
      *                      *-----------------------------------------*
           move      w-tes-prt-att (1)    to   rf-ocr-num-prt         .
      *                      *-----------------------------------------*
      *                      * Numero progressivo nel protocollo       *
      *                      *-----------------------------------------*
           move      w-scr-mov-fil-prg    to   rf-ocr-num-prg         .
      *                      *-----------------------------------------*
      *                      * Tipo documento                          *
      *                      *-----------------------------------------*
           move      w-def-tmo-orc        to   rf-ocr-tmo-orc         .
      *                      *-----------------------------------------*
      *                      * Codice dipendenza : Sede                *
      *                      *-----------------------------------------*
      ...
      *                      * Numero documento                        *
      *                      *-----------------------------------------*
           move      w-tes-ndo-att (1)    to   rf-ocr-num-doc         .
      *                      *-----------------------------------------*
      *                      * Codice archivio                         *
      *                      *-----------------------------------------*
           move      w-tes-cod-dpz        to   rf-ocr-cod-arc         .
      *                      *-----------------------------------------*
      *                      * Codice dipendenza dell'archivio         *
      *                      *-----------------------------------------*
           move      spaces               to   rf-ocr-dpz-arc         .
      *                      *-----------------------------------------*
      *                      * Quantita' ordinata : la stessa          *
      *                      *-----------------------------------------*
      *                      *-----------------------------------------*
      *                      * Valuta                                  *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Normalizzazione tipo ordine             *
      *                      *-----------------------------------------*
           move      spaces               to   rf-ocr-tip-ord         .
      *                      *-----------------------------------------*
      *                      * Normalizzazione flag di chiusura        *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        w-scr-mov-fil-wtr    =    "P"
                     ...
                     add   rf-ocr-imp-rig to   w-tot-tot-rta          .
       scr-mov-fil-240.
      *                  *---------------------------------------------*
      *                  * Scrittura record [ocx] subentrante          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se necessario                      *
      *                      *-----------------------------------------*
           if        rf-ocr-des-ext       not  = 01
                     go to scr-mov-fil-260.
      *                      *-----------------------------------------*
      *                      * Lettura record [ocx] originale          *
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      w-tes-num-prt        to   rf-ocx-num-prt         .
           move      w-sav-num-prg        to   rf-ocx-num-prg         .
      ...
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to scr-mov-fil-260.
      *                      *-----------------------------------------*
      *                      * Composizione record [ocx] subentrante   *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Numero protocollo                   *
      *                          *-------------------------------------*
           move      w-tes-prt-att (1)    to   rf-ocx-num-prt         .
      *                          *-------------------------------------*
      *                          * Numero progressivo nel protocollo   *
      *                          *-------------------------------------*
           move      w-scr-mov-fil-prg    to   rf-ocx-num-prg         .
      *                          *-------------------------------------*
      *                          * Put record [ocx]                    *
      *                          *-------------------------------------*
           move      "PT"                 to   f-ope                  .
       ...
       scr-mov-fil-260.
      *                  *---------------------------------------------*
      *                  * Scrittura records [ocp] subentrante         *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Start su file [ocp] originale           *
      *                      *-----------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "NUMPRT    "         to   f-key                  .
           move      w-tes-num-prt        to   rf-ocp-num-prt         .
           move      w-sav-num-prg        to   rf-ocp-num-prg         .
      ...
      *                      * Test sul massimo                        *
      *                      *-----------------------------------------*
           if        rf-ocp-num-prt       not  = w-tes-num-prt or
                     rf-ocp-num-prg       not  = w-sav-num-prg
                     go to scr-mov-fil-280.
      *                      *-----------------------------------------*
      *                      * Composizione record [ocp] subentrante   *
      *                      * (con lo stesso numero progressivo di    *
      *                      * frammento)                              *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Numero protocollo                   *
      *                          *-------------------------------------*
           move      w-tes-prt-att (1)    to   rf-ocp-num-prt         .
      *                          *-------------------------------------*
      *                          * Numero progressivo nel protocollo   *
      *                          *-------------------------------------*
           move      w-scr-mov-fil-prg    to   rf-ocp-num-prg         .
      *                          *-------------------------------------*
      *                          * Put record [ocp]                    *
      *                          *-------------------------------------*
           move      "PT"                 to   f-ope                  .
           ...
           call      s-pat               using f
                                               rf-ocp                 .
      *                      *-----------------------------------------*
      *                      * Cancellazione record [ocp] originale    *
      *                      * (con lo stesso numero progressivo di    *
      *                      * frammento)                              *
      *                      *-----------------------------------------*
           move      "DE"                 to   f-ope                  .
           ...
           call      s-pat               using f
                                               rf-ocp                 .
      *                      *-----------------------------------------*
      *                      * Riciclo su lettura sequenziale [ocp]    *
      *                      *-----------------------------------------*
           go to     scr-mov-fil-270.
       scr-mov-fil-280.
      *                  *---------------------------------------------*
      ...
      *                  * Numero protocollo                           *
      *                  *---------------------------------------------*
           move      w-tes-prt-att (1)    to   rf-oct-num-prt         .
      *                  *---------------------------------------------*
      *                  * Data documento                              *
      *                  *---------------------------------------------*
      ...
      *                  * Numero documento                            *
      *                  *---------------------------------------------*
           move      w-tes-ndo-att (1)    to   rf-oct-num-doc         .
      *                  *---------------------------------------------*
      *                  * Tipo documento                              *
      *                  *---------------------------------------------*
           move      w-def-tmo-orc        to   rf-oct-tmo-orc         .
      *                  *---------------------------------------------*
      *                  * Codice dipendenza : Sede                    *
      *                  *---------------------------------------------*
      ...
      *                  * Sigla numerazione                           *
      *                  *---------------------------------------------*
           move      "D  "                to   rf-oct-sgl-num         .
      *                  *---------------------------------------------*
      *                  * Codice archivio                             *
      *                  *---------------------------------------------*
           move      w-tes-cod-dpz        to   rf-oct-cod-arc         .
      *                  *---------------------------------------------*
      *                  * Codice dipendenza dell'archivio             *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      w-tot-tot-rig (1)    to   rf-oct-tot-rig (1)     .
           move      w-tot-tot-rig (2)    to   rf-oct-tot-rig (2)     .
           move      w-tot-tot-rig (3)    to   rf-oct-tot-rig (3)     .
           move      w-tot-tot-rig (4)    to   rf-oct-tot-rig (4)     .
           move      w-tot-tot-rig (5)    to   rf-oct-tot-rig (5)     .
           move      w-tot-tot-rig (6)    to   rf-oct-tot-rig (6)     .
           move      w-tot-tot-rig (7)    to   rf-oct-tot-rig (7)     .
           move      w-tot-tot-rig (8)    to   rf-oct-tot-rig (8)     .
           move      w-tot-tot-rig (9)    to   rf-oct-tot-rig (9)     .
      *                  *---------------------------------------------*
      *                  * Totale documento                            *
      *                  *---------------------------------------------*
           add       w-tot-tot-rig (1)
                     ...
                     w-tot-tot-rig (8)
                     w-tot-tot-rig (9)
                     w-tot-tot-rta      giving rf-oct-tot-doc         .
      *                  *---------------------------------------------*
      ...
      *                          *-------------------------------------*
       ...
       scr-mov-fil-800.
      *                  *---------------------------------------------*
      *                  * Aggiornamento record [oct] originale        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Lettura record [oct]                    *
      *                      *-----------------------------------------*
           ...
           call      s-pat               using f
                                               rf-oct                 .
           if        f-sts                not  = e-not-err
                     go to scr-mov-fil-850.
      *                      *-----------------------------------------*
      *                      * Flag di chiusura                        *
      *                      *-----------------------------------------*
           move      "#"                  to   rf-oct-flg-och         .
      *                      *-----------------------------------------*
      *                      * Update record [oct]                     *
      *                      *-----------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-slc-num-oct-c05    to   v-lin                  .
           move      18                   to   v-pos                  .
           move      rf-oct-num-doc       to   w-att-orc-ndo-num      .
           move      w-att-orc-ndo-prg    to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *                  *---------------------------------------------*
      *                  * Codice archivio                             *
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-ocr-cdc-vpf       to   w-cvs-vlt-cdc          .
