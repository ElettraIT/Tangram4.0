      ...
      *                                   Fase:    orc001              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 20/09/22    *
      *                       Ultima revisione:    NdK del 31/03/23    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Personalizzazioni di invio conferme ordini  *
      *                                                                *
      *                   (Solo SUPERVISORE)                           *
      *                                                                *
      *                    PROMEMORIA: Implementare indirizzo mail     *
      *                               'in copia' se diverso da         *
      *                                mittente (da implementare       *
      *                                anche nei moduli 't_sender')    *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      ...
      * ----  ----------  -------------------------------------------- *
      *                                                                *
      *  01   orc540      Parametri di invio via mail conferme ordini  *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " PERSONALIZZAZIONI INVIO ORDINI CLIENTI "       .
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-key.
      *            *---------------------------------------------------*
      *            * Tipo record in archivio [orc]                     *
      *            *---------------------------------------------------*
               10  w-tes-tip-rec          pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Codice record in archivio [orc]                   *
      *            *---------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-exp-mdl-inv.
               10  w-exp-mdl-inv-num      pic  9(02)       value 04   .
      ...
      *                  * Destinatario mail - SDI                     *
      *                  *---------------------------------------------*
______*    perform   pmt-dst-sdi-000      thru pmt-dst-sdi-999        .
      *                  *---------------------------------------------*
      *                  * Fine Pagina 1                               *
      *                  *---------------------------------------------*
           go to     pmt-tes-reg-999.
      ...
      *
           if        w-tes-mdl-inv (1)    =    "t_sender            "
                     ...
                     move  03             to   v-num
           else if   w-tes-mdl-inv (1)    =    "t_sender_tls        "
                     move  04             to   v-num
           else      move  zero           to   v-num                  .
      *
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-tip-rec        to   rf-orc-tip-rec         .
           move      w-tes-cod-rec        to   rf-orc-cod-rec         .
           move      "pgm/orc/fls/ioc/obj/ioforc"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [orc]                        *
      *                          *-------------------------------------*
           move      rf-orc-dat-rec       to   w-tes-dat-rec (1)      .
       rou-let-reg-500.
      *                      *-----------------------------------------*
      ...
      *                      * Write record [orc]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-orc-000      thru wrt-rec-orc-999        .
      ...
      *                      * Rewrite record [orc]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-orc-000      thru rew-rec-orc-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [orc]                             *
      *              *-------------------------------------------------*
           perform   del-rec-orc-000      thru del-rec-orc-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [orc]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-orc-000.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-orc-ide-dat         .
           move      s-ute                to   rf-orc-ide-ute         .
           move      s-fas                to   rf-orc-ide-fas         .
           move      w-tes-dat-rec (1)    to   rf-orc-dat-rec         .
       cmp-rec-orc-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [orc]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-orc-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-orc-000      thru cmp-rec-orc-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
