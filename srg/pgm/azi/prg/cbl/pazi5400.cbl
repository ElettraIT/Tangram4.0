      ...
      *                       Ultima revisione:    NdK del 28/05/18    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma di stampa azi540 *
      *                    Stampa distinte di invio                    *
      *                                                                *
      *                    Le overlay eseguono le seguenti funzioni :  *
      *                    ------------------------------------------  *
      *                    pazi5400 - Main                             *
      *                    pazi540a - Creazione Distinta di invio      *
      *                    pazi540b - Stampa Distinta                  *
      *                    pazi540m - Manutenzione distinta            *
      *                    pazi540d - Invio o re-invio documenti PDF   *
      *                    pazi540e - Cancellazione Distinta           *
      *                                                                *
      *                    ___ DA COMPLETARE E PERFEZIONARE ___        *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  GESTIONE INVIO CIRCOLARI CON ALLEGATO "       .
      ...
      *              *-------------------------------------------------*
           if        rr-tip-ope           =    01
                     move  "pgm/azi/prg/obj/pazi540a                "
                                          to   i-exe-pat
           else if   rr-tip-ope           =    02
                     move  "pgm/azi/prg/obj/pazi540b                "
                                          to   i-exe-pat
           else if   rr-tip-ope           =    03
                     move  "pgm/azi/prg/obj/pazi540m                "
                                          to   i-exe-pat
           else if   rr-tip-ope           =    04
                     move  "pgm/azi/prg/obj/pazi540c                "
                                          to   i-exe-pat
           else if   rr-tip-ope           =    05
                     move  "pgm/azi/prg/obj/pazi540d                "
                                          to   i-exe-pat
      ...
      *              *-------------------------------------------------*
      *              * Lettura della personalizzazione relativa al-    *
      *              * l'agente di spedizione                          *
      *              *-------------------------------------------------*
           move      "PS"                 to   s-ope                  .
           move      "ads-ssm"            to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-alf                to   w-ref-prm-inv-ads      .
       ref-prm-inv-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione dei valori letti                *
      *              *-------------------------------------------------*
           if        w-ref-prm-inv-ads    =    spaces
                     move  "t_sender"     to   w-ref-prm-inv-prm (1)
           else      move  w-ref-prm-inv-ads
                                          to   w-ref-prm-inv-prm (1)  .
           move      w-ref-prm-inv-azi    to   w-ref-prm-inv-prm (2)  .
           move      w-ref-prm-inv-dms    to   w-ref-prm-inv-prm (3)  .
           move      w-ref-prm-inv-dpm    to   w-ref-prm-inv-prm (4)  .
           move      w-ref-prm-inv-usr    to   w-ref-prm-inv-prm (5)  .
           move      w-ref-prm-inv-pwd    to   w-ref-prm-inv-prm (6)  .
       ref-prm-inv-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *                  * Elenco destinatari                          *
      *                  *---------------------------------------------*
           perform   acc-eln-dst-000      thru acc-eln-dst-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * E-mail mittente                             *
      *                  *---------------------------------------------*
           perform   acc-eml-mit-000      thru acc-eml-mit-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "------------------------------ Parametri per l'inv
      -              "io ---------------------------"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Prompt elenco destinatari                       *
      *              *-------------------------------------------------*
           perform   pmt-eln-dst-000      thru pmt-eln-dst-999        .
      *              *-------------------------------------------------*
      *              * Prompt oggetto del messaggio                    *
      *              *-------------------------------------------------*
           perform   pmt-ogg-msg-000      thru pmt-ogg-msg-999        .
      *              *-------------------------------------------------*
      *              * Prompt testo del messaggio                      *
      *              *-------------------------------------------------*
           perform   pmt-txt-msg-000      thru pmt-txt-msg-999        .
      *              *-------------------------------------------------*
      *              * Prompt allegato                                 *
      *              *-------------------------------------------------*
           perform   pmt-pth-all-000      thru pmt-pth-all-999        .
      *              *-------------------------------------------------*
      *              * Prompt e-mail mittente                          *
      *              *-------------------------------------------------*
           perform   pmt-eml-mit-000      thru pmt-eml-mit-999        .
      *              *-------------------------------------------------*
      *              * Esclusione documenti                            *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
      ...
      *    *-----------------------------------------------------------*
       pmt-eln-dst-000.
           ...
           move      12                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Elenco destinatari su file :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-eln-dst-999.
      ...
      *    *-----------------------------------------------------------*
       pmt-pth-all-000.
           ...
           move      16                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Documento PDF da allegare  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-pth-all-999.
      ...
      *    *-----------------------------------------------------------*
       pmt-eml-mit-000.
           ...
           move      18                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Indirizzo e-mail mittente  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-eml-mit-999.
      ...
      *              *-------------------------------------------------*
           ...
           display "PROGRAMMA IN LAVORAZIONE !!! (Digitare 2 spazi per u
      -            "scire)" line 05 position 01.
      ...
      *    *===========================================================*
      *    * Accettazione : Elenco destinatari                         *
      *    *-----------------------------------------------------------*
       acc-eln-dst-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-tip-ope           not  = 01
                     go to acc-eln-dst-999.
      *                  *---------------------------------------------*
      *                  * Eventuale default                           *
      *                  * ___ /abd/asc/master/exp/siri_dcc402.TXT     *
      *                  *---------------------------------------------*
       acc-eln-dst-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      ...
      *                  * sia in Up                                   *
      *                  *---------------------------------------------*
           if        rr-eln-dst           not  = spaces
                     go to acc-eln-dst-450.
           if        v-key                =    "UP  "
                     go to acc-eln-dst-600
           else      go to acc-eln-dst-100.
       acc-eln-dst-450.
      *                  *---------------------------------------------*
      *                  * Che il file esista                          *
      *                  *---------------------------------------------*
           ...
           if        g-sts                =    e-not-err
                     go to acc-eln-dst-456
           else      go to acc-eln-dst-458.
       acc-eln-dst-456.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           go to     acc-eln-dst-500.
       acc-eln-dst-458.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           move      "Non esiste nessun file corrispondente al percorso 
      -              "impostato !    "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           go to     acc-eln-dst-100.
       acc-eln-dst-500.
      *                  *---------------------------------------------*
      *                  * Fine controlli                              *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Elenco destinatari                      *
      *    *-----------------------------------------------------------*
       vis-eln-dst-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Oggetto del messaggio                      *
      *    *-----------------------------------------------------------*
       acc-ogg-msg-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-ogg-msg           not  = spaces
                     go to acc-ogg-msg-500.
           if        v-key                =    "UP  "
                     go to acc-ogg-msg-600
           else      go to acc-ogg-msg-100.
       acc-ogg-msg-500.
      *                  *---------------------------------------------*
      *                  * Fine controlli                              *
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Oggetto del messaggio                   *
      *    *-----------------------------------------------------------*
       vis-ogg-msg-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Testo del messaggio                        *
      *    *-----------------------------------------------------------*
       acc-txt-msg-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * Che il file esista                          *
      *                  *---------------------------------------------*
           move      rr-txt-msg           to   g-pat                  .
           perform   opn-fil-seq-000      thru opn-fil-seq-999        .
           if        g-sts                =    e-not-err
                     go to acc-txt-msg-456
           else      go to acc-txt-msg-458.
       acc-txt-msg-456.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           go to     acc-txt-msg-500.
       acc-txt-msg-458.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           move      "Non esiste nessun file corrispondente al percorso 
      -              "impostato !    "    to   w-err-box-err-msg      .
      ...
      *    *===========================================================*
      *    * Visualizzazione : Testo del messaggio                     *
      *    *-----------------------------------------------------------*
       vis-txt-msg-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           if        rr-pth-all           not  = spaces
                     go to acc-pth-all-450
           else      go to acc-pth-all-600.
       acc-pth-all-450.
      *                  *---------------------------------------------*
      *                  * Che il file esista                          *
      *                  *---------------------------------------------*
           move      rr-pth-all           to   g-pat                  .
           perform   opn-fil-seq-000      thru opn-fil-seq-999        .
           if        g-sts                =    e-not-err
                     go to acc-pth-all-456
           else      go to acc-pth-all-458.
       acc-pth-all-456.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           go to     acc-pth-all-500.
       acc-pth-all-458.
           perform   cls-fil-seq-000      thru cls-fil-seq-999        .
           move      "Non esiste nessun file corrispondente al percorso 
      -              "impostato !    "    to   w-err-box-err-msg      .
      ...
      *                  *---------------------------------------------*
           if        rr-tip-ope           not  = 01
                     go to acc-eml-mit-999.
      *                  *---------------------------------------------*
      *                  * Eventuale default                           *
      *                  *---------------------------------------------*
           if        rr-eml-mit           not  = spaces
                     go to acc-eml-mit-100.
           move      w-ref-prm-inv-prm (4)     to   rr-eml-mit        .
       acc-eml-mit-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-eml-mit           not  = spaces
                     go to acc-eml-mit-500.
           if        v-key                =    "UP  "
                     go to acc-eml-mit-600
           else      go to acc-eml-mit-100.
       acc-eml-mit-500.
      *                  *---------------------------------------------*
      *                  * Che contenga solo 1 '@'                     *
      *                  *---------------------------------------------*
           ...
           inspect   rr-eml-mit
                                    tallying   w-all-str-num
                                          for  all "@"                .
           if        w-all-str-num        =    1
                     go to acc-eml-mit-526
           else if   w-all-str-num        =    0
                     go to acc-eml-mit-522
           else      go to acc-eml-mit-524.
       acc-eml-mit-522.
      *                  *---------------------------------------------*
      *                  * Messaggio di errore                         *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      rr-eml-mit           to   w-all-str-alf          .
      ...
      *    *===========================================================*
      *    * Visualizzazione : E-mail mittente                         *
      *    *-----------------------------------------------------------*
       vis-eml-mit-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Routine di lettura archivio [dea]                         *
      *    *-----------------------------------------------------------*
       let-arc-dst-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-let-arc-dst-prt    to   rf-dea-num-dst         .
           move      "pgm/azi/fls/ioc/obj/iofdea"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
       ...
       let-arc-dst-300.
      *              *-------------------------------------------------*
      *              * Max su [dea], se non superato : ad uscita       *
      *              *-------------------------------------------------*
           if        rf-dea-num-dst       not  = w-let-arc-dst-prt
                     go to let-arc-dst-900.
       let-arc-dst-400.
      *              *-------------------------------------------------*
      *              * Preparazione flag di uscita                     *
