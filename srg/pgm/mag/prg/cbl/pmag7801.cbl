      ...
      *                       Ultima revisione:    NdK del 23/11/18    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pmag7800 *
      *                                                                *
      ...
      *        * Costo di comodo                                       *
      *        *-------------------------------------------------------*
           05  w-lif-sch-lif-cmp          pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Contatori di comodo                                   *
      *        *-------------------------------------------------------*
           05  w-lif-sch-lif-ctr          pic  9(03)                  .
           05  w-lif-sch-lif-c01          pic  9(03)                  .
           05  w-lif-sch-lif-c02          pic  9(03)                  .
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun prodotto da aggiornare entro i limiti asseg
      -              "nati !"             to   m-msg                  .
      ...
      *              *-------------------------------------------------*
       ...
       prn-sel-rec-120.
      *                  *---------------------------------------------*
      *                  * Se scheda non trovata : selezione non supe- *
      *                  * rata                                        *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
       ...
       prn-sel-rec-200.
      *              *-------------------------------------------------*
      *              * Test se presenti scaglioni con rimanenza finale *
      *              * priva di costo medio ponderato                  *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Ciclo di controllo                          *
      *                  *---------------------------------------------*
           move      zero                 to   w-lif-sch-lif-ctr      .
       prn-sel-rec-220.
           add       1                    to   w-lif-sch-lif-ctr      .
           if        w-lif-sch-lif-ctr    >    9
                     go to prn-sel-rec-900.
      *                  *---------------------------------------------*
      *                  * Test su rimanenza finale                    *
      *                  *---------------------------------------------*
           if        rf-mmv-rim-pre
                    (w-lif-sch-lif-ctr)   =    zero
                     go to prn-sel-rec-220.
      *                  *---------------------------------------------*
      *                  * Test su costo medio ponderato               *
      *                  *---------------------------------------------*
           if        rf-mmv-ump-pre
                    (w-lif-sch-lif-ctr)   not  = zero
                     go to prn-sel-rec-220.
       prn-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-mmv                 .
      *                      *-----------------------------------------*
      *                      * Se record non ottenuto, ad uscita       *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *                  *---------------------------------------------*
           move      zero                 to   w-lif-sch-lif-cmp      .
           move      11                   to   w-lif-sch-lif-c01      .
       prn-liv-det-220.
           subtract  1                    from w-lif-sch-lif-c01      .
           if        w-lif-sch-lif-c01    not  > zero
      ...
      *                  * Soppresso                                   *
      *                  *---------------------------------------------*
______*    if        rf-mmv-rim-pre
______*             (w-lif-sch-lif-c01)   =    zero
______*              go to prn-liv-det-220.
      *                  *---------------------------------------------*
      *                  * Test su costo medio ponderato               *
      *                  *---------------------------------------------*
           if        rf-mmv-ump-pre
                    (w-lif-sch-lif-c01)   not  = zero
                     move  rf-mmv-ump-pre
                          (w-lif-sch-lif-c01)
                                          to   w-lif-sch-lif-cmp
                     go to prn-liv-det-220.
      *                  *---------------------------------------------*
      *                  * Scorrimento di un elemento                  *
      *                  *---------------------------------------------*
           if        w-lif-sch-lif-cmp    =    zero
                     go to prn-liv-det-220.
           move      w-lif-sch-lif-cmp    to   rf-mmv-ump-pre
                                              (w-lif-sch-lif-c01)     .
      *                  *---------------------------------------------*
      *                  * Riciclo                                     *
      *                  *---------------------------------------------*
