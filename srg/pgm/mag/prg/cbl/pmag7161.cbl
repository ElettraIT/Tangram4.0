      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pmag7160 *
      *                                                                *
      *                    Stampa schede L.I.F.O. materiali vari       *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " STAMPA SCHEDE L.I.F.O. MATERALI VARI   "       .
      ...
      *    *===========================================================*
      *    * Work area per stampa scheda L.I.F.O.                      *
      *    *-----------------------------------------------------------*
       01  w-stp-sch-lif.
      *        *-------------------------------------------------------*
      *        * Contatori di comodo                                   *
      *        *-------------------------------------------------------*
           05  w-stp-sch-lif-ctr          pic  9(03)                  .
           05  w-stp-sch-lif-c01          pic  9(03)                  .
           05  w-stp-sch-lif-c02          pic  9(03)                  .
      *        *-------------------------------------------------------*
      *        * Segnale di almeno una linea stampata                  *
      *        *-------------------------------------------------------*
           05  w-stp-sch-lif-st1          pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Castelletto da 11 elementi                            *
      *        *-------------------------------------------------------*
           05  w-stp-sch-lif-cst.
      *            *---------------------------------------------------*
      *            * 11 elementi                                       *
      *            *---------------------------------------------------*
               10  w-stp-sch-lif-pre occurs 11.
      *                *-----------------------------------------------*
      *                * Anno                                          *
      *                *-----------------------------------------------*
                   15  w-stp-sch-lif-ann  pic  9(04)                  .
      *                *-----------------------------------------------*
      *                * Rimanenza finale                              *
      *                *-----------------------------------------------*
                   15  w-stp-sch-lif-rim  pic s9(08)v9(03)            .
      *                *-----------------------------------------------*
      *                * Costo medio ponderato                         *
      *                *-----------------------------------------------*
                   15  w-stp-sch-lif-ump  pic  9(09)v9(02)            .
      *                *-----------------------------------------------*
      *                * Valore normale del bene                       *
      *                *-----------------------------------------------*
                   15  w-stp-sch-lif-vnb  pic  9(09)v9(02)            .
      *                *-----------------------------------------------*
      *                * Flag di rivalutazione                         *
      *                * - Spaces : No                                 *
      *                * - S      : Si                                 *
      *                *-----------------------------------------------*
                   15  w-stp-sch-lif-fdr  pic  x(01)                  .
      ...
      *              *-------------------------------------------------*
       ...
       prn-sel-rec-020.
      *              *-------------------------------------------------*
      *              * Normalizzazione preventiva del castelletto da   *
      *              * 11 elementi per il L.I.F.O.                     *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-sch-lif-ctr      .
       prn-sel-rec-022.
           add       1                    to   w-stp-sch-lif-ctr      .
           if        w-stp-sch-lif-ctr    >    11
                     go to prn-sel-rec-100.
           move      zero                 to   w-stp-sch-lif-ann
                                              (w-stp-sch-lif-ctr)     .
           move      zero                 to   w-stp-sch-lif-rim
                                              (w-stp-sch-lif-ctr)     .
           move      zero                 to   w-stp-sch-lif-ump
                                              (w-stp-sch-lif-ctr)     .
           move      zero                 to   w-stp-sch-lif-vnb
                                              (w-stp-sch-lif-ctr)     .
           move      spaces               to   w-stp-sch-lif-fdr
                                              (w-stp-sch-lif-ctr)     .
           go to     prn-sel-rec-022.
       prn-sel-rec-100.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-mmv                 .
      *              *-------------------------------------------------*
      *              * Lettura della scheda L.I.F.O. relativa          *
      *              *-------------------------------------------------*
           move      "RK"                 to   f-ope                  .
       ...
       prn-sel-rec-120.
      *              *-------------------------------------------------*
      *              * Bufferizzazione eventuale della scheda          *
      *              *-------------------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *                  * 1. elemento                                 *
      *                  *---------------------------------------------*
           move      rr-ann-ese           to   w-stp-sch-lif-ann (1)  .
           add       1900                 to   w-stp-sch-lif-ann (1)  .
           move      rf-mmv-vnb-gea       to   w-stp-sch-lif-vnb (1)  .
           move      rf-mmv-fdr-gea       to   w-stp-sch-lif-fdr (1)  .
      *                  *---------------------------------------------*
      *                  * n. elementi da 2 a 11                       *
      *                  *---------------------------------------------*
           move      1                    to   w-stp-sch-lif-ctr      .
       prn-sel-rec-122.
           add       1                    to   w-stp-sch-lif-ctr      .
           if        w-stp-sch-lif-ctr    >    11
                     go to prn-sel-rec-200.
           move      w-stp-sch-lif-ctr    to   w-stp-sch-lif-c01      .
           subtract  1                    from w-stp-sch-lif-c01      .
      *                      *-----------------------------------------*
      *                      * Anno                                    *
      *                      *-----------------------------------------*
           move      rr-ann-ese           to   w-stp-sch-lif-ann
                                              (w-stp-sch-lif-ctr)     .
           add       1900                 to   w-stp-sch-lif-ann
                                              (w-stp-sch-lif-ctr)     .
           subtract  w-stp-sch-lif-c01    from w-stp-sch-lif-ann
                                              (w-stp-sch-lif-ctr)     .
      *                      *-----------------------------------------*
      *                      * Rimanenza finale                        *
      *                      *-----------------------------------------*
           move      rf-mmv-rim-pre
                    (w-stp-sch-lif-c01)   to   w-stp-sch-lif-rim
                                              (w-stp-sch-lif-ctr)     .
      *                      *-----------------------------------------*
      *                      * Costo medio ponderato                   *
      *                      *-----------------------------------------*
           move      rf-mmv-ump-pre
                    (w-stp-sch-lif-c01)   to   w-stp-sch-lif-ump
                                              (w-stp-sch-lif-ctr)     .
      *                      *-----------------------------------------*
      *                      * Valore normale del bene                 *
      *                      *-----------------------------------------*
           move      rf-mmv-vnb-pre
                    (w-stp-sch-lif-c01)   to   w-stp-sch-lif-vnb
                                              (w-stp-sch-lif-ctr)     .
      *                      *-----------------------------------------*
      *                      * Flag di rivalutazione                   *
      *                      *-----------------------------------------*
           move      rf-mmv-fdr-pre
                    (w-stp-sch-lif-c01)   to   w-stp-sch-lif-fdr
                                              (w-stp-sch-lif-ctr)     .
      *                      *-----------------------------------------*
      *                      * Riciclo                                 *
      *                      *-----------------------------------------*
       ...
       prn-sel-rec-200.
      *              *-------------------------------------------------*
      *              * Test per selezione sulle schede                 *
      *              *-------------------------------------------------*
           if        rr-sel-lif           =    01
                     go to prn-sel-rec-800
           else if   rr-sel-lif           =    02
                     go to prn-sel-rec-220
           else if   rr-sel-lif           =    03
                     go to prn-sel-rec-240
           else      go to prn-sel-rec-800.
       prn-sel-rec-220.
      *                  *---------------------------------------------*
      *                  * Se solo le schede esistenti                 *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *              *-------------------------------------------------*
      *              * Se tipo ordinamento :                           *
      *              * - Per classe, gruppo, sottogruppo, codice mat-  *
      *              *   eria varia                                    *
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Stampa eventuale scheda L.I.F.O.                *
      *              *-------------------------------------------------*
           perform   stp-sch-lif-000      thru stp-sch-lif-999        .
       prn-liv-det-999.
           exit.

      *    *===========================================================*
      *    * Stampa scheda L.I.F.O.                                    *
      *    *-----------------------------------------------------------*
       stp-sch-lif-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione segnale di almeno una linea     *
      *              * stampata                                        *
      *              *-------------------------------------------------*
           move      spaces               to   w-stp-sch-lif-st1      .
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore elementi stampati     *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-sch-lif-c02      .
      *              *-------------------------------------------------*
      *              * Ciclo di stampa per 11 elementi                 *
      *              *-------------------------------------------------*
           move      zero                 to   w-stp-sch-lif-ctr      .
       stp-sch-lif-100.
           add       1                    to   w-stp-sch-lif-ctr      .
           if        w-stp-sch-lif-ctr    >    11
                     go to stp-sch-lif-900.
       stp-sch-lif-120.
      *              *-------------------------------------------------*
      *              * Test se linea da stampare                       *
      *              *-------------------------------------------------*
           if        w-stp-sch-lif-rim
                    (w-stp-sch-lif-ctr)   =    zero  and
                     w-stp-sch-lif-ump
                    (w-stp-sch-lif-ctr)   =    zero  and
                     w-stp-sch-lif-vnb
                    (w-stp-sch-lif-ctr)   =    zero
                     go to stp-sch-lif-600.
       stp-sch-lif-140.
      *              *-------------------------------------------------*
      *              * Test se prima linea stampata                    *
      *              *-------------------------------------------------*
           if        w-stp-sch-lif-st1    =    spaces
                     move  "#"            to   w-stp-sch-lif-st1
                     go to stp-sch-lif-300.
       stp-sch-lif-200.
      *              *-------------------------------------------------*
      *              * Test se righe residue sufficienti               *
      *              *-------------------------------------------------*
           if        p-res                >    1
                     go to stp-sch-lif-220.
      *                  *---------------------------------------------*
      *                  * Intestazione pagina                         *
      *                  *---------------------------------------------*
           perform   int-pag-sta-000      thru int-pag-sta-999        .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Incremento contatore elementi stampati      *
      *                  *---------------------------------------------*
           add       1                    to   w-stp-sch-lif-c02      .
      *                  *---------------------------------------------*
      *                  * Anno                                        *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           ...
           move      p-lnr                to   p-lin                  .
           move      065                  to   p-pos                  .
           move      w-stp-sch-lif-ann
                    (w-stp-sch-lif-ctr)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Rimanenza finale                            *
      *                  *---------------------------------------------*
           ...
           move      "N"                  to   p-tip                  .
           move      08                   to   p-car                  .
           move      rf-mtv-dec-qta       to   p-dec                  .
           move      "S"                  to   p-sgn                  .
           move      "GD"                 to   p-edm                  .
           move      p-lnr                to   p-lin                  .
           move      072                  to   p-pos                  .
           move      w-stp-sch-lif-rim
                    (w-stp-sch-lif-ctr)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      128                  to   p-pos                  .
           move      w-stp-sch-lif-fdr
                    (w-stp-sch-lif-ctr)   to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-sch-lif-600.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
           go to     stp-sch-lif-100.
       stp-sch-lif-900.
      *              *-------------------------------------------------*
      *              * Eventuale sottolinetura di separazione          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se stampata piu' di una linea          *
      *                  *---------------------------------------------*
           if        w-stp-sch-lif-c02    not  > 2
                     go to stp-sch-lif-920.
      *                  *---------------------------------------------*
      *                  * Test se righe residue sufficienti           *
      *                  *---------------------------------------------*
           if        p-res                >    1
                     go to stp-sch-lif-910.
      *                  *---------------------------------------------*
      *                  * Intestazione pagina                         *
      *                  *---------------------------------------------*
           perform   int-pag-sta-000      thru int-pag-sta-999        .
      ...
      *                  *---------------------------------------------*
      *                  * Editing dell'anno di esercizio              *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      04                   to   p-car                  .
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Composizione descrizione per la stampa      *
      *                  *---------------------------------------------*
           string    "SCHEDE L.I.F.O. MATERIALI VARI RELATIVE AL "
                                delimited by   size
