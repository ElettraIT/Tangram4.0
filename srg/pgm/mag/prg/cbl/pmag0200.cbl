      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella conti merce                *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-aep occurs 2.
               10  w-tes-des-ctm          pic  x(20)                  .
               10  w-tes-des-key          pic  x(20)                  .
               10  w-tes-com-ctm          pic  x(40)                  .
               10  w-tes-alx-exp.
      ...
      *              *-------------------------------------------------*
           ...
           move      04                   to   w-cod-des-zmm-lin      .
           move      30                   to   w-cod-des-zmm-pos      .
           move      07                   to   w-cod-des-zmm-dln      .
           move      30                   to   w-cod-des-zmm-dps      .
      ...
      *                  *---------------------------------------------*
           perform   acc-des-ctm-000      thru acc-des-ctm-999        .
           if        v-key                =    "NXSC"
      ...
      *                  * Commento                                    *
      *                  *---------------------------------------------*
           perform   acc-com-ctm-000      thru acc-com-ctm-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *              * Descrizione conto merce                         *
      *              *-------------------------------------------------*
           perform   vis-des-ctm-000      thru vis-des-ctm-999        .
      *              *-------------------------------------------------*
      *              * Commento                                        *
      *              *-------------------------------------------------*
           perform   vis-com-ctm-000      thru vis-com-ctm-999        .
           go to     vis-tes-reg-999.
       vis-tes-reg-999.
           exit.
      ...
      *              * Descrizione conto merce                         *
      *              *-------------------------------------------------*
           perform   pmt-des-ctm-000      thru pmt-des-ctm-999        .
      *              *-------------------------------------------------*
      *              * Commento                                        *
      *              *-------------------------------------------------*
           perform   pmt-com-ctm-000      thru pmt-com-ctm-999        .
           go to     pmt-tes-reg-999.
       pmt-tes-reg-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione prompt : Descrizione conto merce          *
      *    *-----------------------------------------------------------*
       pmt-des-ctm-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo testata : Descrizione conto merce      *
      *    *-----------------------------------------------------------*
       acc-des-ctm-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * l'ordinamento                               *
      *                  *---------------------------------------------*
           move      w-tes-des-ctm (1)    to   w-all-str-alf          .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-nok-flg
                             ...
                             go to acc-des-ctm-100.
       acc-des-ctm-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo testata : Descrizione conto merce   *
      *    *-----------------------------------------------------------*
       vis-des-ctm-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           move      spaces               to   w-tes-des-ctm (1)      .
           move      spaces               to   w-tes-des-key (1)      .
           move      spaces               to   w-tes-com-ctm (1)      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-ctm        to   rf-zmm-cod-ctm         .
           move      "pgm/mag/fls/ioc/obj/iofzmm"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [zmm]                        *
      *                          *-------------------------------------*
           move      rf-zmm-des-ctm       to   w-tes-des-ctm (1)      .
           move      rf-zmm-des-key       to   w-tes-des-key (1)      .
           move      rf-zmm-com-ctm       to   w-tes-com-ctm (1)      .
           move      rf-zmm-alx-exp       to   w-tes-alx-exp (1)      .
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zmm]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zmm-000      thru wrt-rec-zmm-999        .
      ...
      *                      * Rewrite record [zmm]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zmm-000      thru rew-rec-zmm-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zmm]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zmm-000      thru del-rec-zmm-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zmm]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zmm-000.
      *              *-------------------------------------------------*
      ...
      *                  * Campi non chiave                            *
      *                  *---------------------------------------------*
           move      w-tes-des-ctm (1)    to   rf-zmm-des-ctm         .
           move      w-tes-des-key (1)    to   rf-zmm-des-key         .
           move      w-tes-com-ctm (1)    to   rf-zmm-com-ctm         .
           move      w-tes-alx-exp (1)    to   rf-zmm-alx-exp         .
       cmp-rec-zmm-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zmm]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zmm-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zmm-000      thru cmp-rec-zmm-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
