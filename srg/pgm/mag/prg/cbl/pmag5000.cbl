      ...
      *                       Ultima revisione:    NdK del 24/04/09    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampa situazioni prodotti in conto merce   *
      *                                                                *
      *                    Main                                        *
      *                                                                *
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase mag500, nella sua globalita', esegue i tipi di stampa  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * STA500      Prodotti presso clienti per articolo      pmag500a *
      * STA502      Prodotti presso clienti per cliente       pmag500b *
      *                                                                *
      * STA505      Prodotti presso fornitori per articolo    pmag500c *
      * STA507      Prodotti presso fornitori per fornitore   pmag500d *
      *                                                                *
      * STA510      Prodotti di clienti per articolo          pmag500e *
      * STA512      Prodotti di clienti per cliente           pmag500f *
      *                                                                *
      * STA515      Prodotti di fornitori per articolo        pmag500g *
      * STA517      Prodotti di fornitori per fornitore       pmag500h *
      *                                                                *
      * STA550      Prodotti presso agenti per articolo       pmag500m *
      * STA552      Prodotti presso agenti per agente         pmag500n *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  SITUAZIONI MERCI IN CONTO - PRODOTTI  "       .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'mag500'           *
      *              *-------------------------------------------------*
           perform   fas-mag-500-000      thru fas-mag-500-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "presso Clienti per prodotto                       
      -              " STA500     pmag500a  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "presso Clienti per cliente                        
      -              " STA502     pmag500b  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "presso Fornitori per prodotto                     
      -              " STA505     pmag500c  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "presso Fornitori per fornitore                    
      -              " STA507     pmag500d  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "di Clienti per prodotto                           
      -              " STA510     pmag500e  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "di Clienti per cliente                            
      -              " STA512     pmag500f  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "di Fornitori per prodotto                         
      -              " STA515     pmag500g  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "di Fornitori per fornitore                        
      -              " STA517     pmag500h  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
      ...
      *
           move      "presso Agenti per prodotto                        
      -              " STA550     pmag500m  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *
           move      "presso Agenti per agente                          
      -              " STA552     pmag500n  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-200.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-mag-500-300
           else if   v-key                =    "PRSC"
                     go to fas-mag-500-400
           else if   v-key                =    "NXSC"
                     go to fas-mag-500-450
           else if   v-key                =    "DOWN"
                     go to fas-mag-500-500
           else if   v-key                =    "EXIT"
                     go to fas-mag-500-250
           else      go to fas-mag-500-200.
       fas-mag-500-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-mag-500-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-mag-500-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-num
                     go to fas-mag-500-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-mag-500-350
           else      go to fas-mag-500-322.
       fas-mag-500-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-mag-500-630
           else if   v-key                =    "PRSC"
                     go to fas-mag-500-600
           else if   v-key                =    "NXSC"
                     go to fas-mag-500-610
           else if   v-key                =    "UP  "
                     go to fas-mag-500-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-mag-500-650
           else if   v-key                =    "EXIT"
                     go to fas-mag-500-660
           else      go to fas-mag-500-570.
       fas-mag-500-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-mag-500-626
           else      go to fas-mag-500-628.
       fas-mag-500-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-mag-500-636
           else      go to fas-mag-500-638.
       fas-mag-500-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-mag-500-725
           else      go to fas-mag-500-750.
       fas-mag-500-725.
      *              *-------------------------------------------------*
      *              * Se variabile esistente                          *
      *              *-------------------------------------------------*
