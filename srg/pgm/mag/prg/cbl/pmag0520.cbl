      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pmag0521:        *
      *                                                                *
      *                    Stampa tabella ubicazioni di magazzino      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA PARAMETRI DI UBICAZIONE MAGAZZINO"       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           ...
           05  rr-cod-min                 pic  x(07)                  .
           05  rr-cod-max                 pic  x(07)                  .
           05  rr-des-min                 pic  x(30)                  .
           05  rr-des-max                 pic  x(30)                  .
           05  rr-inx-min                 pic  9(07)                  .
           05  rr-inx-max                 pic  9(07)                  .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-600.
      *                  *---------------------------------------------*
      *                  * Indice di percorso min                      *
      *                  *---------------------------------------------*
           perform   acc-inx-min-000      thru acc-inx-min-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
       ...
       acc-ric-sel-700.
      *                  *---------------------------------------------*
      *                  * Indice di percorso max                      *
      *                  *---------------------------------------------*
           perform   acc-inx-max-000      thru acc-inx-max-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su tabella ubicazioni  "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-des-zmd-cod      .
           move      12                   to   w-cod-des-zmd-lin      .
           move      18                   to   w-cod-des-zmd-pos      .
           move      zero                 to   w-cod-des-zmd-dln      .
           move      zero                 to   w-cod-des-zmd-dps      .
           move      spaces               to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-des-zmd-ope      .
           move      rr-cod-max           to   w-cod-des-zmd-cod      .
           move      12                   to   w-cod-des-zmd-lin      .
           move      51                   to   w-cod-des-zmd-pos      .
           move      zero                 to   w-cod-des-zmd-dln      .
           move      zero                 to   w-cod-des-zmd-dps      .
           move      spaces               to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Indice di percorso min               *
      *    *-----------------------------------------------------------*
       acc-inx-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-inx-min-999.
       acc-inx-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-inx-min             .
       acc-inx-min-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-inx-min-100.
       acc-inx-min-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo : Indice di percorso max               *
      *    *-----------------------------------------------------------*
       acc-inx-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-inx-max-999.
       acc-inx-max-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-inx-max             .
       acc-inx-max-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        rr-inx-max           =    zero
                     go to tdo-ric-sel-999.
           if        rr-inx-max           not  < rr-cod-min
                     go to tdo-ric-sel-999.
           move      "ME"                 to   v-ope                  .
           move      "L'indice massimo non puo' essere inferiore all'ind
      -              "ice minimo !"       to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
      ...
      *              *-------------------------------------------------*
      *              * Regolarizzazione indice min - max               *
      *              *-------------------------------------------------*
           if        rr-inx-max           not  = zero
                     go to reg-ric-sel-999.
           if        rr-inx-min           =    zero
                     move  9999999        to   rr-inx-max
           else      move  rr-inx-min     to   rr-inx-max             .
       reg-ric-sel-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      zero                 to   rr-inx-min             .
           move      zero                 to   rr-inx-max             .
       nor-ric-sel-999.
           exit.
