      ...
      *                                   Fase:    mag301              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 24/03/92    *
      *                       Ultima revisione:    NdK del 17/04/10    *
      *                    ------------------------------------------- *
      ...
      * -------------------------------------------------------------- *
      *                                                                *
      * La fase mag301, nella sua globalita', esegue i tipi di inter-  *
      ...
      * ----------  -------------------------------------------------- *
      *                                                                *
      * INTMIC      Ricerca movimenti intestati ad un cliente          *
      *                                                                *
      * INTMIF      Ricerca movimenti intestati ad un fornitore        *
      *                                                                *
      * INTMID      Ricerca movimenti intestati ad una ns. dipendenza  *
      *                                                                *
      * INTMIA      Ricerca movimenti intestati ad un agente           *
      *                                                                *
      * INTMDR      Ricerca movimenti per data di registrazione        *
      *                                                                *
      * INTMPV      Ricerca movimenti per prodotto di vendita          *
      *                                                                *
      * INTMSL      Ricerca movimenti per semilavorato                 *
      *                                                                *
      * INTMMP      Ricerca movimenti per materia prima                *
      *                                                                *
      * INTMMV      Ricerca movimenti per materiale vario              *
      *                                                                *
      * DSPPDV      Quadro disponibilita' prodotto di vendita          *
      *                                                                *
      * DSPSEM      Quadro disponibilita' semilavorato                 *
      *                                                                *
      * DSPMAP      Quadro disponibilita' materia prima                *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   RICERCHE PER MOVIMENTI DI MAGAZZINO  "       .
      ...
      *        *-------------------------------------------------------*
      *        * Per chiamata da un'altra fase                         *
      *        *-------------------------------------------------------*
           05  w-ipc-alt-fas.
      *            *---------------------------------------------------*
      *            * Valore della variabile di i.p.c. relativa alla    *
      *            * chiamata da altra fase gestionale                 *
      *            * - S : Si, chiamata da altra fase                  *
      *            * - N : No                                          *
      *            *---------------------------------------------------*
               10  w-ipc-alt-fas-snx      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Valore della variabile di i.p.c. relativa alla    *
      *            * fase che ha operato la chiamata                   *
      *            *---------------------------------------------------*
               10  w-ipc-alt-fas-fas      pic  x(06)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Si/No gestione materie varie attiva                   *
      *        *-------------------------------------------------------*
           05  w-prs-mtv-snx              pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/No gestione tentata vendita attiva                 *
      *        *-------------------------------------------------------*
           05  w-prs-gtv-snx              pic  x(01)                  .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-pre-exe-pgm    not  = spaces
                     go to main-900.
      *              *-------------------------------------------------*
      *              * Esecuzione ciclo per la fase 'mag301'           *
      *              *-------------------------------------------------*
           perform   fas-mag-301-000      thru fas-mag-301-999        .
       main-900.
      *              *-------------------------------------------------*
      ...
      *              * l'eventuale chiamata da un'altra fase           *
      *              *                                                 *
      *              * N.B.: Viene effettuata a questo livello per po- *
      *              *       ter consentire di manipolare il nome del- *
      *              *       la fase del programma per superare even-  *
      *              *       tuali ostacoli dovuti agli accessi.       *
      *              *-------------------------------------------------*
           perform   ipc-alt-fas-000      thru ipc-alt-fas-999        .
      *              *-------------------------------------------------*
      *              * Sigla funzione                                  *
      *              *-------------------------------------------------*
           move      "P+"                 to   s-ope                  .
      ...
      *                  *---------------------------------------------*
      *                  * Se il programma e' chiamato si pone come    *
      *                  * fase il nome del chiamante                  *
      *                  *---------------------------------------------*
           if        w-ipc-alt-fas-snx    =    "S"
                     move  w-ipc-alt-fas-fas
                                          to   s-fas                  .
      *              *-------------------------------------------------*
      *              * Sigla interna del programma                     *
      *              *-------------------------------------------------*
      ...
      *                  * Si/No gestione tentata vendita attiva       *
      *                  *---------------------------------------------*
           perform   prs-gtv-snx-000      thru prs-gtv-snx-999        .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Lettura della variabile eventuale di i.p.c. per la        *
      *    * chiamata da altra fase                                    *
      *    *-----------------------------------------------------------*
       ipc-alt-fas-000.
      *              *-------------------------------------------------*
      *              * Estrazione della variabile 'alt-fas' dallo      *
      *              * stesso livello                                  *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
           move      "alt-fas"            to   s-var                  .
           move      "="                  to   s-dop                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to ipc-alt-fas-200
           else      go to ipc-alt-fas-400.
       ipc-alt-fas-200.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      s-alf                to   w-ipc-alt-fas-snx      .
      *                  *---------------------------------------------*
      *                  * Normalizzazione della variabile             *
      *                  *---------------------------------------------*
           if        w-ipc-alt-fas-snx    not  = "S"
                     go to ipc-alt-fas-400.
      *                  *---------------------------------------------*
      *                  * Estrazione della variabile 'ide-fas' per la *
      *                  * fase del chiamante                          *
      *                  *---------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                      *-----------------------------------------*
      *                      * Deviazione a seconda dell'esito del-    *
      *                      * l'operazione                            *
      *                      *-----------------------------------------*
           if        s-ves                =    spaces
                     go to ipc-alt-fas-220
           else      go to ipc-alt-fas-240.
       ipc-alt-fas-220.
      *                          *-------------------------------------*
      *                          * Se variabile esistente              *
      *                          *-------------------------------------*
      ...
      *                              * Valore letto                    *
      *                              *---------------------------------*
           move      s-alf                to   w-ipc-alt-fas-fas      .
      *                              *---------------------------------*
      *                              * Uscita                          *
      *                              *---------------------------------*
           go to     ipc-alt-fas-999.
       ipc-alt-fas-240.
      *                          *-------------------------------------*
      ...
      *                              *---------------------------------*
           move      i-ide-fas            to   w-ipc-alt-fas-fas      .
      *                              *---------------------------------*
      *                              * Uscita                          *
      *                              *---------------------------------*
           go to     ipc-alt-fas-999.
      ...
      *                  *---------------------------------------------*
           move      "N"                  to   w-ipc-alt-fas-snx      .
           move      spaces               to   w-ipc-alt-fas-fas      .
      *                  *---------------------------------------------*
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
           go to     ipc-alt-fas-999.
      ...
      *    *===========================================================*
      *    * Lettura personalizzazione : Si/No gestione materie varie  *
      *    *                             attiva                        *
      *    *-----------------------------------------------------------*
       prs-mtv-snx-000.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Lettura personalizzazione : Si/No gestione tentata ven-   *
      *    *                             dita attiva                   *
      *    *-----------------------------------------------------------*
       prs-gtv-snx-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/azi[snx-gtv]"   to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-gtv-snx
           else      move  spaces         to   w-prs-gtv-snx          .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-gtv-snx        =    "S" or
                     w-prs-gtv-snx        =    "N"
                     go to prs-gtv-snx-999.
           move      "N"                  to   w-prs-gtv-snx          .
       prs-gtv-snx-999.
           exit.
      ...
      *                  *---------------------------------------------*
           if        w-dpz-ctr-dpz        not  > 1
                     go to loa-tbl-tin-102.
           move      "per nostra Dipendenza                             
      -              " INTMID     pmag301c  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-102.
      ...
      *                  * Overlay 'pmag301t'                          *
      *                  *---------------------------------------------*
           if        w-prs-gtv-snx        not  = "S"
                     go to loa-tbl-tin-103.
           move      "per Agente                                        
      -              " INTMIA     pmag301t  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      ...
      *                  * Overlay 'pmag301d'                          *
      *                  *---------------------------------------------*
           move      "per Data di registrazione                         
      -              " INTMDR     pmag301d  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-prs-dps-snx        not  = "S"
                     go to loa-tbl-tin-104.
           move      "per Semilavorato                                  
      -              " INTMSL     pmag301f  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-104.
      ...
      *                  *---------------------------------------------*
           if        w-prs-dpm-snx        not  = "S"
                     go to loa-tbl-tin-106.
           move      "per Materia prima                                 
      -              " INTMMP     pmag301g  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-106.
      ...
      *                  *---------------------------------------------*
           if        w-prs-mtv-snx        not  = "S"
                     go to loa-tbl-tin-108.
           move      "per Materiale vario                               
      -              " INTMMV     pmag301h  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-108.
      ...
      *                  * Overlay 'pmag301m'                          *
      *                  *---------------------------------------------*
           move      "per Disponibilita' Prodotto                       
      -              " DSPPDV     pmag301m  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-110.
      ...
      *                  *---------------------------------------------*
           ...
           move      "per Disponibilita' Semilavorato                   
      -              " DSPSEM     pmag301n  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-120.
      ...
      *                  *---------------------------------------------*
           ...
           move      "per Disponibilita' Materia prima           
      -              " DSPMAP     pmag301o  "
                                          to   w-tin-ele-wci          .
           perform   loa-tbl-ele-000      thru loa-tbl-ele-999        .
       loa-tbl-tin-200.
      ...
      *    *===========================================================*
      *    * Esecuzione ciclo per la fase 'mag300'                     *
      *    *-----------------------------------------------------------*
       fas-mag-301-000.
      *              *-------------------------------------------------*
      *              * Video in Off                                    *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    spaces or
                     v-key                =    "DO  "
                     go to fas-mag-301-300
           else if   v-key                =    "PRSC"
                     go to fas-mag-301-400
           else if   v-key                =    "NXSC"
                     go to fas-mag-301-450
           else if   v-key                =    "DOWN"
                     go to fas-mag-301-500
           else if   v-key                =    "EXIT"
                     go to fas-mag-301-250
           else      go to fas-mag-301-200.
       fas-mag-301-250.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-key-num-int        =    zero
                     go to fas-mag-301-330.
           move      zero                 to   w-tin-ele-inx          .
       fas-mag-301-322.
           add       1                    to   w-tin-ele-inx          .
           if        w-tin-ele-inx        >    w-tin-ele-num
                     go to fas-mag-301-340.
           if        w-tin-num-tin
                    (w-tin-ele-inx)       =    w-key-num-int
                     go to fas-mag-301-350
           else      go to fas-mag-301-322.
       fas-mag-301-330.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        v-key                =    "DOWN"
                     go to fas-mag-301-630
           else if   v-key                =    "PRSC"
                     go to fas-mag-301-600
           else if   v-key                =    "NXSC"
                     go to fas-mag-301-610
           else if   v-key                =    "UP  "
                     go to fas-mag-301-620
           else if   v-key                =    spaces or
                     v-key                =    "DO  " or
                     v-key                =    "SLCT"
                     go to fas-mag-301-650
           else if   v-key                =    "EXIT"
                     go to fas-mag-301-660
           else      go to fas-mag-301-570.
       fas-mag-301-600.
      *                      *-----------------------------------------*
      *                      * Se Prsc                                 *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-pel
                     go to fas-mag-301-626
           else      go to fas-mag-301-628.
       fas-mag-301-626.
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        w-vis-pag-inx-tbl    =    w-vis-pag-inx-uel
                     go to fas-mag-301-636
           else      go to fas-mag-301-638.
       fas-mag-301-636.
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        s-ves                =    spaces
                     go to fas-mag-301-725
           else      go to fas-mag-301-750.
       fas-mag-301-725.
      *              *-------------------------------------------------*
      *              * Se variabile esistente                          *
      *              *-------------------------------------------------*
