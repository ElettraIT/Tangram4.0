      ...
      *                       Ultima revisione:    NdK del 03/04/00    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma dcc182   *
      *                                                                *
      ...
      *    *===========================================================*
      *    * Work area per buffer di precaricamento tabelle            *
      *    *-----------------------------------------------------------*
       01  w-buf.
      *        *-------------------------------------------------------*
      *        * Flag generico per primo giro                          *
      *        *-------------------------------------------------------*
           05  w-buf-csc-flg-uno          pic  x(01)                  . 
      *        *-------------------------------------------------------*
      *        * Per caricamento tabella sconti in riga clienti        *
      *        *-------------------------------------------------------*
           05  w-buf-csc-cli-ctr          pic  9(05)                  . 
           05  w-buf-csc-cli-max          pic  9(05)                  . 
           05  w-buf-csc-cli occurs 2000.
               10  w-buf-csc-cli-cod      pic  9(05)                  .
      *        *-------------------------------------------------------*
      *        * Per caricamento tabella sconti in riga prodotti       *
      *        *-------------------------------------------------------*
           05  w-buf-csc-pro-ctr          pic  9(05)                  . 
           05  w-buf-csc-pro-max          pic  9(05)                  . 
           05  w-buf-csc-pro occurs 2000.
               10  w-buf-csc-pro-cod      pic  9(05)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Bufferizzazione tabella sconti clienti          *
      *              *-------------------------------------------------*
           perform   pre-buf-cli-000      thru pre-buf-cli-999        .
      *              *-------------------------------------------------*
      *              * Bufferizzazione tabella sconti prodotti         *
      *              *-------------------------------------------------*
           perform   pre-buf-pro-000      thru pre-buf-pro-999        .
      *              *-------------------------------------------------*
      *              * Preparazione contatori e flag                   *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-csc-cli-ctr      .
           move      zero                 to   w-buf-csc-pro-ctr      .
           move      "#"                  to   w-buf-csc-flg-uno      .
       prn-str-ini-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessuno sconto entro i limiti assegnati !"
                                          to   m-msg                  .
      ...
      *              * Test se primo giro                              *
      *              *-------------------------------------------------*
           if        w-buf-csc-flg-uno    =    spaces
                     go to prn-let-seq-200.
      *              *-------------------------------------------------*
      *              * Normalizzazione flag di primo giro              *
      *              *-------------------------------------------------*
           move      spaces               to   w-buf-csc-flg-uno      .
       prn-let-seq-100.
      *              *-------------------------------------------------*
      *              * Se primo giro                                   *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Incremento indice tabella primaria          *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    "C"
      ...
      *                      * Se sconti prodotto                      *
      *                      *-----------------------------------------*
           add       1                    to   w-buf-csc-pro-ctr      .
      *                      *-----------------------------------------*
      *                      * Test sul contatore                      *
      *                      *-----------------------------------------*
           if        w-buf-csc-pro-ctr    >    w-buf-csc-pro-max
                     move  "#"            to   w-cnt-prn-flg-sub
                     go to prn-let-seq-999.
      *                      *-----------------------------------------*
      *                      * Azzeramento indice secondario           *
      *                      *-----------------------------------------*
           move      zero                 to   w-buf-csc-cli-ctr      .
      *                      *-----------------------------------------*
      *                      * Ad incremento indice secondario         *
      *                      *-----------------------------------------*
           go to     prn-let-seq-200.
       prn-let-seq-120.
      *                      *-----------------------------------------*
      *                      * Se sconti cliente                       *
      *                      *-----------------------------------------*
           add       1                    to   w-buf-csc-cli-ctr      .
      *                      *-----------------------------------------*
      *                      * Test sul contatore                      *
      *                      *-----------------------------------------*
           if        w-buf-csc-cli-ctr    >    w-buf-csc-cli-max
                     move  "#"            to   w-cnt-prn-flg-sub
                     go to prn-let-seq-999.
      *                      *-----------------------------------------*
      *                      * Azzeramento indice secondario           *
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura tabella sconti incrociati               *
      *              *-------------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "UNODUE"             to   f-key                  .
           move      01                   to   rf-zsx-tip-inc         .
           move      w-buf-csc-cli-cod
                    (w-buf-csc-cli-ctr)   to   rf-zsx-cod-uno         .
           move      w-buf-csc-pro-cod
                    (w-buf-csc-pro-ctr)   to   rf-zsx-cod-due         .
           move      "pgm/dcc/fls/ioc/obj/iofzsx"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           ...
           if        f-sts                not  = e-not-err
                     move  zero           to   rf-zsx-per-sco (1)
                     move  zero           to   rf-zsx-per-sco (2)
                     move  zero           to   rf-zsx-per-sco (3)
                     move  zero           to   rf-zsx-per-sco (4)
                     move  zero           to   rf-zsx-per-sco (5)     .
      *              *-------------------------------------------------*
      *              * Test su si/no stampa riga tabella a zero        *
      *              *-------------------------------------------------*
           if        rr-snx-zer           not  = "N"
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           if        rr-tip-ord           =    "C"
                     move  w-buf-csc-cli-cod
                          (w-buf-csc-cli-ctr)
                                          to   w-rot-l01-cod-rot
           else      move  w-buf-csc-pro-cod
                          (w-buf-csc-pro-ctr)
                                          to   w-rot-l01-cod-rot      .
       prn-cmp-rot-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Test se salto pagina ad inizio categoria        *
      *              *-------------------------------------------------*
           if        rr-snx-slp           not  = "N"
                     go to prn-ini-cic-400.
       prn-ini-cic-200.
      *              *-------------------------------------------------*
      *              * Se non salto pagina ad inizio categoria         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Intestazione foglio                         *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-ini-cic-400.
      *              *-------------------------------------------------*
      *              * Se salto pagina ad inizio categoria             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Non si esegue salto pagina e si continua    *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-ini-lr1-300.
      *              *-------------------------------------------------*
      *              * Intestazione per inizio categoria               *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           if        rr-tip-ord           =    "C"
                     move   "Sc. cliente  :"
                                          to   p-alf
           else      move   "Sc. prodotto :"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Lettura descrizione sconto              *
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCSC"             to   f-key                  .
           if        rr-tip-ord           =    "C"
                     move  02             to   rf-zcs-tip-csc
           else      move  03             to   rf-zcs-tip-csc         .
           move      w-rot-l01-cod-rot    to   rf-zcs-cod-csc         .
           move      "pgm/dcc/fls/ioc/obj/iofzcs"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
      *                      * Se da stampare il codice mnemonico      *
      *                      *-----------------------------------------*
           if        rr-tip-stp           not  = "M"
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      38                   to   p-pos                  .
           move      w-rot-l01-cod-rot    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-ini-lr1-420.
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           ...
           if        rr-tip-ord           =    "C"
                     move   "Sc. prodotto "
                                          to   p-alf
           else      move   "Sc. cliente  "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                          *-------------------------------------*
      *                          * Bordo superiore per fincatura       *
      *                          *-------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      47                   to   p-car                  .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Sottolineatura di fine livello          *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      66                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           if        rr-tip-ord           =    "C"
                     ...
                     move  w-buf-csc-pro-cod
                          (w-buf-csc-pro-ctr)
                                          to   rf-zcs-cod-csc
           else      move  02             to   rf-zcs-tip-csc
                     move  w-buf-csc-cli-cod
                          (w-buf-csc-cli-ctr)
                                          to   rf-zcs-cod-csc         .
           move      "pgm/dcc/fls/ioc/obj/iofzcs"
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      33                   to   p-pos                  .
           move      rf-zsx-per-sco (1)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Barra di suddivisione                       *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      40                   to   p-pos                  .
           move      rf-zsx-per-sco (2)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Barra di suddivisione                       *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      47                   to   p-pos                  .
           move      rf-zsx-per-sco (3)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Barra di suddivisione                       *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      54                   to   p-pos                  .
           move      rf-zsx-per-sco (4)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Barra di suddivisione                       *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      61                   to   p-pos                  .
           move      rf-zsx-per-sco (5)   to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Barra di suddivisione                       *
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Sottolineatura di fine dettaglio            *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      66                   to   p-car                  .
      ...
      *              *-------------------------------------------------*
           move      "SCONTI INCROCIATI IN RIGA FATTURA"
      ...
      *    *===========================================================*
      *    * Bufferizzazione tabelle sconti in riga clienti            *
      *    *-----------------------------------------------------------*
       pre-buf-cli-000.
      *              *-------------------------------------------------*
      *              * Normalizzazioni iniziali                        *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-csc-cli-max      .
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo ordinamento     *
      *              *-------------------------------------------------*
           if        rr-tip-stp           =    "C"
                     go to pre-buf-cli-100
           else if   rr-tip-stp           =    "D"
                     go to pre-buf-cli-200
           else if   rr-tip-stp           =    "M"
                     go to pre-buf-cli-300.
       pre-buf-cli-100.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "CODCSC    "         to   f-key                  .
           move      02                   to   rf-zcs-tip-csc         .
           move      rr-cod-uno           to   rf-zcs-cod-csc         .
           move      "pgm/dcc/fls/ioc/obj/iofzcs"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to pre-buf-cli-999.
      *                  *---------------------------------------------*
      *                  * Se Ok                                       *
      *                  *---------------------------------------------*
           go to     pre-buf-cli-400.
       pre-buf-cli-200.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       pre-buf-cli-400.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [zcs]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcc/fls/ioc/obj/iofzcs"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           if        rr-cod-uno           =    zero
                     go to pre-buf-cli-500.
           if        rf-zcs-cod-csc       not  = rr-cod-uno
                     go to pre-buf-cli-400.
       pre-buf-cli-500.
      *              *-------------------------------------------------*
      *              * Incremento contatore                            *
      *              *-------------------------------------------------*
           add       1                    to   w-buf-csc-cli-max      .
      *              *-------------------------------------------------*
      *              * Bufferizzazione codice                          *
      *              *-------------------------------------------------*
           move      rf-zcs-cod-csc       to   w-buf-csc-cli-cod
                                              (w-buf-csc-cli-max)     .
      *              *-------------------------------------------------*
      *              * Test se raggiunto il massimo                    *
      *              *-------------------------------------------------*
           if        w-buf-csc-cli-max    =    2000
                     go to pre-buf-cli-999.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Bufferizzazione tabelle sconti in riga prodotti           *
      *    *-----------------------------------------------------------*
       pre-buf-pro-000.
      *              *-------------------------------------------------*
      *              * Normalizzazioni iniziali                        *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-csc-pro-max      .
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo ordinamento     *
      *              *-------------------------------------------------*
           if        rr-tip-stp           =    "C"
                     go to pre-buf-pro-100
           else if   rr-tip-stp           =    "D"
                     go to pre-buf-pro-200
           else if   rr-tip-stp           =    "M"
                     go to pre-buf-pro-300.
       pre-buf-pro-100.
      *              *-------------------------------------------------*
      *              * Se ordinamento per codice                       *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "CODCSC    "         to   f-key                  .
           move      03                   to   rf-zcs-tip-csc         .
           move      rr-cod-due           to   rf-zcs-cod-csc         .
           move      "pgm/dcc/fls/ioc/obj/iofzcs"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to pre-buf-pro-999.
      *                  *---------------------------------------------*
      *                  * Se Ok                                       *
      *                  *---------------------------------------------*
           go to     pre-buf-pro-400.
       pre-buf-pro-200.
      *              *-------------------------------------------------*
      *              * Se ordinamento per descrizione                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        rr-cod-due           =    zero
                     go to pre-buf-pro-500.
           if        rf-zcs-cod-csc       not  = rr-cod-due
                     go to pre-buf-pro-400.
       pre-buf-pro-500.
      *              *-------------------------------------------------*
      *              * Incremento contatore                            *
      *              *-------------------------------------------------*
           add       1                    to   w-buf-csc-pro-max      .
      *              *-------------------------------------------------*
      *              * Bufferizzazione codice                          *
      *              *-------------------------------------------------*
           move      rf-zcs-cod-csc       to   w-buf-csc-pro-cod
                                              (w-buf-csc-pro-max)     .
      *              *-------------------------------------------------*
      *              * Test se raggiunto il massimo                    *
      *              *-------------------------------------------------*
           if        w-buf-csc-pro-max    =    2000
                     go to pre-buf-pro-999.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
