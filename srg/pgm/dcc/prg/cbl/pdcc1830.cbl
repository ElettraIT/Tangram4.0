      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella sconti incrociati tra :    *
      *                    - Cliente                                   *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " SCONTI INCROCIATI CLIENTE/CAT. PRODOTTO"       .
      ...
      *        *-------------------------------------------------------*
           05  w-tes-val-key.
               10  w-tes-cod-uno          pic  9(09)                  .
               10  w-tes-cod-uno-des      pic  x(40)                  .
               10  w-tes-cod-due          pic  9(05)                  .
               10  w-tes-cod-due-des      pic  x(20)                  .
      *        *-------------------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Codice categoria sconto in riga cliente     *
      *                  *---------------------------------------------*
       ...
       acc-key-reg-200.
      *                  *---------------------------------------------*
      *                  * Codice categoria sconto prodotto            *
      *                  *---------------------------------------------*
      ...
      *              * Codice cliente                                  *
      *              *-------------------------------------------------*
           perform   vis-cod-uno-000      thru vis-cod-uno-999        .
      *              *-------------------------------------------------*
      *              * Codice categoria di sconto prodotto             *
      *              *-------------------------------------------------*
           perform   vis-cod-due-000      thru vis-cod-due-999        .
       vis-key-reg-999.
           exit.
      ...
      *              * Codice cliente                                  *
      *              *-------------------------------------------------*
           perform   pmt-cod-uno-000      thru pmt-cod-uno-999        .
      *              *-------------------------------------------------*
      *              * Codice categoria di sconto prodotto             *
      *              *-------------------------------------------------*
           perform   pmt-cod-due-000      thru pmt-cod-due-999        .
      *              *-------------------------------------------------*
      *              * Linea di trattini                               *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice categoria di sconto    *
      *    *                             prodotto                      *
      *    *-----------------------------------------------------------*
       pmt-cod-due-000.
           move      "DS"                 to   v-ope                  .
      ...
      *                  *---------------------------------------------*
           move      w-tes-cod-uno        to   w-sav-cod-uno          .
       acc-cod-uno-050.
      *                  *---------------------------------------------*
      *                  * Preparazione valore di default              *
      *                  *---------------------------------------------*
           if        w-tes-cod-uno        not  = zero
                     go to acc-cod-uno-075.
           if        w-def-cod-uno        =    zero
                     go to acc-cod-uno-075.
           move      w-def-cod-uno        to   w-tes-cod-uno          .
           move      w-tes-cod-uno        to   w-let-arc-cli-cod      .
      *                      *-----------------------------------------*
      *                      * Lettura record [cli]                    *
      *                      *-----------------------------------------*
      ...
      *                      * Lettura record [dcc]                    *
      *                      *-----------------------------------------*
           move      w-tes-cod-uno        to   w-let-arc-dcc-cli      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-uno        to   w-cod-mne-dcc-cod      .
      ...
      *                      *-----------------------------------------*
           move      "DO  "               to   v-key                  .
      *                      *-----------------------------------------*
      *                      * A dipendenze dell'impostazione          *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      w-tes-cod-uno        to   w-def-cod-uno          .
       acc-cod-uno-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       vis-cod-uno-des-000.
           ...
           move      04                   to   v-lin                  .
           move      41                   to   v-pos                  .
           move      w-tes-cod-uno-des    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       vis-cod-uno-des-999.
           exit.
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-due        to   w-cmn-zcs-003-cod      .
           move      05                   to   w-cmn-zcs-003-lin      .
           move      30                   to   w-cmn-zcs-003-pos      .
           move      05                   to   w-cmn-zcs-003-dln      .
      ...
      *                  *---------------------------------------------*
           if        w-tes-cod-due        =    zero
                     go to acc-cod-due-410
           else      go to acc-cod-due-450.
       acc-cod-due-410.
      *                  *---------------------------------------------*
      *                  * Se il codice impostato e' zero : reimposta- *
      *                  * zione, a meno che non sia su tasto Up       *
      *                  *---------------------------------------------*
           if        v-key                =    "UP  "
      ...
      *                      *-----------------------------------------*
           ...
           move      w-tes-cod-due        to   w-let-arc-zcs-cod      .
           perform   let-arc-zcs-000      thru let-arc-zcs-999        .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione descrizione             *
      *                      *-----------------------------------------*
           move      w-let-arc-zcs-des    to   w-tes-cod-due-des      .
      *                      *-----------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       vis-des-due-000.
           ...
           move      05                   to   v-lin                  .
           move      37                   to   v-pos                  .
           move      w-tes-cod-due-des    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       vis-des-due-999.
           exit.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "UP  "
                     ...
                     go to acc-per-sco-710
           else      go to acc-per-sco-800.
       acc-per-sco-650.
      *              *-------------------------------------------------*
      *              * Se Up                                           *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-key-reg-000.
           move      zero                 to   w-tes-cod-uno          .
           move      spaces               to   w-tes-cod-uno-des      .
           move      zero                 to   w-tes-cod-due          .
           move      spaces               to   w-tes-cod-due-des      .
       nor-key-reg-999.
           exit.
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-cod-uno        to   rf-zsx-cod-uno         .
           move      w-tes-cod-due        to   rf-zsx-cod-due         .
           move      "pgm/dcc/fls/ioc/obj/iofzsx"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
           move      rf-zsx-per-sco (1)   to   w-tes-per-sco (1, 1)   .
           move      rf-zsx-per-sco (2)   to   w-tes-per-sco (1, 2)   .
           move      rf-zsx-per-sco (3)   to   w-tes-per-sco (1, 3)   .
           move      rf-zsx-per-sco (4)   to   w-tes-per-sco (1, 4)   .
           move      rf-zsx-per-sco (5)   to   w-tes-per-sco (1, 5)   .
           move      rf-zsx-alx-exp       to   w-tes-alx-exp (1)      .
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zsx]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zsx-000      thru wrt-rec-zsx-999        .
      ...
      *                      * Rewrite record [zsx]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zsx-000      thru rew-rec-zsx-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zsx]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zsx-000      thru del-rec-zsx-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zsx]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zsx-000.
      *              *-------------------------------------------------*
      ...
      *                  * Campi non chiave                            *
      *                  *---------------------------------------------*
           move      w-tes-per-sco (1, 1) to   rf-zsx-per-sco (1)     .
           move      w-tes-per-sco (1, 2) to   rf-zsx-per-sco (2)     .
           move      w-tes-per-sco (1, 3) to   rf-zsx-per-sco (3)     .
           move      w-tes-per-sco (1, 4) to   rf-zsx-per-sco (4)     .
           move      w-tes-per-sco (1, 5) to   rf-zsx-per-sco (5)     .
           move      w-tes-alx-exp (1)    to   rf-zsx-alx-exp         .
       cmp-rec-zsx-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zsx]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zsx-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zsx-000      thru cmp-rec-zsx-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
