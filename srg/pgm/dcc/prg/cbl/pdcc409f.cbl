      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   CONTROLLO CLIENTI CON ANNOTAZIONI    "       .
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   15  filler             pic  x(40) value
                            "per codice Cliente                      ".
                   15  filler             pic  x(40) value
                            "per Annotazione generica / cliente      ".
      ...
      *        *-------------------------------------------------------*
           05  w-stp-int-age.
      *            *---------------------------------------------------*
      *            * Codice annotazione da stampare                    *
      *            *---------------------------------------------------*
               10  w-stp-int-ang-cod      pic  9(03)                  .
      ...
      *                  *---------------------------------------------*
       ...
       int-pag-std-900.
      *              *-------------------------------------------------*
      *              * Stampa eventuale dell'intestazione annotazione  *
      *              *-------------------------------------------------*
           if        rr-tip-ord           not  = 02
                     go to int-pag-std-950.
       int-pag-std-910.
      *                  *---------------------------------------------*
      *                  * Se stampa per annotazione                   *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Editing codice annotazione              *
      *                      *-----------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      03                   to   p-car                  .
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      w-stp-int-ang-cod    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      ...
      *              * Start su [dcx]                                  *
      *              *                                                 *
      *              * Iniziando dal codice 1 per evitare le note ge-  *
      *              * neriche                                         *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CLIANN    "         to   f-key                  .
           move      1                    to   rf-dcx-cod-cli         .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Pulizia codice per annotazione generica     *
      *                  *---------------------------------------------*
           if        rf-dcx-cod-ang       not  numeric
                     move  zero           to   rf-dcx-cod-ang         .
      *                  *---------------------------------------------*
      *                  * Se ordinamento per annotazione generica     *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    2   and
                     rf-dcx-cod-ang       =    zero
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-dcx-cod-cli       to   rf-dcc-cod-cli         .
      *
           if        rf-dcx-dpz-cli       =    "*"
                     move  spaces         to   rf-dcc-dpz-cli
           else      move  rf-dcx-dpz-cli to   rf-dcc-dpz-cli         .
      *
           move      "pgm/dcc/fls/ioc/obj/iofdcc"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
           ...
           if        rr-tip-ord           =    01
                     move  zero           to   srt-cod-ang
           else      move  rf-dcx-cod-ang to   srt-cod-ang            .
      *
           move      rf-dcx-cod-cli       to   srt-cod-cli            .
           move      rf-dcx-dpz-cli       to   srt-dpz-cli            .
           move      rf-dcc-rag-soc       to   srt-dcc-rag-soc        .
           move      rf-dcc-via-dcc       to   srt-dcc-via-cli        .
           move      rf-dcc-loc-dcc       to   srt-dcc-loc-cli        .
           move      rf-dcx-ann-cli       to   srt-dcx-ann-cli        .
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun cliente con annotazioni da segnalare !"
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           if        rr-tip-ord           =    01
                     move  zero           to   w-rot-l02-cod-ang
           else      move  srt-cod-ang    to   w-rot-l02-cod-ang      .
      *
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Intestazione per l'annotazione generica         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  * Se codice a zero                            *
      *                  *---------------------------------------------*
           if        w-rot-l02-cod-ang    =    zero
                     go to prn-ini-lr2-400.
      *                  *---------------------------------------------*
      *                  * Bufferizzazione codice annotazione          *
      *                  *---------------------------------------------*
           move      w-rot-l02-cod-ang    to   w-stp-int-ang-cod      .
       prn-ini-lr2-400.
      *              *-------------------------------------------------*
      *              * Intestazione foglio                             *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      053                  to   p-pos                  .
           move      srt-dcx-ann-rig (1)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * 2.                                          *
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      093                  to   p-pos                  .
           move      srt-dcx-ann-rig (2)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-ini-lr1-400.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      053                  to   p-pos                  .
           move      srt-dcx-ann-rig (3)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * 4.                                          *
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      093                  to   p-pos                  .
           move      srt-dcx-ann-rig (4)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
      *              * Interlinea                                      *
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      053                  to   p-pos                  .
           move      srt-dcx-ann-rig (5)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * 6.                                          *
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      093                  to   p-pos                  .
           move      srt-dcx-ann-rig (6)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
      *              * Interlinea                                      *
