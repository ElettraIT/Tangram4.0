      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma dcc610   *
      *                                                                *
      ...
      *                                                                *
      *                    /abd/asc/UTENTE/exp/AZI_dcc610.csv          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " STAMPA ETICHETTE CLIENTI PER MARKETING "       .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-dcm-cod-cli         .
           move      "pgm/dcc/fls/ioc/obj/iofdcm"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-rag-min           to   rf-dcm-rag-key         .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-mne-min           to   rf-dcm-cod-mne         .
           move      zero                 to   rf-dcm-cod-cli         .
           move      "pgm/dcc/fls/ioc/obj/iofdcm"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-pti-min           to   rf-dcm-prt-iva         .
           move      zero                 to   rf-dcm-cod-cli         .
           move      "pgm/dcc/fls/ioc/obj/iofdcm"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [dcm]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcc/fls/ioc/obj/iofdcm"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           if        rf-dcm-cod-cli       <    rr-cod-min or
                     rf-dcm-cod-cli       >    rr-cod-max or
                     rf-dcm-rag-key       <    rr-rag-min or
                     rf-dcm-rag-key       >    rr-rag-max or
                     rf-dcm-cod-mne       <    rr-mne-min or
                     rf-dcm-cod-mne       >    rr-mne-max or
                     rf-dcm-prt-iva       <    rr-pti-min or
                     rf-dcm-prt-iva       >    rr-pti-max
                     go to  prn-sel-rec-900.
       prn-sel-rec-200.
      *              *-------------------------------------------------*
      *              * Selezione su codice come ns. cliente            *
      *              *-------------------------------------------------*
           if        rf-dcm-cod-cge       not  = zero
                     go to  prn-sel-rec-900.
       prn-sel-rec-300.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-gxc                 .
      *                      *-----------------------------------------*
      *                      * Se record non trovato : uscita per se-  *
      *                      * lezione non superata                    *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-sel-rec-900.
      *                  *---------------------------------------------*
      *                  * Test su codice provincia                    *
      *                  *---------------------------------------------*
           if        rf-gxc-cod-prv       not  = rr-cod-prv
                     go to prn-sel-rec-900.
       prn-sel-rec-400.
      *              *-------------------------------------------------*
      *              * Selezione su categoria SEAT                     *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se codice a spaces                     *
      *                  *---------------------------------------------*
           if        rr-cat-sea           =    spaces
                     go to prn-sel-rec-800.
           if        rf-dcm-cat-sea       not  = rr-cat-sea
                     go to prn-sel-rec-900.
       prn-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "A"                  to   x-tip                  .
           move      20                   to   x-car                  .
           move      "RAGIONE SOCIALE 1"  to   x-alf                  .
           call      "swd/mod/prg/obj/mxport"
                                         using x                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "A"                  to   x-tip                  .
           move      20                   to   x-car                  .
           move      "RAGIONE SOCIALE 2"  to   x-alf                  .
           call      "swd/mod/prg/obj/mxport"
                                         using x                      .
      *                  *---------------------------------------------*
      *                  * Indirizzo cliente                           *
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se non e' stata richiesta la stampa del *
      *                      * nome dell'interlocutore si espone co-   *
      *                      * munque la dicitura standard             *
      *                      *-----------------------------------------*
           if        rr-snx-int           not  = 02
                     go to prn-eti-001-200.
      *                      *-----------------------------------------*
      *                      * Se esiste il nominativo dell'interlocu- *
      *                      * tore si pone come literal di intesta-   *
      *                      * zione                                   *
      *                      *-----------------------------------------*
           if        rf-dcm-nom-int       =    spaces
                     go to prn-eti-001-200.
      *                          *-------------------------------------*
      *                          * Literal 'C.att.  :'                 *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           ...
           move      012                  to   p-pos                  .
           add       w-cnt-eti-num-cps    to   p-pos                  .
           move      rf-dcm-nom-int       to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                          *-------------------------------------*
      *                          * A stampa ragione sociale            *
      *                          *-------------------------------------*
           go to     prn-eti-001-300.
       prn-eti-001-200.
      *                      *-----------------------------------------*
      *                      * Literal 'Spettabile'                    *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           if        w-cnt-eti-num-ccs    >    3
                     move  zero           to   w-cnt-eti-num-ccs
                     go to int-pag-001-000.
           if        w-cnt-eti-num-ccs    =    1
                     go to int-pag-001-100
           else      go to int-pag-001-200.
       int-pag-001-100.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Intestazione foglio per etichette di tipo 02              *
      *    *-----------------------------------------------------------*
       int-pag-002-000.
      *              *-------------------------------------------------*
      *              * Intestazione standard                           *
