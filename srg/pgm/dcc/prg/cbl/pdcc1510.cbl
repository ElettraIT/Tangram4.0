      ...
      *                       Ultima revisione:    NdK del 29/02/16    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio [zfp]            *
      *                                                                *
      *                    Anagrafico forme di pagamento               *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   INTERROGAZIONE FORME DI PAGAMENTO    "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-det.
      *        *-------------------------------------------------------*
      *        * Work per Det descrizioni codici di pagamento          *
      *        *-------------------------------------------------------*
           05  w-det-des-pag.
               10  w-det-des-pag-001      pic  x(40)                  .
               10  w-det-des-pag-002      pic  x(40)                  .
               10  w-det-des-pag-003      pic  x(40)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-rip-str.
      *        *-------------------------------------------------------*
      *        * Work-area per ripristino Start su [zfp]               *
      *        *-------------------------------------------------------*
           05  w-rip-str-zfp.
      *            *---------------------------------------------------*
      *            * Flag di End-of-file                               *
      *            *---------------------------------------------------*
               10  w-rip-str-zfp-eof      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Parametri di Start                                *
      *            *---------------------------------------------------*
               10  w-rip-str-zfp-cod      pic  9(07)                  .
               10  w-rip-str-zfp-des      pic  x(40)                  .
               10  w-rip-str-zfp-mne      pic  x(10)                  .

      *    *===========================================================*
      *    * Work-area per ridefinizione contenuto del mark-point      *
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-cod-fop              pic  9(07)                  .
      ...
      *                  *---------------------------------------------*
       ...
       pre-tip-fun-300.
      *              *-------------------------------------------------*
      *              * Se variabile di i.p.c. per ricerca dicotomica   *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * No richieste ad utente                      *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Se c'e' stato un match : a selezione supera-*
      *                  * ta                                          *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "+"                  to   v-edm                  .
           move      rf-zfp-cod-fop       to   w-mpn-cod-fop          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                not  = "EXPD"
                     go to qry-trt-fun-999.
      *                  *---------------------------------------------*
      *                  * Routine per Expand su record [zfp]          *
      *                  *---------------------------------------------*
           perform   exp-rec-zfp-000      thru exp-rec-zfp-999        .
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       exp-rec-zfp-000.
      *              *-------------------------------------------------*
      *              * Salvataggio parametri per Start su [zfp]        *
      *              *-------------------------------------------------*
           if        w-rip-str-zfp-eof    not  = spaces
                     go to exp-rec-zfp-100.
           move      rf-zfp-cod-fop       to   w-rip-str-zfp-cod      .
           move      rf-zfp-des-key       to   w-rip-str-zfp-des      .
           move      rf-zfp-cod-mne       to   w-rip-str-zfp-mne      .
       exp-rec-zfp-100.
      *              *-------------------------------------------------*
      *              * Video in 'OFF'                                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Prompt per Codice forma di pagamento        *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      09                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      "Sconto in fattura    :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per 1. Codice pagamento              *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
           move      11                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      "1. Codice pagamento  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per 1. Codice pagamento, ammontare   *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
           move      12                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      " . Su tipo ammontare :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per 2. Codice pagamento              *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
           move      14                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      "2. Codice pagamento  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per 2. Codice pagamento, ammontare   *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      17                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      "3. Codice pagamento  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per 3. Codice pagamento, ammontare   *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      22                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      "CODFOP    "         to   f-key                  .
           move      v-cnt                to   w-mpn                  .
           move      w-mpn-cod-fop        to   rf-zfp-cod-fop         .
           move      "pgm/dcc/fls/ioc/obj/iofzfp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Descrizione 1. codice di pagamento          *
      *                  *---------------------------------------------*
           move      rf-zfp-cod-pag (1)   to   w-let-arc-zpg-cod      .
           perform   let-arc-zpg-000      thru let-arc-zpg-999        .
           move      w-let-arc-zpg-des    to   w-det-des-pag-001      .
      *                  *---------------------------------------------*
      *                  * Descrizione 2. codice di pagamento          *
      *                  *---------------------------------------------*
           move      rf-zfp-cod-pag (2)   to   w-let-arc-zpg-cod      .
           perform   let-arc-zpg-000      thru let-arc-zpg-999        .
           move      w-let-arc-zpg-des    to   w-det-des-pag-002      .
      *                  *---------------------------------------------*
      *                  * Descrizione 3. codice di pagamento          *
      *                  *---------------------------------------------*
           move      rf-zfp-cod-pag (3)   to   w-let-arc-zpg-cod      .
           perform   let-arc-zpg-000      thru let-arc-zpg-999        .
           move      w-let-arc-zpg-des    to   w-det-des-pag-003      .
       exp-rec-zfp-700.
      *              *-------------------------------------------------*
      ...
      *                      * Test se a zero                          *
      *                      *-----------------------------------------*
           if        rf-zfp-cod-pag (1)   =    zero
                     go to exp-rec-zfp-720.
      *                      *-----------------------------------------*
      *                      * Codice                                  *
      *                      *-----------------------------------------*
           move      "DS"                 to   v-ope                  .
           ...
           move      11                   to   v-lin                  .
           move      26                   to   v-pos                  .
           move      rf-zfp-cod-pag (1)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Descrizione                             *
      *                      *-----------------------------------------*
           ...
           move      11                   to   v-lin                  .
           move      35                   to   v-pos                  .
           move      w-det-des-pag-001    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Tipo di ammontare                       *
      *                      *-----------------------------------------*
           ...
           move      rf-zfp-tip-amm (1)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Percentuale                             *
      ...
      *                          *-------------------------------------*
           ...
           move      12                   to   v-lin                  .
           move      63                   to   v-pos                  .
           move      rf-zfp-per-toi (1)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       exp-rec-zfp-720.
      *                  *---------------------------------------------*
      ...
      *                      * Test se a zero                          *
      *                      *-----------------------------------------*
           if        rf-zfp-cod-pag (2)   =    zero
                     go to exp-rec-zfp-730.
      *                      *-----------------------------------------*
      *                      * Codice                                  *
      *                      *-----------------------------------------*
           move      "DS"                 to   v-ope                  .
           ...
           move      14                   to   v-lin                  .
           move      26                   to   v-pos                  .
           move      rf-zfp-cod-pag (2)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Descrizione                             *
      *                      *-----------------------------------------*
           ...
           move      14                   to   v-lin                  .
           move      35                   to   v-pos                  .
           move      w-det-des-pag-002    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Tipo di ammontare                       *
      *                      *-----------------------------------------*
           ...
           move      26                   to   v-pos                  .
           move      w-exp-tip-amm-tbl    to   v-txt                  .
           move      rf-zfp-tip-amm (2)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Percentuale                             *
      ...
      *                          *-------------------------------------*
           ...
           move      15                   to   v-lin                  .
           move      63                   to   v-pos                  .
           move      rf-zfp-per-toi (2)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       exp-rec-zfp-730.
      *                  *---------------------------------------------*
      ...
      *                      * Test se a zero                          *
      *                      *-----------------------------------------*
           if        rf-zfp-cod-pag (3)   =    zero
                     go to exp-rec-zfp-740.
      *                      *-----------------------------------------*
      *                      * Codice                                  *
      *                      *-----------------------------------------*
           move      "DS"                 to   v-ope                  .
           ...
           move      17                   to   v-lin                  .
           move      26                   to   v-pos                  .
           move      rf-zfp-cod-pag (3)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Descrizione                             *
      *                      *-----------------------------------------*
           ...
           move      17                   to   v-lin                  .
           move      35                   to   v-pos                  .
           move      w-det-des-pag-003    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Tipo di ammontare                       *
      *                      *-----------------------------------------*
           ...
           move      26                   to   v-pos                  .
           move      w-exp-tip-amm-tbl    to   v-txt                  .
           move      rf-zfp-tip-amm (3)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Percentuale                             *
      ...
      *                          *-------------------------------------*
           ...
           move      18                   to   v-lin                  .
           move      63                   to   v-pos                  .
           move      rf-zfp-per-toi (3)   to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       exp-rec-zfp-740.
      *                  *---------------------------------------------*
      *                  * Nota se acconto in detrazione               *
      *                  *---------------------------------------------*
           if        rf-zfp-dim-act (1)   not  = 02 and
                     rf-zfp-dim-act (2)   not  = 02 and
                     rf-zfp-dim-act (3)   not  = 02
                     go to exp-rec-zfp-750.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      40                   to   v-car                  .
           move      20                   to   v-lin                  .
           move      26                   to   v-pos                  .
           move      "N.B. : Con detrazione dell'acconto      "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       exp-rec-zfp-750.
      ...
      *                  *---------------------------------------------*
           if        rf-zfp-ide-dat       =    zero
                     go to exp-rec-zfp-760.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
           move      07                   to   v-lin                  .
           move      69                   to   v-pos                  .
           move      rf-zfp-ide-dat       to   v-dat                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       exp-rec-zfp-760.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rf-zfp-ide-ute       =    spaces
                     go to exp-rec-zfp-770.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      10                   to   v-car                  .
           move      08                   to   v-lin                  .
      ...
      *                  *---------------------------------------------*
           if        rf-zfp-ide-fas       =    spaces
                     go to exp-rec-zfp-800.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      08                   to   v-car                  .
           move      09                   to   v-lin                  .
      ...
      *                  *---------------------------------------------*
           if        w-rip-str-zfp-eof    not  = spaces
                     go to exp-rec-zfp-999.
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo di Start    *
      *                  * attualmente attivo                          *
      *                  *---------------------------------------------*
           if        rr-tip-ord           =    "C"
                     go to exp-rec-zfp-910
           else if   rr-tip-ord           =    "D" or
                     rr-tip-ord           =    "d"
                     go to exp-rec-zfp-920
           else if   rr-tip-ord           =    "M"
                     go to exp-rec-zfp-930.
       exp-rec-zfp-910.
      *                  *---------------------------------------------*
      *                  * Se start per codice                         *
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODFOP    "         to   f-key                  .
           move      w-rip-str-zfp-cod    to   rf-zfp-cod-fop         .
           move      "pgm/dcc/fls/ioc/obj/iofzfp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-zfp                 .
           if        f-sts                not  = e-not-err
                     go to exp-rec-zfp-990.
           go to     exp-rec-zfp-980.
       exp-rec-zfp-920.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "DESKEY    "         to   f-key                  .
           move      w-rip-str-zfp-des    to   rf-zfp-des-key         .
           move      w-rip-str-zfp-cod    to   rf-zfp-cod-fop         .
           move      "pgm/dcc/fls/ioc/obj/iofzfp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CODMNE    "         to   f-key                  .
           move      w-rip-str-zfp-mne    to   rf-zfp-cod-mne         .
           move      w-rip-str-zfp-cod    to   rf-zfp-cod-fop         .
           move      "pgm/dcc/fls/ioc/obj/iofzfp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
