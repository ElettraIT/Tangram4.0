      ...
      *                                   Fase:    dcc408              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 07/07/93    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pdcc4081:        *
      *                    Stampa rubrica telefonica clienti commer-   *
      *                    ciali                                       *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA RUBRICA TEL. CLIENTI COMMERCIALI "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no senza numeri                         *
      *        *-------------------------------------------------------*
           05  w-exp-snx-num.
               10  w-exp-snx-num-num      pic  9(02)       value 3    .
               10  w-exp-snx-num-lun      pic  9(02)       value 40   .
               10  w-exp-snx-num-tbl.
                   15  filler             pic  x(40)       value
                            "Tutti i clienti con numeri di utenza    ".
                   15  filler             pic  x(40)       value
                            "Tutti i clienti                         ".
                   15  filler             pic  x(40)       value
                            "Tutti i clienti senza numeri di utenza  ".
      *        *-------------------------------------------------------*
      *        * Work per : Si/no salto pagina                         *
      *        *-------------------------------------------------------*
           05  w-exp-snx-slp.
               10  w-exp-snx-slp-num      pic  9(02)       value 2    .
      ...
      *                  * Si/no senza numero                          *
      *                  *---------------------------------------------*
           perform   acc-snx-num-000      thru acc-snx-num-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Selezione su numeri di utenza'          *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           move      17                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su numeri di utenza :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Salto pagina a cambio lettera :'        *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
           move      20                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Salto pagina a cambio lettera :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       pmt-ric-sel-999.
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/no senza numeri         *
      *    *-----------------------------------------------------------*
       acc-snx-num-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-num-lun    to   v-car                  .
           move      w-exp-snx-num-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      17                   to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-exp-snx-num-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-snx-num           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-num-999.
       acc-snx-num-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-num             .
       acc-snx-num-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-num           =    zero
                     go to acc-snx-num-100.
       acc-snx-num-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      zero                 to   rr-snx-num             .
           move      zero                 to   rr-snx-slp             .
       nor-ric-sel-999.
           exit.
      ...
      *              *-------------------------------------------------*
           if        rr-snx-num           =    zero
                     move  01             to   rr-snx-num             .
      *              *-------------------------------------------------*
      *              * Si/no salto pagina                              *
      *              *-------------------------------------------------*
           if        rr-snx-slp           =    zero
                     move  01             to   rr-snx-slp             .
       reg-ric-sel-999.
           exit.
