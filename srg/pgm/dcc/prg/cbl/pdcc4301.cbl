      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pdcc4300 *
      *                                                                *
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-buf-des-pdx-des      .
       prn-sel-rec-100.
      *              *-------------------------------------------------*
      *              * Lettura descrizione prodotto per il cliente     *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Inizio ciclo di lettura                     *
      *                  *---------------------------------------------*
           move      zero                 to   w-buf-des-pdx-ctr      .
       prn-sel-rec-220.
      *                  *---------------------------------------------*
      *                  * Start su file [pdx]                         *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-pdx                 .
      *                      *-----------------------------------------*
      *                      * Se fine file : a test su contatore      *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-sel-rec-700.
       prn-sel-rec-400.
      ...
      *                  * Test sul massimo                            *
      *                  *---------------------------------------------*
           if        rf-pdx-tip-rec       not  = rr-tip-rec     or
                     rf-pdx-cod-arc       not  = rr-cod-cli     or
      ...
      *                  *---------------------------------------------*
           add       1                    to   w-buf-des-pdx-ctr      .
           if        w-buf-des-pdx-ctr    >    10
                     go to prn-sel-rec-700.
      *                  *---------------------------------------------*
      *                  * Bufferizzazione descrizione                 *
      *                  *---------------------------------------------*
           move      rf-pdx-des-pro       to   w-buf-des-pdx-der
                                              (w-buf-des-pdx-ctr)     .
       prn-sel-rec-600.
      *                  *---------------------------------------------*
      *                  * Riciclo                                     *
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se selezionate tutte le descrizioni :   *
      *                      * uscita                                  *
      *                      *-----------------------------------------*
           if        rr-snx-des           =    01
       ...
       prn-sel-rec-710.
      *                      *-----------------------------------------*
      *                      * Se selezionate solo descrizioni         *
      *                      * esistenti                               *
      *                      *-----------------------------------------*
           if        rr-snx-des           =    03
                     go to prn-sel-rec-720.
           if        w-buf-des-pdx-ctr    =    zero
                     go to prn-sel-rec-900
           else      go to prn-sel-rec-800.
       prn-sel-rec-720.
      *                      *-----------------------------------------*
      *                      * Se selezionate solo descrizioni non     *
      *                      * esistenti                               *
      *                      *-----------------------------------------*
           if        w-buf-des-pdx-ctr    =    zero
                     go to prn-sel-rec-800
      ...
      *              *-------------------------------------------------*
           if        w-buf-des-pdx-ctr    =    zero
                     move  1              to   w-buf-des-pdx-res
           else      move  w-buf-des-pdx-ctr
                                          to   w-buf-des-pdx-res      .
      *                  *---------------------------------------------*
      *                  * Test su righe residue in base alla presenza *
      *                  * o meno della descrizione per il cliente     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Eventuale prima riga di descrizione per il  *
      *                  * cliente                                     *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
       ...
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Stampa ennesime eventuali righe di descrizione  *
      *              *-------------------------------------------------*
           if        w-buf-des-pdx-ctr    not  > 1
                     go to prn-liv-det-999.
      *                  *---------------------------------------------*
      *                  * Inizio ciclo di lettura                     *
      *                  *---------------------------------------------*
           move      01                   to   w-buf-des-pdx-c01      .
       prn-liv-det-320.
      *                  *---------------------------------------------*
      *                  * Incremento contatore di comodo              *
      *                  *---------------------------------------------*
           add       01                   to   w-buf-des-pdx-c01      .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Stampa riga di descrizione per il cliente   *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      40                   to   p-car                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "    Codice       Descrizione prodotto ad uso inter
      -              "no           Descrizione per il cliente       "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
