      ...
      *                                   Fase:    dcc481              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 31/01/01    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su tabella annotazioni cli-  *
      *                    enti                                        *
      *                                                                *
      *================================================================*
      ...
      *            *---------------------------------------------------*
               10  w-ipc-dlp-tdi.
      *                *-----------------------------------------------*
      *                * Flag di lettura effettuata                    *
      *                *-----------------------------------------------*
                   15  w-ipc-dlp-tdi-flg  pic  x(01)                  .
      *                *-----------------------------------------------*
      *                * Codice cliente                                *
      *                *-----------------------------------------------*
                   15  w-ipc-dlp-tdi-cli  pic  9(07)                  .
      *                *-----------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      w-ipc-dlp-tdi-cli    to   rr-cod-cli             .
           move      w-ipc-dlp-tdi-dpz    to   rr-dpz-cli             .
      *                  *---------------------------------------------*
      *                  * Completamento cliente                       *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Completamento dipendenza cliente            *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-cli           to   rf-dcx-cod-cli         .
           move      rr-dpz-cli           to   rf-dcx-dpz-cli         .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [dcx]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcc/fls/ioc/obj/iofdcx"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           if        rf-dcx-cod-cli       not  = rr-cod-cli
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-tst-max-999.
      *              *-------------------------------------------------*
      *              * Test su Codice dipendenza cliente               *
      *              *-------------------------------------------------*
           if        rf-dcx-dpz-cli       not  = rr-dpz-cli
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-tst-max-999.
       qry-tst-max-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       qry-cmp-rot-000.
           move      rf-dcx-cod-ann       to   w-rot-l01-cod-ann      .
       qry-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      07                   to   v-pos                  .
           move      rf-dcx-ann-rig (1)   to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-dcx-cod-ann       to   w-mpn-cod-ann          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      59                   to   v-pos                  .
           move      rf-dcx-dat-fin       to   v-dat                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Eventuale codice annotazione generica       *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      03                   to   v-car                  .
           ...
           move      73                   to   v-pos                  .
      *
           if        rf-dcx-cod-cli       =    zero
                     move  zero           to   v-num
           else if   rf-dcx-cod-ang       not  numeric
                     move  zero           to   v-num
           else      move  rf-dcx-cod-ang to   v-num                  .
      *
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-ini-lr1-999.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-ann'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Literal per codice dipendenza cliente       *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Ragione sociale dipendenza del cliente      *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      40                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-ipc-dlp-tdi-flg      .
           move      zero                 to   w-ipc-dlp-tdi-cli      .
      ...
      *              *-------------------------------------------------*
           ...
           move      s-num                to   w-ipc-dlp-tdi-cli      .
           move      "#"                  to   w-ipc-dlp-tdi-flg      .
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "dpz-cli"                                       *
      *              *-------------------------------------------------*
