      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pdcc4350 *
      *                                                                *
      ...
      *    *-----------------------------------------------------------*
       01  w-det.
      *        *-------------------------------------------------------*
      *        * Work per Det codice ultimo ordine                     *
      *        *-------------------------------------------------------*
           05  w-det-cod-orc.
               10  w-det-cod-orc-ele      pic  9(03)                  .
               10  w-det-cod-orc-cod      pic  x(14)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-buf-des-pdx-des.
               10  w-buf-des-pdx-der occurs 10
                                          pic  x(40)                  .
      *        *-------------------------------------------------------*
      *        * Comodo per codice letto                               *
      *        *-------------------------------------------------------*
           05  w-buf-des-pdx-cod          pic  x(20)                  .
      ...
      *            * Literal di uscita                                 *
      *            *---------------------------------------------------*
               10  w-cat-rag-soc-res      pic  x(59)                  .
      *            *---------------------------------------------------*
      *            * Comodo per ospitare il codice cliente             *
      *            *---------------------------------------------------*
               10  w-cat-rag-soc-cli      pic  x(07)                  .
      *            *---------------------------------------------------*
      *            * Puntatori                                         *
      *            *---------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-pdx                 .
      *              *-------------------------------------------------*
      *              * Lettura codice prodotto per il cliente          *
      *              *-------------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "TRCLNG    "         to   f-key                  .
           move      rr-tip-rec           to   rf-pdx-tip-rec         .
           move      rr-cod-cli           to   rf-pdx-cod-arc         .
      ...
      *              *-------------------------------------------------*
           move      rf-pdx-des-pro       to   w-buf-des-pdx-cod      .
       prn-sel-rec-700.
      *              *-------------------------------------------------*
      *              * Test di selezione                               *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           go to     prn-sel-rec-710
                     prn-sel-rec-720
                     prn-sel-rec-730
                     prn-sel-rec-740
                     depending            on   rr-snx-cod             .
       prn-sel-rec-710.
      *                  *---------------------------------------------*
      *                  * Se selezionati tutti i codici : uscita      *
      *                  *---------------------------------------------*
           go to     prn-sel-rec-800.
       prn-sel-rec-720.
      *                  *---------------------------------------------*
      *                  * Se selezionati solo codici esistenti        *
      *                  *---------------------------------------------*
           if        w-buf-des-pdx-cod    =    spaces
                     go to prn-sel-rec-900
           else      go to prn-sel-rec-800.
       prn-sel-rec-730.
      *                  *---------------------------------------------*
      *                  * Se selezionati solo codici inesistenti      *
      *                  *---------------------------------------------*
           if        w-buf-des-pdx-cod    =    spaces
                     go to prn-sel-rec-800
       ...
       prn-sel-rec-740.
      *                  *---------------------------------------------*
      *                  * Se selezionati solo codici difformi da      *
      *                  * ultimo ordine                               *
      *                  *---------------------------------------------*
           if        w-buf-des-pdx-cod    =    spaces
                     go to prn-sel-rec-900.
           perform   prn-sel-rec-orc-000  thru prn-sel-rec-orc-999    .
           if        w-det-cod-orc-ele    =    zero
                     go to prn-sel-rec-900
           else      go to prn-sel-rec-800.
       prn-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *    * Ciclo Report Program : Selezione su record letto          *
      *    *                                                           *
      *    * Subroutine di controllo difformita' in ordini inevasi     *
      *    *-----------------------------------------------------------*
       prn-sel-rec-orc-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore elementi              *
      *              *-------------------------------------------------*
           move      zero                 to   w-det-cod-orc-ele      .
       prn-sel-rec-orc-100.
      *              *-------------------------------------------------*
      *              * Start su file [ocr]                             *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rf-ocr-cod-arc       not  = rr-cod-cli
                     go to prn-sel-rec-orc-900.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        d-qev-roc-qta-dev    =    zero
                     go to prn-sel-rec-orc-200.
      *                  *---------------------------------------------*
      *                  * Confronto con codice prodotto per cliente   *
      *                  *---------------------------------------------*
           if        rf-ocr-cop-scl       =    w-buf-des-pdx-cod
                     go to prn-sel-rec-orc-200.
       prn-sel-rec-orc-600.
      *              *-------------------------------------------------*
      *              * Incremento contatore elementi rilevati          *
      *              *-------------------------------------------------*
           add       1                    to   w-det-cod-orc-ele      .
       prn-sel-rec-orc-800.
      *              *-------------------------------------------------*
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Descrizione prodotto ad uso interno         *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      40                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      061                  to   p-pos                  .
           move      w-buf-des-pdx-cod    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-600.
      *              *-------------------------------------------------*
      *              * Eventuale stampa riferimenti a ordini difformi  *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se previsto da selezioni               *
      *                  *---------------------------------------------*
           if        rr-snx-cod           not  = 04
                     go to prn-liv-det-900.
      *                  *---------------------------------------------*
      *                  * Test su numero elementi difformi            *
      *                  *---------------------------------------------*
           if        w-det-cod-orc-ele    =    zero
                     go to prn-liv-det-900.
      *                  *---------------------------------------------*
      ...
      *    * Ciclo Report Program : Livello di dettaglio               *
      *    *                                                           *
      *    * Subroutine di stampa riferimenti a ordini difformi        *
      *    *-----------------------------------------------------------*
       prn-liv-det-orc-000.
      *              *-------------------------------------------------*
      *              * Start su file [ocr]                             *
      ...
      *              * Preparazione titolo                             *
      *              *-------------------------------------------------*
           move      spaces               to   w-cat-rag-soc-res      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-cat-rag-soc-cli      .
      *                  *---------------------------------------------*
      *                  * Concatenazione literal                      *
      *                  *---------------------------------------------*
           move      zero                 to   w-cat-rag-soc-inx      .
           move      "Cliente :"          to   w-cat-rag-soc-res      .
           inspect   w-cat-rag-soc-res
                                      tallying w-cat-rag-soc-inx
                                          for  trailing spaces        .
           move      59                   to   w-cat-rag-soc-pnt      .
           subtract  w-cat-rag-soc-inx    from w-cat-rag-soc-pnt      .
           add       2                    to   w-cat-rag-soc-pnt      .
           if        w-cat-rag-soc-pnt    >    59
                     go to int-pag-sta-020.
      *                  *---------------------------------------------*
      *                  * Concatenazione codice cliente               *
      *                  *---------------------------------------------*
           move      w-cat-rag-soc-cli    to   w-cat-rag-soc-res
                                              (w-cat-rag-soc-pnt :)   .
           move      zero                 to   w-cat-rag-soc-inx      .
           inspect   w-cat-rag-soc-res
                                      tallying w-cat-rag-soc-inx
           ...
           move      59                   to   w-cat-rag-soc-pnt      .
           subtract  w-cat-rag-soc-inx    from w-cat-rag-soc-pnt      .
           add       3                    to   w-cat-rag-soc-pnt      .
           if        w-cat-rag-soc-pnt    >    59
                     go to int-pag-sta-020.
      *                  *---------------------------------------------*
      *                  * Concatenazione ragione sociale              *
      *                  *---------------------------------------------*
           move      rr-cod-cli-rag       to   w-cat-rag-soc-res
                                              (w-cat-rag-soc-pnt :)   .
       int-pag-sta-020.
      *                  *---------------------------------------------*
      *                  * Titolo preparato                            *
      *                  *---------------------------------------------*
           move      w-cat-rag-soc-res    to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "    Codice         Descrizione prodotto ad uso int
      -              "erno      Codice per il cliente       Note    "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
