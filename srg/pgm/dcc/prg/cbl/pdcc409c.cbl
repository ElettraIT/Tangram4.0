      ...
      *                    per cui il codice selezionato tra i se-     *
      *                    guenti non trova riscontro in tabella :     *
      *                                                                *
      *                    - Forma  di pagamento                       *
      *                    - Codici ABI-CAB                            *
      *                    - Codice agente                             *
      ...
      *        *-------------------------------------------------------*
           05  srt-dat.
      *            *---------------------------------------------------*
      *            * Ragione sociale da [dcc]                          *
      *            *---------------------------------------------------*
               10  srt-dcc-rag-soc        pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Indirizzo da [dcc]                                *
      *            *---------------------------------------------------*
               10  srt-dcc-via-cli        pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Localita' da [dcc]                                *
      *            *---------------------------------------------------*
               10  srt-dcc-loc-cli        pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Status da [dcc]                                   *
      *            *---------------------------------------------------*
               10  srt-dcc-sta-tus        pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Codici errati                                     *
      *            *---------------------------------------------------*
               10  srt-dcc-cod-er1        pic  9(07)                  .
               10  srt-dcc-cod-er2        pic  9(07)                  .
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   CONTROLLO CODICI ERRATI IN CLIENTI   "       .
      ...
      *        *  - 01 : Forma  di pagamento                           *
      *        *  - 02 : Codici ABI - CAB                              *
      *        *  - 03 : Codice agente                                 *
      *        *  - 04 : Codice zona                                   *
      *        *  - 05 : Codice categoria                              *
      *        *  - 06 : Codice statistico                             *
      *        *-------------------------------------------------------*
      ...
      *
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "seguito non trova riscontro nella relativa tabella
      -              ".                             "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *
           ...
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "N.B.: il simbolo '#' in stampa indica la status NO
      -              "N normale                     "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *                      * Lettura tabella                         *
      *                      *-----------------------------------------*
           move      rf-dcc-cod-fop       to   w-let-arc-zfp-cod      .
           perform   let-arc-zfp-000      thru let-arc-zfp-999        .
      *                      *-----------------------------------------*
      *                      * Se codice errato : selezione superata   *
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Lettura tabella per ABI                 *
      *                      *-----------------------------------------*
           move      rf-dcc-cod-abi       to   w-let-arc-axi-cod      .
      ...
      *                      *-----------------------------------------*
           if        w-let-arc-axi-flg    not  = spaces
                     go to stp-srt-inp-300
           else      go to stp-srt-inp-222.
       stp-srt-inp-222.

      *                      *-----------------------------------------*
      *                      * Lettura tabella per CAB                 *
      *                      *-----------------------------------------*
           move      rf-dcc-cod-abi       to   w-let-arc-axs-abi      .
           move      rf-dcc-cod-cab       to   w-let-arc-axs-cab      .
      ...
      *              *-------------------------------------------------*
           ...
           move      rf-dcc-rag-soc       to   srt-dcc-rag-soc        .
           move      rf-dcc-via-dcc       to   srt-dcc-via-cli        .
           move      rf-dcc-loc-dcc       to   srt-dcc-loc-cli        .
           move      rf-dcc-sta-tus       to   srt-dcc-sta-tus        .
      *              *-------------------------------------------------*
      *              * Codice errato                                   *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-310.
           move      rf-dcc-cod-fop       to   srt-dcc-cod-er1        .
           move      zero                 to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-320.
           move      rf-dcc-cod-abi       to   srt-dcc-cod-er1        .
           move      rf-dcc-cod-cab       to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-330.
           move      rf-dcc-cod-age       to   srt-dcc-cod-er1        .
           move      zero                 to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-340.
           move      rf-dcc-cod-zon       to   srt-dcc-cod-er1        .
           move      zero                 to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-350.
           move      rf-dcc-cod-cat       to   srt-dcc-cod-er1        .
           move      zero                 to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-360.
           move      rf-dcc-cod-stt       to   srt-dcc-cod-er1        .
           move      zero                 to   srt-dcc-cod-er2        .
           go to     stp-srt-inp-400.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun cliente con codici errati da segnalare !"
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *              *-------------------------------------------------*
       ...
       prn-liv-det-225.
      *              *-------------------------------------------------*
      *              * Eventuale segnale di status non normale         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se dipendenza                          *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      018                  to   p-pos                  .
           move      srt-dcc-rag-soc      to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Codice errato in funzione del tipo codice ri-   *
      *              * chiesto                                         *
      *              *-------------------------------------------------*
       ...
       prn-liv-det-310.
      *              *-------------------------------------------------*
      *              * Clienti con forma di pagamento errata           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      07                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      "Codice errato :"    to   w-all-str-cat (1)      .
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-320.
      *              *-------------------------------------------------*
      *              * Clienti con codici ABI - CAB errati             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      02                   to   w-all-str-num          .
           move      "Codici errati :"    to   w-all-str-cat (1)      .
           move      p-edt                to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                  *---------------------------------------------*
      *                  * Editing codice errore 2                     *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      07                   to   p-car                  .
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-330.
      *              *-------------------------------------------------*
      *              * Clienti con codice agente errato                *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-340.
      *              *-------------------------------------------------*
      *              * Clienti con codice zona errato                  *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-350.
      *              *-------------------------------------------------*
      *              * Clienti con codice categoria errato             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-360.
      *              *-------------------------------------------------*
      *              * Clienti con codice statistico errato            *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Editing codice errore 1                     *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           go to     int-pag-sta-010
                     ...
                     depending   on rr-tip-cod.
       int-pag-sta-010.
           move      "CLIENTI CON FORMA DI PAGAMENTO ERRATA"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-020.
           move      "CLIENTI CON CODICI ABI-CAB ERRATI"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-030.
           move      "CLIENTI CON CODICE AGENTE ERRATO"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-040.
           move      "CLIENTI CON CODICE ZONA ERRATO"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-050.
           move      "CLIENTI CON CODICE CATEGORIA ERRATO"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-060.
           move      "CLIENTI CON CODICE STATISTICO ERRATO"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-090.
           move      "DT"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "     Codice                Anagrafica cliente     
      -              "                                            Note  
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
