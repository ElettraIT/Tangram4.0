      ...
      *                                   Fase:    abi102              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 27/08/90    *
      *                       Ultima revisione:    NdK del 03/12/01    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma abi102:  *
      ...
      *    *-----------------------------------------------------------*
       01  w-reg.
      *        *-------------------------------------------------------*
      *        * Codice istituto in corso di trattamento               *
      *        *-------------------------------------------------------*
           05  w-reg-cod-abi              pic  9(05) value zero       .
      *        *-------------------------------------------------------*
      *        * Codice agenzia istituto in corso di trattamento       *
      *        *-------------------------------------------------------*
           05  w-reg-cod-cab              pic  9(05) value zero       .
      ...
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "D"
                     ...
                     go to prn-str-ini-600.
       prn-str-ini-200.
      *              *-------------------------------------------------*
      *              * Se ordinamento per denominazione                *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start per denominazione in uppercase        *
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "DENKEY    "         to   f-key                  .
           move      rr-den-min           to   rf-axi-den-key         .
           move      "pgm/abi/fls/ioc/obj/iofaxi"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-mne-min           to   rf-axi-cod-mne         .
           move      "pgm/abi/fls/ioc/obj/iofaxi"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-axi-cod-abi         .
           move      "pgm/abi/fls/ioc/obj/iofaxi"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              * trattamento                                     *
      *              *-------------------------------------------------*
           move      zero                 to   w-reg-cod-abi          .
           move      zero                 to   w-reg-cod-cab          .
       prn-str-ini-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun istituto entro i limiti assegnati !"
                                          to   m-msg                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Se stampa senza agenzie : lettura [axi]         *
      *              *-------------------------------------------------*
           if        rr-snx-sta           not  = "S"
                     go to prn-let-seq-200.
      *              *-------------------------------------------------*
      *              * Se ultimo codice istituto trattato a zero let-  *
      *              * tura [axi]                                      *
      *              *-------------------------------------------------*
           if        w-reg-cod-abi        =    zero
                     go to prn-let-seq-200.
      *              *-------------------------------------------------*
      *              * Se ultimo codice agenzia trattato a zero let-   *
      *              * tura [axi]                                      *
      *              *-------------------------------------------------*
           if        w-reg-cod-cab        =    zero
                     go to prn-let-seq-200.
           go to     prn-let-seq-500.
       prn-let-seq-200.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [axi]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/abi/fls/ioc/obj/iofaxi"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Deviazione in funzione del tipo ord.    *
      *                      *-----------------------------------------*
           if        rr-tip-ord           =    "D"
                     ...
                     go to prn-let-seq-230.
       prn-let-seq-210.
      *                      *-----------------------------------------*
      *                      * Se ordinamento per denominazione        *
      *                      *-----------------------------------------*
           if        rf-axi-den-key       >    rr-den-max
                     move  "#"            to   w-cnt-prn-flg-sub
                     go to prn-let-seq-999
           else      go to prn-let-seq-300.
       prn-let-seq-220.
      *                      *-----------------------------------------*
      *                      * Se ordinamento per mnemonico            *
      *                      *-----------------------------------------*
           if        rf-axi-cod-mne       >    rr-mne-max
                     move  "#"            to   w-cnt-prn-flg-sub
                     go to prn-let-seq-999
           else      go to prn-let-seq-300.
       prn-let-seq-230.
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Selezione su denominazione              *
      *                      *-----------------------------------------*
           if        rf-axi-den-key       <    rr-den-min or
                     rf-axi-den-key       >    rr-den-max
                     go to prn-let-seq-200.
      *                      *-----------------------------------------*
      *                      * Selezione su mnemonico                  *
      *                      *-----------------------------------------*
           if        rf-axi-cod-mne       <    rr-mne-min or
                     rf-axi-cod-mne       >    rr-mne-max
                     go to prn-let-seq-200.
      *                      *-----------------------------------------*
      *                      * Selezione su codice                     *
      *                      *-----------------------------------------*
           if        rf-axi-cod-abi       <    rr-cod-min or
                     rf-axi-cod-abi       >    rr-cod-max
                     go to prn-let-seq-200.
       prn-let-seq-400.
      *                  *---------------------------------------------*
      *                  * Memorizzazione codice istituto in corso di  *
      *                  * trattamento                                 *
      *                  *---------------------------------------------*
           move      rf-axi-cod-abi       to   w-reg-cod-abi          .
      *                  *---------------------------------------------*
      *                  * Se stampa senza agenzie : uscita            *
      *                  *---------------------------------------------*
           if        rr-snx-sta           not  = "S"
                     go to prn-let-seq-999.
      *                  *---------------------------------------------*
      *                  * Inizializzazione codice agenzia in corso di *
      *                  * trattamento                                 *
      *                  *---------------------------------------------*
           move      zero                 to   w-reg-cod-cab          .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-axi-cod-abi       to   rf-axs-cod-abi         .
           ...
           call      s-pat               using f
                                               rf-axs                 .
      *                  *---------------------------------------------*
      *                  * Se errore di start si esce                  *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-let-seq-999.
       prn-let-seq-500.
      *              *-------------------------------------------------*
      *              * Se non primo giro : lettura [axs]               *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                      * Test su codice                          *
      *                      *-----------------------------------------*
           if        rf-axs-cod-abi       not  = w-reg-cod-abi
                     go to  prn-let-seq-600.
           go to     prn-let-seq-700.
       prn-let-seq-600.
      *                  *---------------------------------------------*
      *                  * Se fine agenzie per l'istituto              *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Se non e' mai stata selezionata una a-  *
      *                      * genzia per questo istituto si prosegue  *
      *                      * il ciclo, altrimenti si passa a legge-  *
      *                      * re l'istituto successivo                *
      *                      *-----------------------------------------*
           if        w-reg-cod-cab        =    zero
                     go to prn-let-seq-999
       ...
       prn-let-seq-700.
      *                  *---------------------------------------------*
      *                  * Memorizzazione codice agenzia in corso di   *
      *                  * trattamento                                 *
      *                  *---------------------------------------------*
           move      rf-axs-cod-cab       to   w-reg-cod-cab          .
       prn-let-seq-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
           move      rf-axi-cod-abi       to   w-rot-l01-cod-abi      .
       prn-cmp-rot-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      14                   to   p-pos                  .
           move      w-rot-l01-cod-abi    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Denominazione                               *
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se non deve stampare le agenzie             *
      *                  *---------------------------------------------*
           if        rr-snx-sta           =    "N"
                     go to  prn-liv-det-999.
      *                  *---------------------------------------------*
      *                  * Se il codice agenzia in corso di trattamen- *
      *                  * to e' a zero                                *
      *                  *---------------------------------------------*
           if        w-reg-cod-cab        =    zero
                     go to  prn-liv-det-999.
      ...
      *              *-------------------------------------------------*
           move      "ELENCO ISTITUTI DI CREDITO"
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      01                   to   p-pos                  .
           move      "Mnemonico    A.B.I.           Denominazione della 
      -              "banca             Denominazione di ricerca   "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
