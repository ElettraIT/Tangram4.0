      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio [axs]            *
      *                                                                *
      *                    Anagrafico Sportelli istituti di credito    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    INTERROGAZIONE SPORTELLI BANCARI    "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           ...
           05  rr-der-min                 pic  x(30)                  .
           05  rr-der-max                 pic  x(30)                  .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-200.
      *                  *---------------------------------------------*
      *                  * Denominazione di ricerca min                *
      *                  *---------------------------------------------*
           perform   acc-der-min-000      thru acc-der-min-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
       ...
       acc-ric-sel-300.
      *                  *---------------------------------------------*
      *                  * Denominazione di ricerca max                *
      *                  *---------------------------------------------*
           perform   acc-der-max-000      thru acc-der-max-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su sportelli istituti di credito"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Codice istituto ABI        *
      *    *-----------------------------------------------------------*
       acc-cod-abi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   w-cod-mne-abi-lin      .
           move      30                   to   w-cod-mne-abi-pos      .
           move      06                   to   w-cod-mne-abi-dln      .
      ...
      *                  *---------------------------------------------*
           move      w-let-arc-axi-den    to   rr-cod-abi-den         .
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Denominazione di ric. min  *
      *    *-----------------------------------------------------------*
       acc-der-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-der-min-999.
       acc-der-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-der-min             .
       acc-der-min-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-der-min-100.
       acc-der-min-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Denominazione di ric. max  *
      *    *-----------------------------------------------------------*
       acc-der-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-der-max-999.
       acc-der-max-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-der-max             .
       acc-der-max-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-cab-cab      .
           move      15                   to   w-cod-mne-cab-lin      .
           move      19                   to   w-cod-mne-cab-pos      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-cab-ope      .
           move      rr-cod-abi           to   w-cod-mne-cab-abi      .
           move      rr-cod-max           to   w-cod-mne-cab-cab      .
           move      15                   to   w-cod-mne-cab-lin      .
           move      51                   to   w-cod-mne-cab-pos      .
           move      zero                 to   w-cod-mne-cab-dln      .
           move      zero                 to   w-cod-mne-cab-dps      .
           move      "9B"                 to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-der-min             .
           move      spaces               to   rr-der-max             .
      ...
      *              *-------------------------------------------------*
       tdo-ric-sel-100.
      *                  *---------------------------------------------*
      *                  * Controllo su denominazione di ric. min - max*
      *                  *---------------------------------------------*
           if        rr-der-max           =    spaces
                     go to tdo-ric-sel-200.
           if        rr-der-max           not  < rr-der-min
                     go to tdo-ric-sel-200.
           move      "ME"                 to   v-ope                  .
           move      "La denominazione di ricerca massima non puo' esser
      -              "e inferiore a quella minima !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       ...
       tdo-ric-sel-200.
      *                  *---------------------------------------------*
      *                  * Controllo su mnemonico min - max            *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Regolarizzazione denominazione di ric. min - max*
      *              *-------------------------------------------------*
           if        rr-der-max           =    spaces
                     move  rr-der-min     to   rr-der-max             .
           move      rr-der-max           to   w-all-str-alf          .
           move      30                   to   w-all-str-lun          .
           perform   all-str-pad-000      thru all-str-pad-999        .
           move      w-all-str-alf        to   rr-der-max             .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Start per codice ABI - CAB                      *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "ABICAB    "         to   f-key                  .
           move      rr-cod-abi           to   rf-axs-cod-abi         .
           move      rr-cod-min           to   rf-axs-cod-cab         .
           move      "pgm/abi/fls/ioc/obj/iofaxs"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessuno Sportello bancario entro i limiti assegnat
      -              "i !"                to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [axs]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/abi/fls/ioc/obj/iofaxs"
                                          to   s-pat                  .
      ...
      *              * Test su codice ABI                              *
      *              *-------------------------------------------------*
           if        rf-axs-cod-abi       not = rr-cod-abi
                     go to qry-tst-max-900.
      *              *-------------------------------------------------*
      *              * Test su codice min - max                        *
      *              *-------------------------------------------------*
           if        rf-axs-cod-cab       <   rr-cod-min or
                     rf-axs-cod-cab       >   rr-cod-max
                     go to qry-tst-max-900.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Selezione su denominazione di ricerca           *
      *              *-------------------------------------------------*
           if        rf-axs-den-ric       <    rr-der-min or
                     rf-axs-den-ric       >    rr-der-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su mnemonico                          *
      *              *-------------------------------------------------*
           if        rf-axs-cod-mne       <    rr-mne-min or
                     rf-axs-cod-mne       >    rr-mne-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su codice                             *
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      18                   to   v-pos                  .             
           move      rf-axs-den-spt       to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-axs-cod-cab       to   w-mpn-cod-cab          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-cab'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "cod-cab"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           move      "N"                  to   s-tip                  .
           move      05                   to   s-car                  .
           move      zero                 to   s-dec                  .
