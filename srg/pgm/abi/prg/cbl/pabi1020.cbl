      ...
      *                                   Fase:    abi102              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 16/04/91    *
      *                       Ultima revisione:    NdK del 03/12/01    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pabi1021:        *
      *                                                                *
      *                    Stampa elenco Istituti di credito           *
      *                                                                *
      *================================================================*
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "per Codice          "                    .
      *        *-------------------------------------------------------*
      *        * Work per : No/Si stampa sportelli                     *
      *        *-------------------------------------------------------*
           05  w-exp-snx-sta.
               10  w-exp-snx-sta-num      pic  9(02)       value 2    .
               10  w-exp-snx-sta-lun      pic  9(02)       value 2    .
               10  w-exp-snx-sta-tbl.
      ...
      *                  * Denominazione min                           *
      *                  *---------------------------------------------*
           perform   acc-den-min-000      thru acc-den-min-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Denominazione max                           *
      *                  *---------------------------------------------*
           perform   acc-den-max-000      thru acc-den-max-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
                     go to acc-ric-sel-200.
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-500.
      *                  *---------------------------------------------*
      *                  * Si/No stampa agenzie                        *
      *                  *---------------------------------------------*
           perform   acc-snx-sta-000      thru acc-snx-sta-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su archivio istituti di credito"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
           ...
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Stampa anche gli sportelli :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-ric-sel-999.
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Denominazione minima       *
      *    *-----------------------------------------------------------*
       acc-den-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-den-min-999.
       acc-den-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-den-min             .
       acc-den-min-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-den-min-100.
       acc-den-min-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Denominazione massima      *
      *    *-----------------------------------------------------------*
       acc-den-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-den-max-999.
       acc-den-max-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-den-max             .
       acc-den-max-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-abi-cod      .
           move      15                   to   w-cod-mne-abi-lin      .
           move      19                   to   w-cod-mne-abi-pos      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-abi-ope      .
           move      rr-cod-max           to   w-cod-mne-abi-cod      .
           move      15                   to   w-cod-mne-abi-lin      .
           move      51                   to   w-cod-mne-abi-pos      .
           move      zero                 to   w-cod-mne-abi-dln      .
           move      zero                 to   w-cod-mne-abi-dps      .
           move      "9B"                 to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/No stampa sportelli     *
      *    *-----------------------------------------------------------*
       acc-snx-sta-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-sta-lun    to   v-car                  .
           move      w-exp-snx-sta-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-sta-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           if        rr-snx-sta           =    "N"
                     move  01             to   v-num
           else if   rr-snx-sta           =    "S"
                     move  02             to   v-num
           else      move  zero           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-sta-999.
       acc-snx-sta-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "N"            to   rr-snx-sta
           else if   v-num                =    02
                     move  "S"            to   rr-snx-sta
           else      move  spaces         to   rr-snx-sta             .
       acc-snx-sta-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       tdo-ric-sel-100.
      *                  *---------------------------------------------*
      *                  * Controllo su denominazione min - max        *
      *                  *---------------------------------------------*
           if        rr-den-max           =    spaces
                     go to tdo-ric-sel-200.
           if        rr-den-max           not  < rr-den-min
                     go to tdo-ric-sel-200.
           move      "ME"                 to   v-ope                  .
           move      "La denominazione massima non puo' essere inferiore
      -              " a quella minima !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Regolarizzazione denominazione min - max        *
      *              *-------------------------------------------------*
           if        rr-den-max           =    spaces
                     move  rr-den-min     to   rr-den-max             .
      *
           move      rr-den-max           to   w-all-str-alf          .
           move      40                   to   w-all-str-lun          .
           perform   all-str-pad-000      thru all-str-pad-999        .
           move      w-all-str-alf        to   rr-den-max             .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Regolarizzazione si/no stampa agenzie           *
      *              *-------------------------------------------------*
           if        rr-snx-sta           not  = "S" and
                     rr-snx-sta           not  = "N"
                     move  "N"            to   rr-snx-sta             .
       reg-ric-sel-999.
           exit.
