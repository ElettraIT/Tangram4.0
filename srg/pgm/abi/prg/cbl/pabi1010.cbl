      ...
      *                                   Fase:    abi101              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 29/03/90    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su archivio [axi]            *
      *                                                                *
      *                    Anagrafico Istituti di credito              *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   INTERROGAZIONE ISTITUTI DI CREDITO   "       .
      ...
      *    *-----------------------------------------------------------*
       01  w-sav.
      *        *-------------------------------------------------------*
      *        * Salvataggio valore variabile di i.p.c. 'fkselect'     *
      *        * proveniente dal livello precedente                    *
      *        *-------------------------------------------------------*
           05  w-sav-fky-slc              pic  x(04)                  .
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   15  filler             pic  x(30) value
                            "Per denominazione banca       "          .
                   15  filler             pic  x(30) value
                            "Per denominazione di ricerca  "          .
                   15  filler             pic  x(30) value
                            "Per mnemonico                 "          .
                   15  filler             pic  x(30) value
                            "Per codice                    "          .
      ...
      *              *-------------------------------------------------*
       ...
       pre-tip-fun-200.
      *              *-------------------------------------------------*
      *              * Se variabile di i.p.c. per denominazione di ric.*
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-ipc-dlp-tdi-ddr    to   rr-der-min             .
           move      w-ipc-dlp-tdi-ddr    to   rr-der-max             .
      ...
      *                  *---------------------------------------------*
       ...
       qry-str-ini-300.
      *              *-------------------------------------------------*
      *              * Se ordinamento per denominazione di ricerca     *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start per denominazione in uppercase        *
      *                  *---------------------------------------------*
           ...
           move      rr-der-min           to   rf-axi-den-ric         .
           move      zero                 to   rf-axi-cod-abi         .
           move      "pgm/abi/fls/ioc/obj/iofaxi"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
           if        rf-axi-den-ric       <    rr-der-min or
                     rf-axi-den-ric       >    rr-der-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "+"                  to   v-edm                  .
           move      rf-axi-cod-abi       to   w-mpn-cod-abi          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-det-fky-000.
      *              *-------------------------------------------------*
      *              * Lettura variabile di i.p.c. 'fkselect' prove-   *
      *              * niente dal livello precedente                   *
      *              *-------------------------------------------------*
           move      "GV"                 to   s-ope                  .
           ...
           if        s-ves                =    spaces and
                     s-alf                =    "SLCT"
                     move   "SLCT"        to   w-sav-fky-slc
           else      move   spaces        to   w-sav-fky-slc          .
      *              *-------------------------------------------------*
      *              * Determinazione eventuale function key Slct      *
      *              *-------------------------------------------------*
           if        w-sav-fky-slc        =    "SLCT"
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-abi'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Preparazione variabile di i.p.c. per il co- *
      *                  * dice istituto interessato                   *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "cod-abi"            to   s-var                  .
           move      "+"                  to   s-dop                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                  *---------------------------------------------*
      *                  * Preparazione variabile di i.p.c. per possi- *
      *                  * bilita' di function-key "SLCT" durante      *
      *                  * l'interrogazione                            *
      *                  *---------------------------------------------*
           if        w-sav-fky-slc        not  = "SLCT"
       ...
       qry-trt-fun-700.
      *                  *---------------------------------------------*
      *                  * Richiamo programma di interrogazione agenzie*
      *                  *---------------------------------------------*
           move      "pgm/abi/prg/obj/pabi2010"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat                                            .
           cancel    s-pat                                            .
      *                  *---------------------------------------------*
      *                  * Se e' stata operata una selezione la si gi- *
      *                  * ra al livello precedente                    *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Lettura variabile da livello successivo *
      *                      *-----------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      *                      *-----------------------------------------*
      *                      * Se non esistente : nessuna azione       *
      *                      *-----------------------------------------*
           if        s-ves                not  = spaces
                     go to qry-trt-fun-999.
      *                      *-----------------------------------------*
      *                      * Scrittura variabili a livello preceden- *
      *                      * te                                      *
      *                      *-----------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "cod-cab"            to   s-var                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Mnemonico  Codice         Denominazione della banc
      -              "a         Denomin. di ricerca "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   w-ipc-dlp-tdi-ddr      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "den-ric" per la denominazione di ricerca       *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           move      s-alf                to   w-ipc-dlp-tdi-ddr      .
           go to     ipc-dlp-tdi-999.
       ipc-dlp-tdi-999.
           exit.
