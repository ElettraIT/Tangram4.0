      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pbil4001:        *
      *                                                                *
      *                    Stampa bilancio                             *
      *                                                                *
      *================================================================*
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-200.
      *                  *---------------------------------------------*
      *                  * Data riferimento bilancio 1 periodo         *
      *                  *---------------------------------------------*
           perform   acc-dat-1pr-000      thru acc-dat-1pr-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
       ...
       acc-ric-sel-300.
      *                  *---------------------------------------------*
      *                  * Data riferimento bilancio 2 periodo         *
      *                  *---------------------------------------------*
           perform   acc-dat-2pr-000      thru acc-dat-2pr-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      07                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Data bilancio 1. periodo   :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Data bilancio 2. periodo                        *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Memorizzazione stampa anche voci a zero     *
      *                  *---------------------------------------------*
           move      w-let-arc-ztb-snz    to   rr-tip-bil-snz         .
       acc-tip-bil-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Data bilancio 1. periodo   *
      *    *-----------------------------------------------------------*
       acc-dat-1pr-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-1pr-999.
       acc-dat-1pr-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-1pr             .
       acc-dat-1pr-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *
      *                  *---------------------------------------------*
           if        rr-dat-1pr           not  = zero
                     go to acc-dat-1pr-600.
           if        v-key                =    "UP  "
                     go to acc-dat-1pr-600
           else      go to acc-dat-1pr-100.
       acc-dat-1pr-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-dat-1pr-100.
       acc-dat-1pr-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Data bilancio 2. periodo   *
      *    *-----------------------------------------------------------*
       acc-dat-2pr-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-dat-2pr-999.
       acc-dat-2pr-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   rr-dat-2pr             .
       acc-dat-2pr-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rf-ztb-snx-vaz       to   w-let-arc-ztb-snz      .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       let-arc-ztb-520.
           move      spaces               to   w-let-arc-ztb-snz      .
       let-arc-ztb-999.
           exit.
      ...
      *              *-------------------------------------------------*
      ...
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Controllo : Data bilancio 1. periodo            *
      *              *-------------------------------------------------*
           if        rr-dat-1pr           not  = zero
                     go to tdo-ric-sel-300.
           move      "Manca la data di bilancio per il 1. periodo       
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-300.
      *              *-------------------------------------------------*
      *              * Determinazione se esiste una serie di valori di *
      *              * bilancio riferita alla data del 1. periodo      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start su file [vbv]                         *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
      ...
      -              "                        "
           ...
           move      12                   to   v-lin                  .
           move      04                   to   v-pos                  .
           move      "Non esiste alcuna serie di valori di bilancio rife
      -              "riti alla data del pri- "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      -              "                        "
           ...
           move      16                   to   v-lin                  .
           move      04                   to   v-pos                  .
           move      "Si vuole comunque procedere nella stampa del bilan
      -              "cio (S/N) ? [ ]         "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *                      *-----------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Accettazione risposta dell'operatore    *
      *                      *-----------------------------------------*
       ...
       tdo-ric-sel-320.
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * Eventuale normalizzazione risposta a    *
      *                      * seconda della function key              *
      *                      *-----------------------------------------*
           if        v-key                =    "EXIT"
                     move  "N"            to   v-alf
           else if   v-key                =    "DO   "
                     move  "S"            to   v-alf                  .
      *                      *-----------------------------------------*
      *                      * Test su validita della risposta         *
      *                      *-----------------------------------------*
           if        v-alf                not  = "S" and
                     v-alf                not  = "N"
                     go to tdo-ric-sel-320.
      *                      *-----------------------------------------*
      *                      * Se il caso: flag di uscita ad errore    *
      *                      *-----------------------------------------*
           if        v-alf                =    "N"
      ...
      *              *-------------------------------------------------*
      *              * Determinazione se esiste una serie di valori di *
      *              * bilancio riferita alla data del 2. periodo      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-tip-bil-snz         .
           move      zero                 to   rr-dat-1pr             .
           move      zero                 to   rr-dat-2pr             .
       nor-ric-sel-999.
           exit.
