      ...
      *                       Ultima revisione:    NdK del 02/05/02    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pfab4351:        *
      *                                                                *
      *                    Determinazione scorta minima prodotti di    *
      *                    vendita da ordini clienti                   *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " DETERMINAZIONE SCORTA MINIMA DA ORDINI "       .
      ...
      *        *-------------------------------------------------------*
      *        * Percentuale di variazione media mensile               *
      *        *-------------------------------------------------------*
           05  rr-pdv-mme                 pic s9(02)                  .
      *        *-------------------------------------------------------*
           ...
           05  w-exp-tip-slp.
               ...
               10  w-exp-tip-slp-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "ad ogni Sottogruppo "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Aggiornamento anagrafica                   *
      *        *-------------------------------------------------------*
           05  w-exp-agg-ana.
               10  w-exp-agg-ana-num      pic  9(02)       value 4    .
               10  w-exp-agg-ana-lun      pic  9(02)       value 30   .
               10  w-exp-agg-ana-tbl.
                   15  filler             pic  x(30) value
                            "No                            "          .
                   15  filler             pic  x(30) value
                            "Si                            "          .
                   15  filler             pic  x(30) value
                            "solo se precedentemente a Zero"          .
                   15  filler             pic  x(30) value
                            "Azzeramento valore precedente "          .
      ...
      *                  * Percentuale di variazione media mensile     *
      *                  *---------------------------------------------*
           perform   acc-pdv-mme-000      thru acc-pdv-mme-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Numero clienti minimo da considerare        *
      *                  *---------------------------------------------*
           perform   acc-ncl-mdc-000      thru acc-ncl-mdc-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Aggiornamento anagrafica                    *
      *                  *---------------------------------------------*
           perform   acc-agg-ana-000      thru acc-agg-ana-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
           ...
           move      14                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Percentuale di variazione  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
           ...
           move      15                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "  quantita' vendute medie   "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Numero clienti movimentati minimo           *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
           move      17                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Numero clienti movimentati :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *                  * Percentuale di variazione                   *
      *                  *---------------------------------------------*
           move      zero                 to   rr-pdv-mme             .
           perform   vis-pdv-mme-000      thru vis-pdv-mme-999        .
      *                  *---------------------------------------------*
      *                  * Numero clienti movimentati minimo           *
      *                  *---------------------------------------------*
           move      03                   to   rr-ncl-mdc             .
           perform   vis-ncl-mdc-000      thru vis-ncl-mdc-999        .
      *                  *---------------------------------------------*
      *                  * Aggiornamento anagrafica                    *
      *                  *---------------------------------------------*
           move      01                   to   rr-agg-ana             .
           perform   vis-agg-ana-000      thru vis-agg-ana-999        .
       pmt-ric-sel-999.
           exit.
      ...
      *    *===========================================================*
      *    * Accettazione : Data riferimento iniziale                  *
      *    *-----------------------------------------------------------*
       acc-dat-ini-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        rr-dat-ini           not  = zero
                     go to acc-dat-ini-500.
           if        v-key                =    "UP  "
                     go to acc-dat-ini-600
           else      go to acc-dat-ini-100.
       acc-dat-ini-500.
      *                  *---------------------------------------------*
      *                  * Controllo che la data impostata sia di ini- *
      *                  * zio mese                                    *
      *                  *---------------------------------------------*
      ...
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "La data di riferimento iniziale deve essere di ini
      -              "zio mese       "    to   w-err-box-err-msg      .
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data riferimento iniziale               *
      *    *-----------------------------------------------------------*
       vis-dat-ini-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data riferimento finale                    *
      *    *-----------------------------------------------------------*
       acc-dat-fin-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "La data di riferimento finale deve essere di fine 
      -              "mese           "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
      *                      *-----------------------------------------*
      *                      * Reimpostazione                          *
      *                      *-----------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data riferimento finale                 *
      *    *-----------------------------------------------------------*
       vis-dat-fin-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Percentuale di variazione                  *
      *    *-----------------------------------------------------------*
       acc-pdv-mme-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-pdv-mme-999.
       acc-pdv-mme-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-pdv-mme             .
       acc-pdv-mme-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-pdv-mme-100.
       acc-pdv-mme-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione : Percentuale di variazione               *
      *    *-----------------------------------------------------------*
       vis-pdv-mme-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Numero clienti movimentati minimo          *
      *    *-----------------------------------------------------------*
       acc-ncl-mdc-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-ncl-mdc-999.
       acc-ncl-mdc-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-ncl-mdc             .
       acc-ncl-mdc-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Numero clienti movimentati minimo       *
      *    *-----------------------------------------------------------*
       vis-ncl-mdc-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Aggiornamento anagrafica   *
      *    *-----------------------------------------------------------*
       acc-agg-ana-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-agg-ana-lun    to   v-car                  .
           move      w-exp-agg-ana-num    to   v-ldt                  .
           move      "NSZA#"              to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      20                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-agg-ana-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
           move      rr-agg-ana           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-agg-ana-999.
       acc-agg-ana-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-agg-ana             .
       acc-agg-ana-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * sia in Up                                   *
      *                  *---------------------------------------------*
           if        rr-agg-ana           not  = zero
                     go to acc-agg-ana-600.
           if        v-key                =    "UP  "
                     go to acc-agg-ana-600
           else      go to acc-agg-ana-999.
       acc-agg-ana-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-agg-ana-100.
       acc-agg-ana-999.
           exit.

      *    *===========================================================*
      *    * Visualizzaz. campo selezione : Aggiornamento anagrafica   *
      *    *-----------------------------------------------------------*
       vis-agg-ana-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      *              * Test su data iniziale                           *
      *              *-------------------------------------------------*
           if        rr-dat-ini           not  = zero
                     go to tdo-ric-sel-100.
           move      "Manca la data di riferimento iniziale             
      ...
      *              *-------------------------------------------------*
           ...
           move      "Manca la data di riferimento finale               
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-120.
      *              *-------------------------------------------------*
      *              * Test di congruenza tra data iniziale e finale   *
      *              *-------------------------------------------------*
           if        rr-dat-fin           not  < rr-dat-ini
                     go to tdo-ric-sel-140.
           move      "La data di riferimento finale inferiore a quella i
      ...
      *              *-------------------------------------------------*
           if        rr-tip-slp           =    zero
                     move  01             to   rr-tip-slp             .
      *              *-------------------------------------------------*
      *              * Normalizzazione numero clienti minimo           *
      *              *-------------------------------------------------*
           if        rr-ncl-mdc           =    zero
                     move  01             to   rr-ncl-mdc             .
      *              *-------------------------------------------------*
      *              * Normalizzazione aggiornamento anagrafica        *
      *              *-------------------------------------------------*
           if        rr-agg-ana           =    zero
                     move  01             to   rr-agg-ana             .
       reg-ric-sel-999.
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      zero                 to   rr-dat-fin             .
           move      zero                 to   rr-pdv-mme             .
           move      zero                 to   rr-ncl-mdc             .
           move      zero                 to   rr-agg-ana             .
       nor-ric-sel-999.
           exit.
