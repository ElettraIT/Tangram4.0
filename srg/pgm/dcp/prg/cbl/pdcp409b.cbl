      ...
      *                                   Fase:    dcp409              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 11/03/03    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Stampe di controllo anagrafica prodotti     *
      *                                                                *
      *                    Sottoprogramma per la stampa dei prodotti   *
      *                    per cui manca il codice selezionato tra i   *
      *                    seguenti:                                   *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " CONTROLLO CODICI MANCANTI NEI PRODOTTI "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
      *        *-------------------------------------------------------*
      *        * Tipo codice da controllare                            *
      *        *  - 01 : Codice classe merceologica                    *
      *        *  - 02 : Codice statistico 1                           *
      *        *  - 03 : Codice statistico 1                           *
      *        *  - 04 : Codice statistico 1                           *
      *        *  - 05 : Codice categoria sonto in riga                *
      *        *  - 06 : Codice nomenclatura combinata                 *
      *        *-------------------------------------------------------*
           05  rr-tip-cod                 pic  9(02)                  .
      *        *-------------------------------------------------------*
      *        * Si/no solo prodotti senza codice                      *
      *        *  - 01 : Si                                            *
      *        *  - 02 : No                                            *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Tipo codice da controllare                 *
      *        *-------------------------------------------------------*
           05  w-exp-tip-cod.
               10  w-exp-tip-cod-num      pic  9(02)       value 06   .
               10  w-exp-tip-cod-lun      pic  9(02)       value 40   .
               10  w-exp-tip-cod-tbl.
                   ...
                   15  filler             pic  x(40) value
                            "Codice nomenclatura combinata           ".
      *        *-------------------------------------------------------*
      *        * Work per : Si/no solo prodotti senza codice           *
      *        *-------------------------------------------------------*
           05  w-exp-snx-ssc.
               10  w-exp-snx-ssc-num      pic  9(02)       value 02   .
               10  w-exp-snx-ssc-lun      pic  9(02)       value 02   .
               10  w-exp-snx-ssc-tbl.
      ...
      *                  * Tipo codice da controllare                  *
      *                  *---------------------------------------------*
           perform   acc-tip-cod-000      thru acc-tip-cod-999        .
      ...
      *                  * Si/no solo prodotti senza codice            *
      *                  *---------------------------------------------*
           perform   acc-snx-ssc-000      thru acc-snx-ssc-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *===========================================================*
      *    * Accettazione campo : Tipo codice da controllare           *
      *    *-----------------------------------------------------------*
       acc-tip-cod-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-tip-cod-lun    to   v-car                  .
           move      w-exp-tip-cod-num    to   v-ldt                  .
           move      spaces               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      09                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-tip-cod-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-tip-cod           to   v-num                  .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-tip-cod-999.
       acc-tip-cod-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-tip-cod             .
       acc-tip-cod-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-tip-cod           =    zero
                     go to acc-tip-cod-100.
       acc-tip-cod-600.
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no solo prodotti senza codice     *
      *    *-----------------------------------------------------------*
       acc-snx-ssc-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-ssc-lun    to   v-car                  .
           move      w-exp-snx-ssc-num    to   v-ldt                  .
           move      "SN#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-ssc-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-snx-ssc           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-ssc-999.
       acc-snx-ssc-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-ssc             .
       acc-snx-ssc-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-ssc           =    zero
                     go to acc-snx-ssc-100.
       acc-snx-ssc-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      05                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Questo programma seleziona e stampa i prodotti per
      -              " i quali manca il codice      "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo su tipo codice da controllare         *
      *              *-------------------------------------------------*
           if        rr-tip-cod           not  < 01 and
                     rr-tip-cod           not  > 06
                     go to tdo-ric-sel-200.
           move      "Tipo codice da controllare errato !               
      ...
      *              *-------------------------------------------------*
       ...
       stp-srt-inp-200.
      *              *-------------------------------------------------*
      *              * Selezione in funzione del tipo codice           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se stampa tutti i prodotti             *
      *                  *---------------------------------------------*
           if        rr-snx-ssc           =    02
                     go to stp-srt-inp-300.
      *                  *---------------------------------------------*
      *                  * Deviazione secondo il tipo codice           *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-310.
           move      rf-dcp-cla-pro       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-320.
           move      rf-dcp-cod-s01       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-330.
           move      rf-dcp-cod-s02       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-340.
           move      rf-dcp-cod-s03       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-350.
           move      rf-dcp-cat-scr       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-360.
           move      rf-dcp-cdn-cdm       to   srt-cod-exi            .
           go to     stp-srt-inp-400.
       stp-srt-inp-400.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun prodotto con codici mancanti trovato!"
                                          to   m-msg                  .
           call      "swd/mod/prg/obj/mmessg"
                                         using m                      .
       prn-nes-ela-999.
      ...
      *                  * Test se da stampare                         *
      *                  *---------------------------------------------*
           if        rr-snx-ssc           not  = 02
                     go to prn-liv-det-900.
           if        srt-cod-exi          =    zero
                     go to prn-liv-det-900.
      *                  *---------------------------------------------*
      *                  * Editing codice errore                       *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      08                   to   p-car                  .
      ...
      *              *-------------------------------------------------*
           go to     int-pag-sta-010
                     ...
                     depending   on rr-tip-cod.
       int-pag-sta-010.
           move      "PRODOTTI MANCANTI DELLA CLASSE MERCEOLOGICA"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-020.
           move      "PRODOTTI MANCANTI DEL CODICE STATISTICO 1"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-030.
           move      "PRODOTTI MANCANTI DEL CODICE STATISTICO 2"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-040.
           move      "PRODOTTI MANCANTI DEL CODICE STATISTICO 3"
                                          to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-050.
           move      "PRODOTTI MANCANTI DEI CODICI CATEGORIA SCONTO IN R
      -              "IGA"                to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-060.
           move      "PRODOTTI MANCANTI DEI CODICI NOMENCLATURA COMBINAT
      -              "A"                  to   w-cnt-tit-des-tit      .
           go to     int-pag-sta-090.
       int-pag-sta-090.
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
