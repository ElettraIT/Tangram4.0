      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione di un intero listino               *
      *                                                                *
      *                    Sottoprogramma per esecuzione funzione di : *
      *                                                                *
      *                    Manutenzione prezzi, sconti e provvigioni   *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     " GESTIONE PREZZI, SCONTI E PROVVIGIONI  "       .
      ...
      *            *---------------------------------------------------*
      *            * Numero decimali per codice valuta                 *
      *            *---------------------------------------------------*
               10  w-tes-cod-vlt-dec      pic  9(01)                  .
      *            *---------------------------------------------------*
      ...
      *                *-----------------------------------------------*
                   ...
                   15  filler             pic  x(01)                  .
                   15  w-lin-imm-err-dec  pic  x(01)                  .
                   15  filler             pic  x(03)                  .
                   15  w-lin-imm-des-pro  pic  x(30)                  .
                   15  filler             pic  x(01)                  .
                   15  w-lin-imm-per-sco  pic  x(04)                  .
                   15  filler             pic  x(01)                  .
                   15  w-lin-imm-per-pvg  pic  x(04)                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per : Filtro selezione prodotti   *
      *    *-----------------------------------------------------------*
       pmt-fso-dcp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se codice listino diverso da spaces o codi- *
      *                  * ce valuta diverso da spaces : direttamente  *
      *                  * ad accettazione                             *
      *                  *---------------------------------------------*
           if        w-tes-cod-lst        not  = spaces or
                     w-tes-cod-vlt        not  = spaces
      ...
      *                      *-----------------------------------------*
           ...
           move      w-let-arc-zls-vlt    to   w-tes-cod-vlt          .
      *                      *-----------------------------------------*
      *                      * Per valuta                              *
      *                      *-----------------------------------------*
           ...
           move      w-let-arc-zvl-dec    to   w-tes-cod-vlt-dec      .
      *                  *---------------------------------------------*
      *                  * Pre-visualizzazioni                         *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           perform   vis-des-vlt-000      thru vis-des-vlt-999        .
      *                  *---------------------------------------------*
      *                  * Se codice listino non esistente oppure co-  *
      *                  * dice valuta non esistente : reimpostazione  *
      *                  *---------------------------------------------*
           if        w-let-arc-zls-flg    not  = spaces or
                     w-let-arc-zvl-flg    not  = spaces
                     go to acc-cod-lst-100.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       vis-lin-cor-460.
      *                  *---------------------------------------------*
      *                  * Editing Prezzo di listino impostato         *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "V"                  to   v-tip                  .
           move      09                   to   v-car                  .
      ...
      *                  * li valuta                                   *
      *                  *---------------------------------------------*
           if        w-rig-sgl-vlt        =    w-tes-cod-vlt     and
                     w-rig-dec-vlt        =    w-tes-cod-vlt-dec
      ...
      *                  *---------------------------------------------*
           ...
           move      spaces               to   v-sgn                  .
           move      "BD"                 to   v-edm                  .
           move      w-rig-per-sco (1)    to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-lin-imm-per-sco      .
       vis-lin-cor-540.
      *                  *---------------------------------------------*
      *                  * Editing % di provvigione 1                  *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-lin-imm-per-pvg      .
       vis-lin-cor-800.
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      " N.R.   Cod. prodotto        Prezzo         Descri
      -              "zione prodotto       %Sc. %Pv."
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      ...
      *                  * uscita                                      *
      *                  *---------------------------------------------*
           if        w-rig-sgl-vlt        not  = w-tes-cod-vlt     or
                     w-rig-dec-vlt        not  = w-tes-cod-vlt-dec
      ...
      *                      * : uscita                                *
      *                      *-----------------------------------------*
           if        w-rig-dec-vlt        not  = w-tes-cod-vlt-dec
      ...
      *              * Numero decimali per codice valuta               *
      *              *-------------------------------------------------*
           move      zero                 to   w-tes-cod-vlt-dec      .
      *              *-------------------------------------------------*
      *              * Codice filtro di selezione-ordinamento [dcp]    *
      *              *-------------------------------------------------*
           move      zero                 to   w-tes-fso-dcp          .
      ...
      *
           ...
           move      w-tes-cod-vlt        to   w-det-pbs-sgl-vlt      .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-tes-cod-vlt        to   rf-lst-sgl-vlt         .
      ...
      *                      * listino di testata                      *
      *                      *-----------------------------------------*
           move      w-tes-cod-vlt-dec    to   rf-lst-dec-vlt         .
      *                      *-----------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/dcp/prg/obj/pdcp3202"                       .
       cnc-rlt-sup-999.
           exit.
