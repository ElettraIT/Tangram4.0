      ...
      *                                                                *
      *                    Manutenzione prezzi storicizzati            *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     "GESTIONE PREZZI DI LISTINO STORICIZZATI "       .
      ...
      *        *-------------------------------------------------------*
      *        * Data validita' pre-impostazione                       *
      *        *-------------------------------------------------------*
           05  w-rig-pre-dvf              pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * Prezzo di listino storico                             *
      *        *-------------------------------------------------------*
           05  w-rig-sto-lst              pic  9(09)                  .
      *        *-------------------------------------------------------*
      *        * Data di validita' finale storico                      *
      *        *-------------------------------------------------------*
           05  w-rig-sto-dvf              pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * % di provvigione                                      *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : No/Si cancellazione                        *
      *        *-------------------------------------------------------*
           05  w-exp-del-snx.
               10  w-exp-del-snx-num      pic  9(02)       value 2    .
               10  w-exp-del-snx-lun      pic  9(02)       value 02   .
               10  w-exp-del-snx-tbl.
      ...
      *                  *---------------------------------------------*
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Accettazione Prezzo di listino storico      *
      *                  *---------------------------------------------*
           perform   acc-sto-lst-000      thru acc-sto-lst-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
                     go to acc-cor-reg-900.
       acc-cor-reg-300.
      *                  *---------------------------------------------*
      *                  * Data validita' finale storico               *
      *                  *---------------------------------------------*
           perform   acc-sto-dvf-000      thru acc-sto-dvf-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
                     go to acc-cor-reg-900.
           if        v-key                =    "UP  "
      ...
      *                  * Si/no cancellazione                         *
      *                  *---------------------------------------------*
           perform   acc-del-snx-000      thru acc-del-snx-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
                     go to acc-cor-reg-900.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
           if        w-rig-del-snx        =    "S"
                     move  "*"            to   w-lin-imm-del-snx
           else      move  spaces         to   w-lin-imm-del-snx      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       vis-lin-cor-460.
      *                  *---------------------------------------------*
      *                  * Editing Prezzo di listino storico           *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "V"                  to   v-tip                  .
           move      09                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
      *                  * Editing Data validita' finale storico       *
      *                  *---------------------------------------------*
           move      "ED"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
           move      w-rig-sto-dvf        to   v-dat                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-lin-imm-sto-dvf      .
       vis-lin-cor-800.
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      " N.R.   Cod. prodotto   Prezzo storico       Descr
      -              "izione prodotto      Validita'"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      ...
      *              * Prezzo di listino storico                       *
      *              *-------------------------------------------------*
           perform   vis-sto-lst-000      thru vis-sto-lst-999        .
      *              *-------------------------------------------------*
      *              * Valuta per il prezzo su listino storico         *
      *              *-------------------------------------------------*
           perform   vis-sto-lst-vlt-000  thru vis-sto-lst-vlt-999    .
      *              *-------------------------------------------------*
      *              * Data di validita' finale per il prezzo su li-   *
      *              * stino storico                                   *
      *              *-------------------------------------------------*
           perform   vis-sto-dvf-000      thru vis-sto-dvf-999        .
      *              *-------------------------------------------------*
      *              * Prezzo di listino                               *
      *              *-------------------------------------------------*
           perform   vis-prz-lst-000      thru vis-prz-lst-999        .
      *              *-------------------------------------------------*
      *              * Valuta per il prezzo su listino                 *
      *              *-------------------------------------------------*
      ...
      *              * Si/no cancellazione                             *
      *              *-------------------------------------------------*
           perform   vis-del-snx-000      thru vis-del-snx-999        .
       vis-rig-cor-999.
           exit.
      ...
      *              *-------------------------------------------------*
           perform   pmt-sto-lst-000      thru pmt-sto-lst-999        .
      *              *-------------------------------------------------*
      *              * Data di validita' finale storico                *
      *              *-------------------------------------------------*
           perform   pmt-sto-dvf-000      thru pmt-sto-dvf-999        .
      *              *-------------------------------------------------*
      *              * Prezzo di listino                               *
      *              *-------------------------------------------------*
      ...
      *              * Si/no cancellazione                             *
      *              *-------------------------------------------------*
           perform   pmt-del-snx-000      thru pmt-del-snx-999        .
       pmt-rig-cor-999.
           exit.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Data di validita' finale storico *
      *    *-----------------------------------------------------------*
       pmt-sto-dvf-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Si/no cancellazione              *
      *    *-----------------------------------------------------------*
       pmt-del-snx-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           if        w-rig-sgl-vlt        not  = w-tes-sgl-vlt     or
                     ...
                     go to acc-sto-lst-999.
       acc-sto-lst-050.
      *                  *---------------------------------------------*
      *                  * Salvataggio valore pre-accettazione         *
      *                  *---------------------------------------------*
           move      w-rig-sto-lst        to   w-sav-prz-lst          .
       acc-sto-lst-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Prezzo di listino storico         *
      *    *-----------------------------------------------------------*
       vis-sto-lst-000.
           move      "DS"                 to   v-ope                  .
           move      "V"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data di validita' finale storico           *
      *    *-----------------------------------------------------------*
       acc-sto-dvf-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-sto-dvf-999.
       acc-sto-dvf-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   w-rig-sto-dvf          .
       acc-sto-dvf-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *
      *                  *---------------------------------------------*
           if        w-rig-sto-dvf        not  = zero
                     go to acc-sto-dvf-600.
           if        v-key                =    "UP  "
                     go to acc-sto-dvf-600
           else      go to acc-sto-dvf-100.
       acc-sto-dvf-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-rig-flg
                             ...
                             go to acc-sto-dvf-100.
       acc-sto-dvf-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione : Data di validita' finale storico        *
      *    *-----------------------------------------------------------*
       vis-sto-dvf-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no cancellazione                  *
      *    *-----------------------------------------------------------*
       acc-del-snx-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-del-snx-lun    to   v-car                  .
           move      w-exp-del-snx-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      21                   to   v-lin                  .
           move      78                   to   v-pos                  .
           move      w-exp-del-snx-tbl    to   v-txt                  .
      *
           if        w-rig-del-snx        =    "N"
                     move  1              to   v-num
           else if   w-rig-del-snx        =    "S"
                     move  2              to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-del-snx-999.
       acc-del-snx-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    1
                     move  "N"            to   w-rig-del-snx
           else if   v-num                =    2
                     move  "S"            to   w-rig-del-snx
           else      move  spaces         to   w-rig-del-snx          .
       acc-del-snx-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-rig-flg
                             ...
                             go to acc-del-snx-100.
       acc-del-snx-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Si/no cancellazione               *
      *    *-----------------------------------------------------------*
       vis-del-snx-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      "Manca la data di validita' finale                 
      ...
      *              *-------------------------------------------------*
      *              * Data di validita' pre-impostazione              *
      *              *-------------------------------------------------*
           move      zero                 to   w-rig-pre-dvf          .
      *              *-------------------------------------------------*
      *              * Prezzo di listino storico                       *
      *              *-------------------------------------------------*
           move      zero                 to   w-rig-sto-lst          .
      *              *-------------------------------------------------*
      *              * Data di validita' finale storico                *
      *              *-------------------------------------------------*
           move      zero                 to   w-rig-sto-dvf          .
      *              *-------------------------------------------------*
      *              * % di provvigione                                *
      *              *-------------------------------------------------*
      ...
      *                  * Messaggio di errore                         *
      *                  *---------------------------------------------*
           move      "Nessun prezzo storico da trattare !               
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      24                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "        --- Caricamento prezzi di listino storiciz
      -              "zati in esecuzione ---        "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *              *-------------------------------------------------*
       ...
       car-fil-rap-200.
      *              *-------------------------------------------------*
      *              * Per ogni prodotto si scaricano gli storici      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-tes-cod-lst        to   rf-lsd-cod-lst         .
           move      zero                 to   rf-lsd-cod-cli         .
           move      w-tes-sgl-vlt        to   rf-lsd-sgl-vlt         .
           move      rf-dcp-num-pro       to   rf-lsd-num-pro         .
      ...
      *                  *---------------------------------------------*
           if        rf-lsd-tip-rec       not  = 01             or
                     rf-lsd-cod-lst       not  = w-tes-cod-lst  or
                     rf-lsd-cod-cli       not  = zero           or
                     rf-lsd-sgl-vlt       not  = w-tes-sgl-vlt  or
                     rf-lsd-num-pro       not  = rf-dcp-num-pro
                     go to car-fil-rap-850.
       car-fil-rap-240.
      *              *-------------------------------------------------*
      ...
      *
           ...
           move      zero                 to   w-det-pbs-max-sto      .
      ...
      *                      * Prezzo                                  *
      *                      *-----------------------------------------*
           move      rf-lsd-prz-lst       to   w-rig-sto-lst          .
      *                      *-----------------------------------------*
      *                      * Data di validita' finale                *
      *                      *-----------------------------------------*
           move      rf-lsd-dva-fin       to   w-rig-sto-dvf          .
      *                      *-----------------------------------------*
      *                      * Data di validita' pre-impostazione      *
      *                      *-----------------------------------------*
           move      rf-lsd-dva-fin       to   w-rig-pre-dvf          .
       car-fil-rap-752.
      *                      *-----------------------------------------*
      *                      * Numero prezzi storici trovati           *
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       car-fil-rap-820.
      *              *-------------------------------------------------*
      *              * Riciclo a Read Next su [lsd]                    *
      *              *-------------------------------------------------*
           go to     car-fil-rap-220.
       car-fil-rap-850.
      *              *-------------------------------------------------*
      *              * Riciclo a Read Next su [dcp]                    *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      24                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "       --- Aggiornamento prezzi di listino storici
      -              "zzati in esecuzione ---       "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *              *-------------------------------------------------*
       ...
       sca-fil-rap-120.
      *              *-------------------------------------------------*
      *              * Test per eventuale cancellazione                *
      *              *-------------------------------------------------*
           if        w-rig-del-snx        not  = "S"
                     go to sca-fil-rap-150
           else      go to sca-fil-rap-200.
       sca-fil-rap-150.
      *              *-------------------------------------------------*
      *              * Se data' di validita' finale impostata, pari a  *
      *              * quella pre-impostazione : si ignora il record e *
      *              * si passa a quello successivo                    *
      *              *-------------------------------------------------*
           if        w-rig-sto-dvf        =    w-rig-pre-dvf
      ...
      *                  *---------------------------------------------*
           ...
           move      zero                 to   rf-lsd-cod-cli         .
           move      w-tes-sgl-vlt        to   rf-lsd-sgl-vlt         .
           move      w-rig-num-pro        to   rf-lsd-num-pro         .
           move      w-rig-pre-dvf        to   rf-lsd-dva-fin         .
           move      "pgm/dcp/fls/ioc/obj/ioflsd"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
           move      rf-lsd               to   w-sav-rec-lsd          .
      *                      *-----------------------------------------*
      *                      * Cancellazione record letto              *
      *                      *-----------------------------------------*
           move      "DE"                 to   f-ope                  .
           move      "pgm/dcp/fls/ioc/obj/ioflsd"
                                          to   s-pat                  .
      ...
      *                      *-----------------------------------------*
           ...
           call      s-pat               using f
                                               rf-lsd                 .
      *                      *-----------------------------------------*
      *                      * Ripristino record letto                 *
      *                      *-----------------------------------------*
           move      w-sav-rec-lsd        to   rf-lsd                 .
      *                      *-----------------------------------------*
      *                      * Aggiornamento record                    *
      *                      *-----------------------------------------*
           move      w-rig-sto-lst        to   rf-lsd-prz-lst         .
           move      w-rig-sto-dvf        to   rf-lsd-dva-fin         .
      *                      *-----------------------------------------*
      *                      * Preparazione data di validita' finale   *
      *                      * in formato reverse                      *
      *                      *-----------------------------------------*
