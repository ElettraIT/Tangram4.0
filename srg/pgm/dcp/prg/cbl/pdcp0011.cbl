      ...
      *                       Ultima revisione:    NdK del 21/06/13    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su filtri di ordinamento     *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "INTERROGAZIONE SU FILTRI PER ORDINAMENTO"       .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start per codice mnemonico                  *
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CODMNE    "         to   f-key                  .
           move      rr-tip-rec           to   rf-zos-tip-rec         .
           move      rr-mne-min           to   rf-zos-cod-mne         .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-tip-rec           to   rf-zos-tip-rec         .
           move      rr-des-min           to   rf-zos-des-key         .
           move      zero                 to   rf-zos-cod-flt         .
           move      "pgm/azi/fls/ioc/obj/iofzos"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun filtro entro i limiti assegnati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [zos]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/azi/fls/ioc/obj/iofzos"
                                          to   s-pat                  .
      ...
      *              * Test su tipo record                             *
      *              *-------------------------------------------------*
           if        rf-zos-tip-rec       not  = rr-tip-rec
      ...
      *              *-------------------------------------------------*
       ...
       qry-tst-max-200.
      *              *-------------------------------------------------*
      *              * Se ordinamento per codice mnemonico             *
      *              *-------------------------------------------------*
           if        rf-zos-cod-mne       >    rr-mne-max
                     go to qry-tst-max-900.
      ...
      *              *-------------------------------------------------*
______*    if        rf-zos-cod-mne       =    spaces
______*              move  "#"            to   w-cnt-qry-flg-sub      .
       qry-sel-rec-999.
           exit.
      ...
      *                      *-----------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      22                   to   v-pos                  .
           move      rf-zos-des-flt       to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-zos-cod-flt       to   w-mpn-cod-flt          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Determinazione tipo ordinamento         *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      64                   to   v-pos                  .
      *
           if        w-zos-dcp-tip-ord    =    01
                     move  "Classific./Codice"
                                          to   v-alf
           else if   w-zos-dcp-tip-ord    =    02
                     move  "Classific./Descr."
                                          to   v-alf
           else if   w-zos-dcp-tip-ord    =    03
                     move  "Codice           "
                                          to   v-alf
           else if   w-zos-dcp-tip-ord    =    04
                     move  "Descrizione      "
                                          to   v-alf
           else if   w-zos-dcp-tip-ord    =    89
                     move  "[Catalogo]       "
                                          to   v-alf
           else if   w-zos-dcp-tip-ord    =    99
                     move  "{sequenza a caso}"
                                          to   v-alf
           else      move  "?                "
                                          to   v-alf                  .
      *
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-flt'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Codice   Mnemonico                 Descrizione    
      -              "                Ordinamento   "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
