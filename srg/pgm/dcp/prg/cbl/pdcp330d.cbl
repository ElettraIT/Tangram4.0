      ...
      *                                                                *
      *                    Cancellazione di un intero listino          *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     "   CANCELLAZIONE PREZZI DI UN LISTINO   "       .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-paz.
               10  w-exp-snx-paz-num      pic  9(02)       value 2    .
               10  w-exp-snx-paz-lun      pic  9(02)       value 02   .
               10  w-exp-snx-paz-tbl.
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-cpl.
               10  w-exp-snx-cpl-num      pic  9(02)       value 2    .
      ...
      *                  * Si/no prezzi a zero                         *
      *                  *---------------------------------------------*
           perform   acc-snx-paz-000      thru acc-snx-paz-999        .
           if        w-cnt-tus-acc-key    =    "E"
                     move  spaces         to   w-cnt-tus-acc-key
      ...
      *              * Si/no prezzi a zero                             *
      *              *-------------------------------------------------*
           perform   vis-snx-paz-000      thru vis-snx-paz-999        .
       vis-key-reg-999.
           exit.
      ...
      *              * Si/no prezzi a zero                             *
      *              *-------------------------------------------------*
           perform   pmt-snx-paz-000      thru pmt-snx-paz-999        .
       pmt-key-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Prompt per : Si/no prezzi a zero                          *
      *    *-----------------------------------------------------------*
       pmt-snx-paz-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           if        w-let-arc-zvl-flg    not  = spaces
                     go to acc-cod-lst-100.
      *                  *---------------------------------------------*
      *                  * Cancellazione listino Base non ammessa      *
      *                  *---------------------------------------------*
           if        w-tes-cod-lst        not  = spaces
                     go to acc-cod-lst-600.
      *                      *-----------------------------------------*
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "Non e' ammessa la cancellazione del Listino Base !
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no prezzi a zero                  *
      *    *-----------------------------------------------------------*
       acc-snx-paz-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-paz-lun    to   v-car                  .
           move      w-exp-snx-paz-num    to   v-ldt                  .
           move      "SN#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      07                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-paz-tbl    to   v-txt                  .
           move      w-tes-snx-paz        to   v-num                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     ...
                     go to acc-snx-paz-999.
       acc-snx-paz-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-paz          .
       acc-snx-paz-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * sia su Up                                   *
      *                  *---------------------------------------------*
           if        w-tes-snx-paz        not  = zero
                     go to acc-snx-paz-600.
           if        v-key                =    "UP  "
                     go to acc-snx-paz-600
           else      go to acc-snx-paz-100.
       acc-snx-paz-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-rig-flg
                             ...
                             go to acc-snx-paz-100.
       acc-snx-paz-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione : Si/no prezzi a zero                     *
      *    *-----------------------------------------------------------*
       vis-snx-paz-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Si/no cancellazione prezzo       *
      *    *-----------------------------------------------------------*
       pmt-snx-cpl-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-rig-snx-cpl          .
       acc-snx-cpl-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        w-rig-snx-cpl        =    zero
      ...
      *              *-------------------------------------------------*
           ...
           move      w-rlt-sup-buf        to   w-rig                  .
      *              *-------------------------------------------------*
      *              * Test su si/no cancellazione prezzo              *
      *              *-------------------------------------------------*
           if        w-rig-snx-cpl        =    02
                     go to sca-fil-rap-100.
       sca-fil-rap-300.
      *              *-------------------------------------------------*
      *              * Cancellazione listino diverso da quello Base    *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
