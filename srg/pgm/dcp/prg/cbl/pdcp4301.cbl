      ...
      *                                   Fase:    dcp430              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 03/03/93    *
      *                       Ultima revisione:    NdK del 21/06/02    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma pdcp4300 *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
      *        * Comodo per fincatura                                  *
      *        *-------------------------------------------------------*
           05  w-cix-fin-str              pic  x(40)                  .
      ...
      *    *-----------------------------------------------------------*
       01  w-buf.
      *        *-------------------------------------------------------*
      *        * Work per bufferizzazione righe di descrizione         *
      *        *-------------------------------------------------------*
           05  w-buf-rec-pdx.
               10  w-buf-rec-pdx-flg      pic  x(01)                  .
               10  w-buf-rec-pdx-ctr      pic  9(02)                  .
               10  w-buf-rec-pdx-txt occurs 10.
                   15  w-buf-rec-pdx-rig  pic  x(40)                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-sub      .
      *              *-------------------------------------------------*
      *              * Bufferizzazione eventuale descrizione in lingua *
      *              *-------------------------------------------------*
           perform   buf-rec-pdx-000      thru buf-rec-pdx-999        .
      *              *-------------------------------------------------*
      *              * Test su si/no solo descrizioni in lingua        *
      *              *-------------------------------------------------*
           if        rr-snx-sdl           not  = 01
                     go to prn-sel-rec-999.
           if        w-buf-rec-pdx-flg    not  = spaces
                     move  "#"            to   w-cnt-prn-flg-sub
                     go to prn-sel-rec-999.
       prn-sel-rec-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                  *---------------------------------------------*
      *                  * Descrizione ad uso interno prodotto         *
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "A"                  to   p-tip                  .
           move      40                   to   p-car                  .
       ...
       prn-liv-det-300.
      *                  *---------------------------------------------*
      *                  * Inizio stampa descrizione in lingua         *
      *                  *---------------------------------------------*
           if        w-buf-rec-pdx-flg    not  = spaces
                     go to prn-liv-det-999.
           move      zero                 to   w-cix-ctr-001          .
       prn-liv-det-320.
           add       01                   to   w-cix-ctr-001          .
           if        w-cix-ctr-001        >    w-buf-rec-pdx-ctr
                     go to prn-liv-det-999.
           if        w-cix-ctr-001        =    01
      ...
      *              *-------------------------------------------------*
           move      "DESCRIZIONI PRODOTTI IN LINGUA STRANIERA"
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "    Codice            Descrizione ad uso interno  
      -              "     "              to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      "Descrizione in"     to   w-cix-fin-str          .
           move      zero                 to   w-cix-ctr-001          .
           inspect   w-cix-fin-str    tallying w-cix-ctr-001
                                          for  trailing spaces        .
           move      40                   to   w-cix-ctr-002          .
           subtract  w-cix-ctr-001        from w-cix-ctr-002          .
           add       2                    to   w-cix-ctr-002          .
           if        w-cix-ctr-002        >    40
                     go to int-fin-det-400.
           move      rr-cod-lng-des       to   w-cix-fin-str
                                              (w-cix-ctr-002 :)       .
       int-fin-det-400.
      *                  *---------------------------------------------*
      *                  * Determinazione posizione centrata           *
      *                  *---------------------------------------------*
           move      zero                 to   w-cix-ctr-001          .
           inspect   w-cix-fin-str    tallying w-cix-ctr-001
                                          for  trailing spaces        .
           divide    2                    into w-cix-ctr-001          .
      ...
      *    *===========================================================*
      *    * Routine di Bufferizzazione della descrizione in lingua    *
      *    *-----------------------------------------------------------*
       buf-rec-pdx-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione flag                            *
      *              *-------------------------------------------------*
           move      spaces               to   w-buf-rec-pdx-flg      .
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore righe                 *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-rec-pdx-ctr      .
       buf-rec-pdx-100.
      *              *-------------------------------------------------*
      *              * Start su archivio [pdx]                         *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           ...
           move      rr-cod-lng           to   rf-pdx-cod-lng         .
           ...
           call      s-pat               using f
                                               rf-pdx                 .
      *                  *---------------------------------------------*
      *                  * Se errore di start : a test su contatore    *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to buf-rec-pdx-900.
       buf-rec-pdx-200.
      *              *-------------------------------------------------*
      *              * Read-next su archivio [pdx]                     *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/dcp/fls/ioc/obj/iofpdx"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
           if        rf-pdx-tip-rec       not  = 02             or
                     ...
                     rf-pdx-cod-lng       not  = rr-cod-lng     or
      ...
      *              *-------------------------------------------------*
           add       1                    to   w-buf-rec-pdx-ctr      .
      *              *-------------------------------------------------*
      *              * Test sul numero massimo di righe                *
      *              *-------------------------------------------------*
           if        w-buf-rec-pdx-ctr    >    10
                     go to buf-rec-pdx-999.
      *              *-------------------------------------------------*
      *              * Bufferizzazione riga di testo                   *
      *              *-------------------------------------------------*
           move      rf-pdx-des-pro       to   w-buf-rec-pdx-rig
                                              (w-buf-rec-pdx-ctr)     .
       buf-rec-pdx-800.
      *              *-------------------------------------------------*
      *              * Riciclo a riga successiva                       *
      *              *-------------------------------------------------*
           go to     buf-rec-pdx-200.
       buf-rec-pdx-900.
      *              *-------------------------------------------------*
      *              * Test sul contatore righe lette                  *
      *              *-------------------------------------------------*
           if        w-buf-rec-pdx-ctr    =    zero
                     move   "#"           to   w-buf-rec-pdx-flg      .
       buf-rec-pdx-999.
           exit.
