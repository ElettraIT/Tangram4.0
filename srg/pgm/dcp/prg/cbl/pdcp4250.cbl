      ...
      *                                   Fase:    dcp425              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 14/11/92    *
      *                       Ultima revisione:    NdK del 30/04/10    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pdcp4251:        *
      *                                                                *
      *                    Descrizione prodotti in lingua straniera    *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           ...
           05  rr-des-iln.
               10  rr-ded-rig occurs 10   pic  x(40)                  .
           05  rr-snx-svs                 pic  x(01)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-svs.
               10  w-exp-snx-svs-num      pic  9(02)       value 2    .
               10  w-exp-snx-svs-lun      pic  9(02)       value 02   .
               10  w-exp-snx-svs-tbl.
      ...
      *                  *---------------------------------------------*
           perform   acc-ded-rig-000      thru acc-ded-rig-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
       ...
       acc-ric-sel-400.
      *                  *---------------------------------------------*
      *                  * Si/no sovrascrittura                        *
      *                  *---------------------------------------------*
           perform   acc-snx-svs-000      thru acc-snx-svs-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *===========================================================*
      *    * Accettazione campo : Descrizione in lingua                *
      *    *-----------------------------------------------------------*
       acc-ded-rig-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-ded-rig-999.
       acc-ded-rig-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-txt                to   rr-des-iln             .
       acc-ded-rig-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rr-des-iln           not  = spaces
                     go to acc-ded-rig-450.
           if        v-key                =    "UP  "
                     go to acc-ded-rig-600
           else      go to acc-ded-rig-100.
       acc-ded-rig-450.
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/no sovrascrittura       *
      *    *-----------------------------------------------------------*
       acc-snx-svs-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-svs-lun    to   v-car                  .
           move      w-exp-snx-svs-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      12                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-svs-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           if        rr-snx-svs           =    "N"
                     move  01             to   v-num
           else if   rr-snx-svs           =    "S"
                     move  02             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-svs-999.
       acc-snx-svs-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "N"            to   rr-snx-svs
           else if   v-num                =    02
                     move  "S"            to   rr-snx-svs
           else      move  spaces         to   rr-snx-svs             .
       acc-snx-svs-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-svs           =    spaces
                     go to acc-snx-svs-100.
       acc-snx-svs-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      ...
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Test su descrizione in lingua                   *
      *              *-------------------------------------------------*
           if        rr-ded-rig (1)       not  = spaces
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione si/no sovrascrittura            *
      *              *-------------------------------------------------*
           if        rr-snx-svs           not  = spaces
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-ded-rig (01)        .
           move      spaces               to   rr-ded-rig (02)        .
           move      spaces               to   rr-ded-rig (03)        .
           move      spaces               to   rr-ded-rig (04)        .
           move      spaces               to   rr-ded-rig (05)        .
           move      spaces               to   rr-ded-rig (06)        .
           move      spaces               to   rr-ded-rig (07)        .
           move      spaces               to   rr-ded-rig (08)        .
           move      spaces               to   rr-ded-rig (09)        .
           move      spaces               to   rr-ded-rig (10)        .
           move      zero                 to   rr-snx-svs             .
       nor-ric-sel-999.
           exit.
      ...
      *                  *---------------------------------------------*
           move      rr-ded-rig
                    (w-cmp-rig-dep-ctr)   to   rr-ded-rig
                                              (w-cmp-rig-dep-nrd)     .
      *                  *---------------------------------------------*
      *                  * Riciclo                                     *
      *                  *---------------------------------------------*
