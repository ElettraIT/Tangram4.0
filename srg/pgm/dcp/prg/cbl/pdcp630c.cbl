      ...
      *                                                                *
      *                    Aggiornamento date campagne prodotti        *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-des-tit-pgm-ovy              pic  x(40)       value
                     " AGGIORNAMENTI AUTOMATICI DATE CAMPAGNA "       .
      ...
      *            *---------------------------------------------------*
      *            * Valori per aggiornamento                          *
      *            *---------------------------------------------------*
               10  w-tes-dvp-ini          pic  9(07)                  .
               10  w-tes-dvp-fin          pic  9(07)                  .
      ...
      *                  *---------------------------------------------*
       ...
       acc-key-reg-110.
      *                  *---------------------------------------------*
      *                  * Data inizio campagna - precedente           *
      *                  *---------------------------------------------*
           perform   acc-key-pin-000      thru acc-key-pin-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
       ...
       acc-key-reg-120.
      *                  *---------------------------------------------*
      *                  * Data fine campagna - precedente             *
      *                  *---------------------------------------------*
           perform   acc-key-pfi-000      thru acc-key-pfi-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
                     go to acc-key-reg-110.
       acc-key-reg-130.
      *                  *---------------------------------------------*
      *                  * Data inizio campagna - attuale              *
      *                  *---------------------------------------------*
           perform   acc-key-ain-000      thru acc-key-ain-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
       ...
       acc-key-reg-140.
      *                  *---------------------------------------------*
      *                  * Data fine campagna - attuale                *
      *                  *---------------------------------------------*
           perform   acc-key-afi-000      thru acc-key-afi-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * Data inizio campagna - precedente               *
      *              *-------------------------------------------------*
           perform   vis-key-pin-000      thru vis-key-pin-999        .
      *              *-------------------------------------------------*
      *              * Data fine campagna - precedente                 *
      *              *-------------------------------------------------*
           perform   vis-key-pfi-000      thru vis-key-pfi-999        .
      *              *-------------------------------------------------*
      *              * Data inizio campagna - attuale                  *
      *              *-------------------------------------------------*
           perform   vis-key-ain-000      thru vis-key-ain-999        .
      *              *-------------------------------------------------*
      *              * Data fine campagna - attuale                    *
      *              *-------------------------------------------------*
           perform   vis-key-afi-000      thru vis-key-afi-999        .
       vis-key-reg-999.
           exit.
      ...
      *              * Data inizio campagna - precedente               *
      *              *-------------------------------------------------*
           perform   pmt-key-pin-000      thru pmt-key-pin-999        .
      *              *-------------------------------------------------*
      *              * Data fine campagna - precedente                 *
      *              *-------------------------------------------------*
           perform   pmt-key-pfi-000      thru pmt-key-pfi-999        .
      *              *-------------------------------------------------*
      *              * Data inizio campagna - attuale                  *
      *              *-------------------------------------------------*
           perform   pmt-key-ain-000      thru pmt-key-ain-999        .
      *              *-------------------------------------------------*
      *              * Data fine campagna - attuale                    *
      *              *-------------------------------------------------*
           perform   pmt-key-afi-000      thru pmt-key-afi-999        .
       pmt-key-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Prompt : Data inizio campagna - precedente                *
      *    *-----------------------------------------------------------*
       pmt-key-pin-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Data fine campagna - precedente                  *
      *    *-----------------------------------------------------------*
       pmt-key-pfi-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Data inizio campagna - attuale                   *
      *    *-----------------------------------------------------------*
       pmt-key-ain-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Data fine campagna - attuale                     *
      *    *-----------------------------------------------------------*
       pmt-key-afi-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data inizio campagna - precedente          *
      *    *-----------------------------------------------------------*
       acc-key-pin-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-key-pin-999.
       acc-key-pin-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   w-tes-dvp-ini          .
       acc-key-pin-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-dvp-ini        =    zero
                     go to acc-key-pin-100.
       acc-key-pin-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data inizio campagna - precedente       *
      *    *-----------------------------------------------------------*
       vis-key-pin-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data fine campagna - precedente            *
      *    *-----------------------------------------------------------*
       acc-key-pfi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-key-pfi-999.
       acc-key-pfi-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-dat                to   w-tes-dvp-fin          .
       acc-key-pfi-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * si sia in Up                                *                                   
      *                  *---------------------------------------------*
           if        w-tes-dvp-fin        not  = zero
                     go to acc-key-pfi-600.
           if        v-key                =    "UP  "
                     go to acc-key-pfi-600
           else      go to acc-key-pfi-100.
       acc-key-pfi-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data fine campagna - precedente         *
      *    *-----------------------------------------------------------*
       vis-key-pfi-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data inizio campagna - attuale             *
      *    *-----------------------------------------------------------*
       acc-key-ain-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data inizio campagna - attuale          *
      *    *-----------------------------------------------------------*
       vis-key-ain-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Data fine campagna - attuale               *
      *    *-----------------------------------------------------------*
       acc-key-afi-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           if        w-tes-dva-fin        not  = zero
                     go to acc-key-afi-600.
           if        v-key                =    "UP  "
                     go to acc-key-afi-600
           else      go to acc-key-afi-100.
       acc-key-afi-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Data fine campagna - attuale            *
      *    *-----------------------------------------------------------*
       vis-key-afi-000.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
      ...
      *                  * Data inizio campagna - precedente           *
      *                  *---------------------------------------------*
           if        w-tes-dvp-ini        not  = zero
                     go to cnt-tdo-key-120.
           move      "Manca la data di inizio campagna (precedente)     
      ...
      *                  *---------------------------------------------*
           ...
           move      "Manca la data di fine campagna (precedente)       
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           go to     cnt-tdo-key-900.
       cnt-tdo-key-130.
      ...
      *                  * Data inizio campagna - attuale              *
      *                  *---------------------------------------------*
           if        w-tes-dva-ini        not  = zero
                     go to cnt-tdo-key-140.
           move      "Manca la data di inizio campagna (attuale)        
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           go to     cnt-tdo-key-900.
       cnt-tdo-key-140.
      ...
      *                  *---------------------------------------------*
           ...
           move      "Manca la data di fine campagna (attuale)          
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
           go to     cnt-tdo-key-900.
       cnt-tdo-key-200.
      ...
      *              * data di fine campagna precedenti                *
      *              *-------------------------------------------------*
           if        w-tes-dvp-ini        not  > w-tes-dvp-fin
                     go to cnt-tdo-key-300.
           move      "La data di inizio campagna di vendita non puo' ess
      -              "ere superiore a"    to   w-err-box-err-msg      .
           move      "quella di fine campagna (date precedenti)         
      -              "               "    to   w-err-box-err-m02      .
           perform   box-msg-e02-000      thru box-msg-e02-999        .
           go to     cnt-tdo-key-920.
       cnt-tdo-key-300.
      *              *-------------------------------------------------*
      *              * Controllo di congruenza tra data di inizio e    *
      *              * data di fine campagna attuali                   *
      *              *-------------------------------------------------*
           if        w-tes-dva-ini        not  > w-tes-dva-fin
                     go to cnt-tdo-key-400.
           move      "La data di inizio campagna di vendita non puo' ess
      -              "ere superiore a"    to   w-err-box-err-msg      .
           move      "quella di fine campagna (date attuali)            
      -              "               "    to   w-err-box-err-m02      .
           perform   box-msg-e02-000      thru box-msg-e02-999        .
           go to     cnt-tdo-key-920.
       cnt-tdo-key-400.
      *              *-------------------------------------------------*
      *              * Controllo di congruenza tra data di inizio e    *
      *              * data di fine campagna attuali e precedenti      *
      *              *-------------------------------------------------*
       cnt-tdo-key-800.
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-key-reg-000.
           ...
           move      zero                 to   w-tes-dvp-ini          .
           move      zero                 to   w-tes-dvp-fin          .
      ...
      *                  * Messaggio di errore                         *
      *                  *---------------------------------------------*
           move      "Nessuna condizione di campagna da trattare !      
      -              "               "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-srt-inp-120.
      *                  *---------------------------------------------*
      *                  * Selezione su date di validita' pecedenti    *
      *                  *---------------------------------------------*
           if        rf-lst-dva-ini       not  = w-tes-dvp-ini
                     go to stp-srt-inp-120.
           if        rf-lst-dva-fin       not  = w-tes-dvp-fin
                     go to stp-srt-inp-120.
      *                  *---------------------------------------------*
