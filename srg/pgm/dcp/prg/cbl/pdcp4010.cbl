      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  INTERROGAZIONE SU ARCHIVIO PRODOTTI   "       .
      ...
      *            * Codice minimo                                     *
      *            *---------------------------------------------------*
               10  w-ipc-dlp-cod-min      pic  x(14)                  .
      *            *---------------------------------------------------*
      *            * Codice massimo                                    *
      *            *---------------------------------------------------*
               10  w-ipc-dlp-cod-max      pic  x(14)                  .
      *            *---------------------------------------------------*
      *            * Descrizione minima                                *
      *            *---------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Status commerciale del prodotto            *
      *        *-------------------------------------------------------*
           05  w-exp-sta-tus.
               10  w-exp-sta-tus-num      pic  9(02)       value 8    .
               ...
               10  w-exp-sta-tus-tbl.
                   ...
                   15  filler             pic  x(40) value
                           "Obsoleto                                " .
                   15  filler             pic  x(40) value
                           "Obsoleto, ma sost. da nuovo prodotto    " .
      ...
      *    *-----------------------------------------------------------*
       01  w-rip-str.
      *        *-------------------------------------------------------*
      *        * Work-area per ripristino Start su [dcp]               *
      *        *-------------------------------------------------------*
           05  w-rip-str-dcp.
      *            *---------------------------------------------------*
      *            * Flag di End-of-file                               *
      *            *---------------------------------------------------*
               10  w-rip-str-dcp-eof      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Parametri di Start                                *
      *            *---------------------------------------------------*
               10  w-rip-str-dcp-alf      pic  x(14)                  .
               10  w-rip-str-dcp-num      pic  9(07)                  .
               10  w-rip-str-dcp-syn      pic  x(13)                  .
               10  w-rip-str-dcp-des      pic  x(40)                  .
               10  w-rip-str-dcp-cla      pic  9(05)                  .
               10  w-rip-str-dcp-gru      pic  9(05)                  .
               10  w-rip-str-dcp-sgr      pic  9(05)                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      zero                 to   rr-sta-tus             .
           go to     pre-tip-fun-999.
       pre-tip-fun-300.
      *              *-------------------------------------------------*
      *              * Se variabile di i.p.c. per Codice               *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * No richieste ad utente                      *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      w-ipc-dlp-cod-min    to   rr-cod-min             .
           move      w-ipc-dlp-cod-max    to   rr-cod-max             .
           move      spaces               to   rr-sin-min             .
           move      all   "z"            to   rr-sin-max             .
           move      zero                 to   rr-sta-tus             .
           go to     pre-tip-fun-999.
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Default status prodotto                         *
      *              *-------------------------------------------------*
           move      01                   to   rr-sta-tus             .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      08                   to   w-cod-mne-zp2-lin      .
           move      30                   to   w-cod-mne-zp2-pos      .
           move      08                   to   w-cod-mne-zp2-dln      .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Status prodotto                      *
      *    *-----------------------------------------------------------*
       acc-sta-tus-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *
           if        rr-sta-tus           =    zero
                     move  01             to   v-num
           else if   rr-sta-tus           =    01
                     move  02             to   v-num
           else if   rr-sta-tus           =    11
                     move  03             to   v-num
           else if   rr-sta-tus           =    21
                     move  04             to   v-num
           else if   rr-sta-tus           =    51
                     move  05             to   v-num
           else if   rr-sta-tus           =    52
                     move  06             to   v-num
           else if   rr-sta-tus           =    71
                     move  07             to   v-num
           else if   rr-sta-tus           =    72
                     move  08             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     ...
                     move  11             to   rr-sta-tus
           else if   v-num                =    04
                     move  21             to   rr-sta-tus
           else if   v-num                =    05
                     move  51             to   rr-sta-tus
           else if   v-num                =    06
                     move  52             to   rr-sta-tus
           else if   v-num                =    07
                     move  71             to   rr-sta-tus
           else if   v-num                =    08
                     move  72             to   rr-sta-tus
           else      move zero            to   rr-sta-tus             .
       acc-sta-tus-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Status prodotto                   *
      *    *-----------------------------------------------------------*
       vis-sta-tus-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-rip-str-dcp-eof      .
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessuna anagrafica prodotto entro i limiti assegna
      -              "ti !"               to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           if        rr-sta-tus           =    zero
                     go to qry-sel-rec-999.
           if        rf-dcp-sta-tus       not  = rr-sta-tus
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
       qry-sel-rec-999.
           exit.
      ...
      *                  *---------------------------------------------*
           perform   exp-rec-dcp-000      thru exp-rec-dcp-999        .
       qry-trt-fun-999.
           exit.
      ...
      *    *-----------------------------------------------------------*
       exp-rec-dcp-000.
      *              *-------------------------------------------------*
      *              * Salvataggio parametri per Start su [dcp]        *
      *              *-------------------------------------------------*
           if        w-rip-str-dcp-eof    not  = spaces
                     go to exp-rec-dcp-100.
           move      rf-dcp-alf-pro       to   w-rip-str-dcp-alf      .
           move      rf-dcp-num-pro       to   w-rip-str-dcp-num      .
           move      rf-dcp-des-key       to   w-rip-str-dcp-des      .
           move      rf-dcp-syn-pro       to   w-rip-str-dcp-syn      .
           move      rf-dcp-cla-pro       to   w-rip-str-dcp-cla      .
           move      rf-dcp-gru-pro       to   w-rip-str-dcp-gru      .
           move      rf-dcp-sgr-pro       to   w-rip-str-dcp-sgr      .
       exp-rec-dcp-100.
      *              *-------------------------------------------------*
      *              * Video in 'OFF'                                  *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per Categoria di sconto              *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      18                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per Percentuale Categoria di sconto  *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per Categoria di provvigioni         *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      18                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Prompt per Percentuale di provvigioni       *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      20                   to   v-lin                  .
           move      03                   to   v-pos                  .
           move      "Ultima modifica  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *              *-------------------------------------------------*
      *              * Letture dati prodotto per Expand                *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Lettura record [dcp]                        *
      *                  *---------------------------------------------*
           ...
           move      v-cnt                to   w-mpn                  .
           move      w-mpn-num-pro        to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Sinonimo prodotto           *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      13                   to   v-car                  .
      ...
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-gru-pro       =    zero
                     go to exp-rec-dcp-340.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      05                   to   v-car                  .
           move      zero                 to   v-dec                  .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Codice Iva                  *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-cod-iva       =    zero
                     go to exp-rec-dcp-400.
      *                      *-----------------------------------------*
      *                      * Editing preliminare                     *
      *                      *-----------------------------------------*
           move      rf-dcp-cod-iva       to   w-edt-iva-cod          .
      ...
      *                  *---------------------------------------------*
       ...
       exp-rec-dcp-400.
      *                  *---------------------------------------------*
      *                  * Visualizzazione Codice contropartita        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-ctp-ven       =    zero
                     go to exp-rec-dcp-420.
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Visualizzazione descrizione Contropartita   *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      40                   to   v-car                  .
       ...
       exp-rec-dcp-420.
      *                  *---------------------------------------------*
      *                  * Visualizzazione Prezzo di listino           *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "V"                  to   v-tip                  .
           move      09                   to   v-car                  .
      ...
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-tip-vve       =    spaces
                     go to exp-rec-dcp-440.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      03                   to   v-car                  .
           move      16                   to   v-lin                  .
      ...
      *                  *---------------------------------------------*
       ...
       exp-rec-dcp-440.
      *                  *---------------------------------------------*
      *                  * Visualizzazione Categoria sconto            *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-cat-scr       =    zero
                     go to exp-rec-dcp-460.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      05                   to   v-car                  .
           move      zero                 to   v-dec                  .
      ...
      *                  *---------------------------------------------*
       ...
       exp-rec-dcp-460.
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di sconto 1     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di sconto 2     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di sconto 3     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di sconto 4     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di sconto 5     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Visualizzazione Categoria provvigioni       *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test se codice esistente                *
      *                      *-----------------------------------------*
           if        rf-dcp-cat-pvg       =    zero
                     go to exp-rec-dcp-480.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      05                   to   v-car                  .
           move      zero                 to   v-dec                  .
      ...
      *                  *---------------------------------------------*
       ...
       exp-rec-dcp-480.
      *                  *---------------------------------------------*
      *                  * Visualizzazione Percentuale di provv. 1     *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      02                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      22                   to   v-pos                  .
           move      w-exp-sta-tus-tbl    to   v-txt                  .
           if        rf-dcp-sta-tus       =    zero
                     move  01             to   v-num
           else if   rf-dcp-sta-tus       =    01
                     move  02             to   v-num
      ...
      *                  *---------------------------------------------*
           if        rf-dcp-sta-tus       not  = 21 and
                     ...
                     go to exp-rec-dcp-600.
           if        rf-dcp-sta-tuc       =    zero
                     go to exp-rec-dcp-600.
           move      rf-dcp-sta-tuc       to   w-sav-sta-tuc          .
       exp-rec-dcp-520.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRO    "         to   f-key                  .
           move      w-sav-sta-tuc        to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
                                               rf-dcp                 .
           if        f-sts                not  = e-not-err
                     go to exp-rec-dcp-580.
      *                  *---------------------------------------------*
      *                  * Visualizzazione codice sostitutivo          *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      14                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           if        rf-dcp-ide-dat       =    zero
                     go to exp-rec-dcp-660.
           move      "DS"                 to   v-ope                  .
           move      "D"                  to   v-tip                  .
           move      20                   to   v-lin                  .
           move      22                   to   v-pos                  .
      ...
      *                  *---------------------------------------------*
           if        rf-dcp-ide-ute       =    spaces
                     go to exp-rec-dcp-770.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      10                   to   v-car                  .
           move      20                   to   v-lin                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *              *-------------------------------------------------*
      *              * Ripristino Start su [dcp]                       *
      *              *-------------------------------------------------*
           if        w-rip-str-dcp-eof    not  = spaces
                     go to exp-rec-dcp-999.
      *              *-------------------------------------------------*
      *              * Deviazione in funzione del tipo ordinamento     *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    1
                     go to exp-rec-dcp-820
           else if   rr-tip-ord           =    2
                     go to exp-rec-dcp-840
           else if   rr-tip-ord           =    3
                     go to exp-rec-dcp-860
           else if   rr-tip-ord           =    4
                     go to exp-rec-dcp-880.
       exp-rec-dcp-820.
      *              *-------------------------------------------------*
      *              * Se ordinamento per codice                       *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "ALFPRO    "         to   f-key                  .
           move      w-rip-str-dcp-alf    to   rf-dcp-alf-pro         .
           move      w-rip-str-dcp-num    to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to exp-rec-dcp-990.
           go to     exp-rec-dcp-980.
       exp-rec-dcp-840.
      *              *-------------------------------------------------*
      *              * Se ordinamento per descrizione                  *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "DESKEY    "         to   f-key                  .
           move      w-rip-str-dcp-des    to   rf-dcp-des-key         .
           move      w-rip-str-dcp-num    to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "SYNPRO    "         to   f-key                  .
           move      w-rip-str-dcp-syn    to   rf-dcp-syn-pro         .
           move      w-rip-str-dcp-num    to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "CGSALF    "         to   f-key                  .
           move      w-rip-str-dcp-cla    to   rf-dcp-cla-pro         .
           move      w-rip-str-dcp-gru    to   rf-dcp-gru-pro         .
           move      w-rip-str-dcp-sgr    to   rf-dcp-sgr-pro         .
           move      w-rip-str-dcp-alf    to   rf-dcp-alf-pro         .
           move      w-rip-str-dcp-num    to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   w-ipc-dlp-cod-min      .
           move      spaces               to   w-ipc-dlp-cod-max      .
      ...
      *                  *---------------------------------------------*
       ...
       ipc-dlp-400.
      *              *-------------------------------------------------*
      *              * Se tipo interrogazione per Codice               *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Estrazione variabile di i.p.c. "cod-min"    *
      *                  *---------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           if        s-alf                =    spaces
                     go to ipc-dlp-900.
           move      s-alf                to   w-ipc-dlp-cod-min      .
      *                  *---------------------------------------------*
      *                  * Estrazione variabile di i.p.c. "cod-max"    *
      *                  *---------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           if        s-alf                =    spaces
                     go to ipc-dlp-900.
           move      s-alf                to   w-ipc-dlp-cod-max      .
      *                  *---------------------------------------------*
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
           go to     ipc-dlp-999.
