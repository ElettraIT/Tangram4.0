      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pdcp4301:        *
      *                                                                *
      *                    Stampa archivio prodotti in lingua stra-    *
      *                    niera                                       *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  STAMPA DESCRIZIONI PRODOTTI IN LINGUA "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           10  rr-cod-lng                 pic  x(03)                  .
           10  rr-cod-lng-des             pic  x(20)                  .
           ...
           05  w-exp-tip-slp.
               ...
               10  w-exp-tip-slp-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "Ad ogni sottogruppo "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Si/no stampa descrizione in lingua         *
      *        *-------------------------------------------------------*
           05  w-exp-snx-sdl.
               10  w-exp-snx-sdl-num      pic  9(02)       value 2    .
               10  w-exp-snx-sdl-lun      pic  9(02)       value 02   .
               10  w-exp-snx-sdl-tbl.
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-400.
      *                  *---------------------------------------------*
      *                  * Si/no solo descrizione in lingua            *
      *                  *---------------------------------------------*
           perform   acc-snx-sdl-000      thru acc-snx-sdl-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/no solo descrizione in  *
      *    *                                lingua                     *
      *    *-----------------------------------------------------------*
       acc-snx-sdl-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-sdl-lun    to   v-car                  .
           move      w-exp-snx-sdl-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      12                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-snx-sdl-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
           move      rr-snx-sdl           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-sdl-999.
       acc-snx-sdl-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-snx-sdl             .
       acc-snx-sdl-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-sdl           =    zero
                     go to acc-snx-sdl-100.
       acc-snx-sdl-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Normalizzazione si/no solo descrizione lingua   *
      *              *-------------------------------------------------*
           if        rr-snx-sdl           =    zero
                     move  01             to   rr-snx-sdl             .
       reg-ric-sel-999.
           exit.
