      ...
      *                       Ultima revisione:    NdK del 06/02/18    *
      *                    ------------------------------------------- *
      ...
      *            *---------------------------------------------------*
               ...
               10  w-tes-ric-sos          pic  9(06)v9(03)            .
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-rig.
           05  w-rig-001.
               ...
               10  w-rig-per-ric          pic  9(06)v9(03)            .
           05  w-rig-002.
      ...
      *        *-------------------------------------------------------*
      *        * Salvataggio per % di ricarico                         *
      *        *-------------------------------------------------------*
           05  w-sav-per-ric              pic  9(06)v9(03)            .
      ...
      *        *-------------------------------------------------------*
      *        * Linea e posizione per % di ricarico                   *
      *        *-------------------------------------------------------*
           05  w-lpt-per-ric-lin          pic  9(03)    value 015     .
           05  w-lpt-per-ric-pos          pic  9(03)    value 030     .
      ...
      *                  * % di ricarico                               *
      *                  *---------------------------------------------*
           perform   acc-per-ric-000      thru acc-per-ric-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * % di ricarico                                   *
      *              *-------------------------------------------------*
           perform   vis-per-ric-000      thru vis-per-ric-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * % di ricarico                               *
      *                  *---------------------------------------------*
           perform   pmt-per-ric-000      thru pmt-per-ric-999        .
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : % di ricarico                    *
      *    *-----------------------------------------------------------*
       pmt-per-ric-000.
      *              *-------------------------------------------------*
      *              * Test se prompt da visualizzare                  *
      ...
      *              *-------------------------------------------------*
           ...
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
           move      w-lpt-per-ric-lin    to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "% di ricarico              :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
      ...
      *    *===========================================================*
      *    * Accettazione campo : % di ricarico                        *
      *    *-----------------------------------------------------------*
       acc-per-ric-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * Salvataggio valore attuale                  *
      *                  *---------------------------------------------*
           move      w-tes-per-ric        to   w-sav-per-ric          .
      *                  *---------------------------------------------*
      *                  * Test se richiesta sostituzione              *
      *                  *---------------------------------------------*
           if        w-tes-per-ric-sns    =    zero
                     go to acc-per-ric-100.
      *                  *---------------------------------------------*
      *                  * Forzatura del tasto Pf1                     *
      *                  *---------------------------------------------*
           move      "[1] "               to   v-key                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      "<B"                 to   v-edm                  .
           move      w-lpt-per-ric-lin    to   v-lin                  .
           move      w-lpt-per-ric-pos    to   v-pos                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      "[1] "               to   v-pfk (14)             .
           move      w-tes-per-ric        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Cancellazione eventuali note operative          *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-per-ric-999.
       acc-per-ric-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-per-ric          .
       acc-per-ric-300.
      *              *-------------------------------------------------*
      *              * Se Pf1                                          *
      *              *-------------------------------------------------*
           if        v-key                not  = "[1] "
                     go to acc-per-ric-400.
      *                  *---------------------------------------------*
      *                  * Subroutine di accettazione valore sostitu-  *
      *                  * tivo                                        *
      *                  *---------------------------------------------*
           perform   acc-per-ric-sos-000  thru acc-per-ric-sos-999    .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-per-ric-999.
      *                  *---------------------------------------------*
      *                  * Visualizzazione sostituzione effettuata     *
      *                  *---------------------------------------------*
           perform   vis-per-ric-sos-000  thru vis-per-ric-sos-999    .
      *                  *---------------------------------------------*
      *                  * Se sostituzione non effettuata ma impostati *
      *                  * valori : rivisualizzazione valori impostati *
      *                  *---------------------------------------------*
           if        w-tes-per-ric-sns    not  = zero
                     go to acc-per-ric-800.
      *                  *---------------------------------------------*
      *                  * Visualizzazione codice impostato            *
      *                  *---------------------------------------------*
           perform   vis-per-ric-000      thru vis-per-ric-999        .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : % di ricarico                     *
      *    *-----------------------------------------------------------*
       vis-per-ric-000.
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *                  * Normalizzazione segnale di sostituzione     *
      *                  *---------------------------------------------*
           move      zero                 to   w-tes-per-ric-sns      .
      *                  *---------------------------------------------*
      *                  * Salvataggio immagine video                  *
      *                  *---------------------------------------------*
           move      "SV"                 to   v-ope                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-lpt-per-ric-pos    to   v-pos                  .
           subtract  1                    from v-pos                  .
           move      w-lpt-per-ric-lin    to   v-lto                  .
           add       1                    to   v-lto                  .
           move      w-lpt-per-ric-pos    to   v-pto                  .
           add       50                   to   v-pto                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       acc-per-ric-sos-010.
      ...
      *                  * Visualizzazione valore da sostituire        *
      *                  *---------------------------------------------*
           perform   vis-per-ric-so1-000  thru vis-per-ric-so1-999    .
      *                  *---------------------------------------------*
      *                  * Visualizzazione valore sostitutivo          *
      *                  *---------------------------------------------*
           perform   vis-per-ric-so2-000  thru vis-per-ric-so2-999    .
       acc-per-ric-sos-200.
      *              *-------------------------------------------------*
      *              * Accettazioni                                    *
      *              *-------------------------------------------------*
      ...
      *                  * Valore da sostituire                        *
      *                  *---------------------------------------------*
           perform   acc-per-ric-so1-000  thru acc-per-ric-so1-999    .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-per-ric-sos-600.
           if        v-key                =    "DOWN"
                     go to acc-per-ric-sos-600.
       acc-per-ric-sos-300.
      *                  *---------------------------------------------*
      *                  * Valore sostitutivo                          *
      *                  *---------------------------------------------*
           perform   acc-per-ric-so2-000  thru acc-per-ric-so2-999    .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-per-ric-sos-600.
           if        v-key                =    "UP  "
                     go to acc-per-ric-sos-200.
       acc-per-ric-sos-600.
      *              *-------------------------------------------------*
      *              * Controlli su valori accettati                   *
      *              *-------------------------------------------------*
      ...
      *                  * le impostazioni                             *
      *                  *---------------------------------------------*
           if        w-tes-per-ric        =    zero  and
                     w-tes-ric-sos        =    zero
                     go to acc-per-ric-sos-800.
      *                  *---------------------------------------------*
      *                  * Che i valori non siano uguali               *
      *                  *---------------------------------------------*
           if        w-tes-per-ric        not  = w-tes-ric-sos
                     go to acc-per-ric-sos-800.
      *                      *-----------------------------------------*
      *                      * Normalizzazione valore sostitutivo      *
      *                      *-----------------------------------------*
           move      zero                 to   w-tes-ric-sos          .
      *                      *-----------------------------------------*
      *                      * Segnale di sostituzione non effettuata  *
      *                      *-----------------------------------------*
           move      zero                 to   w-tes-per-ric-sns      .
      ...
      *                  * Segnale di sostituzione effettuata          *
      *                  *---------------------------------------------*
           if        w-tes-per-ric        not  = zero or
                     w-tes-ric-sos        not  = zero
                     move  1              to   w-tes-per-ric-sns      .
       acc-per-ric-sos-900.
      *              *-------------------------------------------------*
      *              * Normalizzazione stato di uscita                 *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-tes-per-ric-sns    =    zero
                     go to vis-per-ric-sos-100
           else      go to vis-per-ric-sos-500.
       vis-per-ric-sos-100.
      *              *-------------------------------------------------*
      *              * Se sostituzione non effettuata                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-per-ric-so2-999.
       acc-per-ric-so2-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-ric-sos          .
       acc-per-ric-so2-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * % di ricarico                               *
      *                  *---------------------------------------------*
           perform   acc-rig-ric-000      thru acc-rig-ric-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
                     go to acc-cor-reg-900.
           if        v-key                =    "UP  "
      ...
      *              * % di ricarico                                   *
      *              *-------------------------------------------------*
           perform   vis-rig-ric-000      thru vis-rig-ric-999        .
       vis-rig-cor-999.
           exit.
      ...
      *              * % di ricarico                                   *
      *              *-------------------------------------------------*
           perform   pmt-rig-ric-000      thru pmt-rig-ric-999        .
       pmt-rig-cor-999.
           exit.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-rig-ric-999.
       acc-rig-ric-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-rig-per-ric          .
       acc-rig-ric-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              * % di ricarico                                   *
      *              *-------------------------------------------------*
           if        w-tes-per-ric        =    zero
                     go to sel-rec-dcp-800.
           if        rf-aaf-per-ric       not  = w-tes-per-ric
                     go to sel-rec-dcp-900.
       sel-rec-dcp-800.
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-aaf-per-ric       to   w-rig-per-ric          .
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-ric-sos        to   w-rig-per-ric          .
       car-eff-rig-sos-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-tmp-cns-sns    not  = zero or
                     ...
                     w-tes-per-mpa-sns    not  = zero or
                     w-tes-per-ric-sns    not  = zero
      ...
      *                  *---------------------------------------------*
           ...
           move      w-rig-per-ric        to   rf-aaf-per-ric         .
       upd-rec-aaf-400.
      *                  *---------------------------------------------*
      *                  * Update record                               *
      *                  *---------------------------------------------*
