      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione tabella annotazioni          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   INTERROGAZIONE TABELLA ANNOTAZIONI   "       .
      ...
      *    *===========================================================*
      *    * Link-area per accettazione codice annotazioni             *
      *    *-----------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-wrk.
      *        *-------------------------------------------------------*
      *        * Work per calcolo linee residue al dettaglio           *
      *        *-------------------------------------------------------*
           05  w-wrk-res-des              pic  9(01)                  .
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su tabella annotazioni"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-zba-cod      .
           move      12                   to   w-cod-mne-zba-lin      .
       ...
       acc-cod-min-120.
           move      w-cod-mne-zba-cod    to   v-alf                  .
       acc-cod-min-150.
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-zba-ope      .
           move      rr-cod-max           to   w-cod-mne-zba-cod      .
           move      12                   to   w-cod-mne-zba-lin      .
           move      52                   to   w-cod-mne-zba-pos      .
           move      zero                 to   w-cod-mne-zba-dln      .
           move      zero                 to   w-cod-mne-zba-dps      .
           move      spaces               to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-zba-cod-ant         .
           move      "pgm/bol/fls/ioc/obj/iofzba"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-des-min           to   rf-zba-des-key         .
           move      spaces               to   rf-zba-cod-ant         .
           move      "pgm/bol/fls/ioc/obj/iofzba"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [zba]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/bol/fls/ioc/obj/iofzba"
                                          to   s-pat                  .
      ...
      *              * Selezione su codice                             *
      *              *-------------------------------------------------*
           if        rf-zba-cod-ant       <    rr-cod-min or
                     rf-zba-cod-ant       >    rr-cod-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su descrizione                        *
      *              *-------------------------------------------------*
           if        rf-zba-des-key       <    rr-des-min or
                     rf-zba-des-key       >    rr-des-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
       qry-sel-rec-999.
           exit.
      ...
      *              *-------------------------------------------------*
           if        rf-zba-rig-ant (2)   =    spaces
                     move   1             to   w-wrk-res-des
           else if   rf-zba-rig-ant (3)   =    spaces
                     move   2             to   w-wrk-res-des
           else      move   3             to   w-wrk-res-des          .
           if        v-res                >    w-wrk-res-des
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      22                   to   v-pos                  .
           move      rf-zba-rig-ant (1)   to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-zba-cod-ant       to   w-mpn-cod-ant          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      22                   to   v-pos                  .
           move      rf-zba-rig-ant (2)   to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           if        rf-zba-rig-ant (3)   =    spaces
                     go to qry-liv-det-999.
      *                  *---------------------------------------------*
      *                  * Interlinea                                  *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      22                   to   v-pos                  .
           move      rf-zba-rig-ant (3)   to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-999.
           exit.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-ant'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
