      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Pulizia documenti di bollettazione          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "   PULIZIA MOVIMENTI DI BOLLETTAZIONE   "       .
      ...
      *        *-------------------------------------------------------*
      *        * Flag indicatore di sottoposizione a fatturazione      *
      *        * differita.                                            *
      *        *                                                       *
      *        *  - S : In caso di bolle di consegna soggette ad es-   *
      *        *        sere successivamente sottoposte a fatturazio-  *
      *        *        ne differita si puliscono solo le bolle gia'   *
      *        *        sottoposte a fatturazione.                     *
      *        *  - N : Anche in caso di bolle di consegna soggette    *
      *        *        ad essere successivamente sottoposte a fattu-  *
      *        *        razione differita non viene eseguito alcun     *
      *        *        test, e i documenti entro la data indicata     *
      *        *        vengono comunque puliti.                       *
      *        *                                                       *
      *        * Inizialmente viene proposto il valore 'S'.            *
      *        *-------------------------------------------------------*
           05  rr-snx-saf                 pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Conferma esecuzione                                   *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/No sottoposizione a fatturazione        *
      *        *-------------------------------------------------------*
           05  w-exp-snx-saf.
               10  w-exp-snx-saf-num      pic  9(02)       value 2    .
               10  w-exp-snx-saf-lun      pic  9(02)       value 02   .
               10  w-exp-snx-saf-tbl.
      ...
      *    *-----------------------------------------------------------*
       01  f-xxx.
           05  f-xxx-nam                  pic  x(04)   value "bit "   .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-300.
      *                  *---------------------------------------------*
      *                  * Si/No solo bolle sottoposte alla fattura-   *
      *                  * zione automatica                            *
      *                  *---------------------------------------------*
           perform   acc-snx-saf-000      thru acc-snx-saf-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
      *              * Si/No sottoposizione a fatturazione             *
      *              *-------------------------------------------------*
           perform   pmt-snx-saf-000      thru pmt-snx-saf-999        .
      *              *-------------------------------------------------*
      *              * Nota bene                                       *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts : Si/No sottoposizione a fattu-   *
      *    *                           razione differita               *
      *    *-----------------------------------------------------------*
       pmt-snx-saf-000.
           move      "DS"                 to   v-ope                  .
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Solo bolle di consegna gia' sottoposte a fatturazi
      -              "one differita .. :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *    *-----------------------------------------------------------*
       pmt-not-ben-000.
           ...
           move      14                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "N.B.: Se la gestione ordini clienti e' attiva, e' 
      -              "necessario consolidare gli    "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
           ...
           move      15                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "      ordini stessi alla data di pulizia prima di 
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Si/No sottoposizione alla  *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-saf-lun    to   v-car                  .
           move      w-exp-snx-saf-num    to   v-ldt                  .
           move      spaces               to   v-edm                  .
           move      09                   to   v-lin                  .
           move      70                   to   v-pos                  .
           move      w-exp-snx-saf-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           if        rr-snx-saf           =    "S"
                     move  01             to   v-num
           else if   rr-snx-saf           =    "N"
                     move  02             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-saf-999.
       acc-snx-saf-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "S"            to   rr-snx-saf
           else if   v-num                =    02
                     move  "N"            to   rr-snx-saf
           else      move  spaces         to   rr-snx-saf             .
       acc-snx-saf-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-snx-saf           not  = "S" and
                     rr-snx-saf           not  = "N"
                     go to acc-snx-saf-100.
       acc-snx-saf-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      ...
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Test su sottoposizione a fatturazione           *
      *              *-------------------------------------------------*
       tdo-ric-sel-210.
           if        rr-snx-saf           not  = spaces
                     go to tdo-ric-sel-220.
           move      "Manca la scelta per la sottoposizione alla fattura
      -              "zione !        "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-220.
           if        rr-snx-saf           =    "S" or
                     rr-snx-saf           =    "N"
                     go to tdo-ric-sel-300.
           move      "Scelta per la sottoposizione alla fatturazione err
      ...
      *                  *---------------------------------------------*
       ...
       pul-let-seq-200.
      *              *-------------------------------------------------*
      *              * Incremento contatore records [bit] letti        *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      09                   to   v-car                  .
      ...
      *              * tare                                            *
      *              *-------------------------------------------------*
           if        rf-bit-dat-doc       >    rr-dat-pul
                     go to pul-sel-rec-800.
       pul-sel-rec-100.
      *              *-------------------------------------------------*
      *              * Selezione su ordini clienti e ordini di spedi-  *
      *              * zione                                           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start su file [bir]                         *
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-bir                 .
      *                      *-----------------------------------------*
      *                      * Test su esito operazione : se Start er- *
      *                      * rata fine ciclo                         *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Controllo ordine cliente                *
      *                      *-----------------------------------------*
           if        rf-bir-coc-prt       =    zero and
                     rf-bir-coc-prg       =    zero
                     go to pul-sel-rec-160.
      *                      *-----------------------------------------*
      *                      * Lettura record [oct]                    *
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
      ...
      *                      *-----------------------------------------*
           if        rf-bir-ods-prt       =    zero and
                     rf-bir-ods-prg       =    zero
                     go to pul-sel-rec-180.
      *                      *-----------------------------------------*
      *                      * Lettura record [ost]                    *
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      rf-bir-ods-prt       to   rf-ost-num-prt         .
           move      "pgm/ods/fls/ioc/obj/iofost"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           go to     pul-sel-rec-120.
       pul-sel-rec-300.
      *              *-------------------------------------------------*
      *              * Se pulizia anche delle bolle da fatturare, il   *
      *              * movimento va' eliminato                         *
      *              *-------------------------------------------------*
           if        rr-snx-saf           =    "N"
                     go to pul-sel-rec-900.
      *              *-------------------------------------------------*
      *              * Se il movimento non deve essere successivamente *
      *              * sottoposto a fatturazione, esso va' eliminato   *
      *              *-------------------------------------------------*
           if        rf-bit-int-ftr       not  = 02
                     go to pul-sel-rec-900.
      *              *-------------------------------------------------*
      *              * Se il movimento e' stato gia' sottoposto a fat- *
      *              * turazione, esso va' eliminato                   *
      *              *-------------------------------------------------*
           if        rf-bit-fat-snx       =    "S"    and
                     rf-bit-fat-dat       not  = zero and
                     rf-bit-fat-num       not  = zero and
                     rf-bit-fat-npb       not  = zero
                     go to pul-sel-rec-900.
       pul-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Incremento contatore records [bit] scritti  *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
           move      09                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
           move      "#"                  to   w-cnt-pul-flg-sub      .
      *                  *---------------------------------------------*
      *                  * Incremento contatore records [bit] elimina- *
      *                  * ti                                          *
      *                  *---------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "N"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Normalizzazione contatore records [bir]     *
      *                  * scritti                                     *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        rf-bir-num-prt       not  = rf-bit-num-prt
                     go to pul-put-app-200.
      *                  *---------------------------------------------*
      *                  * Incremento contatore records [bir] scritti  *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       pul-put-app-200.
      *              *-------------------------------------------------*
      *              * Test su contatore records [bir] scritti : se a  *
      *              * zero si esce                                    *
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       pul-put-app-400.
      *                  *---------------------------------------------*
      *                  * Lettura sequenziale record [bix]            *
      *                  *---------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/bol/fls/ioc/obj/iofbix"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-bix                 .
      *                  *---------------------------------------------*
      *                  * Riciclo su lettura sequenziale record [bix] *
      *                  *---------------------------------------------*
