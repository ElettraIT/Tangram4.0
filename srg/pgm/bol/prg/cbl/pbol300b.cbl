      ...
      *            *---------------------------------------------------*
               ...
               10  w-buf-rig-ods-dst      pic  9(11)                  .
               10  w-buf-rig-ods-prw      pic  9(11)                  .
      ...
      *                  *---------------------------------------------*
           if        w-prs-ods-ape-snc    =    "S"
                     go to acc-dti-ods-460
           else if   w-prs-ods-ape-snc    =    "L"
                     go to acc-dti-ods-480
           else      go to acc-dti-ods-500.
       acc-dti-ods-460.
      *                  *---------------------------------------------*
      *                  * Se determinazione su singolo ordine         *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Preparazione protocollo                 *
      *                      *-----------------------------------------*
           move      rf-ost-num-prt       to   w-buf-rig-ods-prw      .
      *                      *-----------------------------------------*
      *                      * Determinazione                          *
      *                      *-----------------------------------------*
           perform   det-rig-spn-sng-000  thru det-rig-spn-sng-999    .
      *                      *-----------------------------------------*
      *                      * Test                                    *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       acc-dti-ods-480.
      *                  *---------------------------------------------*
      *                  * Se determinazione su check list             *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Determinazione                          *
      *                      *-----------------------------------------*
           perform   det-rig-spn-chk-000  thru det-rig-spn-chk-999    .
      *                      *-----------------------------------------*
      *                      * Test sul protocollo distinta            *
      *                      *-----------------------------------------*
           if        w-buf-rig-ods-dst    not  = zero
                     go to acc-dti-ods-490.
      *                      *-----------------------------------------*
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "Ordine di spedizione non richiamato in alcuna chec
      -              "k list!        "    to   w-err-box-err-msg      .
           perform   box-msg-err-000      thru box-msg-err-999        .
      *                      *-----------------------------------------*
      *                      * A reimpostazione                        *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      "<B"                 to   v-edm                  .
           move      w-buf-rig-ods-dst
                    (06 : 06)             to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-all-str-cat (2)      .
      *                      *-----------------------------------------*
      *                      * Assemblaggio parte due                  *
      *                      *-----------------------------------------*
           move      80                   to   w-all-str-lun          .
           move      03                   to   w-all-str-num          .
           move      "della check-list"   to   w-all-str-cat (1)      .
           move      "non completata!"    to   w-all-str-cat (3)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                      *-----------------------------------------*
      *                      * Messaggio di errore                     *
      *                      *-----------------------------------------*
           move      "Ordine di spedizione non completamente verificato 
      -              "o all'interno  "    to   w-err-box-err-msg      .
      ...
      *                  *---------------------------------------------*
       ...
       acc-dti-ods-530.
      *              *-------------------------------------------------*
      *              * Test di congruenza tra il tipo movimento per la *
      *              * bolla ed un eventuale default presente nella    *
      *              * testata dell'ordine di spedizione               *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se default presente                    *
      *                  *---------------------------------------------*
           if        rf-ost-bcd-tip       =     spaces
                     go to acc-dti-ods-540.
      *                  *---------------------------------------------*
      *                  * Confronto con tipo bolla                    *
      *                  *---------------------------------------------*
           if        rf-ost-bcd-tip       =     w-tes-tip-mpb
                     go to acc-dti-ods-540.
      *                  *---------------------------------------------*
      *                  * Assemblaggio messaggio di errore            *
      *                  *---------------------------------------------*
           move      60                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      "Ordine di spedizione che prevede una bolla di tipo
      -              " "                  to   w-all-str-cat (1)      .
           move      rf-ost-bcd-tip       to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                  *---------------------------------------------*
      *                  * Messaggio di errore                         *
      *                  *---------------------------------------------*
      ...
      *    *                                                           *
      *    * Subroutine per controllo su singolo ordine                *
      *    *-----------------------------------------------------------*
       det-rig-spn-sng-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore elementi              *
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "NUMPRT    "         to   f-key                  .
           move      w-buf-rig-ods-prw    to   rf-osr-num-prt         .
           move      zero                 to   rf-osr-num-prg         .
           move      "pgm/ods/fls/ioc/obj/iofosr"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-osr-num-prt       not  = w-buf-rig-ods-prw
                     go to det-rig-spn-sng-900.
       det-rig-spn-sng-400.
      *              *-------------------------------------------------*
      *              * Selezioni su [osr]                              *
      ...
      *    * Determinazione righe spuntate                             *
      *    *                                                           *
      *    * Subroutine per controllo su check list                    *
      *    *-----------------------------------------------------------*
       det-rig-spn-chk-000.
      *              *-------------------------------------------------*
      *              * Determinazione preliminare numero distinta in   *
      *              * cui compare l'ordine di spedizione in corso di  *
      *              * trattamento                                     *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Subroutine                                  *
      *                  *---------------------------------------------*
           perform   det-rig-spn-dst-000  thru det-rig-spn-dst-999    .
      *                  *---------------------------------------------*
      *                  * Se numero distinta non determinato: ad      *
      *                  * uscita                                      *
      *                  *---------------------------------------------*
           if        w-buf-rig-ods-dst    =    zero
                     go to det-rig-spn-chk-900.
       det-rig-spn-chk-100.
      *              *-------------------------------------------------*
      *              * Start su file [osd]                             *
      *              *-------------------------------------------------*
           ...
           move      w-buf-rig-ods-dst    to   rf-osd-num-dst         .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-osd-num-dst       not  = w-buf-rig-ods-dst
                     go to det-rig-spn-chk-900.
       det-rig-spn-chk-400.
      *              *-------------------------------------------------*
      *              * Selezioni su [osd]                              *
      *              *-------------------------------------------------*
       det-rig-spn-chk-500.
      *              *-------------------------------------------------*
      *              * Preparazione protocollo per il controllo        *
      *              *-------------------------------------------------*
           move      rf-osd-num-prt       to   w-buf-rig-ods-prw      .
      *              *-------------------------------------------------*
      *              * Ciclo di controllo singolo ordine               *
      *              *-------------------------------------------------*
           perform   det-rig-spn-sng-000  thru det-rig-spn-sng-999    .
       det-rig-spn-chk-800.
      *              *-------------------------------------------------*
      *              * Riciclo su distinta                             *
      *              *-------------------------------------------------*
           go to     det-rig-spn-chk-200.
       det-rig-spn-chk-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     det-rig-spn-chk-999.
       det-rig-spn-chk-999.
           exit.
      ...
      *    * Subroutine per controllo su check list                    *
      *    *                                                           *
      *    * Sub-subroutine per determinazione numero distinta         *
      *    *-----------------------------------------------------------*
       det-rig-spn-dst-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione preliminare                     *
      *              *-------------------------------------------------*
           move      zero                 to   w-buf-rig-ods-dst      .
       det-rig-spn-dst-100.
      *              *-------------------------------------------------*
      *              * Start su file [osd]                             *
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      "NUMPRT    "         to   f-key                  .
           move      rf-ost-num-prt       to   rf-osd-num-prt         .
           move      zero                 to   rf-osd-num-prg         .
           move      "pgm/ods/fls/ioc/obj/iofosd"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to det-rig-spn-dst-900.
       det-rig-spn-dst-200.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale file [osd]                  *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/ods/fls/ioc/obj/iofosd"
                                          to   s-pat                  .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-osd-num-prt       not  = rf-ost-num-prt
                     go to det-rig-spn-dst-900.
       det-rig-spn-dst-400.
      *              *-------------------------------------------------*
      *              * Selezioni su [osd]                              *
      ...
      *              *-------------------------------------------------*
           move      rf-osd-num-dst       to   w-buf-rig-ods-dst      .
       det-rig-spn-dst-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     det-rig-spn-dst-999.
       det-rig-spn-dst-999.
           exit.
