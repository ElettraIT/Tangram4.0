      ...
      * ============================================================== *
      *                                                                *
      * Convenzioni    :   le referenze 'prm-inv' contengono tutti     *
      *                    i parametri relativi all'invio tra cui il   *
      *                    tipo di modulo per invio tramite mail       *
      *                    normale (t_sender) o PEC (t_sender_ssl).    *
      *                                                                *
      *                    Sono comunque sovrascritte dai parametri    *
      *                    del file 'bol' gestito da 'bol001'          *
      *                                                                *
      *================================================================*
      ...
      *    *===========================================================*
      *    * Work per subroutines di determinazion numero distinta     *
      *    *-----------------------------------------------------------*
       01  w-num-dst.
           05  w-num-dst-scl-ann          pic  9(03)                  .
           05  w-num-dst-num-dst          pic  9(09)                  .
           05  w-num-dst-snx-aut          pic  x(01)                  .
      ...
      *                  *---------------------------------------------*
           move      "mbol.txt"           to   f-xxx-msg              .
      *                  *---------------------------------------------*
      *                  * Default nome testo soggetto                 *
      *                  *---------------------------------------------*
           move      "sbol.txt"           to   f-xxx-sgt              .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       pre-exe-pgm-500.
      *              *-------------------------------------------------*
      *              * Lettura se in esecuzione batch                  *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/bol/mov/bol540[sec-att]"
                                          to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
      ...
      *              * Forzature                                       *
      *              *-------------------------------------------------*
           move      05                   to   rr-tip-ope             .
      *              *-------------------------------------------------*
      *              * Determinazione ultimo numero distinta attribui- *
      *              * to, se automatico                               *
      *              *-------------------------------------------------*
           perform   det-num-dst-000      thru det-num-dst-999        .
           if        w-num-dst-num-dst    =    zero
                     go to stp-srt-inp-900.
           if        w-num-dst-snx-aut    not  = "A"
                     go to stp-srt-inp-900.
           move      w-num-dst-num-dst    to   rr-num-dst             .
       stp-srt-inp-050.
      *              *-------------------------------------------------*
      ...
      *                          *-------------------------------------*
       ...
       stp-srt-inp-460.
      *                      *-----------------------------------------*
      *                      * Se re-invio documenti con esito negat.  *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Test su esito                       *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-dbe-prt-doc       to   rf-bit-num-prt         .
           move      "pgm/bol/fls/ioc/obj/iofbit"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * documento                           *
      *                          *-------------------------------------*
           move      rf-bit-num-doc       to   w-det-rec-fil-num      .
      *                          *-------------------------------------*
      *                          * Scomposizione percorso allegato per *
      *                          * confrontare nome file con il nume-  *
      *                          * ro documento                        *
      *                          *                                     *
      *                          * Si presume il formato:              *
      *                          *                                     *
      *                          * /abd/asc/doc/bol/AZIE_YY_NNNNNN.pdf *
      *                          *                                     *
      *                          * a meno che sia disattivata la       *
      *                          * funzione 'pdftk' per cui il nome    *
      *                          * file e' privo della sigla azienda   *
      *                          *-------------------------------------*
           move      rf-dbe-eml-all       to   w-all-str-alf          .
           move      "/"                  to   w-all-str-del          .
           perform   all-str-ext-000      thru all-str-ext-999        .
      *
           move      w-all-str-cat (6)    to   w-all-str-alf          .
           move      "."                  to   w-all-str-del          .
           perform   all-str-ext-000      thru all-str-ext-999        .
      *
      ...
      *
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                          *-------------------------------------*
      *                          * Confronto con numero documento      *
      *                          *-------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Incremento contatore records [dbe] letti    *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Incremento contatore records [dbe] aggiornati   *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-dbe-dat-inv         .
      *              *-------------------------------------------------*
      *              * Aggiornamento ora invio                         *
      *              *-------------------------------------------------*
           move      s-ora                to   rf-dbe-ora-inv         .
           multiply  100                  by   rf-dbe-ora-inv         .
           add       s-min                to   rf-dbe-ora-inv         .
      *              *-------------------------------------------------*
      *              * Update record [dbe]                             *
      *              *-------------------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   w-num-dst-snx-aut      .
       det-num-dst-100.
      *              *-------------------------------------------------*
      ...
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
           go to     det-num-dst-999.
       det-num-dst-600.
      *              *-------------------------------------------------*
      *              * Se record esistente                             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Salvataggio valori letti                    *
      *                  *---------------------------------------------*
           move      rn-num-dbe-num-dst   to   w-num-dst-num-dst      .
           move      rn-num-dbe-snx-aut   to   w-num-dst-snx-aut      .
      *                  *---------------------------------------------*
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
           go to     det-num-dst-999.
