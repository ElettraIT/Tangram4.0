      ...
      *                       Ultima revisione:    NdK del 10/07/24    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione overlay per il programma bol540  *
      *                                                                *
      ...
      *              *-------------------------------------------------*
           move      "OP"                 to   f-ope                  .
           move      "pgm/bol/num/ioc/obj/innumdbe"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
           call      s-pat               using f
      ...
      *
           ...
           move      00000000000          to   rr-num-min             .
           move      99999999999          to   rr-num-max             .
           move      zero                 to   rr-cod-cli             .
           move      zero                 to   rr-cod-cli-sns         .
           move      01                   to   rr-sne-doc             .
           move      "eml"                to   rr-prm-inv (7)         .
       stp-srt-inp-050.
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           if        rf-bit-cod-tmb       =    "BVFS "
                     go to stp-srt-inp-490.
           if        rf-bit-cod-tmb       =    "BVFO "
                     go to stp-srt-inp-490.
           go to     stp-srt-inp-200.
       stp-srt-inp-405.
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        rf-bit-num-doc       <    rr-nin-min or
                     rf-bit-num-doc       >    rr-nin-max
                     go to stp-srt-inp-200.
       stp-srt-inp-420.
       stp-srt-inp-440.
       stp-srt-inp-480.
      ...
      *                  *---------------------------------------------*
       stp-srt-inp-500.
      *                  *---------------------------------------------*
      *                  * Test se bolla gia' presente in distinta di  *
      *                  * spedizione                                  *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           perform   stp-srt-inp-dbe-000  thru stp-srt-inp-dbe-999    .
           if        w-det-dst-ctr-ele    >    zero
                     go to stp-srt-inp-200.
       stp-srt-inp-520.
      *                  *---------------------------------------------*
      *                  * Test se bolla gia' verificata               *
      *                  *---------------------------------------------*
       stp-srt-inp-550.
      *                  *---------------------------------------------*
      ...
      *
           ...
           move      rf-bit-cod-age       to   d-con-arc-cod-arc      .
           move      spaces               to   d-con-arc-dpz-arc      .
           move      "bol"                to   d-con-arc-tip-sel      .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-det-dst-num-prt    to   rf-dbe-prt-doc         .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-srt-inp-dbe-900.
       stp-srt-inp-dbe-200.
      *              *-------------------------------------------------*
      *              * Lettura sequenziale [dbe]                       *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/bol/fls/ioc/obj/iofdbe"
                                          to   s-pat                  .
      ...
      *              * Test sul massimo                                *
      *              *-------------------------------------------------*
           if        rf-dbe-prt-doc       not  = w-det-dst-num-prt
                     go to stp-srt-inp-dbe-900.
       stp-srt-inp-dbe-400.
      *              *-------------------------------------------------*
      *              * Selezioni                                       *
      ...
      *              * Riciclo                                         *
      *              *-------------------------------------------------*
           go to     stp-srt-inp-dbe-200.
       stp-srt-inp-dbe-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     stp-srt-inp-dbe-999.
       stp-srt-inp-dbe-999.
           exit.
      ...
      *              *-------------------------------------------------*
       ...
       prn-liv-det-200.
      *              *-------------------------------------------------*
      *              * Preparazione dati per scrittura [dbe]           *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *  - Anno documento                           *
      *                  *  - Sigla numerazione                        *
      *                  *  - Codice dipendenza (ATTUALMENTE NO)       *
      *                  *  - Numero documento                         *
      *                  *                                             *
      *                  * AABBBCC_999999                              *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      spaces               to   p-sgn                  .
           move      "<"                  to   p-edm                  .
           move      rf-bit-num-doc       to   w-wrk-nft-int          .
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-bit-sgl-num       to   w-all-str-cat (2)      .
           move      "_"                  to   w-all-str-cat (3)      .
           move      w-wrk-doc-nde        to   w-all-str-cat (4)      .
           perform   all-str-cat-000      thru all-str-cat-999        .
      *                  *---------------------------------------------*
      ...
      *              * zioni [numdbe]                                  *
      *              *-------------------------------------------------*
           move      w-num-dst-scl-ann    to   rn-num-dbe-scl-ann     .
      *              *-------------------------------------------------*
      *              * Lettura tabella numerazioni [numdbe]            *
      *              *-------------------------------------------------*
           move      "GT"                 to   f-ope                  .
           move      "pgm/bol/num/ioc/obj/innumdbe"
                                          to   s-pat                  .
      ...
      *                  *---------------------------------------------*
           move      rn-num-dbe-num-dst   to   w-num-dst-num-dst      .
      ...
      *                  *---------------------------------------------*
           move      w-num-dst-num-dst    to   rn-num-dbe-num-dst     .
      *
           if        w-gen-flg-btc        =    "S"
                     move  "A"            to   rn-num-dbe-snx-aut
           else      move  "M"            to   rn-num-dbe-snx-aut     .
      *                  *---------------------------------------------*
      *                  * Update record                               *
      *                  *---------------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-dbe-ide-dat         .
           move      s-ute                to   rf-dbe-ide-ute         .
           move      s-fas                to   rf-dbe-ide-fas         .
           move      rr-num-dst           to   rf-dbe-num-dst         .
           move      w-cix-num-ele        to   rf-dbe-num-prg         .
           move      s-dat                to   rf-dbe-dat-reg         .
           move      zero                 to   rf-dbe-tip-dst         .
           move      "C"                  to   rf-dbe-tip-arc         .
           move      srt-cod-cli          to   rf-dbe-cod-arc         .
           move      srt-dpz-cli          to   rf-dbe-dpz-arc         .
           move      zero                 to   rf-dbe-tip-ids         .
           move      rr-prm-inv (4)       to   rf-dbe-eml-mit         .
           move      srt-eml-ind          to   rf-dbe-eml-ind         .
           move      spaces               to   rf-dbe-eml-all         .
           move      zero                 to   rf-dbe-tip-doc         .
           move      rf-bit-cod-tmb       to   rf-dbe-tmo-doc         .
           move      rf-bit-dat-doc       to   rf-dbe-dat-doc         .
           move      w-wrk-doc-nda        to   rf-dbe-num-doc         .
           move      rf-bit-num-prt       to   rf-dbe-prt-doc         .
           ...
           move      srt-eml-tip          to   rf-dbe-tip-con         .
