      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Manutenzione riferimenti fattura differita  *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "GESTIONE RIFERIMENTI A FATTURA DIFFERITA"       .
      ...
      *            *---------------------------------------------------*
      *            * Si/no solo fatturati                              *
      *            *---------------------------------------------------*
               10  w-tes-snx-fat          pic  9(02)                  .
      *            *---------------------------------------------------*
      ...
      *        * Numero fattura                                        *
      *        *-------------------------------------------------------*
           05  w-rig-num-fat              pic  9(11)                  .
      *        *-------------------------------------------------------*
      *        * Data fattura                                          *
      *        *-------------------------------------------------------*
           05  w-rig-dat-fat              pic  9(07)                  .
      *        *-------------------------------------------------------*
      *        * Totale documento                                      *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no da fatturare                         *
      *        *-------------------------------------------------------*
           05  w-exp-snx-fat.
               10  w-exp-snx-fat-num      pic  9(02)       value 03   .
               10  w-exp-snx-fat-lun      pic  9(02)       value 40   .
               10  w-exp-snx-fat-tbl.
                   15  filler             pic  x(40) value
                            "Tutti i documenti                       ".
                   15  filler             pic  x(40) value
                            "Solo quelli Fatturati                   ".
                   15  filler             pic  x(40) value
                            "Solo quelli Da fatturare                ".
      ...
      *                  * Si/no solo sospesi                          *
      *                  *---------------------------------------------*
           perform   acc-snx-fat-000      thru acc-snx-fat-999        .
           if        w-cnt-tus-acc-key    =    "E"
                     move  spaces         to   w-cnt-tus-acc-key
                     go to acc-key-reg-100.
           if        w-cnt-tus-acc-key    not  = spaces
      ...
      *              * Si/no solo sospesi                              *
      *              *-------------------------------------------------*
           perform   vis-snx-fat-000      thru vis-snx-fat-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Si/no solo sospesi                          *
      *                  *---------------------------------------------*
           perform   pmt-snx-fat-000      thru pmt-snx-fat-999        .
       pmt-key-reg-999.
           exit.
      ...
      *    *===========================================================*
      *    * Accettazione : Si/no da fatturare                         *
      *    *-----------------------------------------------------------*
       acc-snx-fat-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-fat-lun    to   v-car                  .
           move      w-exp-snx-fat-num    to   v-ldt                  .
           move      "TFD#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-fat-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-snx-fat        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-fat-999.
       acc-snx-fat-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-fat          .
       acc-snx-fat-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione : Si/no da fatturare                      *
      *    *-----------------------------------------------------------*
       vis-snx-fat-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *                  * Numero fattura                              *
      *                  *---------------------------------------------*
           perform   acc-num-fat-000      thru acc-num-fat-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
      ...
      *                  * Data fattura                                *
      *                  *---------------------------------------------*
           perform   acc-dat-fat-000      thru acc-dat-fat-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
                     go to acc-cor-reg-900.
           if        v-key                =    "UP  "
      ...
      *                  *---------------------------------------------*
       ...
       vis-lin-cor-490.
      *                  *---------------------------------------------*
      *                  * Si/no documento da fatturare                *
      *                  *---------------------------------------------*
           if        w-rig-num-fat        not  = zero
                     move  "*"            to   w-lin-imm-snx-bdi
           else      move  spaces         to   w-lin-imm-snx-bdi      .
       vis-lin-cor-500.
      *                  *---------------------------------------------*
      ...
      *              * Numero fattura                                  *
      *              *-------------------------------------------------*
           perform   vis-num-fat-000      thru vis-num-fat-999        .
      *              *-------------------------------------------------*
      *              * Data fattura                                    *
      *              *-------------------------------------------------*
           perform   vis-dat-fat-000      thru vis-dat-fat-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *              * Numero fattura                                  *
      *              *-------------------------------------------------*
           perform   pmt-num-fat-000      thru pmt-num-fat-999        .
      *              *-------------------------------------------------*
      *              * Data fattura                                    *
      *              *-------------------------------------------------*
           perform   pmt-dat-fat-000      thru pmt-dat-fat-999        .
      *              *-------------------------------------------------*
      *              * Estremi documento                               *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Numero fattura                   *
      *    *-----------------------------------------------------------*
       pmt-num-fat-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Data fattura                     *
      *    *-----------------------------------------------------------*
       pmt-dat-fat-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Numero fattura                       *
      *    *-----------------------------------------------------------*
       acc-num-fat-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  *---------------------------------------------*
           move      w-rig-num-fat        to   w-acc-num-ext          .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-num-fat-999.
      *              *-------------------------------------------------*
      *              * Valore impostato                                *
      *              *-------------------------------------------------*
           if        v-num                =    zero
                     move  zero           to   w-rig-num-fat
                     go to acc-num-fat-400.
           ...
           move      w-acc-num-ext        to   w-rig-num-fat          .
       acc-num-fat-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-rig-flg
                             ...
                             go to acc-num-fat-100.
       acc-num-fat-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Numero fattura                    *
      *    *-----------------------------------------------------------*
       vis-num-fat-000.
      *              *-------------------------------------------------*
      *              * Pre-visualizzazione                             *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-dat-fat-999.
      *              *-------------------------------------------------*
      *              * Valore impostato                                *
      *              *-------------------------------------------------*
           move      v-dat                to   w-rig-dat-fat          .
       acc-dat-fat-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-rig-num-fat        =    zero
                     go to cnt-tdo-rig-800.
           if        w-rig-dat-fat        not  = zero
                     go to cnt-tdo-rig-800.
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Selezione su bolle da fatturare             *
      *                  *---------------------------------------------*
           if        rf-bit-fat-snx       =    "N"
                     go to stp-srt-inp-200.
       stp-srt-inp-420.
      *                  *---------------------------------------------*
      *                  * Selezione su estremi fatturazione           *
      *                  *---------------------------------------------*
           go to     stp-srt-inp-422
                     stp-srt-inp-424
                     stp-srt-inp-426
                     depending            on   w-tes-snx-fat          .
       stp-srt-inp-422.
      *                  *---------------------------------------------*
      *                  * Tutti i documenti                           *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-bit-fat-dat       to   w-rig-dat-fat          .
           move      rf-bit-fat-num       to   w-rig-num-fat          .
      *                  *---------------------------------------------*
      *                  * Valori indiretti                            *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      w-rig-num-fat        to   rf-bit-fat-num         .
      *                  *---------------------------------------------*
      *                  * Aggiornamento data fattura                  *
      *                  *---------------------------------------------*
           move      w-rig-dat-fat        to   rf-bit-fat-dat         .
      *                  *---------------------------------------------*
      *                  * Aggiornamento progressivo bolla in fattura  *
      *                  *---------------------------------------------*
           if        w-rig-num-fat        =    zero
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/bol/prg/obj/pbol9302"                       .
       cnc-rlt-sup-999.
           exit.
