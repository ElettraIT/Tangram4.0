      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione anagrafica nostri c/c postali      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " GESTIONE ANAGRAFICA NOSTRI C/C POSTALI "       .
      ...
      *            *---------------------------------------------------*
      *            * Sigla del conto corrente postale                  *
      *            *---------------------------------------------------*
               10  w-tes-sgl-ccp          pic  x(12)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per bonifici postali attivi  *
      *            *---------------------------------------------------*
               10  w-tes-stc-bpa          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per bonifici postali attivi, *
      *            * descrizione                                       *
      *            *---------------------------------------------------*
               10  w-tes-stc-bpa-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per bonifici postali passivi *
      *            *---------------------------------------------------*
               10  w-tes-stc-bpp          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per bonifici postali passivi,*
      *            * descrizione                                       *
      *            *---------------------------------------------------*
               10  w-tes-stc-bpp-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per oneri postali            *
      *            *---------------------------------------------------*
               10  w-tes-stc-onp          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per oneri postali, descri-   *
      *            * zione                                             *
      *            *---------------------------------------------------*
               10  w-tes-stc-onp-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Area libera per espansioni speciali relative alle *
      *            * informazioni sul nostro c/c postale               *
      *            *---------------------------------------------------*
               10  w-tes-alx-003.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice nostro c/c postale     *
      *    *-----------------------------------------------------------*
       pmt-cod-cbp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              * Tipo record : 03                                *
      *              *-------------------------------------------------*
           move      03                   to   w-tes-tip-cbp          .
       acc-tip-cbp-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo chiave : Codice nostro c/c postale     *
      *    *-----------------------------------------------------------*
       acc-cod-cbp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice nostro c/c postale  *
      *    *-----------------------------------------------------------*
       vis-cod-cbp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                      *-----------------------------------------*
       ...
       acc-tes-reg-110.
      *                      *-----------------------------------------*
      *                      * Sigla del c/c postale                   *
      *                      *-----------------------------------------*
           perform   acc-sgl-ccp-000      thru acc-sgl-ccp-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
       ...
       acc-tes-reg-120.
      *                      *-----------------------------------------*
      *                      * Codice del sottoconto per bonifici po-  *
      *                      * stali attivi                            *
      *                      *-----------------------------------------*
           perform   acc-stc-bpa-000      thru acc-stc-bpa-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *                      * stali passivi                           *
      *                      *-----------------------------------------*
           perform   acc-stc-bpp-000      thru acc-stc-bpp-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
       ...
       acc-tes-reg-140.
      *                      *-----------------------------------------*
      *                      * Sottoconto per oneri postali            *
      *                      *-----------------------------------------*
           perform   acc-stc-onp-000      thru acc-stc-onp-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *                  * Sigla del c/c postale                       *
      *                  *---------------------------------------------*
           perform   vis-sgl-ccp-000      thru vis-sgl-ccp-999        .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per bonifici postali  *
      *                  * attivi                                      *
      *                  *---------------------------------------------*
           perform   vis-stc-bpa-000      thru vis-stc-bpa-999        .
           perform   vis-stc-bpa-des-000  thru vis-stc-bpa-des-999    .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per bonifici postali  *
      *                  * passivi                                     *
      *                  *---------------------------------------------*
           perform   vis-stc-bpp-000      thru vis-stc-bpp-999        .
           perform   vis-stc-bpp-des-000  thru vis-stc-bpp-des-999    .
      *                  *---------------------------------------------*
      *                  * Sottoconto per oneri postali                *
      *                  *---------------------------------------------*
           perform   vis-stc-onp-000      thru vis-stc-onp-999        .
           perform   vis-stc-onp-des-000  thru vis-stc-onp-des-999    .
      *                  *---------------------------------------------*
      *                  * Fine Pagina 1                               *
      *                  *---------------------------------------------*
           go to     vis-tes-reg-999.
      ...
      *                  * Sigla del c/c postale                       *
      *                  *---------------------------------------------*
           perform   pmt-sgl-ccp-000      thru pmt-sgl-ccp-999        .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per bonifici postali  *
      *                  * attivi                                      *
      *                  *---------------------------------------------*
           perform   pmt-stc-bpa-000      thru pmt-stc-bpa-999        .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per bonifici postali  *
      *                  * passivi                                     *
      *                  *---------------------------------------------*
           perform   pmt-stc-bpp-000      thru pmt-stc-bpp-999        .
      *                  *---------------------------------------------*
      *                  * Sottoconto per oneri postali                *
      *                  *---------------------------------------------*
           perform   pmt-stc-onp-000      thru pmt-stc-onp-999        .
      *                  *---------------------------------------------*
      *                  * Fine Pagina 1                               *
      *                  *---------------------------------------------*
           go to     pmt-tes-reg-999.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Sigla del c/c postale            *
      *    *-----------------------------------------------------------*
       pmt-sgl-ccp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Sottoconto per oneri postali     *
      *    *-----------------------------------------------------------*
       pmt-stc-onp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Sigla del c/c postale                *
      *    *-----------------------------------------------------------*
       acc-sgl-ccp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-nok-flg
                             ...
                             go to acc-sgl-ccp-100.
       acc-sgl-ccp-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Sigla del c/c postale             *
      *    *-----------------------------------------------------------*
       vis-sgl-ccp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-stc-bpa (1)    to   w-cod-mne-pdc-cod      .
       ...
       acc-stc-bpa-110.
           ...
           if        w-cod-mne-pdc-ope    =    "F+"
                     go to acc-stc-bpa-115.
           if        w-cod-mne-pdc-ope    =    "AC"
                     go to acc-stc-bpa-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-stc-bpa-115.
           perform   cod-mne-pdc-foi-000  thru cod-mne-pdc-foi-999    .
           go to     acc-stc-bpa-110.
       acc-stc-bpa-120.
      ...
      *                  *---------------------------------------------*
           move      w-tes-stc-bpa (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-bpa-des (1)  .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-stc-bpa (1)    to   w-edt-cod-pdc-cod      .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Codice del sottoconto per boni-   *
      *    *                         fici postali attivi, descrizione  *
      *    *-----------------------------------------------------------*
       vis-stc-bpa-des-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-pdc-ope      .
           move      w-prs-liv-pdc        to   w-cod-mne-pdc-liv      .
           move      w-tes-stc-bpp (1)    to   w-cod-mne-pdc-cod      .
       ...
       acc-stc-bpp-110.
           perform   cod-mne-pdc-cll-000  thru cod-mne-pdc-cll-999    .
           if        w-cod-mne-pdc-ope    =    "F+"
                     go to acc-stc-bpp-115.
           if        w-cod-mne-pdc-ope    =    "AC"
                     go to acc-stc-bpp-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-stc-bpp-115.
           perform   cod-mne-pdc-foi-000  thru cod-mne-pdc-foi-999    .
           go to     acc-stc-bpp-110.
       acc-stc-bpp-120.
           move      w-cod-mne-pdc-cod    to   v-num                  .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      ...
      *                  * Lettura archivio piano dei conti            *
      *                  *---------------------------------------------*
           move      w-tes-stc-bpp (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-bpp-des (1)  .
      *                  *---------------------------------------------*
      *                  * Visualizzazione descrizione sottoconto      *
      *                  *---------------------------------------------*
           perform   vis-stc-bpp-des-000  thru vis-stc-bpp-des-999    .
      ...
      *              *-------------------------------------------------*
           move      w-prs-liv-pdc        to   w-edt-cod-pdc-liv      .
           move      w-tes-stc-bpp (1)    to   w-edt-cod-pdc-cod      .
           move      "B"                  to   w-edt-cod-pdc-edm      .
           perform   edt-pdc-asx-000      thru edt-pdc-asx-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione                                 *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Codice del sottoconto per boni-   *
      *    *                         fici postali passivi, descrizione *
      *    *-----------------------------------------------------------*
       vis-stc-bpp-des-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Sottoconto per oneri postali         *
      *    *-----------------------------------------------------------*
       acc-stc-onp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-pdc-ope      .
           move      w-prs-liv-pdc        to   w-cod-mne-pdc-liv      .
           move      w-tes-stc-onp (1)    to   w-cod-mne-pdc-cod      .
       ...
       acc-stc-onp-110.
           perform   cod-mne-pdc-cll-000  thru cod-mne-pdc-cll-999    .
           if        w-cod-mne-pdc-ope    =    "F+"
                     go to acc-stc-onp-115.
           if        w-cod-mne-pdc-ope    =    "AC"
                     go to acc-stc-onp-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-stc-onp-115.
           perform   cod-mne-pdc-foi-000  thru cod-mne-pdc-foi-999    .
           go to     acc-stc-onp-110.
       acc-stc-onp-120.
           move      w-cod-mne-pdc-cod    to   v-num                  .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      ...
      *                  * Lettura archivio piano dei conti            *
      *                  *---------------------------------------------*
           move      w-tes-stc-onp (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-onp-des (1)  .
      *                  *---------------------------------------------*
      *                  * Visualizzazione descrizione sottoconto      *
      *                  *---------------------------------------------*
           perform   vis-stc-onp-des-000  thru vis-stc-onp-des-999    .
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Sottoconto per oneri postali      *
      *    *-----------------------------------------------------------*
       vis-stc-onp-000.
      *              *-------------------------------------------------*
      *              * Editing con appoggio a sinistra                 *
      *              *-------------------------------------------------*
           move      w-prs-liv-pdc        to   w-edt-cod-pdc-liv      .
           move      w-tes-stc-onp (1)    to   w-edt-cod-pdc-cod      .
           move      "B"                  to   w-edt-cod-pdc-edm      .
           perform   edt-pdc-asx-000      thru edt-pdc-asx-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione                                 *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Sottoconto per oneri postali, de- *
      *    *                         scrizione                         *
      *    *-----------------------------------------------------------*
       vis-stc-onp-des-000.
           move      "DS"                 to   v-ope                  .
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           ...
           move      spaces               to   w-tes-sgl-ccp (1)      .
           move      zero                 to   w-tes-stc-bpa (1)      .
           move      spaces               to   w-tes-stc-bpa-des (1)  .
           move      zero                 to   w-tes-stc-bpp (1)      .
           move      spaces               to   w-tes-stc-bpp-des (1)  .
           move      zero                 to   w-tes-stc-onp (1)      .
           move      spaces               to   w-tes-stc-onp-des (1)  .
           move      spaces               to   w-tes-alx-003 (1)      .
       nor-nok-tes-999.
           exit.
      ...
      *                          *-------------------------------------*
           ...
           move      rf-cbp-sgl-ccp       to   w-tes-sgl-ccp (1)      .
           move      rf-cbp-stc-bpa       to   w-tes-stc-bpa (1)      .
           move      rf-cbp-stc-bpp       to   w-tes-stc-bpp (1)      .
           move      rf-cbp-stc-onp       to   w-tes-stc-onp (1)      .
           move      rf-cbp-alx-003       to   w-tes-alx-003 (1)      .
       rou-let-reg-300.
      *                          *-------------------------------------*
      ...
      *                              *---------------------------------*
      *                                  *-----------------------------*
      *                                  * Bonifici postali attivi     *
      *                                  *-----------------------------*
           move      w-tes-stc-bpa (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-bpa-des (1)  .
      *                                  *-----------------------------*
      *                                  * Bonifici postali passivi    *
      *                                  *-----------------------------*
           move      w-tes-stc-bpp (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-bpp-des (1)  .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-tes-sgl-ccp (1)    to   rf-cbp-sgl-ccp         .
           move      w-tes-stc-bpa (1)    to   rf-cbp-stc-bpa         .
           move      w-tes-stc-bpp (1)    to   rf-cbp-stc-bpp         .
           move      w-tes-stc-onp (1)    to   rf-cbp-stc-onp         .
           move      w-tes-alx-003 (1)    to   rf-cbp-alx-003         .
       cmp-rec-cbp-999.
           exit.
