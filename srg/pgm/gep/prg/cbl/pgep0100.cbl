      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione anagrafica nostre casse            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "    GESTIONE ANAGRAFICA NOSTRE CASSE    "       .
      ...
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per la cassa contanti        *
      *            *---------------------------------------------------*
               10  w-tes-stc-csc          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per la cassa contanti, de-   *
      *            * scrizione                                         *
      *            *---------------------------------------------------*
               10  w-tes-stc-csc-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per la cassa assegni         *
      *            *                                                   *
      *            * Nota : Non utilizzato                             *
      *            *---------------------------------------------------*
               10  w-tes-stc-csa          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per la cassa assegni, de-    *
      *            * scrizione                                         *
      *            *                                                   *
      *            * Nota : Non utilizzato                             *
      *            *---------------------------------------------------*
               10  w-tes-stc-csa-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per oneri su pagamenti       *
      *            *---------------------------------------------------*
               10  w-tes-stc-opc          pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Sottoconto contabile per oneri su pagamenti, de-  *
      *            * scrizione                                         *
      *            *---------------------------------------------------*
               10  w-tes-stc-opc-des      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Area libera per espansioni speciali relative alle *
      *            * informazioni sulla nostra cassa                   *
      *            *---------------------------------------------------*
               10  w-tes-alx-001.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Codice nostra cassa           *
      *    *-----------------------------------------------------------*
       pmt-cod-cbp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *              * Tipo record : 01                                *
      *              *-------------------------------------------------*
           move      01                   to   w-tes-tip-cbp          .
       acc-tip-cbp-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo chiave : Codice nostra cassa           *
      *    *-----------------------------------------------------------*
       acc-cod-cbp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Visualizzazione campo chiave : Codice nostra cassa        *
      *    *-----------------------------------------------------------*
       vis-cod-cbp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *                      *-----------------------------------------*
       ...
       acc-tes-reg-110.
      *                      *-----------------------------------------*
      *                      * Codice del sottoconto per la cassa con- *
      *                      * tanti                                   *
      *                      *-----------------------------------------*
           perform   acc-stc-csc-000      thru acc-stc-csc-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
       ...
       acc-tes-reg-120.
      *                      *-----------------------------------------*
      *                      * Codice del sottoconto per oneri su pa-  *
      *                      * gamenti                                 *
      *                      *-----------------------------------------*
           perform   acc-stc-opc-000      thru acc-stc-opc-999        .
           if        v-key                =    "NXSC"
                     move  "+"            to   w-cnt-tus-acc-tes      .
           if        w-cnt-tus-acc-tes    not  = spaces
                     go to acc-tes-reg-999.
      ...
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per la cassa contanti *
      *                  *---------------------------------------------*
           perform   vis-stc-csc-000      thru vis-stc-csc-999        .
           perform   vis-stc-csc-des-000  thru vis-stc-csc-des-999    .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per oneri su pagamen- *
      *                  * ti                                          *
      *                  *---------------------------------------------*
           perform   vis-stc-opc-000      thru vis-stc-opc-999        .
           perform   vis-stc-opc-des-000  thru vis-stc-opc-des-999    .
      *                  *---------------------------------------------*
      *                  * Fine Pagina 1                               *
      *                  *---------------------------------------------*
           go to     vis-tes-reg-999.
      ...
      *                  * Codice del sottoconto per la cassa contanti *
      *                  *---------------------------------------------*
           perform   pmt-stc-csc-000      thru pmt-stc-csc-999        .
      *                  *---------------------------------------------*
      *                  * Codice del sottoconto per oneri su pagamen- *
      *                  * ti                                          *
      *                  *---------------------------------------------*
           perform   pmt-stc-opc-000      thru pmt-stc-opc-999        .
      *                  *---------------------------------------------*
      *                  * Fine Pagina 1                               *
      *                  *---------------------------------------------*
           go to     pmt-tes-reg-999.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompt : Codice del sottoconto per la     *
      *    *                          cassa contanti                   *
      *    *-----------------------------------------------------------*
       pmt-stc-csc-000.
           move      "DS"                 to   v-ope                  .
           ...
           move      10                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Codice del sottoconto con- :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Codice del sottoconto per la cassa   *
      *    *                      contanti                             *
      *    *-----------------------------------------------------------*
       acc-stc-csc-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-stc-csc (1)    to   w-cod-mne-pdc-cod      .
       ...
       acc-stc-csc-110.
           ...
           if        w-cod-mne-pdc-ope    =    "F+"
                     go to acc-stc-csc-115.
           if        w-cod-mne-pdc-ope    =    "AC"
                     go to acc-stc-csc-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-stc-csc-115.
           perform   cod-mne-pdc-foi-000  thru cod-mne-pdc-foi-999    .
           go to     acc-stc-csc-110.
       acc-stc-csc-120.
      ...
      *                  *---------------------------------------------*
           move      w-tes-stc-csc (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-csc-des (1)  .
      *                  *---------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione campo : Codice del sottoconto per la cas- *
      *    *                         sa contanti                       *
      *    *-----------------------------------------------------------*
       vis-stc-csc-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-stc-csc (1)    to   w-edt-cod-pdc-cod      .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Codice del sottoconto per oneri su   *
      *    *                      pagamenti                            *
      *    *-----------------------------------------------------------*
       acc-stc-opc-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-pdc-ope      .
           move      w-prs-liv-pdc        to   w-cod-mne-pdc-liv      .
           move      w-tes-stc-opc (1)    to   w-cod-mne-pdc-cod      .
       ...
       acc-stc-opc-110.
           perform   cod-mne-pdc-cll-000  thru cod-mne-pdc-cll-999    .
           if        w-cod-mne-pdc-ope    =    "F+"
                     go to acc-stc-opc-115.
           if        w-cod-mne-pdc-ope    =    "AC"
                     go to acc-stc-opc-120.
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
       acc-stc-opc-115.
           perform   cod-mne-pdc-foi-000  thru cod-mne-pdc-foi-999    .
           go to     acc-stc-opc-110.
       acc-stc-opc-120.
           move      w-cod-mne-pdc-cod    to   v-num                  .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      ...
      *                  * Lettura archivio piano dei conti            *
      *                  *---------------------------------------------*
           move      w-tes-stc-opc (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-opc-des (1)  .
      *                  *---------------------------------------------*
      *                  * Visualizzazione descrizione sottoconto      *
      *                  *---------------------------------------------*
           perform   vis-stc-opc-des-000  thru vis-stc-opc-des-999    .
      ...
      *              *-------------------------------------------------*
           move      w-prs-liv-pdc        to   w-edt-cod-pdc-liv      .
           move      w-tes-stc-opc (1)    to   w-edt-cod-pdc-cod      .
           move      "B"                  to   w-edt-cod-pdc-edm      .
           perform   edt-pdc-asx-000      thru edt-pdc-asx-999        .
      *              *-------------------------------------------------*
      *              * Visualizzazione                                 *
      ...
      *    *-----------------------------------------------------------*
       nor-nok-tes-000.
           ...
           move      zero                 to   w-tes-stc-csc (1)      .
           move      spaces               to   w-tes-stc-csc-des (1)  .
           move      zero                 to   w-tes-stc-csa (1)      .
           move      spaces               to   w-tes-stc-csa-des (1)  .
           move      zero                 to   w-tes-stc-opc (1)      .
           move      spaces               to   w-tes-stc-opc-des (1)  .
           move      spaces               to   w-tes-alx-001 (1)      .
       nor-nok-tes-999.
           exit.
      ...
      *                          *-------------------------------------*
           ...
           move      rf-cbp-stc-csc       to   w-tes-stc-csc (1)      .
           move      rf-cbp-stc-csa       to   w-tes-stc-csa (1)      .
           move      rf-cbp-stc-opc       to   w-tes-stc-opc (1)      .
           move      rf-cbp-alx-001       to   w-tes-alx-001 (1)      .
       rou-let-reg-300.
      *                          *-------------------------------------*
      ...
      *                              *---------------------------------*
           ...
           move      w-let-arc-pdc-des    to   w-tes-stc-csc-des (1)  .
      *                              *---------------------------------*
      *                              * Descrizione oneri su pagamenti  *
      *                              *---------------------------------*
           move      w-tes-stc-opc (1)    to   w-let-arc-pdc-cod      .
           perform   let-arc-pdc-000      thru let-arc-pdc-999        .
           move      w-let-arc-pdc-des    to   w-tes-stc-opc-des (1)  .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-tes-stc-csc (1)    to   rf-cbp-stc-csc         .
           move      w-tes-stc-csa (1)    to   rf-cbp-stc-csa         .
           move      w-tes-stc-opc (1)    to   rf-cbp-stc-opc         .
           move      w-tes-alx-001 (1)    to   rf-cbp-alx-001         .
       cmp-rec-cbp-999.
           exit.
