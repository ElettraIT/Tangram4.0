      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma gep202   *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
           05  w-buf-rec-ccc-flg          pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Dati record                                           *
      *        *-------------------------------------------------------*
           05  w-buf-rec-ccc-dat.
               10  w-buf-rec-ccc-chr
                             occurs 512   pic  x(01)                  .
      ...
      *                    *-------------------------------------------*
                       20  w-stp-t01-col-004
                                          pic  x(28)                  .
      *            *---------------------------------------------------*
      *            * Comodi per codici tipi sollecito                  *
      *            *---------------------------------------------------*
               10  w-stp-t01-tso.
                  15  w-stp-t01-tso-ed1   pic  x(05)                  .
                  15  w-stp-t01-tso-ed2   pic  x(05)                  .
                  15  w-stp-t01-tso-ed3   pic  x(05)                  .
      *            *---------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-ccc                 .
      *                  *---------------------------------------------*
      *                  * Lettura anagrafica di controllo [ccc]       *
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCLI    "         to   f-key                  .
      ...
      *                  *---------------------------------------------*
           ...
           else if   rr-sel-anc           =    2
                     go to prn-sel-rec-220
           else if   rr-sel-anc           =    3
                     go to prn-sel-rec-230
           else if   rr-sel-anc           =    4
                     go to prn-sel-rec-240
           else if   rr-sel-anc           =    5
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su esito lettura anagrafica di     *
      *                      * controllo                               *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-sel-rec-900.
      *                      *-----------------------------------------*
      *                      * A bufferizzazione record [ccc]          *
      *                      *-----------------------------------------*
           go to     prn-sel-rec-300.
       prn-sel-rec-220.
      *                  *---------------------------------------------*
      *                  * Solo clienti privi di anagrafica di         *
      *                  * controllo                                   *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su esito lettura anagrafica di     *
      ...
      *                      *-----------------------------------------*
       ...
       prn-sel-rec-230.
      *                  *---------------------------------------------*
      *                  * Solo clienti esclusi dai solleciti          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Test su indicatore                      *
      *                      *-----------------------------------------*
           if        rf-ccc-emi-slc       not  = 99
                     go to prn-sel-rec-900.
      *                      *-----------------------------------------*
      *                      * A bufferizzazione record [ccc]          *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       prn-sel-rec-300.
      *                  *---------------------------------------------*
      *                  * Bufferizzazione record [ccc]                *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Flag di record trovato                  *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     move  "#"            to   w-buf-rec-ccc-flg
           else      move  spaces         to   w-buf-rec-ccc-flg      .
      *                      *-----------------------------------------*
      *                      * Dati del record trovato                 *
      *                      *-----------------------------------------*
           move      rf-ccc               to   w-buf-rec-ccc-dat      .
       prn-sel-rec-800.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       prn-liv-det-p01-410.
      *                      *-----------------------------------------*
      *                      * Dati anagrafica di controllo            *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Test se dati letti                  *
      *                          *-------------------------------------*
           if        w-buf-rec-ccc-dat    =    spaces
      ...
      *                          * Recupero dati letti                 *
      *                          *-------------------------------------*
           move      w-buf-rec-ccc-dat    to   rf-ccc                 .
      *                          *-------------------------------------*
      *                          * Fido                                *
      *                          *-------------------------------------*
           if        rf-ccc-max-fid       =    zero or
                     rf-ccc-max-fid       not numeric
      ...
      *                          *-------------------------------------*
           ...
           move      "Fido max :"         to   w-all-str-cat (1)      .
      ...
      *                          * Data fido                           *
      *                          *-------------------------------------*
           if        rf-ccc-dat-fid       =    zero or
                     rf-ccc-dat-fid       not numeric
      ...
      *                          *-------------------------------------*
           if        rf-ccc-emi-slc       =    zero
                     move  "Sempre da sollecitare      "
                                          to   w-all-str-alf
           else if   rf-ccc-emi-slc       =    01
                     move  "Fino al 1. livello e basta "
                                          to   w-all-str-alf
           else if   rf-ccc-emi-slc       =    02
                     move  "Fino al 2. livello e basta "
                                          to   w-all-str-alf
           else      move  "Escluso dai solleciti      "
                                          to   w-all-str-alf          .
      *                          *-------------------------------------*
      *                          * Incremento contatore colonna 004    *
      *                          *-------------------------------------*
      ...
      *                          * Test di sollecito                   *
      *                          *-------------------------------------*
           if        rf-ccc-emi-slc       =    99
                     go to prn-liv-det-p01-490.
      *                          *-------------------------------------*
      *                          * Editing codice 1.                   *
      *                          *-------------------------------------*
           if        rf-ccc-tdt-slc       =    zero
                     move  "Standard, a livelli        "
                                          to   w-all-str-alf
           else if   rf-ccc-tdt-slc       =    01
                     move  "Standard, ma max livello 1."
                                          to   w-all-str-alf
           else if   rf-ccc-tdt-slc       =    02
                     move  "Standard, ma max livello 2."
                                          to   w-all-str-alf
           else if   rf-ccc-tdt-slc       =    03
                     move  "Standard, ma max livello 3."
                                          to   w-all-str-alf
           else      move  "Testo personalizzato       "
                                          to   w-all-str-alf          .
      *                          *-------------------------------------*
      *                          * Incremento contatore colonna 004    *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
       ...
       prn-liv-det-p01-450.
      *                          *-------------------------------------*
      *                          * Tipi sollecito personalizzati       *
      *                          *-------------------------------------*
           if        rf-ccc-tdt-slc       not  = 99
                     go to prn-liv-det-p01-460.
      *                          *-------------------------------------*
      *                          * Editing codice 1.                   *
      *                          *-------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      rf-ccc-tso-l01       to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-stp-t01-tso-ed1      .
      *                          *-------------------------------------*
      *                          * Editing codice 2.                   *
      *                          *-------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      rf-ccc-tso-l02       to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-stp-t01-tso-ed2      .
      *                          *-------------------------------------*
      *                          * Editing codice 3.                   *
      *                          *-------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           move      spaces               to   p-sgn                  .
           move      "<B"                 to   p-edm                  .
           move      rf-ccc-tso-l03       to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-stp-t01-tso-ed3      .
      *                          *-------------------------------------*
      *                          * Assemblaggio                        *
      *                          *-------------------------------------*
           move      27                   to   w-all-str-lun          .
           ...
           move      w-stp-t01-tso-ed1    to   w-all-str-cat (2)      .
           move      "2:"                 to   w-all-str-cat (3)      .
           move      w-stp-t01-tso-ed2    to   w-all-str-cat (4)      .
           move      "3:"                 to   w-all-str-cat (5)      .
           move      w-stp-t01-tso-ed3    to   w-all-str-cat (6)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                          *-------------------------------------*
      *                          * Incremento contatore colonna 004    *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rf-ccc-snx-acr       =    "N"
                     move  "Assicur. : RIFIUTATA       "
                                          to   w-stp-t01-col-004
                                              (w-stp-t01-cco-004)
           else      move  "Assicur. : accordata       "
                                          to   w-stp-t01-col-004
                                              (w-stp-t01-cco-004)     .
      *                          *-------------------------------------*
      *                          * Editing importo assicurazione       *
      *                          *-------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           move      13                   to   p-car                  .
      ...
      *                          *-------------------------------------*
           move      27                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      "Importo  :"         to   w-all-str-cat (1)      .
           move      p-edt                to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
      *                          *-------------------------------------*
      *                          * Incremento contatore colonna 004    *
      ...
      *                          *-------------------------------------*
           move      w-all-str-alf        to   w-stp-t01-col-004
                                              (w-stp-t01-cco-004)     .
      *                          *-------------------------------------*
      *                          * Editing data aggiornamento          *
      *                          *-------------------------------------*
           move      "ED"                 to   p-ope                  .
           move      "D"                  to   p-tip                  .
           move      rf-ccc-dat-acr       to   p-dat                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                          *-------------------------------------*
      *                          * Assemblaggio                        *
      ...
      *              *-------------------------------------------------*
           if        rr-sel-anc           =    1
                     move  "ELENCO CLIENTI CON ANAGRAFICA DI CONTROLLO"
                                          to   w-cnt-tit-des-tit
           else if   rr-sel-anc           =    2
                     move  "ELENCO CLIENTI SENZA ANAGRAFICA CONTROLLO"
                                          to   w-cnt-tit-des-tit
           else if   rr-sel-anc           =    3
                     move  "ELENCO CLIENTI ESCLUSI DAI SOLLECITI"
                                          to   w-cnt-tit-des-tit
           else if   rr-sel-anc           =    4
                     move  "ELENCO CLIENTI CON ASSICURAZIONE CREDITO"
                                          to   w-cnt-tit-des-tit
           else      move  "ELENCO ANAGRAFICHE DI CONTROLLO CLIENTI"
                                          to   w-cnt-tit-des-tit      .
      *
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
