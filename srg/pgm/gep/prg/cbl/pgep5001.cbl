      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma gep500   *
      *                                                                *
      ...
      *    *-----------------------------------------------------------*
       01  w-prs.
      *        *-------------------------------------------------------*
      *        * Modalita' di trattamento scadenze a vista             *
      *        *-------------------------------------------------------*
           05  w-prs-scd-vis              pic  x(01)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Work per Det se cliente con accrediti aperti          *
      *        *-------------------------------------------------------*
           05  w-det-cli-acc.
      *            *---------------------------------------------------*
      *            * Codice cliente                                    *
      *            *---------------------------------------------------*
               10  w-det-cli-acc-cli      pic  9(07)                  .
      *            *---------------------------------------------------*
      *            * Codice dipendenza del cliente                     *
      *            *                                                   *
      *            * N.B.: attualmente non gestita                     *
      *            *---------------------------------------------------*
               10  w-det-cli-acc-dpz      pic  x(04)                  .
      *            *---------------------------------------------------*
      *            * Contatore elemeti                                 *
      *            *---------------------------------------------------*
               10  w-det-cli-acc-ctr      pic  9(11)                  .
      *        *-------------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Work per subroutine stp-srt-inp-000/999                   *
      *    *-----------------------------------------------------------*
       01  w-stp-srt-inp.
           05  w-stp-srt-inp-dsc          pic  9(07)                  .
      ...
      *                  * Modalita' di trattamento scadenze a vista   *
      *                  *---------------------------------------------*
           perform   prs-scd-vis-000      thru prs-scd-vis-999        .
       pre-exe-pgm-999.
           exit.

      *    *===========================================================*
      *    * Lettura delle personalizzazioni relative alla modalita'   *
      *    * di trattamento scadenze a vista                           *
      *    *-----------------------------------------------------------*
       prs-scd-vis-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "P:"                 to   s-ope                  .
           move      "pgm/gep[scd-vis]"   to   s-alf                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                =    spaces
                     move  s-alf          to   w-prs-scd-vis
           else      move  spaces         to   w-prs-scd-vis          .
      *              *-------------------------------------------------*
      *              * Normalizzazione personalizzazione               *
      *              *-------------------------------------------------*
           if        w-prs-scd-vis        not  = "R" and
                     w-prs-scd-vis        not  = "E"
                     move  "N"            to   w-prs-scd-vis          .
       prs-scd-vis-999.
           exit.
      ...
      *                      *-----------------------------------------*
           if        rr-snx-scv           not  = "S"
                     go to stp-srt-inp-120.
      *                      *-----------------------------------------*
      *                      * Deviazione in funzione delle persona-   *
      *                      * lizzazioni                              *
      *                      *-----------------------------------------*
           if        w-prs-scd-vis        =    "N"
                     go to stp-srt-inp-150
           else if   w-prs-scd-vis        =    "R"
                     move  rf-sdb-dat-ddr to   w-stp-srt-inp-dsc
           else if   w-prs-scd-vis        =    "E"
                     move  rf-sdb-dtr-emi to   w-stp-srt-inp-dsc
           else      move  rf-sdb-dtr-emi to   w-stp-srt-inp-dsc      .
      *                      *-----------------------------------------*
      *                      * Test                                    *
      *                      *-----------------------------------------*
           if        w-stp-srt-inp-dsc    <    rr-dat-sci or
                     w-stp-srt-inp-dsc    >    rr-dat-scf
                     go to stp-srt-inp-120.
           go to     stp-srt-inp-150.
       stp-srt-inp-144.
      *                      *-----------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-180.
      *                  *---------------------------------------------*
      *                  * Selezioni su clienti con scadenze da rag-   *
      *                  * gruppare                                    *
      *                  *---------------------------------------------*
           if        rr-snx-sdr           =   "N"
                     go to stp-srt-inp-200.
           if        rr-snx-sdr           =   "C" and
                     rf-dcc-ipr-iel       not  = 02
                     go to stp-srt-inp-120.
           if        rr-snx-sdr           =   "S" and
                     rf-dcc-ipr-iel       =   02
                     go to stp-srt-inp-120.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Memorizzazione nuovo periodo mensile se scaden- *
      *              * za scaduta in tabella di lavoro                 *
      *              *-------------------------------------------------*
           if        w-rot-l01-sts-scd    not  = 01
                     go to prn-ini-lr1-050.
      *                  *---------------------------------------------*
      *                  * Incremento indice per tabella totali mensi- *
      *                  * li                                          *
      *                  *---------------------------------------------*
           if        w-tot-pms-wix        not  = 400
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Determinazione indice su tabella            *
      *                  *---------------------------------------------*
           if        srt-tip-sdb          =    01 and
                     srt-snx-dlc          =    "N"
                     move  01             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    01 and
                     srt-snx-dlc          =    "S"
                     move  02             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    02
                     move  03             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    03
                     move  04             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    04
                     move  05             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    05
                     move  06             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    06
                     move  07             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    07
                     move  08             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    08
                     move  09             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    09
                     move  10             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    10
                     move  11             to   w-tot-pge-i01
           else if   srt-tip-sdb          =    11
                     move  12             to   w-tot-pge-i01          .
      *                  *---------------------------------------------*
      *                  * Per prospetto generale                      *
      *                  *---------------------------------------------*
           if        w-rot-l01-sts-scd    =    01
      ...
      *                      *-----------------------------------------*
       ...
       prn-liv-det-140.
      *                      *-----------------------------------------*
      *                      * Eventuale segnale di accredito          *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Test se da stampare                 *
      *                          *-------------------------------------*
      ...
      *                          * Determinazione                      *
      *                          *-------------------------------------*
           move      srt-cod-cli          to   w-det-cli-acc-cli      .
           move      spaces               to   w-det-cli-acc-dpz      .
           perform   det-cli-acc-000      thru det-cli-acc-999        .
      *                          *-------------------------------------*
      *                          * Test su contatore                   *
      *                          *-------------------------------------*
           if        w-det-cli-acc-ctr    not  > zero
                     go to prn-liv-det-200.
      *                          *-------------------------------------*
      *                          * Stampa                              *
      *                          *-------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           string    "SCADENZE IN PORTAFOGLIO AL "
                                delimited by   size
                     p-edt      delimited by   spaces
      ...
      *                  *---------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "  Numero     Tipo   Data      Importo   Codice  Di
      -              "p.              Ragione sociale             Tp   N
      -              "umero     Data      Importo   Rt"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Determinazione se cliente con accrediti aperti            *
      *    *-----------------------------------------------------------*
       det-cli-acc-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore                       *
      *              *-------------------------------------------------*
           move      zero                 to   w-det-cli-acc-ctr      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      w-det-cli-acc-cli    to   rf-sdb-cod-dbt         .
      ...
      *                  *---------------------------------------------*
           if        rf-sdb-cod-dbt       not  = w-det-cli-acc-cli
                     go to det-cli-acc-900.
       det-cli-acc-400.
      *              *-------------------------------------------------*
      *              * Selezione su codice dipendenza del cliente      *
      *              *-------------------------------------------------*
           if        w-det-cli-acc-dpz    = spaces
                     go to det-cli-acc-420.
           if        w-det-cli-acc-dpz    not  = "*   " and
                     rf-sdb-dpz-dbt       not  = w-det-cli-acc-dpz
                     go to det-cli-acc-200.
       det-cli-acc-420.
      *              *-------------------------------------------------*
      *              * Determinazione status scadenza                  *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       det-cli-acc-440.
      *              *-------------------------------------------------*
      *              * Selezione su documento riferimento scadenza     *
      *              *-------------------------------------------------*
      ...
      *                   *--------------------------------------------*
           add       1                    to   w-det-cli-acc-ctr      .
       det-cli-acc-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     det-cli-acc-999.
       det-cli-acc-999.
           exit.
