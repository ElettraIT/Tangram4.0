      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogaz. anagrafica nostri c/c postali   *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  INTERROGAZIONE SU NOSTRI C/C POSTALI  "       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           05  rr-tip-cbp                 pic  9(02)       value 03   .
           05  rr-tip-ord                 pic  x(01)                  .
           05  rr-cod-min                 pic  x(10)                  .
           05  rr-cod-max                 pic  x(10)                  .
      ...
      *                  *---------------------------------------------*
       ...
       pre-tip-fun-400.
      *              *-------------------------------------------------*
      *              * Se variabile di i.p.c. per 'find' speciale      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * No richieste ad utente                      *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su nostri c/c postali :            "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-tip-cbp           to   w-cod-des-cbp-tip      .
           move      rr-cod-min           to   w-cod-des-cbp-cod      .
           move      12                   to   w-cod-des-cbp-lin      .
           move      30                   to   w-cod-des-cbp-pos      .
           move      zero                 to   w-cod-des-cbp-dln      .
           move      zero                 to   w-cod-des-cbp-dps      .
           move      spaces               to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-des-cbp-ope      .
           move      rr-tip-cbp           to   w-cod-des-cbp-tip      .
           move      rr-cod-max           to   w-cod-des-cbp-cod      .
           move      12                   to   w-cod-des-cbp-lin      .
           move      52                   to   w-cod-des-cbp-pos      .
           move      zero                 to   w-cod-des-cbp-dln      .
           move      zero                 to   w-cod-des-cbp-dps      .
           move      spaces               to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CODCTP    "         to   f-key                  .
           move      rr-tip-cbp           to   rf-cbp-tip-cbp         .
           move      rr-cod-min           to   rf-cbp-cod-cbp         .
           move      "pgm/gep/fls/ioc/obj/iofcbp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-tip-cbp           to   rf-cbp-tip-cbp         .
           move      rr-des-min           to   rf-cbp-des-key         .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessun c/c postale entro i limiti assegnati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Lettura sequenziale archivio [cbp]              *
      *              *-------------------------------------------------*
           move      "RN"                 to   f-ope                  .
           move      "pgm/gep/fls/ioc/obj/iofcbp"
                                          to   s-pat                  .
      ...
      *              * Test su tipo record                             *
      *              *-------------------------------------------------*
           if        rf-cbp-tip-cbp       not  = rr-tip-cbp
                     move  "#"            to   w-cnt-qry-flg-sub
      ...
      *              * Selezione su codice                             *
      *              *-------------------------------------------------*
           if        rf-cbp-cod-cbp       <    rr-cod-min or
                     rf-cbp-cod-cbp       >    rr-cod-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
      *              *-------------------------------------------------*
      *              * Selezione su descrizione                        *
      *              *-------------------------------------------------*
           if        rf-cbp-des-key       <    rr-des-min or
                     rf-cbp-des-key       >    rr-des-max
                     move  "#"            to   w-cnt-qry-flg-sub
                     go to qry-sel-rec-999.
       qry-sel-rec-999.
           exit.
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      27                   to   v-pos                  .
           move      rf-cbp-des-cbp       to   v-alf                  .
           move      "+"                  to   v-edm                  .
           move      rf-cbp-cod-cbp       to   w-mpn-cod-cbp          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-liv-det-999.
      ...
      *              *-------------------------------------------------*
           ...
           move      spaces               to   v-key                  .
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'cod-cbp'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           if        s-alf                =    "D"
                     ...
                     move  s-alf          to   w-ipc-dlp-tdi-tip
                     go to ipc-dlp-tdi-999
           else      go to ipc-dlp-tdi-999.
       ipc-dlp-tdi-300.
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "des-key" per la descrizione                    *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
