      ...
      *                                   Fase:    gep500              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 24/04/92    *
      *                       Ultima revisione:    NdK del 07/02/23    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pgep5001:        *
      *                                                                *
      *                    Lista scadenze in portafoglio per data      *
      *                                                                *
      *                    ------------------------------------------- *
      *                                                                *
      *                    File di esportazione :                      *
      *                                                                *
      *                    /abd/asc/UTENTE/exp/AZI_gep500.csv          *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
      *        * Si/no evidenza accrediti                              *
      *        *                                                       *
      *        * - 'N' : No                                            *
      *        * - 'S' : Si                                            *
      *        *-------------------------------------------------------*
           05  rr-snx-iac                 pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Si/no clienti con scadenze da raggruppare             *
      *        *                                                       *
      *        * - 'N' : Nessuna selezione                             *
      *        * - 'C' : solo clienti Con raggruppamento               *
      *        * - 'S' : solo clienti Senza raggruppamento             *
      *        *-------------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-prs.
      *        *-------------------------------------------------------*
      *        * Personalizzazioni per riemissione nuova scadenza      *
      *        *-------------------------------------------------------*
           05  w-prs-rie.
      *            *---------------------------------------------------*
      *            * Modalita' di riemissione                          *
      *            *                                                   *
      *            * - 00 : Tutte le richieste esplicite all'operatore,*
      *            *        e di cinseguenza tutte le possibilita'     *
      *            * - 01 : Tutte le richieste esplicite all'operatore,*
      *            *        se pero' non viene richiesta la riemissio- *
      *            *        ne di una scadenza si genera automatica-   *
      *            *        mente una scadenza di tipo rimessa diretta *
      *            *        a vista                                    *
      *            * - 02 : Si forza a Si' il parametro di si/no rie-  *
      *            *        missione, ma si chiede di volta in volta   *
      *            *        il tipo di scadenza da emettere            *
      *            * - 03 : Si forza a Si' il parametro di si/no rie-  *
      *            *        missione, e si forza anche il tipo di sca- *
      *            *        denza da emettere                          *
      *            * - 04 : Si forza a Si' il parametro di si/no rie-  *
      *            *        missione, e si forza anche il tipo di sca- *
      *            *        denza da emettere, inoltre si forza anche  *
      *            *        la data di scadenza                        *
      *            * - 99 : Non viene gestita la riemissione delle     *
      *            *        scadenze                                   *
      *            *                                                   *
      *            *---------------------------------------------------*
               10  w-prs-rie-mod-rie      pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Tipo di scadenza forzato per la riemissione       *
      *            *                                                   *
      *            * - 01 : Rimessa diretta                            *
      ...
      *            *                                                   *
      *            *---------------------------------------------------*
               10  w-prs-rie-tip-sca      pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Indicazione sulla data scadenza forzata per la    *
      *            * riemissione                                       *
      *            *                                                   *
      *            * - AV : A vista                                    *
      *            * - SD : Stessa data scadenza della scadenza che si *
      *            *        sostituisce                                *
      *            * - FM : Alla fine del mese rispetto alla data di   *
      *            *        scadenza che si sostituisce                *
      *            * - +1 : Stessa data scadenza della scadenza che si *
      *            *        sostituisce, ma aumentata di un mese       *
      *            *                                                   *
      *            *---------------------------------------------------*
               10  w-prs-rie-ind-sds      pic  x(02)                  .
      *            *---------------------------------------------------*
      *            * Tipo di ripercussione su anagrafica commerciale   *
      *            * del cliente                                       *
      *            *                                                   *
      *            * - 00 : Nessuna ripercussione                      *
      *            * - 01 : Attivazione blocco ordini                  *
      *            * - 02 : Attivazione blocco consegne                *
      *            * - 03 : Attivazione blocco ordini e consegne       *
      *            *                                                   *
      *            *---------------------------------------------------*
               10  w-prs-rie-snx-blc      pic  9(02)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su blocco clienti                *
      *        *-------------------------------------------------------*
           05  w-exp-sel-blc.
               10  w-exp-sel-blc-num      pic  9(02)       value 04   .
               10  w-exp-sel-blc-lun      pic  9(02)       value 30   .
               10  w-exp-sel-blc-tbl.
                   ...
                   15  filler             pic  x(30) value
                            "solo con Entrambi i blocchi   "          .
      *        *-------------------------------------------------------*
      *        * Work per : Si/No evidenza accrediti                   *
      *        *-------------------------------------------------------*
           05  w-exp-snx-iac.
               10  w-exp-snx-iac-num      pic  9(02)       value 02   .
               10  w-exp-snx-iac-lun      pic  9(02)       value 02   .
               10  w-exp-snx-iac-tbl.
                   15  filler             pic  x(02) value "No"       .
                   15  filler             pic  x(02) value "Si"       .
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-exp-snx-sdr.
               10  w-exp-snx-sdr-num      pic  9(02)       value 03   .
               10  w-exp-snx-sdr-lun      pic  9(02)       value 39   .
               10  w-exp-snx-sdr-tbl.
                   15  filler             pic  x(39) value
                            "Nessuna selezione                      " .
                   15  filler             pic  x(39) value
                            "solo clienti Con raggruppamento        " .
                   15  filler             pic  x(39) value
                            "solo clienti Senza raggruppamento      " .
      *        *-------------------------------------------------------*
      ...
      *                  * Selezione su blocco clienti                 *
      *                  *---------------------------------------------*
           perform   acc-sel-blc-000      thru acc-sel-blc-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Si/no evidenza accrediti                    *
      *                  *---------------------------------------------*
           perform   acc-snx-iac-000      thru acc-snx-iac-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Selezione su blocco clienti                     *
      *              *-------------------------------------------------*
           perform   pmt-sel-blc-000      thru pmt-sel-blc-999        .
      *              *-------------------------------------------------*
      *              * Si/no evidenza accrediti                        *
      *              *-------------------------------------------------*
           perform   pmt-snx-iac-000      thru pmt-snx-iac-999        .
      *              *-------------------------------------------------*
      *              * Si/no clienti con scadenze da raggruppare       *
      *              *-------------------------------------------------*
           perform   pmt-snx-sdr-000      thru pmt-snx-sdr-999        .
      *              *-------------------------------------------------*
      *              * Si/No esportazione archivio                     *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Selezione su blocco clienti   *
      *    * za                                                        *
      *    *-----------------------------------------------------------*
       pmt-sel-blc-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        w-prs-rie-snx-blc    =    zero
                     go to pmt-sel-blc-999.
       pmt-sel-blc-100.
      *              *-------------------------------------------------*
      *              * Visualizzazione                                 *
      *              *-------------------------------------------------*
           ...
           move      14                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su blocco clienti:"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-sel-blc-999.
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Si/no evidenza accrediti      *
      *    *-----------------------------------------------------------*
       pmt-snx-iac-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Si/no clienti con scadenze da *
      *    * raggruppare                                               *
      *    *-----------------------------------------------------------*
       pmt-snx-sdr-000.
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Visualizzazione prompts per Si/No esportazione archivio   *
      *    *-----------------------------------------------------------*
       pmt-snx-exp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Selezione su blocco clienti          *
      *    *-----------------------------------------------------------*
       acc-sel-blc-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-blc-lun    to   v-car                  .
           move      w-exp-sel-blc-num    to   v-ldt                  .
           move      "NOCE#"              to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-sel-blc-tbl    to   v-txt                  .
           move      14                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-sel-blc-100.
       acc-sel-blc-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Si/No evidenza accrediti   *
      *    *-----------------------------------------------------------*
       acc-snx-iac-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-iac-lun    to   v-car                  .
           move      w-exp-snx-iac-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-iac-tbl    to   v-txt                  .
           move      16                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-snx-iac-999.
       acc-snx-iac-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  "N"            to   rr-snx-iac
           else if   v-num                =    02
                     move  "S"            to   rr-snx-iac
           else      move  spaces         to   rr-snx-iac             .
       acc-snx-iac-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/No esportazione archivio          *
      *    *-----------------------------------------------------------*
       acc-snx-exp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      zero                 to   rr-sel-blc             .
           move      spaces               to   rr-snx-iac             .
