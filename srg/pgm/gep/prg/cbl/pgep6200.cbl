      ...
      *                       Ultima revisione:    NdK del 08/09/23    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pgep6201:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su compensazioni                 *
      *        *-------------------------------------------------------*
           05  w-exp-sel-cmp.
               10  w-exp-sel-cmp-num      pic  9(02)       value 03   .
               10  w-exp-sel-cmp-lun      pic  9(02)       value 40   .
               10  w-exp-sel-cmp-tbl.
                   15  filler             pic  x(40) value
                            "Nessuna selezione                       ".
                   15  filler             pic  x(40) value
                            "solo clienti con Accrediti da compensare".
                   15  filler             pic  x(40) value
                            "solo clienti con Scadenze da compensare ".
      *        *-------------------------------------------------------*
      ...
      *                  * Selezione su compensazioni                  *
      *                  *---------------------------------------------*
           perform   acc-sel-cmp-000      thru acc-sel-cmp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Selezione su compensazioni                      *
      *              *-------------------------------------------------*
           perform   pmt-sel-cmp-000      thru pmt-sel-cmp-999        .
      *              *-------------------------------------------------*
      *              * Tipo salto pagina da effettuare                 *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt per Selezione su compensazioni                     *
      *    *-----------------------------------------------------------*
       pmt-sel-cmp-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      18                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su compensazione :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       pmt-sel-cmp-999.
      ...
      *                  *---------------------------------------------*
           if        rr-cod-cli           =    zero
                     ...
                     move  spaces         to   rr-cod-cli-loc
                                          ...
                                          to   rr-cod-cli-rag
                     move  w-let-arc-cli-via
                                          to   rr-cod-cli-via
                     move  w-let-arc-cli-loc
                                          to   rr-cod-cli-loc         .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Trattamento selezione su compensazioni      *
      *                  *---------------------------------------------*
           if        rr-tip-scd           =    01
                     go to acc-tip-scd-800.
           if        rr-sel-cmp           =    01
                     go to acc-tip-scd-800.
           move      01                   to   rr-sel-cmp             .
           perform   vis-sel-cmp-000      thru vis-sel-cmp-999        .
       acc-tip-scd-800.
      *              *-------------------------------------------------*
      *              * Se Do                                           *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Selezione su compensazioni           *
      *    *-----------------------------------------------------------*
       acc-sel-cmp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *                  * Test se campo da accettare                  *
      *                  *---------------------------------------------*
           if        rr-tip-scd           not  = 01
                     go to acc-sel-cmp-999.
           if        rr-sts-eff           not  = 01 and
                     rr-sts-eff           not  = 02
                     go to acc-sel-cmp-999.
       acc-sel-cmp-100.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-cmp-lun    to   v-car                  .
           move      w-exp-sel-cmp-num    to   v-ldt                  .
           move      "NAS#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-sel-cmp-tbl    to   v-txt                  .
           move      18                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sel-cmp           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *                  * che non si sia in Up                        *
      *                  *---------------------------------------------*
           if        rr-sel-cmp           not  = zero
                     go to acc-sel-cmp-600.
           if        v-key                =    "UP  "
                     go to acc-sel-cmp-600
           else      go to acc-sel-cmp-100.
       acc-sel-cmp-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-sel-cmp-100.
       acc-sel-cmp-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Selezione su compensazioni        *
      *    *-----------------------------------------------------------*
       vis-sel-cmp-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
