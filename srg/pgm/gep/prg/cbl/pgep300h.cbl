      ...
      *            *---------------------------------------------------*
      *            * Partita Iva o Codice Fiscale del debitore         *
      *            *---------------------------------------------------*
               10  w-sta-dis-trs-piv      pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Codice del debitore                               *
      *            *---------------------------------------------------*
      ...
      *                      * sentata : uscita con errore             *
      *                      *-----------------------------------------*
           if        w-tes-dtr-pre (1)    not  =    zero
                     go to rou-let-reg-200.
           move      "Attenzione : La distinta non e' mai stata presenta
      -              "ta.            "    to   w-err-box-err-msg      .
           move      "             Occorre pertanto utilizzare l'operazi
      -              "one            "    to   w-err-box-err-m02      .
           move      "             di stampa distinta per controllo.    
      ...
      *                      *-----------------------------------------*
      *                      * Se la data di registrazione e' inferio- *
      *                      * re alla data di presentazione distinta  *
      *                      * si pone la sola Visualizzazione, dopo   *
      *                      * aver emesso un messaggio di errore.     *
      *                      *                                         *
      *                      * N.B. : Il tipo funzionamento in questo  *
      ...
      *                      *        zionale.                         *
      *                      *-----------------------------------------*
           if        w-tes-dat-reg        not  < w-tes-dtr-pre (1)
      ...
      -              "enta la data   "    to   w-err-box-err-msg      .
           move      "             di stampa, non puo' essere inferiore 
      -              "alla data di   "    to   w-err-box-err-m02      .
           move      "             presentazione della distinta. - Stamp
      -              "a non ammessa. "    to   w-err-box-err-m03      .
           perform   box-msg-e03-000      thru box-msg-e03-999        .
           move      "V"                  to   w-cnt-mfu-tip-fun      .
       rou-let-reg-250.
      *                      *-----------------------------------------*
      ...
      *                  * ti relativi alla stampa distinta            *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Flag/contatore di stampa su carta ese-  *
      *                      * guita per la distinta                   *
      *                      *-----------------------------------------*
           if        rf-ddp-flc-stp       <    99
                     add    1             to   rf-ddp-flc-stp         .
       pos-cnf-ins-400.
      *                  *---------------------------------------------*
      *                  * Update record [ddp]                         *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Se la stampa e' stata effettivamente ese-   *
      *                  * guita : uscita                              *
      *                  *---------------------------------------------*
           if        w-stp-frg-inc-flg    =    spaces
                     go to pos-cnf-ins-999.
       pos-cnf-ins-700.
      *                  *---------------------------------------------*
      *                  * Se la stampa non e' stata effettivamente e- *
      *                  * seguita                                     *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Ri-lettura record [ddp] con lock        *
      *                      *-----------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "NUMDDP    "         to   f-key                  .
           move      w-tes-num-ddp (1)    to   rf-ddp-num-ddp         .
           ...
           call      s-pat               using f
                                               rf-ddp                 .
      *                      *-----------------------------------------*
      *                      * Se errori in lettura : uscita           *
      *                      *-----------------------------------------*
           if        f-sts                not  = e-not-err
                     go to pos-cnf-ins-999.
      *                      *-----------------------------------------*
      *                      * Contro-aggiornamento del numero stampe  *
      *                      * eseguite per la distinta                *
      *                      *-----------------------------------------*
           if        rf-ddp-flc-stp       >    zero
                     subtract   1         from rf-ddp-flc-stp         .
      *                      *-----------------------------------------*
      *                      * Update record [ddp]                     *
      *                      *-----------------------------------------*
           move      "UP"                 to   f-ope                  .
      ...
      *                      *-----------------------------------------*
           ...
           move      spaces               to   w-let-arc-dbt-piv      .
      *                      *-----------------------------------------*
      *                      * Uscita                                  *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Partita Iva del debitore da anagrafica  *
      *                      * contabile : in sua assenza, Codice fis- *
      *                      * cale                                    *
      *                      *-----------------------------------------*
           if        w-let-arc-cli-piv    =    zero
                     move  w-let-arc-cli-cfi
                                          to   w-let-arc-dbt-piv
                     go to let-arc-dbt-290.
      *                      *-----------------------------------------*
      *                      * Editing partita Iva                     *
      *                      *-----------------------------------------*
           move      "ED"                 to   v-ope                  .
           ...
           move      spaces               to   v-sgn                  .
           move      "9"                  to   v-edm                  .
           move      w-let-arc-cli-piv    to   v-num                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                      *-----------------------------------------*
      *                      * In campo di destinazione                *
      *                      *-----------------------------------------*
           move      v-edt                to   w-let-arc-dbt-piv      .
       let-arc-dbt-290.
      *                  *---------------------------------------------*
      *                  * Uscita                                      *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           perform   int-pag-sdp-000      thru int-pag-sdp-999        .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Deviazione a seconda del tipo distinta  *
      *                      *-----------------------------------------*
           if        w-tes-tip-ddp (1)    =    01
                     go to int-pag-sdp-121
           else if   w-tes-tip-ddp (1)    =    02
                     go to int-pag-sdp-122
           else if   w-tes-tip-ddp (1)    =    03
                     go to int-pag-sdp-123
           else if   w-tes-tip-ddp (1)    =    04
                     go to int-pag-sdp-124
           else      go to int-pag-sdp-125.
       int-pag-sdp-121.
      *                      *-----------------------------------------*
      *                      * Se tipo distinta 01                     *
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      051                  to   p-pos                  .
           move      " DISTINTA INCASSI ELETTRONICI "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           go to     int-pag-sdp-150.
       int-pag-sdp-122.
      *                      *-----------------------------------------*
      *                      * Se tipo distinta 02                     *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      051                  to   p-pos                  .
           move      "  DISTINTA PAGHERO' CAMBIARI  "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           go to     int-pag-sdp-150.
      ...
      *                  *---------------------------------------------*
           move      w-tes-cod-cbp-abi (1)
                                          to   w-let-arc-axi-cod      .
      ...
      *                  *---------------------------------------------*
           move      w-tes-cod-cbp-abi (1)
                                          to   w-let-arc-axs-abi      .
           move      w-tes-cod-cbp-cab (1)
                                          to   w-let-arc-axs-cab      .
      ...
      *                  *---------------------------------------------*
           if        w-tes-tip-ddp (1)    =    01
                     go to int-pag-sdp-610
           else      go to int-pag-sdp-620.
       int-pag-sdp-610.
      *                  *---------------------------------------------*
      *                  * Se tipo distinta 01 : Incassi elettronici   *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Deviazione a seconda del tipo di pre-   *
      *                      * sentazione                              *
      *                      *-----------------------------------------*
           if        w-tes-tip-pre (1)    =    01
                     go to int-pag-sdp-611
           else if   w-tes-tip-pre (1)    =    02
                     go to int-pag-sdp-612
           else if   w-tes-tip-pre (1)    =    03
                     go to int-pag-sdp-613
           else if   w-tes-tip-pre (1)    =    04
                     go to int-pag-sdp-614
           else      go to int-pag-sdp-615.
       int-pag-sdp-611.
      *                      *-----------------------------------------*
      *                      * Se tipo di presentazione 01             *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Literal                             *
      *                          *-------------------------------------*
      ...
      -              "ire l'accredito S.B.F. a maturazione di valuta per
      -              " le disposizioni sottoelencate. "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                          *-------------------------------------*
      *                          * A tipo avviso richiesto             *
      *                          *-------------------------------------*
           go to     int-pag-sdp-700.
       int-pag-sdp-612.
      *                      *-----------------------------------------*
      *                      * Se tipo di presentazione 02             *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Literal                             *
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "Si richiede a codesto Istituto di Credito di esegu
      -              "ire l'accredito immediato S.B.F. per le disposizio
      -              "ni sottoelencate.               "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "Si richiede a codesto Istituto di Credito di esegu
      -              "ire l'accredito al dopo incasso per le disposizion
      -              "i sottoelencate.                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "Presentiamo per lo sconto a codesto Istituto di Cr
      -              "edito le disposizioni sottoelencate.              
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
       ...
       int-pag-sdp-615.
      *                      *-----------------------------------------*
      *                      * Se tipo di presentazione non ricono-    *
      *                      * sciuto                                  *
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Literal                             *
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "Presentiamo a codesto Istituto di Credito le dispo
      -              "sizioni sottoelencate.                            
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                      *-----------------------------------------*
           if        w-tes-tip-pre (1)    =    01
                     go to int-pag-sdp-621
           else if   w-tes-tip-pre (1)    =    02
                     go to int-pag-sdp-622
           else if   w-tes-tip-pre (1)    =    03
                     go to int-pag-sdp-623
           else if   w-tes-tip-pre (1)    =    04
                     go to int-pag-sdp-624
           else      go to int-pag-sdp-625.
       int-pag-sdp-621.
      *                      *-----------------------------------------*
      *                      * Se tipo di presentazione 01             *
      *                      *-----------------------------------------*
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "Si richiede a codesto Istituto di Credito di esegu
      -              "ire l'accredito immediato S.B.F. per gli effetti s
      -              "ottoelencati.                   "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "Si richiede a codesto Istituto di Credito di esegu
      -              "ire l'accredito al dopo incasso per gli effetti so
      -              "ttoelencati.                    "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "Presentiamo per lo sconto a codesto Istituto di Cr
      -              "edito gli effetti sottoelencati.                  
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "Presentiamo a codesto Istituto di Credito gli effe
      -              "tti sottoelencati.                                
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Deviazione a seconda del tipo di avviso ri- *
      *                  * chiesto                                     *
      *                  *---------------------------------------------*
           if        w-tes-tav-ric (1)    =    01
                     go to int-pag-sdp-710
           else if   w-tes-tav-ric (1)    =    02
                     go to int-pag-sdp-720
           else if   w-tes-tav-ric (1)    =    03
                     go to int-pag-sdp-730
           else if   w-tes-tav-ric (1)    =    04
                     go to int-pag-sdp-740
           else if   w-tes-tav-ric (1)    =    05
                     go to int-pag-sdp-750
           else      go to int-pag-sdp-760.
       int-pag-sdp-710.
      *                  *---------------------------------------------*
      *                  * Se tipo di avviso richiesto 01              *
      *                  *---------------------------------------------*
       int-pag-sdp-711.
      *                      *-----------------------------------------*
      *                      * Literal-1                               *
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "A fronte di tali disposizioni vorrete emettere per
      -              " nostro conto delle richieste di pagamento pagabil
      -              "i presso gli Istituti di Credito"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sdp-712.
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "         aderenti ai circuiti interbancari cui sie
      -              "te associati quando dette disposizioni siano rispo
      -              "ndenti ai requisiti previsti dai"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sdp-713.
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "         circuiti in questione, e/o ricevute banca
      -              "rie pagabili presso altri Istituti di Credito in c
      -              "aso contrario.                  "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                      *-----------------------------------------*
       ...
       int-pag-sdp-720.
      *                  *---------------------------------------------*
      *                  * Se tipo di avviso richiesto 02              *
      *                  *---------------------------------------------*
       int-pag-sdp-721.
      *                      *-----------------------------------------*
      *                      * Literal-1                               *
      ...
      *                          *-------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "         circuiti in questione, e/o richieste di p
      -              "agamento inviate direttamente al domicilio del deb
      -              "itore in caso contrario.        "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sdp-724.
      ...
      *                      *-----------------------------------------*
       ...
       int-pag-sdp-730.
      *                  *---------------------------------------------*
      *                  * Se tipo di avviso richiesto 03              *
      *                  *---------------------------------------------*
       int-pag-sdp-731.
      *                      *-----------------------------------------*
      *                      * Literal-1                               *
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "A fronte di tali disposizioni vorrete emettere per
      -              " nostro conto solo ricevute bancarie Ri.Ba.       
      -              "                                "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      ...
      *                      *-----------------------------------------*
       ...
       int-pag-sdp-740.
      *                  *---------------------------------------------*
      *                  * Se tipo di avviso richiesto 04              *
      *                  *---------------------------------------------*
       int-pag-sdp-741.
      *                      *-----------------------------------------*
      *                      * Literal-1                               *
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "A fronte di tali disposizioni vorrete emettere per
      -              " nostro conto delle richieste di pagamento inviate
      -              " direttamente al domicilio del  "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sdp-742.
      ...
      *                          *-------------------------------------*
           ...
           move      001                  to   p-pos                  .
           move      "A fronte di tali disposizioni vorrete emettere per
      -              " nostro conto degli ordini di addebito preautorizz
      -              "ati in conto corrente, R.I.D.   "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       int-pag-sdp-752.
      ...
      *                      *-----------------------------------------*
       ...
       int-pag-sdp-760.
      *                  *---------------------------------------------*
      *                  * Se tipo di avviso richiesto non riconosciu- *
      *                  * to                                          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Nessuna azione, a fincature             *
      *                      *-----------------------------------------*
           go to     int-pag-sdp-800.
       int-pag-sdp-800.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-tes-tip-ddp (1)    =    01
                     go to int-pag-sdp-821
           else      go to int-pag-sdp-822.
       int-pag-sdp-821.
      *                  *---------------------------------------------*
      *                  * Se tipo distinta 01 : Incassi elettronici   *
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
      ...
      -              "el trassato                     Appoggio bancario 
      -              "e Causale            Nr. interno"
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * A sottolineatura fincatura              *
      *                      *-----------------------------------------*
           go to     int-pag-sdp-850.
       int-pag-sdp-822.
      *                  *---------------------------------------------*
      *                  * Se tipo distinta 02 : Effetti               *
      *                  *                  03 : Paghero' cambiari     *
      ...
      *              * stampa dell'elemento del corpo : comunque 5     *
      *              * anche se potrebbero essere solo 4               *
      *              *-------------------------------------------------*
           move      5                    to   w-sta-dis-num-rnc      .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       sta-cor-dis-415.
      *                  *---------------------------------------------*
      *                  * Debitore, Partita Iva o Codice Fiscale      *
      *                  *---------------------------------------------*
           if        w-sta-dis-trs-piv    =    spaces
           ...
           move      p-lnr                to   p-lin                  .
           move      030                  to   p-pos                  .
           move      w-sta-dis-trs-piv    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       sta-cor-dis-420.
      *                  *---------------------------------------------*
      ...
      *                  * debitore e' a Spaces: no linea 5            *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       det-gdt-inl-100.
      *              *-------------------------------------------------*
      *              * Si legge l'anagrafica contabile per ottenere la *
      ...
      *                  * Spostamento dati                            *
      *                  *---------------------------------------------*
           move      w-let-arc-dbt-piv    to   w-sta-dis-trs-piv      .
      *                  *---------------------------------------------*
      *                  * Assemblaggio con literal                    *
      *                  *---------------------------------------------*
           move      40                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      "P.I. / Codice fiscale :"
                                          to   w-all-str-cat (01)     .
           move      w-sta-dis-trs-piv    to   w-all-str-cat (02)     .
           perform   all-str-csb-000      thru all-str-csb-999        .
           move      w-all-str-alf        to   w-sta-dis-trs-piv      .
       det-gdt-inl-150.
      *              *-------------------------------------------------*
      *              * Deviazione a seconda del tipo di inoltro per la *
      *              * scadenza                                        *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Se la distinta non e' per Incassi elettro-  *
      *                  * nici ne per Effetti : nessun editing        *
      *                  *---------------------------------------------*
           if        w-tes-tip-ddp (1)    not  = 01 and
                     w-tes-tip-ddp (1)    not  = 02
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Subroutine per eventuali riferimenti    *
      *                      * multipli relativi a compensazioni       *
      *                      *                                         *
      *                      * ___ vedi rib-050-sia-200 di pgep300g    *
      *                      * ___ DA IMPLEMENTARE                     *
      *                      *                                         *
      *                      *-----------------------------------------*
      *                      *-----------------------------------------*
      ...
      *    *===========================================================*
      *    * Estrazione voce successiva da stampare, gia' editata, per *
