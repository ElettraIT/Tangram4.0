      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Gestione tabella calcolo spese bollo su ef- *
      *                    fetti.                                      *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "GESTIONE TABELLA SPESE BOLLO SU EFFETTI "       .
      ...
      *        *-------------------------------------------------------*
           05  w-exp-tip-pag.
               10  w-exp-tip-pag-num      pic  9(02)       value 03   .
      ...
      *              *-------------------------------------------------*
           ...
           if        w-tes-tip-pag        =    09
                     move  01             to   v-num
           else if   w-tes-tip-pag        =    10
                     move  02             to   v-num
           else if   w-tes-tip-pag        =    11
                     move  03             to   v-num
      ...
      *              *-------------------------------------------------*
           if        v-num                =    01
                     move  09             to   w-tes-tip-pag
           else if   v-num                =    02
                     move  10             to   w-tes-tip-pag
           else if   v-num                =    03
                     move  11             to   w-tes-tip-pag
           else      move  zero           to   w-tes-tip-pag          .
       acc-tip-pag-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test che il valore sia tra quelli previsti  *
      *                  * oppure che sia a zero                       *
      *                  *---------------------------------------------*
           if        w-tes-tip-pag        =    zero or
                     w-tes-tip-pag        =    09   or
                     w-tes-tip-pag        =    10   or
                     w-tes-tip-pag        =    11
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se tipo automatismo che prevede la tabella  *
      *                  * scaglioni  : Si pagina 2                    *
      *                  * ALtrimenti : No pagina 2                    *
      *                  *---------------------------------------------*
           if        w-tes-tau-spb (1)    =    02 or
                     w-tes-tau-spb (1)    =    03
                     move  spaces         to   w-cnt-sts-imp-snp
      ...
      *              *-------------------------------------------------*
           ...
           move      w-tes-tip-pag        to   rf-zsb-tip-pag         .
           move      "pgm/gep/fls/ioc/obj/iofzsb"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                          * record [zsb]                        *
      *                          *-------------------------------------*
           move      rf-zsb-tau-spb       to   w-tes-tau-spb (1)      .
           move      rf-zsb-per-spb       to   w-tes-per-spb (1)      .
           move      zero                 to   w-cix-gen-001          .
       rou-let-reg-200.
           add       1                    to   w-cix-gen-001          .
           if        w-cix-gen-001        >    10
       ...
       rou-let-reg-220.
           move      rf-zsb-tet-spb       to   w-tes-tet-spb (1)      .
           move      rf-zsb-min-spb       to   w-tes-min-spb (1)      .
           move      rf-zsb-max-spb       to   w-tes-max-spb (1)      .
           move      rf-zsb-tar-spb       to   w-tes-tar-spb (1)      .
           move      rf-zsb-var-spb       to   w-tes-var-spb (1)      .
           move      rf-zsb-alx-exp       to   w-tes-alx-exp (1)      .
      *                      *-----------------------------------------*
      ...
      *                      * Write record [zsb]                      *
      *                      *-----------------------------------------*
           perform   wrt-rec-zsb-000      thru wrt-rec-zsb-999        .
      ...
      *                      * Rewrite record [zsb]                    *
      *                      *-----------------------------------------*
           perform   rew-rec-zsb-000      thru rew-rec-zsb-999        .
       scr-mov-fil-999.
           exit.
      ...
      *              * Delete record [zsb]                             *
      *              *-------------------------------------------------*
           perform   del-rec-zsb-000      thru del-rec-zsb-999        .
       del-mov-fil-999.
           exit.

      *    *===========================================================*
      *    * Composizione record [zsb]                                 *
      *    *-----------------------------------------------------------*
       cmp-rec-zsb-000.
      *              *-------------------------------------------------*
      ...
      *                  * Campi non chiave                            *
      *                  *---------------------------------------------*
           move      w-tes-tau-spb (1)    to   rf-zsb-tau-spb         .
           move      w-tes-per-spb (1)    to   rf-zsb-per-spb         .
           move      zero                 to   w-cix-gen-001          .
       cmp-rec-zsb-100.
           add       1                    to   w-cix-gen-001          .
           if        w-cix-gen-001        >    10
                     go to cmp-rec-zsb-120.
           move      w-tes-tbe-scg
                    (1, w-cix-gen-001)    to   rf-zsb-tbe-scg
                                              (w-cix-gen-001)         .
           move      w-tes-tbe-asc
                    (1, w-cix-gen-001)    to   rf-zsb-tbe-asc
                                              (w-cix-gen-001)         .
           move      w-tes-tbe-psc
                    (1, w-cix-gen-001)    to   rf-zsb-tbe-psc
                                              (w-cix-gen-001)         .
           go to     cmp-rec-zsb-100.
       cmp-rec-zsb-120.
           move      w-tes-tet-spb (1)    to   rf-zsb-tet-spb         .
           move      w-tes-min-spb (1)    to   rf-zsb-min-spb         .
           move      w-tes-max-spb (1)    to   rf-zsb-max-spb         .
           move      w-tes-tar-spb (1)    to   rf-zsb-tar-spb         .
           move      w-tes-var-spb (1)    to   rf-zsb-var-spb         .
           move      w-tes-alx-exp (1)    to   rf-zsb-alx-exp         .
       cmp-rec-zsb-999.
           exit.

      *    *===========================================================*
      *    * Scrittura record [zsb]                                    *
      *    *-----------------------------------------------------------*
       wrt-rec-zsb-000.
      *              *-------------------------------------------------*
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-zsb-000      thru cmp-rec-zsb-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
