      ...
      *                                   Fase:    gep580              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 29/07/96    *
      *                       Ultima revisione:    NdK del 12/04/23    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pgep5801:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Voci da esporre                            *
      *        *-------------------------------------------------------*
           05  w-exp-vde-stp.
               10  w-exp-vde-stp-num      pic  9(02)       value 5    .
               10  w-exp-vde-stp-lun      pic  9(02)       value 40   .
               10  w-exp-vde-stp-tbl.
                   15  filler             pic  x(40) value
                            "[ ] Controllo Fido                      ".
                   15  filler             pic  x(40) value
                            "[ ] Ordini inevasi                      ".
                   15  filler             pic  x(40) value
                            "[ ] Ordini di spedizione inevasi        ".
                   15  filler             pic  x(40) value
                            "[ ] Bolle da fatturare                  ".
                   15  filler             pic  x(40) value
                            "[ ] Scadenze aperte                     ".
      *        *-------------------------------------------------------*
      *        * Work per : Solo clienti con Fido                      *
      *        *-------------------------------------------------------*
           05  w-exp-sns-fid.
               10  w-exp-sns-fid-num      pic  9(02)       value 2    .
               10  w-exp-sns-fid-lun      pic  9(02)       value 02   .
               10  w-exp-sns-fid-tbl.
      ...
      *        *-------------------------------------------------------*
           05  w-exp-sns-det.
               10  w-exp-sns-det-num      pic  9(02)       value 2    .
               10  w-exp-sns-det-lun      pic  9(02)       value 02   .
               10  w-exp-sns-det-tbl.
                   15  filler             pic  x(02) value "No"       .
                   15  filler             pic  x(02) value "Si"       .
      ...
      *                  * Voci da esporre                             *
      *                  *---------------------------------------------*
           perform   acc-vde-stp-000      thru acc-vde-stp-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
                     go to acc-ric-sel-300.
      ...
      *                  * Solo clienti con Fido                       *
      *                  *---------------------------------------------*
           perform   acc-sns-fid-000      thru acc-sns-fid-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Stampa dettaglio                            *
      *                  *---------------------------------------------*
           perform   acc-sns-det-000      thru acc-sns-det-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           move      17                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Voci da esporre            :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Solo clienti con Fido'                  *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Solo clienti con Fido      :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt 'Stampa dettaglio'                       *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      28                   to   v-car                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo : Voci da esporre                      *
      *    *-----------------------------------------------------------*
       acc-vde-stp-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "C"                  to   v-tip                  .
           move      w-exp-vde-stp-lun    to   v-car                  .
           move      w-exp-vde-stp-num    to   v-ldt                  .
           move      "BX"                 to   v-edm                  .
           move      17                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-vde-stp-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-vde-stp           to   v-alf                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-vde-stp-999.
       acc-vde-stp-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-vde-stp             .
       acc-vde-stp-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Test su si/no Fido                          *
      *                  *---------------------------------------------*
           if        rr-vde-stp-fid       not  = spaces
                     go to acc-vde-stp-800.
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-vde-stp-100.
       acc-vde-stp-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo : Solo clienti con Fido                *
      *    *-----------------------------------------------------------*
       acc-sns-fid-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sns-fid-lun    to   v-car                  .
           move      w-exp-sns-fid-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sns-fid-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sns-fid           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
      ...
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sns-fid             .
       acc-sns-fid-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sns-fid           =    zero
                     go to acc-sns-fid-100.
       acc-sns-fid-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Stampa dettaglio           *
      *    *-----------------------------------------------------------*
       acc-sns-det-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sns-det-lun    to   v-car                  .
           move      w-exp-sns-det-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      20                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sns-det-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sns-det           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sns-det-999.
       acc-sns-det-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sns-det             .
       acc-sns-det-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sns-det           =    zero
                     go to acc-sns-det-100.
       acc-sns-det-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      " XXXX"              to   rr-vde-stp             .
           move      zero                 to   rr-sns-fid             .
           move      zero                 to   rr-sns-det             .
       nor-ric-sel-999.
           exit.
      ...
      *              *-------------------------------------------------*
      ...
      -              "re a quella minima !"
       ...
       tdo-ric-sel-400.
      *              *-------------------------------------------------*
      *              * Controllo su voci da esporre                    *
      *              *-------------------------------------------------*
           if        rr-vde-stp           not  = spaces
                     go to tdo-ric-sel-500.
           move      "ME"                 to   v-ope                  .
           move      "Almeno una voce da esporre !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
