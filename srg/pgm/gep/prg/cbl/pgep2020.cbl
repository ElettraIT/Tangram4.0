      ...
      *                                   Fase:    gep202              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 09/06/94    *
      *                       Ultima revisione:    NdK del 07/02/14    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma pgep2021:        *
      *                                                                *
      *                    Stampa anagrafica di controllo crediti      *
      *                    clienti                                     *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA ANAGRAFICA CONTR. CREDITI CLIENTI"       .
      ...
      *        *-------------------------------------------------------*
      *        * Ragione sociale cliente min                           *
      *        *-------------------------------------------------------*
           05  rr-rag-min                 pic  x(20)                  .
      *        *-------------------------------------------------------*
      *        * Ragione sociale cliente max                           *
      *        *-------------------------------------------------------*
           05  rr-rag-max                 pic  x(20)                  .
      *        *-------------------------------------------------------*
      *        * Mnemonico cliente min                                 *
      *        *-------------------------------------------------------*
           05  rr-mne-min                 pic  x(10)                  .
      *        *-------------------------------------------------------*
      *        * Mnemonico cliente max                                 *
      *        *-------------------------------------------------------*
           05  rr-mne-max                 pic  x(10)                  .
      *        *-------------------------------------------------------*
      *        * Selezione su anagrafiche di controllo                 *
      *        *                                                       *
      *        *  - 1 : Solo clienti con anagrafica di controllo       *
      *        *  - 2 : Solo clienti privi di anagrafica di controllo  *
      *        *  - 3 : Solo clienti esclusi dalla gestione solleciti  *
      *        *  - 4 : Solo clienti con copertura assicurativa        *
      *        *  - 5 : Tutti i clienti                                *
      *        *-------------------------------------------------------*
           05  rr-sel-anc                 pic  9(01)                  .
           ...
           05  w-exp-tip-ord.
               ...
               10  w-exp-tip-ord-tbl.
                   ...
                   15  filler             pic  x(20) value
                            "per Mnemonico       "                    .
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su anagrafiche di controllo      *
      *        *-------------------------------------------------------*
           05  w-exp-sel-anc.
               10  w-exp-sel-anc-num      pic  9(02)       value 5    .
               10  w-exp-sel-anc-lun      pic  9(02)       value 50   .
               10  w-exp-sel-anc-tbl.
                   15  filler             pic  x(50) value
                    "solo clienti con Anagrafica di controllo       " .
                   15  filler             pic  x(50) value
                    "solo clienti Privi di anagrafica di controllo  " .
                   15  filler             pic  x(50) value
                    "solo clienti Esclusi dalla gestione solleciti  " .
                   15  filler             pic  x(50) value
                    "solo clienti con Copertura assicurativa        " .
                   15  filler             pic  x(50) value
                    "Tutti i clienti                                " .
      ...
      *                  *---------------------------------------------*
       ...
       acc-ric-sel-450.
      *                  *---------------------------------------------*
      *                  * Selezioni su anagrafiche di controllo       *
      *                  *---------------------------------------------*
           perform   acc-sel-anc-000      thru acc-sel-anc-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      *              *-------------------------------------------------*
      *              * Prompt per selezione su anagrafiche di control- *
      *              * lo                                              *
      *              *-------------------------------------------------*
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           ...
           move      19                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su anagrafiche di   :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
           move      "DS"                 to   v-ope                  .
           ...
           move      20                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "  controllo crediti clienti    "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       pmt-ric-sel-999.
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Selezione su anagrafiche   *
      *    *                                controllo clienti          *
      *    *-----------------------------------------------------------*
       acc-sel-anc-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-anc-lun    to   v-car                  .
           move      w-exp-sel-anc-num    to   v-ldt                  .
           move      "APECT#"             to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      33                   to   v-pos                  .
           move      w-exp-sel-anc-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           if        w-cnt-sts-imp-ric    not  = spaces
                     move  "DO  "         to   v-pfk (05)             .
           move      rr-sel-anc           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sel-anc-999.
       acc-sel-anc-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sel-anc             .
       acc-sel-anc-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
           if        rr-sel-anc           =    zero
                     go to acc-sel-anc-100.
       acc-sel-anc-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Tipo selezione su anagrafiche di controllo      *
      *              *-------------------------------------------------*
           if        rr-sel-anc           =    zero
                     move  1              to   rr-sel-anc             .
       reg-ric-sel-999.
           exit.
