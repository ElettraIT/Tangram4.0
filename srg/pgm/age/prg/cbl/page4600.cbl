      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma page4601:        *
      *                                                                *
      ...
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su conteggi                      *
      *        *-------------------------------------------------------*
           05  w-exp-sel-ctg.
               10  w-exp-sel-ctg-num      pic  9(02)       value 02   .
               10  w-exp-sel-ctg-lun      pic  9(02)       value 50   .
               10  w-exp-sel-ctg-tbl.
                   15  filler             pic  x(50) value
                  "Nessuna                                           ".
                   15  filler             pic  x(50) value
                  "Solo conteggi con residuo da maturare             ".
      *        *-------------------------------------------------------*
      *        * Work per : Selezione su maturazioni                   *
      *        *-------------------------------------------------------*
           05  w-exp-sel-mtz.
               10  w-exp-sel-mtz-num      pic  9(02)       value 02   .
               10  w-exp-sel-mtz-lun      pic  9(02)       value 50   .
               10  w-exp-sel-mtz-tbl.
                   15  filler             pic  x(50) value
                  "Nessuna                                           ".
                   15  filler             pic  x(50) value
                  "Solo conteggi con maturazioni                     ".
      *        *-------------------------------------------------------*
      *        * Work per : Tipo di uso della stampa                   *
      *        *-------------------------------------------------------*
           05  w-exp-tip-uso.
               10  w-exp-tip-uso-num      pic  9(02)       value 03   .
      ...
      *                  * Selezione su conteggi                       *
      *                  *---------------------------------------------*
           perform   acc-sel-ctg-000      thru acc-sel-ctg-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Selezione su maturazioni                    *
      *                  *---------------------------------------------*
           perform   acc-sel-mtz-000      thru acc-sel-mtz-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *              * Selezione su conteggi                           *
      *              *-------------------------------------------------*
           perform   pmt-sel-ctg-000      thru pmt-sel-ctg-999        .
      *              *-------------------------------------------------*
      *              * Selezione su maturazioni                        *
      *              *-------------------------------------------------*
           perform   pmt-sel-mtz-000      thru pmt-sel-mtz-999        .
      *              *-------------------------------------------------*
      *              * Tipo di uso della stampa                        *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Prompt : Selezione su maturazioni                         *
      *    *-----------------------------------------------------------*
       pmt-sel-mtz-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Selezione su conteggi                      *
      *    *-----------------------------------------------------------*
       acc-sel-ctg-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-ctg-lun    to   v-car                  .
           move      w-exp-sel-ctg-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      14                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sel-ctg-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sel-ctg           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sel-ctg-999.
       acc-sel-ctg-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sel-ctg             .
       acc-sel-ctg-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Che sia un valore consentito                *
      *                  *---------------------------------------------*
           if        rr-sel-ctg           not  = 01 and
                     rr-sel-ctg           not  = 02
                     go to acc-sel-ctg-100.
       acc-sel-ctg-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione : Selezione su maturazioni                   *
      *    *-----------------------------------------------------------*
       acc-sel-mtz-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-mtz-lun    to   v-car                  .
           move      w-exp-sel-mtz-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      15                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-exp-sel-mtz-tbl    to   v-txt                  .
           move      "UP  "               to   v-pfk (01)             .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      rr-sel-mtz           to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-sel-mtz-999.
       acc-sel-mtz-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   rr-sel-mtz             .
       acc-sel-mtz-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Che sia un valore consentito                *
      *                  *---------------------------------------------*
           if        rr-sel-mtz           not  = 01 and
                     rr-sel-mtz           not  = 02
                     go to acc-sel-mtz-100.
       acc-sel-mtz-600.
      *              *-------------------------------------------------*
      *              * Dipendenze dall'impostazione                    *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      20                   to   w-cod-mne-dcc-vln      .
           move      41                   to   w-cod-mne-dcc-vps      .
           move      21                   to   w-cod-mne-dcc-lln      .
      ...
      *              *-------------------------------------------------*
           if        rr-sel-ctg           =    zero
                     move  01             to   rr-sel-ctg             .
      *              *-------------------------------------------------*
      *              * Selezione su maturazioni                        *
      *              *-------------------------------------------------*
           if        rr-sel-mtz           =    zero
                     move  01             to   rr-sel-mtz             .
      *              *-------------------------------------------------*
      *              * Eventuale aggiustamento flag di Si/No stampa in *
      *              * funzione del tipo uso richiesto                 *
      *              *-------------------------------------------------*
           if        rr-tip-uso           =    03
      ...
      *              * Selezione su conteggi                           *
      *              *-------------------------------------------------*
           move      zero                 to   rr-sel-ctg             .
      *              *-------------------------------------------------*
      *              * Selezione su maturazioni                        *
      *              *-------------------------------------------------*
           move      zero                 to   rr-sel-mtz             .
      *              *-------------------------------------------------*
      *              * Tipo di uso della stampa                        *
      *              *-------------------------------------------------*
           move      zero                 to   rr-tip-uso             .
      *              *-------------------------------------------------*
      *              * Codice cliente e dati correlati                 *
      *              *-------------------------------------------------*
