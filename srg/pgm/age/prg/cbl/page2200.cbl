      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:    Legami tra agenti e capi area per data     *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " LEGAMI TRA AGENTI E CAPI AREA PER DATA "       .
      ...
      *        *-------------------------------------------------------*
           05  w-rig-val-aep     occurs 2.
               ...
               10  w-rig-sup-age          pic  9(07)                  .
               10  w-rig-sup-age-nom      pic  x(20)                  .
      ...
      *    *===========================================================*
      *    * Work per scrittura record [ags]                           *
      *    *-----------------------------------------------------------*
       01  w-scr-rec-ags.
      *        *-------------------------------------------------------*
      *        * Comodo per assegnazione progressivo                   *
      *        *-------------------------------------------------------*
           05  w-scr-rec-ags-prg          pic  9(05)                  .
      ...
      *    *-----------------------------------------------------------*
       cnc-sub-cat-000.
           cancel    "pgm/age/prg/obj/page2202"                       .
       cnc-sub-cat-999.
           exit.
      ...
      *                  *---------------------------------------------*
           if        w-ipc-dlp-def-age    =    zero
                     go to acc-cod-age-100.
      *                  *---------------------------------------------*
      *                  * Valore di default in campo di destinazione  *
      *                  *---------------------------------------------*
           move      w-ipc-dlp-def-age    to   w-tes-cod-age          .
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
           if        w-rig-sup-age (1)    =    zero
                     move  "(nessun capo area)  "
                                          to   w-lin-imm-nom-age
           else      move  w-rig-sup-age-nom (1)
                                          to   w-lin-imm-nom-age      .
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Nr.     Capo area di riferimento        dal       
      -              "% provvigioni                 "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
      ...
      *                  *---------------------------------------------*
           ...
           move      21                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "% di provvigione per capo area :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       pmt-rig-cor-999.
      ...
      *                  *---------------------------------------------*
           move      w-rig-sup-age (1)    to   w-sav-sup-age          .
       acc-sup-age-100.
      *              *-------------------------------------------------*
      *              * Accettazione valore                             *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           move      w-sav-sup-age        to   w-rig-sup-age (1)      .
       acc-sup-age-105.
      *                  *---------------------------------------------*
      *                  * Parametri generici                          *
      *                  *---------------------------------------------*
           move      "AC"                 to   w-cod-mne-age-ope      .
           move      w-rig-sup-age (1)    to   w-cod-mne-age-cod      .
      ...
      *                  *---------------------------------------------*
           move      v-num                to   w-rig-sup-age (1)      .
      *                  *---------------------------------------------*
      *                  * Lettura archivio [age]                      *
      *                  *---------------------------------------------*
           move      w-rig-sup-age (1)    to   w-let-arc-age-cod      .
           perform   let-arc-age-000      thru let-arc-age-999        .
      *                  *---------------------------------------------*
      *                  * Memorizzazione dati agente                  *
      *                  *---------------------------------------------*
           move      w-let-arc-age-nom    to   w-rig-sup-age-nom (1)  .
      *                  *---------------------------------------------*
      *                  * Visualizzazione dati agente                 *
      *                  *---------------------------------------------*
           perform   vis-sup-age-nom-000  thru vis-sup-age-nom-999    .
      ...
      *              *-------------------------------------------------*
           ...
           move      "BD"                 to   v-edm                  .
           move      21                   to   v-lin                  .
           move      w-edt-per-pvg-c01    to   v-pos                  .
           if        w-edt-per-pvg-c01    >    3
      ...
      *              *-------------------------------------------------*
           ...
           else      go to acc-per-pvg-700.
       acc-per-pvg-630.
      *              *-------------------------------------------------*
      *              * Se Tab                                          *
      *              *-------------------------------------------------*
           if        w-edt-per-pvg-c01    =    03
                     go to acc-per-pvg-999.
      *                  *---------------------------------------------*
      *                  * Incremento contatore                        *
      *                  *---------------------------------------------*
           add       1                    to   w-edt-per-pvg-c01      .
           go to     acc-per-pvg-100.
       acc-per-pvg-650.
      *              *-------------------------------------------------*
      *              * Se Up                                           *
      *              *-------------------------------------------------*
           if        w-edt-per-pvg-c01    =    1
                     go to acc-per-pvg-999.
           subtract  1                    from w-edt-per-pvg-c01      .
           go to     acc-per-pvg-100.
       acc-per-pvg-700.
      *              *-------------------------------------------------*
      *              * Se Down o Return                                *
      ...
      *    *-----------------------------------------------------------*
       vis-per-pvg-000.
           move      spaces               to   w-edt-per-pvg-edt      .
           move      zero                 to   w-edt-per-pvg-c01      .
       vis-per-pvg-200.
           add       1                    to   w-edt-per-pvg-c01      .
           if        w-edt-per-pvg-c01    >    3
           ...
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      v-edt                to   w-edt-per-pvg-per
                                              (w-edt-per-pvg-c01)     .
       ...
       vis-per-pvg-500.
           ...
           move      21                   to   v-lin                  .
           move      34                   to   v-pos                  .
           move      w-edt-per-pvg-edt    to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                        using  v                      .
       vis-per-pvg-999.
           exit.
      ...
      *              *-------------------------------------------------*
           if        w-rig-sup-age (1)    not  = zero or
                     w-rig-dat-val (1)    not  = zero
                     go to cnt-tdo-rig-100.
           move      "Capo area e data, entrambi a zero, non accettabili
      ...
      *              *-------------------------------------------------*
           if        w-rig-sup-age (1)    not  = w-tes-cod-age
                     go to cnt-tdo-rig-200.
           move      "L'agente non e' subordinabile a se stesso !       
      -              "               "    to   w-err-box-err-msg      .
           go to     cnt-tdo-rig-900.
       cnt-tdo-rig-200.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-rig-dat-val (1)    to   w-ctl-val-dop-dat      .
           move      w-cat-rig-cur        to   w-ctl-val-dop-rig      .
      *                  *---------------------------------------------*
      *                  * Routine di controllo                        *
      *                  *---------------------------------------------*
           perform   ctl-val-dop-000      thru ctl-val-dop-999        .
           if        w-ctl-val-dop-flg    =    spaces
      ...
      *              * agente supervisore                              *
      *              *-------------------------------------------------*
           move      w-tes-cod-age        to   w-ctl-non-inv-age      .
           move      w-rig-sup-age (1)    to   w-ctl-non-inv-sge      .
           perform   ctl-non-inv-000      thru ctl-non-inv-999        .
           if        w-ctl-non-inv-flg    = spaces
                     go to cnt-tdo-rig-400.
           move      w-ctl-non-inv-mer    to   w-err-box-err-msg      .
           go to     cnt-tdo-rig-900.
       cnt-tdo-rig-400.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se il codice super-agente e' diverso da     *
      *                  * zero                                        *
      *                  *---------------------------------------------*
           if        w-rig-sup-age (1)    =    zero
                     go to cnt-tdo-rig-450.
      *                      *-----------------------------------------*
      *                      * Nessun controllo                        *
      *                      *-----------------------------------------*
       ...
       cnt-tdo-rig-450.
      *                  *---------------------------------------------*
      *                  * Se il codice super-agente e' uguale a zero  *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Le % provvigioni devono essere a zero   *
      *                      *-----------------------------------------*
           if        w-rig-per-pvg (1, 1) =    zero and
                     w-rig-per-pvg (1, 2) =    zero and
                     w-rig-per-pvg (1, 3) =    zero
                     go to cnt-tdo-rig-500.
           move      "Le % di provvigioni devono essere a zero !        
      -              "               "    to   w-err-box-err-msg      .
           go to     cnt-tdo-rig-900.
       cnt-tdo-rig-500.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      w-tes-cod-age        to   rf-ags-cod-age         .
      ...
      *                  *---------------------------------------------*
           if        rf-ags-cod-age       not  = w-tes-cod-age
                     go to  rou-let-reg-900.
      *                  *---------------------------------------------*
      ...
      *                          * record [ags]                        *
      *                          *-------------------------------------*
           move      rf-ags-num-prg       to   w-rig-num-prg (1)      .
           move      rf-ags-sup-age       to   w-rig-sup-age (1)      .
           move      rf-ags-dat-val       to   w-rig-dat-val (1)      .
           move      rf-ags-ide-dat       to   w-rig-ide-dat (1)      .
           move      rf-ags-ide-ute       to   w-rig-ide-ute (1)      .
           move      rf-ags-ide-fas       to   w-rig-ide-fas (1)      .
           move      rf-ags-cat-pvg       to   w-rig-cat-pvg (1)      .
           move      rf-ags-per-pvg (1)   to   w-rig-per-pvg (1, 1)   .
           move      rf-ags-per-pvg (2)   to   w-rig-per-pvg (1, 2)   .
           move      rf-ags-per-pvg (3)   to   w-rig-per-pvg (1, 3)   .
           move      rf-ags-alx-exp       to   w-rig-alx-exp (1)      .
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-ags-num-prg       to   w-cat-rig-prg          .
           move      w-rig                to   w-cat-rig-buf          .
           perform   cll-sub-cat-000      thru cll-sub-cat-999        .
      *                  *---------------------------------------------*
      *                  * Riciclo a lettura [ags] successivo          *
      *                  *---------------------------------------------*
           go to     rou-let-reg-200.
       rou-let-reg-900.
      *              *-------------------------------------------------*
      ...
      *              * Delete [ags]                                    *
      *              *-------------------------------------------------*
           perform   del-rec-ags-000      thru del-rec-ags-999        .
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Scrittura record [ags]                                    *
      *    *-----------------------------------------------------------*
       scr-rec-ags-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione progressivo                     *
      *              *-------------------------------------------------*
           move      zero                 to   w-scr-rec-ags-prg      .
      *              *-------------------------------------------------*
      *              * Se modifica                                     *
      *              *-------------------------------------------------*
           if        w-cnt-mfu-tip-fun    not  = "M"
                     go to scr-rec-ags-200.
       scr-rec-ags-100.
      *              *-------------------------------------------------*
      *              * Delete record file righe [ags]                  *
      *              *-------------------------------------------------*
           perform   del-mov-fil-000      thru del-mov-fil-999        .
       scr-rec-ags-200.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           return    srt    at end
                            go to scr-rec-ags-900                     .
       scr-rec-ags-400.
      *              *-------------------------------------------------*
      *              * Incremento progressivo                          *
      *              *-------------------------------------------------*
           add       100                  to   w-scr-rec-ags-prg      .
      *              *-------------------------------------------------*
      ...
      *              * Composizione record                             *
      *              *-------------------------------------------------*
           perform   cmp-rec-ags-000      thru cmp-rec-ags-999        .
      *              *-------------------------------------------------*
      *              * Put record                                      *
      *              *-------------------------------------------------*
           move      "PT"                 to   f-ope                  .
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           move      s-dat                to   rf-ags-ide-dat         .
           move      s-ute                to   rf-ags-ide-ute         .
           move      s-fas                to   rf-ags-ide-fas         .
           move      w-tes-cod-age        to   rf-ags-cod-age         .
           move      w-scr-rec-ags-prg    to   rf-ags-num-prg         .
           move      w-rig-sup-age (1)    to   rf-ags-sup-age         .
           move      w-rig-dat-val (1)    to   rf-ags-dat-val         .
           move      w-rig-cat-pvg (1)    to   rf-ags-cat-pvg         .
           move      w-rig-per-pvg (1, 1) to   rf-ags-per-pvg (1)     .
           move      w-rig-per-pvg (1, 2) to   rf-ags-per-pvg (2)     .
           move      w-rig-per-pvg (1, 3) to   rf-ags-per-pvg (3)     .
           move      w-rig-alx-exp (1)    to   rf-ags-alx-exp         .
       cmp-rec-ags-999.
           exit.

      *    *===========================================================*
      *    * Cancellazione record righe [ags]                          *
      *    *-----------------------------------------------------------*
       del-rec-ags-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-tes-cod-age        to   rf-ags-cod-age         .
           move      w-rig-num-prg (2)    to   rf-ags-num-prg         .
      *              *-------------------------------------------------*
      *              * Delete record                                   *
      *              *-------------------------------------------------*
           move      "DE"                 to   f-ope                  .
