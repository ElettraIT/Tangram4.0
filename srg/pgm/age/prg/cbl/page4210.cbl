      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Interrogazione su maturazioni relative ad   *
      *                    un conteggio                                *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "RICERCA MATURAZIONI RELATIVE A CONTEGGIO"       .
      ...
      *        *-------------------------------------------------------*
           05  w-ipc-dlp.
      *            *---------------------------------------------------*
      *            * Per numero conteggio                              *
      *            *---------------------------------------------------*
               10  w-ipc-dlp-num-ctg      pic  9(09)                  .

      *    *===========================================================*
      *    * Work-area per ridefinizione contenuto del mark-point      *
      *    *-----------------------------------------------------------*
       01  w-mpn.
           05  w-mpn-num-mtz              pic  9(09)                  .
      ...
      *              *-------------------------------------------------*
      *              * Estrazione variabili di i.p.c.                  *
      *              *-------------------------------------------------*
           perform   ipc-dlp-ctg-000      thru ipc-dlp-ctg-999        .
      *              *-------------------------------------------------*
      *              * Se nessuna variabile letta : uscita dal pro-    *
      *              * gramma                                          *
      *              *-------------------------------------------------*
           if        w-ipc-dlp-num-ctg    =    zero
                     move  "#"            to   w-cnt-pre-exe-pgm
                     go to pre-exe-pgm-999.
      *              *-------------------------------------------------*
      *              * Valore letto in campo di richiesta              *
      *              *-------------------------------------------------*
           move      w-ipc-dlp-num-ctg    to   rr-num-ctg             .
       pre-exe-pgm-999.
           exit.

      *    *===========================================================*
      *    * Lettura i.p.c. da livello precedente per numero conteggio *
      *    *-----------------------------------------------------------*
       ipc-dlp-ctg-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione iniziale                        *
      *              *-------------------------------------------------*
           move      zero                 to   w-ipc-dlp-num-ctg      .
      *              *-------------------------------------------------*
      *              * Estrazione della eventuale variabile di i.p.c.  *
      *              * "num-ctg" per numero conteggio                  *
      *              *-------------------------------------------------*
           move      "CV"                 to   s-ope                  .
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-ves                not  = spaces
                     go to ipc-dlp-ctg-999.
       ipc-dlp-ctg-200.
      *              *-------------------------------------------------*
      *              * Valore in campo di trattamento                  *
      *              *-------------------------------------------------*
           move      s-num                to   w-ipc-dlp-num-ctg      .
       ipc-dlp-ctg-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-qry-flg-sub      .
      *              *-------------------------------------------------*
      *              * Start per numero conteggio                      *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "CTGMTZ    "         to   f-key                  .
           move      rr-num-ctg           to   rf-gpm-num-ctg         .
      ...
      *    *-----------------------------------------------------------*
       qry-nes-ela-000.
           move      "ME"                 to   v-ope                  .
           move      "Nessuna maturazione entro i limiti assegnati !"
                                          to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
       qry-nes-ela-999.
      ...
      *              *-------------------------------------------------*
       ...
       qry-tst-max-100.
      *              *-------------------------------------------------*
      *              * Test su numero conteggio                        *
      *              *-------------------------------------------------*
           if        rf-gpm-num-ctg       not  = rr-num-ctg
                     move  "#"            to   w-cnt-qry-flg-sub
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      12                   to   v-pos                  .
           move      rf-gpm-dat-mtz       to   v-dat                  .
           move      "+"                  to   v-edm                  .
           move      rf-gpm-num-mtz       to   w-mpn-num-mtz          .
           move      w-mpn                to   v-cnt                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      39                   to   v-pos                  .
           if        rf-gpm-tip-mtz       =    01
                     move  "Immediata "   to   v-alf
           else if   rf-gpm-tip-mtz       =    02
                     move  "f/incasso "   to   v-alf
           else if   rf-gpm-tip-mtz       =    51
                     move  "Storno    "   to   v-alf
           else      move  "?"            to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *                  *---------------------------------------------*
      *                  * Annotazioni per la maturazione              *
      *                  *---------------------------------------------*
           move      "PF"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
           move      31                   to   v-car                  .
      ...
      *                  *---------------------------------------------*
      *                  * Put della variabile di i.p.c.: 'num-mtz'    *
      *                  *---------------------------------------------*
           move      "PV"                 to   s-ope                  .
           move      "num-mtz"            to   s-var                  .
           move      "-"                  to   s-dop                  .
           move      "N"                  to   s-tip                  .
           move      09                   to   s-car                  .
           move      zero                 to   s-dec                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      v-lnr                to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "  Numero     Data   Importo maturato     Tipo     
      -              "         Annotazioni          "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
