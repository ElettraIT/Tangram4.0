      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Manutenzione blocco maturazione conteggi    *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "MANUTENZIONE BLOCCO MATURAZIONE CONTEGGI"       .
      ...
      *            *---------------------------------------------------*
               10  w-tes-snx-blo          pic  9(02)                  .
      *            *---------------------------------------------------*
      *            * Eventuali forzature                               *
      *            *---------------------------------------------------*
               10  w-tes-for-blo          pic  9(02)                  .
      *            *---------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       01  w-exp.
      *        *-------------------------------------------------------*
      *        * Work per : Si/no solo bloccati                        *
      *        *-------------------------------------------------------*
           05  w-exp-snx-blo.
               10  w-exp-snx-blo-num      pic  9(02)       value 03   .
               10  w-exp-snx-blo-lun      pic  9(02)       value 40   .
               10  w-exp-snx-blo-tbl.
                   15  filler             pic  x(40) value
                            "solo quelli Da bloccare                 ".
                   15  filler             pic  x(40) value
                            "solo quelli Bloccati                    ".
                   15  filler             pic  x(40) value
                            "Tutti i conteggi                        ".
      *        *-------------------------------------------------------*
      *        * Work per : Eventuali forzature                        *
      *        *-------------------------------------------------------*
           05  w-exp-for-blo.
               10  w-exp-for-blo-num      pic  9(02)       value 03   .
               10  w-exp-for-blo-lun      pic  9(02)       value 40   .
               10  w-exp-for-blo-tbl.
                   15  filler             pic  x(40) value
                            "Nessuna forzatura                       ".
                   15  filler             pic  x(40) value
                            "Blocco  delle maturazioni relative      ".
                   15  filler             pic  x(40) value
                            "Sblocco delle maturazioni relative      ".
      *        *-------------------------------------------------------*
      *        * Work per : No/Si blocco                               *
      *        *-------------------------------------------------------*
           05  w-exp-sel-blo.
               10  w-exp-sel-blo-num      pic  9(02)       value 2    .
               10  w-exp-sel-blo-lun      pic  9(02)       value 02   .
               10  w-exp-sel-blo-tbl.
      ...
      *    *-----------------------------------------------------------*
       01  w-cmp-rec.
      *        *-------------------------------------------------------*
      *        * Work per composizione record [gpc]                    *
      *        *-------------------------------------------------------*
           05  w-cmp-rec-gpc.
      *            *---------------------------------------------------*
      *            * Protocollo                                        *
      *            *---------------------------------------------------*
               10  w-cmp-rec-gpc-prt      pic  9(09)                  .
      ...
      *                  * Selezioni su conteggi                       *
      *                  *---------------------------------------------*
           perform   acc-snx-blo-000      thru acc-snx-blo-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *                  * Eventuali forzature                         *
      *                  *---------------------------------------------*
           perform   acc-for-blo-000      thru acc-for-blo-999        .
           if        w-cnt-tus-acc-key    not  = spaces
                     go to acc-key-reg-999.
           if        v-key                =    "UP  "
      ...
      *              * Selezioni su conteggi                           *
      *              *-------------------------------------------------*
           perform   vis-snx-blo-000      thru vis-snx-blo-999        .
      *              *-------------------------------------------------*
      *              * Eventuali forzature                             *
      *              *-------------------------------------------------*
           perform   vis-for-blo-000      thru vis-for-blo-999        .
       vis-key-reg-999.
           exit.
      ...
      *                  * Selezioni su conteggi                       *
      *                  *---------------------------------------------*
           perform   pmt-snx-blo-000      thru pmt-snx-blo-999        .
      *                  *---------------------------------------------*
      *                  * Eventuali forzature                         *
      *                  *---------------------------------------------*
           perform   pmt-for-blo-000      thru pmt-for-blo-999        .
       pmt-key-reg-999.
           exit.
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-snx-blo-lun    to   v-car                  .
           move      w-exp-snx-blo-num    to   v-ldt                  .
           move      "DBT#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-snx-blo-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      11                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-snx-blo        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-snx-blo-999.
       acc-snx-blo-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-snx-blo          .
       acc-snx-blo-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-for-blo-lun    to   v-car                  .
           move      w-exp-for-blo-num    to   v-ldt                  .
           move      "NBS#"               to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      w-exp-for-blo-tbl    to   v-txt                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           move      13                   to   v-lin                  .
           move      30                   to   v-pos                  .
           move      w-tes-for-blo        to   v-num                  .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      *              * Se Exit                                         *
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-key
                     go to acc-for-blo-999.
       acc-for-blo-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-tes-for-blo          .
       acc-for-blo-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *                  * Si/no blocco                                *
      *                  *---------------------------------------------*
           perform   acc-sel-blo-000      thru acc-sel-blo-999        .
           if        w-cnt-tus-acc-rig    not  = spaces
      ...
      *              * Si/no blocco                                    *
      *              *-------------------------------------------------*
           perform   vis-sel-blo-000      thru vis-sel-blo-999        .
      *              *-------------------------------------------------*
      *              * Numero conteggio                                *
      *              *-------------------------------------------------*
      ...
      *              * Si/no blocco                                    *
      *              *-------------------------------------------------*
           perform   pmt-sel-blo-000      thru pmt-sel-blo-999        .
      *              *-------------------------------------------------*
      *              * Numero conteggio                                *
      *              *-------------------------------------------------*
      ...
      *    *===========================================================*
      *    * Accettazione campo : Si/no blocco                         *
      *    *-----------------------------------------------------------*
       acc-sel-blo-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
           move      w-exp-sel-blo-lun    to   v-car                  .
           move      w-exp-sel-blo-num    to   v-ldt                  .
           move      "NS#"                to   v-msk                  .
           move      spaces               to   v-edm                  .
           move      19                   to   v-lin                  .
           move      22                   to   v-pos                  .
           move      w-exp-sel-blo-tbl    to   v-txt                  .
           move      w-rig-mat-blo        to   v-num                  .
           move      "DOWN"               to   v-pfk (02)             .
           move      "DO  "               to   v-pfk (05)             .
           perform   exe-acc-cmp-000      thru exe-acc-cmp-999        .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-tus-acc-rig
                     go to acc-sel-blo-999.
       acc-sel-blo-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-num                to   w-rig-mat-blo          .
       acc-sel-blo-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-rig-flg
                             ...
                             go to acc-sel-blo-100.
       acc-sel-blo-999.
           exit.

      *    *===========================================================*
      *    * Visualizzazione campo : Si/no blocco                      *
      *    *-----------------------------------------------------------*
       vis-sel-blo-000.
           move      "DS"                 to   v-ope                  .
           move      "E"                  to   v-tip                  .
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-gpc-num-doc       to   w-all-str-cat (1)      .
      ...
      *    *-----------------------------------------------------------*
       nor-key-reg-000.
           ...
           move      zero                 to   w-tes-snx-blo          .
           move      zero                 to   w-tes-for-blo          .
           move      spaces               to   w-tes-cnf-imp          .
       nor-key-reg-999.
           exit.
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-gpc-mat-blo       to   w-rig-mat-blo          .
      *                      *-----------------------------------------*
      *                      * Anagrafica cliente                      *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * Eventuale forzatura blocco              *
      *                      *-----------------------------------------*
           if        w-tes-for-blo        =    01
                     go to car-fil-rap-800
           else if   w-tes-for-blo        =    02
                     move  02             to   w-rig-mat-blo
           else if   w-tes-for-blo        =    03
                     move  01             to   w-rig-mat-blo
      ...
      *              *-------------------------------------------------*
           move      w-rig-rec-gpc        to   rf-gpc                 .
           move      rf-gpc-num-ctg       to   w-cmp-rec-gpc-prt      .
       sca-fil-rap-500.
      *              *-------------------------------------------------*
      *              * Aggiornamento documenti                         *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Ottenimento record [gpc]                    *
      *                  *---------------------------------------------*
           move      "GK"                 to   f-ope                  .
           move      "NUMCTG    "         to   f-key                  .
           move      w-cmp-rec-gpc-prt    to   rf-gpc-num-ctg         .
           move      "pgm/age/fls/ioc/obj/iofgpc"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        w-rig-mat-blo        =    rf-gpc-mat-blo
      ...
      *                  *---------------------------------------------*
           move      w-rig-mat-blo        to   rf-gpc-mat-blo         .
       sca-fil-rap-700.
      *                  *---------------------------------------------*
      *                  * Update record [gpc]                         *
      *                  *---------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       cnc-rlt-sup-000.
           cancel    "pgm/age/prg/obj/page9302"                       .
       cnc-rlt-sup-999.
           exit.
