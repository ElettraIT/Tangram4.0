      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma page1321:        *
      *                                                                *
      *                    Stampa delle categorie di provvigioni       *
      *                    legate al cliente                           *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "STAMPA CATEGORIE PROVVIGIONI PER CLIENTE"       .
      ...
      *    *-----------------------------------------------------------*
       01  rr.
      *        *-------------------------------------------------------*
      *        * Tipo categoria provvigioni                            *
      *        *-------------------------------------------------------*
           05  rr-tip-cpv                 pic  9(02)                  .
      *        *-------------------------------------------------------*
      ...
      *        *-------------------------------------------------------*
      *        * Codice categoria min                                  *
      *        *-------------------------------------------------------*
           05  rr-cod-min                 pic  9(05)                  .
      *        *-------------------------------------------------------*
      *        * Codice categoria max                                  *
      *        *-------------------------------------------------------*
      ...
      *              * Titolo selezioni                                *
      *              *-------------------------------------------------*
           perform   pmt-tit-sel-000      thru pmt-tit-sel-999        .
      *              *-------------------------------------------------*
      *              * Selezione su codice                             *
      *              *-------------------------------------------------*
           perform   pmt-sel-cod-000      thru pmt-sel-cod-999        .
      *              *-------------------------------------------------*
      *              * Selezione su mnemonico                          *
      *              *-------------------------------------------------*
           perform   pmt-sel-mne-000      thru pmt-sel-mne-999        .
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       pmt-tip-ord-000.
           ...
           move      05                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Ordinamento stampa per le  :"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
      ...
      *    *-----------------------------------------------------------*
       pmt-tit-sel-000.
           ...
           move      10                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezioni su categorie provvigioni per cliente    
      -              "                              "
                                   to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *    *===========================================================*
      *    * Prompt : Selezione per mnemonico                          *
      *    *-----------------------------------------------------------*
       pmt-sel-mne-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Prompt : Selezione per descrizione                        *
      *    *-----------------------------------------------------------*
       pmt-sel-des-000.
           move      "DS"                 to   v-ope                  .
           move      "A"                  to   v-tip                  .
      ...
      *    *===========================================================*
      *    * Accettazione : Codice categoria min                       *
      *    *-----------------------------------------------------------*
       acc-cod-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cmn-zpv-003-cod      .
      ...
      *    *===========================================================*
      *    * Accettazione : Codice categoria max                       *
      *    *-----------------------------------------------------------*
       acc-cod-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cmn-zpv-003-ope      .
           move      rr-cod-max           to   w-cmn-zpv-003-cod      .
           move      14                   to   w-cmn-zpv-003-lin      .
           move      52                   to   w-cmn-zpv-003-pos      .
           move      zero                 to   w-cmn-zpv-003-dln      .
           move      zero                 to   w-cmn-zpv-003-dps      .
           move      "<B"                 to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo su tipo categoria provvigioni         *
      *              *-------------------------------------------------*
           if        rr-tip-cpv           not  = zero
                     go to tdo-ric-sel-200.
           move      "Manca il tipo di categoria di provvigione !       
      ...
      *              *-------------------------------------------------*
      ...
      -              "               "    to   w-err-box-err-msg      .
       ...
       tdo-ric-sel-205.
           ...
           move      "Tipo di ordinamento per la stampa errato !        
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-300.
      *              *-------------------------------------------------*
      *              * Controllo su codice categoria min-max           *
      *              *-------------------------------------------------*
           if        rr-cod-max           =    zero
                     go to tdo-ric-sel-400.
           if        rr-cod-max           not  < rr-cod-min
                     go to tdo-ric-sel-400.
           move      "Codice categoria massimo inferiore a quello minimo
      -              " !             "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-400.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           if        rr-mne-max           not  < rr-mne-min
                     go to tdo-ric-sel-500.
           move      "Mnemonico massimo inferiore a quello minimo !     
      -              "               "    to   w-err-box-err-msg      .
           go to     tdo-ric-sel-900.
       tdo-ric-sel-500.
      *              *-------------------------------------------------*
      *              * Controllo su descrizione min-max                *
      *              *-------------------------------------------------*
