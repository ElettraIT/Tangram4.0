      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma age122   *
      *                                                                *
      *                    Stampa delle categorie di provvigioni       *
      *                    legate alal prodotto                        *
      *                                                                *
      *================================================================*
      ...
      *            * Codice categoria                                  *
      *            *---------------------------------------------------*
               10  srt-cod-cpv            pic  9(05)                  .
      *            *---------------------------------------------------*
      *            * Mnemonico                                         *
      *            *---------------------------------------------------*
               10  srt-mne-cpv            pic  x(05)                  .
      *            *---------------------------------------------------*
      *            * Descrizione                                       *
      *            *---------------------------------------------------*
               10  srt-des-cpv            pic  x(20)                  .
      *            *---------------------------------------------------*
      *            * Area di comodo per contenere il record [zpv]      *
      *            *---------------------------------------------------*
               10  srt-rec-zpv.
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     " STAMPA CATEGORIE PROVVIG. PER PRODOTTO "       .
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-120.
      *                  *---------------------------------------------*
      *                  * Tipo ordinamento stampa : per codice        *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Start                                   *
      *                      *-----------------------------------------*
           ...
           move      rr-tip-cpv           to   rf-zpv-tip-cpv         .
           move      rr-cod-min           to   rf-zpv-cod-cpv         .
           move      "pgm/age/fls/ioc/obj/iofzpv"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-140.
      *                  *---------------------------------------------*
      *                  * Tipo ordinamento stampa : per mnemonico     *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Start                                   *
      *                      *-----------------------------------------*
           ...
           move      rr-tip-cpv           to   rf-zpv-tip-cpv         .
           move      rr-mne-min           to   rf-zpv-mne-cpv         .
      ...
      *                      *-----------------------------------------*
       ...
       stp-srt-inp-160.
      *                  *---------------------------------------------*
      *                  * Tipo ordinamento stampa : per descrizione   *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Start                                   *
      *                      *-----------------------------------------*
           ...
           move      rr-tip-cpv           to   rf-zpv-tip-cpv         .
           move      rr-des-min           to   rf-zpv-des-key         .
           move      zero                 to   rf-zpv-cod-cpv         .
           move      "pgm/age/fls/ioc/obj/iofzpv"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *              *-------------------------------------------------*
       stp-srt-inp-305.
      *                  *---------------------------------------------*
      *                  * Test su tipo categoria provvigione          *
      *                  *---------------------------------------------*
           if        rf-zpv-tip-cpv       not  = rr-tip-cpv
                     go to stp-srt-inp-800.
       stp-srt-inp-310.
      *                  *---------------------------------------------*
      *                  * Deviazione in funzione del tipo ordinamento *
      ...
      *              * Selezione                                       *
      *              *-------------------------------------------------*
           if        rf-zpv-cod-cpv       <    rr-cod-min or
                     rf-zpv-cod-cpv       >    rr-cod-max
                     go to stp-srt-inp-700.
           if        rf-zpv-mne-cpv       <    rr-mne-min or
                     rf-zpv-mne-cpv       >    rr-mne-max
                     go to stp-srt-inp-700.
           if        rf-zpv-des-key       <    rr-des-min or
                     rf-zpv-des-key       >    rr-des-max
                     go to stp-srt-inp-700.
       stp-srt-inp-500.
      *              *-------------------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      spaces               to   srt-k01-mne-cpv        .
           move      spaces               to   srt-k01-des-key        .
           move      rf-zpv-cod-cpv       to   srt-k01-cod-cpv        .
      *                      *-----------------------------------------*
      *                      * Composizione area dati                  *
      *                      *-----------------------------------------*
           move      rf-zpv-cod-cpv       to   srt-cod-cpv            .
           move      rf-zpv-mne-cpv       to   srt-mne-cpv            .
           move      rf-zpv-des-cpv       to   srt-des-cpv            .
      *                      *-----------------------------------------*
      *                      * Composizione area di comodo per conte-  *
      *                      * nere il record                          *
      *                      *-----------------------------------------*
           move      rf-zpv               to   srt-rec-zpv            .
      *                      *-----------------------------------------*
      *                      * Continuazione                           *
      *                      *-----------------------------------------*
      ...
      *                      * 1. linea titolo                         *
      *                      *-----------------------------------------*
           move      "CATEGORIE PROVVIGIONI PER PRODOTTO"
      ...
      *                  *---------------------------------------------*
       ...
       stp-int-pag-400.
      *              *-------------------------------------------------*
      *              * Stampa fincatura per categoria provvigione nel  *
      *              * corpo dello stampato                            *
      *              *-------------------------------------------------*
      ...
      *    * di provvigione nel corpo dello stampato                   *
      *    *-----------------------------------------------------------*
       stp-fnc-ccs-000.
      *              *-------------------------------------------------*
      ...
      *              * Codice categoria di provvigione                 *
      *              *-------------------------------------------------*
           move      srt-cod-cpv          to   w-stp-det-cod-cpv      .
      *              *-------------------------------------------------*
      *              * Mnemonico                                       *
      *              *-------------------------------------------------*
           move      srt-mne-cpv          to   w-stp-det-mne-cpv      .
      *              *-------------------------------------------------*
      *              * Descrizione                                     *
      *              *-------------------------------------------------*
           move      srt-des-cpv          to   w-stp-det-des-cpv      .
       stp-ele-det-p00-999.
           exit.
      ...
      *    * Subroutine per la stampa di un elemento di dettaglio      *
      *    *                                                           *
      *    * Preparazione degli elementi da stampare dal record [zpv]  *
      *    *-----------------------------------------------------------*
       stp-ele-det-p10-000.
      *              *-------------------------------------------------*
      *              * Scaletta % di provvigioni dirette               *
      ...
      *              * Trattamento provvigioni indirette               *
      *              *-------------------------------------------------*
           move      rf-zpv-trt-pvi       to   w-stp-det-trt-pvi      .
       stp-ele-det-p10-400.
      *              *-------------------------------------------------*
      *              * Scaletta % di provvigioni indirette             *
      *              *-------------------------------------------------*
      ...
      *              * Area libera per espansioni speciali             *
      *              *-------------------------------------------------*
           move      rf-zpv-alx-exp       to   w-stp-det-alx-exp      .
       stp-ele-det-p10-999.
           exit.
