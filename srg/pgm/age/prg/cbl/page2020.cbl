      ...
      *                       Ultima revisione:    NdK del 06/11/02    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Richieste per il programma page2021:        *
      *                    Stampa anagrafica agenti                    *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  rr.
           ...
           05  rr-nom-min                 pic  x(20)                  .
           05  rr-nom-max                 pic  x(20)                  .
      ...
      *                  * Nome agente min                             *
      *                  *---------------------------------------------*
           perform   acc-nom-min-000      thru acc-nom-min-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *                  * Nome agente max                             *
      *                  *---------------------------------------------*
           perform   acc-nom-max-000      thru acc-nom-max-999        .
           if        w-cnt-acc-ric-sel    not  = spaces
                     go to acc-ric-sel-999.
           if        v-key                =    "UP  "
      ...
      *    *-----------------------------------------------------------*
       pmt-ric-sel-000.
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Selezione su archivio agenti"
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "DS"                 to   v-ope                  .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Nominativo min             *
      *    *-----------------------------------------------------------*
       acc-nom-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-nom-min-999.
       acc-nom-min-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-nom-min             .
       acc-nom-min-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-nom-min-100.
       acc-nom-min-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Nominativo max             *
      *    *-----------------------------------------------------------*
       acc-nom-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "EXIT"
                     move  "E"            to   w-cnt-acc-ric-sel
                     go to acc-nom-max-999.
       acc-nom-max-200.
      *              *-------------------------------------------------*
      *              * Valore impostato in campo di destinazione       *
      *              *-------------------------------------------------*
           move      v-alf                to   rr-nom-max             .
       acc-nom-max-400.
      *              *-------------------------------------------------*
      *              * Controllo valore impostato                      *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           if        v-key                =    "DO  "
                     ...
                     if      w-cnt-tdo-ric-flg
                             ...
                             go to acc-nom-max-100.
       acc-nom-max-999.
           exit.

      *    *===========================================================*
      *    * Accettazione campo selezione : Ragione sociale min        *
      *    *-----------------------------------------------------------*
       acc-rag-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Ragione sociale max        *
      *    *-----------------------------------------------------------*
       acc-rag-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-cod-min           to   w-cod-mne-age-cod      .
      ...
      *              *-------------------------------------------------*
           move      "AC"                 to   w-cod-mne-age-ope      .
           move      rr-cod-max           to   w-cod-mne-age-cod      .
           move      15                   to   w-cod-mne-age-lin      .
           move      52                   to   w-cod-mne-age-pos      .
           move      zero                 to   w-cod-mne-age-nln      .
           move      zero                 to   w-cod-mne-age-nps      .
           move      "<B"                 to   v-edm                  .
           move      "UP  "               to   v-pfk (01)             .
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Mnemonico min              *
      *    *-----------------------------------------------------------*
       acc-mne-min-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *===========================================================*
      *    * Accettazione campo selezione : Mnemonico max              *
      *    *-----------------------------------------------------------*
       acc-mne-max-000.
      *              *-------------------------------------------------*
      *              * Pre-accettazione                                *
      ...
      *    *-----------------------------------------------------------*
       nor-ric-sel-000.
           ...
           move      spaces               to   rr-nom-min             .
           move      spaces               to   rr-nom-max             .
      ...
      *              *-------------------------------------------------*
       ...
       tdo-ric-sel-100.
      *              *-------------------------------------------------*
      *              * Controllo su Nominativo min e max               *
      *              *-------------------------------------------------*
           if        rr-nom-max           =    spaces
                     go to tdo-ric-sel-200.
           if        rr-nom-max           not  < rr-nom-min
                     go to tdo-ric-sel-200.
           move      "ME"                 to   v-ope                  .
           move      "Il nominativo massimo non puo' essere inferiore a 
      -              "quello minimo !"    to   v-not                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
           move      "#"                  to   w-cnt-tdo-ric-flg      .
           go to     tdo-ric-sel-999.
       tdo-ric-sel-200.
      *              *-------------------------------------------------*
      *              * Controllo su Ragione sociale min e max          *
      *              *-------------------------------------------------*
      ...
      *    *-----------------------------------------------------------*
       reg-ric-sel-000.
      *              *-------------------------------------------------*
      *              * Regolarizzazione Nominativo min - max           *
      *              *-------------------------------------------------*
           if        rr-nom-max           =    spaces
                     move  rr-nom-min     to   rr-nom-max             .
      *
           move      rr-nom-max           to   w-all-str-alf          .
           move      20                   to   w-all-str-lun          .
           perform   all-str-pad-000      thru all-str-pad-999        .
           move      w-all-str-alf        to   rr-nom-max             .
      *              *-------------------------------------------------*
      *              * Regolarizzazione Ragione sociale min - max      *
      *              *-------------------------------------------------*
           if        rr-rag-max           =    spaces
      ...
      *    *===========================================================*
      *    * Tasto funzione "FIND" su archivio agenti                  *
      *    *-----------------------------------------------------------*
       fnd-arc-age-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
           if        s-liv                not  = zero
                     go to  fnd-arc-age-999.
      *              *-------------------------------------------------*
      *              * Richiamo programma di interrogazione            *
      *              *-------------------------------------------------*
           move      "pgm/age/prg/obj/page2010"
                                          to   s-pat                  .
