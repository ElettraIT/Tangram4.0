      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma age202   *
      ...
      *        *-------------------------------------------------------*
           05  w-rot-l01.
      *            *---------------------------------------------------*
      *            * Lettera iniziale per nominativo o mnemonico       *
      *            *---------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-cod-min           to   rf-age-cod-age         .
           move      "pgm/age/fls/ioc/obj/iofage"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
       ...
       prn-str-ini-300.
      *              *-------------------------------------------------*
      *              * Se ordinamento per nominativo                   *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Start per nominativo in uppercase           *
      *                  *---------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "NOMKEY    "         to   f-key                  .
           move      rr-nom-min           to   rf-age-nom-key         .
      ...
      *                  *---------------------------------------------*
           ...
           move      rr-mne-min           to   rf-age-cod-mne         .
           move      zero                 to   rf-age-cod-age         .
           move      "pgm/age/fls/ioc/obj/iofage"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
      ...
      *    *-----------------------------------------------------------*
       prn-nes-ela-000.
           move      "WR"                 to   m-ope                  .
           move      "Nessun agente entro i limiti assegnati !"
                                          to   m-msg                  .
      ...
      *              *-------------------------------------------------*
           if        rf-age-cod-age       <    rr-cod-min or
                     rf-age-cod-age       >    rr-cod-max or
                     rf-age-rag-key       <    rr-rag-min or
                     rf-age-rag-key       >    rr-rag-max or
                     rf-age-cod-mne       <    rr-mne-min or
                     rf-age-cod-mne       >    rr-mne-max or
                     rf-age-nom-key       <    rr-nom-min or
                     rf-age-nom-key       >    rr-nom-max
                     move  "#"            to   w-cnt-prn-flg-sub
      ...
      *    *-----------------------------------------------------------*
       prn-cmp-rot-000.
      *              *-------------------------------------------------*
      *              * Se tipo ordinamento : per nominativo            *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "N"
                     move  rf-age-nom-key to   w-rot-l01-let-ini
                     go to prn-cmp-rot-999.
      *              *-------------------------------------------------*
      *              * Se tipo ordinamento : per mnemonico             *
      *              *-------------------------------------------------*
           if        rr-tip-ord           =    "M"
                     move  rf-age-cod-mne to   w-rot-l01-let-ini
                     go to prn-cmp-rot-999.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Se ordinamento per codice : nessuna azione      *
      *              *-------------------------------------------------*
           if        rr-tip-ord           not  = "N" and
                     rr-tip-ord           not  = "M"
      ...
      *              *-------------------------------------------------*
           move      "ELENCO AGENTI "     to   w-cnt-tit-des-tit      .
           move      "DT"                 to   s-ope                  .
           call      "swd/mod/prg/obj/msegrt"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      001                  to   p-pos                  .
           move      "Mnemonico     Codice        Nominativo            
      -              "            Indirizzo                             
      -              "     Localita'               "
                                          to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
