      ...
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Cancellazione conteggi e maturazioni prov-  *
      *                    vigionali                                   *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  i-ide-des                  pic  x(40) value
                     "  CANCELLAZIONE CONTEGGI E MATURAZIONI  "       .
      ...
      *    *-----------------------------------------------------------*
       pmt-not-ope-000.
           ...
           move      05                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Vengono cancellati tutti i conteggi provvigionali 
      -              "e tutte le maturazioni rela-  "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
           ...
           move      06                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "tivi alle fatture con data compresa tra la data mi
      -              "nima e massima indicate.      "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
           ...
           move      08                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "Se si indica un codice agente saranno selezionate 
      -              "solo le fatture di quell'a-   "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      *
           ...
           move      09                   to   v-lin                  .
           move      01                   to   v-pos                  .
           move      "gente, altrimenti tutte le fatture comprese tra le
      -              " due date.                    "
                                          to   v-alf                  .
           call      "swd/mod/prg/obj/mvideo"
                                         using v                      .
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-210.
      *              *-------------------------------------------------*
      *              * Se il codice agente non corrisponde a quello    *
      *              * impostato                                       * 
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test                                        *
      ...
      *              *-------------------------------------------------*
      *              * Se la fattura relativa al protocollo ha una da- *
      *              * ta documento superiore alla data indicata       *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        rf-fit-dat-doc       not  > rr-dat-max
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-cnt-prn-flg-int      .
      *              *-------------------------------------------------*
      *              * Ciclo di pulizia del flag di elaborazione       *
      *              * agenti per le testate fattura                   *
      *              *-------------------------------------------------*
           perform   prn-fin-cic-ptf-000  thru prn-fin-cic-ptf-999    .
       prn-fin-cic-999.
           exit.
      ...
      *    * Ciclo Report Program : Esecuzione per fine ciclo          *
      *    *                                                           *
      *    * Ciclo di pulizia del flag di elaborazione agenti per le   *
      *    * testate fattura                                           *
      *    *-----------------------------------------------------------*
       prn-fin-cic-ptf-000.
      *              *-------------------------------------------------*
      *              * Ciclo di scansione [fit]                        *
      *              *-------------------------------------------------*
       prn-fin-cic-ptf-100.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      s-pat               using f
                                               rf-fit                 .
      *                  *---------------------------------------------*
      *                  * Se Start errata : fine ciclo interno        *
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-fin-cic-ptf-600.
       prn-fin-cic-ptf-200.
      *                  *---------------------------------------------*
      *                  * Get next su [fit]                           *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to prn-fin-cic-ptf-600.
      *                  *---------------------------------------------*
      *                  * Test max su [fit] per data documento        *
      *                  *---------------------------------------------*
           if        rf-fit-dat-doc       >    rr-dat-max
                     go to prn-fin-cic-ptf-600.
       prn-fin-cic-ptf-300.
      *                  *---------------------------------------------*
      *                  * Selezione sul codice agente                 *
      *                  *---------------------------------------------*
           if        rr-cod-age           =    zero
                     go to prn-fin-cic-ptf-320.
           if        rf-fit-cod-age       =    rr-cod-age
                     go to prn-fin-cic-ptf-320.
      *                      *-----------------------------------------*
      *                      * A release e riciclo                     *
      *                      *-----------------------------------------*
           go to     prn-fin-cic-ptf-500.
       prn-fin-cic-ptf-320.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-300.
      *              *-------------------------------------------------*
      *              * Eliminazione di tutte le maturazioni relative   *
      *              * al conteggio                                    *
      *              *-------------------------------------------------*
       prn-liv-det-310.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       prn-liv-det-350.
      *                  *---------------------------------------------*
      *                  * Get next su [gpm] per numero conteggio      *
      *                  *---------------------------------------------*
           move      "GN"                 to   f-ope                  .
           move      "pgm/age/fls/ioc/obj/iofgpm"
                                          to   s-pat                  .
      ...
      *              *-------------------------------------------------*
           move      "CANCELLAZIONE CONTEGGI E MATURAZIONI"
