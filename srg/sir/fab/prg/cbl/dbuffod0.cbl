      ...
      *                                                                *
      *                         SU MISURA PER SIRI - Vedi label 'SIRI' *
      *                                                                *
      * ============================================================== *
      *                                                                *
      * NOTE OPERATIVE                                                 *
      ...
      *    *-----------------------------------------------------------*
       01  w-rip-str.
      *        *-------------------------------------------------------*
      *        * Work-area per ripristino Start su [ocr]               *
      *        *-------------------------------------------------------*
           05  w-rip-str-ocr.
      *            *---------------------------------------------------*
      *            * Flag di End-of-file                               *
      ...
      *            *---------------------------------------------------*
               ...
               10  w-rip-str-ocr-rch      pic  x(01)                  .
               10  w-rip-str-ocr-mag      pic  9(02)                  .
      ...
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Data consegna richiesta (fornitori) *
      *                          *                                     *
      *                          * SIRI (Norina - 11/05/10)            *
      *                          *                                     *
      *                          * Contromodifica (Norina - 12/05/10)  *
      *                          *-------------------------------------*
           move      rf-ofr-dcn-ric       to   d-buf-fod-dcn-ric
                                              (d-buf-fod-num-ele)     .
______*    move      rf-ofr-dcn-prv       to   d-buf-fod-dcn-ric
______*                                       (d-buf-fod-num-ele)     .
      *                          *-------------------------------------*
      *                          * Quantita' di impegno/copertura      *
      *                          *-------------------------------------*
      ...
      *                  * SIRI                                        *
      *                  *---------------------------------------------*
           if        rf-ocr-cod-dpz       =    d-buf-fod-cod-dpz and
                     rf-ocr-flg-rch       =    spaces            and
                     rf-ocr-tip-mag       =    d-buf-fod-tip-mag and
                     rf-ocr-num-pro       =    d-buf-fod-num-mag
                     go to crz-buf-fod-525.
      *                      *-----------------------------------------*
      *                      * Flag di End-of-file                     *
      *                      *-----------------------------------------*
           move      "#"                  to   w-rip-str-ocr-eof      .
      ...
      *                          * SIRI (Norina - 11/05/10)            *
      *                          *                                     *
      *                          * INIBITO (Norina - 28/05/10)         *
      *                          *-------------------------------------*
           go to     crz-buf-fod-536.
      *                              *---------------------------------*
      *                              * Determinazione se riga ordine   *
      *                              * proveniente da 'forecast'       *
      *                              *---------------------------------*
           move      rf-ocr-num-prt       to   w-det-rel-frc-prt      .
           move      rf-ocr-num-prg       to   w-det-rel-frc-prg      .
           perform   det-rel-frc-000      thru det-rel-frc-999        .
           if        w-det-rel-frc-ctr    =    zero
                     go to crz-buf-fod-536.
           move      w-det-rel-frc-dat    to   d-buf-fod-dat-doc
                                              (d-buf-fod-num-ele)     .
           go to     crz-buf-fod-537.
       crz-buf-fod-536.
      ...
      *                          * SIRI (Norina - 11/05/10)            *
      *                          *                                     *
      *                          * Contromodifica (Norina - 22/06/10)  *
      *                          *-------------------------------------*
           move      rf-ocr-dcn-ric       to   d-buf-fod-dcn-ric
                                              (d-buf-fod-num-ele)     .
______*    move      rf-ocr-dcn-prv       to   d-buf-fod-dcn-ric
______*                                       (d-buf-fod-num-ele)     .
      *                          *-------------------------------------*
      *                          * Quantita' di impegno/copertura      *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           if        rf-ocr-flg-cnf       =    "B"
                     move  rf-ocr-flg-cnf to   d-buf-fod-fds-dcr
                                              (d-buf-fod-num-ele)
                     move  spaces         to   d-buf-fod-flg-cnf
                                              (d-buf-fod-num-ele)
           else      move  rf-ocr-tip-ord to   d-buf-fod-fds-dcr
                                              (d-buf-fod-num-ele)
                     move  rf-ocr-flg-cnf to   d-buf-fod-flg-cnf
                                              (d-buf-fod-num-ele)     .
      *
______*    move      rf-ocr-tip-ord       to   d-buf-fod-fds-dcr
______*                                       (d-buf-fod-num-ele)     .
      *                          *-------------------------------------*
      *                          * Flag di conferma riga ordine        *
      *                          *-------------------------------------*
______*    move      rf-ocr-flg-cnf       to   d-buf-fod-flg-cnf
______*                                       (d-buf-fod-num-ele)     .
      *                          *-------------------------------------*
      *                          * Numero protocollo ordine            *
      *                          *-------------------------------------*
...
______*    if        d-buf-fod-dcn-prv
______*             (w-crz-buf-fod-c02)   <    rf-ocr-dcn-prv
______*              go to crz-buf-fod-540.
      ...
      *                  *---------------------------------------------*
      *                  * Normalizzazione data documento              *
      *                  *---------------------------------------------*
           move      zero                 to   w-det-rel-frc-dat      .
       det-rel-frc-100.
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-rip-str-ocr-eof    not  = spaces
                     go to det-rel-frc-930.
           move      rf-ocr-cod-dpz       to   w-rip-str-ocr-dpz      .
           move      rf-ocr-flg-rch       to   w-rip-str-ocr-rch      .
           move      rf-ocr-tip-mag       to   w-rip-str-ocr-mag      .
      ...
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      rf-ocf-frc-prt       to   rf-ocr-num-prt         .
           move      rf-ocf-frc-prg       to   rf-ocr-num-prg         .
           move      "pgm/orc/fls/ioc/obj/iofocr"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  * Data documento                              *
      *                  *---------------------------------------------*
           move      rf-ocr-dat-doc       to   w-det-rel-frc-dat      .
      *                  *---------------------------------------------*
      *                  * Ripristino della start                      *
      *                  *---------------------------------------------*
           if        w-rip-str-ocr-eof    not  = spaces
                     go to det-rel-frc-990.
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "RCHMAG    "         to   f-key                  .
           move      w-rip-str-ocr-dpz    to   rf-ocr-cod-dpz         .
           move      w-rip-str-ocr-rch    to   rf-ocr-flg-rch         .
           move      w-rip-str-ocr-mag    to   rf-ocr-tip-mag         .
