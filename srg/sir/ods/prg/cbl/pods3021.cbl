      ...
      *                       Ultima revisione:    NdK del 25/09/24    *
      *                    ------------------------------------------- *
      *                                 Autore:    Nicola de Kunovich  *
      *================================================================*
      *                                                                *
      * Descrizione pgm:   Esecuzione stampa per il programma ods302   *
      *                                                                *
      *                    Stampa brogliaccio ordini di spedizione     *
      *                                                                *
      *                    SU MISURA PER SIRI                          *
      *                                                                *
      *                    Solo per scansione file [hbc] per lettura   *
      *                    dati barcode - rintracciabilita'            *
      *                                                                *
      *================================================================*
      ...
      *        *-------------------------------------------------------*
           05  rr-cod-tos                 pic  x(05)                  .
      *        *-------------------------------------------------------*
      *        * Tipo movimento da ricercare, descrizione              *
      *        *-------------------------------------------------------*
           05  rr-cod-tos-des             pic  x(30)                  .
      *        *-------------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           ...
           move      rr-dat-ini           to   rf-ost-dat-doc         .
      ...
      *                  *---------------------------------------------*
       ...
       stp-srt-inp-020.
      *              *-------------------------------------------------*
      *              * Start su archivio [ost] per data ultima modifi- *
      *              * ca                                              *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
           move      "NL"                 to   f-cfr                  .
           move      "DATSYS    "         to   f-key                  .
           move      rr-dat-ini           to   rf-ost-ide-dat         .
      ...
      *                      *-----------------------------------------*
           ...
           if        rf-ost-ide-ute       not  = rr-cod-ute
                     go to stp-srt-inp-100.
       stp-srt-inp-330.
      *                      *-----------------------------------------*
      *                      * Test su codice fase                     *
      *                      *-----------------------------------------*
           ...
           if        rf-ost-ide-fas       not  = rr-cod-fas
                     go to stp-srt-inp-100.
       stp-srt-inp-340.
      *                      *-----------------------------------------*
      *                      * Test su codice tipo movimento           *
      *                      *-----------------------------------------*
           if        rr-cod-tos           =    spaces
                     go to stp-srt-inp-500.
           if        rf-ost-cod-tms       not  = rr-cod-tos
                     go to stp-srt-inp-100.
       stp-srt-inp-500.
      *                  *---------------------------------------------*
      ...
      *                          * Numero documento                    *
      *                          *-------------------------------------*
           move      rf-ost-num-prt       to   w-det-num-doc          .
      ...
      *                          * Tipo documento                      *
      *                          *-------------------------------------*
           move      rf-ost-cod-tms       to   srt-tip-doc            .
      *                          *-------------------------------------*
      *                          * Tipo archivio                       *
      *                          *-------------------------------------*
           move      rf-ost-tip-arc       to   srt-tip-arc            .
      *                          *-------------------------------------*
      *                          * Codice archivio                     *
      *                          *-------------------------------------*
           move      rf-ost-cod-arc       to   srt-cod-arc            .
      *                          *-------------------------------------*
      *                          * Codice dipendenza archivio          *
      *                          *-------------------------------------*
           move      rf-ost-dpz-arc       to   srt-dpz-arc            .
      *                          *-------------------------------------*
      *                          * Sigla valuta documento              *
      *                          *-------------------------------------*
           move      rf-ost-sgl-vpf       to   srt-sgl-vpf            .
      *                          *-------------------------------------*
      *                          * Decimali valuta documento           *
      *                          *-------------------------------------*
           move      rf-ost-dec-vpf       to   srt-dec-vpf            .
      *                          *-------------------------------------*
      ...
      *                          * Data ultima modifica                *
      *                          *-------------------------------------*
           move      rf-ost-ide-dat       to   srt-ide-dat            .
      *                          *-------------------------------------*
      *                          * Utente                              *
      *                          *-------------------------------------*
           move      rf-ost-ide-ute       to   srt-ide-ute            .
      *                          *-------------------------------------*
      *                          * Fase                                *
      *                          *-------------------------------------*
           move      rf-ost-ide-fas       to   srt-ide-fas            .
       stp-srt-inp-800.
      *                  *---------------------------------------------*
      ...
      *                      *-----------------------------------------*
       ...
       prn-liv-det-800.
      *                  *---------------------------------------------*
      *                  * Eventuale stampa letture barcode            *
      *                  *                                             *
      *                  * SIRI                                        *
      *                  *---------------------------------------------*
           perform   prn-liv-det-hbc-000  thru prn-liv-det-hbc-999    .
       prn-liv-det-900.
      *              *-------------------------------------------------*
      *              * Interlinea                                      *
      *              *-------------------------------------------------*
      ...
      *    * Ciclo Report Program : Livello di dettaglio               *
      *    *                                                           *
      *    * Subroutine di stampa eventuali letture barcode            *
      *    *                                                           *
      *    * SIRI                                                      *
      *    *-----------------------------------------------------------*
       prn-liv-det-hbc-000.
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      srt-num-prt          to   rf-hbc-num-prt         .
      ...
      *              *-------------------------------------------------*
           if        rf-hbc-num-prt       not  = srt-num-prt
                     go to prn-liv-det-hbc-900.
       prn-liv-det-hbc-400.
      *              *-------------------------------------------------*
      *              * Selezioni                                       *
      ...
      *              *-------------------------------------------------*
           ...
           move      rf-hbc-num-pro       to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        p-res                >    2
                     go to prn-liv-det-hbc-520.
      *                  *---------------------------------------------*
      *                  * Reintestazione                              *
      *                  *---------------------------------------------*
           perform   int-pag-sta-000      thru int-pag-sta-999        .
           if        w-cnt-prn-flg-int    not  = spaces
                     go to prn-liv-det-hbc-999.
      *                  *---------------------------------------------*
      *                  * Fincatura per dettaglio                     *
      *                  *---------------------------------------------*
           perform   fin-liv-det-000      thru fin-liv-det-999        .
      ...
      *                      *-----------------------------------------*
           ...
           move      p-lnr                to   p-lin                  .
           move      027                  to   p-pos                  .
           move      rf-hbc-dat-bcd (01)  to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      *                      * Quantita' lettura                       *
      ...
      *              *-------------------------------------------------*
           move      "BROGLIACCIO ORDINI DI SPEDIZIONE"
