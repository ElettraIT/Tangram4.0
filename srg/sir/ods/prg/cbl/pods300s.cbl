      ...
      *                                   Fase:    ods300              *
      *                    ------------------------------------------- *
      *                     Versione originale:    001 del 07/04/97    *
      *                       Ultima revisione:    NdK del 19/09/24    *
      *                    ------------------------------------------- *
      ...
      * -------------------------------------------------------------- *
      *                                                                *
      *                   (Agg. 12/06/24 - Riccardo - SOPPRESSA la     *
      *                    stampa con barcode)                         *
      *                                                                *
      * Note operative :   il programma scrive un file 'CSV' in        *
      *                    '/abd/spl/' che viene successivamente       *
      *                    scansionato per la stampa dei barcode da    *
      *                    't_sir_ods_stp' e poi utilizzato da         *
      *                    't_sir_ods_wrt' (attualmente inibito) per   *
      *                    costruire la tabella nel database MySQL per *
      *                    le letture con i terminalini via Web        *
      *                                                                *
      *                    Attualmente solo per i Clienti:             *
      *                                                                *
      *                   (Agg. 19/06/23 - Riccardo)                   *
      *                    - 0001002 ELKAB 2 Srl                       *
      *                    - 0001005 EAS SPA                           *
      *                    - 0001514 EFFEGI ELETTRONICA Srl            *
      *                    - 0001718 POWERSOFT SPA                     *
      *                    - 0001888 JOHNSON ELECTRIC ASTI SRL         *
      *                    - 0001893 CEBI MICROMOTORS                  *
      *                    - 0002144 CAREL INDUSTRIES SPA              *
      *                    - 0002638 TEXA S.P.A.                       *
      *                    - 0005033 CAREL ADRIATIC D.O.O.             *
      *                    - 0005049 ODELO SLOVENIJA D.O.O.            *
      *                    - 0005053 L-TEK ELEKTRONIKA                 *
      *                    - 0005056 CAREL ELECTRONIC (SUZHOU) CO, LTD *
      *                    - 0005060 WAGNER KABLO A.S.                 *
      *                    - 0005061 MELECS EWS GMBH                   *
      *                                                                *
      *                    NOTA BENE:                                  *
      *                                                                *
      *                    Il programma genera al volo un file EPS     *
      *                    relativo al numero protocollo, tramite il   *
      *                    modulo 'dbarcod0' di 'dcp' nel formato      *
      *                    AANNNN.eps che viene memorizzato con lo     *
      *                    stesso criterio dei barcode prodotti.       *
      *                    Senza esclusioni, per tutti i protocolli    *
      *                                                                *
      *================================================================*
      ...
      *    *-----------------------------------------------------------*
       01  w-lpc.
      *        *-------------------------------------------------------*
      *        * Linee per fincatura corpo                             *
      *        *-------------------------------------------------------*
           05  w-lpc-pri-rig-lin          pic  9(03) value  024       .
           05  w-lpc-ult-rig-lin          pic  9(03) value  042       .
      ...
      *                * Nota 1                                        *
      *                *-----------------------------------------------*
                   15  w-stp-tes-not-g01  pic  x(40)                  .
      *                *-----------------------------------------------*
      *                * Nota 2                                        *
      *                *-----------------------------------------------*
                   15  w-stp-tes-not-g02  pic  x(40)                  .
      *                *-----------------------------------------------*
      *                * Nota 3                                        *
      *                *-----------------------------------------------*
                   15  w-stp-tes-not-g03  pic  x(40)                  .
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
      *            * Sub-work per Partita Iva                          *
      *            *---------------------------------------------------*
               10  w-stp-tes-piv.
      *                *-----------------------------------------------*
      *                * Partita Iva o codice fiscale                  *
      *                *-----------------------------------------------*
                   15  w-stp-tes-piv-piv  pic  x(16)                  .
      *            *---------------------------------------------------*
      ...
      *            *---------------------------------------------------*
               10  w-stp-tes-nri.
      *                *-----------------------------------------------*
      *                * Castelletto giorni ed orario di apertura ed   *
      *                * altre annotazioni relative all'intestatario   *
      *                *-----------------------------------------------*
      ...
      *        *-------------------------------------------------------*
           05  w-stp-rig.
      *            *---------------------------------------------------*
      *            * Numero riga corpo in corso di stampa relativo     *
      *            *---------------------------------------------------*
               10  w-stp-rig-ctr-rcs      pic  9(05)                  .
      *            *---------------------------------------------------*
      *            * Numero riga corpo in corso di stampa assoluto     *
      *            *---------------------------------------------------*
               10  w-stp-rig-ctr-rca      pic  9(05)                  .
      *            *---------------------------------------------------*
      ...
      *                *-----------------------------------------------*
                   15  w-stp-rig-des-der.
                       20  w-stp-rig-des-rgd occurs 10
                                          pic  x(40)                  .
      *            *---------------------------------------------------*
      *            * Comodo per rottura su chiave alfanumerica di or-  *
      *            * dinamento                                         *
      *            *---------------------------------------------------*
               10  w-stp-rig-pro-rot      pic  x(54)                  .
      *            *---------------------------------------------------*
      *            * Comodo per stampa quantita' per rottura           *
      *            *---------------------------------------------------*
               10  w-stp-rig-qta-rot      pic s9(08)v9(03)            .
      ...
      *    *-----------------------------------------------------------*
       01  w-cmp.
      *        *-------------------------------------------------------*
      *        * Work per compattamento righe annotazioni cliente      *
      *        *-------------------------------------------------------*
           05  w-cmp-nri-goa.
               10  w-cmp-nri-goa-nri      pic  9(02)                  .
               10  w-cmp-nri-goa-nrd      pic  9(02)                  .
               10  w-cmp-nri-goa-ctr      pic  9(02)                  .
      ...
      *        *-------------------------------------------------------*
      *        * Numero ordini di spedizione aperti oltre a quello ri- *
      *        * chiamato                                              *
      *        *-------------------------------------------------------*
           05  w-ctl-ods-ape-ctr          pic  9(05)                  .
      *        *-------------------------------------------------------*
      ...
      *        * Comodi per messaggi                                   *
      *        *-------------------------------------------------------*
           05  w-ctl-ods-ape-ms1          pic  x(40)                  .
           05  w-ctl-ods-ape-ms2          pic  x(40)                  .
      ...
      *        *-------------------------------------------------------*
           05  w-det-des-dcp.
               10  w-det-des-dcp-pro      pic  9(07)                  .
               10  w-det-des-dcp-des      pic  x(40)                  .
      *        *-------------------------------------------------------*
      ...
      *                        *---------------------------------------*
                           25  w-rig-key-cod-04
                                          pic  x(14)                  .
      *                    *-------------------------------------------*
      *                    * Ridefinizione di tipo '99' - Carel        *
      *                    *-------------------------------------------*
                       20  w-rig-key-alf-99  redefines
                           w-rig-key-alf.
      *                        *---------------------------------------*
      *                        * Codice prodotto del cliente           *
      *                        *---------------------------------------*
                           25  w-rig-key-cod-99
                                          pic  x(14)                  .
      *                        *---------------------------------------*
      ...
      *                    * Numero riga                               *
      *                    *-------------------------------------------*
                       20  w-rig-dti-nur  pic  9(03)       comp-3     .
      *                    *-------------------------------------------*
      *                    * Numero progressivo riga                   *
      *                    *-------------------------------------------*
                       20  w-rig-dti-prg  pic  9(05)       comp-3     .
      *                    *-------------------------------------------*
      *                    * Quantita' cumulata per la riga            *
      *                    *-------------------------------------------*
                       20  w-rig-dti-qcr  pic s9(08)v9(03) comp-3     .
      *        *-------------------------------------------------------*
      *        * Comodo per salvataggio chiave di ordinamento          *
      *        *-------------------------------------------------------*
           05  w-rig-sav-key              pic  x(54)                  .
      *        *-------------------------------------------------------*
      *        * Comodo per rottura su chiave di ordinamento           *
      *        *-------------------------------------------------------*
           05  w-rig-rot-key              pic  x(54)                  .
      *        *-------------------------------------------------------*
      *        * Comodo per quantita' cumulata per rottura             *
      *        *-------------------------------------------------------*
           05  w-rig-rot-qta              pic s9(08)v9(03)            .
      *        *-------------------------------------------------------*
      *        * Comodo per indice elemento di rottura                 *
      *        *-------------------------------------------------------*
           05  w-rig-rot-inx              pic  9(03)                  .
      ...
      *        * Numero massimo di elementi                            *
      *        *-------------------------------------------------------*
           05  w-rcm-max-ele              pic  9(05)     value 999    .
      *        *-------------------------------------------------------*
      *        * Contatore elementi                                    *
      *        *-------------------------------------------------------*
           05  w-rcm-num-ele              pic  9(05)                  .
      *        *-------------------------------------------------------*
      *        * Contatori di comodo                                   *
      *        *-------------------------------------------------------*
           05  w-rcm-ctr-ele              pic  9(05)                  .
      *        *-------------------------------------------------------*
      *        * Castelletto progressivi riga salvati                  *
      *        *-------------------------------------------------------*
           05  w-rcm-cst-rig.
      ...
      *                    * Numero progressivo riga                   *
      *                    *-------------------------------------------*
                       20  w-rcm-dti-prg  pic  9(05)       comp-3     .
      *                    *-------------------------------------------*
      *                    * Numero progressivo nell'ambito dell'ordi- *
      *                    * ne                                        *
      *                    *-------------------------------------------*
                       20  w-rcm-dti-pao  pic  9(05)       comp-3     .
      ...
      *            *---------------------------------------------------*
               ...
               10  w-out-tes-sep-001      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Ragione sociale cliente                           *
      *            *---------------------------------------------------*
               10  w-out-tes-rag-soc      pic  x(40)                  .
               10  w-out-tes-sep-002      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Carattere di fine riga                            *
      *            *---------------------------------------------------*
               10  w-out-tes-fin-rig      pic  x(01)                  .
      *        *-------------------------------------------------------*
      *        * Output riga                                           *
      *        *-------------------------------------------------------*
           05  w-out-rig.
      ...
      *            *---------------------------------------------------*
               ...
               10  w-out-rig-sep-001      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Progressivo riga                                1 *
      *            *---------------------------------------------------*
               10  w-out-rig-num-prg      pic  x(06)                  .
               10  w-out-rig-sep-002      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Codice numerico prodotto                        2 *
      *            *---------------------------------------------------*
               10  w-out-rig-num-pro      pic  x(14)                  .
               10  w-out-rig-sep-003      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Codice alfanumerico prodotto                    3 *
      *            *---------------------------------------------------*
               10  w-out-rig-alf-pro      pic  x(14)                  .
               10  w-out-rig-sep-004      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Codice prodotto per la casa produttrice         4 *
      *            *---------------------------------------------------*
               10  w-out-rig-cdp-pdt      pic  x(30)                  .
               10  w-out-rig-sep-005      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Descrizione prodotto                            5 *
      *            *---------------------------------------------------*
               10  w-out-rig-des-mag      pic  x(40)                  .
               10  w-out-rig-sep-006      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Quantita' prodotto                              6 *
      *            *---------------------------------------------------*
               10  w-out-rig-qta-ven      pic  x(12)                  .
               10  w-out-rig-sep-007      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Lotto prodotto                                  7 *
      *            *---------------------------------------------------*
               10  w-out-rig-qta-lot      pic  x(12)                  .
               10  w-out-rig-sep-008      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Giacenza prodotto                               8 *
      *            *---------------------------------------------------*
               10  w-out-rig-qta-gia      pic  x(12)                  .
               10  w-out-rig-sep-009      pic  x(01)                  .
      *            *---------------------------------------------------*
      *            * Carattere di fine riga                            *
      *            *---------------------------------------------------*
               10  w-out-rig-fin-rig      pic  x(01)                  .
      ...
      *                  *---------------------------------------------*
           if        w-prs-sos-bl1        not  = 11
                     move  00             to   w-prs-sos-bl1          .
           if        w-prs-sos-bl2        not  = 12
                     move  00             to   w-prs-sos-bl2          .
       prs-stp-ods-750.
      *                  *---------------------------------------------*
      *                  * Tipo di ordinamento righe corpo             *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Barcode relativo al numero protocollo CODE128   *
      *              *                                                 *
      *              * SIRI                                            *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           if        w-det-azi-lux-snx    not  = spaces
                     go to rou-stp-doc-010.
      *                  *---------------------------------------------*
      *                  * Preparazione della stringa: 99.9999         *
      *                  *---------------------------------------------*
           move      06                   to   w-all-str-lun          .
           move      02                   to   w-all-str-num          .
           move      w-inp-mst-prt
                    (02 : 02)             to   w-all-str-cat (1)      .
           move      w-inp-mst-prt
                    (08 : 04)             to   w-all-str-cat (2)      .
      ...
      *                  *---------------------------------------------*
           ...
           move      w-all-str-alf        to   d-bar-cod-alf-cod      .
      ...
      *              *-------------------------------------------------*
           move      rf-ost-cod-dpz       to   w-ctl-ods-ape-dpz      .
           move      rf-ost-num-prt       to   w-ctl-ods-ape-prt      .
      ...
      *              *-------------------------------------------------*
      *              * Rilettura record testata da stampare            *
      *              *-------------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRT    "         to   f-key                  .
           move      w-inp-mst-prt        to   rf-ost-num-prt         .
      ...
      *                  *---------------------------------------------*
       ...
       rou-stp-doc-950.
      *              *-------------------------------------------------*
      *              * Close file sequenziale ed esecuzione script     *
      *              *                                                 *
      *              * SIRI                                            *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       rou-stp-doc-960.
      *                  *---------------------------------------------*
      *                  * Test se stampa extra da eseguire            *
      *                  *                                             *
      *                  * (Agg. 01/09/21 - Riccardo)                  *
      *                  *---------------------------------------------*
           if        rf-ost-cod-arc       not  = 0001002 and
                     rf-ost-cod-arc       not  = 0001005 and
                     rf-ost-cod-arc       not  = 0001514 and
                     rf-ost-cod-arc       not  = 0001718 and
                     rf-ost-cod-arc       not  = 0001888 and
                     rf-ost-cod-arc       not  = 0001893 and
                     rf-ost-cod-arc       not  = 0002144 and
                     rf-ost-cod-arc       not  = 0002638 and
                     rf-ost-cod-arc       not  = 0005033 and
                     rf-ost-cod-arc       not  = 0005049 and
                     rf-ost-cod-arc       not  = 0005053 and
                     rf-ost-cod-arc       not  = 0005056 and
                     rf-ost-cod-arc       not  = 0005060 and
                     rf-ost-cod-arc       not  = 0005061
                     go to rou-stp-doc-980.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           else if   rf-ost-tra-cur       =    10
                     move  "Mittente            "
                                          to   p-alf
           else if   rf-ost-tra-cur       =    20
                     move  "Destinatario        "
                                          to   p-alf
           else if   rf-ost-tra-cur       =    30
                     move  "Vettore             "
                                          to   p-alf
      ...
      *                      *-----------------------------------------*
           ...
           move      19                   to   p-lin                  .
           move      074                  to   p-pos                  .
           move      w-stp-tes-vet-abn    to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
      *                      *-----------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       stp-tes-doc-880.
      *              *-------------------------------------------------*
      *              * Apertura sequenziale                            *
      *              *                                                 *
      *              * SIRI                                            *
      *              *-------------------------------------------------*
           perform   stp-tes-doc-seq-000  thru stp-tes-doc-seq-999    .
       stp-tes-doc-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *    * Routine di stampa testata documento                       *
      *    *                                                           *
      *    * Emissione sequenziale - apertura                          *
      *    *                                                           *
      *    * SIRI                                                      *
      *    *-----------------------------------------------------------*
       stp-tes-doc-seq-000.
      ...
      *                  * Ragione sociale cliente                     *
      *                  *---------------------------------------------*
           move      w-stp-tes-int-rag    to   w-out-tes-rag-soc      .
      *                  *---------------------------------------------*
      *                  * Carattere di fine riga                      *
      *                  *---------------------------------------------*
           move      w-det-fin-rig-chr    to   w-out-tes-fin-rig      .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Normalizzazione numero abbonamento per vettore  *
      *              *-------------------------------------------------*
           move      spaces               to   w-stp-tes-vet-abn      .
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                  * Lettura record [dcc] per la sede principale *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Normalizzazione note                    *
      *                      *-----------------------------------------*
           move      spaces               to   w-stp-tes-not-g01      .
           move      spaces               to   w-stp-tes-not-g02      .
           move      spaces               to   w-stp-tes-not-g03      .
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           call      s-pat               using f
                                               rf-dcc                 .
      *                      *-----------------------------------------*
      *                      * Lettura record [dcc] per la sede        *
      *                      *-----------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCLI    "         to   f-key                  .
           ...
           call      s-pat               using f
                                               rf-dcc                 .
      *                          *-------------------------------------*
      *                          * Se record non trovato : note a spa- *
      *                          * ces                                 *
      *                          *-------------------------------------*
           if        f-sts                not  = e-not-err
      ...
      *                      *-----------------------------------------*
      *                      * Bufferizzazione note cliente principale *
      *                      *-----------------------------------------*
           move      rf-dcc-not-g01       to   w-stp-tes-not-g01      .
           move      rf-dcc-not-g02       to   w-stp-tes-not-g02      .
           move      rf-dcc-not-g03       to   w-stp-tes-not-g03      .
       stp-tes-doc-cli-050.
       stp-tes-doc-cli-100.
      *              *-------------------------------------------------*
      *              * Preparazioni                                    *
      ...
      *                      *-----------------------------------------*
      *                      * Test se anagrafica dipendenza           *
      *                      *-----------------------------------------*
           if        rf-ost-dpz-arc       =    spaces
                     go to stp-tes-doc-cli-150.
      *                      *-----------------------------------------*
      *                      * Normalizzazione record [dcc]            *
      *                      *-----------------------------------------*
           move      "NO"                 to   f-ope                  .
      ...
      *                      *-----------------------------------------*
      *                          *-------------------------------------*
      *                          * Numero abbonamento per vettore      *
      *                          *                                     *
      *                          * SIRI                                *
      *                          *-------------------------------------*
           move      rf-dcc-abn-vtt       to   w-stp-tes-vet-abn      .
      *                          *-------------------------------------*
      *                          * A condizioni di pagamento           *
      *                          *-------------------------------------*
           go to     stp-tes-doc-cli-200.
      ...
      *                      *-----------------------------------------*
       ...
       stp-tes-doc-cli-330.
      *                      *-----------------------------------------*
      *                      * Test sulle note sin qui cumulate        *
      *                      *-----------------------------------------*
           if        w-stp-tes-nri        =    spaces
                     move  zero           to   w-cmp-nri-goa-nrd
                     go to stp-tes-doc-cli-335.
      *                      *-----------------------------------------*
      *                      * Compattamento verso l'alto delle righe  *
      *                      * di annotazioni cliente                  *
      *                      *-----------------------------------------*
           perform   cmp-nri-goa-000      thru cmp-nri-goa-999        .
       stp-tes-doc-cli-335.
      *                      *-----------------------------------------*
      *                      * A partire dalla prima riga libera si    *
      *                      * aggiungono le righe di annotazione pre- *                  *
      *                      * senti in anagrafica commerciale cliente *                  *
      *                      *-----------------------------------------*
           move      zero                 to   w-cmp-nri-goa-ctr      .
       stp-tes-doc-cli-340.
           add       1                    to   w-cmp-nri-goa-ctr      .
           add       1                    to   w-cmp-nri-goa-nrd      .
           if        w-cmp-nri-goa-ctr    >    3
                     go to stp-tes-doc-cli-345.
           if        w-cmp-nri-goa-nrd    >    6
                     go to stp-tes-doc-cli-345.
           if        w-cmp-nri-goa-ctr    =    1
                     move  w-stp-tes-not-g01
                                          to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)
           else if   w-cmp-nri-goa-ctr    =    2
                     move  w-stp-tes-not-g02
                                          to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)
           else if   w-cmp-nri-goa-ctr    =    3
                     move  w-stp-tes-not-g03
                                          to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)
           else      move  spaces         to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)     .
           go to     stp-tes-doc-cli-340.
       stp-tes-doc-cli-345.
      *                      *-----------------------------------------*
      *                      * Test sulle note sin qui cumulate        *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
      *                      * A partire dalla prima riga libera si    *
      *                      * aggiunge l'eventuale messaggio di altre *
      *                      * liste aperte per il cliente             *
      *                      *-----------------------------------------*
           if        w-ctl-ods-ape-ms1    =    spaces and
                     w-ctl-ods-ape-ms2    =    spaces
                     go to stp-tes-doc-cli-350.
      *                      *-----------------------------------------*
      *                      * Incremento contatore numero righe se    *
      *                      * possibile                               *
      *                      *-----------------------------------------*
           if        w-cmp-nri-goa-nrd    not  < 6
                     go to stp-tes-doc-cli-350.
           add       1                    to   w-cmp-nri-goa-nrd      .
           move      w-ctl-ods-ape-ms1    to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)     .
           if        w-cmp-nri-goa-nrd    not  < 6
                     go to stp-tes-doc-cli-350.
           add       1                    to   w-cmp-nri-goa-nrd      .
           move      w-ctl-ods-ape-ms2    to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)     .
       stp-tes-doc-cli-350.
      *                  *---------------------------------------------*
      *                  * Annotazioni per la spedizione               *
      ...
      *              *-------------------------------------------------*
           ...
           move      zero                 to   w-stp-rig-ctr-rca      .
      *              *-------------------------------------------------*
      *              * Normalizzazione preliminare chiave alfanumeri-  *
      *              * ca per la rottura                               *
      *              *-------------------------------------------------*
           move      spaces               to   w-stp-rig-pro-rot      .
      *              *-------------------------------------------------*
      ...
      *              * Stampa riga corpo                               *
      *              *-------------------------------------------------*
           perform   stp-cor-doc-src-000  thru stp-cor-doc-src-999    .
      *              *-------------------------------------------------*
      *              * Sequenziale                                     *
      *              *                                                 *
      *              * SIRI                                            *
      *              *                                                 *
      *              * DISABILITATA (12/06/24)                         *
      *              *-------------------------------------------------*
______*    perform   stp-cor-doc-seq-000  thru stp-cor-doc-seq-999    .
       stp-cor-doc-800.
      *              *-------------------------------------------------*
      *              * A record successivo                             *
      *              *-------------------------------------------------*
       ...
       stp-cor-doc-900.
      *              *-------------------------------------------------*
      *              * Stampa eventuali righe di commento cumulate     *
      *              *-------------------------------------------------*
           perform   stp-cor-doc-rcc-000  thru stp-cor-doc-rcc-999    .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     stp-cor-doc-999.
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-out-rig-num-prg      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      rf-osr-alf-pro       to   w-out-rig-num-pro      .
      *                  *---------------------------------------------*
      *                  * Codice alfanumerico prodotto                *
      *                  *---------------------------------------------*
           move      rf-osr-alf-pro       to   w-out-rig-alf-pro      .
      *                  *---------------------------------------------*
      *                  * Codice prodotto per la casa produttrice     *
      *                  *---------------------------------------------*
           move      rf-aaq-cdp-pdt       to   w-out-rig-cdp-pdt      .
      *                  *---------------------------------------------*
      *                  * Descrizione prodotto                        *
      *                  *---------------------------------------------*
           move      rf-osr-des-rig       to   w-out-rig-des-mag      .
      *                  *---------------------------------------------*
      *                  * Quantita'                                   *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-out-rig-qta-ven      .
      *                  *---------------------------------------------*
      *                  * Lotto                                       *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           move      spaces               to   p-sgn                  .
           move      "<BGD"               to   p-edm                  .
           move      rf-dcp-lot-ven       to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-out-rig-qta-lot      .
      *                  *---------------------------------------------*
      *                  * Giacenza                                    *
      *                  *---------------------------------------------*
           move      "ED"                 to   p-ope                  .
           ...
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-edt                to   w-out-rig-qta-gia      .
      *                  *---------------------------------------------*
      *                  * Carattere di fine riga                      *
      *                  *---------------------------------------------*
           move      w-det-fin-rig-chr    to   w-out-rig-fin-rig      .
      *                  *---------------------------------------------*
      *                  * Scrittura record in output                  *
      *                  *---------------------------------------------*
      ...
      *    * Stampa corpo documento                                    *
      *    *                                                           *
      *    * Subroutine per stampa riga corpo con rottura su codice    *
      *    * prodotto                                                  *
      *    *-----------------------------------------------------------*
       stp-cor-doc-src-000.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test se elemento di rottura a spaces        *
      *                  *---------------------------------------------*
           if        w-stp-rig-pro-rot    =    spaces
                     go to stp-cor-doc-src-200.
      *                  *---------------------------------------------*
      *                  * Test effettivo di rottura                   *
      *                  *---------------------------------------------*
           if        w-rig-key-alf
                    (w-rig-ctr-ele)       =    w-stp-rig-pro-rot
                     go to stp-cor-doc-src-600.
       stp-cor-doc-src-200.
      *              *-------------------------------------------------*
      *              * Se elemento che provoca rottura                 *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           if        w-stp-rig-ctr-rca    =    zero
                     subtract 1           from w-stp-rig-ctr-rcs      .
           add       1                    to   w-stp-rig-ctr-rca      .
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * ATTUALMENTE INIBITA (Guido 19/09/2017)  *
      *                      *-----------------------------------------*
           go to     stp-cor-doc-src-280.
      *                      *-----------------------------------------*
      *                      * Stampa                                  *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           ...
           add       w-stp-rig-ctr-rcs    to   p-lin                  .
           move      058                  to   p-pos                  .
           move      w-let-arc-zp3-mne    to   p-alf                  .
           if        p-alf                =    spaces
      ...
      *                  *---------------------------------------------*
           ...
           move      "N"                  to   p-tip                  .
      *
           if        rf-dcp-lot-ven       >    999999 or
                     rf-dcp-lot-ven       <   -999999
                     move  07             to   p-car
      ...
      *                  *---------------------------------------------*
       ...
       stp-cor-doc-src-305.
      *                  *---------------------------------------------*
      *                  * Stampa quantita' cumulata                   *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * In campo di comodo                      *
      *                      *-----------------------------------------*
           move   w-rig-dti-qcr           
                 (w-rig-ctr-ele)          to   w-stp-rig-qta-rot      .
      *                      *-----------------------------------------*
      *                      * Stampa                                  *
      *                      *-----------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           if        w-stp-rig-qta-rot    >    999999 or
                     w-stp-rig-qta-rot    <   -999999
                     move  07             to   p-car
           else      move  06             to   p-car                  .
           move      zero                 to   p-dec                  .
           move      "S"                  to   p-sgn                  .
           ...
           add       w-stp-rig-ctr-rcs    to   p-lin                  .
           move      122                  to   p-pos                  .
           move      w-stp-rig-qta-rot    to   p-num                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
       stp-cor-doc-src-310.
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       stp-cor-doc-src-320.
      *              *-------------------------------------------------*
      *              * Eventuale riga aggiuntiva per Paese di origine  *
      *              *                                                 *
      *              * SIRI                                            *
      *              *-------------------------------------------------*
           perform   stp-cor-doc-rpo-000  thru stp-cor-doc-rpo-999    .
       stp-cor-doc-src-380.
      *                  *---------------------------------------------*
      *                  * Continuazione                               *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-src-620.
       stp-cor-doc-src-600.
      *              *-------------------------------------------------*
      *              * Se elemento che non provoca rottura             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Incremento numero riga corpo in corso di    *
      *                  * stampa                                      *
      ...
      *                  *---------------------------------------------*
           move      "PF"                 to   p-ope                  .
           move      "N"                  to   p-tip                  .
           if        rf-osr-qta-ven       >    999999 or
                     rf-osr-qta-ven       <   -999999
                     move  07             to   p-car
           else      move  06             to   p-car                  .
           move      zero                 to   p-dec                  .
           move      "S"                  to   p-sgn                  .
      ...
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-src-900.
       stp-cor-doc-src-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Aggiornamento elemento di rottura           *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Chiave alfanumerica rappresentata dal   *
      *                      * codice alfanumerico del prodotto        *
      *                      *-----------------------------------------*
           move      w-rig-key-alf
                    (w-rig-ctr-ele)       to   w-stp-rig-pro-rot      .
       stp-cor-doc-src-990.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     stp-cor-doc-src-999.
       stp-cor-doc-src-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *              * Normalizzazione contatore righe commento lette  *
      *              *-------------------------------------------------*
           move      zero                 to   w-rcm-num-ele          .
       stp-cor-doc-pcr-100.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       stp-cor-doc-pcr-400.
      *              *-------------------------------------------------*
      *              * Selezione su eventuali commenti codificati      *
      *              * (Guido - 27/10/22)                              *
      *              *-------------------------------------------------*
           if        rf-osr-tip-rig       =    "C22  " or
                     rf-osr-tip-rig       =    "C47  "
      ...
      *              * Test se riga di commento                        *
      *              *-------------------------------------------------*
           if        w-rdf-tip-rig-wtp    not  = "C"
                     go to stp-cor-doc-pcr-520.
      *              *-------------------------------------------------*
      *              * Subroutine di bufferizzazione righe di commento *
      *              *-------------------------------------------------*
           perform   stp-cor-doc-brc-000  thru stp-cor-doc-brc-999    .
       stp-cor-doc-pcr-520.
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *              * Cumulo della quantita' a rottura prodotto       *
      *              *-------------------------------------------------*
           perform   stp-cor-doc-cqr-000  thru stp-cor-doc-cqr-999    .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *    * Stampa corpo documento                                    *
      *    *                                                           *
      *    * Bufferizzazione righe di commento                         *
      *    *-----------------------------------------------------------*
       stp-cor-doc-brc-000.
      *              *-------------------------------------------------*
      *              * Incremento contatore                            *
      *              *-------------------------------------------------*
           add       1                    to   w-rcm-num-ele          .
      *              *-------------------------------------------------*
      *              * Test sul contatore                              *
      *              *-------------------------------------------------*
           if        w-rcm-num-ele        >    w-rcm-max-ele
                     go to stp-cor-doc-brc-900.
      *              *-------------------------------------------------*
      *              * Bufferizzazione progressivo riga                *
      *              *-------------------------------------------------*
           move      rf-osr-num-prg       to   w-rcm-dti-prg
                                              (w-rcm-num-ele)         .
      *              *-------------------------------------------------*
      *              * Bufferizzazione progressivo riga nell'ambito    *
      *              * dell'ordine                                     *
      *              *-------------------------------------------------*
           move      w-rig-num-ele        to   w-rcm-dti-pao
                                              (w-rcm-num-ele)         .
       stp-cor-doc-brc-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     stp-cor-doc-brc-999.
       stp-cor-doc-brc-999.
           exit.
      ...
      *    * Stampa corpo documento                                    *
      *    *                                                           *
      *    * Cumulo quantita' a rottura codice prodotto                *
      *    *-----------------------------------------------------------*
       stp-cor-doc-cqr-000.
      *              *-------------------------------------------------*
      *              * Normalizzazioni preliminari                     *
      *              *-------------------------------------------------*
           move      spaces               to   w-rig-flg-dpp          .
           move      spaces               to   w-rig-rot-key          .
           move      zero                 to   w-rig-rot-qta          .
           move      zero                 to   w-rig-rot-inx          .
       stp-cor-doc-cqr-010.
      *              *-------------------------------------------------*
      *              * Test se almeno due codici da trattare           *
      *              *-------------------------------------------------*
           if        w-rig-num-ele        <    2
                     go to stp-cor-doc-cqr-999.
       stp-cor-doc-cqr-050.
      *              *-------------------------------------------------*
      *              * Ciclo                                           *
      *              *-------------------------------------------------*
       ...
       stp-cor-doc-cqr-200.
           add       1                    to   w-rig-ctr-001          .
           if        w-rig-ctr-001        >    w-rig-num-ele
                     go to stp-cor-doc-cqr-900.
      *              *-------------------------------------------------*
      *              * Test se elemento con quantita' a zero           *
      *              *-------------------------------------------------*
           if        w-rig-dti-qcr
                    (w-rig-ctr-001)       =    zero
                     go to stp-cor-doc-cqr-200.
      *              *-------------------------------------------------*
      *              * Test se primo passaggio                         *
      *              *-------------------------------------------------*
           if        w-rig-flg-dpp        =    spaces
                     move  "#"            to   w-rig-flg-dpp
                     go to stp-cor-doc-cqr-680.
      *              *-------------------------------------------------*
      *              * Test per la rottura                             *
      *              *-------------------------------------------------*
           if        w-rig-key-alf
                    (w-rig-ctr-001)       =    w-rig-rot-key
                     go to stp-cor-doc-cqr-400
           else      go to stp-cor-doc-cqr-600.
       stp-cor-doc-cqr-400.
      *              *-------------------------------------------------*
      *              * Se elemento uguale                              *
      *              *-------------------------------------------------*
      ...
      *                  *---------------------------------------------*
           add       w-rig-dti-qcr
                    (w-rig-ctr-001)       to   w-rig-rot-qta          .
      *                  *---------------------------------------------*
      *                  * Normalizzazione della quantita'             *
      *                  *---------------------------------------------*
           move      zero                 to   w-rig-dti-qcr
                                              (w-rig-ctr-001)          .
      *                  *---------------------------------------------*
      *                  * A riciclo                                   *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-cqr-800.
       stp-cor-doc-cqr-600.
      *              *-------------------------------------------------*
      *              * Se elemento diverso                             *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Se l'indice di puntamento e' a zero : ad    *
      *                  * aggiornamento rottura                       *
      *                  *---------------------------------------------*
           if        w-rig-rot-inx        =    zero
                     go to stp-cor-doc-cqr-680.
      *                  *---------------------------------------------*
      *                  * Aggiornamento elemento di rottura preceden- *
      *                  * te                                          *
      *                  *---------------------------------------------*
           move      w-rig-rot-qta        to   w-rig-dti-qcr
                                              (w-rig-rot-inx)         .
       stp-cor-doc-cqr-680.
      *                  *---------------------------------------------*
      *                  * Aggiornamento rottura                       *
      ...
      *                      *-----------------------------------------*
           move      w-rig-ctr-001        to   w-rig-rot-inx          .
      *                      *-----------------------------------------*
      *                      * Chiave alfanumerica rappresentata dal   *
      *                      * codice alfanumerico del prodotto        *
      *                      *-----------------------------------------*
           move      w-rig-key-alf
                    (w-rig-ctr-001)       to   w-rig-rot-key          .
      *                      *-----------------------------------------*
      *                      * Quantita' per inizio cumulo             *
      *                      *-----------------------------------------*
           move      w-rig-dti-qcr
                    (w-rig-ctr-001)       to   w-rig-rot-qta          .
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       stp-cor-doc-obk-001.
      *              *-------------------------------------------------*
      *              * Eventuale ritaratura del tipo ordinamento       *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * DISATTIVATO (22/02/19 - Guido)              *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-obk-002.
      *                  *---------------------------------------------*
      *                  * Test se cliente Carel                       *
      *                  *---------------------------------------------*
           if        rf-osr-cod-arc       not  = 0002144
                     go to stp-cor-doc-obk-002.
      *                  *---------------------------------------------*
      *                  * Ad ordinamento prodotti per codice prodotto *
      *                  * del cliente                                 *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Bufferizzazione flag di si/no riga pro- *
      *                      * dotto                                   *
      *                      *-----------------------------------------*
           if        rf-osr-alf-pro       =    spaces
                     move  "#"            to   w-rig-key-snp-04
                                              (w-rig-num-ele)
           else      move  spaces         to   w-rig-key-snp-04
                                              (w-rig-num-ele)         .
      *                      *-----------------------------------------*
      *                      * Determinazione descrizione prodotto     *
      *                      *-----------------------------------------*
           move      rf-osr-num-pro       to   w-det-des-dcp-pro      .
           perform   det-des-dcp-000      thru det-des-dcp-999        .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione descrizione prodotto    *
      *                      *-----------------------------------------*
           if        rf-osr-alf-pro       =    spaces
                     move  spaces         to   w-rig-key-der-04
                                              (w-rig-num-ele)
           else      move  w-det-des-dcp-des
                                          to   w-rig-key-der-04
                                              (w-rig-num-ele)         .
      *                      *-----------------------------------------*
      *                      * Bufferizzazione codice magazzino        *
      *                      *-----------------------------------------*
           move      rf-osr-alf-pro       to   w-rig-key-cod-04
                                              (w-rig-num-ele)         .
      *                  *---------------------------------------------*
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
       ...
       stp-cor-doc-obk-300.
      *              *-------------------------------------------------*
      *              * Se ordinamento per codice prodotto del cliente  *
      *              * Carel (Guido 25/01/19)                          *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Preparazione chiave di ordinamento          *
      *                  *---------------------------------------------*
      *                      *-----------------------------------------*
      *                      * Bufferizzazione codice prodotto cliente *
      *                      *-----------------------------------------*
           move      rf-osr-cop-scl       to   w-rig-key-cod-99
                                              (w-rig-num-ele)         .
      *                  *---------------------------------------------*
      *                  * Ad uscita                                   *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Bufferizzazione numero riga                 *
      *                  *---------------------------------------------*
           move      w-rig-num-ele        to   w-rig-dti-nur
                                              (w-rig-num-ele)         .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione progressivo riga            *
      *                  *---------------------------------------------*
           move      rf-osr-num-prg       to   w-rig-dti-prg
                                              (w-rig-num-ele)         .
      *                  *---------------------------------------------*
      *                  * Bufferizzazione quantita' riga              *
      *                  *---------------------------------------------*
           move      rf-osr-qta-ven       to   w-rig-dti-qcr
                                              (w-rig-num-ele)         .
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
      ...
      *    * Stampa corpo documento                                    *
      *    *                                                           *
      *    * Subroutine per la stampa delle righe di commento cumulate *
      *    *-----------------------------------------------------------*
       stp-cor-doc-rcc-000.
      *              *-------------------------------------------------*
      *              * Contatore elementi                              *
      *              *-------------------------------------------------*
           move      zero                 to   w-rcm-ctr-ele          .
      *              *-------------------------------------------------*
      *              * Test su numero elementi letti                   *
      *              *-------------------------------------------------*
           if        w-rcm-num-ele        =    zero
                     go to stp-cor-doc-rcc-900.
       stp-cor-doc-rcc-200.
      *              *-------------------------------------------------*
      *              * Incremento contatore righe                      *
      *              *-------------------------------------------------*
           add       1                    to   w-rcm-ctr-ele          .
      *              *-------------------------------------------------*
      *              * Test su contatore righe                         *
      *              *-------------------------------------------------*
           if        w-rcm-ctr-ele        >    w-rcm-num-ele
                     go to stp-cor-doc-rcc-900.
           if        w-rcm-ctr-ele        >    w-rcm-max-ele
                     go to stp-cor-doc-rcc-900.
           if        w-rcm-dti-prg
                    (w-rcm-ctr-ele)       =    zero
                     go to stp-cor-doc-rcc-620.
       stp-cor-doc-rcc-300.
      *              *-------------------------------------------------*
      *              * Lettura riga in corso di trattamento            *
      *              *-------------------------------------------------*
           ...
           move      w-rcm-dti-prg
                    (w-rcm-ctr-ele)       to   rf-osr-num-prg         .
           move      "pgm/ods/fls/ioc/obj/iofosr"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *                  *---------------------------------------------*
           if        f-sts                not  = e-not-err
                     go to stp-cor-doc-rcc-200.
       stp-cor-doc-rcc-400.
      *              *-------------------------------------------------*
      *              * Descrizione relativa alla riga ordine in esame  *
      *              * in area di comodo                               *
      *              *-------------------------------------------------*
           perform   det-des-rig-000      thru det-des-rig-999        .
       stp-cor-doc-rcc-500.
      ...
      *                  *---------------------------------------------*
       ...
       stp-cor-doc-rcc-780.
      *                  *---------------------------------------------*
      *                  * Riciclo a riga di descrizione               *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-rcc-680.
       stp-cor-doc-rcc-800.
      *                  *---------------------------------------------*
      *                  * Riciclo a riga commento                     *
      *                  *---------------------------------------------*
           go to     stp-cor-doc-rcc-200.
       stp-cor-doc-rcc-900.
      *              *-------------------------------------------------*
      ...
      *    * Stampa corpo documento                                    *
      *    *                                                           *
      *    * Subroutine per la stampa di una eventuale riga aggiuntiva *
      ...
      *    *-----------------------------------------------------------*
       stp-cor-doc-rpo-000.
      *              *-------------------------------------------------*
      *              * Test su codici cliente                          *
      *              *                                                 *
      *              * (Agg. 01/12/17 - Deborah)                       *
      *              *                                                 *
      *              * - 1893 CEBI MICROMOTORS SWITZERLAND S.A.        *
      *              * - 2269 GRAH AUTOMOTIVE D.O.O.                   *
      *              * - 5025 CAREL ELECTRONIC (SUNZHOU)               *
      *              *-------------------------------------------------*
           if        rf-ost-cod-arc       not  = 0001893 and
                     rf-ost-cod-arc       not  = 0002269 and
                     rf-ost-cod-arc       not  = 0005025
                     go to stp-cor-doc-rpo-900.
       stp-cor-doc-rpo-100.
      *              *-------------------------------------------------*
      *              * Determinazione paese di origine                 *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Normalizzazione [aaq]                       *
      *                  *---------------------------------------------*
      ...
      *                  *---------------------------------------------*
           ...
           move      rf-pdt-cod-naz       to   rf-gxn-cod-naz         .
           move      "pgm/geo/fls/ioc/obj/iofgxn"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *
           if        rf-ost-cod-lng       =    "I  "
                     move  "- Origine della merce :"
                                          to   w-all-str-cat (1)
           else      move  "- Origin of goods :"
                                          to   w-all-str-cat (1)      .
      *
           move      rf-gxn-des-naz       to   w-all-str-cat (2)      .
           perform   all-str-csb-000      thru all-str-csb-999        .
       stp-cor-doc-rpo-400.
      *              *-------------------------------------------------*
      *              * Test su linee residue                           *
      ...
      *                      *-----------------------------------------*
           move      004                  to   w-rou-stp-box-lin      .
           move      001                  to   w-rou-stp-box-pin      .
           move      010                  to   w-rou-stp-box-lfi      .
      ...
      *                      *-----------------------------------------*
           move      010                  to   w-rou-stp-box-lin      .
           move      001                  to   w-rou-stp-box-pin      .
           move      014                  to   w-rou-stp-box-lfi      .
           move      042                  to   w-rou-stp-box-pfi      .
           perform   rou-stp-box-000      thru rou-stp-box-999        .
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      018                  to   w-rou-stp-box-lin      .
           move      091                  to   w-rou-stp-box-pin      .
           move      023                  to   w-rou-stp-box-lfi      .
           move      132                  to   w-rou-stp-box-pfi      .
           perform   rou-stp-box-000      thru rou-stp-box-999        .
      *                      *-----------------------------------------*
      *                      * Box per Vettori                         *
      ...
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      001                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      008                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      008                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      049                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           perform   rou-stp-box-000      thru rou-stp-box-999        .
      *                      *-----------------------------------------*
      *                      * Box per Lotto di vendita                *
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      049                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      057                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      057                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      069                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                          *-------------------------------------*
           perform   rou-stp-box-000      thru rou-stp-box-999        .
      *                      *-----------------------------------------*
      *                      * Box per Codice casa produttrice         *
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      069                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      100                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           move      100                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       2                    to   w-rou-stp-box-lfi      .
           move      110                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      *-----------------------------------------*
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lin      .
           add       2                    to   w-rou-stp-box-lin      .
           move      001                  to   w-rou-stp-box-pin      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-box-lfi      .
           add       w-lpc-ult-rig-lin    to   w-rou-stp-box-lfi      .
           move      008                  to   w-rou-stp-box-pfi      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      ...
      *                      *-----------------------------------------*
           ...
           move      05                   to   w-rou-stp-pmt-lin      .
           move      098                  to   w-rou-stp-pmt-pos      .
           perform   rou-stp-pmt-000      thru rou-stp-pmt-999        .
      *                      *-----------------------------------------*
      *                      * Partita Iva                             *
      *                      *-----------------------------------------*
           move      "- PARTITA IVA -"    to   w-rou-stp-pmt-alf      .
           move      15                   to   w-rou-stp-pmt-car      .
           move      08                   to   w-rou-stp-pmt-lin      .
           move      059                  to   w-rou-stp-pmt-pos      .
           perform   rou-stp-pmt-000      thru rou-stp-pmt-999        .
      *                      *-----------------------------------------*
      *                      * Condizioni di fornitura                 *
      *                      *-----------------------------------------*
      ...
      *                      * Mecom                                   *
      *                      *-----------------------------------------*
           move      "Trasporto a cura :" to   w-rou-stp-pmt-alf      .
           move      18                   to   w-rou-stp-pmt-car      .
           move      17                   to   w-rou-stp-pmt-lin      .
           move      002                  to   w-rou-stp-pmt-pos      .
           perform   rou-stp-pmt-000      thru rou-stp-pmt-999        .
      *                      *-----------------------------------------*
      *                      * Condizioni di pagamento                 *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      19                   to   w-rou-stp-pmt-lin      .
           move      096                  to   w-rou-stp-pmt-pos      .
           perform   rou-stp-pmt-000      thru rou-stp-pmt-999        .
      *                      *-----------------------------------------*
      *                      * Vettori                                 *
      *                      *-----------------------------------------*
      ...
      *                      *-----------------------------------------*
           ...
           move      w-lpc-pri-rig-lin    to   w-rou-stp-pmt-lin      .
           add       1                    to   w-rou-stp-pmt-lin      .
           move      002                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      ...
      *                      * Lotto di vendita                        *
      *                      *-----------------------------------------*
           move      "Lotto v"            to   w-rou-stp-pmt-alf      .
           move      07                   to   w-rou-stp-pmt-car      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-pmt-lin      .
           add       1                    to   w-rou-stp-pmt-lin      .
           move      050                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      * Codice casa produttrice                 *
      *                      *-----------------------------------------*
           move      "Codice per il Produttore"
                                          to   w-rou-stp-pmt-alf      .
           move      24                   to   w-rou-stp-pmt-car      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-pmt-lin      .
           add       1                    to   w-rou-stp-pmt-lin      .
           move      073                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      * Quantita' da  spedire                   *
      *                      *-----------------------------------------*
           move      "Quantita'"          to   w-rou-stp-pmt-alf      .
           move      09                   to   w-rou-stp-pmt-car      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-pmt-lin      .
           add       1                    to   w-rou-stp-pmt-lin      .
           move      112                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *                      * Quantita' totale                        *
      *                      *-----------------------------------------*
           move      "Qta Totale"         to   w-rou-stp-pmt-alf      .
           move      10                   to   w-rou-stp-pmt-car      .
           move      w-lpc-pri-rig-lin    to   w-rou-stp-pmt-lin      .
           add       1                    to   w-rou-stp-pmt-lin      .
           move      122                  to   w-rou-stp-pmt-pos      .
      *                          *-------------------------------------*
      *                          * Correttivo per pagine successive    *
      *                          * alla prima                          *
      *                          *-------------------------------------*
      ...
      *              *-------------------------------------------------*
           subtract  20                   from w-rou-stp-box-lin      .
           subtract  20                   from w-rou-stp-pmt-lin      .
       rou-stp-psp-999.
           exit.
      ...
      *                      *-----------------------------------------*
           ...
           move      rf-ost-cod-arc       to   rf-pdx-cod-arc         .
      ...
      *                      *-----------------------------------------*
           if        rf-pdx-tip-rec       not  = 03             or
                     rf-pdx-cod-arc       not  = rf-ost-cod-arc or
                     rf-pdx-cod-lng       not  = spaces         or
                     rf-pdx-cod-num       not  = rf-osr-num-pro or
      ...
      *                      *-----------------------------------------*
           if        rf-pdx-tip-rec       not  = 02             or
                     rf-pdx-cod-arc       not  = zero           or
                     rf-pdx-cod-lng       not  = rf-osr-cod-lng or
                     rf-pdx-cod-num       not  = rf-osr-num-pro or
                     rf-pdx-for-mat       not  = spaces
                     go to det-des-rig-890.
      *                          *-------------------------------------*
      ...
      *    *===========================================================*
      *    * Determinazione descrizione anagrafica del prodotto        *
      *    *-----------------------------------------------------------*
       det-des-dcp-000.
      *              *-------------------------------------------------*
      *              * Normalizzazione valore di uscita                *
      *              *-------------------------------------------------*
           move      spaces               to   w-det-des-dcp-des      .
       det-des-dcp-010.
      *              *-------------------------------------------------*
      *              * Test preliminare                                *
      *              *-------------------------------------------------*
           if        w-det-des-dcp-pro    =    zero
                     go to det-des-dcp-900.
       det-des-dcp-100.
      *              *-------------------------------------------------*
      *              * Normalizzazione                                 *
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "NUMPRO    "         to   f-key                  .
           move      w-det-des-dcp-pro    to   rf-dcp-num-pro         .
           move      "pgm/dcp/fls/ioc/obj/iofdcp"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              * Valore in uscita                                *
      *              *-------------------------------------------------*
           move      rf-dcp-des-pro       to   w-det-des-dcp-des      .
       det-des-dcp-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     det-des-dcp-999.
       det-des-dcp-999.
           exit.
      ...
      *    *===========================================================*
      *    * Routine di compattamento righe annotazioni cliente        *
      *    *-----------------------------------------------------------*
       cmp-nri-goa-000.
      *              *-------------------------------------------------*
      *              * Determinazione numero righe attuale             *
      *              *-------------------------------------------------*
           move      07                   to   w-cmp-nri-goa-nri      .
       cmp-nri-goa-100.
           subtract  1                    from w-cmp-nri-goa-nri      .
           if        w-stp-tes-nri-goa
                    (w-cmp-nri-goa-nri)   =    spaces
                     go to cmp-nri-goa-100.
      *              *-------------------------------------------------*
      *              * Inizializzazione numero righe di annotazione    *
      *              *-------------------------------------------------*
           move      zero                 to   w-cmp-nri-goa-nrd      .
      *              *-------------------------------------------------*
      ...
      *              *-------------------------------------------------*
       ...
       cmp-nri-goa-200.
           add       1                    to   w-cmp-nri-goa-ctr      .
           if        w-cmp-nri-goa-ctr    >    w-cmp-nri-goa-nri
                     go to cmp-nri-goa-300.
      *                  *---------------------------------------------*
      *                  * Se riga di annotazione a spazi : riciclo    *
      *                  *---------------------------------------------*
           if        w-stp-tes-nri-goa
                    (w-cmp-nri-goa-ctr)   =    spaces
                     go to cmp-nri-goa-200.
      *                  *---------------------------------------------*
      *                  * Incremento contatore numero righe           *
      *                  *---------------------------------------------*
           add       1                    to   w-cmp-nri-goa-nrd      .
      *                  *---------------------------------------------*
      *                  * Spostamento riga di annotazione             *
      *                  *---------------------------------------------*
           move      w-stp-tes-nri-goa
                    (w-cmp-nri-goa-ctr)   to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-nrd)     .
      *                  *---------------------------------------------*
      *                  * Riciclo                                     *
      *                  *---------------------------------------------*
      ...
      *              *-------------------------------------------------*
           move      w-cmp-nri-goa-nrd    to   w-cmp-nri-goa-ctr      .
       cmp-nri-goa-320.
           add       1                    to   w-cmp-nri-goa-ctr      .
           if        w-cmp-nri-goa-ctr    >    w-cmp-nri-goa-nri
                     go to cmp-nri-goa-900.
           move      spaces               to   w-stp-tes-nri-goa
                                              (w-cmp-nri-goa-ctr)     .
           go to     cmp-nri-goa-320.
       cmp-nri-goa-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
           go to     cmp-nri-goa-999.
       cmp-nri-goa-999.
           exit.
      ...
      *              *-------------------------------------------------*
      *              * Normalizzazione eventuali messaggi              *
      *              *-------------------------------------------------*
           move      spaces               to   w-ctl-ods-ape-ms1      .
           move      spaces               to   w-ctl-ods-ape-ms2      .
      *              *-------------------------------------------------*
      *              * Start su file [osr]                             *
      *              *-------------------------------------------------*
           move      "SK"                 to   f-ope                  .
      ...
      *                  * same : riciclo                              *
      *                  *---------------------------------------------*
           if        rf-osr-num-prt       =    w-ctl-ods-ape-prt
      ...
      *              *-------------------------------------------------*
           if        w-ctl-ods-ape-ctr    =    zero
                     go to ctl-ods-ape-900.
      *              *-------------------------------------------------*
      *              * Messaggi                                        *
      *              *-------------------------------------------------*
           move      "Per il cliente esistono altri ordini di "
                                          to   w-ctl-ods-ape-ms1      .
           move      "spedizione aperti                       "
                                          to   w-ctl-ods-ape-ms2      .
       ctl-ods-ape-900.
      *              *-------------------------------------------------*
      *              * Uscita                                          *
      *              *-------------------------------------------------*
