      ...
      *                  *---------------------------------------------*
           move      "CV"                 to   p-ope                  .
           move      07                   to   p-car                  .
           move      w-exe-cod-cli        to   p-alf                  .
           call      "swd/mod/prg/obj/mprint"
                                         using p                      .
           move      p-num                to   w-exe-cnv-cli          .
      *                  *---------------------------------------------*
      *                  * Ragione sociale cliente                     *
      *                  *---------------------------------------------*
           inspect   w-exe-rag-soc
                                replacing all  "+"
                                          by   " "                    .
      *                      *-----------------------------------------*
      *                      * Oltre                                   *
      ...
      *              *-------------------------------------------------*
           if        w-all-str-cat (1)    =    "cod_cli"
                     move  w-all-str-cat (2)
                                          to   w-exe-cod-cli
           else if   w-all-str-cat (1)    =    "rag_soc"
                     move  w-all-str-cat (2)
                                          to   w-exe-rag-soc
      ...
      *              *-------------------------------------------------*
       rou-stp-doc-030.
      *              *-------------------------------------------------*
      *              * Test preliminare su codice cliente              *
      *              *-------------------------------------------------*
      *                  *---------------------------------------------*
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        w-exe-cod-cli        not  = spaces
      ...
      *                  * Messaggio                                   *
      *                  *---------------------------------------------*
           move      "INSERIRE UN CODICE CLIENTE"
                                          to   w-exe-err-msg          .
...
______*    perform   stp-tes-doc-000      thru stp-tes-doc-999        .
      ...
      *                  *---------------------------------------------*
           ...
           call      "swd/mod/prg/obj/mhtml0"
                                         using h                      .
      *                  *---------------------------------------------*
      *                  * Literal per numero codice produtore         *
      *                  *---------------------------------------------*
           move      "TD"                 to   h-ope                  .
           move      "th"                 to   h-sub                  .
           move      "PN PRODUTTORE"      to   h-alf                  .
           move      "1"                  to   h-col                  .
           move      "C"                  to   h-all                  .
           move      "S"                  to   h-wdt                  .
           move      "B"                  to   h-stl                  .
      ...
      *              *-------------------------------------------------*
           ...
           move      w-exe-cnv-cli        to   rf-ost-cod-arc         .
           move      1240101              to   rf-ost-dat-doc         .
      ...
      *              *-------------------------------------------------*
           if        rf-ost-cod-dpz       not  = 01
                     go to  stp-cor-doc-inp-900.
           if        rf-ost-tip-arc       not  = "C"
                     go to  stp-cor-doc-inp-900.
           if        rf-ost-cod-arc       not  = w-exe-cnv-cli
                     go to  stp-cor-doc-inp-900.
       stp-cor-doc-inp-400.
      *              *-------------------------------------------------*
      *              * Selezioni su [ost]                              *
      ...
      *              *-------------------------------------------------*
           ...
           move      "NL"                 to   f-cfr                  .
           move      rf-ost-num-prt       to   rf-hbc-num-prt         .
      ...
      *                  * Test su protocollo                          *
      *                  *---------------------------------------------*
           if        rf-hbc-num-prt       not  = rf-ost-num-prt
                     go to  stp-cor-doc-inp-200.
       stp-cor-doc-inp-540.
      *              *-------------------------------------------------*
      *              * Selezioni su [hbc]                              *
      ...
      *                      * Data ordine                             *
      *                      *-----------------------------------------*
           move      rf-ost-dat-doc       to   srt-dat-doc            .
      *                      *-----------------------------------------*
      *                      * Codice numerico prodotto                *
      *                      *-----------------------------------------*
      ...
      *              * Modulo                                          *
      *              *-------------------------------------------------*
           display   "<form name='ods_000' id='ods_000' method='post' ac
      -              "tion=''>"                                       .
       int-pag-sta-300.
      *              *-------------------------------------------------*
      *              * Allineamento                                    *
      ...
      *              * Codice cliente                                  *
      *              *-------------------------------------------------*
           perform   int-pag-sta-cli-000  thru int-pag-sta-cli-999    .
      *              *-------------------------------------------------*
      *              * Ragione sociale cliente                         *
      *              *-------------------------------------------------*
           perform   int-pag-sta-rag-000  thru int-pag-sta-rag-999    .
      *              *-------------------------------------------------*
      *              * Codice prodotto                                 *
      *              *-------------------------------------------------*
...
______*    display   "<hr>"                                           .
      ...
      *    * Intestazione                                              *
      *    *                                                           *
      *    * Subroutine per accettazione Codice cliente                *
      *    *-----------------------------------------------------------*
       int-pag-sta-cli-000.
      *              *-------------------------------------------------*
      *              * Apertura cella                                  *
      *              *-------------------------------------------------*
           display   "<td id='cel_cli' name='cel_cli' align='center'>".
      *              *-------------------------------------------------*
      *              * Prompt                                          *
      *              *-------------------------------------------------*
           display   "<h1> Codice Cliente </h1>"                      .
      *              *-------------------------------------------------*
      *              * Accettazione                                    *
      *              *-------------------------------------------------*
           move      spaces               to   w-exe-str-dsp          .
           string    "<input type='text' name='cod_cli' id='cod_cli'"
                                delimited by   size
                     " value='"
                                delimited by   size
                     w-exe-cod-cli
      ...
      *    * Intestazione                                              *
      *    *                                                           *
      *    * Subroutine per accettazione Ragione sociale cliente       *
      *    *-----------------------------------------------------------*
       int-pag-sta-rag-000.
      *              *-------------------------------------------------*
      *              * Apertura cella                                  *
      *              *-------------------------------------------------*
           display   "<td id='cel_rag' name='cel_rag' align='center'>".
      *              *-------------------------------------------------*
      *              * Prompt                                          *
      *              *-------------------------------------------------*
           display   "<h1> Ragione sociale </h1>"                     .
       int-pag-sta-rag-100.
      *              *-------------------------------------------------*
      *              * Eventuale lettura                               *
      *              *-------------------------------------------------*
      ...
      *                  * Test                                        *
      *                  *---------------------------------------------*
           if        w-exe-rag-soc        not  = spaces
                     go to int-pag-sta-rag-200.
      *                  *---------------------------------------------*
      *                  * Normalizzazione [cli]                       *
      *                  *---------------------------------------------*
           move      "NO"                 to   f-ope                  .
      ...
      *                  *---------------------------------------------*
           move      "RK"                 to   f-ope                  .
           move      "CODCLI"             to   f-key                  .
           move      w-exe-cnv-cli        to   rf-cli-cod-cli         .
           move      "pgm/cge/fls/ioc/obj/iofcli"
                                          to   s-pat                  .
           call      "swd/mod/prg/obj/mfiltp"
                                         using s                      .
      ...
      *              *-------------------------------------------------*
           move      spaces               to   w-exe-str-dsp          .
           string    "<input type='text' name='rag_soc' id='rag_soc'"
                                delimited by   size
                     " value='"
                                delimited by   size
                     w-exe-rag-soc
